...
model ChillerDXHeatingEconomizer
  ...
  parameter Modelica.SIunits.MassFlowRate mAir_flow_nominal
    ...
    annotation(
      Dialog(
        group="Air design"));
  parameter Modelica.SIunits.Power QHea_flow_nominal(
    min=0)
    "Design heating capacity of heating coil"
    annotation(
      Dialog(
        group="Heating design"));
  parameter Real etaHea_nominal(
    min=0,
    max=1,
    unit="1")
    ...
    annotation(
      Dialog(
        group="Heating design"));
  parameter Modelica.SIunits.Power QCoo_flow_nominal(
    max=0)
    "Design heating capacity of cooling coil"
    annotation(
      Dialog(
        group="Cooling design"));
  ...
  Buildings.Fluid.Sources.Boundary_pT bouPreChi(
    ...
    annotation(
      Placement(
        transformation(
          extent={{50,-172}, {70,-152}})));
  Modelica.Blocks.Math.Gain gaiFan(
    k=mAir_flow_nominal)
...
equation
  ...
  connect(fanSup.P, PFan)
    annotation(
      Line(
        points={{-9, 49}, {-6, 49}, {-6, 140}, {210, 140}},
        color={0, 0, 127}));
  connect(eff.y, QHea_flow)
    annotation(
      Line(
        points={{141, 120}, {210, 120}},
        color={0, 0, 127}));
  ...
  connect(heaCoi.Q_flow, eff.u)
    annotation(
      Line(
        points={{73, 46}, {80, 46}, {80, 120}, {118, 120}},
        color={0, 0, 127}));
  ...
  connect(chi.port_b2, pumChiWat.port_a)
    annotation(
      Line(
        points={{110,-162}, {120,-162}, {120,-100}},
        color={0, 0, 255},
        thickness=0.5));
  connect(souChiWat.ports[1], chi.port_a1)
    annotation(
      Line(
        points={{128,-174}, {128,-174}, {110,-174}},
        color={0, 127, 255}));
  connect(chi.port_b1, out.ports[1])
    annotation(
      Line(
        points={{90,-174}, {-116,-174}, {-116, 42.6667}, {-120, 42.6667}},
        color={0, 127, 255}));
  ...
  connect(pumChiWat.P, PPum)
    annotation(
      Line(
        points={{111,-79}, {111,-52}, {180,-52}, {180, 80}, {210, 80}},
        color={0, 0, 127}));
  connect(chi.P, PCoo)
    annotation(
      Line(
        points={{89,-177}, {84,-177}, {84,-128}, {98,-128}, {98,-50}, {178,-50}, {178, 100}, {210, 100}},
        color={0, 0, 127}));
  connect(ideVal.port_2, chi.port_a2)
    annotation(
      Line(
        points={{86, 0.2}, {86,-162}, {90,-162}},
        color={0, 127, 255}));
  connect(cooCoi.port_a1, pumChiWat.port_b)
    annotation(
      Line(
        points={{110, 28}, {120, 28}, {120,-80}},
        color={0, 127, 255}));
  connect(cooCoi.port_a1, ideVal.port_3)
    annotation(
      Line(
        points={{110, 28}, {120, 28}, {120, 10}, {90, 10}},
        color={0, 127, 255}));
  connect(bouPreChi.ports[1], chi.port_a2)
    annotation(
      Line(
        points={{70,-162}, {90,-162}},
        color={0, 127, 255}));
  ...
  connect(senTSup.port_b, supplyAir)
    annotation(
      Line(
        points={{148, 40}, {200, 40}},
        color={0, 127, 255}));
  connect(gaiFan.y, fanSup.m_flow_in)
    annotation(
      Line(
        points={{-59, 140}, {-20, 140}, {-20, 52}},
        color={0, 0, 127}));
protected
  model IdealValve
  ...
  equation
    connect(senMasFlo.m_flow, pro.u2)
      annotation(
        Line(
          points={{-11,-40}, {-40,-40}, {-40, 10}, {-30, 10}},
          color={0, 0, 127}));
    ...
    connect(y, feedback.u2)
      annotation(
        Line(
          points={{-110, 0}, {-50, 0}, {-50, 14}},
          color={0, 0, 127}));
    connect(preMasFlo.port_a, port_3)
      annotation(
        Line(
          points={{60,-1.33227e-15}, {80,-1.33227e-15}, {80, 0}, {100, 0}},
          color={0, 127, 255}));
    ...
    connect(pro.y, preMasFlo.m_flow_in)
      annotation(
        Line(
          points={{-7, 16}, {56, 16}, {56, 8}},
          color={0, 0, 127}));
    connect(port_1, senMasFlo.port_a)
      annotation(
        Line(
          points={{60, 98}, {60, 60}, {4.44089e-16, 60}, {4.44089e-16,-30}},
          color={0, 127, 255}));
    connect(senMasFlo.port_b, port_2)
      annotation(
        Line(
          points={{-4.44089e-16,-50}, {0,-50}, {0,-72}, {60,-72}, {60,-92}, {60,-92}, {60,-98}, {60,-98}},
          color={0, 127, 255}));
    connect(preMasFlo.port_b, senMasFlo.port_a)
      annotation(
        Line(
          points={{40, 1.33227e-15}, {4.44089e-16, 1.33227e-15}, {4.44089e-16,-30}},
          color={0, 127, 255}));
    annotation(
      Icon(
        graphics={Polygon(
...
equation
  ...
  connect(booToInt.u, chiOn)
    annotation(
      Line(
        points={{58,-90}, {40,-90}, {40,-140}, {-220,-140}},
        color={255, 0, 255}));
  connect(chiOn, chi.on)
    annotation(
      Line(
        points={{-220,-140}, {40,-140}, {40,-188}, {120,-188}, {120,-171}, {112,-171}},
        color={255, 0, 255}));
  connect(gaiFan.u, uFan)
    annotation(
      Line(
        points={{-82, 140}, {-220, 140}},
        color={0, 0, 127}));
  connect(heaCoi.u, uHea)
    annotation(
      Line(
        points={{50, 46}, {40, 46}, {40, 80}, {-220, 80}},
        color={0, 0, 127}));
  ...
  connect(chi.TSet, TSetChi)
    annotation(
      Line(
        points={{112,-165}, {124,-165}, {124,-190}, {-220,-190}},
        color={0, 0, 127}));
  connect(senTMixAir.T, TMix)
    annotation(
      Line(
        points={{-50, 51}, {-50, 70}, {188, 70}, {188,-80}, {210,-80}},
        color={0, 0, 127}));
  connect(senTSup.T, TSup)
    annotation(
      Line(
        points={{138, 51}, {138, 64}, {170, 64}, {170,-120}, {210,-120}},
        color={0, 0, 127}));
  connect(out.ports[2], ideEco.port_1)
    annotation(
      Line(
        points={{-120, 40}, {-120, 40}, {-99.8, 40}},
        color={0, 127, 255}));
  connect(ideEco.port_2, senTMixAir.port_a)
    annotation(
      Line(
        points={{-80.2, 40}, {-60, 40}},
        color={0, 127, 255}));
  connect(uEco, ideEco.y)
    annotation(
      Line(
        points={{-220,-60}, {-148,-60}, {-148, 70}, {-90, 70}, {-90, 57}},
        color={0, 0, 127}));
  ...
  connect(ideEco.port_3, senTRetAir.port_b)
    annotation(
      Line(
        points={{-90, 36}, {-90,-40}, {-40,-40}},
        color={0, 127, 255}));
  connect(senTRetAir.port_b, out.ports[3])
    annotation(
      Line(
        points={{-40,-40}, {-112,-40}, {-112, 36}, {-120, 36}, {-120, 37.3333}},
        color={0, 127, 255}));
  connect(TRet, senTRetAir.T)
    annotation(
      Line(
        points={{210,-100}, {174,-100}, {174,-20}, {-30,-20}, {-30,-29}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-200,-240}, {200, 160}}),
      graphics={Rectangle(
        ...
        origin={-100,-37},
        rotation=90), Line(
        points={{200, 100}, {86, 100}, {86, 46}},
        color={0, 0, 127}), Line(
        points={{198, 118}, {48, 118}, {48, 68}},
        color={0, 0, 127}), Line(
        points={{198, 140}, {-30, 140}, {-30, 50}},
        color={0, 0, 127}), Line(
        points={{104, 0}, {104,-66}},
        color={0, 0, 255}), Line(
        points={{114, 0}, {114,-66}},
        color={0, 0, 255}), Line(
        points={{104,-26}, {114,-26}},
        ...
        fillPattern=FillPattern.Solid,
        origin={115,-28}), Line(
        points={{116,-26}, {122,-26}},
        color={0, 0, 0}), Line(
        points={{122,-24}, {122,-30}},
        color={0, 0, 0}), Ellipse(
        ...
        fillColor={0, 0, 0},
        fillPattern=FillPattern.Solid), Line(
        points={{114,-116}, {114,-124}},
        color={0, 0, 255}), Line(
        points={{104,-116}, {104,-124}},
        ...
        fillColor={0, 0, 0},
        fillPattern=FillPattern.Solid), Line(
        points={{200, 80}, {132, 80}, {132, 46}},
        color={0, 0, 127}), Line(
        points={{124,-54}, {132,-54}, {132,-4}},
        color={0, 0, 127}), Line(
        points={{92,-136}, {86,-136}, {86,-4}},
        color={0, 0, 127})}),
    Diagram(
      coordinateSystem(
        preserveAspectRatio=false,
