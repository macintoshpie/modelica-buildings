...
partial model DoorDiscretized
  ...
  Modelica.SIunits.Velocity v[nCom](
    each nominal=0.01)
...
protected
  ...
  parameter Real hAg[nCom](
    each unit="m2/s2")={Modelica.Constants.g_n*(hA-(i-0.5)*dh) for i in 1 : nCom}
    "Product g*h_i for each compartment";
  parameter Real hBg[nCom](
    each unit="m2/s2")={Modelica.Constants.g_n*(hB-(i-0.5)*dh) for i in 1 : nCom}
    "Product g*h_i for each compartment";
  Modelica.SIunits.AbsolutePressure pA[nCom](
    each nominal=101325)
    "Pressure in compartments of room A";
  Modelica.SIunits.AbsolutePressure pB[nCom](
    each nominal=101325)
...
equation
  ...
  for i in 1 : nCom loop
    ...
    gaiFlo[i]=Buildings.Utilities.Math.Functions.smoothHeaviside(
      x=dV_flow[i],
      delta=VZerCom_flow);
