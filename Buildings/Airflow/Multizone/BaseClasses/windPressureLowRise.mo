...
protected
  ...
  Real a180=Modelica.Math.log(1.248-0.703 + 0.131*Modelica.Math.sin(2*Modelica.Constants.pi*G)^3 + 0.071*G^2)
    "Attenuation factor at 180 degree incidence angle";
  Modelica.SIunits.Angle aR
    "alpha, restricted to 0...pi";
  Modelica.SIunits.Angle incAng2
    "0.5*wind incidence angle";
  Real sinA2
    "=sin(alpha/2)";
  Real cosA2
    "=cos(alpha/2)";
...
algorithm
  ...
  if aR < aRDel then
    Cp := Cp0*Buildings.Utilities.Math.Functions.regStep(y1=Modelica.Math.log(1.248-0.703*sinA2-1.175*Modelica.Math.sin(aR)^2 + 0.131*Modelica.Math.sin(2*aR*G)^3 + 0.769*cosA2 + 0.071*G^2*sinA2^2 + 0.717*cosA2^2), y2=1, x=aR-aRDel2, x_small=aRDel2);
  elseif aR > aRMax then
    Cp := Cp0*Buildings.Utilities.Math.Functions.regStep(y1=a180, y2=Modelica.Math.log(1.248-0.703*sinA2-1.175*Modelica.Math.sin(aR)^2 + 0.131*Modelica.Math.sin(2*aR*G)^3 + 0.769*cosA2 + 0.071*G^2*sinA2^2 + 0.717*cosA2^2), x=aR + aRDel2-Modelica.Constants.pi, x_small=aRDel2);
  else
    Cp := Cp0*Modelica.Math.log(1.248-0.703*sinA2-1.175*Modelica.Math.sin(aR)^2 + 0.131*Modelica.Math.sin(2*aR*G)^3 + 0.769*cosA2 + 0.071*G^2*sinA2^2 + 0.717*cosA2^2);
  end if;
