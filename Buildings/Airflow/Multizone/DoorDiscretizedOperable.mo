...
model DoorDiscretizedOperable
  ...
  parameter Modelica.SIunits.PressureDifference dpCloRat(
    min=0,
    displayUnit="Pa")=4
    "Pressure drop at rating condition of closed door"
    annotation(
      Dialog(
        group="Rating conditions"));
  parameter Real CDCloRat(
    min=0,
    max=1)=1
    ...
    annotation(
      Dialog(
        group="Rating conditions"));
  parameter Modelica.SIunits.Area LClo(
    min=0)
...
protected
  parameter Modelica.SIunits.Area AOpe=wOpe*hOpe
    "Open aperture area";
  parameter Modelica.SIunits.Area AClo(
    fixed=false)
