...
protected
  ...
  Medium.ThermodynamicState sta_a1_inflow=Medium1.setState_phX(port_a1.p, port_b1.h_outflow, port_b1.Xi_outflow)
    "Medium properties in port_a1";
  Medium.ThermodynamicState sta_a2_inflow=Medium1.setState_phX(port_a2.p, port_b2.h_outflow, port_b2.Xi_outflow)
    "Medium properties in port_a2";
equation
  when useDefaultProperties and initial() then
    assert(abs(1-rho_default/((Medium.density(sta_a1_inflow) + Medium.density(sta_a2_inflow))/2)) < 0.2, "Wrong density. Densities need to match.");
  ...
  annotation(Icon(graphics={Text(extent={{-92, 108}, {16, 66}}, lineColor={0, 0, 127}, textString="ACS = %ACS")}), defaultComponentName="floExc", Documentation(info="<html>
<p>
