...
model ZonalFlow_m_flow
  "Zonal flow with input air change per second"
  extends Buildings.Airflow.Multizone.BaseClasses.ZonalFlow;
  Modelica.Blocks.Interfaces.RealInput mAB_flow
    "Mass flow rate from A to B, positive if flow from port_a1 to port_b1"
    annotation(Placement(transformation(extent={{-120, 90}, {-100, 110}})));
  Modelica.Blocks.Interfaces.RealInput mBA_flow
    "Mass flow rate from B to A, positive if flow from port_a2 to port_b2"
    annotation(Placement(transformation(extent={{120,-110}, {100,-90}})));
equation
  port_a1.m_flow=mAB_flow;
  port_a2.m_flow=mBA_flow;
  annotation(defaultComponentName="floExc", Documentation(info="<html>
<p>
