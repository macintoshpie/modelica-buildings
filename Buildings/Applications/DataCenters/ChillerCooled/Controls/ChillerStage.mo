...
model ChillerStage
  ...
  Modelica.StateGraph.Transition con2(
    enableTimer=true,
    waitTime=tWai,
    condition=cooMod == Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FullMechanical) and(-QTot >-(criPoiLoa + dQ) or TCHWSup > criPoiTem + dT))
...
equation
  ...
  connect(oneOn.active, booToInt.u)
    annotation(
      Line(
        points={{-39, 10}, {10, 10}, {10,-40}, {18,-40}},
        color={255, 0, 255}));
  connect(booToInt.y, addInt.u1)
    annotation(
      Line(
        points={{41,-40}, {50,-40}, {50,-54}, {58,-54}},
        color={255, 127, 0}));
  connect(booToInt1.y, addInt.u2)
    annotation(
      Line(
        points={{41,-80}, {50,-80}, {50,-66}, {58,-66}},
        color={255, 127, 0}));
