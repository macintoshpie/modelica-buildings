...
model CoolingMode
  ...
  Modelica.Blocks.Interfaces.RealInput TCHWRetWSE(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC")
    "Temperature of entering chilled water that flows to waterside economizer "
    annotation(Placement(transformation(extent={{-140,-100}, {-100,-60}})));
  Modelica.Blocks.Interfaces.RealInput TCHWSupWSE(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC")
    "Temperature of leaving chilled water that flows out from waterside economizer"
    annotation(Placement(transformation(extent={{-140,-60}, {-100,-20}})));
  Modelica.Blocks.Interfaces.RealInput TCHWSupSet(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC")
    "Supply chilled water temperature setpoint "
    annotation(Placement(transformation(extent={{-140, 60}, {-100, 100}})));
  Modelica.Blocks.Interfaces.RealInput TApp(final quantity="TemperatureDifference", final unit="K", displayUnit="degC")
  ...
  Modelica.StateGraph.Transition con1(enableTimer=true, waitTime=tWai, condition=TCHWSupWSE > TCHWSupSet + deaBan1)
  ...
  Modelica.StateGraph.StepWithSignal parMecCoo(nIn=2, nOut=2)
    ...
    annotation(Placement(transformation(extent={{-10, 10}, {10,-10}}, rotation=-90, origin={-10, 8})));
  ...
  Modelica.StateGraph.StepWithSignal fulMecCoo
    ...
    annotation(Placement(transformation(extent={{-10, 10}, {10,-10}}, rotation=-90, origin={-10,-80})));
  Modelica.StateGraph.Transition con2(enableTimer=true, waitTime=tWai, condition=TCHWRetWSE < TCHWSupWSE + deaBan2)
    "Fire condition 2: partially mechanical cooling to fully mechanical cooling"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=-90, origin={-10,-34})));
  Modelica.StateGraph.Transition con3(enableTimer=true, waitTime=tWai, condition=TCHWRetWSE > TWetBul + TApp + deaBan3)
  ...
  Modelica.StateGraph.Transition con4(enableTimer=true, waitTime=tWai, condition=TCHWSupWSE <= TCHWSupSet + deaBan4)
  ...
  Modelica.Blocks.MathInteger.MultiSwitch swi(y_default=0, nu=3, expr={Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FreeCooling), Integer(Buildings.Applications.DataCenters.Types.CoolingModes.PartialMechanical), Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FullMechanical)})
...
equation
  connect(freCoo.outPort[1], con1.inPort)
    annotation(Line(points={{-10, 59.5}, {-10, 44}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con1.outPort, parMecCoo.inPort[1])
    annotation(Line(points={{-10, 38.5}, {-10, 26}, {-10.5, 26}, {-10.5, 19}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con2.inPort, parMecCoo.outPort[1])
    annotation(Line(points={{-10,-30}, {-10,-10}, {-10.25,-10}, {-10.25,-2.5}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con2.outPort, fulMecCoo.inPort[1])
    annotation(Line(points={{-10,-35.5}, {-10,-69}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(fulMecCoo.outPort[1], con3.inPort)
    annotation(Line(points={{-10,-90.5}, {-10,-98}, {24,-98}, {24,-44}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con4.outPort, freCoo.inPort[1])
    annotation(Line(points={{20, 51.5}, {20, 90}, {-10, 90}, {-10, 81}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con3.outPort, parMecCoo.inPort[2])
    annotation(Line(points={{24,-38.5}, {24, 26}, {-9.5, 26}, {-9.5, 19}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con4.inPort, parMecCoo.outPort[2])
    annotation(Line(points={{20, 46}, {20,-10}, {-9.75,-10}, {-9.75,-2.5}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(swi.y, y)
    annotation(Line(points={{88.6, 0}, {110, 0}}, color={255, 127, 0}));
  connect(freCoo.active, swi.u[1])
    annotation(Line(points={{1, 70}, {40, 70}, {40, 1.2}, {64, 1.2}}, color={255, 0, 255}));
  ...
  connect(fulMecCoo.active, swi.u[3])
    annotation(Line(points={{1,-80}, {40,-80}, {40,-1.2}, {64,-1.2}}, color={255, 0, 255}));
