...
model CoolingMode
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=-90,
          origin={-10, 40})));
  Modelica.StateGraph.StepWithSignal parMecCoo(
    nIn=2,
    nOut=2)
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=-90,
          origin={-10, 8})));
  Modelica.StateGraph.InitialStepWithSignal freCoo(
    nIn=1)
    "Free cooling mode"
    annotation(
      Placement(
  ...
  Modelica.Blocks.MathInteger.MultiSwitch swi(
    y_default=0,
    nu=3,
    expr={Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FreeCooling), Integer(Buildings.Applications.DataCenters.Types.CoolingModes.PartialMechanical), Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FullMechanical)})
...
equation
  ...
  connect(freCoo.active, swi.u[1])
    annotation(
      Line(
        points={{1, 70}, {40, 70}, {40, 1.2}, {64, 1.2}},
        color={255, 0, 255}));
  connect(parMecCoo.active, swi.u[2])
    annotation(
      Line(
        points={{1, 8}, {38, 8}, {38, 0}, {64, 0}},
        color={255, 0, 255}));
  connect(fulMecCoo.active, swi.u[3])
    annotation(
      Line(
        points={{1,-80}, {40,-80}, {40,-1.2}, {64,-1.2}},
        color={255, 0, 255}));
  annotation(
    Documentation(
      info="<html>
