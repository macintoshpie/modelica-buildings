...
model CoolingModeNonIntegrated
  ...
  Modelica.Blocks.MathInteger.MultiSwitch swi(
    nu=2,
    y_default=0,
    expr={Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FreeCooling), Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FullMechanical)})
...
equation
  ...
  connect(fulMecCoo.outPort[1], con2.inPort)
    annotation(
      Line(
        points={{-20,-40.5}, {-20,-60}, {30,-60}, {30, 16}},
        color={0, 0, 0}));
  connect(con2.outPort, freCoo.inPort[1])
    annotation(
      Line(
        points={{30, 21.5}, {30, 80}, {-20, 80}, {-20, 69}},
        color={0, 0, 0}));
  connect(freCoo.active, swi.u[1])
    annotation(
      Line(
        points={{-9, 58}, {54, 58}, {54, 0.9}, {64, 0.9}},
        color={255, 0, 255}));
  connect(fulMecCoo.active, swi.u[2])
    annotation(
      Line(
        points={{-9,-30}, {-9,-30}, {54,-30}, {54,-0.9}, {64,-0.9}},
        color={255, 0, 255}));
  connect(swi.y, y)
    annotation(
      Line(
