...
model CoolingModeNonIntegrated
  ...
  Modelica.StateGraph.InitialStepWithSignal freCoo(nIn=1)
    "Free cooling mode"
    annotation(Placement(transformation(extent={{-10, 10}, {10,-10}}, rotation=-90, origin={-20, 58})));
  ...
  Modelica.StateGraph.Transition con1(enableTimer=true, waitTime=tWai, condition=TWetBul > TSwi + deaBan or TCHWSup > TCHWSupSet + deaBan)
  ...
  Modelica.Blocks.MathInteger.MultiSwitch swi(nu=2, y_default=0, expr={Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FreeCooling), Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FullMechanical)})
    "Switch boolean signals to real signal"
    annotation(Placement(transformation(extent={{64,-6}, {88, 6}})));
  Modelica.StateGraph.Transition con2(enableTimer=true, waitTime=tWai, condition=TWetBul <= TSwi-deaBan and numOnChi == 1)
    "Fire condition 2: fully mechanical cooling to free cooling"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, rotation=-90, origin={30, 20})));
  inner Modelica.StateGraph.StateGraphRoot stateGraphRoot
    annotation(Placement(transformation(extent={{-80, 60}, {-60, 80}})));
equation
  connect(freCoo.outPort[1], con1.inPort)
    annotation(Line(points={{-20, 47.5}, {-20, 47.5}, {-20, 38}, {-20, 38}, {-20, 24}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con1.outPort, fulMecCoo.inPort[1])
    annotation(Line(points={{-20, 18.5}, {-20, 18.5}, {-20,-19}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(fulMecCoo.outPort[1], con2.inPort)
    annotation(Line(points={{-20,-40.5}, {-20,-60}, {30,-60}, {30, 16}}, color={0, 0, 0}));
  connect(con2.outPort, freCoo.inPort[1])
    annotation(Line(points={{30, 21.5}, {30, 80}, {-20, 80}, {-20, 69}}, color={0, 0, 0}));
  connect(freCoo.active, swi.u[1])
    annotation(Line(points={{-9, 58}, {54, 58}, {54, 0.9}, {64, 0.9}}, color={255, 0, 255}));
  connect(fulMecCoo.active, swi.u[2])
    annotation(Line(points={{-9,-30}, {-9,-30}, {54,-30}, {54,-0.9}, {64,-0.9}}, color={255, 0, 255}));
  connect(swi.y, y)
    annotation(Line(points={{88.6, 0}, {110, 0}}, color={0, 0, 127}));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false), graphics={Text(extent={{128, 114}, {-128, 166}}, lineColor={0, 0, 255}, textString="%name")}), Documentation(info="<html>
