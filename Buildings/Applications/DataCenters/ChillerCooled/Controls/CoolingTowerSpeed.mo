...
model CoolingTowerSpeed
  ...
  parameter Modelica.SIunits.Time Ti(min=Modelica.Constants.small)=0.5
    ...
    annotation(Dialog(enable=(controllerType == Modelica.Blocks.Types.SimpleController.PI or controllerType == Modelica.Blocks.Types.SimpleController.PID), tab="Controller"));
  parameter Modelica.SIunits.Time Td(min=0)=0.1
    "Time constant of derivative block"
    annotation(Dialog(enable=(controllerType == Modelica.Blocks.Types.SimpleController.PD or controllerType == Modelica.Blocks.Types.SimpleController.PID), tab="Controller"));
  ...
  Modelica.Blocks.Interfaces.RealInput TCWSupSet(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC")
  ...
  Modelica.Blocks.Interfaces.RealInput TCHWSup(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC")
    ...
    annotation(Placement(transformation(extent={{-20,-20}, {20, 20}}, origin={-120,-74}), iconTransformation(extent={{-140,-100}, {-100,-60}})));
  Modelica.Blocks.Interfaces.RealInput TCWSup(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC")
    "Condenser water supply temperature "
    annotation(Placement(transformation(extent={{20, 20}, {-20,-20}}, rotation=180, origin={-120,-40})));
  ...
  Modelica.Blocks.Sources.BooleanExpression pmcMod(y=cooMod == Integer(Buildings.Applications.DataCenters.Types.CoolingModes.PartialMechanical))
  ...
  Buildings.Controls.Continuous.LimPID conPID(controllerType=controllerType, k=k, Ti=Ti, Td=Td, yMax=yMax, yMin=yMin, reverseAction=reverseAction)
    "PID controller"
...
protected
  ...
  Modelica.Blocks.Logical.Switch swi2
    "Switch 2"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, origin={-30,-66})));
  Modelica.Blocks.Logical.Switch swi3
    "Switch 3"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, origin={74, 0})));
equation
  connect(TCWSupSet, swi1.u1)
    annotation(Line(points={{-120, 80}, {-48, 80}, {-48, 48}, {-32, 48}}, color={0, 0, 127}));
  connect(TCHWSupSet, swi1.u3)
    annotation(Line(points={{-120, 0}, {-48, 0}, {-48, 32}, {-32, 32}}, color={0, 0, 127}));
  connect(swi1.y, conPID.u_s)
    annotation(Line(points={{-9, 40}, {0, 40}, {0,-40}, {18,-40}}, color={0, 0, 127}));
  connect(fmcMod.y, swi2.u2)
    annotation(Line(points={{-59, 40}, {-54, 40}, {-54,-66}, {-42,-66}}, color={255, 0, 255}));
  connect(TCWSup, swi2.u1)
    annotation(Line(points={{-120,-40}, {-50,-40}, {-50,-58}, {-42,-58}}, color={0, 0, 127}));
  connect(TCHWSup, swi2.u3)
    annotation(Line(points={{-120,-74}, {-42,-74}}, color={0, 0, 127}));
  ...
  connect(fmcMod.y, swi1.u2)
    annotation(Line(points={{-59, 40}, {-32, 40}}, color={255, 0, 255}));
  connect(cooMod, fmcMod.u)
    annotation(Line(points={{-120, 40}, {-102, 40}, {-82, 40}}, color={255, 127, 0}));
  annotation(Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 80}})), Documentation(info="<html>
