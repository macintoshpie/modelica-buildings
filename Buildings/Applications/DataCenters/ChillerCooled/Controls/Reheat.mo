...
model Reheat
  ...
  parameter Modelica.SIunits.Time tWai
    "Waiting time";
  Modelica.Blocks.Interfaces.RealInput yVal(min=0, max=1, unit="1")
    "Valve position"
    annotation(Placement(transformation(extent={{-140, 30}, {-100, 70}}), iconTransformation(extent={{-140, 30}, {-100, 70}})));
  Modelica.Blocks.Interfaces.RealInput dT(final quantity="ThermodynamicTemperature", final unit="K")
  ...
  Modelica.StateGraph.StepWithSignal on
    "On"
    annotation(Placement(transformation(extent={{10, 10}, {-10,-10}}, rotation=180, origin={50, 30})));
  Modelica.StateGraph.Transition offToOn(enableTimer=true, waitTime=tWai, condition=yVal <= yValSwi-yValDeaBan and dT < dTSwi-dTDeaBan)
    "Conditions that switch off to on"
    annotation(Placement(transformation(extent={{-10, 20}, {10, 40}})));
  Modelica.StateGraph.Transition onToOff(condition=yVal > yValSwi + yValDeaBan or dT > dTSwi + dTDeaBan, enableTimer=true, waitTime=tWai)
...
equation
  connect(off.outPort[1], offToOn.inPort)
    annotation(Line(points={{-39.5, 30}, {-4, 30}}, color={0, 0, 0}));
  connect(offToOn.outPort, on.inPort[1])
    annotation(Line(points={{1.5, 30}, {39, 30}}, color={0, 0, 0}));
  connect(on.outPort[1], onToOff.inPort)
    annotation(Line(points={{60.5, 30}, {80, 30}, {80,-20}, {4,-20}}, color={0, 0, 0}));
  connect(onToOff.outPort, off.inPort[1])
    annotation(Line(points={{-1.5,-20}, {-80,-20}, {-80, 30}, {-61, 30}}, color={0, 0, 0}));
  connect(on.active, y)
    annotation(Line(points={{50, 19}, {50, 19}, {50, 0}, {110, 0}}, color={255, 0, 255}));
  annotation(Icon(coordinateSystem(preserveAspectRatio=false), graphics={Rectangle(extent={{-100, 100}, {100,-100}}, fillColor={210, 210, 210}, lineThickness=5.0, fillPattern=FillPattern.Solid, borderPattern=BorderPattern.Raised), Text(extent={{-150, 150}, {150, 110}}, textString="%name", lineColor={0, 0, 255}), Line(points={{-12,-64}, {-12, 60}}, color={0, 0, 127}, thickness=0.5), Line(points={{-12, 60}, {68, 60}}, color={0, 0, 127}, thickness=0.5), Line(points={{-74,-64}, {14,-64}, {14, 60}}, color={0, 0, 127}, thickness=0.5)}), Documentation(info="<html>
