...
model ChillerStage
  ...
  Buildings.Applications.DataCenters.ChillerCooled.Controls.ChillerStage chiStaCon(tWai=30, QEva_nominal=-100*3.517*1000, dT=0.5)
    "Staging controller for chillers"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}})));
  Modelica.Blocks.Sources.Constant QTot(k=0.55*chiStaCon.QEva_nominal)
  ...
  Modelica.Blocks.Sources.Pulse TCHWSup(amplitude=2, period=360, offset=273.15 + 5)
    "WSE chilled water supply temperature"
    annotation(Placement(transformation(extent={{-60,-40}, {-40,-20}})));
equation
  connect(QTot.y, chiStaCon.QTot)
    annotation(Line(points={{-39, 0}, {-39, 0}, {-12, 0}}, color={0, 0, 127}));
  connect(cooMod.y, chiStaCon.cooMod)
    annotation(Line(points={{-39, 50}, {-26, 50}, {-26, 6}, {-12, 6}}, color={255, 127, 0}));
  connect(TCHWSup.y, chiStaCon.TCHWSup)
    annotation(Line(points={{-39,-30}, {-26,-30}, {-26,-6}, {-12,-6}}, color={0, 0, 127}));
  annotation(__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/ChillerCooled/Controls/Validation/ChillerStage.mos"
...
</html>"), experiment(StartTime=0, StopTime=1440, Tolerance=1e-06));
end ChillerStage;
