  ...
  Modelica.Blocks.Sources.Sine sig2(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80,-38}, {-60,-18}})));
  Modelica.Blocks.Sources.Constant set1(
    k=0.2)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 0}, {-60, 20}})));
  Modelica.Blocks.Sources.Constant set2(
    k=273.15 + 16)
...
equation
  connect(sig1.y, add1.u1)
    annotation(
      Line(
        points={{-59, 50}, {-52, 50}, {-52, 36}, {-22, 36}},
        color={0, 0, 127}));
  connect(set1.y, add1.u2)
    annotation(
      Line(
        points={{-59, 10}, {-52, 10}, {-52, 24}, {-22, 24}},
        color={0, 0, 127}));
  connect(add1.y, heaCon.yVal)
    annotation(
      Line(
        points={{1, 30}, {20, 30}, {20, 5}, {38, 5}},
        color={0, 0, 127}));
  connect(sig2.y, add2.u1)
    annotation(
      Line(
        points={{-59,-28}, {-52,-28}, {-52,-44}, {-22,-44}},
        color={0, 0, 127}));
  connect(set2.y, add2.u2)
    annotation(
      Line(
        points={{-59,-70}, {-52,-70}, {-52,-56}, {-22,-56}},
        color={0, 0, 127}));
  connect(add2.y, heaCon.dT)
    annotation(
      Line(
        points={{1,-50}, {20,-50}, {20,-5}, {38,-5}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=1200),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/ChillerCooled/Controls/Validation/Reheat.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
