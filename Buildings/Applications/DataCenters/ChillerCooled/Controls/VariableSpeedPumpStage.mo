...
model VariableSpeedPumpStage
  ...
  Modelica.StateGraph.Transition con1(enableTimer=true, waitTime=tWai, condition=speSig > minSpe)
  ...
  Modelica.StateGraph.StepWithSignal twoOn
    ...
    annotation(Placement(transformation(extent={{-10, 10}, {10,-10}}, rotation=-90, origin={-50,-70})));
  Modelica.StateGraph.Transition con2(enableTimer=true, waitTime=tWai, condition=speSig > criPoiSpe + deaBanSpe and masFloPum > criPoiFlo + deaBanFlo)
  ...
  Modelica.StateGraph.Transition con3(enableTimer=true, waitTime=tWai, condition=speSig < criPoiSpe-deaBanSpe or masFloPum < criPoiFlo-deaBanFlo)
  ...
  Modelica.StateGraph.Transition con4(enableTimer=true, waitTime=tWai, condition=speSig <= minSpe)
    "Fire condition 4: partially mechanical cooling to free cooling"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, rotation=-90, origin={-22, 52})));
...
equation
  connect(off.outPort[1], con1.inPort)
    annotation(Line(points={{-50, 59.5}, {-50, 44}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con1.outPort, oneOn.inPort[1])
    annotation(Line(points={{-50, 38.5}, {-50, 26}, {-50.5, 26}, {-50.5, 21}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con2.inPort, oneOn.outPort[1])
    annotation(Line(points={{-50,-26}, {-50,-10}, {-50.25,-10}, {-50.25,-0.5}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con2.outPort, twoOn.inPort[1])
    annotation(Line(points={{-50,-31.5}, {-50,-59}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(twoOn.outPort[1], con3.inPort)
    annotation(Line(points={{-50,-80.5}, {-50,-92}, {-10,-92}, {-10,-44}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(con4.outPort, off.inPort[1])
    annotation(Line(points={{-22, 53.5}, {-22, 88}, {-50, 88}, {-50, 81}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  ...
  connect(con4.inPort, oneOn.outPort[2])
    annotation(Line(points={{-22, 48}, {-22,-10}, {-49.75,-10}, {-49.75,-0.5}}, color={0, 0, 0}, pattern=LinePattern.Dash));
  connect(combiTable1Ds.y, y)
    annotation(Line(points={{91, 0}, {110, 0}}, color={0, 0, 127}));
  connect(booToInt.u, oneOn.active)
    annotation(Line(points={{18,-40}, {10,-40}, {10, 10}, {-39, 10}}, color={255, 0, 255}));
  connect(twoOn.active, booToInt1.u)
    annotation(Line(points={{-39,-70}, {18,-70}}, color={255, 0, 255}));
  connect(booToInt.y, addInt.u1)
    annotation(Line(points={{41,-40}, {58,-40}}, color={255, 127, 0}));
  connect(booToInt1.y, addInt.u2)
    annotation(Line(points={{41,-70}, {48,-70}, {48,-52}, {58,-52}}, color={255, 127, 0}));
  connect(addInt.y, intToRea.u)
    annotation(Line(points={{81,-46}, {90,-46}, {90,-20}, {30,-20}, {30, 0}, {38, 0}}, color={255, 127, 0}));
