...
partial model PartialChillerWSE
  ...
  parameter Real[2] lValChi(
    ...
    annotation(
      Dialog(
        group="Two-way valve"));
  parameter Real[2] kFixedValChi(
    each unit="",
    each min=0)={m1_flow_chi_nominal, m2_flow_chi_nominal} ./ sqrt({dp1_chi_nominal, dp2_chi_nominal})
  ...
  parameter Real[2] lValWSE(
    ...
    annotation(
      Dialog(
        group="Two-way valve"));
  parameter Real[2] kFixedValWSE(
    each unit="",
    each min=0)={m1_flow_wse_nominal/sqrt(dp1_wse_nominal), 0}
    "Flow coefficient of fixed resistance that may be in series with valves
...
equation
  for i in 1 : numChi loop
    connect(chiPar.on[i], on[i])
      annotation(
        Line(
          points={{-62, 34}, {-92, 34}, {-92, 72}, {-120, 72}},
          color={255, 0, 255}));
  end for;
  connect(on[numChi + 1], wse.on[1])
    annotation(
      Line(
        points={{-120, 72}, {-120, 72}, {30, 72}, {30, 34}, {38, 34}},
        color={255, 0, 255}));
  connect(chiPar.TSet, TSet)
    annotation(
      Line(
        points={{-62, 30}, {-84, 30}, {-84, 104}, {-120, 104}},
        color={0, 0, 127}));
  connect(port_a1, chiPar.port_a1)
    annotation(
      Line(
        points={{-100, 60}, {-80, 60}, {-72, 60}, {-72, 36}, {-60, 36}},
        color={0, 127, 255}));
  connect(chiPar.port_b1, port_b1)
    annotation(
      Line(
        points={{-40, 36}, {-20, 36}, {-20, 60}, {100, 60}},
        color={0, 127, 255}));
  connect(wse.port_b1, port_b1)
    annotation(
      Line(
        points={{60, 36}, {80, 36}, {80, 60}, {100, 60}},
        color={0, 127, 255}));
  connect(port_a1, wse.port_a1)
    annotation(
      Line(
        points={{-100, 60}, {-100, 60}, {-72, 60}, {-72, 56}, {34, 56}, {34, 36}, {40, 36}},
        color={0, 127, 255}));
  connect(TSet, wse.TSet)
    annotation(
      Line(
        points={{-120, 104}, {-84, 104}, {-84, 80}, {26, 80}, {26, 30}, {38, 30}},
        color={0, 0, 127}));
  connect(y_reset_in, wse.y_reset_in)
    annotation(
      Line(
        points={{-90,-100}, {-90,-100}, {-90, 10}, {40, 10}, {40, 20}},
        color={0, 0, 127}));
  connect(trigger, wse.trigger)
    annotation(
      Line(
        points={{-60,-100}, {-60,-100}, {-60,-80}, {-88,-80}, {-88, 8}, {44, 8}, {44, 20}},
        color={255, 0, 255}));
  connect(senTem.T, TCHWSupWSE)
    annotation(
      Line(
        points={{18, 35}, {18, 35}, {18, 48}, {86, 48}, {86, 40}, {110, 40}},
        color={0, 0, 127}));
  connect(wse.port_b2, senTem.port_a)
    annotation(
      Line(
        points={{40, 24}, {34, 24}, {28, 24}},
        color={0, 127, 255}));
  connect(chiPar.P, powChi)
    annotation(
      Line(
        points={{-39, 32}, {-6, 32}, {-6, 52}, {90, 52}, {90, 0}, {110, 0}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
      ...
      graphics={Rectangle(
        ...
        textString="T"), Line(
        points={{-90, 60}, {-76, 60}, {-76, 40}, {-72, 40}},
        color={28, 108, 200}), Line(
        points={{-76, 60}, {20, 60}, {20, 40}},
        color={28, 108, 200}), Line(
        points={{20, 40}, {24, 40}},
        color={28, 108, 200}), Line(
        points={{64, 40}, {76, 40}, {76, 60}, {90, 60}},
        color={28, 108, 200}), Line(
        points={{-24, 40}, {6, 40}, {6, 54}, {76, 54}},
        color={28, 108, 200}), Line(
        points={{24, 0}, {12, 0}, {12, 6}},
        color={28, 108, 200}), Line(
        points={{12, 20}},
        color={28, 108, 200}), Line(
        points={{12, 20}, {12, 20}, {12, 48}, {86, 48}, {86, 40}, {102, 40}},
        color={0, 0, 127})}),
    Documentation(
      info="<html>
<p>
