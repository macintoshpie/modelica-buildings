...
partial model PartialChillerWSEInterface
  ...
  // Nominal conditions
  parameter Modelica.SIunits.MassFlowRate m1_flow_chi_nominal(
    min=0)
    "Nominal mass flow rate on the medium 1 side in the chiller"
    annotation(
      Dialog(
        group="Chiller"));
  parameter Modelica.SIunits.MassFlowRate m2_flow_chi_nominal(
    min=0)
    "Nominal mass flow rate on the medium 2 side in the chiller"
    annotation(
      Dialog(
        group="Chiller"));
  parameter Modelica.SIunits.MassFlowRate m1_flow_wse_nominal(
    min=0)
    "Nominal mass flow rate on the medium 1 side in the waterside economizer"
    annotation(
      Dialog(
        group="Waterside economizer"));
  parameter Modelica.SIunits.MassFlowRate m2_flow_wse_nominal(
    min=0)
  ...
  parameter Medium1.MassFlowRate m1_flow_small(
    min=0)=1E-4*abs(m1_flow_chi_nominal)
    "Small mass flow rate for regularization of zero flow"
    annotation(
      Dialog(
        tab="Advanced"));
  parameter Medium2.MassFlowRate m2_flow_small(
    min=0)=1E-4*abs(m2_flow_chi_nominal)
    "Small mass flow rate for regularization of zero flow"
    annotation(
      Dialog(
        tab="Advanced"));
