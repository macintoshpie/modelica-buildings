...
partial model PartialPlantParallel
  ...
  extends Buildings.Applications.DataCenters.ChillerCooled.Equipment.BaseClasses.ValvesParameters(final numVal=2, final m_flow_nominal={m1_flow_nominal, m2_flow_nominal}, rhoStd={Medium1.density_pTX(101325, 273.15 + 4, Medium1.X_default), Medium2.density_pTX(101325, 273.15 + 4, Medium2.X_default)}, final deltaM=deltaM1);
  extends Buildings.Applications.DataCenters.ChillerCooled.Equipment.BaseClasses.SignalFilter(final numFil=num);
  ...
  parameter Real kFixed[2](each unit="", each min=0)={m1_flow_nominal, m2_flow_nominal} ./ sqrt({dp1_nominal, dp2_nominal})
    "Flow coefficient of fixed resistance that may be in series with valve 1, k=m_flow/sqrt(dp), with unit=(kg.m)^(1/2)."
    annotation(Dialog(group="Two-way valve"));
  Buildings.Fluid.Actuators.Valves.TwoWayLinear val2[num](redeclare each replaceable package Medium=Medium2, each final allowFlowReversal=allowFlowReversal2, each final m_flow_nominal=m2_flow_nominal, each dpFixed_nominal=dp2_nominal, each final show_T=show_T, each final homotopyInitialization=homotopyInitialization, each final riseTime=riseTimeValve, each final init=initValve, each final use_inputFilter=false, each final deltaM=deltaM2, each final l=l[2], each final kFixed=kFixed[2], final y_start=yValve_start, each final CvData=Buildings.Fluid.Types.CvTypes.OpPoint, each final from_dp=from_dp2, each final linearized=linearizeFlowResistance2, each final rhoStd=rhoStd[2], each final dpValve_nominal=dpValve_nominal[2])
    "Isolation valves on medium 2 side for on/off use"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=270, origin={-40,-32})));
  Buildings.Fluid.Actuators.Valves.TwoWayLinear val1[num](redeclare each replaceable package Medium=Medium1, each final allowFlowReversal=allowFlowReversal1, each final m_flow_nominal=m1_flow_nominal, each dpFixed_nominal=dp1_nominal, each final show_T=show_T, each final homotopyInitialization=homotopyInitialization, each final use_inputFilter=false, each final riseTime=riseTimeValve, each final init=initValve, final y_start=yValve_start, each final deltaM=deltaM1, each final l=l[1], each final kFixed=kFixed[1], each final dpValve_nominal=dpValve_nominal[1], each final CvData=Buildings.Fluid.Types.CvTypes.OpPoint, each final from_dp=from_dp1, each final linearized=linearizeFlowResistance1, each final rhoStd=rhoStd[1])
...
equation
  ...
  if use_inputFilter then
    connect(booToRea.y, filter.u)
      annotation(Line(points={{-67.4, 40}, {-60, 40}, {-60, 84}, {-55.2, 84}}, color={0, 0, 127}));
  else
    connect(booToRea.y, y_actual)
      annotation(Line(points={{-67.4, 40}, {-60, 40}, {-60, 74}, {-20, 74}}, color={0, 0, 127}));
  end if;
  connect(on, booToRea.u)
    annotation(Line(points={{-120, 40}, {-81.2, 40}, {-81.2, 40}}, color={255, 0, 255}));
  connect(y_actual, val1.y)
    annotation(Line(points={{-20, 74}, {-20, 66}, {20, 66}, {20, 32}, {28, 32}}, color={0, 0, 127}));
