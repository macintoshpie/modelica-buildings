...
protected
  Modelica.Blocks.Interfaces.RealOutput[numFil] y_actual
  ...
  Modelica.Blocks.Interfaces.RealOutput[numFil] y_filtered if use_inputFilter
    ...
    annotation(Placement(transformation(extent={{-28, 76}, {-12, 92}}), iconTransformation(extent={{60, 50}, {80, 70}})));
equation
  connect(filter.y, y_filtered)
    annotation(Line(points={{-41.4, 84}, {-32, 84}, {-20, 84}}, color={0, 0, 127}));
  if use_inputFilter then
    connect(filter.y, y_actual)
      annotation(Line(points={{-41.4, 84}, {-32, 84}, {-32, 74}, {-20, 74}}, color={0, 0, 127}));
  end if;
