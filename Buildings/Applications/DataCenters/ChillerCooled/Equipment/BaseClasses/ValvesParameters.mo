...
partial model ValvesParameters
  "Model with parameters for multiple valves"
  parameter Integer numVal
    "Number of valves";
  ...
  parameter Real[numVal] Kv(each fixed=if CvData == Buildings.Fluid.Types.CvTypes.Kv then true else false)
    "Kv (metric) flow coefficient [m3/h/(bar)^(1/2)]"
    annotation(Dialog(group="Two-way valve", enable=(CvData == Buildings.Fluid.Types.CvTypes.Kv)));
  parameter Real[numVal] Cv(each fixed=if CvData == Buildings.Fluid.Types.CvTypes.Cv then true else false)
    "Cv (US) flow coefficient [USG/min/(psi)^(1/2)]"
    annotation(Dialog(group="Two-way valve", enable=(CvData == Buildings.Fluid.Types.CvTypes.Cv)));
  parameter Modelica.SIunits.Area[numVal] Av(each fixed=if CvData == Buildings.Fluid.Types.CvTypes.Av then true else false)
  ...
  parameter Modelica.SIunits.MassFlowRate[numVal] m_flow_nominal
    ...
    annotation(Dialog(group="Two-way valve"));
  parameter Modelica.SIunits.PressureDifference[numVal] dpValve_nominal(each displayUnit="Pa", each min=0, each fixed=if CvData == Buildings.Fluid.Types.CvTypes.OpPoint then true else false)=fill(6000, numVal)
...
protected
  parameter Real[numVal] Kv_SI(each min=0, each fixed=false)
