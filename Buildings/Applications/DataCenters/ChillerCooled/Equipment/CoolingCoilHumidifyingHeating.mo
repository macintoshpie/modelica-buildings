...
model CoolingCoilHumidifyingHeating
  ...
  parameter Modelica.SIunits.Efficiency etaHea=1.0
    ...
    annotation(
      Dialog(
        group="Electric heater"));
  parameter Modelica.SIunits.HeatFlowRate QHeaMax_flow(
    min=0)
  ...
  parameter Real yValSwi(
    min=0,
    max=1,
    unit="1")
    "Switch point for valve signal";
  parameter Real yValDeaBan(
    min=0,
    max=1,
    unit="1")=0.1
  ...
  Modelica.Blocks.Interfaces.RealInput XSet_w(
    ...
    annotation(
      Placement(
        ...
        iconTransformation(
          extent={{-120, 0}, {-100, 20}})));
  Modelica.Blocks.Sources.RealExpression dT(
    y(
      final unit="K")=T_inflow_hea-TSet)
...
protected
  Medium2.Temperature T_inflow_hea=Medium2.temperature(
    state=Medium2.setState_phX(port_b2.p, noEvent(actualStream(port_b2.h_outflow)), noEvent(actualStream(port_b2.Xi_outflow))))
    "Temperature of inflowing fluid at port_a of reheater";
equation
  connect(TSet, eleHea.TSet)
    annotation(
      Line(
        points={{-120,-20}, {-88,-20}, {-88,-28}, {-4,-28}, {-4,-52}, {-10,-52}},
        color={0, 0, 127}));
  connect(XSet_w, hum.X_w)
    annotation(
      Line(
        points={{-120, 0}, {-80, 0}, {-80,-20}, {36,-20}, {36,-54}, {32,-54}},
        color={0, 0, 127}));
  connect(fan.port_a, eleHea.port_b)
    annotation(
      Line(
        points={{-50,-60}, {-41,-60}, {-32,-60}},
        color={0, 127, 255}));
  ...
  connect(hum.port_a, cooCoi.port_b2)
    annotation(
      Line(
        points={{30,-60}, {30,-60}, {48,-60}, {42,-60}, {60,-60}},
        color={0, 127, 255}));
  connect(eleHea.P, PHea)
    annotation(
      Line(
        points={{-33,-66}, {-40,-66}, {-40,-76}, {18,-76}, {18,-110}},
        color={0, 0, 127}));
  connect(uFan, fan.y)
    annotation(
      Line(
        points={{-120,-50}, {-120,-50}, {-90,-50}, {-90,-50}, {-90,-50}, {-90,-40}, {-60,-40}, {-60,-48}, {-60,-48}},
        color={0, 0, 127}));
  connect(heaCon.y, eleHea.on)
    annotation(
      Line(
        points={{1, 10}, {4, 10}, {4,-57}, {-10,-57}},
        color={255, 0, 255}));
  connect(dT.y, heaCon.dT)
    annotation(
      Line(
        points={{-39, 6}, {-22, 6}, {-22, 5}},
        color={0, 0, 127}));
  connect(heaCon.yVal, watVal.y_actual)
    annotation(
      Line(
        points={{-22, 15}, {-32, 15}, {-32, 16}, {-32, 40}, {73, 40}, {73,-5}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
