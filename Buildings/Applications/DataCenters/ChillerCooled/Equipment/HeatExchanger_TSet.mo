...
model HeatExchanger_TSet
  "Heat exchanger"
  extends Buildings.Applications.DataCenters.ChillerCooled.Equipment.BaseClasses.PartialHeatExchanger(final activate_ThrWayVal=use_controller);
  extends Buildings.Applications.DataCenters.ChillerCooled.Equipment.BaseClasses.PartialControllerInterface;
  Modelica.Blocks.Interfaces.RealInput TSet(final unit="K", final quantity="ThermodynamicTemperature", displayUnit="degC") if use_controller
...
protected
  Medium2.Temperature T_outflow=Medium2.temperature(state=Medium2.setState_phX(p=port_b2.p, h=actualStream(port_b2.h_outflow), X=actualStream(port_b2.Xi_outflow)))
    "Temperature of outflowing fluid at port_b on medium 2 side";
equation
  if use_controller then
    connect(T_b2.y, con.u_m)
      annotation(Line(points={{-59, 70}, {-44, 70}, {-44, 20}, {-70, 20}, {-70, 28}}, color={0, 0, 127}));
  ...
  connect(y_reset_in, con.y_reset_in)
    annotation(Line(points={{-90,-100}, {-90,-100}, {-90,-68}, {-78,-68}, {-78, 20}, {-88, 20}, {-88, 32}, {-82, 32}}, color={0, 0, 127}));
  connect(trigger, con.trigger)
    annotation(Line(points={{-60,-100}, {-60,-100}, {-60,-40}, {-78,-40}, {-78, 28}}, color={255, 0, 255}));
