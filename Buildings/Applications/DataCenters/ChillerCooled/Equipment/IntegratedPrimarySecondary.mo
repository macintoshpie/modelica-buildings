...
model IntegratedPrimarySecondary
  ...
  extends Buildings.Applications.DataCenters.ChillerCooled.Equipment.BaseClasses.PartialChillerWSE(
    final numVal=5,
    final m_flow_nominal={m1_flow_chi_nominal, m2_flow_chi_nominal, m1_flow_chi_nominal, m2_flow_wse_nominal, numChi*m2_flow_chi_nominal},
    rhoStd={Medium1.density_pTX(101325, 273.15 + 4, Medium1.X_default), Medium2.density_pTX(101325, 273.15 + 4, Medium2.X_default), Medium1.density_pTX(101325, 273.15 + 4, Medium1.X_default), Medium2.density_pTX(101325, 273.15 + 4, Medium2.X_default), Medium2.density_pTX(101325, 273.15 + 4, Medium2.X_default)});
  // Dynamics
  ...
  parameter Integer numPum=numChi
    ...
    annotation(
      Dialog(
        group="Pump"));
  parameter Modelica.SIunits.MassFlowRate m_flow_pum_nominal(
    min=0)=m2_flow_chi_nominal
  ...
  parameter Modelica.Blocks.Types.Init initPum=initValve
    ...
    annotation(
      Dialog(
        ...
        group="Filtered flowrate",
        enable=use_inputFilter));
  parameter Real[numPum] yPum_start(
    each min=0)=fill(0, numPum)
    ...
    annotation(
      Dialog(
        ...
        group="Filtered flowrate",
        enable=use_inputFilter));
  parameter Real[numPum] m_flow_start(
    each min=0)=fill(0, numPum)
...
equation
  connect(wse.port_a2, port_a2)
    annotation(
      Line(
        points={{60, 24}, {80, 24}, {80,-60}, {100,-60}},
        color={0, 127, 255}));
  connect(port_a2, val5.port_a)
    annotation(
      Line(
        points={{100,-60}, {100,-60}, {80,-60}, {80,-20}, {60,-20}},
        color={0, 127, 255}));
  connect(pum.port_b, chiPar.port_a2)
    annotation(
      Line(
        points={{-10,-20}, {-20,-20}, {-20, 24}, {-40, 24}},
        color={0, 127, 255}));
  connect(val5.y, yVal5)
    annotation(
      Line(
        points={{50,-8}, {50, 6}, {-94, 6}, {-94, 26}, {-120, 26}},
        color={0, 0, 127}));
  connect(chiPar.port_b2, port_b2)
    annotation(
      Line(
        points={{-60, 24}, {-78, 24}, {-78,-60}, {-100,-60}},
        color={0, 127, 255}));
  connect(val5.port_b, bypFlo.port_b)
    annotation(
      Line(
        points={{40,-20}, {30,-20}, {30,-60}, {-20,-60}},
        color={0, 127, 255}));
  ...
  connect(senTem.port_b, val5.port_b)
    annotation(
      Line(
        points={{8, 24}, {2, 24}, {2, 0}, {30, 0}, {30,-20}, {40,-20}},
        color={0, 127, 255}));
  connect(yPum, pum.u)
    annotation(
      Line(
        points={{-120,-44}, {-96,-44}, {-40,-44}, {-40,-6}, {16,-6}, {16,-16}, {12,-16}},
        color={0, 0, 127}));
  connect(pum.port_a, val5.port_b)
    annotation(
      Line(
        points={{10,-20}, {25,-20}, {40,-20}},
        color={0, 127, 255}));
  connect(pum.P, powPum)
    annotation(
      Line(
        points={{-11,-16}, {-14,-16}, {-14, 50}, {88, 50}, {88,-40}, {110,-40}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
      ...
      graphics={Ellipse(
        ...
        points={{38,-40}, {8,-40}},
        color={0, 128, 255}), Line(
        points={{-14,-42}, {-18,-42}, {-18, 0}, {-24, 0}},
        color={0, 128, 255}), Line(
        points={{-72, 0}, {-76, 0}, {-76,-60}},
        color={0, 128, 255}), Line(
        points={{-92,-60}, {-76,-60}},
        color={0, 128, 255}), Line(
        points={{12, 0}, {12,-40}},
        color={0, 128, 255}), Line(
        points={{12,-40}, {12,-60}, {-76,-60}},
        color={0, 128, 255})}),
    Documentation(
      info="<html>
<p>
