...
model IntegratedPrimarySecondary
  ...
  parameter Modelica.SIunits.Time tauPump=1
    ...
    annotation(Dialog(tab="Dynamics", group="Pump", enable=not energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState));
  //Pump
  ...
  replaceable parameter Buildings.Fluid.Movers.Data.Generic perPum[numPum]
    ...
    annotation(Dialog(group="Pump"), Placement(transformation(extent={{38, 78}, {58, 98}})));
  ...
  parameter Real yVal5_start=0
    ...
    annotation(Dialog(tab="Dynamics", group="Filtered opening", enable=use_inputFilter));
  Modelica.Blocks.Interfaces.RealInput yPum[numPum](each final unit="1", each min=0, each max=1)
    "Prescribed normalized flowrate for primary pumps: 1 - nominal flowrate, 0 - no flowrate"
    annotation(Placement(transformation(extent={{-140,-60}, {-100,-20}}), iconTransformation(extent={{-130,-50}, {-100,-20}})));
  Modelica.Blocks.Interfaces.RealInput yVal5(final unit="1", min=0, max=1)
    "Actuator position for valve 5 (0: closed, 1: open)"
    annotation(Placement(transformation(extent={{-20,-20}, {20, 20}}, origin={-120, 26}), iconTransformation(extent={{-16,-16}, {16, 16}}, origin={-116, 30})));
  Modelica.Blocks.Interfaces.RealOutput powPum[numPum](each final quantity="Power", each final unit="W")
...
equation
  connect(wse.port_a2, port_a2)
    annotation(Line(points={{60, 24}, {80, 24}, {80,-60}, {100,-60}}, color={0, 127, 255}));
  connect(port_a2, val5.port_a)
    annotation(Line(points={{100,-60}, {100,-60}, {80,-60}, {80,-20}, {60,-20}}, color={0, 127, 255}));
  connect(pum.port_b, chiPar.port_a2)
    annotation(Line(points={{-10,-20}, {-20,-20}, {-20, 24}, {-40, 24}}, color={0, 127, 255}));
  connect(val5.y, yVal5)
    annotation(Line(points={{50,-8}, {50, 6}, {-94, 6}, {-94, 26}, {-120, 26}}, color={0, 0, 127}));
  connect(chiPar.port_b2, port_b2)
    annotation(Line(points={{-60, 24}, {-78, 24}, {-78,-60}, {-100,-60}}, color={0, 127, 255}));
  connect(val5.port_b, bypFlo.port_b)
    annotation(Line(points={{40,-20}, {30,-20}, {30,-60}, {-20,-60}}, color={0, 127, 255}));
  connect(bypFlo.port_a, port_b2)
    annotation(Line(points={{-40,-60}, {-100,-60}}, color={0, 127, 255}));
  connect(senTem.port_b, val5.port_b)
    annotation(Line(points={{8, 24}, {2, 24}, {2, 0}, {30, 0}, {30,-20}, {40,-20}}, color={0, 127, 255}));
  connect(yPum, pum.u)
    annotation(Line(points={{-120,-44}, {-96,-44}, {-40,-44}, {-40,-6}, {16,-6}, {16,-16}, {12,-16}}, color={0, 0, 127}));
  connect(pum.port_a, val5.port_b)
    annotation(Line(points={{10,-20}, {25,-20}, {40,-20}}, color={0, 127, 255}));
  connect(pum.P, powPum)
    annotation(Line(points={{-11,-16}, {-14,-16}, {-14, 50}, {88, 50}, {88,-40}, {110,-40}}, color={0, 0, 127}));
  annotation(Icon(coordinateSystem(preserveAspectRatio=false), graphics={Ellipse(extent={{-14,-30}, {8,-52}}, lineColor={0, 0, 0}, fillPattern=FillPattern.Sphere, fillColor={0, 128, 255}), Polygon(points={{-14,-42}, {-2,-52}, {-2,-30}, {-14,-42}}, lineColor={0, 0, 0}, pattern=LinePattern.None, fillPattern=FillPattern.HorizontalCylinder, fillColor={255, 255, 255}), Polygon(points={{-6,-7}, {-6, 9}, {3, 0}, {-6,-7}}, lineColor={0, 0, 0}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid, origin={44,-41}), Polygon(points={{-7,-6}, {9,-6}, {0, 3}, {-7,-6}}, lineColor={0, 0, 0}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid, origin={48,-41}, rotation=90), Line(points={{64, 0}, {74, 0}, {74,-60}, {94,-60}}, color={0, 128, 255}), Line(points={{-164, 74}}, color={0, 128, 255}), Line(points={{54,-40}, {74,-40}}, color={0, 128, 255}), Line(points={{38,-40}, {8,-40}}, color={0, 128, 255}), Line(points={{-14,-42}, {-18,-42}, {-18, 0}, {-24, 0}}, color={0, 128, 255}), Line(points={{-72, 0}, {-76, 0}, {-76,-60}}, color={0, 128, 255}), Line(points={{-92,-60}, {-76,-60}}, color={0, 128, 255}), Line(points={{12, 0}, {12,-40}}, color={0, 128, 255}), Line(points={{12,-40}, {12,-60}, {-76,-60}}, color={0, 128, 255})}), Documentation(info="<html>
<p>
