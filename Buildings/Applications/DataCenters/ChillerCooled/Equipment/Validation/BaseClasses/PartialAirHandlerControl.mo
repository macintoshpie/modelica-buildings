...
partial model PartialAirHandlerControl
  ...
  Buildings.Fluid.Sources.Boundary_pT sin_2(redeclare package Medium=Medium2, nPorts=1, use_p_in=false, p(displayUnit="Pa"), T=303.15)
    "Sink for water"
    annotation(Placement(transformation(extent={{-140, 0}, {-120, 20}})));
  Buildings.Fluid.Sources.Boundary_pT sou_2(redeclare package Medium=Medium2, X={0.02, 1-0.02}, use_T_in=true, use_X_in=true, p(displayUnit="Pa"))
    "Source for water"
    annotation(Placement(transformation(extent={{174, 0}, {154, 20}})));
  ...
  Buildings.Fluid.Sources.Boundary_pT sou_1(redeclare package Medium=Medium1, use_T_in=true, p=300000 + 12000, nPorts=1)
    "Sink for air"
  ...
  Modelica.Blocks.Sources.Step TWat(offset=T_a1_nominal, height=3, startTime=1500)
    "Water temperature"
    annotation(Placement(transformation(extent={{-110, 44}, {-90, 64}})));
  Buildings.Fluid.Sensors.MassFractionTwoPort masFra(redeclare package Medium=Medium2, m_flow_nominal=m2_flow_nominal)
    "Sensor for mass fraction"
    annotation(Placement(transformation(extent={{-54, 0}, {-74, 20}})));
  Buildings.Fluid.Sensors.TemperatureTwoPort temSenWat1(redeclare package Medium=Medium1, m_flow_nominal=m2_flow_nominal)
    "Temperature sensor for water"
    annotation(Placement(transformation(extent={{-40, 40}, {-20, 60}})));
  Buildings.Fluid.Sensors.TemperatureTwoPort temSenAir2(redeclare package Medium=Medium2, m_flow_nominal=m2_flow_nominal)
    "Temperature for air"
    annotation(Placement(transformation(extent={{-26, 0}, {-46, 20}})));
  Buildings.Fluid.Sensors.TemperatureTwoPort temSenWat2(redeclare package Medium=Medium1, m_flow_nominal=m2_flow_nominal)
    "Temperature for water"
    annotation(Placement(transformation(extent={{96, 40}, {116, 60}})));
equation
  connect(sin_1.ports[1], res_1.port_b)
    annotation(Line(points={{154, 50}, {144, 50}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(sin_2.ports[1], res_2.port_b)
    annotation(Line(points={{-120, 10}, {-108, 10}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(x_pTphi.X, sou_2.X_in)
    annotation(Line(points={{205,-42}, {212,-42}, {212,-44}, {220,-44}, {220, 6}, {176, 6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(relHum.y, x_pTphi.phi)
    annotation(Line(points={{155,-70}, {170,-70}, {170,-48}, {182,-48}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(temSou_2.y, x_pTphi.T)
    annotation(Line(points={{155,-38}, {168,-38}, {168,-42}, {182,-42}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(temSou_2.y, sou_2.T_in)
    annotation(Line(points={{155,-38}, {168,-38}, {168,-10}, {194,-10}, {194, 14}, {176, 14}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TWat.y, sou_1.T_in)
    annotation(Line(points={{-89, 54}, {-72, 54}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  annotation(Icon(coordinateSystem(preserveAspectRatio=false, extent={{-160,-100}, {220, 120}})), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-160,-100}, {220, 120}})), Documentation(info="<html>
