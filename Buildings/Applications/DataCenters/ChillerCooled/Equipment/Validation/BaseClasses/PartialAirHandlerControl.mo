...
partial model PartialAirHandlerControl
  ...
  Modelica.Blocks.Sources.Constant TSet(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-70, 80}, {-50, 100}})));
  Modelica.Blocks.Sources.Constant relHum(
    k=0.8)
    "Relative humidity"
    annotation(
      Placement(
  ...
  Buildings.Utilities.Psychrometrics.X_pTphi x_pTphi(
    ...
    annotation(
      Placement(
        transformation(
          extent={{184,-52}, {204,-32}})));
  Modelica.Blocks.Sources.Constant temSou_2(
    k=T_a2_nominal)
    "Temperature boundary for source 2"
    annotation(
      Placement(
        transformation(
          extent={{134,-48}, {154,-28}})));
...
equation
  ...
  connect(temSenWat1.port_a, sou_1.ports[1])
    annotation(
      Line(
        points={{-40, 50}, {-45, 50}, {-50, 50}},
        color={0, 127, 255}));
  connect(temSenAir2.port_b, masFra.port_a)
    annotation(
      Line(
        points={{-46, 10}, {-50, 10}, {-54, 10}},
        color={0, 127, 255}));
  connect(temSenWat2.port_b, res_1.port_a)
    annotation(
      Line(
        points={{116, 50}, {120, 50}, {124, 50}},
        color={0, 127, 255}));
  annotation(
    Icon(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-160,-100}, {220, 120}})),
    Diagram(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-160,-100}, {220, 120}})),
