...
partial model PartialAirHandlerMassFlow
  ...
  Buildings.Fluid.Sources.Boundary_pT sin_2(redeclare package Medium=Medium2, T=T_a2_nominal, nPorts=1)
    "Sink for air"
    annotation(Placement(transformation(extent={{-60, 10}, {-40, 30}})));
  Buildings.Fluid.Sources.Boundary_pT sou_2(redeclare package Medium=Medium2, T=T_a2_nominal, X={0.02, 1-0.02}, use_T_in=true, use_X_in=true)
    "Source for air"
    annotation(Placement(transformation(extent={{140, 10}, {120, 30}})));
  Buildings.Fluid.Sources.MassFlowSource_T sin_1(redeclare package Medium=Medium1, T=T_a1_nominal, use_m_flow_in=true, nPorts=1)
    "Source for water"
    annotation(Placement(transformation(extent={{150, 50}, {130, 70}})));
  Buildings.Fluid.Sources.Boundary_pT sou_1(redeclare package Medium=Medium1, use_T_in=false, T=T_a1_nominal, nPorts=1)
    "Sink for water"
  ...
  Modelica.Blocks.Sources.TimeTable mWatGai(table=[
    0, 1;
    3600*0.1, 1;
    3600*0.2, 0.1;
    3600*0.3, 0.1])
...
equation
  ...
  connect(mWatGai.y, mWat_flow.u)
    annotation(Line(points={{-59, 110}, {-44, 110}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(mWat_flow.y, sin_1.m_flow_in)
    annotation(Line(points={{-21, 110}, {160, 110}, {160, 68}, {150, 68}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  annotation(Icon(coordinateSystem(preserveAspectRatio=false, extent={{-120,-100}, {200, 180}})), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,-100}, {200, 180}})), Documentation(revisions="<html>
<ul>
<li>
