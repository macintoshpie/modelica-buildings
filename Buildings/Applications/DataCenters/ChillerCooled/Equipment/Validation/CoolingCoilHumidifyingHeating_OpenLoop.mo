  ...
  Buildings.Fluid.Sensors.RelativeHumidityTwoPort senRelHum(
    ...
    annotation(
      Placement(
        transformation(
          extent={{34, 10}, {14, 30}})));
  Modelica.Blocks.Sources.Constant uVal(
    k=0.2)
    ...
    annotation(
      Placement(
        transformation(
          extent={{0, 80}, {20, 100}})));
  Modelica.Blocks.Sources.Constant temSet(
    k=15 + 273.15)
    ...
    annotation(
      Placement(
        transformation(
          extent={{0,-30}, {20,-10}})));
  Modelica.Blocks.Sources.Constant XSet(
    k=0.01)
...
equation
  ...
  connect(uVal.y, ahu.uVal)
    annotation(
      Line(
        points={{21, 90}, {46, 90}, {46, 30}, {53, 30}},
        color={0, 0, 127}));
  connect(uFan.y, ahu.uFan)
    annotation(
      Line(
        points={{21,-80}, {48,-80}, {48, 22}, {48, 23}, {53, 23}, {53, 22}},
        color={0, 0, 127}));
  connect(temSet.y, ahu.TSet)
    annotation(
      Line(
        points={{21,-20}, {40,-20}, {40, 25}, {53, 25}},
        color={0, 0, 127}));
  connect(XSet.y, ahu.XSet_w)
    annotation(
      Line(
        points={{21,-50}, {44,-50}, {44, 28}, {53, 28}, {53, 27}},
        color={0, 0, 127}));
  ...
  connect(temSenWat1.port_b, ahu.port_a1)
    annotation(
      Line(
        points={{0, 60}, {20, 60}, {40, 60}, {40, 32}, {54, 32}},
        color={0, 127, 255}));
  connect(temSenWat2.port_a, ahu.port_b1)
    annotation(
      Line(
        points={{88, 60}, {80, 60}, {80, 32}, {74, 32}},
        color={0, 127, 255}));
  annotation(
    Diagram(
      coordinateSystem(
    ...
    experiment(
      ...
      StopTime=1000),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/ChillerCooled/Equipment/Validation/CoolingCoilHumidifyingHeating_OpenLoop.mos"
        "Simulate and PLot"),
    Documentation(
      info="<html>
<p>
