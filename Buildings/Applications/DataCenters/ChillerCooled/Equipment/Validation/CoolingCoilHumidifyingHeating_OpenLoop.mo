...
model CoolingCoilHumidifyingHeating_OpenLoop
  ...
  extends Buildings.Applications.DataCenters.ChillerCooled.Equipment.Validation.BaseClasses.PartialAirHandlerMassFlow(sou_2(nPorts=1), relHum(k=0.5));
  parameter Modelica.SIunits.ThermalConductance UA_nominal=m2_flow_nominal*1006*(12-26)/Buildings.Fluid.HeatExchangers.BaseClasses.lmtd(T_a1_nominal, T_b1_nominal, T_a2_nominal, T_b2_nominal)
...
equation
  connect(ahu.port_a2, sou_2.ports[1])
    annotation(Line(points={{74, 20}, {120, 20}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(ahu.port_b2, senRelHum.port_a)
    annotation(Line(points={{54, 20}, {34, 20}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(uVal.y, ahu.uVal)
    annotation(Line(points={{21, 90}, {46, 90}, {46, 30}, {53, 30}}, color={0, 0, 127}));
  connect(uFan.y, ahu.uFan)
    annotation(Line(points={{21,-80}, {48,-80}, {48, 22}, {48, 23}, {53, 23}, {53, 22}}, color={0, 0, 127}));
  connect(temSet.y, ahu.TSet)
    annotation(Line(points={{21,-20}, {40,-20}, {40, 25}, {53, 25}}, color={0, 0, 127}));
  connect(XSet.y, ahu.XSet_w)
    annotation(Line(points={{21,-50}, {44,-50}, {44, 28}, {53, 28}, {53, 27}}, color={0, 0, 127}));
  ...
  connect(temSenWat1.port_b, ahu.port_a1)
    annotation(Line(points={{0, 60}, {20, 60}, {40, 60}, {40, 32}, {54, 32}}, color={0, 127, 255}));
  connect(temSenWat2.port_a, ahu.port_b1)
    annotation(Line(points={{88, 60}, {80, 60}, {80, 32}, {74, 32}}, color={0, 127, 255}));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100}, {200, 200}})), experiment(Tolerance=1E-6, StopTime=1000), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/ChillerCooled/Equipment/Validation/CoolingCoilHumidifyingHeating_OpenLoop.mos"
