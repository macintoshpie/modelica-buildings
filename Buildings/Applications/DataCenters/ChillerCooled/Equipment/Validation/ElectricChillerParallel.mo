...
model ElectricChillerParallel
  ...
  extends Buildings.Fluid.Chillers.Examples.BaseClasses.PartialElectric(
    ...
    sou1(
      nPorts=1,
      m_flow=2*mCon_flow_nominal),
    sou2(
      nPorts=1,
      m_flow=2*mEva_flow_nominal));
...
equation
  connect(chiPar.port_b1, res1.port_a)
    annotation(
      Line(
        points={{10, 16}, {20, 16}, {20, 40}, {32, 40}},
        color={0, 127, 255}));
  connect(chiPar.port_a1, sou1.ports[1])
    annotation(
      Line(
        points={{-10, 16}, {-26, 16}, {-40, 16}},
        color={0, 127, 255}));
  ...
  connect(chiPar.port_a2, sou2.ports[1])
    annotation(
      Line(
        points={{10, 4}, {26, 4}, {40, 4}},
        color={0, 127, 255}));
  connect(greaterThreshold.y, chiPar.on[1])
    annotation(
      Line(
        points={{-19, 90}, {-16, 90}, {-16, 13}, {-12, 13}},
        color={255, 0, 255}));
  connect(greaterThreshold.y, chiPar.on[2])
    annotation(
      Line(
        points={{-19, 90}, {-16, 90}, {-16, 15}, {-12, 15}},
        color={255, 0, 255}));
  connect(TSet.y, chiPar.TSet)
    annotation(
      Line(
        points={{-59, 60}, {-28, 60}, {-28, 10}, {-12, 10}},
        color={0, 0, 127}));
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/ChillerCooled/Equipment/Validation/ElectricChillerParallel.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
