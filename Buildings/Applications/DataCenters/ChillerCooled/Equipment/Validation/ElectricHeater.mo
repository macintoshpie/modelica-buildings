  ...
  Buildings.Applications.DataCenters.ChillerCooled.Equipment.ElectricHeater eleHea(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-20,-50}, {0,-30}})));
  Buildings.Controls.SetPoints.Table tab(
    table=[
      0, 273.15 + 15;
      1, 273.15 + 30])
    ...
    annotation(
      Placement(
        transformation(
          extent={{-30, 20}, {-10, 40}})));
  Modelica.Blocks.Sources.BooleanStep uHea(
    startTime(
      displayUnit="min")=60000)
    "On/off signal"
    annotation(
      Placement(
        transformation(
...
equation
  ...
  connect(conPI.y, tab.u)
    annotation(
      Line(
        points={{-39, 30}, {-35.5, 30}, {-32, 30}},
        color={0, 0, 127}));
  connect(tab.y, eleHea.TSet)
    annotation(
      Line(
        points={{-9, 30}, {-4, 30}, {-4,-16}, {-30,-16}, {-30,-32}, {-22,-32}},
        color={0, 0, 127}));
  connect(uHea.y, eleHea.on)
    annotation(
      Line(
        points={{-59,-88}, {-36,-88}, {-36,-37}, {-22,-37}},
        color={255, 0, 255}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=172800),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/ChillerCooled/Equipment/Validation/ElectricHeater.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
