...
model HeatExchanger
  ...
  Buildings.Fluid.Sources.Boundary_pT sin1(nPorts=2, redeclare package Medium=MediumW)
    "Sink on medium 1 side"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, origin={78, 70})));
  Buildings.Fluid.Sources.MassFlowSource_T sou1(use_T_in=true, nPorts=2, redeclare package Medium=MediumW, m_flow=2*m1_flow_nominal, T=298.15)
  ...
  Buildings.Fluid.Sources.Boundary_pT sin2(nPorts=2, redeclare package Medium=MediumW)
    "Sink on medium 2 side"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, origin={-82, 0})));
  Buildings.Fluid.Sources.MassFlowSource_T sou2_2(nPorts=1, redeclare package Medium=MediumW, m_flow=m2_flow_nominal, use_T_in=true)
  ...
  Buildings.Fluid.Sources.MassFlowSource_T sou2_1(nPorts=1, redeclare package Medium=MediumW, m_flow=m2_flow_nominal, use_T_in=true)
    "Source on medium 2 side"
...
equation
  ...
  connect(sou1.ports[1], hex1.port_a1)
    annotation(Line(points={{-40, 78}, {-20, 78}, {-20, 62}, {-12, 62}}, color={0, 127, 255}));
  connect(hex1.port_b1, sin1.ports[1])
    annotation(Line(points={{8, 62}, {56, 62}, {56, 72}, {68, 72}}, color={0, 127, 255}));
  ...
  connect(hex1.port_b2, TSen1.port_a)
    annotation(Line(points={{-12, 50}, {-22, 50}, {-30, 50}, {-30, 20}, {-40, 20}}, color={0, 127, 255}));
  connect(TSen1.port_b, sin2.ports[1])
    annotation(Line(points={{-60, 20}, {-70, 20}, {-70, 2}, {-72, 2}}, color={0, 127, 255}));
  connect(sou1.ports[2], hex2.port_a1)
    annotation(Line(points={{-40, 74}, {-22, 74}, {-22,-16}, {-12,-16}}, color={0, 127, 255}));
  connect(hex2.port_b2, TSen2.port_a)
    annotation(Line(points={{-12,-28}, {-40,-28}}, color={0, 127, 255}));
  connect(TSen2.port_b, sin2.ports[2])
    annotation(Line(points={{-60,-28}, {-68,-28}, {-68,-2}, {-72,-2}}, color={0, 127, 255}));
  connect(hex2.port_b1, sin1.ports[2])
    annotation(Line(points={{8,-16}, {30,-16}, {60,-16}, {60, 68}, {68, 68}}, color={0, 127, 255}));
  connect(hex2.port_a2, sou2_2.ports[1])
    annotation(Line(points={{8,-28}, {20,-28}, {20,-74}, {38,-74}}, color={0, 127, 255}));
  connect(hex1.port_a2, sou2_1.ports[1])
    annotation(Line(points={{8, 50}, {18, 50}, {28, 50}, {28, 24}, {34, 24}}, color={0, 127, 255}));
  connect(TEva_in1.y, sou2_1.T_in)
    annotation(Line(points={{69, 28}, {56, 28}}, color={0, 0, 127}));
  annotation(__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/ChillerCooled/Equipment/Validation/HeatExchanger.mos"
...
</html>"), experiment(StartTime=0, StopTime=3600, Tolerance=1e-06));
end HeatExchanger;
