...
model IntegratedPrimaryLoadSide
  ...
  extends Buildings.Applications.DataCenters.ChillerCooled.Equipment.Validation.BaseClasses.PartialPlant(sou1(nPorts=2, m_flow=2*mCW_flow_nominal), sin1(nPorts=2), TSet(k=273.15 + 5.56), TEva_in(k=273.15 + 15.28), sin2(nPorts=2));
  ...
  Modelica.Blocks.Sources.RealExpression yVal5(y=if onChi.y and not onWSE.y then 1 else 0)
    "On/off signal for valve 5"
    annotation(Placement(transformation(extent={{40, 70}, {20, 90}})));
  Modelica.Blocks.Sources.RealExpression yVal6(y=if not onChi.y and onWSE.y then 1 else 0)
  ...
  Modelica.Blocks.Sources.BooleanStep onWSE(startTime(displayUnit="h")=14400, startValue=true)
    "On and off signal for the WSE"
    annotation(Placement(transformation(extent={{-100, 50}, {-80, 70}})));
  Buildings.Fluid.Sources.Boundary_pT sou2(redeclare package Medium=MediumCHW, nPorts=2, use_T_in=true)
    "Source on medium 2 side"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, origin={50,-74})));
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort TSup2(redeclare package Medium=MediumCHW, m_flow_nominal=mCHW_flow_nominal)
  ...
  parameter Fluid.Movers.Data.Generic[numChi] perPum(each pressure=Buildings.Fluid.Movers.BaseClasses.Characteristics.flowParameters(V_flow=mCHW_flow_nominal/1000*{0.2, 0.6, 1.0, 1.2}, dp=dpCHW_nominal*{1.2, 1.1, 1.0, 0.6}))
    "Pump performance data"
    annotation(Placement(transformation(extent={{60, 80}, {80, 100}})));
equation
  connect(onChi.y, intWSEPri1.on[1])
    annotation(Line(points={{-79, 90}, {-26, 90}, {-26,-30.4}, {-11.6,-30.4}}, color={255, 0, 255}));
  connect(onWSE.y, intWSEPri1.on[2])
    annotation(Line(points={{-79, 60}, {-26, 60}, {-26,-30.4}, {-11.6,-30.4}}, color={255, 0, 255}));
  connect(TSet.y, intWSEPri1.TSet)
    annotation(Line(points={{-79, 30}, {-28, 30}, {-28,-27.2}, {-11.6,-27.2}}, color={0, 0, 127}));
  connect(yVal5.y, intWSEPri1.yVal5)
    annotation(Line(points={{19, 80}, {-24, 80}, {-24,-35}, {-11.6,-35}}, color={0, 0, 127}));
  connect(yVal6.y, intWSEPri1.yVal6)
    annotation(Line(points={{19, 60}, {2, 60}, {2, 50}, {-22, 50}, {-22,-38.2}, {-11.6,-38.2}}, color={0, 0, 127}));
  connect(yPum.y, intWSEPri1.yPum[1])
    annotation(Line(points={{19, 30}, {-20, 30}, {-20,-42.4}, {-11.6,-42.4}}, color={0, 0, 127}));
  connect(intWSEPri1.port_a1, sou1.ports[1])
    annotation(Line(points={{-10,-32}, {-32,-32}, {-32,-4}, {-40,-4}}, color={0, 127, 255}));
  ...
  connect(intWSEPri1.port_b1, sin1.ports[1])
    annotation(Line(points={{10,-32}, {22,-32}, {22,-4}, {70,-4}}, color={0, 127, 255}));
  connect(intWSEPri1.port_a2, sou2.ports[1])
    annotation(Line(points={{10,-44}, {26,-44}, {26,-72}, {40,-72}}, color={0, 127, 255}));
  ...
  connect(intWSEPri2.port_a2, sou2.ports[2])
    annotation(Line(points={{10,-126}, {26,-126}, {26,-76}, {40,-76}}, color={0, 127, 255}));
  connect(intWSEPri2.port_b1, sin1.ports[2])
    annotation(Line(points={{10,-114}, {22,-114}, {22,-4}, {70,-4}}, color={0, 127, 255}));
  connect(intWSEPri2.port_a1, sou1.ports[2])
    annotation(Line(points={{-10,-114}, {-32,-114}, {-32,-4}, {-40,-4}}, color={0, 127, 255}));
  connect(TSup2.port_b, sin2.ports[2])
    annotation(Line(points={{-60,-126}, {-64,-126}, {-64,-70}, {-70,-70}}, color={0, 127, 255}));
  connect(onChi.y, intWSEPri2.on[1])
    annotation(Line(points={{-79, 90}, {-26, 90}, {-26,-112.4}, {-11.6,-112.4}}, color={255, 0, 255}));
  connect(onWSE.y, intWSEPri2.on[2])
    annotation(Line(points={{-79, 60}, {-26, 60}, {-26,-112.4}, {-11.6,-112.4}}, color={255, 0, 255}));
  connect(TSet.y, intWSEPri2.TSet)
    annotation(Line(points={{-79, 30}, {-28, 30}, {-28,-109.2}, {-11.6,-109.2}}, color={0, 0, 127}));
  connect(yVal5.y, intWSEPri2.yVal5)
    annotation(Line(points={{19, 80}, {-24, 80}, {-24,-117}, {-11.6,-117}}, color={0, 0, 127}));
  connect(yVal6.y, intWSEPri2.yVal6)
    annotation(Line(points={{19, 60}, {2, 60}, {2, 50}, {-22, 50}, {-22,-120.2}, {-11.6,-120.2}}, color={0, 0, 127}));
  connect(yPum.y, intWSEPri2.yPum[1])
    annotation(Line(points={{19, 30}, {-20, 30}, {-20,-124.4}, {-11.6,-124.4}}, color={0, 0, 127}));
  connect(tri.y, intWSEPri2.trigger)
    annotation(Line(points={{-79,-150}, {-6,-150}, {-6,-130}}, color={255, 0, 255}));
  annotation(__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/ChillerCooled/Equipment/Validation/IntegratedPrimaryLoadSide.mos"
...
</html>"), experiment(StartTime=0, StopTime=21600, Tolerance=1e-06), Diagram(coordinateSystem(extent={{-100,-160}, {100, 100}})));
