...
partial model PartialDataCenter
  ...
  replaceable Buildings.Applications.DataCenters.ChillerCooled.Equipment.BaseClasses.PartialChillerWSE chiWSE(
    ...
    redeclare each Buildings.Fluid.Chillers.Data.ElectricEIR.ElectricEIRChiller_York_YT_1055kW_5_96COP_Vanes perChi,
  ...
  Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaData(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-360,-80}, {-340,-60}})));
  Buildings.BoundaryConditions.WeatherData.Bus weaBus
    "Weather data bus"
    annotation(
      Placement(
        transformation(
          extent={{-338,-30}, {-318,-10}})));
  ...
  Modelica.Blocks.Sources.RealExpression TCWSupSet(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-260, 176}, {-240, 196}})));
  Modelica.Blocks.Sources.Constant TAirSupSet(
    k=TSupAirSet)
    "Supply air temperature setpoint"
    annotation(
      Placement(
  ...
  Buildings.Controls.Continuous.LimPID ahuFanSpeCon(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-120,-170}, {-100,-150}})));
  Modelica.Blocks.Sources.Constant TAirRetSet(
    k=TRetAirSet)
    "Return air temperature setpoint"
    annotation(
      Placement(
        transformation(
          extent={{-180,-170}, {-160,-150}})));
  Utilities.Psychrometrics.X_pTphi XAirSupSet(
    use_p_in=false)
  ...
  Modelica.Blocks.Sources.Constant phiAirRetSet(
    k=0.5)
...
equation
  ...
  for i in 1 : numChi loop
    connect(cooTow[i].TAir, weaBus.TWetBul.TWetBul)
      annotation(
        Line(
          points={{22, 144}, {44, 144}, {44, 200}, {-340, 200}, {-340,-20}, {-328,-20}},
          color={255, 204, 51},
          thickness=0.5));
    ...
    connect(TCWRet.port_b, val[i].port_a)
      annotation(
        Line(
          points={{102, 60}, {110, 60}, {110, 140}, {70, 140}},
          color={0, 127, 255},
          thickness=0.5));
  end for;
  connect(senRelPre.port_b, ahu.port_b1)
    annotation(
      Line(
        points={{18,-96}, {30,-96}, {30,-114}, {20,-114}},
        color={0, 127, 255},
        thickness=0.5));
  ...
  connect(TCWRet.port_b, expVesCW.ports[1])
    annotation(
      Line(
        points={{102, 60}, {110, 60}, {110, 140.5}, {122, 140.5}},
        color={0, 127, 255},
        thickness=0.5));
  ...
  connect(gai1.y, pumSpe.u_m)
    annotation(
      Line(
        points={{-221,-60}, {-236,-60}, {-236,-32}},
        color={0, 0, 127}));
  connect(gai1.u, senRelPre.p_rel)
    annotation(
      Line(
        points={{-198,-60}, {8,-60}, {8,-87}},
        color={0, 0, 127}));
  annotation(
    Diagram(
      coordinateSystem(
