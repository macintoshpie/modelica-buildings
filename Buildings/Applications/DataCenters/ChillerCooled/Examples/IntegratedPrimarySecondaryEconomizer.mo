...
model IntegratedPrimarySecondaryEconomizer
  ...
  parameter Buildings.Fluid.Movers.Data.Generic[numChi] perPumPri(each pressure=Buildings.Fluid.Movers.BaseClasses.Characteristics.flowParameters(V_flow=m2_flow_chi_nominal/1000*{0.2, 0.6, 1.0, 1.2}, dp=(dp2_chi_nominal + dp2_wse_nominal + 6000)*{1.5, 1.3, 1.0, 0.6}))
    "Performance data for secondary chilled water pumps";
  Buildings.Applications.DataCenters.ChillerCooled.Controls.CoolingMode cooModCon(tWai=tWai, deaBan1=1.1, deaBan2=0.5, deaBan3=1.1, deaBan4=0.5)
  ...
  Modelica.Blocks.Sources.RealExpression yVal5(y=if cooModCon.y == Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FullMechanical) then 1 else 0)
    "On/off signal for valve 5"
    annotation(Placement(transformation(extent={{-60, 32}, {-40, 52}})));
  Modelica.Blocks.Sources.RealExpression cooLoaChi(y=-chiWSE.port_a2.m_flow*4180*(chiWSE.TCHWSupWSE-TCHWSupSet.y))
    "Cooling load in chillers"
    annotation(Placement(transformation(extent={{-260, 122}, {-240, 142}})));
  Buildings.Applications.DataCenters.ChillerCooled.Equipment.FlowMachine_y secPum(redeclare package Medium=MediumW, dpValve_nominal=6000, per=perPumSec, addPowerToMedium=false, m_flow_nominal=m2_flow_chi_nominal, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
    "Secondary pumps"
    annotation(Placement(transformation(extent={{-10, 10}, {10,-10}}, rotation=-90, origin={-46,-34})));
  ...
  Modelica.Blocks.Math.Product priPumSpe[numChi]
    "Primary pump speed signal"
    annotation(Placement(transformation(extent={{-104, 22}, {-84, 42}})));
  Modelica.Blocks.Sources.RealExpression notFreCoo(y=if cooModCon.y == Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FreeCooling) then 0 else 1)
    "Not free cooling mode"
    annotation(Placement(transformation(extent={{-140, 30}, {-120, 50}})));
equation
  connect(yVal5.y, chiWSE.yVal5)
    annotation(Line(points={{-39, 42}, {-20, 42}, {-20, 33}, {-1.6, 33}}, color={0, 0, 127}));
  connect(cooLoaChi.y, chiStaCon.QTot)
    annotation(Line(points={{-239, 132}, {-184, 132}, {-184, 140}, {-172, 140}}, color={0, 0, 127}));
  connect(TCHWSup.port_b, secPum.port_a)
    annotation(Line(points={{-36, 0}, {-36, 0}, {-44, 0}, {-44, 0}, {-46, 0}, {-46,-24}, {-46,-24}}, color={0, 127, 255}, thickness=0.5));
  connect(secPum.port_b, ahu.port_a1)
    annotation(Line(points={{-46,-44}, {-46,-114}, {0,-114}}, color={0, 127, 255}, thickness=0.5));
  connect(pumSpeSig.y, secPum.u)
    annotation(Line(points={{-99,-10}, {-78,-10}, {-78,-10}, {-50,-10}, {-50,-22}}, color={0, 0, 127}));
  ...
  connect(towTApp.y, cooModCon.TApp)
    annotation(Line(points={{-307, 110}, {-210, 110}}, color={0, 0, 127}));
  connect(weaBus.TWetBul.TWetBul, cooModCon.TWetBul)
    annotation(Line(points={{-328,-20}, {-340,-20}, {-340, 200}, {-218, 200}, {-218, 114}, {-210, 114}}, color={255, 204, 51}, thickness=0.5));
  connect(TCHWRet.port_b, chiWSE.port_a2)
    annotation(Line(points={{80, 0}, {76, 0}, {76, 24}, {20, 24}}, color={0, 127, 255}, thickness=0.5));
  connect(cooModCon.TCHWRetWSE, TCHWRet.T)
    annotation(Line(points={{-210, 102}, {-226, 102}, {-226, 204}, {148, 204}, {148, 20}, {90, 20}, {90, 11}}, color={0, 0, 127}));
  connect(chiWSE.TCHWSupWSE, cooModCon.TCHWSupWSE)
    annotation(Line(points={{21, 34}, {146, 34}, {146, 202}, {-222, 202}, {-222, 106}, {-210, 106}}, color={0, 0, 127}));
  connect(TCHWSupSet.y, cooModCon.TCHWSupSet)
    annotation(Line(points={{-239, 160}, {-214, 160}, {-214, 118}, {-210, 118}}, color={0, 0, 127}));
  connect(TCHWSup.T, chiStaCon.TCHWSup)
    annotation(Line(points={{-26, 11}, {-26, 18}, {-60, 18}, {-60, 18}, {-180, 18}, {-180, 134}, {-172, 134}}, color={0, 0, 127}));
  connect(PriPumCon.numOnChi, chiNumOn.y)
    annotation(Line(points={{-174, 27}, {-182, 27}, {-182, 65}, {-236.9, 65}}, color={255, 127, 0}));
  connect(PriPumCon.cooMod, cooModCon.y)
    annotation(Line(points={{-174, 37}, {-174, 36}, {-182, 36}, {-182, 110}, {-187, 110}}, color={255, 127, 0}));
  connect(cooTowSpeCon.cooMod, cooModCon.y)
    annotation(Line(points={{-172, 182.444}, {-182, 182.444}, {-182, 110}, {-187, 110}}, color={255, 127, 0}));
  connect(intToBoo.u, cooModCon.y)
    annotation(Line(points={{-172, 110}, {-187, 110}}, color={255, 127, 0}));
  connect(CWPumCon.cooMod, cooModCon.y)
    annotation(Line(points={{-174, 75}, {-174, 74}, {-182, 74}, {-182, 110}, {-187, 110}}, color={255, 127, 0}));
  connect(cooModCon.y, sigCha.u)
    annotation(Line(points={{-187, 110}, {-182, 110}, {-182, 206}, {150, 206}, {150, 160}, {178, 160}}, color={255, 127, 0}));
  connect(cooModCon.y, chiStaCon.cooMod)
    annotation(Line(points={{-187, 110}, {-182, 110}, {-182, 146}, {-172, 146}}, color={255, 127, 0}));
  connect(notFreCoo.y, priPumSpe[1].u1)
    annotation(Line(points={{-119, 40}, {-114, 40}, {-114, 38}, {-106, 38}}, color={0, 0, 127}));
  connect(notFreCoo.y, priPumSpe[2].u1)
    annotation(Line(points={{-119, 40}, {-114, 40}, {-114, 38}, {-106, 38}}, color={0, 0, 127}));
  connect(PriPumCon.y, priPumSpe.u2)
    annotation(Line(points={{-151, 32}, {-114, 32}, {-114, 26}, {-106, 26}}, color={0, 0, 127}));
  connect(priPumSpe.y, chiWSE.yPum)
    annotation(Line(points={{-83, 32}, {-20, 32}, {-20, 26.5}, {-1.5, 26.5}}, color={0, 0, 127}));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-360,-200}, {300, 220}})), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/ChillerCooled/Examples/IntegratedPrimarySecondaryEconomizer.mos"
