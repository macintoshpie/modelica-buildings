...
model IntegratedPrimarySecondaryEconomizer
  ...
  extends Buildings.Applications.DataCenters.ChillerCooled.Examples.BaseClasses.PostProcess(
    ...
    PHVAC(
      y=cooTow[1].PFan + cooTow[2].PFan + pumCW[1].P + pumCW[2].P + sum(chiWSE.powChi + chiWSE.powPum) + sum(secPum.P) + ahu.PFan + ahu.PHea),
    PIT(
      y=roo.QSou.Q_flow));
  ...
  parameter Buildings.Fluid.Movers.Data.Generic[numChi] perPumPri(
    ...
    "Performance data for secondary chilled water pumps";
  Buildings.Applications.DataCenters.ChillerCooled.Controls.CoolingMode cooModCon(
    tWai=tWai,
    deaBan1=1.1,
    deaBan2=0.5,
    deaBan3=1.1,
    ...
    annotation(
      Placement(
        transformation(
          extent={{-208, 100}, {-188, 120}})));
  Modelica.Blocks.Sources.RealExpression towTApp(
    y=cooTow[1].TApp_nominal)
    "Cooling tower approach temperature"
    annotation(
      Placement(
        transformation(
          extent={{-328, 100}, {-308, 120}})));
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=-90,
          origin={-46,-34})));
  Buildings.Applications.DataCenters.ChillerCooled.Controls.ConstantSpeedPumpStage PriPumCon(
    tWai=0)
    "Chilled water primary pump controller"
    annotation(
      Placement(
        transformation(
          extent={{-172, 22}, {-152, 42}})));
  Modelica.Blocks.Math.Product priPumSpe[numChi]
    ...
    annotation(
      Placement(
        transformation(
          extent={{-104, 22}, {-84, 42}})));
  Modelica.Blocks.Sources.RealExpression notFreCoo(
    y=if cooModCon.y == Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FreeCooling) then 0 else 1)
...
equation
  connect(yVal5.y, chiWSE.yVal5)
    annotation(
      Line(
        points={{-39, 42}, {-20, 42}, {-20, 33}, {-1.6, 33}},
        color={0, 0, 127}));
  ...
  connect(cooModCon.TCHWRetWSE, TCHWRet.T)
    annotation(
      Line(
        points={{-210, 102}, {-226, 102}, {-226, 204}, {148, 204}, {148, 20}, {90, 20}, {90, 11}},
        color={0, 0, 127}));
  ...
  connect(TCHWSup.T, chiStaCon.TCHWSup)
    annotation(
      Line(
        points={{-26, 11}, {-26, 18}, {-60, 18}, {-60, 18}, {-180, 18}, {-180, 134}, {-172, 134}},
        color={0, 0, 127}));
  ...
  connect(cooModCon.y, chiStaCon.cooMod)
    annotation(
      Line(
        points={{-187, 110}, {-182, 110}, {-182, 146}, {-172, 146}},
        color={255, 127, 0}));
  connect(notFreCoo.y, priPumSpe[1].u1)
    annotation(
      Line(
        points={{-119, 40}, {-114, 40}, {-114, 38}, {-106, 38}},
        color={0, 0, 127}));
  ...
  connect(PriPumCon.y, priPumSpe.u2)
    annotation(
      Line(
        points={{-151, 32}, {-114, 32}, {-114, 26}, {-106, 26}},
        color={0, 0, 127}));
  connect(priPumSpe.y, chiWSE.yPum)
    annotation(
      Line(
        points={{-83, 32}, {-20, 32}, {-20, 26.5}, {-1.5, 26.5}},
        color={0, 0, 127}));
  annotation(
    Diagram(
      coordinateSystem(
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/ChillerCooled/Examples/IntegratedPrimarySecondaryEconomizer.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
