...
model Compressor
  ...
  Buildings.Controls.Continuous.LimPID dxSpe(Td=1, final controllerType=Modelica.Blocks.Types.SimpleController.PI, final k=k, final Ti=Ti, final yMax=yMax, final yMin=yMin, reverseAction=reverseAction, reset=Buildings.Types.Reset.Parameter, y_reset=yMin)
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput y(min=0, max=1, final unit="1")
    "Compressor speed"
    annotation(Placement(transformation(extent={{100,-10}, {120, 10}})));
  Modelica.Blocks.Interfaces.RealInput TMixAirSet(final unit="K", final quantity="ThermodynamicTemperature", displayUnit="degC")
    "Mixed air setpoint temperature"
    annotation(Placement(transformation(extent={{-140, 40}, {-100, 80}})));
  Modelica.Blocks.Interfaces.RealInput TMixAirMea(final unit="K", final quantity="ThermodynamicTemperature", displayUnit="degC")
...
protected
  Buildings.Controls.OBC.CDL.Integers.Sources.Constant conInt(final k=Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FreeCooling))
...
equation
  connect(cooMod, freCoo.u1)
    annotation(Line(points={{-120,-70}, {-92,-70}, {-92,-50}, {-42,-50}}, color={255, 127, 0}));
  connect(conInt.y, freCoo.u2)
    annotation(Line(points={{-59,-70}, {-52,-70}, {-52,-58}, {-42,-58}}, color={255, 127, 0}));
  connect(freCoo.y, switch1.u2)
    annotation(Line(points={{-19,-50}, {40,-50}, {40, 0}, {58, 0}}, color={255, 0, 255}));
  connect(const.y, switch1.u1)
    annotation(Line(points={{41, 40}, {48, 40}, {48, 8}, {58, 8}}, color={0, 0, 127}));
  connect(dxSpe.y, switch1.u3)
    annotation(Line(points={{-19, 60}, {10, 60}, {10,-8}, {58,-8}}, color={0, 0, 127}));
  connect(switch1.y, y)
  ...
  connect(freCoo.y, not1.u)
    annotation(Line(points={{-19,-50}, {0,-50}, {0,-20}, {-80,-20}, {-80, 30}, {-70, 30}}, color={255, 0, 255}));
  annotation(Icon(coordinateSystem(preserveAspectRatio=false), graphics={Rectangle(extent={{-100, 100}, {100,-100}}, lineColor={0, 0, 127}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Text(extent={{128, 114}, {-128, 166}}, lineColor={0, 0, 255}, textString="%name")}), Diagram(coordinateSystem(preserveAspectRatio=false)), Documentation(info="<html>
<p>
