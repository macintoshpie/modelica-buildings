...
model AirsideEconomizer
  ...
  Buildings.Applications.DataCenters.DXCooled.Controls.AirsideEconomizer con(minOAFra=0, Ti=1)
    "Outdoor air damper controller"
    annotation(Placement(transformation(extent={{10, 40}, {30, 60}})));
  Modelica.Blocks.Sources.Constant mixAirSetPoi(k=291.15)
    "Mixed air temperature setpoint"
    annotation(Placement(transformation(extent={{-60, 70}, {-40, 90}})));
  Modelica.Blocks.Sources.CombiTimeTable cooMod(table=[
    0, Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FreeCooling);
  ...
  Buildings.Fluid.Sources.Boundary_pT OA(redeclare package Medium=Medium, nPorts=2, use_T_in=true, p(displayUnit="Pa")=100000)
  ...
  Buildings.Fluid.Sources.Boundary_pT roo(redeclare package Medium=Medium, nPorts=1, p=100000 + 30, T(displayUnit="degC")=298.15)
    "Boundary conditions for room air"
    annotation(Placement(transformation(extent={{72,-72}, {52,-52}})));
  Modelica.Blocks.Sources.CombiTimeTable outAir(table=[
    0, 273.15 + 5;
    120, 273.15 + 5;
    120, 273.15 + 20;
    240, 273.15 + 20;
  ...
  Buildings.Fluid.Sources.Boundary_pT supAir(redeclare package Medium=Medium, nPorts=1, p(displayUnit="Pa")=100000-30)
    "Boundary conditions for supply air"
...
equation
  connect(mixAirSetPoi.y, con.TMixAirSet)
    annotation(Line(points={{-39, 80}, {-20, 80}, {-20, 56}, {8, 56}}, color={0, 0, 127}));
  connect(OA.ports[1], eco.port_Out)
    annotation(Line(points={{-30,-32}, {-20,-32}}, color={0, 127, 255}));
  connect(eco.port_Exh, OA.ports[2])
    annotation(Line(points={{-20,-44}, {-24,-44}, {-24,-36}, {-30,-36}}, color={0, 127, 255}));
  connect(eco.port_Sup, senTemMixAir.port_a)
    annotation(Line(points={{0,-32}, {10,-32}}, color={0, 127, 255}));
  connect(senTemMixAir.T, con.TMixAirMea)
    annotation(Line(points={{20,-21}, {20, 8}, {-12, 8}, {-12, 50}, {8, 50}}, color={0, 0, 127}));
  connect(con.y, eco.y)
    annotation(Line(points={{31, 50}, {40, 50}, {40,-12}, {-10,-12}, {-10,-26}}, color={0, 0, 127}));
  connect(eco.port_Ret, roo.ports[1])
    annotation(Line(points={{0,-44}, {4,-44}, {4,-62}, {52,-62}}, color={0, 127, 255}));
  ...
  annotation(__Dymola_Commands(file="Resources/Scripts/Dymola/Applications/DataCenters/DXCooled/Controls/Validation/AirsideEconomizer.mos"
...
</html>"), experiment(StopTime=360, Tolerance=1e-06));
