  ...
  Buildings.Applications.DataCenters.DXCooled.Controls.Compressor dxSpeCon
    ...
    annotation(
      Placement(
        transformation(
          extent={{20,-10}, {40, 10}})));
  Modelica.Blocks.Sources.Constant TSupSet(
    k=291.15)
...
equation
  connect(TSupSet.y, dxSpeCon.TMixAirSet)
    annotation(
      Line(
        points={{-39, 70}, {10, 70}, {10, 6}, {18, 6}},
        color={0, 0, 127}));
  connect(TSupMea.y, dxSpeCon.TMixAirMea)
    annotation(
      Line(
        points={{-39, 20}, {6, 20}, {6, 0}, {18, 0}},
        color={0, 0, 127}));
  ...
  connect(reaToInt.y, dxSpeCon.cooMod)
    annotation(
      Line(
        points={{1,-40}, {10,-40}, {10,-7}, {18,-7}},
        color={255, 127, 0}));
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/DXCooled/Controls/Validation/Compressor.mos"
        "Simulate and Plot"),
    Documentation(
      revisions="<html>
<ul>
