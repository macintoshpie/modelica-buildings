...
model CoolingMode
  ...
  Buildings.Applications.DataCenters.DXCooled.Controls.CoolingMode cooModCon(tWai=1200)
  ...
  Modelica.Blocks.Sources.Constant TRet(k=295.15)
    "Return air temperature"
    annotation(Placement(transformation(extent={{-60,-78}, {-40,-58}})));
  BoundaryConditions.WeatherData.ReaderTMY3 weaDat(pAtmSou=Buildings.BoundaryConditions.Types.DataSource.Parameter, filNam=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/weatherdata/USA_IL_Chicago-OHare.Intl.AP.725300_TMY3.mos"))
    "Weather data reader"
...
equation
  connect(TSupSet.y, cooModCon.TSupSet)
    annotation(Line(points={{-39, 70}, {-20, 70}, {-20, 5}, {-12, 5}}, color={0, 0, 127}));
  connect(TRet.y, cooModCon.TRet)
    annotation(Line(points={{-39,-68}, {-20,-68}, {-20,-5}, {-12,-5}}, color={0, 0, 127}));
  connect(weaDat.weaBus, weaBus)
    annotation(Line(points={{-60, 0}, {-60, 0}, {-48, 0}}, color={255, 204, 51}, thickness=0.5));
  connect(weaBus.TDryBul, cooModCon.TOutDryBul)
    annotation(Line(points={{-48, 0}, {-20, 0}, {-12, 0}}, color={255, 204, 51}, thickness=0.5), Text(textString="%first", index=-1, extent={{-6, 3}, {-6, 3}}));
  annotation(__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/DXCooled/Controls/Validation/CoolingMode.mos"
...
</html>"), experiment(StartTime=8928000, StopTime=9000000, Tolerance=1e-06));
end CoolingMode;
