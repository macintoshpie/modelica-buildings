...
model DXCooledAirsideEconomizer
  ...
  extends Buildings.Applications.DataCenters.ChillerCooled.Examples.BaseClasses.PostProcess(
    fulMecCooSig(
      y=if cooModCon.y == Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FullMechanical) then 1 else 0),
    parMecCooSig(
      y=if cooModCon.y == Integer(Buildings.Applications.DataCenters.Types.CoolingModes.PartialMechanical) then 1 else 0),
    PHVAC(
      y=varSpeDX.P + fan.P),
    PIT(
      y=roo.QRoo_flow),
    freCooSig(
      y=if cooModCon.y == Integer(Buildings.Applications.DataCenters.Types.CoolingModes.FreeCooling) then 1 else 0));
  ...
  Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-280, 60}, {-260, 80}})));
  Buildings.BoundaryConditions.WeatherData.Bus weaBus
    "Weather data bus"
    annotation(
      Placement(
        transformation(
          extent={{-250, 60}, {-230, 80}}),
        iconTransformation(
          extent={{-250, 60}, {-230, 80}})));
  Buildings.Fluid.Movers.SpeedControlled_y fan(
    ...
    per(
      pressure(
        V_flow=mA_flow_nominal*{0, 1, 2}/1.2,
        dp=500*{2, 1, 0})),
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=90,
          origin={120,-94})));
  Modelica.Blocks.Sources.Constant TRooAirSet(
    k=TRooSet)
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort senTemSupAir(
    ...
    annotation(
      Placement(
        transformation(
          extent={{30,-70}, {50,-50}})));
  Modelica.Blocks.Sources.Constant SATSetPoi(
    k=TAirSupSet)
    "Supply air temperature set point"
    annotation(
      Placement(
        transformation(
          extent={{-240, 90}, {-220, 110}})));
  Buildings.Fluid.HeatExchangers.DXCoils.AirCooled.Data.Generic.DXCoil datCoi(
    ...
    sta={Buildings.Fluid.HeatExchangers.DXCoils.AirCooled.Data.Generic.BaseClasses.Stage(
      ...
      nomVal=Buildings.Fluid.HeatExchangers.DXCoils.AirCooled.Data.Generic.BaseClasses.NominalValues(
        ...
        TEvaIn_nominal=TRooSet,
        m_flow_nominal=mA_flow_nominal),
      perCur=Buildings.Fluid.HeatExchangers.DXCoils.AirCooled.Examples.PerformanceCurves.Curve_III())})
    "Coil data"
    annotation(
      Placement(
        transformation(
...
equation
  ...
  connect(dam1.port_b, senTemSupAir.port_a)
    annotation(
      Line(
        points={{0, 12}, {20, 12}, {20,-60}, {30,-60}},
        color={0, 127, 255}));
  connect(SATSetPoi.y, cooModCon.TSupSet)
    annotation(
      Line(
        points={{-219, 100}, {-180, 100}, {-180, 75}, {-172, 75}},
        color={0, 0, 127}));
  connect(eco.port_Sup, senTemMixAir.port_a)
    annotation(
      Line(
        points={{-140, 12}, {-130, 12}},
        color={0, 127, 255}));
  ...
  connect(senTemMixAir.port_b, dam2.port_a)
    annotation(
      Line(
        points={{-110, 12}, {-100, 12}, {-100,-60}, {-90,-60}},
        color={0, 127, 255}));
  connect(SATSetPoi.y, ecoCon.TMixAirSet)
    annotation(
      Line(
        points={{-219, 100}, {-134, 100}, {-134, 86}, {-122, 86}},
        color={0, 0, 127}));
  connect(senTemMixAir.T, ecoCon.TMixAirMea)
    annotation(
      Line(
        points={{-120, 23}, {-120, 54}, {-134, 54}, {-134, 80}, {-122, 80}},
        color={0, 0, 127}));
  connect(ecoCon.y, eco.y)
    annotation(
      Line(
        points={{-99, 80}, {-92, 80}, {-92, 40}, {-150, 40}, {-150, 18}},
        color={0, 0, 127}));
  connect(feedback1.y, dam2.y)
    annotation(
      Line(
        points={{-21, 130}, {0, 130}, {0, 40}, {-80, 40}, {-80,-48}},
        color={0, 0, 127}));
  connect(out.ports[1], eco.port_Out)
    annotation(
      Line(
        points={{-200, 12}, {-160, 12}},
        color={0, 127, 255}));
  connect(eco.port_Exh, out.ports[2])
    annotation(
      Line(
        points={{-160, 0}, {-172, 0}, {-172, 8}, {-200, 8}},
        color={0, 127, 255}));
  ...
  connect(fan.port_b, roo.airPorts[1])
    annotation(
      Line(
        points={{120,-104}, {120,-132}, {52.475,-132}, {52.475,-118.7}},
        color={0, 127, 255}));
  connect(eco.port_Ret, roo.airPorts[2])
    annotation(
      Line(
        points={{-140, 0}, {-130, 0}, {-130,-132}, {48.425,-132}, {48.425,-118.7}},
        color={0, 127, 255}));
  connect(const.y, feedback1.u1)
    annotation(
      Line(
        points={{-59, 130}, {-38, 130}},
        color={0, 0, 127}));
  connect(cooModCon.y, ecoCon.cooMod)
    annotation(
      Line(
        points={{-149, 70}, {-142, 70}, {-142, 74}, {-122, 74}},
        color={255, 127, 0}));
  connect(TRooAirSet.y, fanSpe.u_s)
    annotation(
      Line(
        points={{61, 0}, {68, 0}, {68,-32}, {78,-32}},
        color={0, 0, 127}));
  connect(roo.TRooAir, fanSpe.u_m)
    annotation(
      Line(
        points={{61,-110}, {90,-110}, {90,-44}},
        color={0, 0, 127}));
  connect(fanSpe.y, fan.y)
    annotation(
      Line(
        points={{101,-32}, {150,-32}, {150,-94}, {132,-94}},
        color={0, 0, 127}));
  connect(roo.TRooAir, cooModCon.TRet)
    annotation(
      Line(
        points={{61,-110}, {90,-110}, {90,-88}, {-180,-88}, {-180, 65}, {-172, 65}},
        color={0, 0, 127}));
  ...
  annotation(
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Applications/DataCenters/DXCooled/Examples/DXCooledAirsideEconomizer.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
