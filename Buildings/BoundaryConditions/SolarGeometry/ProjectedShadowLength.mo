...
block ProjectedShadowLength
  ...
  parameter Modelica.SIunits.Angle lon(
    displayUnit="deg")=Buildings.BoundaryConditions.WeatherData.BaseClasses.getLongitudeTMY3(absFilNam)
    "Longitude"
    annotation(
      Evaluate=true,
      Dialog(
  ...
  parameter Modelica.SIunits.Angle lat(
    displayUnit="deg")=Buildings.BoundaryConditions.WeatherData.BaseClasses.getLatitudeTMY3(absFilNam)
    "Latitude"
    annotation(
      Evaluate=true,
      Dialog(
  ...
  parameter Modelica.SIunits.Time timZon(
    displayUnit="h")=Buildings.BoundaryConditions.WeatherData.BaseClasses.getTimeZoneTMY3(absFilNam)
    "Time zone"
    annotation(
      Evaluate=true,
      Dialog(
...
protected
  ...
  Modelica.Blocks.Math.Tan tan
    ...
    annotation(
      Placement(
        transformation(
          extent={{-30,-90}, {-10,-70}})));
  Modelica.Blocks.Math.Gain shaLen(
    k=-h)
    "Length of shadow"
    annotation(
      Placement(
  ...
  BaseClasses.SolarAzimuth solAzi(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-30,-60}, {-10,-40}})));
  Modelica.Blocks.Sources.Constant surAzi(
    final k=azi)
    "Surface azimuth"
    annotation(
      Placement(
  ...
  Modelica.Blocks.Sources.RealExpression pShaLen(
    ...
    annotation(
      Placement(
        transformation(
          extent={{102,-10}, {122, 10}})));
equation
  connect(tan.u, zen.zen)
    annotation(
      Line(
        points={{-32,-80}, {-32,-80}, {-49,-80}},
        color={0, 0, 127}));
  connect(tan.y, shaLen.u)
    annotation(
      Line(
        points={{-9,-80}, {-9,-80}, {-2,-80}},
        color={0, 0, 127}));
  connect(solAzi.zen, zen.zen)
    annotation(
      Line(
        points={{-32,-44}, {-40,-44}, {-40,-80}, {-49,-80}},
        color={0, 0, 127}));
  connect(add.u1, surAzi.y)
    annotation(
      Line(
        points={{8,-24}, {0,-24}, {0,-10}, {-9,-10}},
        color={0, 0, 127}));
  connect(solAzi.solAzi, add.u2)
    annotation(
      Line(
        points={{-9,-50}, {0,-50}, {0,-36}, {8,-36}},
        color={0, 0, 127}));
  connect(add.y, cos.u)
    annotation(
      Line(
  ...
  connect(shaLen.y, proShaLen.u2)
    annotation(
      Line(
        points={{21,-80}, {40,-80}, {40,-86}, {68,-86}},
        color={0, 0, 127}));
  connect(proShaLen.u1, cos.y)
    annotation(
      Line(
        points={{68,-74}, {64,-74}, {64,-60}, {64,-60}, {64,-30}, {61,-30}},
        color={0, 0, 127}));
  connect(solHouAng.solHouAng, zen.solHouAng)
    annotation(
      Line(
        points={{81, 120}, {100, 120}, {100, 80}, {-80, 80}, {-80,-84}, {-72,-84}, {-72,-84.8}},
        color={0, 0, 127}));
  ...
  connect(eqnTim.eqnTim, solTim.equTim)
    annotation(
      Line(
        points={{1, 140}, {12, 140}, {12, 126}, {18, 126}},
        color={0, 0, 127}));
  connect(locTim.locTim, solTim.locTim)
    annotation(
      Line(
        points={{1, 100}, {12, 100}, {12, 114.6}, {18, 114.6}},
        color={0, 0, 127}));
  connect(solHouAng.solTim, solTim.solTim)
    annotation(
      Line(
        points={{58, 120}, {56, 120}, {41, 120}},
        color={0, 0, 127}));
  connect(decAng.nDay, modTim.y)
    annotation(
      Line(
        points={{-122,-50}, {-128,-50}, {-128, 60}, {-128, 0}, {-139, 0}},
        color={0, 0, 127}));
  connect(zen.decAng, decAng.decAng)
    annotation(
      Line(
        points={{-72,-74.6}, {-88,-74.6}, {-88,-50}, {-99,-50}},
        color={0, 0, 127}));
  connect(solAzi.decAng, decAng.decAng)
    annotation(
      Line(
        points={{-32,-50}, {-88,-50}, {-99,-50}},
        color={0, 0, 127}));
  connect(solAzi.solTim, solTim.solTim)
    annotation(
      Line(
        points={{-32,-56}, {-42,-56}, {-74,-56}, {-74, 72}, {48, 72}, {48, 120}, {41, 120}},
        color={0, 0, 127}));
  connect(pShaLen.y, y)
    annotation(
      Line(
