...
protected
  Buildings.BoundaryConditions.SolarIrradiation.BaseClasses.DiffuseIsotropic HDifTilIso(til=til, rho=rho)
    "Diffuse isotropic irradiation on tilted surface"
    annotation(Placement(transformation(extent={{-20,-10}, {0, 10}})));
equation
  connect(weaBus.HGloHor, HDifTilIso.HGloHor)
    annotation(Line(points={{-100, 5.55112e-16}, {-51.5, 5.55112e-16}, {-51.5, 4}, {-22, 4}}, color={255, 204, 51}, thickness=0.5), Text(textString="%first", index=-1, extent={{-6, 3}, {-6, 3}}));
  connect(weaBus.HDifHor, HDifTilIso.HDifHor)
    annotation(Line(points={{-100, 5.55112e-16}, {-51.5, 5.55112e-16}, {-51.5,-4}, {-22,-4}}, color={255, 204, 51}, thickness=0.5), Text(textString="%first", index=-1, extent={{-6, 3}, {-6, 3}}));
  connect(HDifTilIso.HSkyDifTil, add.u1)
    annotation(Line(points={{1, 4}, {24, 4}, {24, 6}, {38, 6}}, color={0, 0, 127}));
  connect(HDifTilIso.HGroDifTil, add.u2)
    annotation(Line(points={{1,-4}, {24,-4}, {24,-6}, {38,-6}}, color={0, 0, 127}));
  connect(add.y, H)
    annotation(Line(points={{61, 6.10623e-16}, {81.5, 6.10623e-16}, {81.5, 5.55112e-16}, {110, 5.55112e-16}}, color={0, 0, 127}));
  connect(HDifTilIso.HSkyDifTil, HSkyDifTil)
    annotation(Line(points={{1, 4}, {14, 4}, {14, 60}, {110, 60}}, color={0, 0, 127}));
  connect(HDifTilIso.HGroDifTil, HGroDifTil)
    annotation(Line(points={{1,-4}, {14,-4}, {14,-60}, {110,-60}}, color={0, 0, 127}));
  annotation(defaultComponentName="HDifTilIso", Documentation(info="<html>
<p>
