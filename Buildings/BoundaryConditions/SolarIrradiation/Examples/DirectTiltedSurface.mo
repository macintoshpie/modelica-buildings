...
model DirectTiltedSurface
  ...
  Buildings.BoundaryConditions.SolarIrradiation.DirectTiltedSurface HDirRoo(til=Buildings.Types.Tilt.Ceiling, lat=0.6457718232379, azi=0.78539816339745)
    "Direct irradiation on roof"
    annotation(Placement(transformation(extent={{20, 20}, {40, 40}})));
  Buildings.BoundaryConditions.SolarIrradiation.DirectTiltedSurface HDirFlo(til=Buildings.Types.Tilt.Floor, lat=0.6457718232379, azi=0.78539816339745)
    "Direct irradiation on floor"
    annotation(Placement(transformation(extent={{20,-60}, {40,-40}})));
  Buildings.BoundaryConditions.SolarIrradiation.DirectTiltedSurface HDirWal(til=Buildings.Types.Tilt.Wall, lat=0.6457718232379, azi=0.78539816339745)
...
equation
  ...
  connect(const.y, assEqu.u2)
    annotation(Line(points={{-19,-62}, {58,-62}}, color={0, 0, 127}));
  connect(weaDat.weaBus, HDirRoo.weaBus)
    annotation(Line(points={{-40, 30}, {20, 30}}, color={255, 204, 51}, thickness=0.5));
  connect(HDirWal.weaBus, weaDat.weaBus)
    annotation(Line(points={{20,-10}, {-10,-10}, {-10, 30}, {-40, 30}}, color={255, 204, 51}, thickness=0.5));
  connect(HDirFlo.weaBus, weaDat.weaBus)
    annotation(Line(points={{20,-50}, {-10,-50}, {-10, 30}, {-40, 30}}, color={255, 204, 51}, thickness=0.5));
  annotation(experiment(StartTime=1.82304e+07, Tolerance=1e-6, StopTime=1.83168e+07), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/BoundaryConditions/SolarIrradiation/Examples/DirectTiltedSurface.mos"
