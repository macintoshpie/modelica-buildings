...
model CheckTemperature
  ...
  Buildings.BoundaryConditions.WeatherData.BaseClasses.CheckDryBulbTemperature cheTemDryBul
  ...
  Buildings.BoundaryConditions.WeatherData.BaseClasses.ConvertTime conTim(weaDatStaTim=0, weaDatEndTim=31536000)
...
protected
  Modelica.Blocks.Tables.CombiTable1Ds datRea(tableOnFile=true, tableName="tab1", fileName=Modelica.Utilities.Files.loadResource(Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/weatherdata/USA_IL_Chicago-OHare.Intl.AP.725300_TMY3.mos")), columns=2 : 30, smoothness=Modelica.Blocks.Types.Smoothness.ContinuousDerivative)
    "Data reader"
...
equation
  ...
  connect(datRea.y[1], from_degC.u)
    annotation(Line(points={{1, 10}, {10, 10}, {10, 30}, {17.8, 30}}, color={0, 0, 127}));
  connect(from_degC.y, cheTemDryBul.TIn)
    annotation(Line(points={{43.1, 30}, {58, 30}}, color={0, 0, 127}));
  connect(datRea.y[2], from_degC1.u)
    annotation(Line(points={{1, 10}, {10, 10}, {10,-10}, {17.8,-10}}, color={0, 0, 127}));
  connect(from_degC1.y, cheTemDewPoi.TIn)
    annotation(Line(points={{43.1,-10}, {58,-10}}, color={0, 0, 127}));
...
</html>"), experiment(Tolerance=1e-6, StartTime=0, StopTime=8640000), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/BoundaryConditions/WeatherData/BaseClasses/Examples/CheckTemperature.mos"
    "Simulate and plot"));
