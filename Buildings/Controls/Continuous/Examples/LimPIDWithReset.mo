...
protected
  model Plant
    ...
    Modelica.Thermal.HeatTransfer.Components.HeatCapacitor cap(C=10, T(fixed=true, start=293.15))
      "Heat capacitor"
      annotation(Placement(transformation(extent={{-38, 0}, {-18, 20}})));
    Modelica.Thermal.HeatTransfer.Components.HeatCapacitor cap1(C=10, T(fixed=true, start=293.15))
      "Heat capacitor"
  ...
  equation
    ...
    connect(cap.port, theCon1.port_a)
      annotation(Line(points={{-28, 0}, {-28, 0}, {-10, 0}}, color={191, 0, 0}));
    ...
    connect(theCon.port_b, cap.port)
      annotation(Line(points={{-30,-40}, {-30,-40}, {-20,-40}, {-20, 0}, {-28, 0}}, color={191, 0, 0}));
...
First implementation.
  ...
  model Controller
    ...
    Modelica.Blocks.Interfaces.RealInput u_m(unit="K")
      ...
      annotation(Placement(transformation(rotation=90, extent={{-10,-10}, {10, 10}}, origin={0,-110}), iconTransformation(extent={{-10,-10}, {10, 10}}, rotation=90, origin={0,-110})));
    Modelica.Blocks.Interfaces.RealOutput y
      "Control signal"
      annotation(Placement(transformation(extent={{100,-10}, {120, 10}}), iconTransformation(extent={{100,-10}, {120, 10}})));
    Buildings.Controls.Continuous.LimPID conPID(final reset=reset, yMax=1, yMin=0, controllerType=Modelica.Blocks.Types.SimpleController.PI, Ti=1, k=10)
      "PI controller"
      annotation(Placement(transformation(extent={{-10,-10}, {10, 10}})));
  ...
  equation
    connect(conPID.y, gain.u)
      annotation(Line(points={{11, 0}, {11, 0}, {28, 0}}, color={0, 0, 127}));
    connect(trigger.y, conPID.trigger)
      annotation(Line(points={{-19,-30}, {-8,-30}, {-8,-12}}, color={255, 0, 255}));
    connect(u_m, conPID.u_m)
      annotation(Line(points={{0,-110}, {0,-12}}, color={0, 0, 127}));
    connect(conPID.u_s, TSet)
      annotation(Line(points={{-12, 0}, {-70, 0}, {-120, 0}}, color={0, 0, 127}));
    connect(trigger.u, TSet)
      annotation(Line(points={{-42,-30}, {-70,-30}, {-70, 0}, {-120, 0}}, color={0, 0, 127}));
    connect(gain.y, y)
...
equation
  connect(plaWitRes.T, conWitRes.u_m)
    annotation(Line(points={{41, 50}, {60, 50}, {60, 20}, {-30, 20}, {-30, 39}}, color={0, 0, 127}));
  connect(conWitRes.y, plaWitRes.Q_flow)
    annotation(Line(points={{-19, 50}, {19, 50}}, color={0, 0, 127}));
  connect(plaNoRes.T, conNoRes.u_m)
    annotation(Line(points={{41,-10}, {60,-10}, {60,-40}, {-30,-40}, {-30,-21}}, color={0, 0, 127}));
  connect(conNoRes.y, plaNoRes.Q_flow)
    annotation(Line(points={{-19,-10}, {19,-10}}, color={0, 0, 127}));
  connect(TSet.y, conWitRes.TSet)
    annotation(Line(points={{-59, 0}, {-50, 0}, {-50, 50}, {-42, 50}}, color={0, 0, 127}));
  connect(TSet.y, conNoRes.TSet)
    annotation(Line(points={{-59, 0}, {-50, 0}, {-50,-10}, {-42,-10}}, color={0, 0, 127}));
  annotation(experiment(Tolerance=1e-06, StopTime=600), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Controls/Continuous/Examples/LimPIDWithReset.mos"
