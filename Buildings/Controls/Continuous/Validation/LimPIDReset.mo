...
model LimPIDReset
  ...
  Modelica.Blocks.Sources.Sine setPoi(
    freqHz=1)
    "Set point signal"
    annotation(
      Placement(
  ...
  Buildings.Controls.Continuous.LimPID limPIDPar(
    ...
    annotation(
      Placement(
        transformation(
          extent={{20,-20}, {40, 0}})));
  Modelica.Blocks.Sources.Constant mea(
    k=0.5)
    "Measured signal"
    annotation(
      Placement(
  ...
  Buildings.Controls.Continuous.LimPID limPIDInp(
    ...
    annotation(
      Placement(
        transformation(
          extent={{20,-80}, {40,-60}})));
  Modelica.Blocks.Sources.Constant conRes(
    k=0.9)
...
equation
  connect(setPoi.y, limPIDPar.u_s)
    annotation(
      Line(
        points={{-19,-10}, {0,-10}, {18,-10}},
        color={0, 0, 127}));
  connect(mea.y, limPIDPar.u_m)
    annotation(
      Line(
        points={{-19,-40}, {30,-40}, {30,-22}},
        color={0, 0, 127}));
  connect(setPoi.y, limPIDDef.u_s)
    annotation(
      Line(
        points={{-19,-10}, {-6,-10}, {-6, 30}, {18, 30}},
        color={0, 0, 127}));
  connect(mea.y, limPIDDef.u_m)
    annotation(
      Line(
        points={{-19,-40}, {-12,-40}, {-12, 10}, {30, 10}, {30, 18}},
        color={0, 0, 127}));
  connect(trigger.y, limPIDPar.trigger)
    annotation(
      Line(
        points={{-19, 20}, {-10, 20}, {8, 20}, {8,-30}, {22,-30}, {22,-22}},
        color={255, 0, 255}));
  connect(setPoi.y, limPIDInp.u_s)
    annotation(
      Line(
        points={{-19,-10}, {-6,-10}, {-6,-70}, {18,-70}},
        color={0, 0, 127}));
  connect(mea.y, limPIDInp.u_m)
    annotation(
      Line(
        points={{-19,-40}, {-12,-40}, {-12,-90}, {30,-90}, {30,-88}, {30,-82}},
        color={0, 0, 127}));
  connect(trigger.y, limPIDInp.trigger)
    annotation(
      Line(
        points={{-19, 20}, {-19, 20}, {8, 20}, {8,-88}, {22,-88}, {22,-82}},
        color={255, 0, 255}));
  connect(conRes.y, limPIDInp.y_reset_in)
    annotation(
      Line(
        points={{-19,-80}, {0,-80}, {0,-78}, {18,-78}},
        color={0, 0, 127}));
  ...
  connect(mea.y, limPIDOri.u_m)
    annotation(
      Line(
        points={{-19,-40}, {-12,-40}, {-12, 50}, {30, 50}, {30, 58}},
        color={0, 0, 127}));
  connect(assEqu.u1, limPIDOri.y)
    annotation(
      Line(
        points={{58, 56}, {48, 56}, {48, 70}, {41, 70}},
        color={0, 0, 127}));
  connect(limPIDDef.y, assEqu.u2)
    annotation(
      Line(
        points={{41, 30}, {48, 30}, {48, 44}, {58, 44}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=1.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Controls/Continuous/Validation/LimPIDReset.mos"
        "Simulate and plot"),
    Documentation(
      revisions="<html>
<ul>
