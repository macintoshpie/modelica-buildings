...
protected
  Buildings.Controls.Predictors.ElectricalLoad basLin(final nSam=nSam, final nPre=nPre, final nHis=nHis, final predictionModel=predictionModel)
...
equation
  connect(basLin.ECon, ECon)
    annotation(Line(points={{-12, 0}, {-62, 0}, {-62,-30}, {-120,-30}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TOut, basLin.TOut)
    annotation(Line(points={{-120,-60}, {-40,-60}, {-40,-6}, {-12,-6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(basLin.PPre, PPre)
    annotation(Line(points={{11, 0}, {40, 0}, {40,-80}, {110,-80}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(stoHis.u, isEventDay)
    annotation(Line(points={{-90, 6}, {-96, 6}, {-96, 40}, {-110, 40}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(stoHis.y, basLin.storeHistory)
    annotation(Line(points={{-67, 6}, {-40, 6}, {-40, 5}, {-12, 5}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(basLin.typeOfDay, typeOfDay)
    annotation(Line(points={{-12, 10}, {-60, 10}, {-60, 80}, {-110, 80}}, color={0, 127, 0}, smooth=Smooth.None));
  connect(basLin.TOutFut, TOutFut)
    annotation(Line(points={{-12,-10}, {-36,-10}, {-36,-90}, {-120,-90}}, color={0, 0, 127}, smooth=Smooth.None));
