...
model Client
  ...
  parameter Buildings.Controls.Predictors.Types.PredictionModel predictionModel=Buildings.Controls.Predictors.Types.PredictionModel.WeatherRegression
  ...
  Modelica.Blocks.Interfaces.BooleanInput shed
    ...
    annotation(Placement(transformation(extent={{-120,-40}, {-100,-20}}), iconTransformation(extent={{-120,-40}, {-100,-20}})));
  ...
  Modelica.Blocks.Interfaces.RealInput TOut(unit="K", displayUnit="degC") if(predictionModel == Buildings.Controls.Predictors.Types.PredictionModel.WeatherRegression)
    "Outside air temperature"
    annotation(Placement(transformation(extent={{-120,-80}, {-100,-60}}), iconTransformation(extent={{-120,-80}, {-100,-60}})));
  Modelica.Blocks.Interfaces.RealInput TOutFut[nPre-1](each unit="K") if(predictionModel == Buildings.Controls.Predictors.Types.PredictionModel.WeatherRegression)
    ...
    annotation(Placement(transformation(extent={{-120,-100}, {-100,-80}}), iconTransformation(extent={{-120,-100}, {-100,-80}})));
...
protected
  ...
  BaseClasses.BaselinePrediction comBasLin(final nSam=nSam, final nHis=nHis, final nPre=nPre, final predictionModel=predictionModel, nIn=3)
    "Baseline prediction"
  ...
  Modelica.StateGraph.TransitionWithSignal t2(enableTimer=false)
    "State transition"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, rotation=270, origin={10, 20})));
  Modelica.StateGraph.TransitionWithSignal t3
    "State transition"
    annotation(Placement(transformation(extent={{10, 10}, {-10,-10}}, origin={-2,-30})));
  ...
  Modelica.StateGraph.TransitionWithSignal t4(enableTimer=false)
    "State transition"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, rotation=270, origin={-40, 10})));
...
equation
  connect(initialStep.outPort[1], transition.inPort)
    annotation(Line(points={{-59.5, 80}, {-34, 80}}, color={0, 0, 0}, smooth=Smooth.None));
  connect(transition.outPort, comBasLin.inPort[1])
    annotation(Line(points={{-28.5, 80}, {6, 80}, {6, 50.5}, {19, 50.5}, {19, 50.6667}}, color={0, 0, 0}, smooth=Smooth.None));
  connect(t1.inPort, comBasLin.outPort[1])
    annotation(Line(points={{60, 24}, {60, 50}, {40.5, 50}}, color={0, 0, 0}, smooth=Smooth.None));
  connect(t1.outPort, norOpe.inPort[1])
    annotation(Line(points={{60, 18.5}, {60,-30}, {41,-30}}, color={0, 0, 0}, smooth=Smooth.None));
  connect(comBasLin.ECon, ECon)
    annotation(Line(points={{19, 48}, {-64, 48}, {-64, 36}, {-94, 36}, {-94, 4.44089e-16}, {-110, 4.44089e-16}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(comBasLin.PPre[1], norOpe.PCon)
    annotation(Line(points={{41, 42}, {80, 42}, {80,-38}, {42,-38}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(t2.outPort, comBasLin.inPort[2])
    annotation(Line(points={{10, 21.5}, {10, 50}, {19, 50}}, color={0, 0, 0}, smooth=Smooth.None));
  connect(t3.outPort, she.inPort[1])
    annotation(Line(points={{-3.5,-30}, {-9,-30}}, color={0, 0, 0}, smooth=Smooth.None));
  connect(t4.inPort, she.outPort[1])
    annotation(Line(points={{-40, 6}, {-40,-30}, {-30.5,-30}}, color={0, 0, 0}, smooth=Smooth.None));
  connect(t4.outPort, comBasLin.inPort[3])
    annotation(Line(points={{-40, 11.5}, {-40, 50}, {-20, 50}, {-20, 49.3333}, {19, 49.3333}}, color={0, 0, 0}, smooth=Smooth.None));
  connect(t4.condition, tri.y)
    annotation(Line(points={{-52, 10}, {-66, 10}, {-66, 20}, {-69, 20}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(t2.condition, tri.y)
    annotation(Line(points={{-2, 20}, {-69, 20}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(norOpe.PPre, she.PCon)
    annotation(Line(points={{19,-38}, {6,-38}, {6,-39}, {-8,-39}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(norOpe.active, switch.u2)
    annotation(Line(points={{30,-41}, {30,-70}, {38,-70}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(norOpe.outPort[1], t2.inPort)
    annotation(Line(points={{19.5,-29.75}, {10,-29.75}, {10, 16}}, color={0, 0, 0}, smooth=Smooth.None));
  connect(norOpe.outPort[2], t3.inPort)
    annotation(Line(points={{19.5,-30.25}, {12,-30.25}, {12,-30}, {2,-30}}, color={0, 0, 0}, smooth=Smooth.None));
  connect(shed, t3.condition)
    annotation(Line(points={{-110,-30}, {-80,-30}, {-80,-10}, {-2,-10}, {-2,-18}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(switch.u1, she.PCon)
    annotation(Line(points={{38,-62}, {10,-62}, {10,-39}, {-8,-39}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(switch.u3, she.PPre)
    annotation(Line(points={{38,-78}, {-40,-78}, {-40,-38}, {-31,-38}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(isEventDay, comBasLin.isEventDay)
    annotation(Line(points={{-110, 40}, {-90, 40}, {-90, 54}, {19, 54}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(comBasLin.TOut, TOut)
    annotation(Line(points={{19, 45}, {-60, 45}, {-60,-70}, {-110,-70}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(typeOfDay, comBasLin.typeOfDay)
    annotation(Line(points={{-110, 80}, {-90, 80}, {-90, 58}, {19, 58}}, color={0, 127, 0}, smooth=Smooth.None));
  connect(comBasLin.TOutFut, TOutFut)
    annotation(Line(points={{19, 41}, {-56, 41}, {-56,-90}, {-110,-90}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(yShed, she.yShed)
    annotation(Line(points={{-110,-50}, {-48,-50}, {-48,-14}, {-6,-14}, {-6,-32}, {-9,-32}}, color={0, 0, 127}, smooth=Smooth.None));
  // Only PPre[1] will take into account the shedded load.
  connect(switch.y, PPre)
    annotation(Line(points={{61,-70}, {90,-70}, {90, 0}, {110, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(comBasLin.PPre, PPreNoShe)
    annotation(Line(points={{41, 42}, {80, 42}, {80, 50}, {110, 50}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(Icon(graphics={Text(extent={{-70, 54}, {74,-64}}, lineColor={0, 0, 255}, textString="DR")}), Documentation(info="<html>
<p>
