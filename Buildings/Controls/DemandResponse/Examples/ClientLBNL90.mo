...
model ClientLBNL90
  ...
  Modelica.Blocks.Sources.CombiTimeTable bui90(
    ...
    fileName=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/Data/Controls/DemandResponse/Examples/B90_DR_Data.mos"),
    columns={2, 3, 4})
    ...
    annotation(
      Placement(
        transformation(
          extent={{-90,-20}, {-70, 0}})));
  Modelica.Blocks.Logical.GreaterThreshold drSig(
    threshold=0.5)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-50, 14}, {-30, 34}})));
  Modelica.Blocks.Math.Add errorAverage(
    k2=-1)
  ...
  Client clientWeather(
    ...
    annotation(
      Placement(
        transformation(
          extent={{10,-40}, {30,-20}})));
  Modelica.Blocks.Math.Add errorWeather(
    k2=-1)
    "Difference between predicted minus actual load"
    annotation(
      Placement(
        transformation(
          extent={{42,-74}, {62,-54}})));
  Modelica.Blocks.Math.Gain relErrAverage(
    k=1/400000)
    ...
    annotation(
      Placement(
        transformation(
          extent={{72,-10}, {92, 10}})));
  Modelica.Blocks.Math.Gain relErrWeather(
    k=1/400000)
    "Relative error, normalized by a value that is close to the peak power consumption"
    annotation(
      Placement(
        transformation(
          extent={{74,-74}, {94,-54}})));
  Modelica.Blocks.Continuous.Integrator ene(
    u(
      unit="W"))
    ...
    annotation(
      Placement(
        transformation(
          extent={{-48,-40}, {-28,-20}})));
  Modelica.Blocks.Sources.Constant yShed(
    k=0.5)
...
equation
  ...
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Controls/DemandResponse/Examples/ClientLBNL90.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=1.728e+06),
