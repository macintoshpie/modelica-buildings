...
block ReturnFanDirectPressure
  ...
  parameter Real dpBuiSet(final unit="Pa", final quantity="PressureDifference", final max=30)=12
    "Building static pressure difference relative to ambient (positive to pressurize the building)";
  parameter Real dpDisMin(final unit="Pa", final quantity="PressureDifference", final min=0, final max=1000)=2.4
    "Minimum return fan discharge static pressure difference setpoint";
  parameter Real dpDisMax(final unit="Pa", final quantity="PressureDifference", final min=0, final max=1000)=40
    "Maximum return fan discharge static pressure setpoint";
  parameter Real k(final unit="1")=1
    "Gain, normalized using dpBuiSet";
  Buildings.Controls.OBC.CDL.Interfaces.RealInput dpBui(final unit="Pa", displayUnit="Pa")
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput uFan
    ...
    annotation(Placement(transformation(extent={{-180,-110}, {-140,-70}}), iconTransformation(extent={{-140,-80}, {-100,-40}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput dpDisSet(final unit="Pa", displayUnit="Pa", min=0)
    "Return fan discharge static pressure setpoint"
    annotation(Placement(transformation(extent={{120,-110}, {160,-70}}), iconTransformation(extent={{100,-80}, {140,-40}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput yExhDam(final unit="1", min=0, max=1)
    "Exhaust damper control signal (0: closed, 1: open)"
    annotation(Placement(transformation(extent={{120, 0}, {160, 40}}), iconTransformation(extent={{100, 40}, {140, 80}})));
  ...
  Buildings.Controls.OBC.CDL.Continuous.LimPID conP(final controllerType=Buildings.Controls.OBC.CDL.Types.SimpleController.P, final k=k, reverseAction=true)
...
protected
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant dpBuiSetPoi(final k=dpBuiSet)
    "Building pressure setpoint"
    annotation(Placement(transformation(extent={{-130, 40}, {-110, 60}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant retFanDisPreMin(final k=dpDisMin)
    "Return fan discharge static pressure minimum setpoint"
    annotation(Placement(transformation(extent={{0,-30}, {20,-10}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant retFanDisPreMax(final k=dpDisMax)
...
equation
  ...
  connect(retFanDisPreMin.y, linRetFanStaPre.f1)
    annotation(Line(points={{22,-20}, {30,-20}, {30,-36}, {58,-36}}, color={0, 0, 127}));
  connect(retFanDisPreMax.y, linRetFanStaPre.f2)
    annotation(Line(points={{22,-70}, {30,-70}, {30,-48}, {58,-48}}, color={0, 0, 127}));
  connect(linRetFanStaPre.y, swi.u1)
    annotation(Line(points={{82,-40}, {100,-40}, {100,-60}, {60,-60}, {60,-82}, {78,-82}}, color={0, 0, 127}));
  connect(uFan, swi1.u2)
    annotation(Line(points={{-160,-90}, {-100,-90}, {-100, 20}, {78, 20}}, color={255, 0, 255}));
  connect(linExhAirDam.y, swi1.u1)
    annotation(Line(points={{82, 110}, {100, 110}, {100, 70}, {60, 70}, {60, 28}, {78, 28}}, color={0, 0, 127}));
  connect(swi1.y, yExhDam)
    annotation(Line(points={{102, 20}, {140, 20}}, color={0, 0, 127}));
  connect(zer1.y, swi1.u3)
    annotation(Line(points={{-18, 52}, {30, 52}, {30, 12}, {78, 12}}, color={0, 0, 127}));
  ...
  connect(dpBuiSetPoi.y, div.u2)
    annotation(Line(points={{-108, 50}, {-90, 50}, {-90, 74}, {-82, 74}}, color={0, 0, 127}));
  connect(movMea.y, div.u1)
    annotation(Line(points={{-108, 80}, {-100, 80}, {-100, 86}, {-82, 86}}, color={0, 0, 127}));
  ...
  annotation(defaultComponentName="buiPreCon", Icon(graphics={Text(extent={{-98, 142}, {102, 102}}, lineColor={0, 0, 255}, textString="%name"), Rectangle(extent={{-100,-100}, {100, 100}}, lineColor={0, 0, 127}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Line(points={{-78, 52}, {-78,-32}, {78,-32}, {78, 52}, {78, 52}}, color={28, 108, 200}), Line(points={{-78,-32}, {4, 52}}, color={0, 0, 0}, thickness=0.5), Line(points={{4,-32}, {78, 52}}, color={0, 0, 0}, pattern=LinePattern.Dash, thickness=0.5), Text(extent={{-50,-36}, {60,-62}}, lineColor={28, 108, 200}, textString="Building pressure control loop signal")}), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-140,-140}, {120, 140}}), graphics={Rectangle(extent={{-138, 138}, {118, 4}}, lineColor={0, 0, 0}, fillColor={215, 215, 215}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Rectangle(extent={{-138,-4}, {118,-138}}, lineColor={0, 0, 0}, fillColor={215, 215, 215}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Text(extent={{-138, 140}, {-62, 122}}, lineColor={0, 0, 255}, fillColor={215, 215, 215}, fillPattern=FillPattern.Solid, horizontalAlignment=TextAlignment.Left, textString="Exhaust air damper control"), Text(extent={{-136,-98}, {-12,-120}}, lineColor={0, 0, 255}, fillColor={215, 215, 215}, fillPattern=FillPattern.Solid, horizontalAlignment=TextAlignment.Left, textString="Return fan discharge static pressure setpoint")}), Documentation(info="<html>
<p>
