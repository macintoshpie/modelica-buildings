...
model ReturnFanDirectPressure
  ...
  Buildings.Controls.OBC.ASHRAE.G36_PR1.AHUs.MultiZone.VAV.SetPoints.ReturnFanDirectPressure retFanPre(
    k=0.1)
    ...
    annotation(
      Placement(
        transformation(
          extent={{20, 60}, {40, 80}})));
  CDL.Logical.Sources.Pulse yFan(
    period=4000)
    "Supply fan status"
    annotation(
      Placement(
  ...
  CDL.Continuous.Sources.Ramp dpBui(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 10}, {-60, 30}})));
  Buildings.Controls.OBC.ASHRAE.G36_PR1.AHUs.MultiZone.VAV.SetPoints.ReturnFanDirectPressure retFanPre1(
    k=0.5)
    "Return fan control with direct building pressure"
    annotation(
      Placement(
...
equation
  connect(yFan.y, retFanPre.uFan)
    annotation(
      Line(
        points={{-59, 70}, {-20, 70}, {-20, 64}, {18, 64}},
        color={255, 0, 255}));
  connect(dpBui.y, retFanPre.dpBui)
    annotation(
      Line(
        points={{-59, 20}, {0, 20}, {0, 76}, {18, 76}},
        color={0, 0, 127}));
  connect(yFan.y, retFanPre1.uFan)
    annotation(
      Line(
        points={{-59, 70}, {-20, 70}, {-20, 14}, {18, 14}},
        color={255, 0, 255}));
  connect(yFan.y, retFanPre2.uFan)
    annotation(
      Line(
        points={{-59, 70}, {-20, 70}, {-20,-36}, {18,-36}},
        color={255, 0, 255}));
  connect(dpBui.y, retFanPre1.dpBui)
    annotation(
      Line(
        points={{-59, 20}, {0, 20}, {0, 26}, {18, 26}},
        color={0, 0, 127}));
  connect(dpBui.y, retFanPre2.dpBui)
    annotation(
      Line(
        points={{-59, 20}, {0, 20}, {0,-24}, {18,-24}},
        color={0, 0, 127}));
  annotation(
    experiment(
      StopTime=3600,
      Tolerance=1e-6),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/ASHRAE/G36_PR1/AHUs/MultiZone/VAV/SetPoints/Validation/ReturnFanDirectPressure.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
