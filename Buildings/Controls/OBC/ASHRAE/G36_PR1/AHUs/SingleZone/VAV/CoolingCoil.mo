...
model CoolingCoil
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TSupCoo(final unit="K", final displayUnit="degC", final quantity="ThermodynamicTemperature")
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput yCooCoi
    ...
    annotation(Placement(transformation(extent={{100,-10}, {120, 10}}), iconTransformation(extent={{100,-20}, {140, 20}})));
protected
  ...
  Buildings.Controls.OBC.CDL.Continuous.LimPID cooCoiPI(reverseAction=true, reset=Buildings.Controls.OBC.CDL.Types.Reset.Parameter, controllerType=controllerTypeCooCoi, k=kCooCoi, Ti=TiCooCoi, Td=TdCooCoi)
    "Cooling coil control signal"
    annotation(Placement(transformation(extent={{-10, 70}, {10, 90}})));
  Buildings.Controls.OBC.CDL.Logical.Switch switch
    "Switch to assign cooling coil control signal"
    annotation(Placement(transformation(extent={{72,-10}, {92, 10}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant const(k=0)
    "Cooling off mode"
    annotation(Placement(transformation(extent={{40,-30}, {60,-10}})));
  Buildings.Controls.OBC.CDL.Logical.And and2
    "Conditions for cooling state"
    annotation(Placement(transformation(extent={{0,-30}, {20,-10}})));
equation
  connect(const.y, switch.u3)
    annotation(Line(points={{62,-20}, {66,-20}, {66,-8}, {70,-8}}, color={0, 0, 127}));
  connect(cooCoiPI.trigger, uSupFan)
    annotation(Line(points={{-8, 68}, {-8,-80}, {-120,-80}}, color={255, 0, 255}));
  connect(cooCoiPI.u_s, TSupCoo)
  ...
  connect(and2.u2, uSupFan)
    annotation(Line(points={{-2,-28}, {-8,-28}, {-8,-80}, {-120,-80}}, color={255, 0, 255}));
  connect(and2.y, switch.u2)
    annotation(Line(points={{22,-20}, {30,-20}, {30, 0}, {70, 0}}, color={255, 0, 255}));
  connect(conInt.y, intEqu.u1)
    annotation(Line(points={{-58,-20}, {-42,-20}}, color={255, 127, 0}));
  connect(uZonSta, intEqu.u2)
    annotation(Line(points={{-120,-40}, {-50,-40}, {-50,-28}, {-42,-28}}, color={255, 127, 0}));
  connect(cooCoiPI.y, switch.u1)
    annotation(Line(points={{12, 80}, {60, 80}, {60, 8}, {70, 8}}, color={0, 0, 127}));
  annotation(defaultComponentName="cooCoi", Icon(coordinateSystem(preserveAspectRatio=false), graphics={Rectangle(extent={{-100,-100}, {100, 100}}, lineColor={0, 0, 127}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Text(extent={{-150, 150}, {150, 110}}, textString="%name", lineColor={0, 0, 255})}), Diagram(coordinateSystem(preserveAspectRatio=false)), Documentation(info="<html>
<p>
