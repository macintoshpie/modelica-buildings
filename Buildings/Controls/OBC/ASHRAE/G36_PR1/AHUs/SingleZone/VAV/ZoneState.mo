...
block ZoneState
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput uHea
    ...
    annotation(Placement(transformation(extent={{-180, 20}, {-140, 60}}), iconTransformation(extent={{-140, 20}, {-100, 60}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput uCoo
    "Cooling control signal"
    annotation(Placement(transformation(extent={{-180,-60}, {-140,-20}}), iconTransformation(extent={{-140,-60}, {-100,-20}})));
  Buildings.Controls.OBC.CDL.Interfaces.IntegerOutput yZonSta
    "Zone state"
    annotation(Placement(transformation(extent={{140,-20}, {180, 20}}), iconTransformation(extent={{100,-10}, {120, 10}})));
protected
  Buildings.Controls.OBC.CDL.Conversions.BooleanToInteger booToIntHea(final integerFalse=0, final integerTrue=Buildings.Controls.OBC.ASHRAE.G36_PR1.Types.ZoneStates.heating)
    "Convert Boolean to Integer number"
    annotation(Placement(transformation(extent={{0, 30}, {20, 50}})));
  Buildings.Controls.OBC.CDL.Conversions.BooleanToInteger booToIntCoo(final integerFalse=0, final integerTrue=Buildings.Controls.OBC.ASHRAE.G36_PR1.Types.ZoneStates.cooling)
    "Convert Boolean to Integer number"
    annotation(Placement(transformation(extent={{28,-30}, {48,-10}})));
  Buildings.Controls.OBC.CDL.Logical.Nor isDea
    "In deadband state"
    annotation(Placement(transformation(extent={{30,-80}, {50,-60}})));
  Buildings.Controls.OBC.CDL.Conversions.BooleanToInteger booToIntDea(final integerFalse=0, final integerTrue=Buildings.Controls.OBC.ASHRAE.G36_PR1.Types.ZoneStates.deadband)
    "Convert Boolean to Integer number"
    annotation(Placement(transformation(extent={{70,-80}, {90,-60}})));
  Buildings.Controls.OBC.CDL.Logical.And isHea
    "In heating state if both conditions are true"
    annotation(Placement(transformation(extent={{-72, 30}, {-52, 50}})));
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hysUHea(final uLow=uLow, final uHigh=uHigh)
    "Check if it is in heating state"
    annotation(Placement(transformation(extent={{-120, 60}, {-100, 80}})));
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hysUCoo(final uLow=uLow, final uHigh=uHigh)
    "Check if it is in cooling state"
    annotation(Placement(transformation(extent={{-100,-50}, {-80,-30}})));
  Buildings.Controls.OBC.CDL.Continuous.Add uHeaMinUCoo(final k2=-1)
  ...
  Buildings.Controls.OBC.CDL.Logical.And isCoo
    "In cooling state if both inputs are true"
    annotation(Placement(transformation(extent={{-10,-30}, {10,-10}})));
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hysU(final uLow=-uLow, final uHigh=uLow)
...
equation
  ...
  connect(uCoo, uHeaMinUCoo.u2)
    annotation(Line(points={{-160,-40}, {-136,-40}, {-136, 4}, {-132, 4}}, color={0, 0, 127}));
  connect(uHea, uHeaMinUCoo.u1)
    annotation(Line(points={{-160, 40}, {-136, 40}, {-136, 16}, {-132, 16}}, color={0, 0, 127}));
  connect(isCoo.y, booToIntCoo.u)
    annotation(Line(points={{12,-20}, {26,-20}}, color={255, 0, 255}));
  connect(hysUCoo.y, isCoo.u2)
    annotation(Line(points={{-78,-40}, {-20,-40}, {-20,-28}, {-12,-28}}, color={255, 0, 255}));
  ...
  connect(isHea.y, notHea.u)
    annotation(Line(points={{-50, 40}, {-48, 40}, {-48, 10}, {-46, 10}}, color={255, 0, 255}));
  connect(hysU.y, isHea.u2)
    annotation(Line(points={{-78, 10}, {-76, 10}, {-76, 32}, {-74, 32}}, color={255, 0, 255}));
  connect(notHea.y, isCoo.u1)
    annotation(Line(points={{-22, 10}, {-20, 10}, {-20,-20}, {-12,-20}}, color={255, 0, 255}));
  connect(isCoo.y, isDea.u2)
    annotation(Line(points={{12,-20}, {20,-20}, {20,-78}, {28,-78}}, color={255, 0, 255}));
  connect(isHea.y, isDea.u1)
    annotation(Line(points={{-50, 40}, {-48, 40}, {-48,-70}, {28,-70}}, color={255, 0, 255}));
  connect(booToIntHea.y, addInt.u1)
    annotation(Line(points={{22, 40}, {60, 40}, {60, 36}, {68, 36}}, color={255, 127, 0}));
  connect(booToIntCoo.y, addInt.u2)
    annotation(Line(points={{50,-20}, {60,-20}, {60, 24}, {68, 24}}, color={255, 127, 0}));
  connect(addInt.y, addInt1.u1)
    annotation(Line(points={{92, 30}, {100, 30}, {100, 6}, {108, 6}}, color={255, 127, 0}));
  connect(booToIntDea.y, addInt1.u2)
    annotation(Line(points={{92,-70}, {100,-70}, {100,-6}, {108,-6}}, color={255, 127, 0}));
  connect(addInt1.y, yZonSta)
    annotation(Line(points={{132, 0}, {160, 0}}, color={255, 127, 0}));
  annotation(defaultComponentName="zonSta", Icon(coordinateSystem(preserveAspectRatio=false), graphics={Rectangle(extent={{-100,-100}, {100, 100}}, lineColor={0, 0, 127}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Text(extent={{-150, 150}, {150, 110}}, textString="%name", lineColor={0, 0, 255})}), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-140,-100}, {140, 100}})), Documentation(info="<html>
<p>
