...
block TrimAndRespond
  ...
  parameter Real maxSet
    "Maximum setpoint";
  parameter Real delTim(final unit="s", final quantity="Time", final min=100*1E-15)
    "Delay time";
  parameter Real samplePeriod(final unit="s", final quantity="Time", final min=1E-3)
  ...
  parameter Real resAmo
    "Respond amount (must have opposite sign of triAmo)";
  parameter Real maxRes
    "Maximum response per time interval (must have same sign as resAmo)";
  Buildings.Controls.OBC.CDL.Interfaces.IntegerInput numOfReq
    "Number of requests from zones/systems"
    annotation(Placement(transformation(extent={{-260,-70}, {-220,-30}}), iconTransformation(extent={{-140,-100}, {-100,-60}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput uDevSta
    "On/Off status of the associated device"
    annotation(Placement(transformation(extent={{-260, 150}, {-220, 190}}), iconTransformation(extent={{-140, 60}, {-100, 100}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput y
    "Setpoint that have been reset"
    annotation(Placement(transformation(extent={{220, 150}, {260, 190}}), iconTransformation(extent={{100,-20}, {140, 20}})));
  Buildings.Controls.OBC.CDL.Logical.TrueDelay tim(final delayTime=delTim + samplePeriod, final delayOnInit=true)
  ...
  Buildings.Controls.OBC.CDL.Logical.Switch netRes
    "Net setpoint reset value"
    annotation(Placement(transformation(extent={{160,-20}, {180,-40}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant resAmoCon(final k=resAmo)
  ...
  Buildings.Controls.OBC.CDL.Discrete.UnitDelay uniDel(final samplePeriod=samplePeriod, final y_start=iniSet)
...
protected
  ...
  Buildings.Controls.OBC.CDL.Utilities.Assert assMes(final message="Trim amount 'triAmo' and respond amount 'resAmo' must have opposite signs.")
    "Generate alarm message"
    annotation(Placement(transformation(extent={{-80,-110}, {-60,-90}})));
  Buildings.Controls.OBC.CDL.Utilities.Assert assMes2(final message="Respond amount 'resAmo' and maximum respond amount 'maxRes' must have same sign.")
    "Generate alarm message"
...
equation
  connect(numIgnReqCon.y, difReqIgnReq.u1)
    annotation(Line(points={{-138,-10}, {-120,-10}, {-120,-24}, {-102,-24}}, color={0, 0, 127}));
  connect(difReqIgnReq.y, greThr.u)
    annotation(Line(points={{-78,-30}, {-40,-30}, {-40,-50}, {18,-50}}, color={0, 0, 127}));
  connect(pro.y, minInp.u1)
    annotation(Line(points={{2,-100}, {10,-100}, {10,-114}, {18,-114}}, color={0, 0, 127}));
  connect(triAmoCon.y, add2.u1)
    annotation(Line(points={{-178,-80}, {118,-80}}, color={0, 0, 127}));
  connect(add2.y, netRes.u1)
    annotation(Line(points={{142,-86}, {150,-86}, {150,-38}, {158,-38}}, color={0, 0, 127}));
  connect(iniSetCon.y, swi.u3)
    annotation(Line(points={{-78, 190}, {80, 190}, {80, 178}, {158, 178}}, color={0, 0, 127}));
  connect(swi.y, y)
    annotation(Line(points={{182, 170}, {240, 170}}, color={0, 0, 127}));
  connect(maxSetCon.y, min1.u2)
    annotation(Line(points={{-18, 70}, {-10, 70}, {-10, 94}, {-2, 94}}, color={0, 0, 127}));
  connect(add1.y, min1.u1)
    annotation(Line(points={{-18, 100}, {-10, 100}, {-10, 106}, {-2, 106}}, color={0, 0, 127}));
  connect(uniDel.y, add1.u1)
    annotation(Line(points={{-78, 106}, {-42, 106}}, color={0, 0, 127}));
  connect(sampler.y, difReqIgnReq.u2)
    annotation(Line(points={{-138,-50}, {-120,-50}, {-120,-36}, {-102,-36}}, color={0, 0, 127}));
  connect(triAmoCon.y, swi1.u1)
    annotation(Line(points={{-178,-80}, {0,-80}, {0, 18}, {118, 18}}, color={0, 0, 127}));
  connect(zerTri.y, swi1.u3)
    annotation(Line(points={{82,-10}, {100,-10}, {100, 2}, {118, 2}}, color={0, 0, 127}));
  connect(greThr.y, and2.u2)
    annotation(Line(points={{42,-50}, {60,-50}, {60,-38}, {118,-38}}, color={255, 0, 255}));
  connect(and2.y, netRes.u2)
    annotation(Line(points={{142,-30}, {158,-30}}, color={255, 0, 255}));
  connect(iniSetCon.y, swi2.u1)
    annotation(Line(points={{-78, 190}, {80, 190}, {80, 148}, {98, 148}}, color={0, 0, 127}));
  connect(swi2.y, swi.u1)
    annotation(Line(points={{122, 140}, {140, 140}, {140, 162}, {158, 162}}, color={0, 0, 127}));
  connect(swi2.y, uniDel.u)
    annotation(Line(points={{122, 140}, {140, 140}, {140, 160}, {-120, 160}, {-120, 106}, {-102, 106}}, color={0, 0, 127}));
  connect(uDevSta, not1.u)
    annotation(Line(points={{-240, 170}, {-210, 170}, {-210, 140}, {-102, 140}}, color={255, 0, 255}));
  connect(not1.y, swi2.u2)
    annotation(Line(points={{-78, 140}, {98, 140}}, color={255, 0, 255}));
  connect(min1.y, maxInp.u1)
    annotation(Line(points={{22, 100}, {30, 100}, {30, 106}, {38, 106}}, color={0, 0, 127}));
  connect(minSetCon.y, maxInp.u2)
    annotation(Line(points={{22, 70}, {30, 70}, {30, 94}, {38, 94}}, color={0, 0, 127}));
  ...
  connect(difReqIgnReq.y, pro.u1)
    annotation(Line(points={{-78,-30}, {-40,-30}, {-40,-94}, {-22,-94}}, color={0, 0, 127}));
  ...
  connect(tim.y, swi1.u2)
    annotation(Line(points={{-178, 170}, {-140, 170}, {-140, 10}, {118, 10}}, color={255, 0, 255}));
  connect(and2.u1, tim.y)
    annotation(Line(points={{118,-30}, {-20,-30}, {-20, 10}, {-140, 10}, {-140, 170}, {-178, 170}}, color={255, 0, 255}));
  connect(maxInp.y, swi2.u3)
    annotation(Line(points={{62, 100}, {80, 100}, {80, 132}, {98, 132}}, color={0, 0, 127}));
  connect(triAmoCon.y, pro1.u1)
    annotation(Line(points={{-178,-80}, {-170,-80}, {-170,-94}, {-162,-94}}, color={0, 0, 127}));
  connect(resAmoCon.y, pro1.u2)
    annotation(Line(points={{-178,-130}, {-170,-130}, {-170,-106}, {-162,-106}}, color={0, 0, 127}));
  ...
  connect(resAmoCon.y, pro2.u1)
    annotation(Line(points={{-178,-130}, {-170,-130}, {-170,-164}, {-162,-164}}, color={0, 0, 127}));
  connect(maxResCon.y, pro2.u2)
    annotation(Line(points={{-178,-200}, {-170,-200}, {-170,-176}, {-162,-176}}, color={0, 0, 127}));
  ...
  connect(abs.y, pro.u2)
    annotation(Line(points={{-98,-130}, {-40,-130}, {-40,-106}, {-22,-106}}, color={0, 0, 127}));
  connect(maxResCon.y, abs1.u)
    annotation(Line(points={{-178,-200}, {-122,-200}}, color={0, 0, 127}));
  connect(abs1.y, minInp.u2)
    annotation(Line(points={{-98,-200}, {-20,-200}, {-20,-126}, {18,-126}}, color={0, 0, 127}));
  connect(minInp.y, swi3.u1)
    annotation(Line(points={{42,-120}, {60,-120}, {60,-142}, {118,-142}}, color={0, 0, 127}));
  connect(resAmoCon.y, greEquThr1.u)
    annotation(Line(points={{-178,-130}, {-170,-130}, {-170,-150}, {18,-150}}, color={0, 0, 127}));
  connect(greEquThr1.y, swi3.u2)
    annotation(Line(points={{42,-150}, {118,-150}}, color={255, 0, 255}));
  connect(netRes.y, add1.u2)
    annotation(Line(points={{182,-30}, {200,-30}, {200, 36}, {-60, 36}, {-60, 94}, {-42, 94}}, color={0, 0, 127}));
  ...
  connect(gai.y, swi3.u3)
    annotation(Line(points={{102,-180}, {110,-180}, {110,-158}, {118,-158}}, color={0, 0, 127}));
  connect(swi3.y, add2.u2)
    annotation(Line(points={{142,-150}, {160,-150}, {160,-120}, {100,-120}, {100,-92}, {118,-92}}, color={0, 0, 127}));
  annotation(defaultComponentName="triRes", Icon(coordinateSystem(extent={{-100,-100}, {100, 100}}), graphics={Rectangle(extent={{-100,-100}, {100, 100}}, lineColor={0, 0, 127}, fillColor={223, 211, 169}, lineThickness=5.0, borderPattern=BorderPattern.Raised, fillPattern=FillPattern.Solid), Text(extent={{-114, 146}, {106, 108}}, lineColor={0, 0, 255}, textString="%name"), Text(extent={{-88, 58}, {90,-42}}, lineColor={192, 192, 192}, textString="Trim & Respond")}), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-220,-220}, {220, 220}}), graphics={Rectangle(extent={{-218, 28}, {218,-218}}, lineColor={0, 0, 0}, fillColor={215, 215, 215}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Rectangle(extent={{-218, 218}, {218, 42}}, lineColor={0, 0, 0}, fillColor={215, 215, 215}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Text(extent={{-214, 122}, {-124, 98}}, lineColor={0, 0, 255}, horizontalAlignment=TextAlignment.Left, textString="Check device status,
Count time"), Text(extent={{-216, 22}, {-110,-6}}, lineColor={0, 0, 255}, horizontalAlignment=TextAlignment.Left, textString="Reset setpoint based
on request number")}), Documentation(info="<html>
<p>
