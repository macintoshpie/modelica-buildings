  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant ySet(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-40, 30}, {-20, 50}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant yMea(
    k=0.75)
    "Measured value"
    annotation(
      Placement(
...
equation
  ...
  connect(yMea.y, limPID.u_m)
    annotation(
      Line(
        points={{-18, 10}, {30, 10}, {30, 18}},
        color={0, 0, 127}));
  connect(yMea.y, limPD.u_m)
    annotation(
      Line(
        points={{-18, 10}, {-4, 10}, {-4,-50}, {30,-50}, {30,-42}},
        color={0, 0, 127}));
  annotation(
    experiment(
      StopTime=0.2,
      Tolerance=1e-06),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/CDL/Continuous/Validation/LimPIDInitialDerivativeOutput.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
