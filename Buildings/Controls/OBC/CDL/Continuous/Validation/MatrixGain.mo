...
model MatrixGain
  ...
  Buildings.Controls.OBC.CDL.Continuous.MatrixGain matGai(K=[
    1, 0;
    1, 1;
    1, 2])
    "Block that outputs the product of a matrix gain value with the input signal"
    annotation(Placement(transformation(extent={{20, 30}, {40, 50}})));
  Buildings.Controls.OBC.CDL.Continuous.MatrixGain matGai1(K=[
    1, 2;
    3, 4])
    "Block that outputs the product of a matrix gain value with the input signal"
    annotation(Placement(transformation(extent={{20,-50}, {40,-30}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp ramp(duration=1, offset=0, height=2)
    "Block that generates ramp signal"
    annotation(Placement(transformation(extent={{-40, 30}, {-20, 50}})));
  Sources.Ramp ramp1(duration=1, offset=0, height=1)
    "Block that generates ramp signal"
    annotation(Placement(transformation(extent={{-40,-30}, {-20,-10}})));
  Sources.Ramp ramp2(duration=1, offset=0, height=2)
    "Block that generates ramp signal"
...
equation
  ...
  connect(ramp.y, matGai.u[2])
    annotation(Line(points={{-19, 40}, {18, 40}}, color={0, 0, 127}));
  annotation(experiment(StopTime=1.0, Tolerance=1e-06), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/CDL/Continuous/Validation/MatrixGain.mos"
