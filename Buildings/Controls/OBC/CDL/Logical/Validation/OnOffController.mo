...
equation
  ...
  connect(sin1.y, onOffController.u)
    annotation(
      Line(
        points={{-11,-6}, {6,-6}, {24,-6}},
        color={0, 0, 127}));
  connect(cons2.y, onOffController.reference)
    annotation(
      Line(
        points={{-11, 28}, {6, 28}, {6, 6}, {24, 6}},
        color={0, 0, 127}));
  annotation(
    experiment(
      StopTime=5.0,
      Tolerance=1e-06),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/CDL/Logical/Validation/OnOffController.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
