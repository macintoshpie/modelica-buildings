...
model Switch
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp ramp1(duration=5, offset=-1, height=6)
    "Block that generates ramp signal"
    annotation(Placement(transformation(extent={{-26, 22}, {-6, 42}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp ramp2(duration=5, offset=-1, height=2)
    "Block that generates ramp signal"
    annotation(Placement(transformation(extent={{-26,-42}, {-6,-22}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.Pulse booPul(width=0.5, period=2)
...
equation
  ...
  connect(ramp1.y, switch1.u1)
    annotation(Line(points={{-4, 32}, {8, 32}, {8, 8}, {24, 8}}, color={0, 0, 127}));
  annotation(experiment(StopTime=5.0, Tolerance=1e-06), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/CDL/Logical/Validation/Switch.mos"
