  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant TDryBul(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-94, 74}, {-74, 94}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant p2(
    k=101325)
    "Pressure"
    annotation(
      Placement(
...
equation
  connect(humRat.p_w, TDewPoi.p_w)
    annotation(
      Line(
        points={{9, 14}, {12, 14}, {17, 14}},
        color={0, 0, 127}));
  connect(X_pTphi.X[1], humRat.X_w)
    annotation(
      Line(
        points={{-25, 14}, {-22, 14}, {-13, 14}},
        color={0, 0, 127}));
  connect(TDryBul.y, dewBulPhi.TDryBul)
    annotation(
      Line(
        points={{-72, 84}, {-68, 84}, {-68, 92}, {14, 92}},
        color={0, 0, 127}));
  connect(phi.y, dewBulPhi.phi)
    annotation(
      Line(
        points={{-72, 50}, {-62, 50}, {-62, 84}, {14, 84}},
        color={0, 0, 127}));
  connect(p.y, dewBulPhi.p)
    annotation(
      Line(
        points={{-72, 18}, {-56, 18}, {-56, 76}, {14, 76}},
        color={0, 0, 127}));
  connect(p.y, X_pTphi.p_in)
    annotation(
      Line(
        points={{-72, 18}, {-56, 18}, {-56, 20}, {-48, 20}},
        color={0, 0, 127}));
  connect(TDryBul.y, X_pTphi.T)
    annotation(
      Line(
        points={{-72, 84}, {-68, 84}, {-68, 14}, {-48, 14}},
        color={0, 0, 127}));
  connect(phi.y, X_pTphi.phi)
    annotation(
      Line(
        points={{-72, 50}, {-62, 50}, {-62, 8}, {-48, 8}},
        color={0, 0, 127}));
  connect(TDewPoi.T, add.u2)
    annotation(
      Line(
        points={{39, 14}, {44, 14}, {44, 44}, {50, 44}},
        color={0, 0, 127}));
  connect(dewBulPhi.TDewPoi, add.u1)
    annotation(
      Line(
        points={{38, 84}, {42, 84}, {42, 56}, {50, 56}},
        color={0, 0, 127}));
  connect(TDryBul2.y, dewBulPhi1.TDryBul)
    annotation(
      Line(
        points={{-72,-18}, {-68,-18}, {-68,-10}, {16,-10}},
        color={0, 0, 127}));
  connect(TDryBul2.y, X_pTphi1.T)
    annotation(
      Line(
        points={{-72,-18}, {-68,-18}, {-68,-88}, {-46,-88}},
        color={0, 0, 127}));
  connect(p2.y, X_pTphi1.p_in)
    annotation(
      Line(
        points={{-72,-84}, {-56,-84}, {-56,-82}, {-46,-82}},
        color={0, 0, 127}));
  connect(p2.y, dewBulPhi1.p)
    annotation(
      Line(
        points={{-72,-84}, {-56,-84}, {-56,-26}, {16,-26}},
        color={0, 0, 127}));
  connect(phi2.y, dewBulPhi1.phi)
    annotation(
      Line(
        points={{-72,-50}, {-62,-50}, {-62,-18}, {16,-18}},
        color={0, 0, 127}));
  connect(phi2.y, X_pTphi1.phi)
    annotation(
      Line(
        points={{-72,-50}, {-62,-50}, {-62,-94}, {-46,-94}},
        color={0, 0, 127}));
  connect(X_pTphi1.X[1], humRat1.X_w)
    annotation(
      Line(
        points={{-23,-88}, {-11,-88}, {-11,-88}},
        color={0, 0, 127}));
  connect(humRat1.p_w, TDewPoi1.p_w)
    annotation(
      Line(
        points={{11,-88}, {19,-88}, {19,-88}},
        color={0, 0, 127}));
  connect(dewBulPhi1.TDewPoi, add1.u1)
    annotation(
      Line(
        points={{40,-18}, {44,-18}, {44,-44}, {50,-44}},
        color={0, 0, 127}));
  connect(TDewPoi1.T, add1.u2)
    annotation(
      Line(
        points={{41,-88}, {46,-88}, {46,-56}, {50,-56}},
        color={0, 0, 127}));
  annotation(
    experiment(
      StopTime=1.0,
      Tolerance=1e-06),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/CDL/Psychrometrics/Validation/TDewPoi_TDryBulPhi.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
