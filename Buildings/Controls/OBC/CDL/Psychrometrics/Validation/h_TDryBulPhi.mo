...
equation
  connect(TDryBul.y, hBulPhi.TDryBul)
    annotation(
      Line(
        points={{-43, 34}, {0, 34}, {0, 8}, {45, 8}},
        color={0, 0, 127}));
  connect(phi.y, hBulPhi.phi)
    annotation(
      Line(
        points={{-43, 0}, {0, 0}, {45, 0}},
        color={0, 0, 127}));
  connect(p.y, hBulPhi.p)
    annotation(
      Line(
        points={{-43,-32}, {0,-32}, {0,-8}, {45,-8}},
        color={0, 0, 127}));
  annotation(
    experiment(
      StopTime=1.0,
      Tolerance=1e-06),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/CDL/Psychrometrics/Validation/h_TDryBulPhi.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
