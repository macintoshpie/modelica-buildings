...
equation
  connect(ram.y, extIndSig.u[1])
    annotation(
      Line(
        points={{-59, 80}, {8, 80}, {8, 78.4}, {38, 78.4}},
        color={0, 0, 127}));
  connect(pul.y, extIndSig.u[2])
    annotation(
      Line(
        points={{-59, 50}, {4, 50}, {4, 79.2}, {38, 79.2}},
        color={0, 0, 127}));
  connect(pul1.y, extIndSig.u[3])
    annotation(
      Line(
        points={{-59, 20}, {-10, 20}, {-10, 80}, {38, 80}},
        color={0, 0, 127}));
  connect(ram1.y, extIndSig.u[4])
    annotation(
      Line(
        points={{-59,-10}, {0,-10}, {0, 80.8}, {38, 80.8}},
        color={0, 0, 127}));
  connect(ram2.y, extIndSig.u[5])
    annotation(
      Line(
        points={{-59,-40}, {-4,-40}, {-4, 81.6}, {38, 81.6}},
        color={0, 0, 127}));
  connect(conInt.y, extIndSig.index)
    annotation(
      Line(
        points={{41, 30}, {50, 30}, {50, 68}},
        color={255, 127, 0}));
  connect(conInt1.y, extIndSig1.index)
    annotation(
      Line(
        points={{41,-80}, {50,-80}, {50,-52}},
        color={255, 127, 0}));
  connect(ram.y, extIndSig1.u[1])
    annotation(
      Line(
        points={{-59, 80}, {8, 80}, {8,-41.6}, {38,-41.6}},
        color={0, 0, 127}));
  connect(pul.y, extIndSig1.u[2])
    annotation(
      Line(
        points={{-59, 50}, {4, 50}, {4,-40.8}, {38,-40.8}},
        color={0, 0, 127}));
  connect(pul1.y, extIndSig1.u[3])
    annotation(
      Line(
        points={{-59, 20}, {-10, 20}, {-10,-40}, {38,-40}},
        color={0, 0, 127}));
  connect(ram1.y, extIndSig1.u[4])
    annotation(
      Line(
        points={{-59,-10}, {0,-10}, {0,-39.2}, {38,-39.2}},
        color={0, 0, 127}));
  connect(ram2.y, extIndSig1.u[5])
    annotation(
      Line(
        points={{-59,-40}, {-4,-40}, {-4,-38.4}, {38,-38.4}},
        color={0, 0, 127}));
  annotation(
    experiment(
      StopTime=1.0,
      Tolerance=1e-06),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/CDL/Routing/Validation/RealExtractor.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
