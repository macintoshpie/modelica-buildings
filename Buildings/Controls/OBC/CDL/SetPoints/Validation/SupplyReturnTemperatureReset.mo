  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp TOut(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60, 50}, {-40, 70}})));
  Buildings.Controls.OBC.CDL.SetPoints.SupplyReturnTemperatureReset heaCur1(
    m=1,
    dTOutHeaBal=15,
    TSup_nominal=333.15,
    TRet_nominal=313.15,
...
equation
  connect(TOut1.y, heaCur1.TOut)
    annotation(
      Line(
        points={{-38,-20}, {-38,-20}, {0,-20}, {0,-34}, {18,-34}},
        color={0, 0, 127}));
  connect(TOut.y, heaCur.TOut)
    annotation(
      Line(
        points={{-38, 60}, {-38, 60}, {0, 60}, {0, 46}, {18, 46}},
        color={0, 0, 127}));
  connect(TRoo.y, heaCur.TSetZon)
    annotation(
      Line(
        points={{-38, 20}, {0, 20}, {0, 34}, {18, 34}},
        color={0, 0, 127}));
  connect(TRoo1.y, heaCur1.TSetZon)
    annotation(
      Line(
        points={{-38,-60}, {0,-60}, {0,-46}, {18,-46}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=1.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/CDL/SetPoints/Validation/SupplyReturnTemperatureReset.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
