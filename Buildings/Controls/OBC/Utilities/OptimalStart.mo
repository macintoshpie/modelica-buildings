...
block OptimalStart
  ...
  parameter Real tOptMax(final quantity="Time", final unit="s", displayUnit="h", final min=0, max=21600)=10800
  ...
  parameter Real uLow(final quantity="TemperatureDifference", final unit="K", final min=0)=0
    "Threshold to determine if the zone temperature reaches the occupied setpoint,
     must be a non-negative number";
  parameter Real uHigh(final quantity="TemperatureDifference", final unit="K", final min=0)=0.5
    "Threshold to determine the need to start the HVAC system before occupancy,
     must be greater than uLow";
  parameter Real thrOptOn(final quantity="Time", final unit="s", displayUnit="h", final min=0, max=10800)=60
    "Threshold time, optimal start on signal becomes true when tOpt larger than thrOptOn";
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TSetZonHea(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC", min=200) if computeHeating
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TZon(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC", min=200)
    "Zone temperature"
    annotation(Placement(transformation(extent={{-180,-50}, {-140,-10}}), iconTransformation(extent={{-140,-50}, {-100,-10}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TSetZonCoo(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC", min=200) if computeCooling
  ...
  Buildings.Controls.OBC.Utilities.BaseClasses.OptimalStartCalculation optHea(final tOptMax=tOptMax, final thrOptOn=thrOptOn, final tOptDef=tOptDef, final nDay=nDay, final uLow=uLow, final uHigh=uHigh) if computeHeating
    "Optimal start time for heating system"
    annotation(Placement(transformation(extent={{20, 60}, {40, 80}})));
  Buildings.Controls.OBC.Utilities.BaseClasses.OptimalStartCalculation optCoo(final tOptMax=tOptMax, final thrOptOn=thrOptOn, final tOptDef=tOptDef, final nDay=nDay, final uLow=uLow, final uHigh=uHigh) if computeCooling
    "Optimal start time for cooling system"
    annotation(Placement(transformation(extent={{20,-80}, {40,-60}})));
protected
  parameter Real tOptDef(final quantity="Time", final unit="s", displayUnit="h")=3600
  ...
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hysSta(pre_y_start=false, uHigh=60, uLow=0)
    "Hysteresis to activate the optimal start boolean output"
    annotation(Placement(transformation(extent={{-70,-10}, {-50, 10}})));
  Buildings.Controls.OBC.CDL.Logical.Or or2
    "Get the optimal start boolean output"
    annotation(Placement(transformation(extent={{100,-50}, {120,-30}})));
  Buildings.Controls.OBC.CDL.Continuous.Add dTHea(final k1=+ 1, final k2=-1) if computeHeating
    "Temperature difference between heating setpoint and zone temperature"
    annotation(Placement(transformation(extent={{-80, 70}, {-60, 90}})));
  Buildings.Controls.OBC.CDL.Continuous.Add dTCoo(final k1=+ 1, final k2=-1) if computeCooling
...
equation
  ...
  annotation(defaultComponentName="optSta", Documentation(info="<html>
<p>
...
</html>"), Diagram(coordinateSystem(extent={{-140,-100}, {140, 100}})), Icon(graphics={Rectangle(extent={{-100,-100}, {100, 100}}, lineColor={0, 0, 127}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Line(points={{-70, 40}, {10, 40}, {10,-28}, {70,-28}}, color={28, 108, 200}), Line(points={{-34, 40}, {-20, 32}, {-12, 22}, {-6, 2}, {0,-16}, {10,-28}}, smooth=Smooth.Bezier, color={238, 46, 47}), Text(extent={{-68, 56}, {-44, 40}}, lineColor={28, 108, 200}, textString="TSet"), Polygon(points={{-70, 92}, {-78, 70}, {-62, 70}, {-70, 92}}, lineColor={192, 192, 192}, fillColor={192, 192, 192}, fillPattern=FillPattern.Solid), Line(points={{-70, 70}, {-70,-78}}, color={192, 192, 192}), Line(points={{-88,-60}, {70,-60}}, color={192, 192, 192}), Polygon(points={{92,-60}, {70,-52}, {70,-68}, {92,-60}}, lineColor={192, 192, 192}, fillColor={192, 192, 192}, fillPattern=FillPattern.Solid), Line(points={{10, 40}, {24, 32}, {32, 22}, {38, 2}, {44,-16}, {54,-28}}, smooth=Smooth.Bezier, color={28, 108, 200}, pattern=LinePattern.Dot), Text(lineColor={0, 0, 255}, extent={{-150, 110}, {150, 150}}, textString="%name"), Ellipse(extent={{79,-33}, {93,-47}}, lineColor=DynamicSelect({235, 235, 235}, if optOn then {0, 255, 0} else {235, 235, 235}), fillColor=DynamicSelect({235, 235, 235}, if optOn then {0, 255, 0} else {235, 235, 235}), fillPattern=FillPattern.Solid), Text(extent={{226, 106}, {106, 56}}, lineColor={0, 0, 0}, textString=DynamicSelect("", String(tOpt, leftjustified=false, significantDigits=3)))}));
end OptimalStart;
