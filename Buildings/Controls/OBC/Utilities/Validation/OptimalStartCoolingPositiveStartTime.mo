...
equation
  connect(dT.y, UA.u)
    annotation(
      Line(
        points={{-138, 10}, {-122, 10}},
        color={0, 0, 127}));
  connect(TOut.y, dT.u2)
    annotation(
      Line(
        points={{-172,-10}, {-166,-10}, {-166, 4}, {-162, 4}},
        color={0, 0, 127}));
  ...
  connect(QCoo.y, dTdt.u2)
    annotation(
      Line(
        points={{-98,-50}, {-88,-50}, {-88, 4}, {-82, 4}},
        color={0, 0, 127}));
  connect(TRoo.y, optStaCoo.TZon)
    annotation(
      Line(
        points={{-19, 10}, {-8, 10}, {-8, 7}, {18, 7}},
        color={0, 0, 127}));
  connect(TRoo.y, dT.u1)
    annotation(
      Line(
        points={{-19, 10}, {-14, 10}, {-14, 36}, {-166, 36}, {-166, 16}, {-162, 16}},
        color={0, 0, 127}));
  connect(TSetCooOcc.y, optStaCoo.TSetZonCoo)
    annotation(
      Line(
        points={{-18, 80}, {0, 80}, {0, 13}, {18, 13}},
        color={0, 0, 127}));
  connect(UA.y, dTdt.u1)
    annotation(
      Line(
        points={{-98, 10}, {-90, 10}, {-90, 16}, {-82, 16}},
        color={0, 0, 127}));
  connect(optStaCoo.optOn, booToRea.u)
    annotation(
      Line(
        points={{42, 6}, {50, 6}, {50, 10}, {58, 10}},
        color={255, 0, 255}));
  connect(add.y, conPID.u_s)
    annotation(
      Line(
        points={{142, 10}, {158, 10}},
        color={0, 0, 127}));
  connect(TRoo.y, conPID.u_m)
    annotation(
      Line(
        points={{-19, 10}, {-14, 10}, {-14,-20}, {170,-20}, {170,-2}},
        color={0, 0, 127}));
  connect(occSch.tNexOcc, optStaCoo.tNexOcc)
    annotation(
      Line(
        points={{-19,-44}, {0,-44}, {0, 2}, {18, 2}},
        color={0, 0, 127}));
  connect(conPID.y, QCoo.u)
    annotation(
      Line(
        points={{182, 10}, {188, 10}, {188,-80}, {-130,-80}, {-130,-50}, {-122,-50}},
        color={0, 0, 127}));
  connect(occSch.occupied, TSetCoo.u)
    annotation(
      Line(
        points={{-19,-56}, {10,-56}, {10,-50}, {58,-50}},
        color={255, 0, 255}));
  connect(TSetCoo.y, add.u2)
    annotation(
      Line(
        points={{82,-50}, {100,-50}, {100, 4}, {118, 4}},
        color={0, 0, 127}));
  connect(booToRea.y, add.u1)
    annotation(
      Line(
        points={{82, 10}, {100, 10}, {100, 16}, {118, 16}},
        color={0, 0, 127}));
  annotation(
    experiment(
      StartTime=34000,
      ...
      Tolerance=1e-06),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/Utilities/Validation/OptimalStartCoolingPositiveStartTime.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
