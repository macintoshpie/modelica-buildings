...
block ElectricalLoad
  ...
  Modelica.Blocks.Interfaces.RealInput TOut(
    unit="K") if(predictionModel == Buildings.Controls.Predictors.Types.PredictionModel.WeatherRegression)
...
protected
  ...
  parameter Modelica.SIunits.Time samStart(
    fixed=false)
...
algorithm
  when sampleTrigger then
    ...
    _storeHistory := if not pre(_storeHistory) and(not isMidNight(
      t=time)) then false else storeHistory;
    _typeOfDay := getTypeOfDays(
      t=time,
      typeOfDay=typeOfDay,
      dt=dt,
      nPre=nPre);
