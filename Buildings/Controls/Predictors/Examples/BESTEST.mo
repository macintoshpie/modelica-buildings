...
equation
  connect(dayType.y, preReg.typeOfDay)
    annotation(Line(points={{141, 70}, {154, 70}, {154, 30}, {158, 30}}, color={0, 127, 0}, smooth=Smooth.None));
  connect(storeResults.y, preReg.storeHistory)
    annotation(Line(points={{141, 40}, {150, 40}, {150, 25}, {158, 25}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(EHea.y, preReg.ECon)
    annotation(Line(points={{-11.6, 40}, {32, 40}, {32, 20}, {158, 20}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(weaBus.TDryBul, preReg.TOut)
    annotation(Line(points={{4,-88}, {80,-88}, {80, 14}, {158, 14}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  connect(preErr.u2, gaiHea.y)
    annotation(Line(points={{198, 4}, {100, 4}, {100, 34}, {-49.6, 34}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(preErr.u1, preReg.PPre[1])
    annotation(Line(points={{198, 16}, {190, 16}, {190, 20}, {181, 20}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(Diagram(coordinateSystem(extent={{-100,-100}, {240, 100}}, preserveAspectRatio=false), graphics), experiment(StopTime=2.6784e+06, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Controls/Predictors/Examples/BESTEST.mos"
