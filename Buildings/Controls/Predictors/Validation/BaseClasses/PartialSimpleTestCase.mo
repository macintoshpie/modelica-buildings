...
partial model PartialSimpleTestCase
  ...
  ElectricalLoad baseLoad(final nPre=nPre, use_dayOfAdj=false, predictionModel=Buildings.Controls.Predictors.Types.PredictionModel.Average)
    "Baseload prediction"
    annotation(Placement(transformation(extent={{60,-10}, {80, 10}})));
  Modelica.Blocks.Sources.BooleanPulse tri(width=4/24*100/7, period=7*tPeriod, startTime=1.5*tPeriod)
    "Sample trigger"
...
equation
  connect(tri.y, notEventDay.u)
    annotation(Line(points={{-19, 80}, {-2, 80}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(notEventDay.y, baseLoad.storeHistory)
    annotation(Line(points={{21, 80}, {44, 80}, {44, 5}, {58, 5}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(dayType.y, baseLoad.typeOfDay)
    annotation(Line(points={{-19, 40}, {18, 40}, {18, 10}, {58, 10}}, color={0, 127, 0}, smooth=Smooth.None));
