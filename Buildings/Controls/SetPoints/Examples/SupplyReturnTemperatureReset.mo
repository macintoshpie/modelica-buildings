...
model SupplyReturnTemperatureReset
  ...
  Buildings.Controls.SetPoints.SupplyReturnTemperatureReset heaCur(m=1, TSup_nominal=333.15, TRet_nominal=313.15, TOut_nominal=263.15)
    "Temperature reset"
  ...
  Buildings.Controls.SetPoints.SupplyReturnTemperatureReset heaCur1(m=1, use_TRoo_in=true, TSup_nominal=333.15, TRet_nominal=313.15, TOut_nominal=263.15, dTOutHeaBal=15)
    "Temperature reset that uses room temperature as input"
    annotation(Placement(transformation(extent={{-40, 0}, {-20, 20}})));
  Modelica.Blocks.Sources.Step TRoo(offset=273.15 + 20, startTime=0.5, height=-5)
    "Night set back from 20 to 15 deg C"
...
equation
  connect(TOut.y, heaCur.TOut)
    annotation(Line(points={{-59, 50}, {-50, 50}, {-50, 56}, {-42, 56}}, color={0, 0, 127}));
  connect(TOut2.y, heaCur1.TOut)
    annotation(Line(points={{-59, 10}, {-50, 10}, {-50, 16}, {-42, 16}}, color={0, 0, 127}));
  connect(TRoo.y, heaCur1.TRoo_in)
    annotation(Line(points={{-59,-30}, {-50,-30}, {-50, 4}, {-41.9, 4}}, color={0, 0, 127}));
  annotation(experiment(Tolerance=1e-6, StopTime=1.0), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Controls/SetPoints/Examples/SupplyReturnTemperatureReset.mos"
