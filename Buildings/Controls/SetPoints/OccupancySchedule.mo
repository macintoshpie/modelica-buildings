...
protected
  ...
  output Integer nexStaInd
    "Next index when occupancy starts";
  output Integer nexStoInd
    "Next index when occupancy stops";
  ...
  output Modelica.SIunits.Time tOcc
    "Time when next occupancy starts";
...
initial algorithm
  ...
  assert(period >= occupancy[nRow], "The parameter \"period\" must be greater than or equal to the last element of \"occupancy\"." + "\n   Received period      = " + String(period) + "\n            occupancy[" + String(nRow) + "] = " + String(occupancy[nRow]));
  for i in 1 : nRow-1 loop
    assert(occupancy[i] < occupancy[i + 1], "The elements of the parameter \"occupancy\" must be strictly increasing.");
  end for;
...
equation
  when time >= pre(tOcc) then
    ...
    iPerSta=if pre(nexStaInd) + 2 <= nRow then pre(iPerSta) else(pre(iPerSta) + 1);
    tOcc=occupancy[nexStaInd] + iPerSta*period;
  ...
  elsewhen time >= pre(tNonOcc) then
    ...
    iPerSto=if pre(nexStoInd) + 2 <= nRow then pre(iPerSto) else(pre(iPerSto) + 1);
    tNonOcc=occupancy[nexStoInd] + iPerSto*period;
    occupied=not pre(occupied);
  ...
  annotation(Icon(graphics={Line(points={{-62,-68}, {-38,-20}, {-14,-70}}, color={0, 0, 255}), Line(points={{-38,-20}, {-38, 44}}, color={0, 0, 255}), Ellipse(extent={{-54, 74}, {-22, 44}}, lineColor={0, 0, 255}), Line(points={{-66, 22}, {-38, 36}}, color={0, 0, 255}), Line(points={{-38, 36}, {-6, 20}}, color={0, 0, 255}), Text(extent={{34, 74}, {90, 50}}, lineColor={0, 0, 255}, textString="occupancy"), Text(extent={{32, 16}, {92,-16}}, lineColor={0, 0, 255}, textString="non-occupancy"), Text(extent={{34,-44}, {94,-76}}, lineColor={0, 0, 255}, textString="occupied")}), defaultComponentName="occSch", Documentation(info="<html>
<p>
