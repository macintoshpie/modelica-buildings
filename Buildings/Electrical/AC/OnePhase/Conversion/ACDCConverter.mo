...
model ACDCConverter
  "AC DC converter"
  extends Buildings.Electrical.Interfaces.PartialConversion(redeclare package PhaseSystem_p=PhaseSystems.TwoConductor, redeclare package PhaseSystem_n=PhaseSystems.OnePhase, redeclare replaceable Interfaces.Terminal_n terminal_n constrainedby Interfaces.Terminal_n(i(start=zeros(PhaseSystem_n.n), each stateSelect=StateSelect.prefer)), redeclare DC.Interfaces.Terminal_p terminal_p(i(start=zeros(PhaseSystem_p.n), each stateSelect=StateSelect.prefer)));
  ...
  parameter Boolean ground_AC=false
    "Connect AC side of converter to ground"
    annotation(Evaluate=true, Dialog(tab="Ground", group="AC side"));
  parameter Boolean ground_DC=true
    "Connect DC side of converter to ground"
    annotation(Evaluate=true, Dialog(tab="Ground", group="DC side"));
protected
  ...
  Modelica.SIunits.Power P_n[2](each start=0)=PhaseSystem_n.phasePowers_vi(terminal_n.v, terminal_n.i)
    "Power transmitted at pin n (primary)";
equation
  //voltage relation
  v_p=v_n*conversionFactor;
  // Power losses
  PLoss=(1-eta)*Buildings.Utilities.Math.Functions.spliceFunction(P_p[1], P_n[1], i_p, deltax=0.1);
  P_n + P_p={PLoss, 0};
  if ground_AC then
  ...
  annotation(defaultComponentName="conACDC", Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 100}}), graphics={Line(points={{2, 60}, {2, 60}, {82, 60}, {2, 60}, {82,-60}, {2,-60}, {2, 60}, {2,-60}}, color={0, 0, 255}, smooth=Smooth.None), Text(extent={{36, 54}, {96, 14}}, lineColor={0, 0, 255}, textString="DC"), Line(points={{-2, 60}, {-2, 60}, {-82, 60}, {-2, 60}, {-82,-60}, {-2,-60}, {-2, 60}, {-2,-60}}, color={0, 120, 120}, smooth=Smooth.None), Text(extent={{-100, 52}, {-40, 12}}, lineColor={0, 120, 120}, textString="AC"), Text(extent={{-100, 92}, {100, 60}}, lineColor={0, 0, 0}, textString="%name"), Text(extent={{-100,-60}, {100,-92}}, lineColor={0, 0, 0}, textString="%conversionFactor"), Text(extent={{-100,-100}, {100,-132}}, lineColor={0, 0, 0}, textString="%eta"), Line(visible=ground_DC == true, points={{100,-100}, {100,-12}}, color={0, 0, 255}, smooth=Smooth.None), Line(visible=ground_DC == true, points={{80,-100}, {120,-100}}, color={0, 0, 255}, smooth=Smooth.None), Line(visible=ground_DC == true, points={{88,-106}, {112,-106}}, color={0, 0, 255}, smooth=Smooth.None), Line(visible=ground_DC == true, points={{94,-112}, {108,-112}}, color={0, 0, 255}, smooth=Smooth.None), Line(visible=ground_AC == true, points={{-80,-40}, {-120,-40}}, color={0, 120, 120}, smooth=Smooth.None, thickness=0.5), Line(visible=ground_AC == true, points={{-80,-40}, {-106,-14}}, color={0, 120, 120}, smooth=Smooth.None, thickness=0.5), Line(visible=ground_AC == true, points={{-102,-16}, {-114,-24}, {-118,-42}}, color={0, 120, 120}, smooth=Smooth.Bezier)}), Documentation(info="<html>
<p>
