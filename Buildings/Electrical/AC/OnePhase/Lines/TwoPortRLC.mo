...
model TwoPortRLC
  ...
  extends Buildings.Electrical.Transmission.BaseClasses.PartialTwoPortRLC(V_nominal(start=110), redeclare package PhaseSystem_p=PhaseSystems.OnePhase, redeclare package PhaseSystem_n=PhaseSystems.OnePhase, redeclare replaceable Interfaces.Terminal_n terminal_n(redeclare package PhaseSystem=PhaseSystem_n), redeclare replaceable Interfaces.Terminal_p terminal_p(redeclare package PhaseSystem=PhaseSystem_p));
...
equation
  ...
  L/2*omega*Buildings.Electrical.PhaseSystems.OnePhase.j(terminal_p.i) + terminal_p.i*diagonal(ones(PhaseSystem_p.n)*R_actual/2)=terminal_p.v-Vc;
  L/2*omega*Buildings.Electrical.PhaseSystems.OnePhase.j(terminal_n.i) + terminal_n.i*diagonal(ones(PhaseSystem_n.n)*R_actual/2)=terminal_n.v-Vc;
  if C > 0 then
  ...
  // Joule losses
  LossPower=R_actual/2*(terminal_p.i[1]^2 + terminal_p.i[2]^2) + R_actual/2*(terminal_n.i[1]^2 + terminal_n.i[2]^2);
  annotation(defaultComponentName="lineRLC", Diagram(graphics={Rectangle(extent={{-70, 30}, {70,-30}}, lineColor={0, 0, 0}), Line(points={{-90, 0}, {-70, 0}}, color={0, 0, 0}), Line(points={{70, 0}, {90, 0}}, color={0, 0, 0})}), Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 100}}), graphics={Text(extent={{-140, 80}, {140, 40}}, lineColor={0, 0, 0}, textString="%name")}), Documentation(info="<html>
<p>
