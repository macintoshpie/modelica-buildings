...
equation
  ...
  if mode == Buildings.Electrical.Types.Load.FixedZ_dynamic then
    ...
    Z[2]=-Modelica.Fluid.Utilities.regRoot(1-pf^2,
      delta=0.001)*(V_nominal^2)/(P_nominal/pf);
    ...
    if linearized then
      i[1]=-homotopy(
        actual=(v[2]*Q + v[1]*P)/(V_nominal^2),
        simplified=v[1]*Modelica.Constants.eps*1e3);
      i[2]=-homotopy(
        actual=(v[2]*P-v[1]*Q)/(V_nominal^2),
      ...
      if initMode == Buildings.Electrical.Types.InitMode.zero_current then
        i[1]=-homotopy(
          actual=(v[2]*Q + v[1]*P)/(v[1]^2 + v[2]^2),
          simplified=0.0);
        i[2]=-homotopy(
          actual=(v[2]*P-v[1]*Q)/(v[1]^2 + v[2]^2),
          simplified=0.0);
      else
        i[1]=-homotopy(
          actual=(v[2]*Q + v[1]*P)/(v[1]^2 + v[2]^2),
  ...
  annotation(
    ...
    Icon(
      ...
      graphics={Rectangle(
        ...
        extent={{-100, 100}, {100,-100}},
        lineColor={255, 255, 255}), Ellipse(
        extent={{-10,-10}, {10, 10}},
        origin={10, 0},
        rotation=360), Ellipse(
        extent={{40,-10}, {60, 10}}), Ellipse(
        extent={{20,-10}, {40, 10}}), Rectangle(
        extent={{0, 0}, {60,-12}},
        fillColor={255, 255, 255},
