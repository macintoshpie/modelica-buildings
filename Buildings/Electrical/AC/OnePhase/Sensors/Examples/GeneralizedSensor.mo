...
model GeneralizedSensor
  ...
  Buildings.Electrical.AC.OnePhase.Loads.Capacitive loa(mode=Buildings.Electrical.Types.Load.FixedZ_dynamic, P_nominal=-100, V_nominal=120)
    "Constant load"
...
equation
  ...
  connect(sou.terminal, res.terminal_n)
    annotation(Line(points={{-40, 10}, {-32, 10}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(res.terminal_p, sen.terminal_n)
    annotation(Line(points={{-12, 10}, {-4.44089e-16, 10}}, color={0, 120, 120}, smooth=Smooth.None));
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Electrical/AC/OnePhase/Sensors/Examples/GeneralizedSensor.mos"
    "Simulate and plot"));
