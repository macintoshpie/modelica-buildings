...
model TransformerExample
  ...
  Sources.FixedVoltage sou(f=60, V=V_primary)
    "Voltage source"
    annotation(Placement(transformation(extent={{-70,-10}, {-50, 10}})));
  replaceable Buildings.Electrical.AC.ThreePhasesUnbalanced.Conversion.BaseClasses.PartialConverter tra
    "Transformer model"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}})));
  Loads.Resistive load(loadConn=Buildings.Electrical.Types.LoadConnection.wye_to_wyeg, P_nominal=-1800e3, V_nominal=V_secondary, linearized=true)
    "Load model"
  ...
  Sensors.ProbeDelta probe_D_1(perUnit=false, V_nominal=V_primary)
    ...
    annotation(Placement(transformation(extent={{-40,-30}, {-20,-50}})));
  replaceable Sensors.BaseClasses.GeneralizedProbe probe_2 constrainedby Sensors.BaseClasses.GeneralizedProbe(perUnit=false, V_nominal=V_secondary)
    "Probe that measures the voltage at the secondary side"
    annotation(Placement(transformation(extent={{20, 30}, {40, 50}})));
equation
  connect(sou.terminal, tra.terminal_n)
    annotation(Line(points={{-50, 0}, {-10, 0}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(probe_Y_1.term, tra.terminal_n)
    annotation(Line(points={{-30, 31}, {-30, 0}, {-10, 0}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(probe_D_1.term, tra.terminal_n)
    annotation(Line(points={{-30,-31}, {-30, 0}, {-10, 0}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(tra.terminal_p, load.terminal)
    annotation(Line(points={{10, 0}, {50, 0}}, color={0, 120, 120}, smooth=Smooth.None));
