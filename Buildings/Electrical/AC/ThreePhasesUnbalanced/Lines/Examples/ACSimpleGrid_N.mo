...
model ACSimpleGrid_N
  ...
  Network_N network(redeclare Buildings.Electrical.Transmission.Grids.TestGrid2Nodes grid, V_nominal=480)
    "Network model that represents the connection between the source and the load"
    annotation(Placement(transformation(extent={{20,-10}, {0, 10}})));
  Loads.Inductive_N load(V_nominal=480, mode=Buildings.Electrical.Types.Load.VariableZ_P_input, plugPhase3=false)
    "Load connected to the network"
    annotation(Placement(transformation(extent={{-34, 10}, {-54, 30}})));
  Sources.FixedVoltage_N E(f=60, V=480, definiteReference=true)
    "Voltage source"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, rotation=180, origin={-50,-20})));
  Modelica.Blocks.Sources.Ramp load_inputs(height=5000, offset=-2000, duration=0.5, startTime=0.25)
    "Input signal for the power consumption of the loads"
    annotation(Placement(transformation(extent={{-90, 10}, {-70, 30}})));
equation
  connect(E.terminal, network.terminal[1])
    annotation(Line(points={{-40,-20}, {-20,-20}, {-20, 0}, {0, 0}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(load.terminal, network.terminal[2])
    annotation(Line(points={{-34, 20}, {-20, 20}, {-20, 0}, {0, 0}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(load_inputs.y, load.Pow1)
    annotation(Line(points={{-69, 20}, {-62, 20}, {-62, 28}, {-56, 28}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(load_inputs.y, load.Pow2)
    annotation(Line(points={{-69, 20}, {-56, 20}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(experiment(StopTime=1.0, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Electrical/AC/ThreePhasesUnbalanced/Lines/Examples/ACSimpleGrid_N.mos"
