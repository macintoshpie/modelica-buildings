...
partial model BaseImpedance
  ...
  replaceable Buildings.Electrical.AC.OnePhase.Loads.Impedance load1(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10, 30}, {10, 50}})));
  replaceable Buildings.Electrical.AC.OnePhase.Loads.Impedance load2(
    inductive=inductive,
    R=R,
    L=L,
    C=C,
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10,-10}, {10, 10}})));
  replaceable Buildings.Electrical.AC.OnePhase.Loads.Impedance load3(
    inductive=inductive,
    R=R,
    L=L,
    C=C,
  ...
  parameter Modelica.SIunits.Inductance L(
    ...
    annotation(
      Dialog(
        enable=inductive and(not use_L_in)));
  parameter Modelica.SIunits.Capacitance C(
    ...
    annotation(
      Dialog(
        enable=(not inductive) and(not use_C_in)));
  parameter Boolean use_R_in=false
    ...
    annotation(
      ...
      Dialog(
        tab="Variable load",
        group="Resistance"));
  parameter Modelica.SIunits.Resistance RMin(
    start=R,
    min=Modelica.Constants.eps)=1e-4
    ...
    annotation(
      ...
      Dialog(
        ...
        tab="Variable load",
        group="Resistance"));
  parameter Modelica.SIunits.Resistance RMax(
    start=R,
    min=Modelica.Constants.eps)=1e2
  ...
  parameter Boolean use_C_in=false
    ...
    annotation(
      ...
      Dialog(
        tab="Variable load",
        group="Capacitance"));
  parameter Modelica.SIunits.Capacitance CMin(
    start=C,
    min=Modelica.Constants.eps)=1e-4
    ...
    annotation(
      ...
      Dialog(
        ...
        tab="Variable load",
        group="Capacitance"));
  parameter Modelica.SIunits.Capacitance CMax(
    start=C,
    min=Modelica.Constants.eps)=1e2
  ...
  parameter Boolean use_L_in=false
    ...
    annotation(
      ...
      Dialog(
        tab="Variable load",
        group="Inductance"));
  parameter Modelica.SIunits.Inductance LMin(
    start=L,
    min=Modelica.Constants.eps)=1e-4
    ...
    annotation(
      ...
      Dialog(
        ...
        tab="Variable load",
        group="Inductance"));
  parameter Modelica.SIunits.Inductance LMax(
    start=L,
    min=Modelica.Constants.eps)=1e2
    ...
    annotation(
      ...
      Dialog(
        ...
        tab="Variable load",
        group="Inductance"));
  Modelica.Blocks.Interfaces.RealInput y_R(
    min=0,
    max=1) if use_R_in
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=270,
          origin={-40, 100})));
  Modelica.Blocks.Interfaces.RealInput y_C(
    min=0,
    max=1) if use_C_in
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=270,
          origin={0, 100})));
  Modelica.Blocks.Interfaces.RealInput y_L(
    min=0,
    max=1) if use_L_in
...
equation
  ...
  if plugPhase1 then
    connect(load1.terminal, adaDel.terminals[1])
      annotation(
        Line(
          points={{-10, 40}, {-26, 40}, {-26,-40.5333}, {-40,-40.5333}},
          color={0, 120, 120}));
    connect(load1.terminal, adaWye.terminals[1])
      annotation(
        Line(
          points={{-10, 40}, {-26, 40}, {-26,-70.5333}, {-40,-70.5333}},
          color={0, 120, 120}));
  end if;
  if plugPhase2 then
    connect(load2.terminal, adaDel.terminals[2])
      annotation(
        Line(
          points={{-10, 0}, {-22, 0}, {-22,-40}, {-40,-40}},
          color={0, 120, 120}));
    connect(load2.terminal, adaWye.terminals[2])
      annotation(
        Line(
          points={{-10, 0}, {-22, 0}, {-22,-70}, {-40,-70}},
          color={0, 120, 120}));
  end if;
  if plugPhase3 then
    connect(load3.terminal, adaDel.terminals[3])
      annotation(
        Line(
          points={{-10,-40}, {-20,-40}, {-20,-40}, {-30,-40}, {-30,-39.4667}, {-40,-39.4667}},
          color={0, 120, 120}));
    connect(load3.terminal, adaWye.terminals[3])
      annotation(
        Line(
          points={{-10,-40}, {-20,-40}, {-20,-69.4667}, {-40,-69.4667}},
          color={0, 120, 120}));
  end if;
  connect(adaDel.terminal, wyeToDelta.delta)
    annotation(
      Line(
        points={{-60,-40}, {-64,-40}, {-64,-20}, {-28,-20}, {-28, 10}, {-44, 10}},
        color={0, 120, 120}));
  connect(adaWye.terminal, wyeToWyeGround.wyeg)
    annotation(
      Line(
        points={{-60,-70}, {-66,-70}, {-66,-18}, {-30,-18}, {-30,-10}, {-44,-10}},
        color={0, 120, 120}));
  annotation(
    Documentation(
      info="<html>
