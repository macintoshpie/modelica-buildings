...
model Loads_N
  ...
  Sources.FixedVoltage_N sou(definiteReference=true, f=60, V=480)
    "Voltage source"
    annotation(Placement(transformation(extent={{-94,-10}, {-74, 10}})));
  Modelica.Blocks.Sources.Ramp ph_1(offset=-1000, duration=0.5, startTime=0.25, height=-500)
  ...
  Resistive_N loaR_N(mode=Buildings.Electrical.Types.Load.VariableZ_P_input, V_nominal=480)
    "Resistive load with neutral cable"
    annotation(Placement(transformation(extent={{-8,-10}, {12, 10}})));
  Inductive_N loaRL_N(mode=Buildings.Electrical.Types.Load.VariableZ_P_input, pf=0.9, V_nominal=480)
    "Inductive load with neutral cable"
    annotation(Placement(transformation(extent={{-8,-40}, {12,-20}})));
  Capacitive_N loaRC_N(mode=Buildings.Electrical.Types.Load.VariableZ_P_input, pf=0.7, V_nominal=480)
...
equation
  ...
  connect(sou.terminal, sen.terminal_n)
    annotation(Line(points={{-74, 0}, {-60, 0}}, color={127, 0, 127}, smooth=Smooth.None));
  connect(sen.terminal_p, loaR_N.terminal)
    annotation(Line(points={{-40, 0}, {-8, 0}}, color={127, 0, 127}, smooth=Smooth.None));
  connect(sen.terminal_p, loaRL_N.terminal)
    annotation(Line(points={{-40, 0}, {-26, 0}, {-26,-30}, {-8,-30}}, color={127, 0, 127}, smooth=Smooth.None));
  connect(sen.terminal_p, loaRC_N.terminal)
    annotation(Line(points={{-40, 0}, {-26, 0}, {-26,-70}, {-8,-70}}, color={127, 0, 127}, smooth=Smooth.None));
...
</html>"), experiment(Tolerance=1e-6, StopTime=1.0), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Electrical/AC/ThreePhasesUnbalanced/Loads/Examples/Loads_N.mos"
    "Simulate and plot"));
end Loads_N;
