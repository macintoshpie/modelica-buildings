...
model Sensors
  ...
  Modelica.Blocks.Sources.Ramp ramp(height=2e4, duration=0.5, offset=-1e4, startTime=0.25)
    annotation(Placement(transformation(extent={{80,-10}, {60, 10}})));
equation
  connect(probeWye.term, source.terminal)
    annotation(Line(points={{-36, 49}, {-36, 30}, {-60, 30}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(probeDelta.term, source.terminal)
    annotation(Line(points={{-10, 49}, {-10, 30}, {-60, 30}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(source_N.terminal, probeWye_N.term)
    annotation(Line(points={{-60,-30}, {-20,-30}, {-20,-11}}, color={127, 0, 127}, smooth=Smooth.None));
  connect(source_N.terminal, sen_N.terminal_n)
    annotation(Line(points={{-60,-30}, {-8,-30}}, color={127, 0, 127}, smooth=Smooth.None));
  connect(sen_N.terminal_p, load_N.terminal)
    annotation(Line(points={{12,-30}, {20,-30}}, color={127, 0, 127}, smooth=Smooth.None));
  connect(sen.terminal_p, load.terminal)
    annotation(Line(points={{16, 30}, {20, 30}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(sen.terminal_n, source.terminal)
    annotation(Line(points={{-4, 30}, {-60, 30}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(ramp.y, load.Pow1)
    annotation(Line(points={{59, 0}, {50, 0}, {50, 38}, {42, 38}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(ramp.y, load.Pow2)
    annotation(Line(points={{59, 0}, {50, 0}, {50, 30}, {42, 30}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(ramp.y, load.Pow3)
    annotation(Line(points={{59, 0}, {50, 0}, {50, 22}, {42, 22}}, color={0, 0, 127}, smooth=Smooth.None));
...
</html>"), experiment(StopTime=1.0, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Electrical/AC/ThreePhasesUnbalanced/Sensors/Examples/Sensors.mos"
    "Simulate and plot"));
end Sensors;
