...
model FixedVoltageSource
  ...
  FixedVoltage grid(f=60, V=480, definiteReference=true, phiSou=0.17453292519943)
  ...
  FixedVoltage_N grid_N(f=60, V=480, definiteReference=true, phiSou=0.17453292519943)
    "AC one phase electrical grid"
...
equation
  connect(grid.terminal, loa.terminal)
    annotation(Line(points={{-20, 40}, {20, 40}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(grid.terminal, sen.term)
    annotation(Line(points={{-20, 40}, {0, 40}, {0, 61}}, color={0, 120, 120}, smooth=Smooth.None));
  connect(grid_N.terminal, loa_N.terminal)
    annotation(Line(points={{-20,-60}, {20,-60}}, color={127, 0, 127}, smooth=Smooth.None));
  connect(grid_N.terminal, sen_N.term)
    annotation(Line(points={{-20,-60}, {0,-60}, {0,-39}}, color={127, 0, 127}, smooth=Smooth.None));
  annotation(experiment(StopTime=1.0, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Electrical/AC/ThreePhasesUnbalanced/Sources/Examples/FixedVoltageSource.mos"
