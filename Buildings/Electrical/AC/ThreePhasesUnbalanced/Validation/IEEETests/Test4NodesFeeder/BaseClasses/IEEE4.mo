...
partial model IEEE4
  ...
  parameter Modelica.SIunits.Angle Theta2_ref[3](
    each displayUnit="deg")={-0.3,-120.3, 119.6}
    "Reference voltage phase angle node 2 - IEEE results";
  parameter Modelica.SIunits.Angle Theta3_ref[3](
    each displayUnit="deg")={-3.7,-123.5, 116.4}
    "Reference voltage phase angle node 3 - IEEE results";
  parameter Modelica.SIunits.Angle Theta4_ref[3](
    each displayUnit="deg")={-9.1,-128.3, 110.9}
  ...
  Modelica.SIunits.Angle err_Theta2[3](
    each displayUnit="deg")=node2.theta-Theta2_ref
    "Error on voltage at node 2";
  Modelica.SIunits.Angle err_Theta3[3](
    each displayUnit="deg")=node3.theta-Theta3_ref
    "Error on voltage at node 3";
  Modelica.SIunits.Angle err_Theta4[3](
    each displayUnit="deg")=node4.theta-Theta4_ref
    "Error on voltage at node 4";
  ...
  Buildings.Electrical.AC.ThreePhasesUnbalanced.Loads.Inductive loadRL(
    ...
    annotation(
      Placement(
        transformation(
          extent={{54, 0}, {74, 20}})));
  replaceable Buildings.Electrical.AC.ThreePhasesUnbalanced.Sensors.BaseClasses.GeneralizedProbe node1(
    perUnit=false,
    V_nominal=VLL_side1)
    "Probe at source"
    annotation(
      Placement(
        transformation(
          extent={{-84, 28}, {-64, 48}})));
  replaceable Buildings.Electrical.AC.ThreePhasesUnbalanced.Sensors.BaseClasses.GeneralizedProbe node2(
    perUnit=false,
    V_nominal=VLL_side1)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-52, 28}, {-32, 48}})));
  replaceable Buildings.Electrical.AC.ThreePhasesUnbalanced.Sensors.BaseClasses.GeneralizedProbe node3(
    perUnit=false,
    V_nominal=VLL_side2)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-4, 28}, {16, 48}})));
  replaceable Buildings.Electrical.AC.ThreePhasesUnbalanced.Sensors.BaseClasses.GeneralizedProbe node4(
    perUnit=false,
    V_nominal=VLL_side2)
    "Probe at the load"
    annotation(
