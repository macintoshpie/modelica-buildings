...
model RCModel
  ...
  TwoPortRCLine RC_ss(C=1, V_nominal=50, R=8)
    "Line resistance"
  ...
  TwoPortRCLine RC_dyn(C=1, V_nominal=50, use_C=true, R=8)
    "Line resistance"
...
equation
  connect(ground.p, constantVoltage.n)
    annotation(Line(points={{-62, 4}, {-62, 20}, {-60, 20}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(constantVoltage.terminal, RC_ss.terminal_n)
    annotation(Line(points={{-40, 20}, {-30, 20}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(RC_ss.terminal_p, sen_ss.terminal_n)
    annotation(Line(points={{-10, 20}, {0, 20}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(sen_ss.terminal_p, sc_ss.terminal)
    annotation(Line(points={{20, 20}, {30, 20}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(RC_dyn.terminal_p, sen_dyn.terminal_n)
    annotation(Line(points={{-8,-10}, {2,-10}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(sen_dyn.terminal_p, sc_dyn.terminal)
    annotation(Line(points={{22,-10}, {32,-10}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(constantVoltage.terminal, RC_dyn.terminal_n)
    annotation(Line(points={{-40, 20}, {-34, 20}, {-34,-10}, {-28,-10}}, color={0, 0, 255}, smooth=Smooth.None));
  annotation(__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Electrical/DC/Lines/Examples/RCModel.mos"
    "Simulate and plot"), experiment(StopTime=15.0, Tolerance=1e-06), Documentation(info="<html>
<p>
