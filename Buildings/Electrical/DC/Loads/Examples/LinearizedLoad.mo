...
model LinearizedLoad
  ...
  Buildings.Electrical.DC.Loads.Conductor NonlinearLoad(linearized=false, mode=Buildings.Electrical.Types.Load.VariableZ_P_input, V_nominal=100, P_nominal=0)
  ...
  Buildings.Electrical.DC.Loads.Conductor LinearLoad(mode=Buildings.Electrical.Types.Load.VariableZ_P_input, V_nominal=100, linearized=true, P_nominal=0)
  ...
  Modelica.Blocks.Sources.Ramp ramp(duration=0.5, startTime=0.2, offset=-50, height=-950)
    "Power consumption"
    annotation(Placement(transformation(extent={{90,-10}, {70, 10}})));
equation
  connect(sou.terminal, Rline1.terminal_n)
    annotation(Line(points={{-70, 0}, {-60, 0}, {-60,-30}, {-50,-30}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(Rline1.terminal_p, sen_nlin.terminal_n)
    annotation(Line(points={{-30,-30}, {-10,-30}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(sen_nlin.terminal_p, NonlinearLoad.terminal)
    annotation(Line(points={{10,-30}, {30,-30}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(LinearLoad.terminal, sen_lin.terminal_p)
    annotation(Line(points={{30, 30}, {10, 30}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(sou.n, gro.p)
    annotation(Line(points={{-90, 0}, {-90,-12}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(sou.terminal, Rline2.terminal_n)
    annotation(Line(points={{-70, 0}, {-60, 0}, {-60, 30}, {-50, 30}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(Rline2.terminal_p, sen_lin.terminal_n)
    annotation(Line(points={{-30, 30}, {-10, 30}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(LinearLoad.Pow, ramp.y)
    annotation(Line(points={{50, 30}, {60, 30}, {60, 6.66134e-16}, {69, 6.66134e-16}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(NonlinearLoad.Pow, ramp.y)
    annotation(Line(points={{50,-30}, {60,-30}, {60, 6.66134e-16}, {69, 6.66134e-16}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(experiment(StopTime=1.0, Tolerance=1e-06), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Electrical/DC/Loads/Examples/LinearizedLoad.mos"
