...
model TestVoltageCTRL
  ...
  VoltageControl voltageControl(redeclare Buildings.Electrical.DC.Interfaces.Terminal_p terminal, redeclare package PhaseSystem=Buildings.Electrical.PhaseSystems.TwoConductor, V_nominal=120, tDelay=2)
    "Voltage controller"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}})));
  DC.Sources.VoltageSource sou
    "Varriable voltage source"
    annotation(Placement(transformation(extent={{-60,-10}, {-40, 10}})));
  Modelica.Electrical.Analog.Basic.Ground ground
    annotation(Placement(transformation(extent={{-70,-36}, {-50,-16}})));
  Modelica.Blocks.Sources.Sine cosine(amplitude=20, freqHz=0.1, phase=0, offset=120)
...
equation
  connect(sou.terminal, voltageControl.terminal)
    annotation(Line(points={{-40, 0}, {-10, 0}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(ground.p, sou.n)
    annotation(Line(points={{-60,-16}, {-60, 0}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(cosine.y, sou.V_in)
    annotation(Line(points={{-73, 6}, {-60, 6}}, color={0, 0, 127}, smooth=Smooth.None));
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Electrical/Utilities/Examples/TestVoltageCTRL.mos"
    "Simulate and plot"), experiment(StopTime=10, Tolerance=1e-6));
end TestVoltageCTRL;
