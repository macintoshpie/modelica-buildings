...
model TrimAndRespond
  ...
  Modelica.Blocks.Sources.Sine act3(
    freqHz=0.002,
    offset=0.2)
    annotation(
      Placement(
        transformation(
          extent={{-80,-20}, {-60, 0}})));
  Modelica.Blocks.Sources.Pulse act1(
    period=500)
    annotation(
      Placement(
        transformation(
          extent={{-80, 60}, {-60, 80}})));
  Modelica.Blocks.Sources.Sine act2(
    freqHz=0.001,
    offset=0.3)
    annotation(
      Placement(
...
equation
  ...
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Examples/ChillerPlant/BaseClasses/Controls/Examples/TrimAndRespond.mos"
        "Simulate and plot"),
    experiment(
      StopTime=3600,
      Tolerance=1e-6),
