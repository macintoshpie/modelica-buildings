...
block TrimAndRespond
  ...
  parameter Real uTri
    "Value to triggering the request for actuator";
  parameter Real yEqu0
    "y setpoint when equipment starts";
  parameter Real yDec(max=0)
    "y decrement (must be negative)";
  parameter Real yInc(min=0)
    "y increment (must be positive)";
  ...
  UnitDelay uniDel1(y_start=yEqu0, samplePeriod=samplePeriod, startTime=samplePeriod)
...
equation
  connect(lim.y, y)
    annotation(Line(points={{41, 6.10623e-16}, {70, 6.10623e-16}, {70, 5.55112e-16}, {110, 5.55112e-16}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(add.y, lim.u)
    annotation(Line(points={{1, 6.10623e-16}, {9.5, 6.10623e-16}, {9.5, 6.66134e-16}, {18, 6.66134e-16}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(uniDel1.y, add.u2)
    annotation(Line(points={{-39,-6}, {-22,-6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(incY.y, swi.u2)
    annotation(Line(points={{1, 60}, {58, 60}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(sam.y, incY.u)
    annotation(Line(points={{-39, 60}, {-22, 60}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(lim.y, uniDel1.u)
    annotation(Line(points={{41, 6.66134e-16}, {60, 6.66134e-16}, {60,-40}, {-80,-40}, {-80,-6}, {-62,-6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(swi.y, add.u1)
    annotation(Line(points={{81, 60}, {88, 60}, {88, 20}, {-30, 20}, {-30, 6}, {-22, 6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(swi.u3, conYDec.y)
    annotation(Line(points={{58, 52}, {50, 52}, {50, 40}, {41, 40}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(conYInc.y, swi.u1)
    annotation(Line(points={{41, 80}, {50, 80}, {50, 68}, {58, 68}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(sam.u, u)
    annotation(Line(points={{-62, 60}, {-80, 60}, {-80, 1.11022e-15}, {-120, 1.11022e-15}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(defaultComponentName="triAndRes", Documentation(info="<html>
<p>
