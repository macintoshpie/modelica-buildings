...
partial model DataCenter
  ...
  Buildings.Fluid.HeatExchangers.DryCoilCounterFlow cooCoi(
    ...
    dp1_nominal(
      displayUnit="Pa")=1000 + 89580)
    "Cooling coil"
    annotation(
      Placement(
        transformation(
          extent={{300,-180}, {280,-160}})));
  Modelica.Blocks.Sources.Constant mFanFlo(
    k=mAir_flow_nominal)
    "Mass flow rate of fan"
    annotation(
      Placement(
        transformation(
          extent={{298,-210}, {318,-190}})));
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10,-10}, {10, 10}},
          origin={-150,-29})));
  Modelica.Blocks.Sources.RealExpression expTowTApp(
    y=cooTow.TApp_nominal)
  ...
  Buildings.Examples.ChillerPlant.BaseClasses.Controls.ChillerSwitch chiSwi(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-226, 83}, {-206, 103}})));
  Buildings.Examples.ChillerPlant.BaseClasses.Controls.LinearPiecewiseTwo linPieTwo(
    x0=0,
    x2=1,
    x1=0.5,
    y11=1,
    ...
    annotation(
      Placement(
        transformation(
          extent={{10,-10}, {-10, 10}},
          origin={330, 119})));
  Modelica.Blocks.Sources.Constant cooTowFanCon(
    k=1)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10,-10}, {10, 10}},
          origin={288, 20})));
  Buildings.Examples.ChillerPlant.BaseClasses.Controls.KMinusU KMinusU(
    k=1)
    annotation(
      Placement(
        transformation(
  ...
  Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaData(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-360,-100}, {-340,-80}})));
  BoundaryConditions.WeatherData.Bus weaBus
    annotation(
      Placement(
        transformation(
          extent={{-332,-98}, {-312,-78}})));
  ...
  Modelica.Blocks.Math.Feedback feedback
    annotation(
      Placement(
        transformation(
          extent={{-210, 190}, {-190, 210}})));
  Modelica.Blocks.Logical.GreaterThreshold greaterThreshold
    annotation(
      Placement(
        transformation(
          extent={{-10, 190}, {10, 210}})));
  Modelica.Blocks.Logical.Or or1
    annotation(
      Placement(
        transformation(
          extent={{20, 190}, {40, 210}})));
  Modelica.Blocks.Math.BooleanToReal mCWFlo(
    realTrue=mCW_flow_nominal)
  ...
  Modelica.Blocks.Sources.RealExpression PHVAC(
    y=fan.P + pumCHW.P + pumCW.P + cooTow.PFan + chi.P)
...
equation
  ...
  connect(linPieTwo.y[2], chi.TSet)
    annotation(
      Line(
        points={{-99, 200.3}, {-82, 200}, {-64, 200}, {-64, 125}, {284, 125}, {284, 90}, {276, 90}},
        color={0, 0, 127},
        smooth=Smooth.None,
        pattern=LinePattern.Dash));
  ...
  connect(wseCon.wseCWST, TCWLeaTow.T)
    annotation(
      Line(
        points={{-162,-36.5294}, {-300,-36.5294}, {-300, 290}, {380, 290}, {380, 137}, {330, 137}, {330, 130}},
        color={0, 0, 127},
        smooth=Smooth.None,
        pattern=LinePattern.Dash));
  ...
  connect(or1.u2, chiSwi.y)
    annotation(
      Line(
        points={{18, 192}, {18, 192}, {16, 192}, {16, 192}, {12, 192}, {12, 128}, {-182, 128}, {-182, 92}, {-205, 92}, {-205, 92.4}},
        color={255, 0, 255},
        pattern=LinePattern.Dash,
        smooth=Smooth.None));
