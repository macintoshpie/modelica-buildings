...
block CoolingCoilTemperatureSetpoint
  ...
  Modelica.Blocks.Sources.RealExpression TSetPoi(
    y=if(mode.y == Integer(OperationModes.occupied) or mode.y == Integer(OperationModes.unoccupiedPreCool) or mode.y == Integer(OperationModes.safety)) then TOn else TOff)
...
equation
  ...
  connect(TSetPoi.y, TSet)
    annotation(
      Line(
        points={{1, 6.10623e-16}, {54.5, 6.10623e-16}, {54.5, 5.55112e-16}, {110, 5.55112e-16}},
        color={0, 0, 127},
        smooth=Smooth.None));
  annotation(
    Icon(
