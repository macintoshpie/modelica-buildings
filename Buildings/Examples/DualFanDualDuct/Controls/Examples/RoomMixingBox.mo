...
model RoomMixingBox
  ...
  Buildings.Examples.DualFanDualDuct.Controls.RoomMixingBox conMix(
    m_flow_min=1)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-20, 0}, {0, 20}})));
  Modelica.Blocks.Sources.Constant TH(
    k=273.15 + 20)
    annotation(
      Placement(
        transformation(
          extent={{-80, 30}, {-60, 50}})));
  Modelica.Blocks.Sources.Constant TC(
    k=273.15 + 26)
    annotation(
      Placement(
        transformation(
  ...
  Modelica.Blocks.Sources.Trapezoid m_flow(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80,-30}, {-60,-10}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.Constant yFan(
    k=true)
    "Fan control signal"
    annotation(
      Placement(
...
equation
  ...
  connect(yFan.y, conMix.yFan)
    annotation(
      Line(
        points={{-59,-50}, {-32,-50}, {-32, 2}, {-22, 2}},
        color={255, 0, 255}));
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Examples/DualFanDualDuct/Controls/Examples/RoomMixingBox.mos"
        "Simulate and plot"),
    experiment(
      StopTime=10800,
      Tolerance=1e-06));
