...
model PreHeatCoil
  ...
  Buildings.Controls.Continuous.LimPID preHeaCoiCon(yMax=1, yMin=0, Td=60, initType=Modelica.Blocks.Types.InitPID.InitialState, Ti=120, strict=true, controllerType=Modelica.Blocks.Types.SimpleController.PI, k=0.1)
...
equation
  connect(preHeaOn.reference, TPreHeaOn.y)
    annotation(Line(points={{-42, 36}, {-52, 36}, {-52, 40}, {-59, 40}}, color={0, 0, 127}));
  connect(zero.y, swiPumPreCoi.u3)
    annotation(Line(points={{1,-30}, {40,-30}, {40, 2}, {58, 2}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(one.y, swiPumPreCoi.u1)
    annotation(Line(points={{1, 0}, {30, 0}, {30, 18}, {58, 18}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TAirSup, preHeaCoiCon.u_m)
    annotation(Line(points={{-110,-60}, {-110,-60}, {-90,-60}, {-90,-80}, {-10,-80}, {-10,-72}}, color={0, 0, 127}));
  connect(TSupSetHea, preHeaCoiCon.u_s)
    annotation(Line(points={{-110, 0}, {-80, 0}, {-80,-60}, {-22,-60}}, color={0, 0, 127}));
  connect(TMix, preHeaOn.u)
    annotation(Line(points={{-110, 60}, {-110, 60}, {-94, 60}, {-94, 24}, {-42, 24}}, color={0, 0, 127}));
  connect(yPum, swiPumPreCoi.y)
    annotation(Line(points={{110, 50}, {88, 50}, {88, 10}, {81, 10}}, color={0, 0, 127}));
  connect(zero.y, swiValOp.u3)
    annotation(Line(points={{1,-30}, {16,-30}, {40,-30}, {40,-68}, {58,-68}}, color={0, 0, 127}));
  connect(swiValOp.y, yVal)
    annotation(Line(points={{81,-60}, {90,-60}, {90,-50}, {98,-50}, {110,-50}}, color={0, 0, 127}));
  ...
  connect(toOff.outPort, pumOff.inPort[1])
    annotation(Line(points={{71.5, 70}, {88, 70}, {88, 92}, {-50, 92}, {-50, 70}, {-41, 70}}, color={0, 0, 0}));
  connect(pumOn.active, swiPumPreCoi.u2)
    annotation(Line(points={{30, 59}, {30, 59}, {30, 52}, {36, 52}, {48, 52}, {48, 10}, {58, 10}}, color={255, 0, 255}));
  connect(pumOn.active, swiValOp.u2)
    annotation(Line(points={{30, 59}, {30, 52}, {48, 52}, {48,-60}, {58,-60}}, color={255, 0, 255}));
  connect(preHeaCoiCon.y, swiValOp.u1)
    annotation(Line(points={{1,-60}, {20,-60}, {20,-52}, {58,-52}}, color={0, 0, 127}));
