...
model TwoRoomsWithStorage
  ...
  Buildings.Fluid.Movers.SpeedControlled_y pumBoi(
    ...
    per(
      pressure(
        V_flow=mBoi_flow_nominal/1000*{0.5, 1},
        dp=(3000 + 2000)*{2, 1})),
  ...
  parameter HeatTransfer.Data.OpaqueConstructions.Insulation100Concrete200 matLayExt
    ...
    annotation(
      Placement(
        transformation(
          extent={{460, 560}, {480, 580}})));
  parameter HeatTransfer.Data.OpaqueConstructions.Brick120 matLayPar
    "Construction material for partition walls"
    annotation(
      Placement(
        transformation(
          extent={{500, 560}, {520, 580}})));
  parameter HeatTransfer.Data.OpaqueConstructions.Generic matLayFlo(
    material={HeatTransfer.Data.Solids.Concrete(
      x=0.2), HeatTransfer.Data.Solids.InsulationBoard(
      x=0.15), HeatTransfer.Data.Solids.Concrete(
      x=0.05)},
    final nLay=3)
  ...
  Buildings.Fluid.Boilers.BoilerPolynomial boi(
    ...
    annotation(
      Placement(
        transformation(
          extent={{2,-180}, {22,-160}})));
  Buildings.HeatTransfer.Sources.FixedTemperature TAmb(
    T=288.15)
    "Ambient temperature in boiler room"
    annotation(
      Placement(
        transformation(
          extent={{-40,-150}, {-20,-130}})));
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor TRoo2
    annotation(
      Placement(
        transformation(
          extent={{480, 216}, {500, 236}})));
  Modelica.Blocks.Sources.Constant pumRadOn(
    k=1)
    "Pump on signal"
    annotation(
      Placement(
  ...
  Buildings.Controls.SetPoints.SupplyReturnTemperatureReset heaCha(
    ...
    annotation(
      Placement(
        transformation(
          extent={{80,-56}, {100,-36}})));
  Controls.SetPoints.OccupancySchedule occSch1(
    occupancy=3600*{7, 8, 10, 11, 11.5, 15, 19, 21})
    "Occupancy schedule"
    annotation(
      Placement(
        transformation(
          extent={{300, 556}, {320, 576}})));
  Modelica.Blocks.Logical.Switch switch1
    annotation(
      Placement(
        transformation(
          extent={{340, 550}, {360, 570}})));
  Modelica.Blocks.Sources.RealExpression occ1(
    y=1/6/4)
    "Heat gain if occupied in room 1"
    annotation(
      Placement(
        transformation(
          extent={{300, 580}, {320, 600}})));
  ...
  Controls.SetPoints.OccupancySchedule occSch2(
    ...
    annotation(
      Placement(
        transformation(
          extent={{300, 286}, {320, 306}})));
  Modelica.Blocks.Logical.Switch switch2
    annotation(
      Placement(
        transformation(
          extent={{340, 280}, {360, 300}})));
  Modelica.Blocks.Sources.RealExpression occ2(
    y=1/6/4)
    "Heat gain if occupied in room 2"
    annotation(
      Placement(
        transformation(
          extent={{300, 310}, {320, 330}})));
  Controls.SetPoints.OccupancySchedule occSch
    "Occupancy schedule"
    annotation(
      Placement(
        transformation(
          extent={{480, 358}, {500, 378}})));
  Modelica.Blocks.Logical.Switch swi
    "Switch to select set point"
    annotation(
      Placement(
        transformation(
          extent={{640, 370}, {660, 390}})));
  Modelica.Blocks.Sources.Constant TRooNig(
    k=273.15 + 16)
    "Room temperature set point at night"
    annotation(
      Placement(
        transformation(
          extent={{480, 330}, {500, 350}})));
  Modelica.Blocks.Sources.Constant TRooSet(
    k=273.15 + 21)
    annotation(
      Placement(
        transformation(
          extent={{480, 390}, {500, 410}})));
  Buildings.Utilities.Math.Max maxYVal(
    nin=2)
  ...
  Modelica.Blocks.Logical.Switch swiPum
    ...
    annotation(
      Placement(
        transformation(
          extent={{80, 60}, {100, 80}})));
  Modelica.Blocks.Sources.Constant pumRadOff(
    k=0)
    "Pump off signal"
    annotation(
      Placement(
  ...
  Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 330}, {-60, 350}})));
  Buildings.BoundaryConditions.WeatherData.Bus weaBus
    "Bus with weather data"
    annotation(
      Placement(
        transformation(
          extent={{-50, 330}, {-30, 350}})));
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10,-10}, {10, 10}},
          origin={330, 214})));
  HeatTransfer.Conduction.MultiLayer parWal(
    A=4*3,
    layers=matLayPar,
  ...
  Buildings.Fluid.Movers.FlowControlled_m_flow fanSup(
    ...
    annotation(
      Placement(
        transformation(
          extent={{70, 490}, {90, 510}})));
  Modelica.Blocks.Sources.Constant m_flow_out(
    k=2*VRoo*1.2*0.37/3600)
  ...
  Airflow.Multizone.Orifice lea2(
    ...
    annotation(
      Placement(
        transformation(
          extent={{320, 170}, {340, 190}})));
  Modelica.Blocks.MathBoolean.Or pumOnSig(
    nu=3)
    "Signal for pump being on"
    annotation(
      Placement(
        transformation(
          extent={{660,-100}, {680,-80}})));
  Modelica.Blocks.Math.BooleanToReal booToReaBoi
    "Signal converter for boiler"
    annotation(
      Placement(
        transformation(
          extent={{420,-100}, {400,-80}})));
  Modelica.Blocks.Math.MatrixGain gai1(
    ...
    annotation(
      Placement(
        transformation(
          extent={{380, 550}, {400, 570}})));
  Modelica.Blocks.Math.MatrixGain gai2(
    K=[
      35;
      70;
      30])
    ...
    annotation(
      Placement(
        transformation(
          extent={{380, 280}, {400, 300}})));
  Modelica.Blocks.Sources.Constant dTThr(
    k=1)
    "Threshold to switch boiler off"
    annotation(
      Placement(
        transformation(
          extent={{310,-210}, {330,-190}})));
  Modelica.Blocks.Math.Add add1(
    k2=-1)
    annotation(
      Placement(
        transformation(
          extent={{350,-186}, {370,-166}})));
  Modelica.Blocks.Sources.Constant TRooOff(
    k=273.15-5)
    "Low room temperature set point to switch heating off"
    annotation(
      Placement(
        transformation(
          extent={{600, 300}, {620, 320}})));
  Modelica.Blocks.Logical.Switch swi1
    "Switch to select set point"
    annotation(
      Placement(
        transformation(
          extent={{540, 380}, {560, 400}})));
  Modelica.Blocks.Logical.OnOffController onOff(
    bandwidth=2)
    "On/off switch"
    annotation(
      Placement(
        transformation(
          extent={{580, 334}, {600, 354}})));
  Modelica.Blocks.Continuous.FirstOrder aveTOut(
    ...
    annotation(
      Placement(
        transformation(
          extent={{540, 300}, {560, 320}})));
  Modelica.Blocks.Sources.Constant TOutSwi(
    k=16 + 293.15)
    "Outside air temperature to switch heating on or off"
    annotation(
      Placement(
        transformation(
          extent={{540, 340}, {560, 360}})));
...
equation
  ...
  connect(conRoo1.y, maxYVal.u[1])
    annotation(
      Line(
        points={{561, 510}, {580, 510}, {580, 420}, {160, 420}, {160, 240}, {-20, 240}, {-20, 69}, {-2, 69}},
        color={0, 0, 127},
        smooth=Smooth.None));
  connect(conRoo2.y, maxYVal.u[2])
    annotation(
      Line(
        points={{561, 250}, {580, 250}, {580, 150}, {160, 150}, {160, 240}, {-20, 240}, {-20, 71}, {-2, 71}},
        color={0, 0, 127},
        smooth=Smooth.None));
  ...
  connect(swi.y, heaCha.TRoo_in)
    annotation(
      Line(
        points={{661, 380}, {680, 380}, {680, 150}, {160, 150}, {160, 240}, {-20, 240}, {-20,-52}, {78.1,-52}},
        color={0, 0, 127},
        smooth=Smooth.None));
  ...
  connect(gai1.y, roo1.qGai_flow)
    annotation(
      Line(
        points={{401, 560}, {410, 560}, {410, 540}, {346, 540}, {346, 494}, {346, 494}, {346, 494}, {354.4, 494}, {354.4, 492}},
        color={0, 0, 127},
        smooth=Smooth.None));
  ...
  connect(TRoo1.T, cooCon.TRoo)
    annotation(
      Line(
        points={{500, 484}, {508, 484}, {508, 530}, {250, 530}, {250, 580}, {90, 580}, {90, 546}, {98, 546}},
        color={0, 0, 127},
        smooth=Smooth.None));
  ...
  connect(coo.port_b, roo1.ports[3])
    annotation(
      Line(
        points={{260, 510}, {338, 510}, {338, 486}, {350, 486}, {350, 476.667}, {361, 476.667}},
        color={0, 127, 255},
        smooth=Smooth.None));
  connect(coo.port_b, roo2.ports[3])
    annotation(
      Line(
        points={{260, 510}, {282, 510}, {282, 228}, {360, 228}, {360, 218.667}, {373, 218.667}},
        color={0, 127, 255},
        smooth=Smooth.None));
  ...
  connect(and1.y, T1.condition)
    annotation(
      Line(
        points={{461,-50}, {480,-50}, {480,-22}},
        color={255, 0, 255}));
  connect(greThr.y, T2.condition)
    annotation(
      Line(
        points={{421,-230}, {452,-230}, {600,-230}, {600,-22}},
        color={255, 0, 255}));
  connect(boiOn.active, booToReaBoi.u)
    annotation(
      Line(
        points={{570,-21}, {570,-90}, {422,-90}},
        color={255, 0, 255}));
  connect(pumOn2.active, pumOnSig.u[1])
    annotation(
      Line(
        points={{630,-21}, {630,-85.3333}, {660,-85.3333}},
        color={255, 0, 255}));
  connect(boiOn.active, pumOnSig.u[2])
    annotation(
      Line(
        points={{570,-21}, {570,-21}, {570,-88}, {570,-90}, {660,-90}},
        color={255, 0, 255}));
  connect(pumOn.active, pumOnSig.u[3])
    annotation(
      Line(
        points={{510,-21}, {510,-94.6667}, {660,-94.6667}},
        color={255, 0, 255}));
  connect(off.outPort[1], T1.inPort)
    annotation(
      Line(
        points={{460.5,-10}, {468.25,-10}, {476,-10}},
        color={0, 0, 0}));
  connect(T1.outPort, pumOn.inPort[1])
    annotation(
      Line(
        points={{481.5,-10}, {499,-10}},
        color={0, 0, 0}));
  connect(pumOn.outPort[1], T3.inPort)
    annotation(
      Line(
        points={{520.5,-10}, {527.25,-10}, {536,-10}},
        color={0, 0, 0}));
  connect(T3.outPort, boiOn.inPort[1])
    annotation(
      Line(
        points={{541.5,-10}, {559,-10}},
        color={0, 0, 0}));
  connect(boiOn.outPort[1], T2.inPort)
    annotation(
      Line(
        points={{580.5,-10}, {596,-10}},
        color={0, 0, 0}));
  connect(T2.outPort, pumOn2.inPort[1])
    annotation(
      Line(
        points={{601.5,-10}, {619,-10}},
        color={0, 0, 0}));
  connect(pumOn2.outPort[1], T4.inPort)
    annotation(
      Line(
        points={{640.5,-10}, {656,-10}},
        color={0, 0, 0}));
  connect(T4.outPort, off.inPort[1])
    annotation(
      Line(
        points={{661.5,-10}, {680,-10}, {680, 20}, {420, 20}, {420,-10}, {439,-10}},
        color={0, 0, 0}));
  annotation(
    Diagram(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-120,-260}, {700, 600}})),
    Documentation(
      info="<html>
<p>
...
First implementation.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Examples/HydronicHeating/TwoRoomsWithStorage.mos"
        "Simulate and plot"),
    experiment(
      StopTime=604800,
      Tolerance=1e-6));
