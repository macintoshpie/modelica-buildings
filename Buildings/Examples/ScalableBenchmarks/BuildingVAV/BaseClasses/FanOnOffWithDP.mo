...
block FanOnOffWithDP
  ...
  Modelica.Blocks.Sources.Constant on(k=preRis)
    "On signal"
    annotation(Placement(transformation(extent={{-40,-10}, {-20, 10}})));
equation
  connect(off.y, extractor.u[Integer(OperationModes.unoccupiedOff)])
    annotation(Line(points={{-19,-40}, {0,-40}, {0, 0}, {18, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(on.y, extractor.u[Integer(OperationModes.unoccupiedNightSetBack)])
    annotation(Line(points={{-19, 0}, {18, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(controlBus.controlMode, extractor.index)
    annotation(Line(points={{-60, 80}, {-60,-20}, {30,-20}, {30,-12}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None), Text(textString="%first", index=-1, extent={{-6, 3}, {-6, 3}}));
  connect(extractor.y, y)
    annotation(Line(points={{41, 0}, {41, 0}, {60, 0}, {110, 0}}, color={0, 0, 127}));
  annotation(defaultComponentName="fanOnOffDp", Icon(graphics={Rectangle(extent={{-100,-100}, {100, 100}}, lineColor={0, 0, 127}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Text(extent={{-90,-50}, {96,-96}}, lineColor={0, 0, 255}, textString="prescribed_dP=%dP_pre"), Text(extent={{-120, 140}, {120, 104}}, textString="%name", lineColor={0, 0, 255})}), Documentation(info="<html>
