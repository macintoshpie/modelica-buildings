...
model OneFloor_OneZone
  ...
  Buildings.Fluid.HeatExchangers.WetCoilCounterFlow cooCoi[nFlo](
    ...
    each UA_nominal=m_flow_nominal*1000*15/Buildings.Fluid.HeatExchangers.BaseClasses.lmtd(
      T_a1=26.2,
      T_b1=12.8,
      T_a2=6,
      T_b2=16),
  ...
  Buildings.Examples.VAVReheat.Controls.Economizer conEco[nFlo](
    ...
    annotation(
      Placement(
        transformation(
          extent={{-288, 88}, {-276, 100}})));
  Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
    filNam=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/weatherdata/USA_IL_Chicago-OHare.Intl.AP.725300_TMY3.mos"))
    annotation(
      Placement(
        transformation(
          extent={{-360, 160}, {-340, 180}})));
  Buildings.BoundaryConditions.WeatherData.Bus weaBus
    "Weather Data Bus"
    annotation(
      Placement(
        transformation(
          extent={{-334, 160}, {-314, 180}})));
  ...
  Buildings.Utilities.Math.Min min1[nFlo](
    ...
    annotation(
      Placement(
        transformation(
          extent={{108, 94}, {120, 106}})));
  Buildings.Examples.VAVReheat.Controls.FanVFD conFanRet[nFlo](
    each xSet_nominal(
      displayUnit="m3/s")=m_flow_nominal/1.2,
    each r_N_min=0.2)
    "Controller for fan"
    annotation(
      Placement(
        transformation(
          extent={{14, 152}, {28, 166}})));
  ...
  Buildings.Controls.SetPoints.OccupancySchedule occSch(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-128, 70}, {-116, 82}})));
  Buildings.Examples.ScalableBenchmarks.BuildingVAV.BaseClasses.FanOnOffWithDP fan_dP_On_Off[nFlo](
    each preRis=dP_pre)
...
equation
  for iFlo in 1 : nFlo loop
    connect(eco[iFlo].port_Sup, TMix[iFlo].port_a)
      annotation(
        Line(
          points={{-232, 22}, {-220, 22}, {-220,-30}, {-208,-30}},
          color={0, 127, 255},
          thickness=0.5));
    connect(TMix[iFlo].port_b, fil[iFlo].port_a)
      annotation(
        Line(
          points={{-192,-30}, {-184,-30}, {-176,-30}},
          color={0, 127, 255},
          thickness=0.5));
    connect(hex[iFlo].port_b2, sinHea[iFlo].ports[1])
      annotation(
        Line(
          points={{-144,-42}, {-148,-42}, {-148,-66}},
          color={0, 127, 255}));
    ...
    connect(TCoiHeaOut[iFlo].T, heaCoiCon[iFlo].u_m)
      annotation(
        Line(
          points={{-95,-21.2}, {-95,-18}, {-106,-18}, {-106,-84}, {-172,-84}, {-172,-74}, {-186,-74}, {-186,-67.2}},
          color={0, 0, 127},
          pattern=LinePattern.Dash));
    connect(TSup[iFlo].T, cooCoiCon[iFlo].u_m)
      annotation(
        Line(
          points={{12,-21.2}, {12,-16}, {0,-16}, {0,-100}, {-186,-100}, {-186,-95.2}},
          color={0, 0, 127},
          pattern=LinePattern.Dash));
    ...
    connect(TRet[iFlo].T, conEco[iFlo].TRet)
      annotation(
        Line(
          points={{-189, 134.8}, {-189, 142}, {-294, 142}, {-294, 98.4}, {-288.8, 98.4}},
          color={0, 0, 127},
          pattern=LinePattern.Dash));
    connect(TMix[iFlo].T, conEco[iFlo].TMix)
      annotation(
        Line(
          points={{-200,-21.2}, {-200,-21.2}, {-200, 106}, {-200, 114}, {-298, 114}, {-298, 96}, {-288.8, 96}},
          color={0, 0, 127},
          pattern=LinePattern.Dash));
    connect(VOut1[iFlo].V_flow, conEco[iFlo].VOut_flow)
      annotation(
        Line(
          points={{-284, 30.8}, {-284, 30.8}, {-284, 54}, {-284, 64}, {-302, 64}, {-302, 93.6}, {-288.8, 93.6}},
          color={0, 0, 127},
          pattern=LinePattern.Dash));
    ...
    connect(conEco[iFlo].yOA, eco[iFlo].y)
      annotation(
        Line(
          points={{-275.6, 95.2}, {-268, 95.2}, {-268, 6}, {-247, 6}, {-247, 13}},
          color={0, 0, 127},
          pattern=LinePattern.Dash));
    ...
    connect(senRetFlo[iFlo].V_flow, conFanRet[iFlo].u_m)
      annotation(
        Line(
          points={{20, 134.8}, {20, 150.6}, {21, 150.6}},
          color={0, 0, 127},
          pattern=LinePattern.Dash));
    connect(conFanRet[iFlo].u, senSupFlo[iFlo].V_flow)
      annotation(
        Line(
          points={{12.6, 159}, {0, 159}, {0, 0}, {40, 0}, {40,-21.2}},
          color={0, 0, 127},
          pattern=LinePattern.Dash));
    connect(conFanRet[iFlo].y, fanRet[iFlo].y)
      annotation(
        Line(
          points={{28.7, 159}, {36, 159}, {36, 180}, {-20, 180}, {-20, 138}},
          color={0, 0, 127},
          pattern=LinePattern.Dash));
    ...
    connect(controlBus[iFlo], conEco[iFlo].controlBus)
      annotation(
        Line(
          points={{-68, 54}, {-68, 54}, {-134, 54}, {-134, 104}, {-285.6, 104}, {-285.6, 94.4}},
          color={255, 204, 51},
          thickness=0.5));
    connect(controlBus[iFlo], modeSelector[iFlo].cb)
      annotation(
        Line(
          points={{-68, 54}, {-121.728, 54}, {-121.728, 53.4545}, {-175.455, 53.4545}},
          color={255, 204, 51},
          thickness=0.5));
    connect(controlBus[iFlo], fan_dP_On_Off[iFlo].controlBus)
      annotation(
        Line(
          points={{-68, 54}, {-68, 54}, {-68,-1.4}, {-67.2,-1.4}},
          color={255, 204, 51},
          thickness=0.5),
        Text(
          textString="%first",
    ...
    connect(TSetHea.y[1], controlBus[iFlo].TRooSetHea)
      annotation(
        Line(
          points={{-117.4, 36}, {-92, 36}, {-92, 54.05}, {-67.95, 54.05}},
          color={255, 204, 51},
          thickness=0.5),
        Text(
          textString="%second",
    ...
    connect(TSetCoo1.y[1], controlBus[iFlo].TRooSetCoo)
      annotation(
        Line(
          points={{-117.4, 16}, {-92, 16}, {-92, 54.05}, {-67.95, 54.05}},
          color={255, 204, 51},
          thickness=0.5),
        Text(
          textString="%second",
    ...
    connect(occSch.tNexOcc, controlBus[iFlo].dTNexOcc)
      annotation(
        Line(
          points={{-115.4, 79.6}, {-92, 79.6}, {-92, 54.05}, {-67.95, 54.05}},
          color={255, 204, 57},
          thickness=0.5),
        Text(
          textString="%second",
    ...
    connect(occSch.occupied, controlBus[iFlo].occupied)
      annotation(
        Line(
          points={{-115.4, 72.4}, {-92, 72.4}, {-92, 54.05}, {-67.95, 54.05}},
          color={255, 204, 51},
          thickness=0.5),
        Text(
          textString="%second",
    ...
    connect(TSup[iFlo].port_b, senSupFlo[iFlo].port_a)
      annotation(
        Line(
          points={{20,-30}, {20,-30}, {32,-30}},
          color={0, 127, 255},
          thickness=0.5));
    connect(fan_dP_On_Off[iFlo].y, fan[iFlo].dp_in)
      annotation(
        Line(
          points={{-55.3,-7}, {-30,-7}, {-30,-18}},
          color={0, 0, 127}));
  end for;
  for iFlo in 1 : nFlo loop
    for iZon in 1 : nZon loop
      ...
      connect(vavTer[iZon, iFlo].TRooHeaSet, controlBus[iFlo].TRooSetHea)
        annotation(
          Line(
            points={{50, 30}, {-68, 30}, {-68, 30}, {-67.95, 30}, {-67.95, 54.05}},
            color={0, 0, 127}),
          Text(
            textString="%second",
            index=1,
      ...
      connect(vavTer[iZon, iFlo].TRooCooSet, controlBus[iFlo].TRooSetCoo)
        annotation(
          Line(
            points={{50, 26}, {-68, 26}, {-68, 28}, {-67.95, 28}, {-67.95, 54.05}},
            color={0, 0, 127}),
          Text(
            textString="%second",
            index=1,
  ...
  connect(modeSelector.yFan, conFanRet.uFan)
    annotation(
      Line(
        points={{-161.636, 48}, {-161.636, 48}, {-146, 48}, {-146, 163.2}, {12.6, 163.2}},
        color={255, 0, 255}));
  annotation(
    experiment(
      StopTime=604800,
      Tolerance=1e-06),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Examples/ScalableBenchmarks/BuildingVAV/Examples/OneFloor_OneZone.mos"
        "Simulate and plot"),
    Diagram(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-360,-120}, {140, 200}})),
    Documentation(
      info="<html>
<p>
