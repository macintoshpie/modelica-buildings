...
model System7
  ...
  Buildings.Fluid.FixedResistances.Junction spl2(
    ...
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
    dp_nominal={0, 0, 0},
    m_flow_nominal={mRad_flow_nominal,-mRadVal_flow_nominal,-mRad_flow_nominal + mRadVal_flow_nominal})
    annotation(
      Placement(
        transformation(
          extent={{-10,-10}, {10, 10}},
  ...
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor senTOut
    ...
    annotation(
      Placement(
        transformation(
          extent={{-318, 20}, {-298, 40}})));
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal booToReaRad1(
  ...
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal booToReaRad2(
    realTrue=1)
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant TSetBoiRet(
    k=TBoiRet_min)
    "Temperature setpoint for boiler return"
    annotation(
      Placement(
        transformation(
          extent={{120,-270}, {140,-250}})));
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant TSupMax(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-260,-40}, {-240,-20}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant TRooMin(
    k=273.15 + 19)
  ...
  BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-380, 60}, {-360, 80}})));
  BoundaryConditions.WeatherData.Bus weaBus
    annotation(
      Placement(
        transformation(
          extent={{-320, 60}, {-300, 80}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature TOut
    "Outside temperature"
    annotation(
      Placement(
        transformation(
          extent={{-260, 60}, {-240, 80}})));
  ...
  Modelica.StateGraph.StepWithSignal boilerOn(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-228,-200}, {-208,-180}})));
  Modelica.StateGraph.TransitionWithSignal T2
    "Transition to boiler on"
    annotation(
      Placement(
        transformation(
          extent={{-258,-200}, {-238,-180}})));
  Buildings.Controls.OBC.CDL.Continuous.GreaterThreshold greThrBoi(
    threshold=273.15 + 90)
    "Threshold for boiler control"
    annotation(
      Placement(
        transformation(
          extent={{-420,-320}, {-400,-300}})));
  ...
  Modelica.StateGraph.TransitionWithSignal T4(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-270,-160}, {-250,-140}})));
  Modelica.StateGraph.Step pumpsOn(
    nIn=1,
    nOut=1)
    "Pumps are on"
    annotation(
  ...
  Buildings.Controls.OBC.CDL.Continuous.LessThreshold lessThreshold(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-420,-210}, {-400,-190}})));
  Buildings.Controls.OBC.CDL.Continuous.LessThreshold lessThreshold1(
    threshold=273.15 + 16)
    annotation(
      Placement(
        transformation(
          extent={{-420,-240}, {-400,-220}})));
  Buildings.Controls.OBC.CDL.Logical.And and3
    annotation(
      Placement(
        transformation(
          extent={{-380,-232}, {-360,-212}})));
  Buildings.Controls.OBC.CDL.Continuous.LessThreshold lessThreshold2(
    threshold=273.15 + 70)
    "Threshold for boiler control"
    annotation(
      Placement(
        transformation(
          extent={{-420,-290}, {-400,-270}})));
  Buildings.Controls.OBC.CDL.Continuous.GreaterThreshold greThrTRoo(
    threshold=273.15 + 21)
  ...
  Buildings.Controls.OBC.CDL.Continuous.GreaterThreshold greThrTROut(
    threshold=273.15 + 17)
    "Threshold for room temperature"
    annotation(
      Placement(
        transformation(
          extent={{-420,-160}, {-400,-140}})));
  Buildings.Controls.OBC.CDL.Logical.And and1
    annotation(
      Placement(
        transformation(
          extent={{-380,-152}, {-360,-132}})));
  Buildings.Controls.OBC.CDL.Logical.Not not1
    "Negate output of hysteresis"
    annotation(
      Placement(
        transformation(
          extent={{-180,-100}, {-160,-80}})));
equation
  ...
  connect(TSetBoiRet.y, conPIDBoi.u_s)
    annotation(
      Line(
        points={{142,-260}, {158,-260}},
        color={0, 0, 127}));
  connect(temRet.T, conPIDBoi.u_m)
    annotation(
      Line(
        points={{71,-280}, {120,-280}, {170,-280}, {170,-272}},
        color={0, 0, 127}));
  connect(T1.condition, and3.y)
    annotation(
      Line(
        points={{-200,-162}, {-200,-162}, {-200,-222}, {-358,-222}},
        color={255, 0, 255}));
  connect(booToReaRad2.u, boilerOn.active)
    annotation(
      Line(
        points={{-102,-330}, {-102,-330}, {-218,-330}, {-218,-201}},
        color={255, 0, 255}));
  connect(lessThreshold2.y, T2.condition)
    annotation(
      Line(
        points={{-398,-280}, {-398,-280}, {-332,-280}, {-248,-280}, {-248,-202}},
        color={255, 0, 255}));
  connect(T4.condition, and1.y)
    annotation(
      Line(
        points={{-260,-162}, {-260,-162}, {-260,-170}, {-260,-216}, {-338,-216}, {-338,-142}, {-358,-142}},
        color={255, 0, 255}));
  connect(T3.condition, greThrBoi.y)
    annotation(
      Line(
        points={{-186,-202}, {-186,-202}, {-186,-310}, {-398,-310}},
        color={255, 0, 255}));
  connect(allOff.active, not1.u)
    annotation(
      Line(
        points={{-230,-161}, {-230,-166}, {-210,-166}, {-210,-90}, {-182,-90}},
        color={255, 0, 255}));
  connect(pumpsOn.outPort[1], alternative.inPort)
    annotation(
      Line(
        points={{-299.5,-170}, {-292.04,-170}},
        color={0, 0, 0}));
  connect(alternative.outPort, pumpsOn.inPort[1])
    annotation(
      Line(
        points={{-152.64,-170}, {-146,-170}, {-146,-120}, {-328,-120}, {-328,-170}, {-321,-170}},
        color={0, 0, 0}));
  connect(T4.inPort, alternative.split[1])
    annotation(
      Line(
        points={{-264,-150}, {-264,-170}, {-275.72,-170}},
        color={0, 0, 0}));
  connect(T2.inPort, alternative.split[2])
    annotation(
      Line(
        points={{-252,-190}, {-275.72,-190}, {-275.72,-170}},
        color={0, 0, 0}));
  connect(T1.outPort, alternative.join[1])
    annotation(
      Line(
        points={{-198.5,-150}, {-184,-150}, {-184,-170}, {-168.28,-170}},
        color={0, 0, 0}));
  connect(T3.outPort, alternative.join[2])
    annotation(
      Line(
        points={{-184.5,-190}, {-176,-190}, {-176,-170}, {-168.28,-170}},
        color={0, 0, 0}));
  connect(TSetSup.x1, TRooMin.y)
    annotation(
      Line(
        points={{-222,-52}, {-230,-52}, {-230, 10}, {-238, 10}},
        color={0, 0, 127}));
  connect(TSupMax.y, TSetSup.f1)
    annotation(
      Line(
        points={{-238,-30}, {-234,-30}, {-234,-56}, {-222,-56}},
        color={0, 0, 127}));
  connect(TSupMin.y, TSetSup.f2)
    annotation(
      Line(
        points={{-238,-90}, {-230,-90}, {-230,-68}, {-222,-68}},
        color={0, 0, 127}));
  connect(TSupMin.y, TSetSup.x2)
    annotation(
      Line(
        points={{-238,-90}, {-230,-90}, {-230,-64}, {-222,-64}},
        color={0, 0, 127}));
  connect(TSetSup.u, temRoo.T)
    annotation(
      Line(
        points={{-222,-60}, {-270,-60}, {-270, 30}, {-50, 30}},
        color={0, 0, 127}));
  connect(conPIDRad.u_s, TSetSup.y)
    annotation(
      Line(
        points={{-182,-10}, {-188,-10}, {-188,-60}, {-198,-60}},
        color={0, 0, 127}));
  annotation(
    Documentation(
      info="<html>
...
First implementation.
    ...
    Diagram(
      coordinateSystem(
        ...
        extent={{-460,-360}, {240, 100}})),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Examples/Tutorial/Boiler/System7.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=172800));
