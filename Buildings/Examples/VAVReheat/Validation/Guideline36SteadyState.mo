...
model Guideline36SteadyState
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant solRad(
    k=0)
    "Solar radiation"
    annotation(
      Placement(
        transformation(
          extent={{-400, 160}, {-380, 180}})));
equation
  connect(weaDat.HDifHor_in, solRad.y)
    annotation(
      Line(
        points={{-361, 170.5}, {-370, 170.5}, {-370, 170}, {-378, 170}},
        color={0, 0, 127}));
  connect(weaDat.HGloHor_in, solRad.y)
    annotation(
      Line(
        points={{-361, 167}, {-370, 167}, {-370, 170}, {-378, 170}},
        color={0, 0, 127}));
  annotation(
    experiment(
      StopTime=604800,
      Tolerance=1e-06),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Examples/VAVReheat/Validation/Guideline36SteadyState.mos"
        "Simulate and plot"),
    Diagram(
      coordinateSystem(
        extent={{-420,-300}, {1360, 660}})),
    Icon(
      coordinateSystem(
