...
model SubStationBoundaryCondition
  ...
  Modelica.Blocks.Math.Add add(
    k1=if warmSide then-1 else + 1)
    annotation(
      Placement(
        transformation(
          extent={{-50,-4}, {-30, 16}})));
  Modelica.Blocks.Sources.Constant TAppr(
    k=dTApp)
    "Approach temperature"
    annotation(
      Placement(
...
equation
  ...
  connect(add.u2, TOut)
    annotation(
      Line(
        points={{-52, 0}, {-82, 0}, {-120, 0}},
        color={0, 0, 127}));
  connect(add.y, bou.T_in)
    annotation(
      Line(
        points={{-29, 6}, {-22, 6}, {-22, 4}, {-12, 4}},
        color={0, 0, 127}));
  connect(TAppr.y, add.u1)
    annotation(
      Line(
        points={{-59, 50}, {-56, 50}, {-56, 12}, {-52, 12}},
        color={0, 0, 127}));
  connect(add.y, preTem.T)
    annotation(
      Line(
        points={{-29, 6}, {-22, 6}, {-22,-60}, {-12,-60}},
        color={0, 0, 127}));
  annotation(
    Icon(
      graphics={Ellipse(
