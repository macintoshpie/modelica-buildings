  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort temCol(
    ...
    annotation(
      Placement(
        transformation(
          extent={{20,-50}, {0,-30}})));
  replaceable Modelica.Blocks.Sources.Constant TWatWar(
    k=273.15 + 4) constrainedby Modelica.Blocks.Interfaces.SO
    "Water temperature"
    annotation(
      Placement(
        transformation(
          extent={{90, 12}, {70, 32}})));
  Modelica.Blocks.Sources.Constant TWatSou(
    k=273.15 + 15)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 70}, {-60, 90}})));
  Modelica.Blocks.Sources.Constant TSouHea(
    k=273.15 + 20)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 40}, {-60, 60}})));
  Modelica.Blocks.Sources.Constant TSouCoo(
    k=273.15 + 18)
...
equation
  connect(hex.TSetHea, TSetH.y)
    annotation(
      Line(
        points={{-32, 6}, {-54, 6}, {-54,-30}, {-59,-30}},
        color={0, 0, 127}));
  connect(TSetC.y, hex.TSetCoo)
    annotation(
      Line(
        points={{-59,-70}, {-59,-70}, {-50,-70}, {-50, 2}, {-32, 2}},
        color={0, 0, 127}));
  connect(hex.port_b2, hex.port_a1)
    annotation(
      Line(
        points={{-30,-16}, {-36,-16}, {-36,-4}, {-30,-4}},
        color={0, 127, 255}));
  connect(TWatCol.y, floSou.T_in)
    annotation(
      Line(
        points={{69,-60}, {60,-60}, {60,-36}, {52,-36}},
        color={0, 0, 127}));
  connect(m_flow.y, floSou.m_flow_in)
    annotation(
      Line(
        points={{69,-20}, {60,-20}, {60,-32}, {50,-32}},
        color={0, 0, 127}));
  connect(hex.port_b1, temWar.port_a)
    annotation(
      Line(
        points={{-10,-4}, {-6,-4}, {-6, 0}, {0, 0}},
        color={0, 127, 255}));
  connect(temWar.port_b, bou.ports[1])
    annotation(
      Line(
        points={{20, 0}, {30, 0}, {32, 0}},
        color={0, 127, 255}));
  connect(temCol.port_a, floSou.ports[1])
    annotation(
      Line(
        points={{20,-40}, {25,-40}, {30,-40}},
        color={0, 127, 255}));
  connect(temCol.port_b, hex.port_a2)
    annotation(
      Line(
        points={{0,-40}, {-4,-40}, {-4,-16}, {-10,-16}},
        color={0, 127, 255}));
  connect(TWatWar.y, bou.T_in)
    annotation(
      Line(
        points={{69, 22}, {60, 22}, {60, 4}, {54, 4}},
        color={0, 0, 127}));
  connect(TWatSou.y, hex.TSouWat)
    annotation(
      Line(
        points={{-59, 80}, {-40, 80}, {-40, 18}, {-34, 18}, {-34, 18}, {-32, 18}, {-32, 18}},
        color={0, 0, 127}));
  connect(TSouHea.y, hex.TSouHea)
    annotation(
      Line(
        points={{-59, 50}, {-44, 50}, {-44, 14}, {-32, 14}},
        color={0, 0, 127}));
  connect(TSouCoo.y, hex.TSouCoo)
    annotation(
      Line(
        points={{-59, 20}, {-48, 20}, {-48, 10}, {-32, 10}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=3600),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/Plants/Validation/LakeWaterHeatExchanger_T_Heating.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
