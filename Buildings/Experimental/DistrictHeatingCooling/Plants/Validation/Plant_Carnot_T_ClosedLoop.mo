...
equation
  ...
  connect(pum.port_b, vol.ports[1])
    annotation(
      Line(
        points={{10, 0}, {20, 0}, {32, 0}, {32,-28}, {50,-28}},
        color={0, 127, 255}));
  connect(vol.ports[2], pla.port_a)
    annotation(
      Line(
        points={{50,-32}, {0,-32}, {-56,-32}, {-56, 0}, {-50, 0}},
        color={0, 127, 255}));
  ...
  connect(heaFlo.port, vol.heatPort)
    annotation(
      Line(
        points={{78, 80}, {90, 80}, {90, 20}, {60, 20}, {60,-20}},
        color={191, 0, 0}));
  connect(m_flow.y, pum.m_flow_in)
    annotation(
      Line(
        points={{-19, 80}, {-0.2, 80}, {-0.2, 12}},
        color={0, 0, 127}));
  connect(TSetC.y, pla.TSetCoo)
    annotation(
      Line(
        points={{-69, 20}, {-62, 20}, {-62, 4}, {-52, 4}},
        color={0, 0, 127}));
  connect(pla.TSetHea, TSetH.y)
    annotation(
      Line(
        points={{-52, 8}, {-60, 8}, {-60, 50}, {-69, 50}},
        color={0, 0, 127}));
  connect(TOut.y[1], pla.TSink)
    annotation(
      Line(
        points={{-71,-10}, {-62,-10}, {-62,-6}, {-52,-6}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=259200),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/Plants/Validation/Plant_Carnot_T_ClosedLoop.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
