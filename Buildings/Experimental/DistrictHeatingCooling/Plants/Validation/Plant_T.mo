...
equation
  connect(TWar.y, war.T_in)
    annotation(
      Line(
        points={{-59, 50}, {44, 50}, {44, 42}},
        color={0, 0, 127}));
  connect(TCoo.y, coo.T_in)
    annotation(
      Line(
        points={{-59,-70}, {-24,-70}, {-24,-52}},
        color={0, 0, 127}));
  connect(war.ports[1], pla.port_b)
    annotation(
      Line(
        points={{40, 20}, {40, 20}, {40, 0}, {20, 0}},
        color={0, 127, 255}));
  connect(m_flow.y, war.m_flow_in)
    annotation(
      Line(
        points={{-59, 80}, {-10, 80}, {48, 80}, {48, 40}},
        color={0, 0, 127}));
  connect(coo.ports[1], pla.port_a)
    annotation(
      Line(
        points={{-20,-30}, {-20,-30}, {-20, 0}, {0, 0}},
        color={0, 127, 255}));
  connect(pla.TSetHea, TSetH.y)
    annotation(
      Line(
        points={{-2, 8}, {-40, 8}, {-40, 10}, {-59, 10}},
        color={0, 0, 127}));
  connect(TSetC.y, pla.TSetCoo)
    annotation(
      Line(
        points={{-59,-20}, {-40,-20}, {-40, 4}, {-2, 4}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=86400),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/Plants/Validation/Plant_T.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
