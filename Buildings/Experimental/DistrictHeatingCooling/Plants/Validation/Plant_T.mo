...
model Plant_T
  ...
  Buildings.Fluid.Sources.MassFlowSource_T war(redeclare package Medium=Medium, use_T_in=true, use_m_flow_in=true, nPorts=1)
    "Warm pipe"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=270, origin={40, 30})));
  Buildings.Fluid.Sources.Boundary_pT coo(redeclare package Medium=Medium, use_T_in=true, nPorts=1)
    "Cool pipe"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=90, origin={-20,-40})));
  Modelica.Blocks.Sources.Ramp TWar(height=6, duration=86400, offset=273.15 + 12)
    "Temperature of warm supply"
    annotation(Placement(transformation(extent={{-80, 40}, {-60, 60}})));
  Modelica.Blocks.Sources.Ramp TCoo(duration=86400, height=12, offset=273.15 + 4)
    "Temperature of cold supply"
    annotation(Placement(transformation(extent={{-80,-80}, {-60,-60}})));
  Modelica.Blocks.Sources.Ramp m_flow(duration=86400, height=2*m_flow_nominal, offset=-m_flow_nominal)
    "Mass flow rate in warm pipe"
...
equation
  connect(TWar.y, war.T_in)
    annotation(Line(points={{-59, 50}, {44, 50}, {44, 42}}, color={0, 0, 127}));
  connect(TCoo.y, coo.T_in)
    annotation(Line(points={{-59,-70}, {-24,-70}, {-24,-52}}, color={0, 0, 127}));
  connect(war.ports[1], pla.port_b)
    annotation(Line(points={{40, 20}, {40, 20}, {40, 0}, {20, 0}}, color={0, 127, 255}));
  connect(m_flow.y, war.m_flow_in)
    annotation(Line(points={{-59, 80}, {-10, 80}, {48, 80}, {48, 40}}, color={0, 0, 127}));
  connect(coo.ports[1], pla.port_a)
    annotation(Line(points={{-20,-30}, {-20,-30}, {-20, 0}, {0, 0}}, color={0, 127, 255}));
  connect(pla.TSetHea, TSetH.y)
    annotation(Line(points={{-2, 8}, {-40, 8}, {-40, 10}, {-59, 10}}, color={0, 0, 127}));
  connect(TSetC.y, pla.TSetCoo)
    annotation(Line(points={{-59,-20}, {-40,-20}, {-40, 4}, {-2, 4}}, color={0, 0, 127}));
  annotation(experiment(Tolerance=1e-6, StopTime=86400), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/Plants/Validation/Plant_T.mos"
