...
model Plant_T_ClosedLoop
  ...
  Ideal_T pla(redeclare package Medium=Medium, show_T=true, m_flow_nominal=m_flow_nominal)
    "Heating and cooling plant"
    annotation(Placement(transformation(extent={{-50,-10}, {-30, 10}})));
  Buildings.Fluid.Sources.Boundary_pT pre(redeclare package Medium=Medium, nPorts=1)
    "Pressure source"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=90, origin={-60,-62})));
  Buildings.Fluid.MixingVolumes.MixingVolume vol(nPorts=2, redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, V=0.25*3600*m_flow_nominal/1000)
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=270, origin={60,-2})));
  Buildings.Fluid.Movers.FlowControlled_m_flow pum(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, addPowerToMedium=false, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState)
    "Pump"
...
equation
  connect(pre.ports[1], pla.port_a)
    annotation(Line(points={{-60,-52}, {-60,-52}, {-60, 0}, {-50, 0}}, color={0, 127, 255}));
  connect(pla.port_b, pum.port_a)
    annotation(Line(points={{-30, 0}, {-10, 0}}, color={0, 127, 255}));
  connect(pum.port_b, vol.ports[1])
    annotation(Line(points={{10, 0}, {20, 0}, {32, 0}, {32, 1.77636e-15}, {50, 1.77636e-15}}, color={0, 127, 255}));
  connect(vol.ports[2], pla.port_a)
    annotation(Line(points={{50,-4}, {50,-4}, {40,-4}, {40,-4}, {40,-4}, {40,-20}, {-60,-20}, {-60, 0}, {-50, 0}}, color={0, 127, 255}));
  ...
  connect(heaFlo.Q_flow, Q_flow.y)
    annotation(Line(points={{58, 80}, {58, 80}, {41, 80}}, color={0, 0, 127}));
  connect(heaFlo.port, vol.heatPort)
    annotation(Line(points={{78, 80}, {90, 80}, {90, 20}, {60, 20}, {60, 8}}, color={191, 0, 0}));
  connect(m_flow.y, pum.m_flow_in)
    annotation(Line(points={{-19, 80}, {0, 80}, {0, 12}}, color={0, 0, 127}));
  connect(TSetC.y, pla.TSetCoo)
    annotation(Line(points={{-69, 10}, {-62, 10}, {-62, 4}, {-52, 4}}, color={0, 0, 127}));
  connect(pla.TSetHea, TSetH.y)
    annotation(Line(points={{-52, 8}, {-60, 8}, {-60, 40}, {-69, 40}}, color={0, 0, 127}));
  connect(pla.QCoo_flow, ECoo.u)
    annotation(Line(points={{-29, 4}, {-24, 4}, {-20, 4}, {-20,-80}, {18,-80}}, color={0, 0, 127}));
  connect(pla.QHea_flow, EHea.u)
    annotation(Line(points={{-29, 6}, {-22, 6}, {-12, 6}, {-12,-40}, {18,-40}}, color={0, 0, 127}));
  annotation(experiment(Tolerance=1e-6, StopTime=172800), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/Plants/Validation/Plant_T_ClosedLoop.mos"
