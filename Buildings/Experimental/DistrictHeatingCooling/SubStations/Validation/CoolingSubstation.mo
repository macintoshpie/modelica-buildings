  ...
  Buildings.Experimental.DistrictHeatingCooling.SubStations.Cooling subSta(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-8, 0}, {12, 20}})));
  Modelica.Blocks.Sources.TimeTable QCoo(
    table=[
      0,-100E3;
      6,-100E3;
      6,-50E3;
...
equation
  ...
  connect(coo.ports[1], subSta.port_a)
    annotation(
      Line(
        points={{-20,-32}, {-20,-32}, {-20, 10}, {-8, 10}},
        color={0, 127, 255}));
  connect(subSta.port_b, war.ports[1])
    annotation(
      Line(
        points={{12, 10}, {32, 10}, {32, 40}},
        color={0, 127, 255}));
  connect(QCoo.y, subSta.Q_flow)
    annotation(
      Line(
        points={{-59, 16}, {-34, 16}, {-10, 16}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=86400),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/SubStations/Validation/CoolingSubstation.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
