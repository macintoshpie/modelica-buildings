  ...
  Modelica.Blocks.Sources.Ramp TWar(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 64}, {-60, 84}})));
  Modelica.Blocks.Sources.TimeTable QHea(
    table=[
      0, 100E3;
      6, 100E3;
      6, 50E3;
...
equation
  connect(TWar.y, war.T_in)
    annotation(
      Line(
        points={{-59, 74}, {-8, 74}, {-8, 62}},
        color={0, 0, 127}));
  connect(war.ports[1], subSta.port_a)
    annotation(
      Line(
        points={{-12, 40}, {-12, 40}, {-12, 10}, {-6, 10}, {0, 10}},
        color={0, 127, 255}));
  connect(subSta.port_b, coo.ports[1])
    annotation(
      Line(
        points={{20, 10}, {30, 10}, {30,-16}, {30,-24}, {30,-30}},
        color={0, 127, 255}));
  connect(TCoo.y, coo.T_in)
    annotation(
      Line(
        points={{-59,-70}, {26,-70}, {26,-52}},
        color={0, 0, 127}));
  connect(QHea.y, subSta.Q_flow)
    annotation(
      Line(
        points={{-59, 16}, {-30, 16}, {-2, 16}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=86400),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/SubStations/Validation/HeatingSubstation.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
