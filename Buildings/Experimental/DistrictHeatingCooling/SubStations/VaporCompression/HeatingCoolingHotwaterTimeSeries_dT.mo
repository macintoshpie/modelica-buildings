...
model HeatingCoolingHotwaterTimeSeries_dT
  ...
  parameter Real gaiCoo(
    min=0)=1
    "Gain to scale cooling load";
  parameter Real gaiHea(
    min=0)=gaiCoo
    "Gain to scale heating load";
  parameter Real gaiHotWat(
    min=0)=gaiHea
  ...
  parameter String filNam
    ...
    annotation(
      Dialog(
        loadSelector(
          filter="Load file (*.mos)",
          caption="Select load file")));
  parameter Modelica.SIunits.HeatFlowRate QCoo_flow_nominal(
    max=-Modelica.Constants.eps)=gaiCoo*Buildings.Experimental.DistrictHeatingCooling.SubStations.VaporCompression.BaseClasses.getPeakLoad(
  ...
  parameter Modelica.SIunits.HeatFlowRate QHotWat_flow_nominal(
    min=Modelica.Constants.eps)=gaiHotWat*Buildings.Experimental.DistrictHeatingCooling.SubStations.VaporCompression.BaseClasses.getPeakLoad(
  ...
  parameter Modelica.SIunits.Temperature TOut_nominal
    ...
    annotation(
      Dialog(
        group="Nominal conditions"));
  parameter Modelica.SIunits.TemperatureDifference dTHotWatCon_nominal(
    min=0)=60-40
  ...
  parameter Modelica.SIunits.Pressure dp_nominal(
    ...
    annotation(
      Dialog(
        group="Design parameter"));
  final parameter Modelica.SIunits.MassFlowRate mCooCon_flow_nominal(
    min=0)=-QCoo_flow_nominal/cp_default/dTCooCon_nominal
    "Design mass flow rate for cooling at district side"
    annotation(
      Dialog(
        group="Design parameter"));
  final parameter Modelica.SIunits.MassFlowRate mHeaEva_flow_nominal(
    min=0)=-QHea_flow_nominal/cp_default/dTHeaEva_nominal
    "Design mass flow rate for space heating at district side"
    annotation(
      Dialog(
        group="Design parameter"));
  final parameter Modelica.SIunits.MassFlowRate mHotWatEva_flow_nominal(
    min=0)=QHotWat_flow_nominal/cp_default/dTHotWatCon_nominal
  ...
  Modelica.Blocks.Interfaces.RealOutput PHea(
    ...
    annotation(
      Placement(
        transformation(
          extent={{280, 270}, {300, 290}})));
  Modelica.Blocks.Interfaces.RealOutput PHotWat(
    unit="W")
    "Electrical power consumed for hot water"
    annotation(
      Placement(
        transformation(
          extent={{280, 230}, {300, 250}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput QCoo_flow(
    ...
    annotation(
      Placement(
        transformation(
          extent={{280, 50}, {300, 70}})));
  BoundaryConditions.WeatherData.Bus weaBus
    annotation(
      Placement(
        transformation(
          extent={{-20, 360}, {20, 400}}),
        iconTransformation(
          extent={{-10, 224}, {10, 244}})));
...
protected
  ...
  Modelica.Blocks.Sources.CombiTimeTable loa(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-260, 400}, {-240, 420}})));
  Modelica.Blocks.Routing.DeMultiplex3 deMul
    "De multiplex"
    annotation(
      Placement(
        transformation(
          extent={{-178, 400}, {-158, 420}})));
    ...
    annotation(
      Placement(
        transformation(
          extent={{10,-10}, {-10, 10}},
          origin={70, 230})));
  Modelica.Blocks.Math.Add dTHeaAct(
    k2=-1)
    "Temperature difference for heating"
    annotation(
      Placement(
        transformation(
          extent={{-220, 160}, {-200, 180}})));
  Modelica.Blocks.Math.Division mConFlow
    "Mass flow rate for condenser"
    annotation(
      Placement(
        transformation(
          extent={{-80, 160}, {-60, 180}})));
  Modelica.Blocks.Math.Gain gainH(
    k=cp_default)
    "Gain for heating"
    annotation(
      Placement(
        transformation(
          extent={{-140, 154}, {-120, 174}})));
  Modelica.Blocks.Math.Add QEva_flow(
    k1=-1)
    "Heat flow rate at evaporator"
    annotation(
      Placement(
        transformation(
          extent={{-40, 320}, {-20, 340}})));
  Modelica.Blocks.Math.Add PHeaAct
    "Power consumption for heating"
    annotation(
      Placement(
        transformation(
          extent={{80, 310}, {100, 330}})));
  Modelica.Blocks.Math.Add QEvaHotWat_flow(
    k1=-1)
    "Heat flow rate at evaporator"
    annotation(
      Placement(
  ...
  Modelica.Blocks.Math.Division mConHotWatFlow
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60,-78}, {-40,-58}})));
  Modelica.Blocks.Sources.Constant gainHotWat(
    k=dTHotWatCon_nominal*cp_default)
    "Gain for hot water"
    annotation(
      Placement(
        transformation(
  ...
  Buildings.Fluid.Sources.MassFlowSource_T sou1(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-18,-360}, {-38,-340}})));
  Modelica.Blocks.Math.Gain mPumCoo_flow(
    k=1/(cp_default*dTCooCon_nominal))
    "Mass flow rate for cooling loop"
    annotation(
      Placement(
        transformation(
          extent={{-180,-280}, {-160,-260}})));
  Modelica.Blocks.Math.Add QCon_flow(
    k1=-1)
  ...
  Modelica.Blocks.Math.Add PCooAct
    ...
    annotation(
      Placement(
        transformation(
          extent={{80,-310}, {100,-290}})));
  Modelica.Blocks.Sources.Constant TChiSup(
    k=TChiSup_nominal)
    "Supply water temperature set point for chilled water loop"
    annotation(
      Placement(
        transformation(
          extent={{-180,-344}, {-160,-324}})));
  Buildings.Utilities.Math.SmoothMax smoothMax(
    deltaX=(THeaSup_nominal-THeaRet_nominal)*0.005)
    annotation(
      Placement(
        transformation(
          extent={{-180, 154}, {-160, 174}})));
  Modelica.Blocks.Sources.Constant dTHeaPumConMin(
    k=4)
  ...
  Modelica.Blocks.Sources.Constant TSetHotWat(
    k=273.15 + 60)
  ...
  Modelica.Blocks.Math.Add THeaPumSup
    ...
    annotation(
      Placement(
        transformation(
          extent={{-140, 250}, {-120, 270}})));
  Modelica.Blocks.Sources.Constant dTChiEvaMin(
    k=-dTCooEva_nominal)
    "Temperature difference over evaporator"
    annotation(
      Placement(
        transformation(
          extent={{-180,-394}, {-160,-374}})));
  Modelica.Blocks.Math.Add TEvaIn
    "Evaporator inlet temperature"
    annotation(
      Placement(
        transformation(
          extent={{-100,-384}, {-80,-364}})));
...
equation
  connect(pumHea.port_b, heaPum.port_a2)
    annotation(
      Line(
        points={{50, 300}, {50, 300}, {100, 300}, {100, 216}, {42, 216}},
        color={0, 127, 255}));
  connect(dTHeaAct.u1, TSupHeaSet.TSup)
    annotation(
      Line(
        points={{-222, 176}, {-230, 176}, {-230, 192}, {-180, 192}, {-180, 256}, {-199, 256}},
        color={0, 0, 127}));
  connect(TSupHeaSet.TRet, dTHeaAct.u2)
    annotation(
      Line(
        points={{-199, 244}, {-186, 244}, {-186, 214}, {-186, 200}, {-238, 200}, {-238, 164}, {-222, 164}},
        color={0, 0, 127}));
  connect(mConFlow.u1, deMul.y2[1])
    annotation(
      Line(
        points={{-82, 176}, {-100, 176}, {-100, 350}, {-100, 382}, {-100, 410}, {-157, 410}},
        color={0, 0, 127}));
  connect(gainH.y, mConFlow.u2)
    annotation(
      Line(
        points={{-119, 164}, {-119, 164}, {-82, 164}},
        color={0, 0, 127}));
  connect(mConFlow.y, sou.m_flow_in)
    annotation(
      Line(
        points={{-59, 170}, {-50, 170}, {-50, 236}, {-40, 236}},
        color={0, 0, 127}));
  connect(sou.ports[1], heaPum.port_a1)
    annotation(
      Line(
        points={{-20, 228}, {10, 228}, {22, 228}},
        color={0, 127, 255}));
  connect(heaPum.port_b1, sin1.ports[1])
    annotation(
      Line(
        points={{42, 228}, {42, 228}, {60, 228}, {60, 230}},
        color={0, 127, 255}));
  connect(sou.T_in, TSupHeaSet.TRet)
    annotation(
      Line(
        points={{-42, 232}, {-60, 232}, {-60, 244}, {-190, 244}, {-200, 244}, {-199, 244}},
        color={0, 0, 127}));
  connect(QEva_flow.u1, deMul.y2[1])
    annotation(
      Line(
        points={{-42, 336}, {-100, 336}, {-100, 398}, {-100, 410}, {-157, 410}},
        color={0, 0, 127}));
  connect(QEva_flow.u2, heaPum.P)
    annotation(
      Line(
        points={{-42, 324}, {-60, 324}, {-60, 280}, {-30, 280}, {48, 280}, {48, 222}, {43, 222}},
        color={0, 0, 127}));
  connect(PHeaAct.y, PHea)
    annotation(
      Line(
        points={{101, 320}, {220, 320}, {220, 280}, {290, 280}},
        color={0, 0, 127}));
  connect(PHeaAct.u1, pumHea.P)
    annotation(
      Line(
        points={{78, 326}, {58, 326}, {58, 309}, {51, 309}},
        color={0, 0, 127}));
  connect(PHeaAct.u2, heaPum.P)
    annotation(
      Line(
        points={{78, 314}, {60, 314}, {60, 280}, {48, 280}, {48, 222}, {43, 222}},
        color={0, 0, 127}));
  connect(QEvaHotWat_flow.u2, heaPumHotWat.P)
    annotation(
      Line(
        points={{-42, 34}, {-50, 34}, {-50, 0}, {-50,-20}, {48,-20}, {48,-82}, {41,-82}},
        color={0, 0, 127}));
  connect(PHotWatAct.u2, heaPumHotWat.P)
    annotation(
      Line(
        points={{98, 14}, {60, 14}, {60,-20}, {48,-20}, {48,-82}, {41,-82}},
        color={0, 0, 127}));
  connect(PHotWatAct.u1, pumHotWat.P)
    annotation(
      Line(
        points={{98, 26}, {58, 26}, {58, 9}, {51, 9}},
        color={0, 0, 127}));
  connect(heaPumHotWat.port_b1, sinHotWat.ports[1])
    annotation(
      Line(
        points={{40,-76}, {40,-74}, {46,-74}, {52,-74}},
        color={0, 127, 255}));
  connect(souHotWat.ports[1], heaPumHotWat.port_a1)
    annotation(
      Line(
        points={{-2,-76}, {0,-76}, {20,-76}},
        color={0, 127, 255}));
  ...
  connect(gainHotWat.y, mConHotWatFlow.u2)
    annotation(
      Line(
        points={{-79,-90}, {-80,-90}, {-80,-90}, {-78,-90}, {-70,-90}, {-70,-74}, {-62,-74}},
        color={0, 0, 127}));
  connect(QEvaHotWat_flow.u1, deMul.y3[1])
    annotation(
      Line(
        points={{-42, 46}, {-42, 44}, {-92, 44}, {-92, 403}, {-157, 403}},
        color={0, 0, 127}));
  connect(PHotWatAct.y, PHotWat)
    annotation(
      Line(
        points={{121, 20}, {160, 20}, {160, 240}, {290, 240}},
        color={0, 0, 127}));
  connect(pumHotWat.port_b, heaPumHotWat.port_a2)
    annotation(
      Line(
        points={{50, 0}, {64, 0}, {80, 0}, {80,-88}, {40,-88}},
        color={0, 127, 255},
        thickness=0.5));
  connect(mConHotWatFlow.u1, deMul.y3[1])
    annotation(
      Line(
        points={{-62,-62}, {-92,-62}, {-92, 403}, {-157, 403}},
        color={0, 0, 127}));
  connect(chi.port_b2, sin2.ports[1])
    annotation(
      Line(
        points={{-80,-350}, {-100,-350}, {-120,-350}},
        color={0, 127, 255}));
  connect(chi.port_a2, sou1.ports[1])
    annotation(
      Line(
        points={{-60,-350}, {-38,-350}},
        color={0, 127, 255}));
  connect(mPumCoo_flow.y, pumChi.m_flow_in)
    annotation(
      Line(
        points={{-159,-270}, {-159,-270}, {-112,-270}, {-112,-276}, {-112,-276}, {-112,-282}},
        color={0, 0, 127}));
  connect(mPumCoo_flow.u, QCon_flow.y)
    annotation(
      Line(
        points={{-182,-270}, {-182,-270}, {-182,-268}, {-182,-270}, {-190,-270}, {-199,-270}},
        color={0, 0, 127}));
  connect(pumChi.port_b, chi.port_a1)
    annotation(
      Line(
        points={{-102,-294}, {-88,-294}, {-88,-338}, {-80,-338}},
        color={0, 127, 255},
        thickness=0.5));
  connect(QCon_flow.u1, deMul.y1[1])
    annotation(
      Line(
        points={{-222,-264}, {-240,-264}, {-240,-220}, {-106,-220}, {-106, 417}, {-157, 417}},
        color={0, 0, 127}));
  connect(QCon_flow.u2, chi.P)
    annotation(
      Line(
        points={{-222,-276}, {-240,-276}, {-240,-314}, {-50,-314}, {-50,-344}, {-59,-344}},
        color={0, 0, 127}));
  connect(PCooAct.y, PCoo)
    annotation(
      Line(
        points={{101,-300}, {168,-300}, {168, 200}, {290, 200}},
        color={0, 0, 127}));
  connect(PCooAct.u1, pumChi.P)
    annotation(
      Line(
        points={{78,-294}, {66,-294}, {66,-285}, {-101,-285}},
        color={0, 0, 127}));
  connect(PCooAct.u2, chi.P)
    annotation(
      Line(
        points={{78,-306}, {66,-306}, {66,-314}, {-50,-314}, {-50,-344}, {-59,-344}},
        color={0, 0, 127}));
  connect(TChiSup.y, chi.TSet)
    annotation(
      Line(
        points={{-159,-334}, {-82,-334}, {-82,-335}},
        color={0, 0, 127}));
  connect(dTHeaAct.y, smoothMax.u1)
    annotation(
      Line(
        points={{-199, 170}, {-199, 170}, {-182, 170}},
        color={0, 0, 127}));
  connect(dTHeaPumConMin.y, smoothMax.u2)
    annotation(
      Line(
        points={{-199, 130}, {-190, 130}, {-190, 158}, {-182, 158}},
        color={0, 0, 127}));
  connect(TSetHotWat.y, heaPumHotWat.TSet)
    annotation(
      Line(
        points={{-59,-30}, {8,-30}, {8,-73}, {18,-73}},
        color={0, 0, 127}));
  connect(gainH.u, smoothMax.y)
    annotation(
      Line(
        points={{-142, 164}, {-146, 164}, {-150, 164}, {-159, 164}},
        color={0, 0, 127}));
  connect(THeaPumSup.u1, TSupHeaSet.TRet)
    annotation(
      Line(
        points={{-142, 266}, {-170, 266}, {-170, 244}, {-199, 244}},
        color={0, 0, 127}));
  connect(THeaPumSup.u2, smoothMax.y)
    annotation(
      Line(
        points={{-142, 254}, {-148, 254}, {-148, 164}, {-159, 164}},
        color={0, 0, 127}));
  connect(THeaPumSup.y, heaPum.TSet)
    annotation(
      Line(
        points={{-119, 260}, {-106, 260}, {0, 260}, {0, 231}, {20, 231}},
        color={0, 0, 127}));
  connect(TChiSup.y, TEvaIn.u1)
    annotation(
      Line(
        points={{-159,-334}, {-150,-334}, {-150,-368}, {-102,-368}},
        color={0, 0, 127}));
  connect(dTChiEvaMin.y, TEvaIn.u2)
    annotation(
      Line(
        points={{-159,-384}, {-130,-384}, {-130,-380}, {-102,-380}},
        color={0, 0, 127}));
  connect(TEvaIn.y, sou1.T_in)
    annotation(
      Line(
        points={{-79,-374}, {-40,-374}, {-10,-374}, {-10,-346}, {-16,-346}},
        color={0, 0, 127}));
  connect(del_a.ports[1], port_a)
    annotation(
      Line(
        points={{-263, 10}, {-263, 0}, {-280, 0}},
        color={0, 127, 255}));
  connect(pumHea.port_a, del_a.ports[2])
    annotation(
      Line(
        points={{30, 300}, {30, 300}, {-242, 300}, {-242, 2}, {-262, 2}, {-262, 0}, {-262, 6}, {-261, 6}, {-261, 10}},
        color={0, 127, 255}));
  ...
  connect(chi.port_b1, del_a.ports[4])
    annotation(
      Line(
        points={{-60,-338}, {-46,-338}, {-46,-332}, {-46,-240}, {-256,-240}, {-256,-116}, {-257,-116}, {-257, 10}},
        color={0, 127, 255},
        thickness=0.5));
  connect(heaPum.port_b2, del_b.ports[1])
    annotation(
      Line(
        points={{22, 216}, {18, 216}, {18, 180}, {200, 180}, {200, 6}, {257, 6}, {257, 8}, {257, 10}},
        color={0, 127, 255}));
  ...
  connect(pumChi.port_a, del_b.ports[3])
    annotation(
      Line(
        points={{-122,-294}, {-134,-294}, {-134,-244}, {260,-244}, {260, 10}, {261, 10}},
        color={0, 127, 255},
        thickness=0.5));
  connect(port_b, del_b.ports[4])
    annotation(
      Line(
        points={{280, 0}, {263, 0}, {263, 10}},
        color={0, 127, 255}));
  ...
  connect(deMul.y2[1], QHea_flow)
    annotation(
      Line(
        points={{-157, 410}, {-157, 408}, {240, 408}, {240, 140}, {290, 140}},
        color={0, 0, 127}));
  connect(deMul.y3[1], QHotWat_flow)
    annotation(
      Line(
        points={{-157, 403}, {-157, 404}, {236, 404}, {236, 100}, {290, 100}},
        color={0, 0, 127}));
  connect(deMul.y1[1], QCoo_flow)
    annotation(
      Line(
        points={{-157, 417}, {232, 417}, {232, 60}, {290, 60}},
        color={0, 0, 127}));
  connect(loa.y, gaiLoa.u)
    annotation(
      Line(
        points={{-239, 410}, {-222, 410}},
        color={0, 0, 127}));
  connect(gaiLoa.y, deMul.u)
    annotation(
      Line(
        points={{-199, 410}, {-180, 410}},
        color={0, 0, 127}));
  connect(del_a.heatPort, heatPort_a)
    annotation(
      Line(
        points={{-270, 20}, {-276, 20}, {-276,-90}, {-280,-90}},
        color={191, 0, 0}));
  connect(del_b.heatPort, heatPort_b)
    annotation(
      Line(
        points={{250, 20}, {244, 20}, {244,-90}, {254,-90}, {280,-90}},
        color={191, 0, 0}));
  ...
  connect(mPumHotWat_flow.y, pumHotWat.m_flow_in)
    annotation(
      Line(
        points={{21, 40}, {40, 40}, {40, 12}},
        color={0, 0, 127}));
  connect(QEva_flow.y, mPumHea_flow.u)
    annotation(
      Line(
        points={{-19, 330}, {-2, 330}, {-2, 330}},
        color={0, 0, 127}));
  connect(mPumHea_flow.y, pumHea.m_flow_in)
    annotation(
      Line(
        points={{21, 330}, {40, 330}, {40, 312}},
        color={0, 0, 127}));
  connect(deMul.y1[1], mPumCooEva_flow.u)
    annotation(
      Line(
        points={{-157, 417}, {-106, 417}, {-106,-220}, {50,-220}, {50,-340}, {42,-340}},
        color={0, 0, 127}));
  connect(mPumCooEva_flow.y, sou1.m_flow_in)
    annotation(
      Line(
        points={{19,-340}, {-18,-340}, {-18,-342}},
        color={0, 0, 127}));
  annotation(
