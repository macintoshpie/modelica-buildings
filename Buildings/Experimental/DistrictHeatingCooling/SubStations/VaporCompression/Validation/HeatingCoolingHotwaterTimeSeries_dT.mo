...
model HeatingCoolingHotwaterTimeSeries_dT
  ...
  package Medium=Buildings.Media.Water
    "Fluid in the pipes";
  Modelica.Blocks.Sources.Constant TCoo(
    k=273.15 + 12)
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=270,
          origin={10, 50})));
  Modelica.Blocks.Sources.Constant TWar(
    k=273.15 + 18)
...
equation
  connect(TWar.y, war.T_in)
    annotation(
      Line(
        points={{-59, 70}, {14, 70}, {14, 62}},
        color={0, 0, 127}));
  connect(TCoo.y, coo1.T_in)
    annotation(
      Line(
        points={{-59,-70}, {36,-70}, {36,-62}},
        color={0, 0, 127}));
  ...
  connect(subSta.port_a, war.ports[1])
    annotation(
      Line(
        points={{0, 0}, {-10, 0}, {-10, 30}, {10, 30}, {10, 40}},
        color={0, 127, 255}));
  connect(subSta.port_b, coo1.ports[1])
    annotation(
      Line(
        points={{19.9286, 0}, {40, 0}, {40,-40}},
        color={0, 127, 255}));
  annotation(
    experiment(
      Tolerance=1e-08,
      StopTime=864000),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/SubStations/VaporCompression/Validation/HeatingCoolingHotwaterTimeSeries_dT.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
