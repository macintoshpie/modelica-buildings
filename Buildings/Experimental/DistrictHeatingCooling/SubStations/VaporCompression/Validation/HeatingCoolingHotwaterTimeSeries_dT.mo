...
model HeatingCoolingHotwaterTimeSeries_dT
  ...
  Buildings.Fluid.Sources.Boundary_pT war(redeclare package Medium=Medium, use_T_in=true, nPorts=1)
    "Warm pipe"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=270, origin={10, 50})));
  Modelica.Blocks.Sources.Constant TWar(k=273.15 + 18)
    "Temperature of warm supply"
    annotation(Placement(transformation(extent={{-80, 60}, {-60, 80}})));
  Buildings.Experimental.DistrictHeatingCooling.SubStations.VaporCompression.HeatingCoolingHotwaterTimeSeries_dT subSta(redeclare package Medium=Medium, filNam=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/Data/Experimental/DistrictHeatingCooling/SubStations/VaporCompression/RefBldgLargeOfficeNew2004_7.1_5.0_3C_USA_CA_SAN_FRANCISCO.mos"), show_T=true, TOut_nominal=273.15)
    "Substation model"
  ...
  Buildings.Fluid.Sources.Boundary_pT coo1(redeclare package Medium=Medium, use_T_in=true, nPorts=1)
    "Cool pipe"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=90, origin={40,-50})));
equation
  connect(TWar.y, war.T_in)
    annotation(Line(points={{-59, 70}, {14, 70}, {14, 62}}, color={0, 0, 127}));
  connect(TCoo.y, coo1.T_in)
    annotation(Line(points={{-59,-70}, {36,-70}, {36,-62}}, color={0, 0, 127}));
  connect(weaDat.weaBus, subSta.weaBus)
    annotation(Line(points={{-60, 10}, {-34, 10}, {10, 10}, {10, 8.35714}}, color={255, 204, 51}, thickness=0.5));
  ...
  annotation(experiment(Tolerance=1e-08, StopTime=864000), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/SubStations/VaporCompression/Validation/HeatingCoolingHotwaterTimeSeries_dT.mos"
