...
model HeatingCoolingHotWaterSmall
  ...
  Plants.LakeWaterHeatExchanger_T bayWatHex(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-120,-20}, {-100, 20}})));
  BoundaryConditions.WeatherData.Bus weaBus
    annotation(
      Placement(
        transformation(
          extent={{-168, 100}, {-152, 116}}),
        iconTransformation(
  ...
  Modelica.Blocks.Sources.CombiTimeTable watTem(
    ...
    y(
      each unit="K"),
    fileName=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/Data/Experimental/DistrictHeatingCooling/Plants/AlamedaOceanT.mos"))
...
protected
  ...
  Modelica.Blocks.Sources.Constant TSetH(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-260, 100}, {-240, 120}})));
equation
  connect(pip.port_b, splSup.port_1)
    annotation(
      Line(
        points={{-50, 60}, {-50, 60}, {-40, 60}},
        color={0, 127, 255}));
  connect(splSup.port_3, larOff.port_a)
    annotation(
      Line(
        points={{-30, 50}, {-30, 50}, {-30, 0}, {-18, 0}},
        color={0, 127, 255}));
  connect(splSup.port_2, ret.port_a)
    annotation(
      Line(
        points={{-20, 60}, {6, 60}, {60, 60}, {60, 0}, {80, 0}},
        color={0, 127, 255}));
  connect(larOff.port_b, splRet.port_3)
    annotation(
      Line(
        points={{21.8571, 0}, {40, 0}, {40,-50}},
        color={0, 127, 255}));
  connect(pip1.port_a, splRet.port_1)
    annotation(
      Line(
        points={{-52,-60}, {-52,-60}, {30,-60}},
        color={0, 127, 255}));
  connect(splRet.port_2, ret.port_b)
    annotation(
      Line(
        points={{50,-60}, {50,-60}, {132,-60}, {132, 0}, {119.857, 0}},
        color={0, 127, 255}));
  connect(TSetH.y, pla.TSetHea)
    annotation(
      Line(
        points={{-239, 110}, {-228, 110}, {-228, 58}, {-212, 58}},
        color={0, 0, 127}));
  connect(TSetC.y, pla.TSetCoo)
    annotation(
      Line(
        points={{-239, 70}, {-232, 70}, {-232, 54}, {-212, 54}},
        color={0, 0, 127}));
  connect(pip.port_a, bayWatHex.port_b1)
    annotation(
      Line(
        points={{-70, 60}, {-84, 60}, {-84,-4}, {-100,-4}},
        color={0, 127, 255}));
  connect(bayWatHex.port_a1, pla.port_b)
    annotation(
      Line(
        points={{-120,-4}, {-180,-4}, {-180, 50}, {-190, 50}},
        color={0, 127, 255}));
  connect(bayWatHex.port_b2, pla.port_a)
    annotation(
      Line(
        points={{-120,-16}, {-120,-2}, {-258,-2}, {-258, 50}, {-210, 50}},
        color={0, 127, 255}));
  connect(bayWatHex.port_a2, pip1.port_b)
    annotation(
      Line(
        points={{-100,-16}, {-80,-16}, {-80,-60}, {-72,-60}},
        color={0, 127, 255}));
  connect(bayWatHex.TSetHea, TSetH.y)
    annotation(
      Line(
        points={{-122, 6}, {-228, 6}, {-228, 110}, {-239, 110}},
        color={0, 0, 127}));
  connect(pSet.ports[1], pla.port_a)
    annotation(
      Line(
        points={{-260,-80}, {-260,-80}, {-260,-42}, {-260, 50}, {-210, 50}},
        color={0, 127, 255}));
  ...
  connect(pla.TSink, weaBus.TDryBul)
    annotation(
      Line(
        points={{-212, 44}, {-220, 44}, {-220, 88}, {-218, 88}, {-160, 88}, {-160, 108}},
        color={0, 0, 127}));
  ...
  connect(bayWatHex.TSouWat, watTem.y[1])
    annotation(
      Line(
        points={{-122, 18}, {-140, 18}, {-149, 18}},
        color={0, 0, 127}));
  connect(bayWatHex.TSouHea, weaBus.TDryBul)
    annotation(
      Line(
        points={{-122, 14}, {-130, 14}, {-130, 108}, {-160, 108}},
        color={0, 0, 127}));
  connect(bayWatHex.TSetCoo, TSetC.y)
    annotation(
      Line(
        points={{-122, 2}, {-232, 2}, {-232, 70}, {-239, 70}, {-239, 70}},
        color={0, 0, 127}));
  connect(bayWatHex.TSouCoo, weaBus.TDryBul)
    annotation(
      Line(
        points={{-122, 10}, {-130, 10}, {-130, 108}, {-160, 108}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-06,
      StopTime=31536000),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/Validation/HeatingCoolingHotWaterSmall.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
