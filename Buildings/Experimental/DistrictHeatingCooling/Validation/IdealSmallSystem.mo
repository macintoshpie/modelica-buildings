...
equation
  connect(pla.port_b, pip.port_a)
    annotation(
      Line(
        points={{-100, 60}, {-92, 60}, {-88, 60}},
        color={0, 127, 255}));
  connect(pla.port_a, pip1.port_b)
    annotation(
      Line(
        points={{-120, 60}, {-128, 60}, {-128,-40}, {-90,-40}},
        color={0, 127, 255}));
  connect(pSet.ports[1], pip1.port_b)
    annotation(
      Line(
        points={{-110,-60}, {-110,-40}, {-90,-40}},
        color={0, 127, 255}));
  connect(QHea1.y[1], hea1.Q_flow)
    annotation(
      Line(
        points={{-19, 90}, {-12, 90}, {-12, 6}, {0, 6}},
        color={0, 0, 127}));
  connect(QHea2.y[1], hea2.Q_flow)
    annotation(
      Line(
        points={{81, 90}, {88, 90}, {88, 6}, {96, 6}},
        color={0, 0, 127}));
  connect(QCoo1.y[1], coo1.Q_flow)
    annotation(
      Line(
        points={{161, 90}, {170, 90}, {170, 6}, {178, 6}},
        color={0, 0, 127}));
  connect(QCoo2.y[1], coo2.Q_flow)
    annotation(
      Line(
        points={{241, 90}, {241, 90}, {250, 90}, {250, 6}, {258, 6}},
        color={0, 0, 127}));
  connect(pip.port_b, spl.port_1)
    annotation(
      Line(
        points={{-68, 60}, {-68, 60}, {-40, 60}},
        color={0, 127, 255}));
  connect(spl.port_2, spl1.port_1)
    annotation(
      Line(
        points={{-20, 60}, {22, 60}, {50, 60}},
        color={0, 127, 255}));
  ...
  connect(spl.port_3, hea1.port_a)
    annotation(
      Line(
        points={{-30, 50}, {-30, 50}, {-30, 0}, {-28, 0}, {2, 0}},
        color={0, 127, 255}));
  connect(spl1.port_3, hea2.port_a)
    annotation(
      Line(
        points={{60, 50}, {60, 50}, {60, 10}, {60, 0}, {98, 0}},
        color={0, 127, 255}));
  connect(spl2.port_2, coo2.port_b)
    annotation(
      Line(
        points={{230, 60}, {230, 60}, {290, 60}, {290, 0}, {280, 0}},
        color={0, 127, 255}));
  connect(coo1.port_b, spl2.port_3)
    annotation(
      Line(
        points={{200, 0}, {220, 0}, {220, 50}},
        color={0, 127, 255}));
  connect(pip1.port_a, spl3.port_1)
    annotation(
      Line(
        points={{-70,-40}, {-26,-40}, {20,-40}},
        color={0, 127, 255}));
  connect(spl3.port_3, hea1.port_b)
    annotation(
      Line(
        points={{30,-30}, {30,-30}, {30, 0}, {22, 0}},
        color={0, 127, 255}));
  connect(hea2.port_b, spl4.port_3)
    annotation(
      Line(
        points={{118, 0}, {130, 0}, {130,-30}},
        color={0, 127, 255}));
  connect(spl4.port_1, spl3.port_2)
    annotation(
      Line(
        points={{120,-40}, {120,-40}, {40,-40}},
        color={0, 127, 255}));
  ...
  connect(spl5.port_3, coo1.port_a)
    annotation(
      Line(
        points={{160,-30}, {160,-30}, {160, 0}, {180, 0}},
        color={0, 127, 255}));
  connect(spl5.port_2, coo2.port_a)
    annotation(
      Line(
        points={{170,-40}, {208,-40}, {240,-40}, {240, 0}, {260, 0}},
        color={0, 127, 255}));
  connect(TSetH.y, pla.TSetHea)
    annotation(
      Line(
        points={{-139, 120}, {-130, 120}, {-130, 68}, {-122, 68}},
        color={0, 0, 127}));
  connect(TSetC.y, pla.TSetCoo)
    annotation(
      Line(
        points={{-139, 90}, {-134, 90}, {-134, 64}, {-122, 64}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1E-06,
      StopTime=86400),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Experimental/DistrictHeatingCooling/Validation/IdealSmallSystem.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
