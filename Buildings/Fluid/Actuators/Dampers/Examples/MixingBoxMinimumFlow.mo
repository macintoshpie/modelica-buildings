  ...
  Modelica.Blocks.Sources.Constant PAtm(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-100, 10}, {-80, 30}})));
  Modelica.Blocks.Sources.Constant yDamMin(
    k=0.5)
    annotation(
      Placement(
        transformation(
...
equation
  connect(yDamMin.y, mixBox.yOutMin)
    annotation(
      Line(
        points={{-19, 50}, {18, 50}, {18, 6.66134e-16}},
        color={0, 0, 127}));
  connect(yDam.y, mixBox.y)
    annotation(
      Line(
        points={{-19, 80}, {24, 80}, {24, 6.66134e-16}},
        color={0, 0, 127}));
  connect(bouIn.p_in, PAtm.y)
    annotation(
      Line(
        points={{-62, 20}, {-72, 20}, {-79, 20}},
        color={0, 0, 127}));
  connect(PRet.y, bouRet.p_in)
    annotation(
      Line(
        points={{81,-40}, {90,-40}, {90,-72}, {70,-72}},
        color={0, 0, 127}));
  connect(bouSup.p_in, PSup.y)
    annotation(
      Line(
        points={{70, 8}, {92, 8}, {92, 50}, {81, 50}},
        color={0, 0, 127}));
  ...
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Actuators/Dampers/Examples/MixingBoxMinimumFlow.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=240),
