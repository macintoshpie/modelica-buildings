...
model MixingBoxMinimumFlow
  ...
  parameter Modelica.SIunits.PressureDifference dpDamOutMin_nominal(min=0, displayUnit="Pa")
    "Pressure drop of damper in minimum outside air leg"
    annotation(Dialog(group="Nominal condition"));
  parameter Modelica.SIunits.PressureDifference dpFixOutMin_nominal(min=0, displayUnit="Pa")=0
  ...
  Modelica.Fluid.Interfaces.FluidPort_a port_OutMin(redeclare package Medium=Medium, m_flow(start=0, min=if allowFlowReversal then-Constants.inf else 0))
  ...
  Modelica.Blocks.Interfaces.RealInput yOutMin
    ...
    annotation(Placement(transformation(extent={{-20,-20}, {20, 20}}, rotation=270, origin={-60, 120}), iconTransformation(extent={{-20,-20}, {20, 20}}, rotation=270, origin={-60, 120})));
  Modelica.Blocks.Interfaces.RealOutput yOutMin_actual
    "Actual valve position"
    annotation(Placement(transformation(extent={{-52, 58}, {-32, 78}}), iconTransformation(extent={{-52, 58}, {-32, 78}})));
  Buildings.Fluid.Actuators.Dampers.Exponential damOAMin(redeclare final package Medium=Medium, final from_dp=from_dp, final linearized=linearized, final use_deltaM=use_deltaM, final deltaM=deltaM, final roundDuct=roundDuct, final ReC=ReC, final a=a, final b=b, final yL=yL, final yU=yU, final k1=k1, final l=l, final use_constant_density=use_constant_density, final allowFlowReversal=allowFlowReversal, final m_flow_nominal=mOutMin_flow_nominal, final dpDamper_nominal=dpDamOutMin_nominal, final dpFixed_nominal=dpFixOutMin_nominal, final use_inputFilter=false)
    "Damper for minimum outside air intake"
...
protected
  Modelica.Blocks.Interfaces.RealOutput yOutMin_filtered if use_inputFilter
    ...
    annotation(Placement(transformation(extent={{-32, 78}, {-12, 98}}), iconTransformation(extent={{60, 50}, {80, 70}})));
  Modelica.Blocks.Continuous.Filter filterOutMin(order=2, f_cut=5/(2*Modelica.Constants.pi*riseTime), final init=init, final y_start=yOutMin_start, final analogFilter=Modelica.Blocks.Types.AnalogFilter.CriticalDamping, final filterType=Modelica.Blocks.Types.FilterType.LowPass, x(each stateSelect=StateSelect.always)) if use_inputFilter
    "Second order filter to approximate valve opening time, and to improve numerics"
    annotation(Placement(transformation(extent={{-56, 81}, {-42, 95}})));
equation
  connect(filterOutMin.y, yOutMin_filtered)
    annotation(Line(points={{-41.3, 88}, {-22, 88}}, color={0, 0, 127}, smooth=Smooth.None));
  if use_inputFilter then
    connect(yOutMin, filterOutMin.u)
      annotation(Line(points={{-60, 120}, {-60, 88}, {-57.4, 88}}, color={0, 0, 127}, smooth=Smooth.None));
    connect(filterOutMin.y, yOutMin_actual)
      annotation(Line(points={{-41.3, 88}, {-36, 88}, {-36, 68}, {-42, 68}}, color={0, 0, 127}, smooth=Smooth.None));
  else
    connect(yOutMin, yOutMin_actual)
      annotation(Line(points={{-60, 120}, {-60, 68}, {-42, 68}}, color={0, 0, 127}, smooth=Smooth.None));
  end if;
  //////
  connect(port_OutMin, damOAMin.port_a)
    annotation(Line(points={{-100, 100}, {-80, 100}, {-80, 72}, {-68, 72}, {-68, 42}, {48, 42}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(damOAMin.port_b, port_Sup)
    annotation(Line(points={{68, 42}, {80, 42}, {80, 60}, {100, 60}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(yOutMin_actual, damOAMin.y)
    annotation(Line(points={{-42, 68}, {-12, 68}, {-12, 58}, {58, 58}, {58, 54}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 100}}), graphics={Rectangle(extent={{-60, 34}, {80, 28}}, lineColor={0, 0, 255}, fillColor={0, 0, 255}, fillPattern=FillPattern.Solid), Text(extent={{-126, 144}, {-86, 112}}, lineColor={0, 0, 127}, fillColor={0, 0, 0}, fillPattern=FillPattern.Solid, textString="yMin"), Rectangle(extent={{-98, 98}, {-54, 92}}, lineColor={0, 0, 255}, fillColor={0, 0, 255}, fillPattern=FillPattern.Solid), Rectangle(extent={{-60, 92}, {-54, 34}}, lineColor={0, 0, 255}, fillColor={0, 0, 255}, fillPattern=FillPattern.Solid), Polygon(points={{-68, 62}, {-50, 84}, {-42, 84}, {-60, 62}, {-68, 62}}, lineColor={0, 0, 0}, fillColor={0, 0, 0}, fillPattern=FillPattern.Solid)}), defaultComponentName="eco", Documentation(revisions="<html>
<ul>
<li>
