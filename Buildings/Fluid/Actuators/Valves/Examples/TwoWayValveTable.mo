...
model TwoWayValveTable
  ...
  Buildings.Fluid.Sources.Boundary_pT sou(redeclare package Medium=Medium, use_p_in=false, p(displayUnit="Pa")=306000, T=293.15, nPorts=1)
    "Boundary condition for flow source"
    annotation(Placement(transformation(extent={{-90, 10}, {-70, 30}})));
  Buildings.Fluid.Sources.Boundary_pT sin(redeclare package Medium=Medium, p(displayUnit="Pa")=3E5, T=293.15, nPorts=1)
    "Boundary condition for flow sink"
    annotation(Placement(transformation(extent={{72, 10}, {52, 30}})));
  Valves.TwoWayTable valTab(redeclare package Medium=Medium, use_inputFilter=false, from_dp=true, flowCharacteristics=datVal, CvData=Buildings.Fluid.Types.CvTypes.Kv, Kv=0.65, m_flow_nominal=0.04)
    "Valve model with opening characteristics based on a table"
    annotation(Placement(transformation(extent={{-20, 10}, {0, 30}})));
  parameter Data.Generic datVal(y={0, 0.1667, 0.3333, 0.5, 0.6667, 1}, phi={0, 0.19, 0.35, 0.45, 0.5, 0.65}/0.65)
  ...
  Sensors.VolumeFlowRate senVolFlo(redeclare package Medium=Medium, m_flow_nominal=0.04)
...
equation
  connect(y.y, valTab.y)
    annotation(Line(points={{-19, 60}, {-19, 60}, {-10, 60}, {-10, 32}}, color={0, 0, 127}));
  ...
  connect(valTab.port_b, senRelPre.port_b)
    annotation(Line(points={{4.44089e-16, 20}, {4.44089e-16,-10}}, color={0, 127, 255}));
  connect(valTab.port_b, sin.ports[1])
    annotation(Line(points={{4.44089e-16, 20}, {52, 20}}, color={0, 127, 255}));
  connect(to_bar.u, senRelPre.p_rel)
    annotation(Line(points={{-2,-36}, {-10,-36}, {-10,-19}}, color={0, 0, 127}));
  connect(sqrt1.u, to_bar.y)
    annotation(Line(points={{26,-36}, {21,-36}}, color={0, 0, 127}));
  connect(senVolFlo.V_flow, to_m3_h.u)
    annotation(Line(points={{-50, 31}, {-50, 36}, {-30, 36}, {-30,-64}, {-12,-64}}, color={0, 0, 127}));
  connect(to_m3_h.y, kv.u1)
    annotation(Line(points={{11,-64}, {58,-64}}, color={0, 0, 127}));
  connect(sqrt1.y, kv.u2)
    annotation(Line(points={{49,-36}, {54,-36}, {54,-76}, {58,-76}}, color={0, 0, 127}));
  annotation(experiment(Tolerance=1e-6, StopTime=1.0), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Actuators/Valves/Examples/TwoWayValveTable.mos"
