...
model TwoWayPolynomial
  ...
  extends Buildings.Fluid.Actuators.BaseClasses.PartialTwoWayValveKv(phi=max(0, l + pol_y*(1-l)));
  parameter Real[:] c
...
initial equation
  ...
  if size(c, 1) >= 2 then
    for i in 0 : nP/2 loop
      assert(sum({c[j + 1]*j*(i/nP)^(j-1) for j in 1 : size(c, 1)-1}) >= 0, "The provided valve polynomial coefficients 
