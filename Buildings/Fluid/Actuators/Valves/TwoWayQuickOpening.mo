...
model TwoWayQuickOpening
  ...
  extends BaseClasses.PartialTwoWayValveKv(
    phi=max(0, if homotopyInitialization then homotopy(
      actual=l + Modelica.Fluid.Utilities.regPow(y_actual, alpInv, delta0)*(1-l),
      simplified=l + y_actual*(1-l)) else l + Modelica.Fluid.Utilities.regPow(y_actual, alpInv, delta0)*(1-l)));
