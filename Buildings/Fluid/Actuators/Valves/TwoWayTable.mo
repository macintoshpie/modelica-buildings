...
protected
  Modelica.Blocks.Tables.CombiTable1D phiLooUp(
    ...
    final table=[
      flowCharacteristics.y, cat(1, {max(flowCharacteristics.phi[1], 1E-8)}, {flowCharacteristics.phi[i] for i in 2 : size(flowCharacteristics.phi, 1)})],
    final columns=2 : 2,
...
initial equation
  ...
  assert(Buildings.Utilities.Math.Functions.isMonotonic(
    ...
    strict=true), "The values for y in flowCharacteristics must be strictly monotone increasing.");
  assert(Buildings.Utilities.Math.Functions.isMonotonic(
    x=flowCharacteristics.phi,
    strict=true), "The values for phi in flowCharacteristics must be strictly monotone increasing.");
equation
...
First implementation.
    ...
    Icon(
      ...
      graphics={Rectangle(
        ...
        extent={{-12,-11}, {12, 11}},
        radius=5.0), Line(
        points={{-68,-90}, {-44,-90}}), Line(
        points={{-56,-74}, {-56,-96}}), Line(
        points={{-68,-82}, {-44,-82}})}));
end TwoWayTable;
