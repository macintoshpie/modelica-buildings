...
model TwoWayTable
  ...
  extends BaseClasses.PartialTwoWayValveKv(phi=max(0, phiLooUp.y[1]), final l=phiLooUp.table[1, 2]);
...
protected
  Modelica.Blocks.Tables.CombiTable1D phiLooUp(final tableOnFile=false, final table=[
    flowCharacteristics.y, cat(1, {max(flowCharacteristics.phi[1], 1E-8)}, {flowCharacteristics.phi[i] for i in 2 : size(flowCharacteristics.phi, 1)})], final columns=2 : 2, final smoothness=Modelica.Blocks.Types.Smoothness.ContinuousDerivative)
    "Normalized mass flow rate for the given valve position under the assumption of a constant pressure"
    annotation(Placement(transformation(extent={{70, 60}, {90, 80}})));
initial equation
  assert(abs(flowCharacteristics.y[1]) < Modelica.Constants.eps, "flowCharateristics.y[1] must be 0.");
  ...
  assert(abs(flowCharacteristics.phi[size(flowCharacteristics.phi, 1)]-1) < Modelica.Constants.eps, "flowCharateristics.phi[end] must be 1.");
  // Assert that the sequences are strictly monotonic increasing
  assert(Buildings.Utilities.Math.Functions.isMonotonic(x=flowCharacteristics.y, strict=true), "The values for y in flowCharacteristics must be strictly monotone increasing.");
  assert(Buildings.Utilities.Math.Functions.isMonotonic(x=flowCharacteristics.phi, strict=true), "The values for phi in flowCharacteristics must be strictly monotone increasing.");
equation
  connect(phiLooUp.u[1], y_actual)
    annotation(Line(points={{68, 70}, {50, 70}}, color={0, 0, 127}));
  annotation(defaultComponentName="val", Documentation(info="<html>
<p>
...
</html>"), Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 100}}), graphics={Rectangle(origin={-56,-85}, lineColor={64, 64, 64}, fillColor={255, 215, 136}, fillPattern=FillPattern.Solid, extent={{-12,-11}, {12, 11}}, radius=5.0), Line(points={{-68,-90}, {-44,-90}}), Line(points={{-56,-74}, {-56,-96}}), Line(points={{-68,-82}, {-44,-82}})}));
end TwoWayTable;
