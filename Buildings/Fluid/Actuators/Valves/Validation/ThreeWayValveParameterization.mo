...
equation
  connect(sou1.ports[1], valOPPoi.port_1)
    annotation(
      Line(
        points={{-48, 33}, {-32, 33}, {-32, 90}, {-10, 90}},
        color={0, 127, 255}));
  connect(sou1.ports[2], valKv.port_1)
    annotation(
      Line(
        points={{-48, 31}, {-42, 31}, {-42, 30}, {-10, 30}},
        color={0, 127, 255}));
  connect(sou1.ports[3], valCv.port_1)
    annotation(
      Line(
        points={{-48, 29}, {-32, 29}, {-32,-30}, {-10,-30}},
        color={0, 127, 255}));
  connect(sou1.ports[4], valAv.port_1)
    annotation(
      Line(
        points={{-48, 27}, {-32, 27}, {-32,-84}, {-10,-84}},
        color={0, 127, 255}));
  connect(sou3.ports[1], valOPPoi.port_3)
    annotation(
      Line(
        points={{-50,-7}, {-20,-7}, {-20, 70}, {0, 70}, {0, 80}},
        color={0, 127, 255}));
  connect(sou3.ports[2], valKv.port_3)
    annotation(
      Line(
        points={{-50,-9}, {-20,-9}, {-20, 12}, {0, 12}, {0, 20}},
        color={0, 127, 255}));
  connect(sou3.ports[3], valCv.port_3)
    annotation(
      Line(
        points={{-50,-11}, {-20,-11}, {-20,-48}, {0,-48}, {0,-40}},
        color={0, 127, 255}));
  connect(sou3.ports[4], valAv.port_3)
    annotation(
      Line(
        points={{-50,-13}, {-20,-13}, {-20,-110}, {0,-110}, {0,-94}},
        color={0, 127, 255}));
  ...
  connect(senM_flowOpPoi.port_b, sin.ports[1])
    annotation(
      Line(
        points={{40, 90}, {52, 90}, {52, 92}, {90, 92}, {90, 3}, {100, 3}},
        color={0, 127, 255}));
  connect(senM_flowKv.port_b, sin.ports[2])
    annotation(
      Line(
        points={{40, 30}, {76, 30}, {76, 1}, {100, 1}},
        color={0, 127, 255}));
  connect(senM_flowCv.port_b, sin.ports[3])
    annotation(
      Line(
        points={{40,-30}, {90,-30}, {90,-1}, {100,-1}},
        color={0, 127, 255}));
  connect(senM_flowAv.port_b, sin.ports[4])
    annotation(
      Line(
        points={{40,-84}, {92,-84}, {92,-3}, {100,-3}},
        color={0, 127, 255}));
  connect(y.y, valOPPoi.y)
    annotation(
      Line(
        points={{-59, 110}, {0, 110}, {0, 102}},
        color={0, 0, 127}));
  connect(y.y, valKv.y)
    annotation(
      Line(
        points={{-59, 110}, {-40, 110}, {-40, 52}, {0, 52}, {0, 42}},
        color={0, 0, 127}));
  connect(y.y, valCv.y)
    annotation(
      Line(
        points={{-59, 110}, {-40, 110}, {-40, 0}, {0, 0}, {0,-18}},
        color={0, 0, 127}));
  connect(y.y, valAv.y)
    annotation(
      Line(
        points={{-59, 110}, {-40, 110}, {-40,-60}, {0,-60}, {0,-72}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=1.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Actuators/Valves/Validation/ThreeWayValveParameterization.mos"
        "Simulate and plot"),
    Diagram(
      coordinateSystem(
        extent={{-100,-140}, {140, 140}})),
    Documentation(
      info="<html>
<p>
