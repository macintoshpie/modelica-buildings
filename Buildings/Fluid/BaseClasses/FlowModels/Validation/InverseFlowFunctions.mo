...
model InverseFlowFunctions
  ...
  Modelica.SIunits.PressureDifference dp(
    displayUnit="Pa")
    "Pressure difference";
  Modelica.SIunits.PressureDifference dpCalc(
    displayUnit="Pa")
    "Pressure difference computed by the flow functions";
  Modelica.SIunits.Pressure deltaDp(
    displayUnit="Pa")
...
equation
  ...
  m_flow=FlowModels.basicFlowFunction_dp(
    dp=dp,
    k=k,
    m_flow_turbulent=m_flow_nominal*0.3);
  dpCalc=FlowModels.basicFlowFunction_m_flow(
    m_flow=m_flow,
    k=k,
    m_flow_turbulent=m_flow_nominal*0.3);
  deltaDp=dp-dpCalc;
  annotation(
    experiment(
      ...
      StopTime=1),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/BaseClasses/FlowModels/Validation/InverseFlowFunctions.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
