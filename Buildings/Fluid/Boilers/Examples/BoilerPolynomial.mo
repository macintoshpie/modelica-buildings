...
model BoilerPolynomial
  ...
  Buildings.Fluid.Sources.Boundary_pT sin(redeclare package Medium=Medium, nPorts=2, p(displayUnit="Pa")=300000, T=333.15)
    "Sink"
    annotation(Placement(transformation(extent={{90,-68}, {70,-48}})));
  Buildings.Fluid.Sources.Boundary_pT sou(nPorts=2, redeclare package Medium=Medium, p=300000 + dp_nominal, T=303.15)
    annotation(Placement(transformation(extent={{-80,-68}, {-60,-48}})));
  Modelica.Blocks.Sources.TimeTable y(table=[
    0, 0;
    1800, 1;
    1800, 0;
    2400, 0;
  ...
  Buildings.Fluid.Boilers.BoilerPolynomial boi2(a={0.9}, effCur=Buildings.Fluid.Types.EfficiencyCurves.Constant, Q_flow_nominal=Q_flow_nominal, m_flow_nominal=m_flow_nominal, redeclare package Medium=Medium, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, massDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, dp_nominal=dp_nominal, fue=Buildings.Fluid.Data.Fuels.NaturalGasLowerHeatingValue(), from_dp=true, T_start=293.15)
    "Boiler"
...
equation
  connect(TAmb1.port, boi1.heatPort)
    annotation(Line(points={{-10, 38}, {0, 38}, {0, 15.2}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(TAmb2.port, boi2.heatPort)
    annotation(Line(points={{-12,-30}, {-2,-30}, {-2,-52.8}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(sou.ports[1], boi1.port_a)
    annotation(Line(points={{-60,-56}, {-36,-56}, {-36, 8}, {-10, 8}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(sou.ports[2], boi2.port_a)
    annotation(Line(points={{-60,-60}, {-12,-60}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(boi2.port_b, sin.ports[2])
    annotation(Line(points={{8,-60}, {70,-60}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(boi1.port_b, sin.ports[1])
    annotation(Line(points={{10, 8}, {40, 8}, {40,-56}, {70,-56}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(y.y, boi1.y)
    annotation(Line(points={{-59,-20}, {-50,-20}, {-50, 16}, {-12, 16}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(y.y, boi2.y)
    annotation(Line(points={{-59,-20}, {-50,-20}, {-50,-52}, {-14,-52}}, color={0, 0, 127}, smooth=Smooth.None));
