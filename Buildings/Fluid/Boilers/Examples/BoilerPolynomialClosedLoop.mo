...
model BoilerPolynomialClosedLoop
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=90,
          origin={0,-20})));
  Modelica.Blocks.Sources.Constant m_flow_pum(
    k=m_flow_nominal)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80,-30}, {-60,-10}})));
  Modelica.Blocks.Sources.Constant TSetBoi(
    k=273.15 + 70)
    "Temperature setpoint for boiler"
    annotation(
      Placement(
        transformation(
          extent={{-170,-74}, {-150,-54}})));
  ...
  MixingVolumes.MixingVolume vol(
    ...
    annotation(
      Placement(
        transformation(
          extent={{40, 162}, {60, 182}})));
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor thermalConductor(
    G=0.1*Q_flow_nominal/(40-20))
    annotation(
      Placement(
        transformation(
          extent={{-80, 162}, {-60, 182}})));
  Modelica.Thermal.HeatTransfer.Sources.FixedTemperature TBou(
    T=293.15)
    "Fixed temperature boundary condition"
    annotation(
      Placement(
        transformation(
          extent={{-120, 162}, {-100, 182}})));
...
equation
  ...
  connect(val.port_2, spl.port_1)
    annotation(
      Line(
        points={{1.1119e-15, 50}, {0, 54}, {9.99197e-16, 56}, {-1.12703e-16, 56}, {-1.12703e-16, 60}},
        color={0, 127, 255},
        smooth=Smooth.None));
  connect(spl.port_2, pumLoa.port_a)
    annotation(
      Line(
        points={{1.1119e-15, 80}, {0, 86}, {9.99197e-16, 90}, {-1.12703e-16, 90}, {-1.12703e-16, 100}},
        color={0, 127, 255},
        smooth=Smooth.None));
  connect(pumBoi.port_b, spl1.port_1)
    annotation(
      Line(
        points={{1.1119e-15,-10}, {0,-8}, {9.99197e-16,-6}, {-1.12703e-16,-6}, {-1.12703e-16,-5.55112e-16}},
        color={0, 127, 255},
        smooth=Smooth.None));
  connect(spl1.port_2, val.port_1)
    annotation(
      Line(
        points={{1.1119e-15, 20}, {0, 24}, {9.99197e-16, 26}, {-1.12703e-16, 26}, {-1.12703e-16, 30}},
        color={0, 127, 255},
        smooth=Smooth.None));
  ...
  annotation(
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Boilers/Examples/BoilerPolynomialClosedLoop.mos"
        "Simulate and plot"),
    experiment(
      StopTime=14400,
      Tolerance=1e-6),
