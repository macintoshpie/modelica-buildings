...
model AssertFuelFlow
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput mFue_flow(final unit="kg/s")
    "Fuel mass flow rate"
    annotation(Placement(transformation(extent={{-140,-20}, {-100, 20}}), iconTransformation(extent={{-140,-20}, {-100, 20}})));
  Buildings.Controls.OBC.CDL.Utilities.Assert assMes(final message="Rate of change in the fuel mass flow rate is outside boundaries!")
...
protected
  ...
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hys(final uLow=0.99*dmFueMax_flow, final uHigh=1.01*dmFueMax_flow + 1e-6)
    "Check if fuel mass flow rate is changing too much"
    annotation(Placement(transformation(extent={{0,-10}, {20, 10}})));
  Buildings.Controls.OBC.CDL.Continuous.Derivative floChaRat(final x_start=0)
