...
model AssertPower
  ...
  Buildings.Controls.OBC.CDL.Utilities.Assert assMesP(final message="Electric power is outside boundaries!")
    "Generate warning when the electric power demand is out of the range"
    annotation(Placement(transformation(extent={{70, 10}, {90, 30}})));
  Buildings.Controls.OBC.CDL.Utilities.Assert assMesDP(final message="Rate of change in power output is outside boundaries!") if use_powerRateLimit
...
protected
  ...
  Buildings.Controls.OBC.CDL.Logical.Not not2 if use_powerRateLimit
    "Logical Nand"
    annotation(Placement(transformation(extent={{40,-50}, {60,-30}})));
  Buildings.Controls.OBC.CDL.Continuous.Derivative demRat(final x_start=0) if use_powerRateLimit
    "Power demand rate"
    annotation(Placement(transformation(extent={{-80,-50}, {-60,-30}})));
  Buildings.Controls.OBC.CDL.Continuous.Abs abs1 if use_powerRateLimit
    "Absolute value"
    annotation(Placement(transformation(extent={{-40,-50}, {-20,-30}})));
  Buildings.Controls.OBC.CDL.Continuous.GreaterEqualThreshold maxRat(final threshold=dPEleMax) if use_powerRateLimit
    "Check if demand rate is more than the maximum rate"
    annotation(Placement(transformation(extent={{0,-50}, {20,-30}})));
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis maxPow(final uLow=0.99*PEleMax, final uHigh=1.01*PEleMax + 1e-6)
    "Check if the electric power demand is more than the maximum power production"
    annotation(Placement(transformation(extent={{-40, 30}, {-20, 50}})));
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis minPow(final uLow=0.99*PEleMin-1e-6, final uHigh=1.01*PEleMin)
...
equation
  ...
  connect(not2.y, assMesDP.u)
    annotation(Line(points={{62,-40}, {68,-40}}, color={255, 0, 255}));
  connect(PEleDem, maxPow.u)
    annotation(Line(points={{-120, 0}, {-90, 0}, {-90, 40}, {-42, 40}}, color={0, 0, 127}));
  connect(maxPow.y, nor.u1)
    annotation(Line(points={{-18, 40}, {0, 40}, {0, 20}, {38, 20}}, color={255, 0, 255}));
  connect(PEleDem, minPow.u)
    annotation(Line(points={{-120, 0}, {-82, 0}}, color={0, 0, 127}));
  connect(minPow.y, not1.u)
    annotation(Line(points={{-58, 0}, {-42, 0}}, color={255, 0, 255}));
  connect(not1.y, nor.u2)
    annotation(Line(points={{-18, 0}, {0, 0}, {0, 12}, {38, 12}}, color={255, 0, 255}));
  connect(maxRat.y, not2.u)
    annotation(Line(points={{22,-40}, {38,-40}}, color={255, 0, 255}));
  annotation(defaultComponentName="assPow", Diagram(coordinateSystem(extent={{-100,-80}, {100, 80}})), Icon(coordinateSystem(extent={{-100,-100}, {100, 100}}), graphics={Rectangle(extent={{-100,-100}, {100, 100}}, lineColor={0, 0, 127}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Polygon(points={{0, 80}, {-80,-60}, {80,-60}, {0, 80}}, fillColor={0, 0, 0}, fillPattern=FillPattern.Solid, pattern=LinePattern.None, lineColor={0, 0, 0}), Polygon(points={{0, 72}, {-72,-56}, {72,-56}, {0, 72}}, lineColor={0, 0, 0}, fillColor={255, 255, 170}, fillPattern=FillPattern.Solid), Rectangle(extent={{-4, 38}, {2,-24}}, fillColor={0, 0, 0}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Ellipse(extent={{-6,-32}, {4,-42}}, pattern=LinePattern.None, fillColor={0, 0, 0}, fillPattern=FillPattern.Solid)}), Documentation(info="<html>
<p>
