...
model AssertWaterTemperature
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TWat(final unit="K", displayUnit="degC")
    "Water outlet temperature"
    annotation(Placement(transformation(extent={{-140,-20}, {-100, 20}}), iconTransformation(extent={{-140,-20}, {-100, 20}})));
  Buildings.Controls.OBC.CDL.Utilities.Assert assMes(final message="Water temperature is higher than the maximum!")
    "Assert function for checking water temperature"
    annotation(Placement(transformation(extent={{60,-10}, {80, 10}})));
protected
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hys(final uLow=TWatMax-THys, final uHigh=TWatMax + THys)
