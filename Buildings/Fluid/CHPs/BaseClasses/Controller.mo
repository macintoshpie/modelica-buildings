...
model Controller
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TEng(
    ...
    annotation(
      Placement(
        ...
        iconTransformation(
          extent={{-140,-40}, {-100, 0}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput PEleNet(
    final unit="W") if not per.warmUpByTimeDelay
    "Net power output"
    annotation(
      Placement(
        transformation(
          extent={{-300,-60}, {-260,-20}}),
        iconTransformation(
  ...
  Buildings.Fluid.CHPs.BaseClasses.Interfaces.ModeTypeOutput opeMod
    ...
    annotation(
      Placement(
        transformation(
          extent={{260,-20}, {300, 20}}),
        iconTransformation(
          extent={{100,-10}, {120, 10}})));
  Modelica.StateGraph.Step staBy(
    nOut=2)
    "Standby step"
    annotation(
      Placement(
        transformation(
          extent={{-60,-10}, {-40, 10}})));
  Modelica.StateGraph.Step pumOn(
    nOut=2)
    "Pump on step"
    annotation(
      Placement(
        transformation(
          extent={{0,-10}, {20, 10}})));
  Modelica.StateGraph.StepWithSignal warUp(
    nIn=2,
    nOut=2)
    "Warm-up step"
    annotation(
      Placement(
        transformation(
          extent={{70,-10}, {90, 10}})));
  Modelica.StateGraph.StepWithSignal cooDow(
    nIn=2,
    nOut=2)
    "Cool-down step"
    annotation(
...
protected
  ...
  Modelica.StateGraph.InitialStep plaOff(
    final nIn=3)
    "Plant is off"
    annotation(
      Placement(
...
equation
  ...
  connect(nor.outPort[1], transition8.inPort)
    annotation(
      Line(
        points={{200.5, 0}, {216, 0}},
        color={0, 0, 0}));
  connect(TEng, warUpCtr.TEng)
    annotation(
      Line(
        points={{-280, 0}, {-220, 0}, {-220,-38}, {88,-38}},
        color={0, 0, 127}));
  ...
  connect(transition6.outPort, nor.inPort[1])
    annotation(
      Line(
        points={{161.5, 0}, {179, 0}},
        color={0, 0, 0}));
  connect(assWatMas.mWat_flow, mWat_flow)
    annotation(
      Line(
        points={{38, 36}, {-120, 36}, {-120, 40}, {-280, 40}},
        color={0, 0, 127}));
  connect(minWatFlo.y, max.u1)
    annotation(
      Line(
        points={{-198, 200}, {-180, 200}, {-180, 186}, {-162, 186}},
        color={0, 0, 127}));
  connect(con.y, max.u2)
    annotation(
      Line(
        points={{-198, 160}, {-180, 160}, {-180, 174}, {-162, 174}},
        color={0, 0, 127}));
  connect(max.y, add2.u1)
    annotation(
      Line(
        points={{-138, 180}, {-120, 180}, {-120, 186}, {-102, 186}},
        color={0, 0, 127}));
  connect(add2.y, hys.u)
    annotation(
      Line(
        points={{-78, 180}, {-62, 180}},
        color={0, 0, 127}));
  connect(hys.y, goSig.u1)
    annotation(
      Line(
        points={{-38, 180}, {-22, 180}},
        color={255, 0, 255}));
  connect(mWat_flow, add2.u2)
    annotation(
      Line(
        points={{-280, 40}, {-120, 40}, {-120, 174}, {-102, 174}},
        color={0, 0, 127}));
  connect(runSig, goSig.u2)
    annotation(
      Line(
        points={{-280, 120}, {-30, 120}, {-30, 172}, {-22, 172}},
        color={255, 0, 255}));
  connect(runSig, notRunSig.u)
    annotation(
      Line(
        points={{-280, 120}, {-240, 120}, {-240, 80}, {-222, 80}},
        color={255, 0, 255}));
  connect(runSig, assWatMas.runSig)
    annotation(
      Line(
        points={{-280, 120}, {-240, 120}, {-240, 44}, {38, 44}},
        color={255, 0, 255}));
  connect(avaSig, transition1.condition)
    annotation(
      Line(
        points={{-280, 180}, {-250, 180}, {-250,-20}, {-80,-20}, {-80,-12}},
        color={255, 0, 255}));
  connect(runSig, transition2.condition)
    annotation(
      Line(
        points={{-280, 120}, {-240, 120}, {-240,-24}, {-20,-24}, {-20,-12}},
        color={255, 0, 255}));
  connect(avaSig, notAvaSig.u)
    annotation(
      Line(
        points={{-280, 180}, {-250, 180}, {-250,-180}, {-222,-180}},
        color={255, 0, 255}));
  connect(goSig.y, transition4.condition)
    annotation(
      Line(
        points={{2, 180}, {20, 180}, {20, 56}, {-140, 56}, {-140,-28}, {40,-28}, {40,-12}},
        color={255, 0, 255}));
  connect(and1.y, transition6.condition)
    annotation(
      Line(
        points={{142,-20}, {160,-20}, {160,-12}},
        color={255, 0, 255}));
  connect(notAvaSig.y, transition3.condition)
    annotation(
      Line(
        points={{-198,-180}, {-20,-180}, {-20,-92}},
        color={255, 0, 255}));
  connect(and3.y, transition7.condition)
    annotation(
      Line(
        points={{122,-160}, {220,-160}, {220,-92}},
        color={255, 0, 255}));
  connect(and4.y, transition10.condition)
    annotation(
      Line(
        points={{230,-200}, {240,-200}, {240,-72}},
        color={255, 0, 255}));
  connect(transition3.outPort, plaOff.inPort[1])
    annotation(
      Line(
        points={{-18.5,-80}, {0,-80}, {0,-100}, {-130,-100}, {-130, 0.666667}, {-121, 0.666667}},
        color={0, 0, 0}));
  connect(transition5.outPort, plaOff.inPort[2])
    annotation(
      Line(
        points={{41.5,-80}, {48,-80}, {48,-102}, {-132,-102}, {-132, 0}, {-121, 0}},
        color={0, 0, 0}));
  connect(transition10.outPort, plaOff.inPort[3])
    annotation(
      Line(
        points={{241.5,-60}, {250,-60}, {250,-104}, {-134,-104}, {-134,-0.666667}, {-121,-0.666667}},
        color={0, 0, 0}));
  connect(goSig.y, and3.u2)
    annotation(
      Line(
        points={{2, 180}, {20, 180}, {20, 56}, {-140, 56}, {-140,-152}, {98,-152}},
        color={255, 0, 255}));
  connect(optCooDow.y, and3.u1)
    annotation(
      Line(
        points={{82,-180}, {90,-180}, {90,-160}, {98,-160}},
        color={255, 0, 255}));
  ...
  connect(staBy.outPort[1], transition2.inPort)
    annotation(
      Line(
        points={{-39.5, 0.25}, {-32, 0.25}, {-32, 0}, {-24, 0}},
        color={0, 0, 0}));
  connect(staBy.outPort[2], transition3.inPort)
    annotation(
      Line(
        points={{-39.5,-0.25}, {-32,-0.25}, {-32,-80}, {-24,-80}},
        color={0, 0, 0}));
  ...
  connect(pumOn.outPort[1], transition4.inPort)
    annotation(
      Line(
        points={{20.5, 0.25}, {28, 0}, {36, 0}},
        color={0, 0, 0}));
  connect(pumOn.outPort[2], transition5.inPort)
    annotation(
      Line(
        points={{20.5,-0.25}, {26,-0.25}, {26,-80}, {36,-80}},
        color={0, 0, 0}));
  connect(warUp.outPort[1], transition6.inPort)
    annotation(
      Line(
        points={{90.5, 0.25}, {110, 0.25}, {110, 0}, {156, 0}},
        color={0, 0, 0}));
  connect(warUp.outPort[2], transition9.inPort)
    annotation(
      Line(
        points={{90.5,-0.25}, {96,-0.25}, {96,-20}, {66,-20}, {66,-80}, {86,-80}},
        color={0, 0, 0}));
  connect(warUp.active, warUpCtr.actWarUp)
    annotation(
      Line(
        points={{80,-11}, {80,-34}, {88,-34}},
        color={255, 0, 255}));
  connect(warUpCtr.y, and1.u2)
    annotation(
      Line(
        points={{112,-40}, {114,-40}, {114,-28}, {118,-28}},
        color={255, 0, 255}));
  connect(goSig.y, and1.u1)
    annotation(
      Line(
        points={{2, 180}, {100, 180}, {100,-20}, {118,-20}},
        color={255, 0, 255}));
  connect(transition4.outPort, warUp.inPort[1])
    annotation(
      Line(
        points={{41.5, 0}, {50, 0}, {50, 0.5}, {69, 0.5}},
        color={0, 0, 0}));
  connect(transition7.outPort, warUp.inPort[2])
    annotation(
      Line(
        points={{221.5,-80}, {230,-80}, {230,-120}, {60,-120}, {60, 0}, {69, 0}, {69,-0.5}},
        color={0, 0, 0}));
  connect(transition8.outPort, cooDow.inPort[1])
    annotation(
      Line(
        points={{221.5, 0}, {240, 0}, {240,-40}, {140,-40}, {140,-79.5}, {159,-79.5}},
        color={0, 0, 0}));
  connect(transition9.outPort, cooDow.inPort[2])
    annotation(
      Line(
        points={{91.5,-80}, {140,-80}, {140,-80.5}, {159,-80.5}},
        color={0, 0, 0}));
  connect(cooDow.outPort[1], transition10.inPort)
    annotation(
      Line(
        points={{180.5,-79.75}, {200,-79.75}, {200,-60}, {236,-60}},
        color={0, 0, 0}));
  connect(cooDow.outPort[2], transition7.inPort)
    annotation(
      Line(
        points={{180.5,-80.25}, {182,-80.25}, {182,-80}, {216,-80}},
        color={0, 0, 0}));
  connect(timer.y, timeDel.u)
    annotation(
      Line(
        points={{174,-180}, {178,-180}},
        color={0, 0, 127}));
  connect(cooDow.active, timer.u)
    annotation(
      Line(
        points={{170,-91}, {170,-140}, {140,-140}, {140,-180}, {150,-180}},
        color={255, 0, 255}));
  connect(timeDel.y, and4.u1)
    annotation(
      Line(
        points={{202,-180}, {204,-180}, {204,-200}, {206,-200}},
        color={255, 0, 255}));
  connect(PEleNet, warUpCtr.PEleNet)
    annotation(
      Line(
        points={{-280,-40}, {80,-40}, {80,-42}, {88,-42}},
        color={0, 0, 127}));
  connect(PEle, warUpCtr.PEle)
    annotation(
      Line(
        points={{-280,-80}, {-40,-80}, {-40,-46}, {88,-46}},
        color={0, 0, 127}));
  connect(notRunSig.y, transition5.condition)
    annotation(
      Line(
        points={{-198, 80}, {-180, 80}, {-180,-120}, {40,-120}, {40,-92}},
        color={255, 0, 255}));
  connect(goSig.y, noGo.u)
    annotation(
      Line(
        points={{2, 180}, {20, 180}, {20, 100}, {38, 100}},
        color={255, 0, 255}));
  connect(noGo.y, transition8.condition)
    annotation(
      Line(
        points={{62, 100}, {220, 100}, {220, 12}},
        color={255, 0, 255}));
  connect(noGo.y, and4.u2)
    annotation(
      Line(
        points={{62, 100}, {80, 100}, {80, 60}, {-144, 60}, {-144,-208}, {206,-208}},
        color={255, 0, 255}));
  connect(noGo.y, transition9.condition)
    annotation(
      Line(
        points={{62, 100}, {80, 100}, {80, 60}, {-144, 60}, {-144,-140}, {90,-140}, {90,-92}},
        color={255, 0, 255}));
  annotation(
