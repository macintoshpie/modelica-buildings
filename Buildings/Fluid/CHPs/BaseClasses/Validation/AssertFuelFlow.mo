...
model AssertFuelFlow
  ...
  Buildings.Fluid.CHPs.BaseClasses.AssertFuelFlow assFue(final dmFueMax_flow=per.dmFueMax_flow)
    "Assert if fuel mass flow rate is outside boundaries"
    annotation(Placement(transformation(extent={{40,-10}, {60, 10}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.TimeTable mFue_flow(final table=[
    0, 0;
    300, 1;
    600, 3.5;
    900, 0], final smoothness=Buildings.Controls.OBC.CDL.Types.Smoothness.ConstantSegments)
    "Fuel mass flow rate"
    annotation(Placement(transformation(extent={{-60,-10}, {-40, 10}})));
equation
  connect(mFue_flow.y[1], assFue.mFue_flow)
    annotation(Line(points={{-38, 0}, {38, 0}}, color={0, 0, 127}));
  annotation(experiment(StopTime=1200, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/CHPs/BaseClasses/Validation/AssertFuelFlow.mos"
