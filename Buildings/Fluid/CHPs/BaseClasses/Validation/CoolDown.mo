...
model CoolDown
  ...
  Buildings.Fluid.CHPs.BaseClasses.Types.Mode actMod
    "Mode indicator";
  Modelica.Blocks.Sources.BooleanTable runSig(final startValue=true, table={300, 600, 660, 690})
    "Plant run signal"
  ...
  Buildings.Controls.OBC.CDL.Logical.And and1
    "Plant run and cool-down is optional"
    annotation(Placement(transformation(extent={{-20,-70}, {0,-50}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.Constant cooDowOpt(final k=per.coolDownOptional)
...
protected
  ...
  Modelica.StateGraph.Step off(final nIn=1, final nOut=1)
    "Off mode"
...
equation
  ...
  connect(transition4.outPort, norm.inPort[1])
    annotation(Line(points={{-41.5, 0}, {-90, 0}, {-90, 50.5}, {-81, 50.5}}, color={0, 0, 0}));
  connect(transition1.inPort, norm.outPort[1])
    annotation(Line(points={{-44, 70}, {-52, 70}, {-52, 50.25}, {-59.5, 50.25}}, color={0, 0, 0}));
  connect(off.inPort[1], transition2.outPort)
    annotation(Line(points={{39, 50}, {21.5, 50}}, color={0, 0, 0}));
  ...
  connect(transition3.outPort, norm.inPort[2])
    annotation(Line(points={{81.5, 50}, {90, 50}, {90,-20}, {-90,-20}, {-90, 49.5}, {-81, 49.5}}, color={0, 0, 0}));
  connect(transition5.inPort, norm.outPort[2])
    annotation(Line(points={{-44, 30}, {-52, 30}, {-52, 49.75}, {-59.5, 49.75}}, color={0, 0, 0}));
  connect(runSig.y, notRun.u)
    annotation(Line(points={{-59,-100}, {-22,-100}}, color={255, 0, 255}));
  connect(notRun.y, transition1.condition)
    annotation(Line(points={{2,-100}, {40,-100}, {40,-10}, {-26,-10}, {-26, 52}, {-40, 52}, {-40, 58}}, color={255, 0, 255}));
  connect(cooDowOpt.y, and1.u1)
    annotation(Line(points={{-58,-60}, {-22,-60}}, color={255, 0, 255}));
  connect(runSig.y, and1.u2)
    annotation(Line(points={{-59,-100}, {-40,-100}, {-40,-68}, {-22,-68}}, color={255, 0, 255}));
  connect(runSig.y, transition3.condition)
    annotation(Line(points={{-59,-100}, {-40,-100}, {-40,-40}, {80,-40}, {80, 38}}, color={255, 0, 255}));
  connect(and1.y, transition4.condition)
    annotation(Line(points={{2,-60}, {20,-60}, {20,-30}, {-40,-30}, {-40,-12}}, color={255, 0, 255}));
  connect(cooDow.outPort[1], transition2.inPort)
    annotation(Line(points={{0.5, 50.25}, {8, 50.25}, {8, 50}, {16, 50}}, color={0, 0, 0}));
  connect(timer.y, timeDel.u)
    annotation(Line(points={{22, 10}, {28, 10}}, color={0, 0, 127}));
  connect(cooDow.outPort[2], transition4.inPort)
    annotation(Line(points={{0.5, 49.75}, {10, 49.75}, {10, 30}, {-20, 30}, {-20, 0}, {-36, 0}}, color={0, 0, 0}));
  connect(transition1.outPort, cooDow.inPort[1])
    annotation(Line(points={{-38.5, 70}, {-32, 70}, {-32, 50}, {-22, 50}, {-22, 50.5}, {-21, 50.5}}, color={0, 0, 0}));
  connect(transition5.outPort, cooDow.inPort[2])
    annotation(Line(points={{-38.5, 30}, {-32, 30}, {-32, 49.5}, {-21, 49.5}}, color={0, 0, 0}));
  ...
  connect(timeDel.y, transition2.condition)
    annotation(Line(points={{52, 10}, {60, 10}, {60, 30}, {20, 30}, {20, 38}}, color={255, 0, 255}));
  annotation(experiment(StopTime=900, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/CHPs/BaseClasses/Validation/CoolDown.mos"
