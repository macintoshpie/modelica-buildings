...
model EnergyConversionNormal
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp PEle(final height=5000, final duration=360, final offset=0, final startTime=600)
    "Electric power"
...
equation
  connect(TWatIn.y, opeModBas.TWatIn)
    annotation(Line(points={{-38,-30}, {0,-30}, {0, 4}, {38, 4}}, color={0, 0, 127}));
  connect(mWat_flow.y, opeModBas.mWat_flow)
    annotation(Line(points={{-38, 10}, {38, 10}}, color={0, 0, 127}));
  connect(PEle.y, opeModBas.PEle)
    annotation(Line(points={{-38, 50}, {0, 50}, {0, 16}, {38, 16}}, color={0, 0, 127}));
  annotation(experiment(StopTime=1500, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/CHPs/BaseClasses/Validation/EnergyConversionNormal.mos"
