...
model EngineTemperature
  ...
  Buildings.Fluid.CHPs.BaseClasses.EngineTemperature eng(final UAHex=per.UAHex, final UALos=per.UALos, final capEng=per.capEng, final TEngIni=273.15 + 20)
    "Heat exchange within the engine control volume"
    annotation(Placement(transformation(extent={{40, 0}, {60, 20}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp QGen(final height=5000, final duration=360, final offset=0, final startTime=600)
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant TWat(y(final unit="K", displayUnit="degC"), final k=273.15 + 60)
    "Water temperature"
...
equation
  ...
  connect(rooTem.port, eng.TRoo)
    annotation(Line(points={{0, 50}, {20, 50}, {20, 15.8}, {40, 15.8}}, color={191, 0, 0}));
  ...
  connect(cooWatTem.port, eng.TWat)
    annotation(Line(points={{0,-30}, {20,-30}, {20, 4}, {40, 4}}, color={191, 0, 0}));
  annotation(experiment(StopTime=1500, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/CHPs/BaseClasses/Validation/EngineTemperature.mos"
