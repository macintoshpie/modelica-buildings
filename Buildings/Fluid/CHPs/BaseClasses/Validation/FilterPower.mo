...
model FilterPower
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.TimeTable PEleDem(table=[
    0, 0;
    300, 500;
    600, 2000;
    900, 3000;
    1200, 0;
    1500, 6000;
    1800, 6000], smoothness=Buildings.Controls.OBC.CDL.Types.Smoothness.ConstantSegments)
    "Electricity demand"
    annotation(Placement(transformation(extent={{-60,-10}, {-40, 10}})));
  Buildings.Fluid.CHPs.BaseClasses.FilterPower filPow(final PEleMax=per.PEleMax, final PEleMin=per.PEleMin, final dPEleMax=per.dPEleMax, final use_powerRateLimit=per.use_powerRateLimit)
    "Constraints for electric power"
    annotation(Placement(transformation(extent={{20,-10}, {40, 10}})));
equation
  connect(PEleDem.y[1], filPow.PEleDem)
    annotation(Line(points={{-38, 0}, {18, 0}}, color={0, 0, 127}));
  annotation(experiment(StopTime=1800, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/CHPs/BaseClasses/Validation/FilterPower.mos"
