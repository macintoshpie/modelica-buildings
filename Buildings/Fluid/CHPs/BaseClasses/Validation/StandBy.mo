...
model StandBy
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant mWat_flow(final k=0.5)
...
protected
  ...
  Modelica.StateGraph.TransitionWithSignal transition3(final enableTimer=true, final waitTime=60)
    "Pump on to off mode"
...
equation
  ...
  connect(transition4.outPort, off.inPort[1])
    annotation(Line(points={{-41.5, 40}, {-90, 40}, {-90, 80.5}, {-81, 80.5}}, color={0, 0, 0}));
  connect(transition3.outPort, off.inPort[2])
    annotation(Line(points={{81.5, 80}, {92, 80}, {92, 20}, {-90, 20}, {-90, 79.5}, {-81, 79.5}}, color={0, 0, 0}));
  connect(avaSig.y, transition1.condition)
    annotation(Line(points={{-59,-40}, {-50,-40}, {-50, 60}, {-40, 60}, {-40, 68}}, color={255, 0, 255}));
  connect(avaSig.y, notAva.u)
    annotation(Line(points={{-59,-40}, {-50,-40}, {-50,-20}, {-90,-20}, {-90, 0}, {-82, 0}}, color={255, 0, 255}));
  ...
  connect(minWatFlo.y, cheMinFlo.u2)
    annotation(Line(points={{2,-80}, {10,-80}, {10,-48}, {18,-48}}, color={0, 0, 127}));
  ...
  connect(pumOn.outPort[1], transition3.inPort)
    annotation(Line(points={{60.5, 80.25}, {68, 80.25}, {68, 80}, {76, 80}}, color={0, 0, 0}));
  ...
  connect(staBy.outPort[1], transition2.inPort)
    annotation(Line(points={{0.5, 80.25}, {8, 80.25}, {8, 80}, {16, 80}}, color={0, 0, 0}));
  ...
  connect(staBy.outPort[2], transition4.inPort)
    annotation(Line(points={{0.5, 79.75}, {10, 79.75}, {10, 40}, {-36, 40}}, color={0, 0, 0}));
  annotation(experiment(StopTime=1500, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/CHPs/BaseClasses/Validation/StandBy.mos"
