...
model WarmUpTemperature
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp TEng(final height=200, final duration=360, final offset=273.15, final startTime=0)
    "Engine temperature"
    annotation(Placement(transformation(extent={{-100, 70}, {-80, 90}})));
  Modelica.Blocks.Sources.BooleanTable runSig(final startValue=true, final table={600})
    "Plant run signal"
...
protected
  Modelica.StateGraph.InitialStep off(final nIn=2, final nOut=2)
    "Off mode"
  ...
  Modelica.StateGraph.Transition transition5(final condition=false, final waitTime=0)
    annotation(Placement(transformation(extent={{-50,-20}, {-30, 0}})));
  Buildings.Controls.OBC.CDL.Logical.Not notRun
    "Plant not run"
    annotation(Placement(transformation(extent={{20,-90}, {40,-70}})));
  WarmUpLeaving warUpCtr(final timeDelayStart=per.timeDelayStart, final TEngNom=per.TEngNom, final warmUpByTimeDelay=per.warmUpByTimeDelay)
...
equation
  ...
  connect(transition4.outPort, off.inPort[1])
    annotation(Line(points={{-41.5,-40}, {-90,-40}, {-90, 30.5}, {-81, 30.5}}, color={0, 0, 0}));
  connect(transition1.inPort, off.outPort[1])
    annotation(Line(points={{-44, 30}, {-52, 30}, {-52, 30.25}, {-59.5, 30.25}}, color={0, 0, 0}));
  ...
  connect(transition3.outPort, off.inPort[2])
    annotation(Line(points={{81.5, 30}, {90, 30}, {90, 60}, {-90, 60}, {-90, 29.5}, {-81, 29.5}}, color={0, 0, 0}));
  connect(transition5.inPort, off.outPort[2])
    annotation(Line(points={{-44,-10}, {-52,-10}, {-52, 29.75}, {-59.5, 29.75}}, color={0, 0, 0}));
  connect(runSig.y, notRun.u)
    annotation(Line(points={{-69,-80}, {18,-80}}, color={255, 0, 255}));
  connect(runSig.y, transition1.condition)
    annotation(Line(points={{-69,-80}, {-60,-80}, {-60, 10}, {-40, 10}, {-40, 18}}, color={255, 0, 255}));
  connect(notRun.y, transition3.condition)
    annotation(Line(points={{42,-80}, {80,-80}, {80, 18}}, color={255, 0, 255}));
  connect(notRun.y, transition4.condition)
    annotation(Line(points={{42,-80}, {80,-80}, {80,-60}, {-40,-60}, {-40,-52}}, color={255, 0, 255}));
  ...
  connect(transition1.outPort, warUp.inPort[1])
    annotation(Line(points={{-38.5, 30}, {-30, 30}, {-30, 29.5}, {-21, 29.5}}, color={0, 0, 0}));
  connect(warUp.outPort[1], transition2.inPort)
    annotation(Line(points={{0.5, 29.75}, {8, 29.75}, {8, 30}, {16, 30}}, color={0, 0, 0}));
  connect(transition5.outPort, warUp.inPort[2])
    annotation(Line(points={{-38.5,-10}, {-30,-10}, {-30, 30.5}, {-21, 30.5}}, color={0, 0, 0}));
  connect(warUp.outPort[2], transition4.inPort)
    annotation(Line(points={{0.5, 30.25}, {8, 30.25}, {8,-40}, {-36,-40}}, color={0, 0, 0}));
  connect(TEng.y, warUpCtr.TEng)
    annotation(Line(points={{-78, 80}, {-60, 80}, {-60, 100}, {-20, 100}, {-20, 82}, {-2, 82}}, color={0, 0, 127}));
  connect(warUpCtr.y, transition2.condition)
    annotation(Line(points={{22, 80}, {30, 80}, {30, 0}, {20, 0}, {20, 18}}, color={255, 0, 255}));
  connect(PEle[1].y, warUpCtr.PEleNet)
    annotation(Line(points={{-28, 80}, {-20, 80}, {-20, 78}, {-2, 78}}, color={0, 0, 127}));
  connect(PEle[2].y, warUpCtr.PEle)
    annotation(Line(points={{-28, 80}, {-20, 80}, {-20, 74}, {-2, 74}}, color={0, 0, 127}));
  annotation(experiment(StopTime=900, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/CHPs/BaseClasses/Validation/WarmUpTemperature.mos"
