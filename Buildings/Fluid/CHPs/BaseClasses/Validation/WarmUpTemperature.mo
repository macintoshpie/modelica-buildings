  ...
  Modelica.StateGraph.StepWithSignal warUp(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-20, 40}, {0, 20}})));
  Controls.OBC.CDL.Continuous.Sources.Constant PEle[2](
    k={1, 2})
...
equation
  ...
  connect(transition4.outPort, off.inPort[1])
    annotation(
      Line(
        points={{-41.5,-40}, {-90,-40}, {-90, 30.5}, {-81, 30.5}},
        color={0, 0, 0}));
  connect(transition1.inPort, off.outPort[1])
    annotation(
      Line(
        points={{-44, 30}, {-52, 30}, {-52, 30.25}, {-59.5, 30.25}},
        color={0, 0, 0}));
  connect(nor.inPort[1], transition2.outPort)
    annotation(
      Line(
        points={{39, 30}, {21.5, 30}},
        color={0, 0, 0}));
  connect(nor.outPort[1], transition3.inPort)
    annotation(
      Line(
        points={{60.5, 30}, {76, 30}},
        color={0, 0, 0}));
  connect(transition3.outPort, off.inPort[2])
    annotation(
      Line(
        points={{81.5, 30}, {90, 30}, {90, 60}, {-90, 60}, {-90, 29.5}, {-81, 29.5}},
        color={0, 0, 0}));
  connect(transition5.inPort, off.outPort[2])
    annotation(
      Line(
        points={{-44,-10}, {-52,-10}, {-52, 29.75}, {-59.5, 29.75}},
        color={0, 0, 0}));
  ...
  annotation(
    experiment(
      ...
      Tolerance=1e-6),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/CHPs/BaseClasses/Validation/WarmUpTemperature.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
