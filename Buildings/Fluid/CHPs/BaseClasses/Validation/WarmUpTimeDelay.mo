...
protected
  ...
  WarmUpLeaving warUpCtr(final timeDelayStart=per.timeDelayStart, final TEngNom=per.TEngNom, final PEleMax=per.PEleMax, final warmUpByTimeDelay=per.warmUpByTimeDelay)
...
equation
  ...
  connect(transition3.outPort, off.inPort[2])
    annotation(Line(points={{81.5, 30}, {90, 30}, {90, 50}, {-90, 50}, {-90, 29.5}, {-81, 29.5}}, color={0, 0, 0}));
  ...
  connect(runSig.y, notRun.u)
    annotation(Line(points={{-69,-80}, {-22,-80}}, color={255, 0, 255}));
  connect(notRun.y, transition3.condition)
    annotation(Line(points={{2,-80}, {80,-80}, {80, 18}}, color={255, 0, 255}));
  connect(notRun.y, transition4.condition)
    annotation(Line(points={{2,-80}, {20,-80}, {20,-60}, {-40,-60}, {-40,-52}}, color={255, 0, 255}));
  ...
  connect(warUp.outPort[2], transition4.inPort)
    annotation(Line(points={{0.5, 30.25}, {10, 30.25}, {10,-40}, {-36,-40}}, color={0, 0, 0}));
  connect(transition5.outPort, warUp.inPort[2])
    annotation(Line(points={{-38.5,-10}, {-30,-10}, {-30, 30}, {-21, 30}, {-21, 30.5}}, color={0, 0, 0}));
  annotation(experiment(StopTime=900, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/CHPs/BaseClasses/Validation/WarmUpTimeDelay.mos"
