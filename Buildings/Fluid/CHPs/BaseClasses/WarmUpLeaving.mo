...
model WarmUpLeaving
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TEng(final unit="K", displayUnit="degC") if not warmUpByTimeDelay
    "Engine temperature"
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput PEle(final unit="W") if not warmUpByTimeDelay
    "Power demand"
    annotation(Placement(transformation(extent={{-140,-100}, {-100,-60}}), iconTransformation(extent={{-140,-80}, {-100,-40}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput PEleNet(final unit="W") if not warmUpByTimeDelay
    "Net power output"
    annotation(Placement(transformation(extent={{-140,-60}, {-100,-20}}), iconTransformation(extent={{-140,-40}, {-100, 0}})));
protected
  ...
  Buildings.Controls.OBC.CDL.Continuous.GreaterEqualThreshold timeDel(final threshold=timeDelayStart) if warmUpByTimeDelay
    "Check if it has been in warm-up mode by longer than specified time"
    annotation(Placement(transformation(extent={{-40, 50}, {-20, 70}})));
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hysTem(uLow=-0.5, uHigh=0) if not warmUpByTimeDelay
    "Check if actual engine temperature is higher than norminal value"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}})));
  Buildings.Controls.OBC.CDL.Continuous.Add add(final k2=-1) if not warmUpByTimeDelay
    "Difference between actual engine temperature and nominal value"
    annotation(Placement(transformation(extent={{-50,-10}, {-30, 10}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant temEngNom(y(final unit="K", displayUnit="degC"), final k=TEngNom)
    "Nominal engine temperature"
    annotation(Placement(transformation(extent={{-88,-30}, {-68,-10}})));
  Buildings.Controls.OBC.CDL.Continuous.Add add1(final k2=-1) if not warmUpByTimeDelay
    "Difference between actual power output and demand"
    annotation(Placement(transformation(extent={{-50,-70}, {-30,-50}})));
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hysPow(final uLow=-0.01*PEleMax-1e-6, final uHigh=0) if not warmUpByTimeDelay
    "Check if actual power output is higher than demand"
    annotation(Placement(transformation(extent={{-10,-70}, {10,-50}})));
  Buildings.Controls.OBC.CDL.Logical.Or or2 if not warmUpByTimeDelay
    "OR evaluation"
...
equation
  ...
  connect(PEle, add1.u2)
    annotation(Line(points={{-120,-80}, {-80,-80}, {-80,-66}, {-52,-66}}, color={0, 0, 127}));
  connect(hysTem.y, or2.u1)
    annotation(Line(points={{12, 0}, {20, 0}, {20,-20}, {28,-20}}, color={255, 0, 255}));
  connect(hysPow.y, or2.u2)
    annotation(Line(points={{12,-60}, {20,-60}, {20,-28}, {28,-28}}, color={255, 0, 255}));
  connect(timeDel.y, y)
    annotation(Line(points={{-18, 60}, {80, 60}, {80, 0}, {120, 0}}, color={255, 0, 255}));
  connect(or2.y, pre.u)
    annotation(Line(points={{52,-20}, {58,-20}}, color={255, 0, 255}));
  connect(pre.y, y)
    annotation(Line(points={{82,-20}, {90,-20}, {90, 0}, {120, 0}}, color={255, 0, 255}));
  connect(PEleNet, add1.u1)
    annotation(Line(points={{-120,-40}, {-80,-40}, {-80,-54}, {-52,-54}}, color={0, 0, 127}));
