...
model WarmUpLeaving
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput PEle(
    final unit="W") if not warmUpByTimeDelay
    "Power demand"
    annotation(
      Placement(
        transformation(
...
protected
  ...
  Buildings.Controls.OBC.CDL.Continuous.Add add(
    final k2=-1) if not warmUpByTimeDelay
...
equation
  ...
  connect(TEng, add.u1)
    annotation(
      Line(
        points={{-120, 20}, {-80, 20}, {-80, 6}, {-52, 6}},
        color={0, 0, 127}));
  connect(temEngNom.y, add.u2)
    annotation(
      Line(
        points={{-66,-20}, {-60,-20}, {-60,-6}, {-52,-6}},
        color={0, 0, 127}));
  ...
  connect(PEle, add1.u2)
    annotation(
      Line(
        points={{-120,-80}, {-80,-80}, {-80,-66}, {-52,-66}},
        color={0, 0, 127}));
  connect(hysTem.y, or2.u1)
    annotation(
      Line(
        points={{12, 0}, {20, 0}, {20,-20}, {28,-20}},
        color={255, 0, 255}));
  connect(hysPow.y, or2.u2)
    annotation(
      Line(
        points={{12,-60}, {20,-60}, {20,-28}, {28,-28}},
        color={255, 0, 255}));
  connect(timeDel.y, y)
    annotation(
      Line(
        points={{-18, 60}, {80, 60}, {80, 0}, {120, 0}},
        color={255, 0, 255}));
  connect(or2.y, pre.u)
    annotation(
      Line(
  ...
  connect(pre.y, y)
    annotation(
      Line(
        points={{82,-20}, {90,-20}, {90, 0}, {120, 0}},
        color={255, 0, 255}));
  connect(PEleNet, add1.u1)
    annotation(
      Line(
        points={{-120,-40}, {-80,-40}, {-80,-54}, {-52,-54}},
        color={0, 0, 127}));
  annotation(
    defaultComponentName="warUpCtr",
    Documentation(
      info="<html>
<p>
