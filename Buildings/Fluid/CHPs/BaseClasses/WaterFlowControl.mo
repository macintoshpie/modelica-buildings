...
model WaterFlowControl
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TWatIn(final unit="K", displayUnit="degC")
    ...
    annotation(Placement(transformation(extent={{-140,-80}, {-100,-40}}), iconTransformation(extent={{-140,-90}, {-100,-50}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput mWatSet_flow(final unit="kg/s", final quantity="MassFlowRate")
...
protected
  Modelica.Blocks.Sources.BooleanExpression offStaBy(final y=opeMod == Buildings.Fluid.CHPs.BaseClasses.Types.Mode.Off or opeMod == Buildings.Fluid.CHPs.BaseClasses.Types.Mode.StandBy)
    "Check if off mode or stand-by mode"
    annotation(Placement(transformation(extent={{0,-10}, {20, 10}})));
  Buildings.Utilities.Math.Biquadratic mWatIntCon(final a=per.coeMasWat)
    "Internal control of water flow rate "
    annotation(Placement(transformation(extent={{0,-50}, {20,-30}})));
  Buildings.Controls.OBC.CDL.Logical.Switch watFloSet
    "Water flow setpoint"
...
equation
  connect(mWatIntCon.y, watFloSet.u3)
    annotation(Line(points={{21,-40}, {40,-40}, {40,-8}, {58,-8}}, color={0, 0, 127}));
  connect(watFloSet.y, mWatSet_flow)
    annotation(Line(points={{82, 0}, {120, 0}}, color={0, 0, 127}));
  connect(const.y, watFloSet.u1)
    annotation(Line(points={{22, 40}, {40, 40}, {40, 8}, {58, 8}}, color={0, 0, 127}));
  connect(offStaBy.y, watFloSet.u2)
    annotation(Line(points={{21, 0}, {58, 0}}, color={255, 0, 255}));
  connect(PEle, mWatIntCon.u1)
    annotation(Line(points={{-120, 0}, {-20, 0}, {-20,-34}, {-2,-34}}, color={0, 0, 127}));
  connect(TWatIn, to_degC.u)
    annotation(Line(points={{-120,-60}, {-92,-60}, {-92,-46}, {-62,-46}}, color={0, 0, 127}));
  connect(mWatIntCon.u2, to_degC.y)
    annotation(Line(points={{-2,-46}, {-38,-46}}, color={0, 0, 127}));
  annotation(defaultComponentName="conWat", Diagram(coordinateSystem(extent={{-100,-100}, {100, 100}})), Icon(coordinateSystem(extent={{-100,-100}, {100, 100}})), Documentation(info="<html>
<p>
