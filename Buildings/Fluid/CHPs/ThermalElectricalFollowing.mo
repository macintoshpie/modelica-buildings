...
model ThermalElectricalFollowing
  ...
  parameter Controls.OBC.CDL.Types.SimpleController watOutCon=Buildings.Controls.OBC.CDL.Types.SimpleController.PI
    ...
    annotation(
      Dialog(
        group="Cooling water outlet temperature controller",
  ...
  parameter Modelica.SIunits.Time Ti=0.5
    ...
    annotation(
      Dialog(
        group="Cooling water outlet temperature controller",
        enable=switchThermalElectricalFollowing and(watOutCon == Buildings.Controls.OBC.CDL.Types.SimpleController.PI or watOutCon == Buildings.Controls.OBC.CDL.Types.SimpleController.PID)));
  parameter Modelica.SIunits.Time Td=0.1
    "Time constant of derivative block"
    annotation(
      Dialog(
        group="Cooling water outlet temperature controller",
        enable=switchThermalElectricalFollowing and(watOutCon == Buildings.Controls.OBC.CDL.Types.SimpleController.PD or watOutCon == Buildings.Controls.OBC.CDL.Types.SimpleController.PID)));
  ...
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis hys(
    ...
    final uHigh=per.PEleMax*10^(-3))
    "Determine if demand larger than zero"
    annotation(
      Placement(
        transformation(
          extent={{-60, 100}, {-40, 120}})));
  ...
  Buildings.Fluid.CHPs.BaseClasses.WaterFlowControl conWat(
    final per=per) if per.compute_coolingWaterFlowRate
    "Internal controller for water mass flow rate"
    annotation(
      Placement(
        transformation(
          extent={{120, 130}, {140, 150}})));
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.Constant optFol(
    ...
    annotation(
      Placement(
        transformation(
          extent={{40, 230}, {60, 250}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant zer1(
    final k=0) if not switchThermalElectricalFollowing
    "Constant zero"
    annotation(
      Placement(
        transformation(
          extent={{40, 270}, {60, 290}})));
  Buildings.Controls.OBC.CDL.Continuous.Gain gai(
    ...
    annotation(
      Placement(
        transformation(
          extent={{120,-110}, {140,-90}})));
  inner Modelica.StateGraph.StateGraphRoot stateGraphRoot
    annotation(
      Placement(
        transformation(
          extent={{140, 300}, {160, 320}})));
...
equation
  connect(fil.PEle, hys.u)
    annotation(
      Line(
        points={{-118, 90}, {-80, 90}, {-80, 110}, {-62, 110}},
        color={0, 0, 127}));
  connect(runSig.y, opeMod.runSig)
    annotation(
      Line(
        points={{2, 130}, {10, 130}, {10, 164}, {48, 164}},
        color={255, 0, 255}));
  connect(conWat.PEle, fil.PEle)
    annotation(
      Line(
        points={{118, 140}, {90, 140}, {90, 90}, {-118, 90}},
        color={0, 0, 127}));
  connect(mWat_flow.y, eneCon.mWat_flow)
    annotation(
      Line(
        points={{-119, 40}, {-100, 40}, {-100, 32}, {-62, 32}},
        color={0, 0, 127}));
  connect(eng.TEng, eneCon.TEng)
    annotation(
      Line(
        points={{22,-130}, {30,-130}, {30,-40}, {-80,-40}, {-80, 23}, {-62, 23}},
        color={0, 0, 127}));
  connect(mWat_flow.y, opeMod.mWat_flow)
    annotation(
      Line(
        points={{-119, 40}, {-100, 40}, {-100, 68}, {20, 68}, {20, 161}, {48, 161}},
        color={0, 0, 127}));
  connect(TWatIn.y, conWat.TWatIn)
    annotation(
      Line(
        points={{-119, 60}, {100, 60}, {100, 133}, {118, 133}},
        color={0, 0, 127}));
  connect(eneCon.TWatIn, TWatIn.y)
    annotation(
      Line(
        points={{-62, 29}, {-90, 29}, {-90, 60}, {-119, 60}},
        color={0, 0, 127}));
  connect(conWat.mWatSet_flow, mWatSet_flow)
    annotation(
      Line(
        points={{142, 140}, {200, 140}},
        color={0, 0, 127}));
  connect(hys.y, runSig.u2)
    annotation(
      Line(
        points={{-38, 110}, {-30, 110}, {-30, 122}, {-22, 122}},
        color={255, 0, 255}));
  connect(opeMod.opeMod, eneCon.opeMod)
    annotation(
      Line(
        points={{71, 160}, {80, 160}, {80, 80}, {-70, 80}, {-70, 35}, {-61, 35}},
        color={0, 127, 0}));
  connect(opeMod.avaSig, avaSig)
    annotation(
      Line(
        points={{48, 167}, {-30, 167}, {-30, 140}, {-200, 140}},
        color={255, 0, 255}));
  connect(runSig.u1, avaSig)
    annotation(
      Line(
        points={{-22, 130}, {-30, 130}, {-30, 140}, {-200, 140}},
        color={255, 0, 255}));
  connect(conWat.opeMod, opeMod.opeMod)
    annotation(
      Line(
        points={{119, 147}, {80, 147}, {80, 160}, {71, 160}},
        color={0, 127, 0}));
  ...
  connect(eneCon.mFue_flow, mFue_flow)
    annotation(
      Line(
        points={{-38, 33}, {160, 33}, {160, 20}, {200, 20}},
        color={0, 0, 127}));
  connect(eneCon.PEleNet, PEleNet)
    annotation(
      Line(
        points={{-38, 38}, {160, 38}, {160, 60}, {200, 60}},
        color={0, 0, 127}));
  connect(mFue_flow, mFue_flow)
    annotation(
      Line(
        points={{200, 20}, {200, 20}},
        color={0, 0, 127}));
  connect(TWatOut.port, vol.heatPort)
    annotation(
      Line(
        points={{60,-60}, {-20,-60}, {-20,-10}, {-9,-10}},
        color={191, 0, 0}));
  connect(powCon.opeMod, opeMod.opeMod)
    annotation(
      Line(
        points={{119, 100}, {80, 100}, {80, 160}, {71, 160}},
        color={0, 127, 0}));
  connect(powCon.PCon, PCon)
    annotation(
      Line(
        points={{142, 100}, {200, 100}},
        color={0, 0, 127}));
  connect(TRoo, QLos.port_b)
    annotation(
      Line(
        points={{-180,-140}, {-70,-140}},
        color={191, 0, 0}));
  connect(eng.TRoo, QLos.port_a)
    annotation(
      Line(
        points={{0,-124.2}, {-28,-124.2}, {-28,-140}, {-50,-140}},
        color={191, 0, 0}));
  connect(fil.PEle, eneCon.PEle)
    annotation(
      Line(
        points={{-118, 90}, {-80, 90}, {-80, 38}, {-62, 38}},
        color={0, 0, 127}));
  connect(watHea.port_b, eng.TWat)
    annotation(
      Line(
        points={{-20,-110}, {-20,-136}, {0,-136}},
        color={191, 0, 0}));
  connect(vol.heatPort, watHea.port_a)
    annotation(
      Line(
        points={{-9,-10}, {-20,-10}, {-20,-90}},
        color={191, 0, 0}));
  connect(eneCon.QGen_flow, eng.QGen_flow)
    annotation(
      Line(
        points={{-38, 22}, {-20, 22}, {-20,-8}, {-40,-8}, {-40,-130}, {-2,-130}},
        color={0, 0, 127}));
  connect(eng.TEng, opeMod.TEng)
    annotation(
      Line(
        points={{22,-130}, {24,-130}, {24, 158}, {48, 158}},
        color={0, 0, 127}));
  connect(TWatOutSet, cooWatCon.u_s)
    annotation(
      Line(
        points={{-200, 350}, {-62, 350}},
        color={0, 0, 127}));
  ...
  connect(theFol, swi.u2)
    annotation(
      Line(
        points={{-200, 260}, {98, 260}},
        color={255, 0, 255}));
  connect(PEleDem, swi.u3)
    annotation(
      Line(
        points={{-200, 200}, {90, 200}, {90, 252}, {98, 252}},
        color={0, 0, 127}));
  connect(TWatOut.T, cooWatCon.u_m)
    annotation(
      Line(
        points={{80,-60}, {100,-60}, {100,-80}, {-150,-80}, {-150, 330}, {-50, 330}, {-50, 338}},
        color={0, 0, 127}));
  connect(watHea.Q_flow, gai.u)
    annotation(
      Line(
        points={{-10,-100}, {118,-100}},
        color={0, 0, 127}));
  ...
  connect(zer1.y, swi.u1)
    annotation(
      Line(
        points={{62, 280}, {80, 280}, {80, 268}, {98, 268}},
        color={0, 0, 127}));
  connect(optFol.y, swi.u2)
    annotation(
      Line(
        points={{62, 240}, {80, 240}, {80, 260}, {98, 260}},
        color={255, 0, 255}));
  connect(swi.y, fil.PEleDem)
    annotation(
      Line(
        points={{122, 260}, {160, 260}, {160, 180}, {-160, 180}, {-160, 90}, {-142, 90}},
        color={0, 0, 127}));
  connect(eneCon.PEleNet, opeMod.PEleNet)
    annotation(
      Line(
        points={{-38, 38}, {28, 38}, {28, 155}, {48, 155}},
        color={0, 0, 127}));
  connect(PEleDem, opeMod.PEle)
    annotation(
      Line(
        points={{-200, 200}, {40, 200}, {40, 152}, {48, 152}},
        color={0, 0, 127}));
  connect(TRoo, TRooSen.port)
    annotation(
      Line(
        points={{-180,-140}, {-160,-140}, {-160,-120}, {-140,-120}},
        color={191, 0, 0}));
  connect(TRooSen.T, eneCon.TRoo)
    annotation(
      Line(
        points={{-120,-120}, {-90,-120}, {-90, 26}, {-62, 26}},
        color={0, 0, 127}));
  connect(theFol, cooWatCon.trigger)
    annotation(
      Line(
        points={{-200, 260}, {-58, 260}, {-58, 338}},
        color={255, 0, 255}));
  connect(elePowDem.y, swi.u1)
    annotation(
      Line(
        points={{22, 350}, {80, 350}, {80, 268}, {98, 268}},
        color={0, 0, 127}));
  annotation(
    defaultComponentName="eleFol",
    Diagram(
      coordinateSystem(
        extent={{-180,-160}, {180, 380}})),
    Icon(
      coordinateSystem(
