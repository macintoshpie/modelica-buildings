...
block AbsorptionIndirectSteam
  ...
  Modelica.Blocks.Interfaces.RealInput TEvaLvg(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-124,-84}, {-100,-60}}),
        iconTransformation(
          extent={{-120,-80}, {-100,-60}})));
  Modelica.Blocks.Interfaces.RealInput QEva_flow_set(
    final unit="W")
    "Evaporator setpoint heat flow rate"
    annotation(
      Placement(
        transformation(
          extent={{-124,-44}, {-100,-20}}),
        iconTransformation(
  ...
  Modelica.SIunits.Efficiency EIRP(
    min=0)
...
equation
  ...
  if on then
    capFunEva=Buildings.Utilities.Math.Functions.smoothMax(
      x1=1E-6,
      x2=Buildings.Utilities.Math.Functions.polynomial(
        x=TEvaLvg_degC,
        a=per.capFunEva),
      deltaX=Q_flow_small);
    ...
    genConT=Buildings.Utilities.Math.Functions.polynomial(
      x=TConEnt_degC,
      a=per.genConT);
    genEvaT=Buildings.Utilities.Math.Functions.polynomial(
      x=TEvaLvg_degC,
      a=per.genEvaT);
    EIRP=Buildings.Utilities.Math.Functions.polynomial(
      x=PLR,
      a=per.EIRP);
