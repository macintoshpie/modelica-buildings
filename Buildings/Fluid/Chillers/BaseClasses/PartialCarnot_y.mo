...
partial model PartialCarnot_y
  ...
  extends Carnot(
    ...
    final QEva_flow_nominal=if COP_is_for_cooling then-P_nominal*COP_nominal else-P_nominal*(COP_nominal-1),
...
protected
  ...
  Modelica.SIunits.HeatFlowRate QEva_flow_internal(
    start=QEva_flow_nominal)=if COP_is_for_cooling then-COP*P else(1-COP)*P
...
equation
  ...
  connect(PEle.u, y)
    annotation(
      Line(
        points={{58, 0}, {58, 0}, {40, 0}, {40, 90}, {-92, 90}, {-120, 90}},
        color={0, 0, 127}));
  connect(yEva_flow_in.y, eva.u)
    annotation(
      Line(
        points={{-59,-40}, {20,-40}, {20,-54}, {12,-54}},
        color={0, 0, 127}));
  connect(yCon_flow_in.y, con.u)
    annotation(
      Line(
        points={{-59, 40}, {-48, 40}, {-40, 40}, {-40, 66}, {-12, 66}},
        color={0, 0, 127}));
  connect(con.Q_flow, QCon_flow)
    annotation(
      Line(
        points={{11, 66}, {20, 66}, {80, 66}, {80, 90}, {110, 90}},
        color={0, 0, 127}));
  connect(eva.Q_flow, QEva_flow)
    annotation(
      Line(
        points={{-11,-54}, {-20,-54}, {-20,-90}, {110,-90}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
