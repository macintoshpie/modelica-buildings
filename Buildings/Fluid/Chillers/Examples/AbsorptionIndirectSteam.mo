  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp TEvaSet(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60,-40}, {-40,-20}})));
  Modelica.Blocks.Sources.BooleanPulse onOff(
    period=3600/2)
    "Control input"
    annotation(
      Placement(
...
equation
  ...
  connect(absIndSte.port_a2, evaPum.ports[1])
    annotation(
      Line(
        points={{10, 4}, {20, 4}, {20,-30}, {40,-30}},
        color={0, 127, 255}));
  connect(absIndSte.TSet, TEvaSet.y)
    annotation(
      Line(
        points={{-11, 8}, {-30, 8}, {-30,-30}, {-38,-30}},
        color={0, 0, 127}));
  connect(conPum.ports[1], absIndSte.port_a1)
    annotation(
      Line(
        points={{-40, 70}, {-20, 70}, {-20, 16}, {-10, 16}},
        color={0, 127, 255}));
  ...
  annotation(
    experiment(
      ...
      StopTime=14400),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Chillers/Examples/AbsorptionIndirectSteam.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
