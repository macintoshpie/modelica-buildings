  ...
  Buildings.Fluid.Sources.Boundary_pT heaBou(
    ...
    annotation(
      Placement(
        transformation(
          extent={{20, 30}, {0, 50}})));
  Controls.OBC.CDL.Logical.Sources.Constant on(
    k=true)
    "On signal"
    annotation(
      Placement(
        transformation(
          extent={{-130, 20}, {-110, 40}})));
  ...
  Controls.OBC.CDL.Continuous.Gain gai(
    ...
    annotation(
      Placement(
        transformation(
          extent={{52,-50}, {72,-30}})));
  Controls.OBC.CDL.Continuous.Sources.Constant TEnt(
    k=273.15 + 15)
...
equation
  connect(chi.on, on.y)
    annotation(
      Line(
        points={{-41, 2}, {-100, 2}, {-100, 30}, {-108, 30}},
        color={255, 0, 255}));
  connect(conPum.ports[1], chi.port_a1)
    annotation(
      Line(
        points={{-50, 42}, {-42, 42}, {-42, 6}, {-40, 6}},
        color={0, 127, 255}));
  connect(chi.port_b1, heaBou.ports[1])
    annotation(
      Line(
        points={{-20, 6}, {-10, 6}, {-10, 40}, {0, 40}},
        color={0, 127, 255}));
  ...
  connect(proSou.port_b, chi.port_a2)
    annotation(
      Line(
        points={{0,-60}, {10,-60}, {10,-6}, {-20,-6}},
        color={0, 127, 255}));
  connect(pum.port_a, chi.port_b2)
    annotation(
      Line(
        points={{-60,-60}, {-70,-60}, {-70,-6}, {-40,-6}},
        color={0, 127, 255}));
  ...
  connect(chi.QEva_flow, gai.u)
    annotation(
      Line(
        points={{-19,-8.6}, {34,-8.6}, {34,-40}, {50,-40}},
        color={0, 0, 127}));
  connect(QEva_P.u2, chi.P)
    annotation(
      Line(
        points={{88,-26}, {74,-26}, {74,-2}, {-19,-2}},
        color={0, 0, 127}));
  connect(QEva_QGen.u2, chi.QGen_flow)
    annotation(
      Line(
        points={{88, 4}, {34, 4}, {34, 2}, {-19, 2}},
        color={0, 0, 127}));
  connect(gai.y, QEva_P.u1)
    annotation(
      Line(
        points={{74,-40}, {80,-40}, {80,-14}, {88,-14}},
        color={0, 0, 127}));
  connect(gai.y, QEva_QGen.u1)
    annotation(
      Line(
        points={{74,-40}, {80,-40}, {80, 16}, {88, 16}},
        color={0, 0, 127}));
  connect(mPum_flow.y, pum.m_flow_in)
    annotation(
      Line(
        points={{-78,-30}, {-50,-30}, {-50,-48}},
        color={0, 0, 127}));
  connect(TEnt.y, proSou.T_in)
    annotation(
      Line(
        points={{-18,-30}, {-14,-30}, {-14,-48}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Chillers/Examples/AbsorptionIndirectSteamVaryingLoad.mos"
        "Simulate and plot"),
    experiment(
      StartTime=0,
      StopTime=86400,
