...
model AbsorptionIndirectSteam_EnergyPlus
  ...
  Modelica.Blocks.Sources.CombiTimeTable datRea(
    tableOnFile=true,
    fileName=ModelicaServices.ExternalReferences.loadResource("modelica://Buildings/Resources/Data/Fluid/Chillers/Validation/IndirectAbsorptionChiller/IndirectAbsorptionChiller.dat"),
    columns=2 : 11,
  ...
  Modelica.Blocks.Sources.RealExpression QGen_EP(
    y=datRea.y[10])
    "EnergyPlus results: generator heat flow rate"
    annotation(
      Placement(
        transformation(
          extent={{-132,-68}, {-112,-48}})));
  Modelica.Blocks.Sources.RealExpression QCon_EP(
    y=datRea.y[6])
    "EnergyPlus results: condenser heat flow rate"
    annotation(
      Placement(
        transformation(
          extent={{-132,-50}, {-112,-30}})));
  Modelica.Blocks.Sources.RealExpression QEva_EP(
    y=-1*datRea.y[2])
    "EnergyPlus results: evaporator heat flow rate"
    annotation(
      Placement(
        transformation(
          extent={{-132,-34}, {-112,-14}})));
...
equation
  connect(evaPum.ports[1], absChi.port_a2)
    annotation(
      Line(
        points={{60,-30}, {50,-30}, {50,-8}, {42,-8}},
        color={0, 127, 255}));
  connect(absChi.on, realToBoolean.y)
    annotation(
      Line(
        points={{21, 0}, {-10, 0}, {-10, 30}, {-59, 30}},
        color={255, 0, 255}));
  connect(conPum.ports[1], absChi.port_a1)
    annotation(
      Line(
        points={{-20, 50}, {20, 50}, {20, 4}, {22, 4}},
        color={0, 127, 255}));
  ...
  connect(absChi.port_b2, cooVol.ports[1])
    annotation(
      Line(
        points={{22,-8}, {20,-8}, {20,-20}, {-20,-20}},
        color={0, 127, 255}));
  connect(datRea.y[1], realToBoolean.u)
    annotation(
      Line(
        points={{-109, 70}, {-100, 70}, {-100, 30}, {-82, 30}},
        color={0, 0, 127}));
  connect(datRea.y[5], evaPum.m_flow_in)
    annotation(
      Line(
        points={{-109, 70}, {-100, 70}, {-100,-82}, {100,-82}, {100,-38}, {82,-38}},
        color={0, 0, 127}));
  connect(datRea.y[9], conPum.m_flow_in)
    annotation(
      Line(
        points={{-109, 70}, {-100, 70}, {-100, 42}, {-42, 42}},
        color={0, 0, 127}));
  ...
  connect(TConIn.y, conPum.T_in)
    annotation(
      Line(
        points={{-58, 70}, {-52, 70}, {-52, 46}, {-42, 46}},
        color={0, 0, 127}));
  connect(datRea.y[4], TEvaSet.u)
    annotation(
      Line(
        points={{-109, 70}, {-100, 70}, {-100,-4}, {-82,-4}},
        color={0, 0, 127}));
  ...
  connect(datRea.y[3], TEvaIn.u)
    annotation(
      Line(
        points={{-109, 70}, {-100, 70}, {-100,-60}, {-82,-60}},
        color={0, 0, 127}));
  connect(TEvaIn.y, evaPum.T_in)
    annotation(
      Line(
        points={{-58,-60}, {90,-60}, {90,-34}, {82,-34}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Chillers/Validation/AbsorptionIndirectSteam_EnergyPlus.mos"
        "Simulate and plot"),
    experiment(
      StartTime=23500,
      StopTime=32400,
