...
model CarnotVerifyEtaCarnot
  ...
  final parameter Modelica.SIunits.SpecificHeatCapacity cp_default=Medium.specificHeatCapacityCp(Medium.setState_pTX(
    p=Medium.p_default,
    T=Medium.T_default,
    X=Medium.X_default))
  ...
  Sources.MassFlowSource_T bouEva(
    ...
    annotation(
      Placement(
        transformation(
          extent={{80, 20}, {60, 40}})));
  Modelica.Blocks.Sources.Constant TEvaLvg(
    k=273.15 + 5)
...
equation
  connect(bouCon.ports[1], chi_TEva.port_a1)
    annotation(
      Line(
        points={{-60, 56}, {-60, 56}, {-8, 56}},
        color={0, 127, 255}));
  connect(TEvaLvg.y, chi_TEva.TSet)
    annotation(
      Line(
        points={{-29, 80}, {-20, 80}, {-20, 59}, {-10, 59}},
        color={0, 0, 127}));
  connect(bou.ports[1], chi_TEva.port_b2)
    annotation(
      Line(
        points={{-60, 30}, {-34, 30}, {-34, 44}, {-8, 44}},
        color={0, 127, 255}));
  connect(chi_TEva.port_a2, bouEva.ports[1])
    annotation(
      Line(
        points={{12, 44}, {20, 44}, {40, 44}, {40, 30}, {60, 30}},
        color={0, 127, 255}));
  connect(bou1.ports[1], chi_TEva.port_b1)
    annotation(
      Line(
        points={{60, 70}, {40, 70}, {40, 56}, {12, 56}},
        color={0, 127, 255}));
  connect(bouCon1.ports[1], chi_y.port_a1)
    annotation(
      Line(
        points={{-60,-34}, {-60,-34}, {-8,-34}},
        color={0, 127, 255}));
  connect(bou2.ports[1], chi_y.port_b2)
    annotation(
      Line(
        points={{-60,-60}, {-44,-60}, {-44,-46}, {-8,-46}},
        color={0, 127, 255}));
  connect(chi_y.port_a2, bouEva1.ports[1])
    annotation(
      Line(
        points={{12,-46}, {10,-46}, {30,-46}, {30,-60}, {60,-60}},
        color={0, 127, 255}));
  connect(bou3.ports[1], chi_y.port_b1)
    annotation(
      Line(
        points={{60,-20}, {30,-20}, {30,-34}, {12,-34}},
        color={0, 127, 255}));
  connect(y.y, chi_y.y)
    annotation(
      Line(
        points={{-29,-10}, {-20,-10}, {-20,-31}, {-10,-31}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=3600),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Chillers/Validation/CarnotVerifyEtaCarnot.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
