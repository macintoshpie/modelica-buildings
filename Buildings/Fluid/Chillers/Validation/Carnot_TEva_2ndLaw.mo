  ...
  Modelica.Blocks.Sources.Constant TEvaIn(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 70}, {-60, 90}})));
  Modelica.Blocks.Sources.Constant dTEva(
    k=dTEva_nominal)
...
protected
  model Chiller
    ...
    Modelica.Blocks.Math.Add SOut_flow
      ...
      annotation(
        Placement(
          transformation(
            extent={{60, 10}, {80, 30}})));
    Modelica.Blocks.Math.Add dS_flow(
      k1=-1)
    ...
    Modelica.Blocks.Interfaces.RealInput TSetEvaLea(
      unit="K")
      "Set point for evaporator leaving temperature"
      annotation(
        Placement(
          transformation(
            extent={{-144, 10}, {-120, 30}})));
    Modelica.Blocks.Interfaces.RealOutput SGen_flow
      "Entropy generated"
      annotation(
        Placement(
          transformation(
            extent={{140, 30}, {164, 50}})));
  ...
  to compute the Carnot efficiency, which can lead to non-physical results.",
    ...
    connect(SIn_flow.y, dS_flow.u1)
      annotation(
        Line(
          points={{81, 50}, {88, 50}, {88, 46}, {98, 46}},
          color={0, 0, 127}));
    connect(sin1.ports[1], S_a2.port_b)
      annotation(
        Line(
          points={{-78,-50}, {-78,-50}, {-50,-50}},
          color={0, 127, 255}));
    connect(sou1.ports[1], S_a1.port_a)
      annotation(
        Line(
          points={{-80,-10}, {-80,-10}, {-50,-10}},
          color={0, 127, 255}));
    connect(S_a1.port_b, chi.port_a1)
      annotation(
        Line(
          points={{-30,-10}, {-12,-10}, {-12,-32}, {6,-32}},
          color={0, 127, 255}));
    connect(chi.port_b1, S_a4.port_a)
      annotation(
        Line(
          points={{26,-32}, {32,-32}, {32,-10}, {38,-10}},
          color={0, 127, 255}));
    connect(S_a4.port_b, sin2.ports[1])
      annotation(
        Line(
          points={{58,-10}, {64,-10}, {78,-10}},
          color={0, 127, 255}));
    connect(S_a3.port_a, sou2.ports[1])
      annotation(
        Line(
          points={{58,-50}, {70,-50}, {82,-50}},
          color={0, 127, 255}));
    connect(S_a2.port_a, chi.port_b2)
      annotation(
        Line(
          points={{-30,-50}, {-12,-50}, {-12,-44}, {6,-44}},
          color={0, 127, 255}));
    connect(S_a3.port_b, chi.port_a2)
      annotation(
        Line(
          points={{38,-50}, {36,-50}, {36,-44}, {26,-44}},
          color={0, 127, 255}));
    connect(S_a4.S_flow, SOut_flow.u2)
      annotation(
        Line(
          points={{48, 1}, {48, 14}, {58, 14}},
          color={0, 0, 127}));
    connect(S_a2.S_flow, SOut_flow.u1)
      annotation(
        Line(
          points={{-40,-39}, {-40,-30}, {-20,-30}, {-20, 26}, {58, 26}},
          color={0, 0, 127}));
    connect(S_a3.S_flow, SIn_flow.u2)
      annotation(
        Line(
          points={{48,-39}, {48,-28}, {30,-28}, {30, 44}, {58, 44}},
          color={0, 0, 127}));
    connect(S_a1.S_flow, SIn_flow.u1)
      annotation(
        Line(
          points={{-40, 1}, {-40, 1}, {-40, 56}, {58, 56}},
          color={0, 0, 127}));
    connect(SOut_flow.y, dS_flow.u2)
      annotation(
        Line(
          points={{81, 20}, {88, 20}, {88, 34}, {98, 34}},
          color={0, 0, 127}));
    connect(TSetEvaLea, chi.TSet)
      annotation(
        Line(
          points={{-132, 20}, {-132, 20}, {-6, 20}, {-6,-28}, {4,-28}, {4,-29}},
          color={0, 0, 127}));
    connect(SGen_flow, dS_flow.y)
      annotation(
        Line(
          points={{152, 40}, {126, 40}, {121, 40}},
          color={0, 0, 127}));
    connect(TConIn, sou1.T_in)
      annotation(
        Line(
          points={{-132,-20}, {-132,-20}, {-116,-20}, {-110,-20}, {-110,-6}, {-102,-6}},
          color={0, 0, 127}));
    connect(TEvaIn, sou2.T_in)
      annotation(
        Line(
          points={{152,-46}, {152,-46}, {104,-46}},
          color={0, 0, 127}));
    annotation(
      Diagram(
        coordinateSystem(
...
equation
  connect(TSetEvaLvg.u1, TEvaIn.y)
    annotation(
      Line(
        points={{-42, 66}, {-50, 66}, {-50, 80}, {-59, 80}},
        color={0, 0, 127}));
  connect(dTEva.y, TSetEvaLvg.u2)
    annotation(
      Line(
        points={{-59, 50}, {-50, 50}, {-50, 54}, {-42, 54}},
        color={0, 0, 127}));
  connect(TSetEvaLvg.y, TConIn.u1)
    annotation(
      Line(
        points={{-19, 60}, {-10, 60}, {-10, 22}, {-2, 22}},
        color={0, 0, 127}));
  connect(dTConEva.y, TConIn.u2)
    annotation(
      Line(
        points={{-59, 10}, {-30, 10}, {-2, 10}},
        color={0, 0, 127}));
  connect(TEvaIn.y, chi_b.TEvaIn)
    annotation(
      Line(
        points={{-59, 80}, {-59, 80}, {90, 80}, {90,-24}, {90,-34.6}, {80.9231,-34.6}},
        color={0, 0, 127}));
  connect(TSetEvaLvg.y, chi_b.TSetEvaLea)
    annotation(
      Line(
        points={{-19, 60}, {-10, 60}, {-10,-28}, {59.0769,-28}},
        color={0, 0, 127}));
  connect(chi_b.TConIn, TConIn.y)
    annotation(
      Line(
        points={{59.0769,-32}, {40,-32}, {40, 16}, {21, 16}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-06,
      StopTime=1.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Chillers/Validation/Carnot_TEva_2ndLaw.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
