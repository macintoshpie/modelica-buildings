...
partial model Example1
  ...
  Fluid.Movers.FlowControlled_m_flow pump(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, use_inputFilter=false, allowFlowReversal=allowFlowReversal.k, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, nominalValuesDefineDefaultPressureCurve=true)
  ...
  Buildings.Fluid.FixedResistances.PressureDrop[nRes.k] res(redeclare package Medium=Medium, each allowFlowReversal=allowFlowReversal.k, each m_flow_nominal=m_flow_nominal, each dp_nominal=1000, each from_dp=from_dp.k)
...
equation
  connect(bou.ports[1], hea.port_a)
    annotation(Line(points={{-40, 20}, {-30, 20}, {-30, 30}, {-20, 30}}, color={0, 127, 255}));
  connect(pulse.y, hea.TSet)
    annotation(Line(points={{-39, 80}, {-30, 80}, {-30, 38}, {-24, 38}, {-24, 38}, {-22, 38}, {-22, 38}}, color={0, 0, 127}));
  connect(pump.m_flow_in, gain.y)
    annotation(Line(points={{49.8, 42}, {49.8, 80}, {21, 80}}, color={0, 0, 127}));
  ...
  connect(const.y, val.y)
    annotation(Line(points={{-39, 50}, {20, 50}, {20, 42}}, color={0, 0, 127}));
  connect(val.port_3, hea.port_a)
    annotation(Line(points={{20, 20}, {20,-10}, {-30,-10}, {-30, 30}, {-20, 30}}, color={0, 127, 255}));
  for i in 1 : nRes.k loop
    connect(pump.port_b, res[i].port_a)
      annotation(Line(points={{60, 30}, {70, 30}}, color={0, 127, 255}));
  end for;
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-20}, {100, 100}}), graphics), Documentation(info="<html>
<p>
