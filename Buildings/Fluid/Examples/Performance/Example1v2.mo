...
model Example1v2
  ...
  Fluid.Delays.DelayFirstOrder[nRes.k] vol(redeclare each package Medium=Medium, each m_flow_nominal=m_flow_nominal, each allowFlowReversal=allowFlowReversal.k, each nPorts=2, each tau=tau, each energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
...
equation
  for i in 1 : nRes.k loop
    connect(vol[i].ports[1], res[i].port_b)
      annotation(Line(points={{72,-8}, {72,-10}, {100,-10}, {100, 30}, {90, 30}}, color={0, 127, 255}));
    connect(vol[i].ports[2], val.port_3)
      annotation(Line(points={{68,-8}, {68,-10}, {20,-10}, {20, 20}}, color={0, 127, 255}));
  end for;
  annotation(experiment(Tolerance=1e-6, StopTime=20), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-20}, {100, 100}}), graphics), Documentation(info="<html>
<p>
...
</html>"), __Dymola_Commands(file="Resources/Scripts/Dymola/Fluid/Examples/Performance/Example1v2.mos"
    "Simulate and plot"));
end Example1v2;
