...
model Example2
  ...
  Fluid.Movers.FlowControlled_dp pump_dp(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, use_inputFilter=false, allowFlowReversal=false, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, nominalValuesDefineDefaultPressureCurve=true)
  ...
  FixedResistances.PressureDrop[nRes.k] res(redeclare each package Medium=Medium, each m_flow_nominal=m_flow_nominal, each from_dp=from_dp.k, each allowFlowReversal=false, dp_nominal={dp_nominal*(1 + mod(i, 3)) for i in 1 : nRes.k})
...
equation
  connect(pump_dp.port_a, bou.ports[1])
    annotation(Line(points={{-60, 0}, {-80, 0}}, color={0, 127, 255}));
  connect(pump_dp.dp_in, pulse.y)
    annotation(Line(points={{-50.2, 12}, {-50.2, 30}, {-79, 30}}, color={0, 0, 127}));
  connect(res[1].port_a, pump_dp.port_b)
  ...
  connect(res[nRes.k].port_b, pump_dp.port_a)
    annotation(Line(points={{0, 0}, {10, 0}, {10,-18}, {-60,-18}, {-60, 0}}, color={0, 127, 255}));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,-40}, {40, 60}}), graphics), experiment(Tolerance=1e-6, StopTime=20), Documentation(info="<html>
<p>
...
</html>"), __Dymola_Commands(file="Resources/Scripts/Dymola/Fluid/Examples/Performance/Example2.mos"
    "Simulate and plot"));
end Example2;
