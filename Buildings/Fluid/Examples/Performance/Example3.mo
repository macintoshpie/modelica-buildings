...
model Example3
  ...
  FixedResistances.PressureDrop[nRes.k] res(
    ...
    dp_nominal={dp_nominal*(1 + mod(i, 3)) +(if mergeDp.k then 0 else dp_nominal) for i in 1 : nRes.k})
    "First pressure drop component"
    annotation(
      Placement(
        transformation(
          extent={{-20,-10}, {0, 10}})));
  Modelica.Blocks.Sources.BooleanConstant mergeDp(
    k=true)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60,-42}, {-40,-22}})));
  Modelica.Blocks.Sources.BooleanConstant from_dp(
    k=true)
  ...
  FixedResistances.PressureDrop[nRes.k] res1(
    ...
    annotation(
      Placement(
        transformation(
          extent={{20,-10}, {40, 10}})));
  Modelica.Blocks.Sources.IntegerConstant nRes(
    k=6)
...
Submitted: 11th Modelica Conference. Paris, France. Sep. 2015.
    ...
    __Dymola_Commands(
      file="Resources/Scripts/Dymola/Fluid/Examples/Performance/Example3.mos"
        "Simulate and plot"));
end Example3;
