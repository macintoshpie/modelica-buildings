...
model Example3
  ...
  Fluid.Movers.FlowControlled_m_flow pump(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, use_inputFilter=false, allowFlowReversal=false, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, nominalValuesDefineDefaultPressureCurve=true)
  ...
  FixedResistances.PressureDrop[nRes.k] res(redeclare each package Medium=Medium, each m_flow_nominal=m_flow_nominal, each from_dp=from_dp.k, each allowFlowReversal=false, dp_nominal={dp_nominal*(1 + mod(i, 3)) +(if mergeDp.k then 0 else dp_nominal) for i in 1 : nRes.k})
    "First pressure drop component"
  ...
  FixedResistances.PressureDrop[nRes.k] res1(redeclare package Medium=Medium, each m_flow_nominal=m_flow_nominal, each from_dp=from_dp.k, each allowFlowReversal=false, each dp_nominal=if mergeDp.k then 0 else dp_nominal)
...
equation
  connect(pump.port_a, bou.ports[1])
    annotation(Line(points={{-60, 0}, {-80, 0}}, color={0, 127, 255}));
  connect(pulse.y, pump.m_flow_in)
    annotation(Line(points={{-79, 24}, {-50.2, 24}, {-50.2, 12}}, color={0, 0, 127}));
  connect(res.port_b, res1.port_a)
    annotation(Line(points={{0, 0}, {20, 0}}, color={0, 127, 255}));
  for i in 1 : nRes.k loop
    connect(res[i].port_a, pump.port_b)
      annotation(Line(points={{-20, 0}, {-40, 0}}, color={0, 127, 255}));
    connect(res1[i].port_b, pump.port_a)
      annotation(Line(points={{40, 0}, {50, 0}, {50, 20}, {-60, 20}, {-60, 0}}, color={0, 127, 255}));
  end for;
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-60}, {60, 40}}), graphics), experiment(Tolerance=1e-6, StopTime=20), Documentation(revisions="<html>
<ul>
<li>
...
</html>"), __Dymola_Commands(file="Resources/Scripts/Dymola/Fluid/Examples/Performance/Example3.mos"
    "Simulate and plot"));
end Example3;
