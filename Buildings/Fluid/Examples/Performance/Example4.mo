...
model Example4
  ...
  Fluid.MixingVolumes.MixingVolumeMoistAir vol(nPorts=2, ports(m_flow(min={0,-Modelica.Constants.inf})), redeclare package Medium=Medium, massDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, m_flow_nominal=1, V=1, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, allowFlowReversal=allowFlowReversal, prescribedHeatFlowRate=true)
    "Mixing volume for extracting moisture"
    annotation(Placement(transformation(extent={{0, 16}, {20, 36}})));
  Fluid.HeatExchangers.ConstantEffectiveness hex(redeclare package Medium1=Medium, redeclare package Medium2=Medium, m1_flow_nominal=1, m2_flow_nominal=1, dp1_nominal=0, dp2_nominal=0, allowFlowReversal1=allowFlowReversal, allowFlowReversal2=allowFlowReversal)
    "Heat exchanger"
    annotation(Placement(transformation(extent={{-20, 20}, {-40, 0}})));
  Fluid.Sources.Boundary_pT bou(redeclare package Medium=Medium, p=Medium.p_default + 1000, use_p_in=false, use_T_in=true, X={0.02, 0.98}, nPorts=2)
    "Sink and source"
    annotation(Placement(transformation(extent={{-70,-4}, {-50, 16}})));
  Fluid.Sources.MassFlowSource_T sou(redeclare package Medium=Medium, m_flow=1, T=273.15, nPorts=1)
    "Air source"
  ...
  Modelica.Blocks.Sources.Ramp Tin(duration=1, height=20, offset=293.15)
    "Inlet temperature ramp"
    annotation(Placement(transformation(extent={{-52, 30}, {-72, 50}})));
  FixedResistances.PressureDrop res(redeclare package Medium=Medium, m_flow_nominal=1, dp_nominal=100, allowFlowReversal=allowFlowReversal, from_dp=true)
    "Pressure drop component"
    annotation(Placement(transformation(extent={{30, 6}, {50, 26}})));
  Fluid.Sensors.TemperatureTwoPort senTem(redeclare package Medium=Medium, allowFlowReversal=false, m_flow_nominal=1, tau=0)
    "Temperature sensor"
    annotation(Placement(transformation(extent={{-16, 10}, {-4, 22}})));
equation
  connect(phiSat.y, xSat.phi)
    annotation(Line(points={{-11.2, 48}, {28, 48}}, color={0, 0, 127}));
  connect(mCond.y, vol.mWat_flow)
    annotation(Line(points={{-19, 30}, {-16, 30}, {-16, 34}, {-2, 34}}, color={0, 0, 127}));
  connect(Tin.y, bou.T_in)
    annotation(Line(points={{-73, 40}, {-80, 40}, {-80, 10}, {-72, 10}}, color={0, 0, 127}));
  connect(res.port_b, sin.ports[1])
    annotation(Line(points={{50, 16}, {58, 16}}, color={0, 127, 255}));
  connect(senTem.port_b, vol.ports[1])
    annotation(Line(points={{-4, 16}, {8, 16}}, color={0, 127, 255}));
  connect(senTem.T, xSat.T)
    annotation(Line(points={{-10, 22.6}, {-10, 42}, {28, 42}}, color={0, 0, 127}));
  connect(vol.ports[2], res.port_a)
    annotation(Line(points={{12, 16}, {30, 16}}, color={0, 127, 255}));
  connect(hex.port_a1, sou.ports[1])
    annotation(Line(points={{-20, 4}, {0, 4}}, color={0, 127, 255}));
  connect(hex.port_a2, bou.ports[1])
    annotation(Line(points={{-40, 16}, {-46, 16}, {-46, 8}, {-50, 8}}, color={0, 127, 255}));
  connect(hex.port_b1, bou.ports[2])
    annotation(Line(points={{-40, 4}, {-50, 4}}, color={0, 127, 255}));
  connect(hex.port_b2, senTem.port_a)
    annotation(Line(points={{-20, 16}, {-16, 16}}, color={0, 127, 255}));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-80,-20}, {80, 60}}), graphics), Documentation(revisions="<html>
<ul>
<li>
...
</html>"), experiment(Tolerance=1e-6, StopTime=20), __Dymola_Commands(file="Resources/Scripts/Dymola/Fluid/Examples/Performance/Example4.mos"
    "Simulate and plot"));
end Example4;
