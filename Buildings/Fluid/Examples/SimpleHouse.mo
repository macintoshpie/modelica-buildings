...
model SimpleHouse
  ...
  MixingVolumes.MixingVolume zone(
    ...
    annotation(
      Placement(
        transformation(
          extent={{102, 140}, {82, 160}})));
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor convRes(
    R=1/2/A_wall)
  ...
  HeatExchangers.ConstantEffectiveness hexRec(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-54, 114}, {-84, 146}})));
  Modelica.Blocks.Logical.Hysteresis hysRad(
    uLow=273.15 + 20,
    uHigh=273.15 + 22)
  ...
  Modelica.Blocks.Logical.Not not1
    ...
    annotation(
      Placement(
        transformation(
          extent={{-46,-110}, {-26,-90}})));
  Modelica.Blocks.Sources.Constant const_dp(
    k=dpAir_nominal)
    "Pressure head"
    annotation(
      Placement(
        transformation(
          extent={{-52, 150}, {-32, 170}})));
  Modelica.Blocks.Math.Gain gaiWin(
    k=A_win*g_win)
  ...
  Controls.Continuous.LimPID conDam(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-20, 80}, {0, 100}})));
  Modelica.Blocks.Sources.Constant TSetRoo(
    k=273.15 + 24)
  ...
  Modelica.Blocks.Sources.Constant TSupAirCoo(
    k=273.15 + 20)
...
equation
  connect(convRes.port_b, walCap.port)
    annotation(
      Line(
        points={{132, 12}, {132, 12}, {132,-8}},
        color={191, 0, 0}));
  connect(convRes.port_a, zone.heatPort)
    annotation(
      Line(
        points={{132, 32}, {132, 150}, {102, 150}},
        color={191, 0, 0}));
  ...
  connect(wallRes.port_b, walCap.port)
    annotation(
      Line(
        points={{86, 0}, {132, 0}, {132,-6}, {132,-8}},
        color={191, 0, 0}));
  connect(Tout.T, weaBus.TDryBul)
    annotation(
      Line(
        points={{-22, 0}, {-152, 0}, {-152,-8}},
        color={0, 0, 127}));
  ...
  connect(heaWat.port_b, rad.port_a)
    annotation(
      Line(
        points={{64,-106}, {84,-106}, {104,-106}},
        color={0, 127, 255}));
  connect(bouWat.ports[1], heaWat.port_a)
    annotation(
      Line(
        points={{2,-170}, {40,-170}, {40,-106}, {44,-106}},
        color={0, 127, 255}));
  connect(rad.port_b, pump.port_a)
    annotation(
      Line(
        points={{124,-106}, {130,-106}, {130,-170}, {80,-170}},
        color={0, 127, 255}));
  connect(senTemZonAir.port, zone.heatPort)
    annotation(
      Line(
        points={{80, 180}, {80, 180}, {112, 180}, {112, 150}, {102, 150}},
        color={191, 0, 0}));
  connect(window.port, walCap.port)
    annotation(
      Line(
        points={{0,-26}, {132,-26}, {132,-12}, {132,-8}},
        color={191, 0, 0}));
  connect(bouAir.ports[1], hexRec.port_b1)
    annotation(
      Line(
        points={{-102, 142}, {-102, 139.6}, {-84, 139.6}},
        color={0, 127, 255}));
  connect(hexRec.port_a1, zone.ports[1])
    annotation(
      Line(
        points={{-54, 139.6}, {85, 139.6}, {85, 140}, {94, 140}},
        color={0, 127, 255}));
  connect(rad.heatPortCon, zone.heatPort)
    annotation(
      Line(
        points={{112,-98.8}, {112,-98.8}, {112, 48}, {112, 150}, {102, 150}},
        color={191, 0, 0}));
  connect(rad.heatPortRad, walCap.port)
    annotation(
      Line(
        points={{116,-98.8}, {116,-98.8}, {116,-70}, {116,-26}, {132,-26}, {132,-8}},
        color={191, 0, 0}));
  connect(not1.y, booleanToReal1.u)
    annotation(
      Line(
        points={{-25,-100}, {-22,-100}, {-18,-100}},
        color={255, 0, 255}));
  connect(not1.u, hysRad.y)
    annotation(
      Line(
        points={{-48,-100}, {-52,-100}, {-53,-100}},
        color={255, 0, 255}));
  connect(booleanToReal1.y, heaWat.u)
    annotation(
      Line(
        points={{5,-100}, {16,-100}, {26,-100}, {42,-100}},
        color={0, 0, 127}));
  connect(heaWat.port_a, pump.port_b)
    annotation(
      Line(
        points={{44,-106}, {40,-106}, {40,-112}, {40,-170}, {60,-170}},
        color={0, 127, 255}));
  connect(const_dp.y, fan.dp_in)
    annotation(
      Line(
        points={{-31, 160}, {-22, 160}, {-22, 132}, {-22, 132}},
        color={0, 0, 127}));
  connect(gaiWin.y, window.Q_flow)
    annotation(
      Line(
        points={{-39,-26}, {-34,-26}, {-30,-26}, {-20,-26}},
        color={0, 0, 127}));
  connect(gaiWin.u, weaBus.HGloHor)
    annotation(
      Line(
        points={{-62,-26}, {-90,-26}, {-152,-26}, {-152,-8}},
        color={0, 0, 127}));
  connect(booleanToInt.u, not1.y)
    annotation(
      Line(
        points={{-18,-134}, {-22,-134}, {-22,-100}, {-25,-100}},
        color={255, 0, 255}));
  connect(booleanToInt.y, pump.stage)
    annotation(
      Line(
        points={{5,-134}, {32,-134}, {70,-134}, {70,-158}},
        color={255, 127, 0}));
  connect(bouAir.ports[2], hexRec.port_a2)
    annotation(
      Line(
        points={{-102, 138}, {-102, 142}, {-90, 142}, {-90, 120.4}, {-84, 120.4}},
        color={0, 127, 255}));
  connect(hexRec.port_b2, fan.port_a)
    annotation(
      Line(
        points={{-54, 120.4}, {-44, 120.4}, {-44, 120}, {-32, 120}},
        color={0, 127, 255}));
  connect(vavDam.port_b, zone.ports[2])
    annotation(
      Line(
        points={{82, 120}, {90, 120}, {90, 140}},
        color={0, 127, 255}));
  connect(senTemZonAir.T, hysRad.u)
    annotation(
      Line(
        points={{60, 180}, {60, 180}, {-132, 180}, {-132,-100}, {-76,-100}},
        color={0, 0, 127}));
  connect(senTemZonAir.T, conDam.u_s)
    annotation(
      Line(
        points={{60, 180}, {60, 180}, {-132, 180}, {-132, 90}, {-62, 90}, {-22, 90}},
        color={0, 0, 127}));
  connect(conDam.y, vavDam.y)
    annotation(
      Line(
        points={{1, 90}, {26, 90}, {72, 90}, {72, 108}},
        color={0, 0, 127}));
  connect(TSetRoo.y, conDam.u_m)
    annotation(
      Line(
        points={{-39, 70}, {-40, 70}, {-36, 70}, {-40, 70}, {-10, 70}, {-10, 78}},
        color={0, 0, 127}));
  connect(fan.port_b, cooAir.port_a)
    annotation(
      Line(
        points={{-12, 120}, {30, 120}},
        color={0, 127, 255}));
  connect(cooAir.port_b, vavDam.port_a)
    annotation(
      Line(
        points={{50, 120}, {50, 120}, {62, 120}},
        color={0, 127, 255}));
  connect(TSupAirCoo.y, cooAir.TSet)
    annotation(
      Line(
        points={{9, 160}, {20, 160}, {20, 128}, {28, 128}},
        color={0, 0, 127}));
  connect(bouAir.T_in, weaBus.TDryBul)
    annotation(
      Line(
        points={{-124, 144}, {-152, 144}, {-152,-8}},
        color={0, 0, 127}));
  annotation(
    Diagram(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-240,-220}, {200, 220}}),
      graphics={Rectangle(
        ...
        fillPattern=FillPattern.Solid,
        textString="Cooling and ventilation"), Rectangle(
        extent={{40, 40}, {180,-46}},
        fillColor={238, 238, 238},
        fillPattern=FillPattern.Solid,
        pattern=LinePattern.None), Text(
...
It servers as a demonstration case of how the <code>Buildings</code> library can be used.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Examples/SimpleHouse.mos"
        "Simulate and plot"));
end SimpleHouse;
