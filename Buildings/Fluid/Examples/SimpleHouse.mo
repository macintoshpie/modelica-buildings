...
model SimpleHouse
  ...
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor walCap(T(fixed=true), C=10*A_wall*0.05*1000*1000)
    "Thermal mass of walls"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=270, origin={142,-8})));
  MixingVolumes.MixingVolume zone(redeclare package Medium=MediumAir, V=V_zone, nPorts=2, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, m_flow_nominal=mAir_flow_nominal, massDynamics=Modelica.Fluid.Types.Dynamics.DynamicFreeInitial)
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor convRes(R=1/2/A_wall)
    ...
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=270, origin={132, 22})));
  HeatExchangers.Radiators.RadiatorEN442_2 rad(redeclare package Medium=MediumWater, T_a_nominal=273.15 + 50, T_b_nominal=273.15 + 40, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, allowFlowReversal=allowFlowReversal, Q_flow_nominal=QHea_nominal)
    "Radiator"
    annotation(Placement(transformation(extent={{104,-116}, {124,-96}})));
  Sources.Boundary_pT bouAir(redeclare package Medium=MediumAir, nPorts=2, use_T_in=true)
    "Air boundary with constant temperature"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, origin={-112, 140})));
  Sources.Boundary_pT bouWat(redeclare package Medium=MediumWater, nPorts=1)
    "Pressure bound for water circuit"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, origin={-8,-170})));
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor wallRes(R=0.25/A_wall/0.04)
  ...
  HeatExchangers.HeaterCooler_u heaWat(redeclare package Medium=MediumWater, m_flow_nominal=mWat_flow_nominal, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, allowFlowReversal=allowFlowReversal, dp_nominal=5000, Q_flow_nominal=QHea_nominal)
    "Heater for water circuit"
    annotation(Placement(transformation(extent={{44,-116}, {64,-96}})));
  Movers.FlowControlled_m_flow pump(redeclare package Medium=MediumWater, use_inputFilter=false, m_flow_nominal=mWat_flow_nominal, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, allowFlowReversal=allowFlowReversal, nominalValuesDefineDefaultPressureCurve=true, inputType=Buildings.Fluid.Types.InputType.Stages, massFlowRates=mWat_flow_nominal*{1})
    "Pump"
    annotation(Placement(transformation(extent={{80,-180}, {60,-160}})));
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor senTemZonAir
    "Zone air temperature sensor"
    annotation(Placement(transformation(extent={{80, 170}, {60, 190}})));
  Actuators.Dampers.Exponential vavDam(redeclare package Medium=MediumAir, from_dp=true, m_flow_nominal=mAir_flow_nominal, dpDamper_nominal=10, dpFixed_nominal=dpAir_nominal-10)
    "Damper"
    annotation(Placement(transformation(extent={{-10, 10}, {10,-10}}, origin={72, 120})));
  Movers.FlowControlled_dp fan(redeclare package Medium=MediumAir, dp_nominal=dpAir_nominal, use_inputFilter=false, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, nominalValuesDefineDefaultPressureCurve=true, m_flow_nominal=mAir_flow_nominal, show_T=true)
    "Constant head fan"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, origin={-22, 120})));
  ...
  HeatExchangers.ConstantEffectiveness hexRec(redeclare package Medium1=MediumAir, redeclare package Medium2=MediumAir, dp1_nominal=0, dp2_nominal=0, m1_flow_nominal=mAir_flow_nominal, m2_flow_nominal=mAir_flow_nominal, eps=0.85)
  ...
  Controls.Continuous.LimPID conDam(controllerType=Modelica.Blocks.Types.SimpleController.P, yMin=0.1)
  ...
  HeatExchangers.SensibleCooler_T cooAir(energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, allowFlowReversal=allowFlowReversal, m_flow_nominal=mAir_flow_nominal, dp_nominal=0, redeclare package Medium=MediumAir)
    "Cooling for supply air"
...
equation
  ...
  connect(weaDat.weaBus, weaBus)
    annotation(Line(points={{-180,-8}, {-180,-8}, {-152,-8}}, color={255, 204, 51}, thickness=0.5));
  connect(wallRes.port_b, walCap.port)
    annotation(Line(points={{86, 0}, {132, 0}, {132,-6}, {132,-8}}, color={191, 0, 0}));
  ...
  connect(heaWat.port_b, rad.port_a)
    annotation(Line(points={{64,-106}, {84,-106}, {104,-106}}, color={0, 127, 255}));
  connect(bouWat.ports[1], heaWat.port_a)
    annotation(Line(points={{2,-170}, {40,-170}, {40,-106}, {44,-106}}, color={0, 127, 255}));
  connect(rad.port_b, pump.port_a)
    annotation(Line(points={{124,-106}, {130,-106}, {130,-170}, {80,-170}}, color={0, 127, 255}));
  connect(senTemZonAir.port, zone.heatPort)
    annotation(Line(points={{80, 180}, {80, 180}, {112, 180}, {112, 150}, {102, 150}}, color={191, 0, 0}));
  connect(window.port, walCap.port)
    annotation(Line(points={{0,-26}, {132,-26}, {132,-12}, {132,-8}}, color={191, 0, 0}));
  connect(bouAir.ports[1], hexRec.port_b1)
    annotation(Line(points={{-102, 142}, {-102, 139.6}, {-84, 139.6}}, color={0, 127, 255}));
  connect(hexRec.port_a1, zone.ports[1])
    annotation(Line(points={{-54, 139.6}, {85, 139.6}, {85, 140}, {94, 140}}, color={0, 127, 255}));
  connect(rad.heatPortCon, zone.heatPort)
    annotation(Line(points={{112,-98.8}, {112,-98.8}, {112, 48}, {112, 150}, {102, 150}}, color={191, 0, 0}));
  connect(rad.heatPortRad, walCap.port)
    annotation(Line(points={{116,-98.8}, {116,-98.8}, {116,-70}, {116,-26}, {132,-26}, {132,-8}}, color={191, 0, 0}));
  connect(not1.y, booleanToReal1.u)
    annotation(Line(points={{-25,-100}, {-22,-100}, {-18,-100}}, color={255, 0, 255}));
  connect(not1.u, hysRad.y)
    annotation(Line(points={{-48,-100}, {-52,-100}, {-53,-100}}, color={255, 0, 255}));
  connect(booleanToReal1.y, heaWat.u)
    annotation(Line(points={{5,-100}, {16,-100}, {26,-100}, {42,-100}}, color={0, 0, 127}));
  connect(heaWat.port_a, pump.port_b)
    annotation(Line(points={{44,-106}, {40,-106}, {40,-112}, {40,-170}, {60,-170}}, color={0, 127, 255}));
  connect(const_dp.y, fan.dp_in)
    annotation(Line(points={{-31, 160}, {-22, 160}, {-22, 132}, {-22, 132}}, color={0, 0, 127}));
  connect(gaiWin.y, window.Q_flow)
    annotation(Line(points={{-39,-26}, {-34,-26}, {-30,-26}, {-20,-26}}, color={0, 0, 127}));
  connect(gaiWin.u, weaBus.HGloHor)
    annotation(Line(points={{-62,-26}, {-90,-26}, {-152,-26}, {-152,-8}}, color={0, 0, 127}));
  connect(booleanToInt.u, not1.y)
    annotation(Line(points={{-18,-134}, {-22,-134}, {-22,-100}, {-25,-100}}, color={255, 0, 255}));
  connect(booleanToInt.y, pump.stage)
    annotation(Line(points={{5,-134}, {32,-134}, {70,-134}, {70,-158}}, color={255, 127, 0}));
  connect(bouAir.ports[2], hexRec.port_a2)
    annotation(Line(points={{-102, 138}, {-102, 142}, {-90, 142}, {-90, 120.4}, {-84, 120.4}}, color={0, 127, 255}));
  connect(hexRec.port_b2, fan.port_a)
    annotation(Line(points={{-54, 120.4}, {-44, 120.4}, {-44, 120}, {-32, 120}}, color={0, 127, 255}));
  ...
  connect(senTemZonAir.T, hysRad.u)
    annotation(Line(points={{60, 180}, {60, 180}, {-132, 180}, {-132,-100}, {-76,-100}}, color={0, 0, 127}));
  connect(senTemZonAir.T, conDam.u_s)
    annotation(Line(points={{60, 180}, {60, 180}, {-132, 180}, {-132, 90}, {-62, 90}, {-22, 90}}, color={0, 0, 127}));
  connect(conDam.y, vavDam.y)
    annotation(Line(points={{1, 90}, {26, 90}, {72, 90}, {72, 108}}, color={0, 0, 127}));
  connect(TSetRoo.y, conDam.u_m)
    annotation(Line(points={{-39, 70}, {-40, 70}, {-36, 70}, {-40, 70}, {-10, 70}, {-10, 78}}, color={0, 0, 127}));
  ...
  connect(TSupAirCoo.y, cooAir.TSet)
    annotation(Line(points={{9, 160}, {20, 160}, {20, 128}, {28, 128}}, color={0, 0, 127}));
  connect(bouAir.T_in, weaBus.TDryBul)
    annotation(Line(points={{-124, 144}, {-152, 144}, {-152,-8}}, color={0, 0, 127}));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-240,-220}, {200, 220}}), graphics={Rectangle(extent={{-222, 200}, {180, 50}}, fillColor={238, 238, 238}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Rectangle(extent={{-220,-60}, {180,-200}}, fillColor={238, 238, 238}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Rectangle(extent={{-220, 40}, {20,-48}}, fillColor={238, 238, 238}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Text(extent={{-78, 182}, {-212, 198}}, lineColor={0, 0, 127}, fillColor={255, 213, 170}, fillPattern=FillPattern.Solid, textString="Cooling and ventilation"), Rectangle(extent={{40, 40}, {180,-46}}, fillColor={238, 238, 238}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Text(extent={{98, 20}, {32, 38}}, lineColor={0, 0, 127}, fillColor={255, 213, 170}, fillPattern=FillPattern.Solid, textString="Wall"), Text(extent={{-148,-86}, {-214,-68}}, lineColor={0, 0, 127}, fillColor={255, 213, 170}, fillPattern=FillPattern.Solid, textString="Heating"), Text(extent={{-154, 20}, {-212, 38}}, lineColor={0, 0, 127}, fillColor={255, 213, 170}, fillPattern=FillPattern.Solid, textString="Weather")}), experiment(Tolerance=1e-06, StopTime=3.1536e+07), Documentation(revisions="<html>
<ul>
<li>
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Examples/SimpleHouse.mos"
    "Simulate and plot"));
end SimpleHouse;
