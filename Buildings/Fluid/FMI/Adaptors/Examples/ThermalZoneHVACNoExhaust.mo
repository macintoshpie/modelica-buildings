...
equation
  connect(mov.port_b, hea.port_a)
    annotation(
      Line(
        points={{-70, 10}, {-70, 10}, {-40, 10}},
        color={0, 127, 255}));
  connect(hex.port_b1, mov.port_a)
    annotation(
      Line(
        points={{-100,-14}, {-94,-14}, {-94, 10}, {-90, 10}},
        color={0, 127, 255}));
  connect(out.ports[1], hex.port_b2)
    annotation(
      Line(
        points={{-140,-22}, {-130,-22}, {-130,-26}, {-120,-26}},
        color={0, 127, 255}));
  connect(out.ports[2], hex.port_a1)
    annotation(
      Line(
        points={{-140,-18}, {-130,-18}, {-130,-14}, {-120,-14}},
        color={0, 127, 255}));
  connect(conPI.y, hea.u)
    annotation(
      Line(
        points={{-49, 50}, {-46, 50}, {-46, 16}, {-42, 16}},
        color={0, 0, 127}));
  ...
  connect(vol.heatPort, theCon.port_b)
    annotation(
      Line(
        points={{140, 30}, {140, 30}, {122, 30}, {122,-50}, {130,-50}},
        color={191, 0, 0}));
  ...
  connect(TOutZon.y, TBou.T)
    annotation(
      Line(
        points={{209,-50}, {182,-50}},
        color={0, 0, 127}));
  connect(hvacAda.TAirZon[1], conPI.u_m)
    annotation(
      Line(
        points={{24,-1}, {24,-10}, {-60,-10}, {-60, 38}},
        color={0, 0, 127}));
  connect(con.ports[1], vol.ports[1])
    annotation(
      Line(
        points={{100, 12}, {148, 12}, {148, 20}},
        color={0, 127, 255}));
  connect(vol.heatPort, con.heaPorAir)
    annotation(
      Line(
        points={{140, 30}, {130, 30}, {122, 30}, {122, 2}, {100, 2}},
        color={191, 0, 0}));
  connect(con.ports[2], vol.ports[2])
    annotation(
      Line(
        points={{100, 8}, {120, 8}, {152, 8}, {152, 20}},
        color={0, 127, 255}));
  connect(hvacAda.fluPor, con.fluPor)
    annotation(
      Line(
        points={{41, 10}, {57.5, 10}, {57.5, 10}, {77.8, 10}},
        color={0, 0, 255}));
  connect(hea.port_b, hvacAda.ports[1])
    annotation(
      Line(
        points={{-20, 10}, {20, 10}, {20, 12}},
        color={0, 127, 255}));
  connect(hvacAda.ports[2], hex.port_a2)
    annotation(
      Line(
        points={{20, 8}, {-2, 8}, {-2,-26}, {-100,-26}},
        color={0, 127, 255}));
  connect(TOutHVAC.y, out.T_in)
    annotation(
      Line(
        points={{-179,-24}, {-162,-24}, {-162,-24}},
        color={0, 0, 127}));
  annotation(
    Diagram(
      coordinateSystem(
        extent={{-220,-120}, {260, 160}}),
      graphics={Rectangle(
...
First implementation.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FMI/Adaptors/Examples/ThermalZoneHVACNoExhaust.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=172800));
