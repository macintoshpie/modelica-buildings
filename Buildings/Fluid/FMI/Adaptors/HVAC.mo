...
model HVAC
  ...
  Interfaces.Outlet fluPor[nPorts](redeclare each final package Medium=Medium, each final allowFlowReversal=true, each final use_p_in=false)
    "Fluid connector"
  ...
  Modelica.Blocks.Interfaces.RealOutput TAirZon[nPorts](each final unit="K", each displayUnit="degC")
  ...
  Modelica.Blocks.Interfaces.RealOutput X_wZon[nPorts](each final unit="kg/kg") if Medium.nXi > 0
  ...
  Modelica.Blocks.Interfaces.RealOutput CZon[nPorts, Medium.nC](final quantity=fill(Medium.extraPropertiesNames, nPorts))
    "Trace substances of the backward flowing medium in the connector outlet"
    annotation(Placement(transformation(extent={{20, 20}, {-20,-20}}, rotation=90, origin={60,-120}), iconTransformation(extent={{20, 20}, {-20,-20}}, rotation=90, origin={60,-120})));
protected
  Sources.MassFlowSource_T bou(final nPorts=nPorts, redeclare final package Medium=Medium, final use_m_flow_in=false, final use_T_in=true, final use_X_in=Medium.nXi > 0, final use_C_in=Medium.nC > 0, final m_flow=0)
    "Boundary conditions for HVAC system"
    annotation(Placement(transformation(extent={{0,-10}, {-20, 10}})));
  Buildings.Fluid.FMI.Conversion.AirToOutlet con[nPorts](redeclare each final package Medium=Medium, each final allowFlowReversal=true)
    "Converter between the different connectors"
    annotation(Placement(transformation(extent={{60, 60}, {80, 80}})));
  Sensors.MassFlowRate senMasFlo[nPorts](redeclare each final package Medium=Medium, each allowFlowReversal=true)
    "Mass flow rate sensor"
    annotation(Placement(transformation(extent={{-80,-10}, {-60, 10}})));
  Buildings.Fluid.FMI.BaseClasses.X_w_toX x_w_toX(redeclare final package Medium=Medium) if Medium.nXi > 0
    "Conversion from X_w to X"
    annotation(Placement(transformation(extent={{40,-40}, {20,-20}})));
  Modelica.Blocks.Sources.RealExpression hSup[nPorts](final y={inStream(ports[i].h_outflow) for i in 1 : nPorts})
    "Supply air specific enthalpy"
    annotation(Placement(transformation(extent={{-40, 50}, {-20, 70}})));
  RealVectorExpression XiSup[nPorts](each final n=Medium.nXi, final y={inStream(ports[i].Xi_outflow) for i in 1 : nPorts}) if Medium.nXi > 0
    "Water vapor concentration of supply air"
    annotation(Placement(transformation(extent={{-40, 30}, {-20, 50}})));
  RealVectorExpression CSup[nPorts](each final n=Medium.nC, final y={inStream(ports[i].C_outflow) for i in 1 : nPorts}) if Medium.nC > 0
...
equation
  connect(con.outlet, fluPor)
    annotation(Line(points={{81, 70}, {90, 70}, {90, 0}, {106, 0}, {106, 0}, {110, 0}, {110, 0}}, color={0, 0, 255}));
  connect(senMasFlo.m_flow, con.m_flow)
    annotation(Line(points={{-70, 11}, {-70, 11}, {-70, 78}, {58, 78}}, color={0, 0, 127}));
  connect(hSup.y, con.h)
    annotation(Line(points={{-19, 60}, {20, 60}, {20, 74}, {58, 74}}, color={0, 0, 127}));
  for i in 1 : nPorts loop
    connect(XiSup[i].y, con[i].Xi)
      annotation(Line(points={{-19, 40}, {4, 40}, {28, 40}, {28, 66}, {58, 66}}, color={0, 0, 127}));
    connect(CSup[i].y, con[i].C)
      annotation(Line(points={{-19, 20}, {32, 20}, {32, 62}, {58, 62}}, color={0, 0, 127}));
  end for;
  ...
  connect(con[1].X_wZon, x_w_toX.X_w)
    annotation(Line(points={{70, 58}, {70, 4}, {70,-30}, {42,-30}}, color={0, 0, 127}));
  connect(x_w_toX.X, bou.X_in)
    annotation(Line(points={{18,-30}, {12,-30}, {12,-4}, {2,-4}}, color={0, 0, 127}));
  connect(con[1].CZon, bou.C_in)
    annotation(Line(points={{76, 58}, {76, 58}, {76, 10}, {76,-8}, {2,-8}}, color={0, 0, 127}));
  connect(con.TAirZon, TAirZon)
    annotation(Line(points={{64, 58}, {64,-80}, {-60,-80}, {-60,-120}}, color={0, 0, 127}));
  connect(con.X_wZon, X_wZon)
    annotation(Line(points={{70, 58}, {70, 58}, {70, 4}, {70,-86}, {0,-86}, {0,-120}}, color={0, 0, 127}));
  connect(con.CZon, CZon)
    annotation(Line(points={{76, 58}, {76, 58}, {76,-42}, {76,-92}, {60,-92}, {60,-120}}, color={0, 0, 127}));
  annotation(defaultComponentName="theZonAda", Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 100}}), graphics={Rectangle(extent={{-100, 100}, {100,-100}}, lineColor={0, 0, 0}, fillPattern=FillPattern.Solid, fillColor={255, 255, 255}), Polygon(points={{-100,-100}, {100, 100}, {100,-100}, {-100,-100}}, lineColor={0, 0, 0}, fillColor={215, 215, 215}, fillPattern=FillPattern.Solid), Text(extent={{-150, 110}, {150, 150}}, textString="%name", lineColor={0, 0, 255}), Rectangle(extent={{-90, 20}, {40, 14}}, lineColor={0, 0, 0}, fillPattern=FillPattern.HorizontalCylinder, fillColor={0, 127, 255}), Rectangle(extent={{-90,-8}, {40,-14}}, lineColor={0, 0, 0}, fillPattern=FillPattern.HorizontalCylinder, fillColor={0, 127, 255}), Rectangle(extent={{40,-22}, {86, 30}}, lineColor={95, 95, 95}, fillColor={95, 95, 95}, fillPattern=FillPattern.Solid), Rectangle(extent={{46, 24}, {80,-18}}, pattern=LinePattern.None, lineColor={117, 148, 176}, fillColor={170, 213, 255}, fillPattern=FillPattern.Sphere), Rectangle(extent={{80, 14}, {86,-8}}, lineColor={95, 95, 95}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Rectangle(extent={{82, 10}, {84,-6}}, lineColor={95, 95, 95}, fillColor={170, 213, 255}, fillPattern=FillPattern.Solid), Text(extent={{-92,-42}, {-6,-98}}, lineColor={0, 0, 127}, textString="[%nPorts]", horizontalAlignment=TextAlignment.Left), Ellipse(extent={{-76, 36}, {-40, 0}}, lineColor={0, 0, 255}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Polygon(points={{-66, 34}, {-40, 18}, {-66, 2}, {-66, 34}}, lineColor={0, 0, 255}, fillColor={0, 0, 255}, fillPattern=FillPattern.Solid), Bitmap(extent={{30,-102}, {92,-40}}, fileName="modelica://Buildings/Resources/Images/Fluid/FMI/FMI_icon.png"), Bitmap(extent={{-96, 52}, {-24, 108}}, fileName="modelica://Buildings/Resources/Images/Fluid/FMI/modelica_icon.png")}), Documentation(info="<html>
<p>
