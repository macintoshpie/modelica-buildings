...
protected
  ...
  RealVectorExpression XiSup[nPorts](
    ...
    final y={inStream(ports[i].Xi_outflow) for i in 1 : nPorts}) if Medium.nXi > 0
  ...
  RealVectorExpression CSup[nPorts](
    ...
    final y={inStream(ports[i].C_outflow) for i in 1 : nPorts}) if Medium.nC > 0
...
equation
  connect(con.outlet, fluPor)
    annotation(
      Line(
        points={{81, 70}, {90, 70}, {90, 0}, {106, 0}, {106, 0}, {110, 0}, {110, 0}},
        color={0, 0, 255}));
  connect(senMasFlo.m_flow, con.m_flow)
    annotation(
      Line(
        points={{-70, 11}, {-70, 11}, {-70, 78}, {58, 78}},
        color={0, 0, 127}));
  connect(hSup.y, con.h)
    annotation(
      Line(
        points={{-19, 60}, {20, 60}, {20, 74}, {58, 74}},
        color={0, 0, 127}));
  for i in 1 : nPorts loop
    connect(XiSup[i].y, con[i].Xi)
      annotation(
        Line(
          points={{-19, 40}, {4, 40}, {28, 40}, {28, 66}, {58, 66}},
          color={0, 0, 127}));
    connect(CSup[i].y, con[i].C)
      annotation(
        Line(
          points={{-19, 20}, {32, 20}, {32, 62}, {58, 62}},
          color={0, 0, 127}));
  end for;
  connect(senMasFlo.port_b, bou.ports)
    annotation(
      Line(
        points={{-60, 0}, {-40, 0}, {-20, 0}},
        color={0, 127, 255}));
  ...
  connect(con[1].TAirZon, bou.T_in)
    annotation(
      Line(
        points={{64, 58}, {64, 4}, {2, 4}},
        color={0, 0, 127}));
  connect(con[1].X_wZon, x_w_toX.X_w)
    annotation(
      Line(
        points={{70, 58}, {70, 4}, {70,-30}, {42,-30}},
        color={0, 0, 127}));
  connect(x_w_toX.X, bou.X_in)
    annotation(
      Line(
        points={{18,-30}, {12,-30}, {12,-4}, {2,-4}},
        color={0, 0, 127}));
  connect(con[1].CZon, bou.C_in)
    annotation(
      Line(
        points={{76, 58}, {76, 58}, {76, 10}, {76,-8}, {2,-8}},
        color={0, 0, 127}));
  connect(con.TAirZon, TAirZon)
    annotation(
      Line(
        points={{64, 58}, {64,-80}, {-60,-80}, {-60,-120}},
        color={0, 0, 127}));
  connect(con.X_wZon, X_wZon)
    annotation(
      Line(
        points={{70, 58}, {70, 58}, {70, 4}, {70,-86}, {0,-86}, {0,-120}},
        color={0, 0, 127}));
  connect(con.CZon, CZon)
    annotation(
      Line(
        points={{76, 58}, {76, 58}, {76,-42}, {76,-92}, {60,-92}, {60,-120}},
        color={0, 0, 127}));
  annotation(
    defaultComponentName="theZonAda",
    Icon(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-100,-100}, {100, 100}}),
      graphics={Rectangle(
        ...
        lineColor={0, 0, 127},
        textString="[%nPorts]",
        horizontalAlignment=TextAlignment.Left), Ellipse(
        extent={{-76, 36}, {-40, 0}},
        lineColor={0, 0, 255},
        fillColor={255, 255, 255},
        ...
        extent={{-96, 52}, {-24, 108}},
        fileName="modelica://Buildings/Resources/Images/Fluid/FMI/modelica_icon.png")}),
    Documentation(
      info="<html>
<p>
