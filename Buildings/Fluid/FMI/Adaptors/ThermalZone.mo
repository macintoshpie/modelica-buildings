...
protected
  ...
  RealVectorExpression XiSup(
    final n=Medium.nXi,
    final y=inStream(ports[1].Xi_outflow)) if Medium.nXi > 0
  ...
  RealVectorExpression CSup(
    final n=Medium.nC,
    final y=inStream(ports[1].C_outflow)) if Medium.nC > 0
  ...
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor senTemAir
    ...
    annotation(
      Placement(
        transformation(
          extent={{72,-90}, {52,-70}})));
  BaseClasses.X_w_toX x_w_toX[nPorts](
    redeclare final package Medium=Medium) if Medium.nXi > 0
  ...
  block RealVectorExpression
    ...
    Modelica.Blocks.Interfaces.RealOutput[n] y
      ...
      annotation(
        Dialog(
          group="Time varying output signal"),
        Placement(
          transformation(
    ...
    annotation(
      Icon(
        ...
        graphics={Rectangle(
          ...
          borderPattern=BorderPattern.Raised), Rectangle(
          extent={{-92, 30}, {100,-44}},
          lineColor={0, 0, 0},
          lineThickness=5.0,
...
First implementation.
  ...
  block x_i_toX_w
    ...
    Modelica.Blocks.Interfaces.RealInput Xi[Medium.nXi](
      each final unit="kg/kg") if Medium.nXi > 0
...
equation
  for i in 1 : nPorts loop
    connect(bou[i].ports[1], ports[i])
      annotation(
        Line(
          points={{22, 48}, {22, 48}, {80, 48}, {80, 0}, {100, 0}},
          color={0, 127, 255}));
  end for;
  connect(con.inlet, fluPor)
    annotation(
      Line(
        points={{-81, 60}, {-90, 60}, {-90, 60}, {-90, 0}, {-110, 0}},
        color={0, 0, 255}));
  connect(con.X_w, x_w_toX.X_w)
    annotation(
      Line(
        points={{-58, 56}, {-58, 56}, {-42, 56}},
        color={0, 0, 127}));
  connect(x_w_toX.X, bou.X_in)
    annotation(
      Line(
        points={{-18, 56}, {-18, 56}, {-14, 56}, {-14, 44}, {0, 44}},
        color={0, 0, 127}));
  connect(con.C, bou.C_in)
    annotation(
      Line(
        points={{-58, 52}, {-50, 52}, {-50, 40}, {0, 40}, {0, 40}},
        color={0, 0, 127}));
  connect(heaPorAir, senTemAir.port)
    annotation(
      Line(
        points={{100,-80}, {80,-80}, {72,-80}},
        color={191, 0, 0}));
  ...
  connect(con.T, bou.T_in)
    annotation(
      Line(
        points={{-58, 64}, {-58, 64}, {-44, 64}, {-44, 74}, {-12, 74}, {-12, 52}, {0, 52}},
        color={0, 0, 127}));
  connect(con[1 : nPorts].m_flow, multiSum.u[1 : nPorts])
    annotation(
      Line(
        points={{-58, 68}, {-54, 68}, {-50, 68}, {-50, 78}, {4, 78}},
        color={0, 0, 127}));
  connect(multiSum.y, assEqu.u2)
    annotation(
      Line(
        points={{17.02, 78}, {26, 78}, {26, 60}, {30, 60}, {68, 60}},
        color={0, 0, 127}));
  connect(const.y, assEqu.u1)
    annotation(
      Line(
        points={{51, 78}, {60, 78}, {60, 72}, {68, 72}},
        color={0, 0, 127}));
  for i in 1 : nPorts loop
    connect(senTemAir.T, con[i].TAirZon)
      annotation(
        Line(
          points={{52,-80}, {40,-80}, {40, 20}, {-76, 20}, {-76, 48}},
          color={0, 0, 127}));
    if Medium.nXi > 0 then
      connect(x_i_toX.X_w, con[i].X_wZon)
        annotation(
          Line(
            points={{-42,-20}, {-42,-20}, {-70,-20}, {-70, 48}},
            color={0, 0, 127}));
    end if;
    connect(CSup.y, con[i].CZon)
      annotation(
        Line(
          points={{-1,-60}, {-64,-60}, {-64, 48}},
          color={0, 0, 127}));
  end for;
  connect(bou.m_flow_in, con.m_flow)
    annotation(
      Line(
        points={{0, 56}, {-8, 56}, {-8, 78}, {-50, 78}, {-50, 68}, {-58, 68}},
        color={0, 0, 127}));
  annotation(
    defaultComponentName="hvacAda",
    Icon(
      coordinateSystem(
        preserveAspectRatio=false),
      graphics={Rectangle(
        ...
        extent={{-96,-98}, {-30,-50}},
        fileName="modelica://Buildings/Resources/Images/Fluid/FMI/FMI_icon.png"), Bitmap(
        extent={{0, 50}, {100, 98}},
        fileName="modelica://Buildings/Resources/Images/Fluid/FMI/modelica_icon.png"), Rectangle(
        extent={{38,-22}, {84, 30}},
        lineColor={95, 95, 95},
        fillColor={95, 95, 95},
        fillPattern=FillPattern.Solid), Rectangle(
