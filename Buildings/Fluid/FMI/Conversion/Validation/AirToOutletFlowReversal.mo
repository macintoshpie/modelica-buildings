...
model AirToOutletFlowReversal
  ...
  extends Buildings.Fluid.FMI.Conversion.Validation.AirToOutlet(allowFlowReversal=true);
  BoundaryCondition bouAirNoC(redeclare package Medium=Buildings.Media.Air(X_default={0.015, 0.985}))
    "Boundary condition"
    annotation(Placement(transformation(extent={{40, 50}, {60, 70}})));
  BoundaryCondition bouAirWithC(redeclare package Medium=Buildings.Media.Air(X_default={0.015, 0.985}, extraPropertiesNames={"CO2"}))
    "Boundary condition"
    annotation(Placement(transformation(extent={{40, 10}, {60, 30}})));
  BoundaryCondition bouDryAirNoC(redeclare package Medium=Modelica.Media.Air.SimpleAir)
    "Boundary condition"
    annotation(Placement(transformation(extent={{40,-40}, {60,-20}})));
  BoundaryCondition bouDryAirWithC(redeclare package Medium=Modelica.Media.Air.SimpleAir(extraPropertiesNames={"CO2"}))
    "Boundary condition"
...
protected
  model BoundaryCondition
    ...
    Adaptors.Inlet bouInlAirNoC(redeclare package Medium=Medium, allowFlowReversal=true, use_p_in=false)
      "Boundary condition for air inlet"
      annotation(Placement(transformation(extent={{-60,-10}, {-40, 10}})));
    Sources.Boundary_pT bou(redeclare package Medium=Medium, nPorts=1, p=Medium.p_default + 1000, T=298.15, C=fill(0.7, Medium.nC))
      "Boundary condition"
      annotation(Placement(transformation(extent={{60,-10}, {40, 10}})));
    Interfaces.Inlet inlet(use_p_in=use_p_in, redeclare package Medium=Medium, allowFlowReversal=true)
      "Fluid inlet"
      annotation(Placement(transformation(extent={{-120,-10}, {-100, 10}})));
    FixedResistances.PressureDrop res(redeclare package Medium=Medium, m_flow_nominal=0.1, dp_nominal=1000, linearized=true)
      "Flow resistance"
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FMI/Conversion/Validation/AirToOutletFlowReversal.mos"
