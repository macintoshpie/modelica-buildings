...
block HVACZone
  ...
  Movers.FlowControlled_m_flow fan(redeclare package Medium=MediumA, m_flow_nominal=mA_flow_nominal, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, allowFlowReversal=allowFlowReversal, nominalValuesDefineDefaultPressureCurve=true)
    "Supply air fan"
  ...
  Buildings.Fluid.Sources.Outside out(redeclare package Medium=MediumA, nPorts=2)
    "Outside air boundary condition"
    annotation(Placement(transformation(extent={{-112, 84}, {-92, 104}})));
  Sources.MassFlowSource_T souWat(redeclare package Medium=MediumW, nPorts=1, use_m_flow_in=true, T=TWSup_nominal)
  ...
  FixedResistances.PressureDrop resSup1(redeclare package Medium=MediumA, linearized=true, dp_nominal=10, m_flow_nominal=mA_flow_nominal)
    "Fixed resistance for supply air inlet"
    annotation(Placement(transformation(extent={{86, 130}, {106, 150}})));
  FixedResistances.PressureDrop resRet1(redeclare package Medium=MediumA, dp_nominal=200, linearized=true, m_flow_nominal=mA_flow_nominal)
    "Fixed resistance for return air duct"
    annotation(Placement(transformation(extent={{70, 60}, {50, 80}})));
equation
  connect(zero.y, QGaiRad_flow)
  ...
  connect(zero.y, QGaiLat_flow)
    annotation(Line(points={{121,-90}, {140,-90}, {140,-140}, {180,-140}}, color={0, 0, 127}));
  connect(out.ports[1], hex.port_a1)
    annotation(Line(points={{-92, 96}, {-82, 96}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(out.ports[2], hex.port_b2)
    annotation(Line(points={{-92, 92}, {-88, 92}, {-88, 84}, {-82, 84}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(souWat.ports[1], cooCoi.port_a1)
    annotation(Line(points={{-10, 16}, {10, 16}, {10, 88}, {-8, 88}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(fan.m_flow_in, mAir_flow.y)
    annotation(Line(points={{60, 112}, {60, 140}, {21, 140}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(hex.port_b1, senTemHXOut.port_a)
    annotation(Line(points={{-62, 96}, {-60, 96}, {-60, 100}, {-54, 100}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(senTemHXOut.port_b, cooCoi.port_a2)
    annotation(Line(points={{-34, 100}, {-28, 100}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(cooCoi.port_b2, senTemSupAir.port_a)
    annotation(Line(points={{-8, 100}, {20, 100}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(senTemSupAir.port_b, fan.port_a)
    annotation(Line(points={{40, 100}, {50, 100}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(TRooSetPoi.y, con.reference)
    annotation(Line(points={{-129, 22}, {-114, 22}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(con.y, mWat_flow.u)
    annotation(Line(points={{-91, 16}, {-74, 16}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(mWat_flow.y, souWat.m_flow_in)
    annotation(Line(points={{-51, 16}, {-42, 16}, {-42, 24}, {-32, 24}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(TOut, weaBus.TDryBul)
    annotation(Line(points={{0,-180}, {0,-180}, {0,-140}, {0, 120}, {-60, 120}, {-60, 140}}, color={0, 0, 127}));
  connect(sinWat.ports[1], cooCoi.port_b1)
    annotation(Line(points={{-52, 50}, {-52, 50}, {-28, 50}, {-28, 88}, {-28, 88}}, color={0, 127, 255}));
  connect(con.u, hvacAda.TAirZon[1])
    annotation(Line(points={{-114, 10}, {-124, 10}, {-124,-10}, {124,-10}, {124, 128}}, color={0, 0, 127}));
  connect(resSup1.port_b, hvacAda.ports[1])
    annotation(Line(points={{106, 140}, {113, 140}, {120, 140}}, color={0, 127, 255}));
  connect(resRet1.port_a, hvacAda.ports[2])
    annotation(Line(points={{70, 70}, {110, 70}, {110, 140}, {120, 140}}, color={0, 127, 255}));
  connect(fan.port_b, resSup1.port_a)
    annotation(Line(points={{70, 100}, {74, 100}, {80, 100}, {80, 140}, {86, 140}}, color={0, 127, 255}));
  connect(resRet1.port_b, hex.port_a2)
    annotation(Line(points={{50, 70}, {50, 72}, {-60, 72}, {-60, 84}, {-62, 84}}, color={0, 127, 255}));
  connect(out.weaBus, weaBus)
    annotation(Line(points={{-112, 94.2}, {-116, 94.2}, {-116, 94}, {-120, 94}, {-120, 120}, {-60, 120}, {-60, 140}}, color={255, 204, 51}, thickness=0.5));
  annotation(Icon(coordinateSystem(preserveAspectRatio=false, extent={{-160,-160}, {160, 160}}), graphics={Text(extent={{-24,-132}, {26,-152}}, lineColor={0, 0, 127}, textString="TOut")}), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-160,-160}, {160, 160}})), Documentation(info="<html>
<p>
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FMI/ExportContainers/Examples/FMUs/HVACZone.mos"
    "Export FMU"));
end HVACZone;
