...
block HVACZones
  ...
  /////////////////////////////////////////////////////////
  // HVAC models
  Modelica.Blocks.Sources.Constant zer[nZon](
    each k=0)
    "Zero output signal"
    annotation(
      Placement(
  ...
  BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-152, 130}, {-132, 150}})));
  BoundaryConditions.WeatherData.Bus weaBus
    "Weather data bus"
    annotation(
      Placement(
        transformation(
          extent={{-70, 130}, {-50, 150}}),
        iconTransformation(
          extent={{-142, 94}, {-122, 114}})));
...
equation
  connect(zer.y, QGaiRad_flow)
    annotation(
      Line(
        points={{121,-90}, {140,-90}, {140,-40}, {180,-40}},
        color={0, 0, 127}));
  connect(zer.y, QGaiSenCon_flow)
    annotation(
      Line(
        points={{121,-90}, {180,-90}},
        color={0, 0, 127}));
  connect(zer.y, QGaiLat_flow)
    annotation(
      Line(
        points={{121,-90}, {140,-90}, {140,-140}, {180,-140}},
        color={0, 0, 127}));
  ...
  connect(TOut, weaBus.TDryBul)
    annotation(
      Line(
        points={{0,-180}, {0,-174}, {0,-140}, {0, 120}, {-60, 120}, {-60, 140}},
        color={0, 0, 127}));
  connect(sinWat.ports[1], cooCoi.port_b1)
    annotation(
      Line(
        points={{-36, 82}, {-36, 88}, {-28, 88}},
        color={0, 127, 255}));
  connect(min.y, con.u)
    annotation(
      Line(
        points={{-101,-70}, {-110,-70}, {-110,-36}, {-100,-36}},
        color={0, 0, 127}));
  connect(fan2.port_b, res.port_a)
    annotation(
      Line(
        points={{60, 0}, {60, 0}, {40, 0}},
        color={0, 127, 255}));
  connect(res.port_b, out.ports[3])
    annotation(
      Line(
        points={{20, 0}, {-96, 0}, {-96, 42}, {-96, 87.3333}, {-100, 87.3333}},
        color={0, 127, 255}));
  connect(fan.port_b, resSup1.port_a)
    annotation(
      Line(
        points={{60, 100}, {66, 100}, {66, 116}, {70, 116}},
        color={0, 127, 255}));
  connect(fan.port_b, resSup2.port_a)
    annotation(
      Line(
        points={{60, 100}, {66, 100}, {66, 86}, {70, 86}},
        color={0, 127, 255}));
  connect(resSup1.port_b, hvacAda[1].ports[1])
    annotation(
      Line(
        points={{90, 116}, {100, 116}, {100, 140}, {120, 140}},
        color={0, 127, 255}));
  connect(resSup2.port_b, hvacAda[2].ports[1])
    annotation(
      Line(
        points={{90, 86}, {102, 86}, {102, 140}, {120, 140}},
        color={0, 127, 255}));
  connect(hvacAda[1].ports[3], fan2.port_a)
    annotation(
      Line(
        points={{120, 140}, {116, 140}, {116, 0}, {80, 0}},
        color={0, 127, 255}));
  ...
  connect(hvacAda[1].TAirZon[1], min.u1)
    annotation(
      Line(
        points={{124, 128}, {124, 128}, {124, 60}, {124,-64}, {-78,-64}},
        color={0, 0, 127}));
  connect(hvacAda[2].TAirZon[1], min.u2)
    annotation(
      Line(
        points={{124, 128}, {124, 128}, {124, 54}, {124,-64}, {-60,-64}, {-60,-64}, {-60,-76}, {-78,-76}},
        color={0, 0, 127}));
  connect(resRet1.port_b, hex.port_a2)
    annotation(
      Line(
        points={{20, 60}, {20, 60}, {-60, 60}, {-60, 84}, {-68, 84}},
        color={0, 127, 255}));
  connect(resRet2.port_b, hex.port_a2)
    annotation(
      Line(
        points={{20, 30}, {-28, 30}, {-60, 30}, {-60, 84}, {-68, 84}},
        color={0, 127, 255}));
  connect(resRet1.port_a, hvacAda[1].ports[2])
    annotation(
      Line(
        points={{40, 60}, {70, 60}, {106, 60}, {106, 140}, {120, 140}},
        color={0, 127, 255}));
  connect(resRet2.port_a, hvacAda[2].ports[2])
    annotation(
      Line(
        points={{40, 30}, {72, 30}, {106, 30}, {106, 140}, {120, 140}},
        color={0, 127, 255}));
...
First implementation.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FMI/ExportContainers/Examples/FMUs/HVACZones.mos"
        "Export FMU"));
end HVACZones;
