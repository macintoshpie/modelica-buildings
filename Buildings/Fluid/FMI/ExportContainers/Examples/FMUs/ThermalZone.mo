...
model ThermalZone
  ...
  extends Buildings.Fluid.FMI.ExportContainers.ThermalZone(redeclare final package Medium=MediumA, nPorts=2);
  ...
  MixingVolumes.MixingVolumeMoistAir vol(redeclare package Medium=MediumA, m_flow_nominal=mA_flow_nominal, V=V, mSenFac=3, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, nPorts=2)
...
equation
  connect(weaDat.weaBus, weaBus)
    annotation(Line(points={{80, 160}, {-10, 160}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None), Text(textString="%second", index=1, extent={{6, 3}, {6, 3}}));
  ...
  connect(TAirOut.T, weaBus.TDryBul)
    annotation(Line(points={{-32, 120}, {-40, 120}, {-40, 160}, {-10, 160}}, color={0, 0, 127}), Text(textString="%second", index=1, extent={{6, 3}, {6, 3}}));
  connect(TAirOut.port, theCon.port_a)
    annotation(Line(points={{-10, 120}, {-10, 120}, {30, 120}}, color={191, 0, 0}));
  connect(theZonAda.heaPorAir, vol.heatPort)
    annotation(Line(points={{-120, 152}, {60, 152}, {60, 40}, {80, 40}}, color={191, 0, 0}));
  connect(TOut, weaBus.TDryBul)
    annotation(Line(points={{0,-160}, {0,-160}, {0,-120}, {0, 160}, {-10, 160}}, color={0, 0, 127}));
  connect(vol.mWat_flow, mWat_flow.y)
    annotation(Line(points={{78, 48}, {78, 48}, {54, 48}, {54, 80}, {31, 80}}, color={0, 0, 127}));
  connect(theZonAda.ports, senMasFlo.port_a)
    annotation(Line(points={{-120, 160}, {-100, 160}, {-100, 120}, {-88, 120}}, color={0, 127, 255}));
  connect(senMasFlo.port_b, vol.ports[1 : 2])
    annotation(Line(points={{-68, 120}, {-60, 120}, {-60, 20}, {92, 20}, {92, 30}}, color={0, 127, 255}));
  connect(temSen.port, vol.heatPort)
    annotation(Line(points={{-60, 0}, {60, 0}, {60, 40}, {80, 40}}, color={191, 0, 0}));
  connect(temSen.T, TRad)
    annotation(Line(points={{-80, 0}, {-170, 0}}, color={0, 0, 127}));
  annotation(Icon(coordinateSystem(preserveAspectRatio=false, extent={{-160,-140}, {160, 180}}), graphics={Text(extent={{-22,-112}, {28,-132}}, lineColor={0, 0, 127}, textString="TOut")}), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-160,-140}, {160, 180}}), graphics={Rectangle(extent={{4, 134}, {140, 4}}, fillColor={215, 215, 215}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Text(extent={{76, 134}, {114, 110}}, pattern=LinePattern.None, lineColor={0, 0, 127}, horizontalAlignment=TextAlignment.Left, textString="Simplified model of
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FMI/ExportContainers/Examples/FMUs/ThermalZone.mos"
    "Export FMU"));
end ThermalZone;
