...
model ThermalZones
  ...
  BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
    ...
    annotation(
      Placement(
        transformation(
          extent={{150, 130}, {130, 150}})));
  BoundaryConditions.WeatherData.Bus weaBus
    "Weather data bus"
    annotation(
      Placement(
        transformation(
          extent={{110, 130}, {130, 150}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput TRad1(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-160, 10}, {-180, 30}})));
  Modelica.Blocks.Interfaces.RealOutput TRad2(
    final unit="K")
    "Radiative temperature"
    annotation(
      Placement(
        transformation(
          extent={{-160,-30}, {-180,-10}})));
  Buildings.HeatTransfer.Sources.PrescribedTemperature TAirOut
    "Outside air temperature"
    annotation(
      Placement(
        transformation(
          extent={{-30, 110}, {-10, 130}})));
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor theCon1(
    G=10000/30)
  ...
  Modelica.Thermal.HeatTransfer.Sources.FixedHeatFlow preHea1(
    Q_flow=QRooInt_flow)
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor theCon2(
    ...
    annotation(
      Placement(
        transformation(
          extent={{30,-26}, {50,-6}})));
  Modelica.Thermal.HeatTransfer.Sources.FixedHeatFlow preHea2(
    Q_flow=QRooInt_flow)
    "Prescribed heat flow for internal gains"
    annotation(
      Placement(
...
equation
  ...
  connect(theCon1.port_b, vol1.heatPort)
    annotation(
      Line(
        points={{50, 120}, {50, 120}, {60, 120}, {60, 40}, {80, 40}},
        color={191, 0, 0}));
  connect(preHea1.port, vol1.heatPort)
    annotation(
      Line(
        points={{80, 90}, {60, 90}, {60, 40}, {80, 40}},
        color={191, 0, 0}));
  connect(TAirOut.T, weaBus.TDryBul)
    annotation(
      Line(
        points={{-32, 120}, {-40, 120}, {-40, 140}, {120, 140}},
        color={0, 0, 127}),
      Text(
        textString="%second",
        index=1,
  ...
  connect(TAirOut.port, theCon1.port_a)
    annotation(
      Line(
        points={{-10, 120}, {-10, 120}, {30, 120}},
        color={191, 0, 0}));
  connect(TOut, weaBus.TDryBul)
    annotation(
      Line(
        points={{0,-160}, {0,-160}, {0,-134}, {-40,-134}, {-40, 140}, {120, 140}},
        color={0, 0, 127}));
  connect(theCon2.port_b, vol2.heatPort)
    annotation(
      Line(
        points={{50,-16}, {50,-16}, {60,-16}, {60,-96}, {80,-96}},
        color={191, 0, 0}));
  connect(preHea2.port, vol2.heatPort)
    annotation(
      Line(
        points={{80,-46}, {60,-46}, {60,-96}, {80,-96}},
        color={191, 0, 0}));
  connect(TAirOut.port, theCon2.port_a)
    annotation(
      Line(
        points={{-10, 120}, {-10, 120}, {0, 120}, {0,-16}, {30,-16}},
        color={191, 0, 0}));
  connect(vol1.heatPort, theZonAda[1].heaPorAir)
    annotation(
      Line(
        points={{80, 40}, {60, 40}, {60, 152}, {-120, 152}},
        color={191, 0, 0}));
  connect(vol2.heatPort, theZonAda[2].heaPorAir)
    annotation(
      Line(
        points={{80,-96}, {60,-96}, {60, 4}, {10, 4}, {10, 152}, {-120, 152}},
        color={191, 0, 0}));
  connect(theZonAda[2].ports, senMasFlo1.port_a)
    annotation(
      Line(
        points={{-120, 160}, {-112, 160}, {-112, 60}, {-90, 60}},
        color={0, 127, 255}));
  connect(senMasFlo.port_a, theZonAda[1].ports)
    annotation(
      Line(
        points={{-90, 100}, {-100, 100}, {-100, 160}, {-120, 160}},
        color={0, 127, 255}));
  connect(senMasFlo1.port_b, vol2.ports[1 : 3])
    annotation(
      Line(
        points={{-70, 60}, {-60, 60}, {-60,-120}, {92.6667,-120}, {92.6667,-106}},
        color={0, 127, 255}));
  connect(senMasFlo.port_b, vol1.ports[1 : 3])
    annotation(
      Line(
        points={{-70, 100}, {-50, 100}, {-20, 100}, {-20, 22}, {92.6667, 22}, {92.6667, 30}},
        color={0, 127, 255}));
  ...
  connect(temSen2.port, vol2.heatPort)
    annotation(
      Line(
        points={{-80,-20}, {-58,-20}, {0,-20}, {0,-96}, {80,-96}},
        color={191, 0, 0}));
  connect(temSen1.T, TRad1)
    annotation(
      Line(
        points={{-100, 20}, {-170, 20}, {-170, 20}},
        color={0, 0, 127}));
...
First implementation.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FMI/ExportContainers/Examples/FMUs/ThermalZones.mos"
        "Export FMU"));
end ThermalZones;
