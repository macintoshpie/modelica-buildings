...
block ReplaceableTwoPort
  ...
  replaceable Buildings.Fluid.Interfaces.PartialTwoPort com constrainedby Buildings.Fluid.Interfaces.PartialTwoPort(redeclare final package Medium=Medium, final allowFlowReversal=allowFlowReversal)
    "Component that holds the actual model"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}})));
  Modelica.Blocks.Sources.RealExpression dpCom(y=com.port_a.p-com.port_b.p) if use_p_in
...
equation
  connect(pOut.u1, bouIn.p)
    annotation(Line(points={{-8,-60}, {-70,-60}, {-70,-11}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(com.port_a, bouIn.port_b)
    annotation(Line(points={{-10, 0}, {-60, 0}}, color={0, 127, 255}, smooth=Smooth.None));
  ...
  connect(dpCom.y, pOut.u2)
    annotation(Line(points={{-19,-80}, {0,-80}, {0,-68}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(pOut.y, bouOut.p)
    annotation(Line(points={{9,-60}, {70,-60}, {70,-12}}, color={0, 0, 127}, smooth=Smooth.None));
