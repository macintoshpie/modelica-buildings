...
protected
  Sources.MassFlowSource_T sou(
    ...
    annotation(
      Placement(
        transformation(
          extent={{20, 130}, {40, 150}})));
equation
  connect(bou.ports[1], ret.port_b)
    annotation(
      Line(
        points={{40, 100}, {40, 100}, {60, 100}},
        color={0, 127, 255}));
  ...
  connect(sup.port_b, hvacAda.ports[1])
    annotation(
      Line(
        points={{80, 140}, {100, 140}, {120, 140}},
        color={0, 127, 255}));
  connect(ret.port_a, hvacAda.ports[2])
    annotation(
      Line(
        points={{80, 100}, {100, 100}, {100, 140}, {120, 140}},
        color={0, 127, 255}));
  connect(sou.ports[1], sup.port_a)
    annotation(
      Line(
        points={{40, 140}, {50, 140}, {60, 140}},
        color={0, 127, 255}));
  annotation(
    Documentation(
      info="<html>
...
First implementation.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FMI/ExportContainers/Validation/FMUs/HVACThermalZoneAir1.mos"
        "Export FMU"));
