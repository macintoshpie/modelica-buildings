  ...
  MixingVolumes.MixingVolume vol(
    ...
    annotation(
      Placement(
        transformation(
          extent={{40, 52}, {60, 72}})));
  Modelica.Blocks.Sources.Constant TOutSou(
    k=303.15)
    "Outdoor temperature"
    annotation(
      Placement(
        transformation(
          extent={{-100, 102}, {-80, 122}})));
protected
...
public
  Sensors.MassFlowRate senMasFlo[nPorts](
    ...
    annotation(
      Placement(
        transformation(
          extent={{-106, 150}, {-86, 170}})));
equation
  connect(theCon.port_b, vol.heatPort)
    annotation(
      Line(
        points={{0, 112}, {0, 112}, {20, 112}, {20, 62}, {40, 62}},
        color={191, 0, 0}));
  connect(preHea.port, vol.heatPort)
    annotation(
      Line(
        points={{40, 112}, {20, 112}, {20, 62}, {40, 62}},
        color={191, 0, 0}));
  connect(TOut1.port, theCon.port_a)
    annotation(
      Line(
        points={{-40, 112}, {-40, 112}, {-20, 112}},
        color={191, 0, 0}));
  connect(vol.heatPort, theZonAda.heaPorAir)
    annotation(
      Line(
        points={{40, 62}, {40, 62}, {20, 62}, {20, 140}, {-114, 140}, {-120, 140}, {-120, 152}},
        color={191, 0, 0}));
  connect(TOutSou.y, TOut1.T)
    annotation(
      Line(
        points={{-79, 112}, {-62, 112}},
        color={0, 0, 127}));
  connect(TOut, TOutSou.y)
    annotation(
      Line(
        points={{0, 200}, {0, 162}, {-70, 162}, {-70, 112}, {-79, 112}},
        color={0, 0, 127}));
  connect(res2.port_b, vol.ports[1])
    annotation(
      Line(
        points={{100, 140}, {110, 140}, {110, 38}, {48, 38}, {48, 52}},
        color={0, 127, 255}));
  connect(res1.port_b, vol.ports[2])
    annotation(
      Line(
        points={{100, 160}, {112, 160}, {114, 160}, {114, 44}, {114, 42}, {52, 42}, {52, 52}},
        color={0, 127, 255}));
  connect(senMasFlo.port_a, theZonAda.ports)
    annotation(
      Line(
        points={{-106, 160}, {-114, 160}, {-114, 160}, {-120, 160}},
        color={0, 127, 255}));
  connect(senMasFlo[1].port_b, res1.port_a)
    annotation(
      Line(
        points={{-86, 160}, {-20, 160}, {80, 160}},
        color={0, 127, 255}));
  connect(senMasFlo[2].port_b, res2.port_a)
    annotation(
      Line(
        points={{-86, 160}, {-86, 160}, {30, 160}, {30, 140}, {80, 140}},
        color={0, 127, 255}));
  annotation(
    Icon(
      coordinateSystem(
...
First implementation.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FMI/ExportContainers/Validation/FMUs/ThermalZoneAir1.mos"
        "Export FMU"));
end ThermalZoneAir1;
