...
model HeaterFanPressureDriven
  ...
  ExportContainers.Examples.FMUs.Fan floMac(m_flow_nominal=m_flow_nominal, dp_nominal=dp_nominal, use_p_in=use_p_in)
    "Flow machine with pressure raise as an input"
    annotation(Placement(transformation(extent={{20,-10}, {40, 10}})));
  ExportContainers.Examples.FMUs.HeaterCooler_u hea(m_flow_nominal=m_flow_nominal, dp_nominal=dp_nominal, Q_flow_nominal=Q_flow_nominal, use_p_in=use_p_in)
    "Heater"
    annotation(Placement(transformation(extent={{-20,-10}, {0, 10}})));
  Modelica.Blocks.Sources.Constant dp(k=dp_nominal)
    "Pressure raise of fan"
  ...
  Modelica.Blocks.Sources.Constant CBac[Medium.nC](each k=0.01) if Medium.nC > 0
    "Trace substances for back flow"
    annotation(Placement(transformation(extent={{120,-60}, {100,-40}})));
  Modelica.Blocks.Sources.Constant C[Medium.nC](each k=0.01) if Medium.nC > 0
    "Trace substances for forward flow"
...
equation
  connect(uHea.y, hea.u)
    annotation(Line(points={{-39, 50}, {-30, 50}, {-30, 6}, {-22, 6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(dp.y, floMac.dp_in)
    annotation(Line(points={{1, 50}, {10, 50}, {10, 6}, {18, 6}, {18, 6.8}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(sou.outlet, hea.inlet)
    annotation(Line(points={{-39, 0}, {-21, 0}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(hea.outlet, floMac.inlet)
    annotation(Line(points={{1, 0}, {19, 0}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(sin.inlet, floMac.outlet)
    annotation(Line(points={{59, 0}, {41, 0}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(sin.T_in, TBac.y)
    annotation(Line(points={{82, 8}, {92, 8}, {92, 50}, {99, 50}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(CBac.y, sin.C_in)
    annotation(Line(points={{99,-50}, {92,-50}, {92,-2}, {82,-2}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(sou.p_in, pIn.y)
    annotation(Line(points={{-62, 4.8}, {-66, 4.8}, {-66, 6}, {-72, 6}, {-72, 40}, {-79, 40}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TIn.y, sou.T_in)
    annotation(Line(points={{-79, 10}, {-74, 10}, {-74, 0}, {-62, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(C.y, sou.C_in)
    annotation(Line(points={{-79,-60}, {-68,-60}, {-68,-10}, {-62,-10}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(pIn.y, invBloCon.u1)
    annotation(Line(points={{-79, 40}, {-72, 40}, {-72,-72}, {-22,-72}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(sin.p, invBloCon.u2)
    annotation(Line(points={{82,-8}, {86,-8}, {86,-92}, {0,-92}, {0,-72}, {-16,-72}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(invBloCon.y1, sou.m_flow_in)
    annotation(Line(points={{21,-72}, {50,-72}, {50, 70}, {-68, 70}, {-68, 10}, {-62, 10}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(X_w_in.y, sou.X_w_in)
    annotation(Line(points={{-79,-20}, {-74,-20}, {-74,-5}, {-62,-5}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(X_w_bac.y, sin.X_w_in)
    annotation(Line(points={{99, 0}, {92, 0}, {92, 3}, {82, 3}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,-100}, {140, 100}}), graphics), Documentation(info="<html>
<p>
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FMI/Validation/HeaterFanPressureDriven.mos"
    "Simulate and plot"), experiment(Tolerance=1e-6, StopTime=1.0));
