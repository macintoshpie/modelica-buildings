...
model HeaterFan_noReverseFlow
  ...
  ExportContainers.Examples.FMUs.Fan floMac(redeclare final package Medium=Medium, m_flow_nominal=m_flow_nominal, dp_nominal=dp_nominal, final allowFlowReversal=allowFlowReversal, use_p_in=use_p_in)
    "Flow machine with pressure raise as an input"
    annotation(Placement(transformation(extent={{60,-10}, {80, 10}})));
  ExportContainers.Examples.FMUs.HeaterCooler_u hea(redeclare final package Medium=Medium, m_flow_nominal=m_flow_nominal, dp_nominal=dp_nominal, Q_flow_nominal=Q_flow_nominal, final allowFlowReversal=allowFlowReversal, use_p_in=use_p_in)
    "Heater"
  ...
  Source_T sou(redeclare package Medium=Medium, final allowFlowReversal=allowFlowReversal, use_p_in=use_p_in)
...
equation
  connect(uHea.y, hea.u)
    annotation(Line(points={{1, 50}, {10, 50}, {10, 6}, {18, 6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(dp.y, floMac.dp_in)
    annotation(Line(points={{43, 50}, {50, 50}, {50, 6}, {58, 6}, {58, 6.8}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(hea.outlet, floMac.inlet)
    annotation(Line(points={{41, 0}, {59, 0}}, color={0, 0, 255}, smooth=Smooth.None));
  connect(m_flow.y, sou.m_flow_in)
    annotation(Line(points={{-39, 70}, {-28, 70}, {-28, 10}, {-22, 10}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(sou.p_in, pIn.y)
    annotation(Line(points={{-22, 4.8}, {-26, 4.8}, {-26, 6}, {-32, 6}, {-32, 40}, {-39, 40}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TIn.y, sou.T_in)
    annotation(Line(points={{-39, 10}, {-34, 10}, {-34, 0}, {-22, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(C.y, sou.C_in)
    annotation(Line(points={{-39,-60}, {-28,-60}, {-28,-10}, {-22,-10}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(X_w_in.y, sou.X_w_in)
    annotation(Line(points={{-39,-20}, {-30,-20}, {-30,-5}, {-22,-5}}, color={0, 0, 127}, smooth=Smooth.None));
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FMI/Validation/HeaterFan_noReverseFlow.mos"
