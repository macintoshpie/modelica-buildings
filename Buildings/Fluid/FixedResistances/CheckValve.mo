...
model CheckValve
  ...
  extends Buildings.Fluid.BaseClasses.PartialResistance(dp(nominal=2000), final dp_nominal=dpValve_nominal + dpFixed_nominal, final m_flow_turbulent=deltaM*abs(m_flow_nominal), final from_dp=true, final linearized=false, allowFlowReversal=true);
  ...
  parameter Real kFixed(unit="", min=0)=if dpFixed_nominal > Modelica.Constants.eps then m_flow_nominal/sqrt(dpFixed_nominal) else 0
...
initial equation
  assert(dpFixed_nominal >-Modelica.Constants.eps, "In " + getInstanceName() + ": We require dpFixed_nominal >= 0. 
    Received dpFixed_nominal = " + String(dpFixed_nominal) + " Pa.");
  assert(l >-Modelica.Constants.eps, "In " + getInstanceName() + ": We require l >= 0. Received l = " + String(l));
equation
  ...
  annotation(Icon(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100}, {100, 100}}), graphics={Polygon(points={{100,-42}, {-100,-42}, {-100, 40}, {100, 40}, {100,-42}}, lineColor={255, 255, 255}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Line(points={{-100, 0}, {-70, 0}}, color={0, 128, 255}, lineThickness=0.5), Line(points={{0, 70}, {-70, 0}}, color={0, 128, 255}, lineThickness=0.5), Line(points={{0,-70}, {-70, 0}}, color={0, 128, 255}, lineThickness=0.5), Ellipse(extent={{-40,-55}, {70, 55}}, lineColor={0, 128, 255}, fillColor={255, 255, 255}, lineThickness=0.5, fillPattern=FillPattern.Solid), Line(points={{70, 0}, {100, 0}}, color={0, 128, 255}, lineThickness=0.5)}), defaultComponentName="cheVal", Documentation(info="<html>
<p>
