...
model HydraulicDiameter
  ...
  package Medium=Buildings.Media.Water
    "Medium model";
  Modelica.Blocks.Sources.Ramp P(duration=1, height=4000, offset=300000-2000)
  ...
  Buildings.Fluid.Sources.Boundary_pT sin(redeclare package Medium=Medium, T=273.15 + 10, nPorts=2, p(displayUnit="Pa")=300000)
    "Pressure boundary condition"
    annotation(Placement(transformation(extent={{50,-10}, {30, 10}})));
  Buildings.Fluid.FixedResistances.HydraulicDiameter res(redeclare package Medium=Medium, m_flow_nominal=0.2, v_nominal=1, from_dp=true, length=1)
    "Fixed resistance with specified hydraulic diameter"
    annotation(Placement(transformation(extent={{-10,-8}, {10, 12}})));
  Buildings.Fluid.FixedResistances.HydraulicDiameter resLarPip(redeclare package Medium=Medium, from_dp=true, m_flow_nominal=10, length=100)
...
equation
  ...
  connect(resLarPip.port_a, sou.ports[2])
    annotation(Line(points={{-10,-40}, {-16,-40}, {-16,-2}, {-30,-2}}, color={0, 127, 255}));
  connect(resLarPip.port_b, sin.ports[2])
    annotation(Line(points={{10,-40}, {20,-40}, {20,-2}, {30,-2}}, color={0, 127, 255}));
  annotation(experiment(Tolerance=1e-6, StopTime=1.0), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FixedResistances/Examples/HydraulicDiameter.mos"
