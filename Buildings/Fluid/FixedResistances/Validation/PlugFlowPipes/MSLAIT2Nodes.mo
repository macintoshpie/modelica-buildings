...
model MSLAIT2Nodes
  ...
  Data.PipeDataAIT151218 pipeDataAIT151218
    ...
    annotation(
      Placement(
        transformation(
          extent={{-30,-158}, {-10,-138}})));
  Modelica.Blocks.Sources.RealExpression m_flow_p3(
    y=-DataReader.y[7])
    "Mass flow rate for substation 3"
    annotation(
      Placement(
        transformation(
          extent={{-112,-126}, {-72,-106}})));
  Modelica.Blocks.Sources.RealExpression m_flow_p4(
    y=-DataReader.y[8])
    "Mass flow rate for substation 4"
    annotation(
      Placement(
        transformation(
          extent={{220, 166}, {180, 186}})));
  Modelica.Blocks.Sources.RealExpression m_flow_p2(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-146, 172}, {-106, 192}})));
  Modelica.Blocks.Sources.RealExpression T_p1(
    y=DataReader.y[1])
    "Inlet temperature"
    annotation(
      Placement(
        transformation(
          extent={{18,-132}, {58,-112}})));
  ...
  Modelica.Fluid.Pipes.DynamicPipe pip0(
    ...
    redeclare model FlowModel=Modelica.Fluid.Pipes.BaseClasses.FlowModels.NominalTurbulentPipeFlow(
      dp_nominal(
        displayUnit="Pa")=10*pip0.length,
      m_flow_nominal=0.3),
    nNodes=2,
  ...
  inner Modelica.Fluid.System system
    annotation(
      Placement(
        transformation(
          extent={{-140, 140}, {-120, 160}})));
  parameter Real R(
    unit="(m.K)/W")=1/(2*kIns*Modelica.Constants.pi)*log(0.18/0.0899) + 1/(2*2.4*Modelica.Constants.pi)*log(2/0.18)
    "Thermal resistance per unit length of main pipes";
  parameter Real R80(
    unit="(m.K)/W")=1/(2*0.024*Modelica.Constants.pi)*log(0.07/0.0337) + 1/(2*2.4*Modelica.Constants.pi)*log(2/0.07)
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor res0[pip0.nNodes](
    each R=2*R/pip0.length)
    "Thermal resistances for elements of pipe 0"
    annotation(
      Placement(
        transformation(
          extent={{94,-18}, {114, 2}})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector col0(
    m=pip0.nNodes)
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=180,
          origin={38, 10})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector col1(
    m=pip1.nNodes)
    "Combine heat flows to one"
    annotation(
      Placement(
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=90,
          origin={-96, 42})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector col2(
    m=pip2.nNodes)
    "Combine heat flows to one"
    annotation(
      Placement(
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=90,
          origin={-48,-30})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector col3(
    m=pip3.nNodes)
    "Combine heat flows to one"
    annotation(
      Placement(
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=270,
          origin={18, 40})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector col4(
    m=pip4.nNodes)
    "Combine heat flows to one"
    annotation(
      Placement(
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=0,
          origin={-28, 10})));
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector col5(
    m=pip5.nNodes)
    "Combine heat flows to one"
    annotation(
      Placement(
  ...
  Modelica.Blocks.Logical.Switch switch
    ...
    annotation(
      Placement(
        transformation(
          extent={{94, 158}, {74, 178}})));
  Modelica.Blocks.Sources.RealExpression m_flow_zero(
    y=0)
    "Default value when mass flow rate is below measurement noise threshold"
    annotation(
      Placement(
        transformation(
          extent={{220, 150}, {180, 170}})));
...
equation
  ...
  connect(m_flow_zero.y, switch.u3)
    annotation(
      Line(
        points={{178, 160}, {96, 160}},
        color={0, 0, 127}));
  connect(switch.u1, m_flow_p4.y)
    annotation(
      Line(
        points={{96, 176}, {178, 176}},
        color={0, 0, 127}));
  connect(Point4.m_flow_in, switch.y)
    annotation(
      Line(
        points={{30, 162}, {30, 168}, {73, 168}},
        color={0, 0, 127}));
  connect(switch.u2, lessThreshold.y)
    annotation(
      Line(
        points={{96, 168}, {108, 168}, {108, 134}, {123, 134}},
        color={255, 0, 255}));
  connect(lessThreshold.u, m_flow_p4.y)
    annotation(
      Line(
        points={{146, 134}, {152, 134}, {152, 176}, {178, 176}},
        color={0, 0, 127}));
  ...
  connect(lessThreshold.y, switch1.u2)
    annotation(
      Line(
        points={{123, 134}, {108, 134}, {108, 94}, {98, 94}},
        color={255, 0, 255}));
  connect(m_flow_p4.y, switch1.u3)
    annotation(
      Line(
        points={{178, 176}, {156, 176}, {156, 86}, {98, 86}},
        color={0, 0, 127}));
  connect(m_flow_zero.y, switch1.u1)
    annotation(
      Line(
        points={{178, 160}, {160, 160}, {160, 102}, {98, 102}},
        color={0, 0, 127}));
  connect(Point5.ports[1], senTem_p4.port_a)
    annotation(
      Line(
        points={{38, 102}, {18, 102}},
        color={0, 127, 255}));
  connect(pip2.heatPorts, res2.port_a)
    annotation(
      Line(
        points={{-91.6, 42.1}, {-71.8, 42.1}, {-71.8, 58}, {-72, 58}},
        color={127, 0, 0}));
  ...
  connect(res5.port_a, pip5.heatPorts)
    annotation(
      Line(
        points={{-28, 58}, {-28, 54}, {-28.1, 54}, {-28.1, 14.4}},
        color={191, 0, 0}));
  connect(res4.port_a, pip4.heatPorts)
    annotation(
      Line(
        points={{-6.66134e-016, 58}, {-6.66134e-016, 40.1}, {13.6, 40.1}},
        color={191, 0, 0}));
  connect(res4.port_b, col4.port_a)
    annotation(
      Line(
        points={{4.44089e-016, 78}, {4.44089e-016, 98}, {-1.33227e-015, 98}},
        color={191, 0, 0}));
  connect(col2.port_b, col5.port_b)
    annotation(
      Line(
        points={{-72, 118}, {-72, 184}, {-28, 184}, {-28, 118}},
        color={191, 0, 0}));
  connect(col5.port_b, col4.port_b)
    annotation(
      Line(
        points={{-28, 118}, {-28, 184}, {1.33227e-015, 184}, {1.33227e-015, 118}},
        color={191, 0, 0}));
  connect(pip1.heatPorts, res1.port_a)
    annotation(
      Line(
        points={{37.9, 14.4}, {37.9, 54}, {56, 54}},
        color={127, 0, 0}));
  ...
  connect(col4.port_b, col1.port_b)
    annotation(
      Line(
        points={{1.33227e-015, 118}, {1.33227e-015, 184}, {166, 184}, {166, 54}, {118, 54}},
        color={191, 0, 0}));
  connect(pip0.heatPorts, res0.port_a)
    annotation(
      Line(
        points={{86.4,-7.9}, {89.2,-7.9}, {89.2,-8}, {94,-8}},
        color={127, 0, 0}));
  connect(res0.port_b, col0.port_a)
    annotation(
      Line(
        points={{114,-8}, {118,-8}, {122,-8}},
        color={191, 0, 0}));
  connect(col1.port_b, col0.port_b)
    annotation(
      Line(
        points={{118, 54}, {166, 54}, {166,-8}, {142,-8}},
        color={191, 0, 0}));
  connect(col0.port_b, prescribedTemperature.port)
    annotation(
      Line(
        points={{142,-8}, {166,-8}, {166,-148}, {60,-148}},
        color={191, 0, 0}));
  connect(pip3.heatPorts, res3.port_a)
    annotation(
      Line(
        points={{-43.6,-30.1}, {-35.8,-30.1}, {-35.8,-30}, {-30,-30}},
        color={127, 0, 0}));
  ...
  connect(col3.port_b, prescribedTemperature.port)
    annotation(
      Line(
        points={{28,-30}, {166,-30}, {166,-148}, {60,-148}},
        color={191, 0, 0}));
  annotation(
    Diagram(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-160,-160}, {220, 200}})),
    experiment(
      StopTime=603900,
      Tolerance=1e-006),
    __Dymola_Commands(
      file="Resources/Scripts/Dymola/Fluid/FixedResistances/Validation/PlugFlowPipes/MSLAIT2Nodes.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
