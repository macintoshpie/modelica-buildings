...
model PlugFlowULg
  ...
  Modelica.Blocks.Math.UnitConversions.From_degC Tout
    ...
    annotation(
      Placement(
        transformation(
          extent={{40,-110}, {60,-90}})));
  Buildings.HeatTransfer.Sources.FixedTemperature TBou(
    T=295.15)
  ...
  Modelica.Blocks.Math.Add heatLossSim(
    k1=-1)
...
equation
  ...
  connect(DataReader.y[1], gain.u)
    annotation(
      Line(
        points={{21,-70}, {32,-70}, {32,-40}, {50,-40}},
        color={0, 0, 127}));
  connect(senTem_in.port_a, Boiler.port_b)
    annotation(
      Line(
        points={{0, 0}, {12, 0}, {12, 1.33227e-15}, {20, 1.33227e-15}},
        color={0, 127, 255}));
  connect(Boiler.port_a, WaterCityNetwork.ports[1])
    annotation(
      Line(
        points={{40,-1.33227e-15}, {40, 0}, {60, 0}},
        color={0, 127, 255}));
  connect(gain.y, WaterCityNetwork.m_flow_in)
    annotation(
      Line(
        points={{73,-40}, {104,-40}, {104, 8}, {82, 8}},
        color={0, 0, 127}));
  connect(Tin.y, Boiler.TSet)
    annotation(
      Line(
        points={{61,-70}, {94,-70}, {94, 28}, {52, 28}, {52, 8}, {42, 8}},
        color={0, 0, 127}));
  connect(Sewer1.ports[1], senTem_out.port_b)
    annotation(
      Line(
        points={{-200,-1.11022e-15}, {-188,-1.11022e-15}, {-188, 0}, {-180, 0}},
        color={0, 127, 255}));
  ...
  connect(heaLosMea.y, heaLosDiff.u2)
    annotation(
      Line(
        points={{181,-70}, {200,-70}, {200, 52}},
        color={0, 0, 127}));
  connect(heatLossSim.y, heaLosDiff.u1)
    annotation(
      Line(
        points={{101, 60}, {192, 60}},
        color={0, 0, 127}));
  connect(heaLosDiff.y, eneLosInt.u)
    annotation(
      Line(
        points={{209, 60}, {228, 60}},
        color={0, 0, 127}));
  connect(delT.u1, Tin.y)
    annotation(
      Line(
        points={{118,-84}, {104,-84}, {104,-70}, {61,-70}},
        color={0, 0, 127}));
  connect(Tout.y, delT.u2)
    annotation(
      Line(
        points={{61,-100}, {88,-100}, {88,-96}, {118,-96}},
        color={0, 0, 127}));
  connect(heaLosMea.u1, gain3.y)
    annotation(
      Line(
        points={{158,-64}, {148,-64}, {148,-40}, {141,-40}},
        color={0, 0, 127}));
  connect(heaLosMea.u2, delT.y)
    annotation(
      Line(
        points={{158,-76}, {150,-76}, {150,-90}, {141,-90}},
        color={0, 0, 127}));
  connect(heatLossSim.u2, senEntIn.H_flow)
    annotation(
      Line(
        points={{78, 54}, {-52, 54}, {-52, 11}},
        color={0, 0, 127}));
  connect(heatLossSim.u1, senEntOut.H_flow)
    annotation(
      Line(
        points={{78, 66}, {-130, 66}, {-130, 11}},
        color={0, 0, 127}));
  annotation(
    Documentation(
      info="<html>
...
<li>Januar 26, 2016 by Carles Ribas:<br/>First implementation.
    ...
    experiment(
      ...
      Tolerance=1e-006),
    __Dymola_Commands(
      file="Resources/Scripts/Dymola/Fluid/FixedResistances/Validation/PlugFlowPipes/PlugFlowULg.mos"
        "Simulate and plot"),
    Diagram(
      coordinateSystem(
        extent={{-260,-120}, {260, 120}})));
end PlugFlowULg;
