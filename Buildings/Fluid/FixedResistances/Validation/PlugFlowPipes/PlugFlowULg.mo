...
model PlugFlowULg
  ...
  parameter Modelica.SIunits.SpecificHeatCapacity cp_default=Medium.specificHeatCapacityCp(state=sta_default)
  ...
  Fluid.Sources.MassFlowSource_T WaterCityNetwork(redeclare package Medium=Medium, m_flow=1.245, use_m_flow_in=true, nPorts=1)
    "Mass flow source"
    annotation(Placement(transformation(extent={{-10, 10}, {10,-10}}, rotation=180, origin={70, 0})));
  Fluid.HeatExchangers.Heater_T Boiler(redeclare package Medium=Medium, m_flow_nominal=1, dp_nominal=0)
    "Boiler with adjustable outlet temperature"
  ...
  Fluid.Sources.Boundary_pT Sewer1(redeclare package Medium=Medium, nPorts=1)
    "Mass flow sink"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, rotation=180, origin={-210, 0})));
  Fluid.Sensors.TemperatureTwoPort senTem_out(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, tau=0, T_start=T_start_out)
    "Temperature sensor"
    annotation(Placement(transformation(extent={{-160,-10}, {-180, 10}})));
  Fluid.Sensors.TemperatureTwoPort senTem_in(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, tau=0, T_start=T_start_in)
    "Temperature sensor"
    annotation(Placement(transformation(extent={{0,-10}, {-20, 10}})));
  Modelica.Blocks.Sources.CombiTimeTable DataReader(tableOnFile=true, tableName="dat", fileName=pipeDataULg.filNam, columns=2 : pipeDataULg.nCol, extrapolation=Modelica.Blocks.Types.Extrapolation.HoldLastPoint)
    "Measurement data"
  ...
  Buildings.HeatTransfer.Sources.FixedTemperature TBou(T=295.15)
    ...
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=270, origin={-90, 32})));
  ...
  PlugFlowPipe pipe(redeclare package Medium=Medium, dh=0.05248, length=39, dIns(displayUnit="mm")=0.013, kIns=0.04, m_flow_nominal=m_flow_nominal, thickness=3.9e-3, T_start_out=T_start_out, T_start_in=T_start_in, R=((1/(2*pipe.kIns)*log((0.0603/2 + pipe.dIns)/(0.0603/2))) + 1/(5*(0.0603 + 2*pipe.dIns)))/Modelica.Constants.pi, nPorts=1, initDelay=true, m_flow_start=pipeDataULg.m_flowIni, cPip=500, rhoPip=8000)
    "Pipe"
    annotation(Placement(transformation(extent={{-80,-10}, {-100, 10}})));
  Fluid.Sensors.EnthalpyFlowRate senEntOut(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal)
    "Outlet enthalpy sensor"
  ...
  Modelica.Blocks.Continuous.Integrator eneLosInt
    "Integrate model error"
    annotation(Placement(transformation(extent={{230, 50}, {250, 70}})));
  Fluid.Sensors.EnthalpyFlowRate senEntIn(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal)
    "Inlet enthalpy sensor"
...
equation
  connect(DataReader.y[3], Tout.u)
    annotation(Line(points={{21,-70}, {32,-70}, {32,-100}, {38,-100}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(DataReader.y[5], Tin.u)
    annotation(Line(points={{21,-70}, {38,-70}}, color={0, 0, 127}));
  connect(DataReader.y[1], gain.u)
    annotation(Line(points={{21,-70}, {32,-70}, {32,-40}, {50,-40}}, color={0, 0, 127}));
  connect(senTem_in.port_a, Boiler.port_b)
    annotation(Line(points={{0, 0}, {12, 0}, {12, 1.33227e-15}, {20, 1.33227e-15}}, color={0, 127, 255}));
  connect(Boiler.port_a, WaterCityNetwork.ports[1])
    annotation(Line(points={{40,-1.33227e-15}, {40, 0}, {60, 0}}, color={0, 127, 255}));
  connect(gain.y, WaterCityNetwork.m_flow_in)
    annotation(Line(points={{73,-40}, {104,-40}, {104, 8}, {82, 8}}, color={0, 0, 127}));
  connect(Tin.y, Boiler.TSet)
    annotation(Line(points={{61,-70}, {94,-70}, {94, 28}, {52, 28}, {52, 8}, {42, 8}}, color={0, 0, 127}));
  connect(Sewer1.ports[1], senTem_out.port_b)
    annotation(Line(points={{-200,-1.11022e-15}, {-188,-1.11022e-15}, {-188, 0}, {-180, 0}}, color={0, 127, 255}));
  ...
  connect(delT.u1, Tin.y)
    annotation(Line(points={{118,-84}, {104,-84}, {104,-70}, {61,-70}}, color={0, 0, 127}));
  connect(Tout.y, delT.u2)
    annotation(Line(points={{61,-100}, {88,-100}, {88,-96}, {118,-96}}, color={0, 0, 127}));
  connect(heaLosMea.u1, gain3.y)
    annotation(Line(points={{158,-64}, {148,-64}, {148,-40}, {141,-40}}, color={0, 0, 127}));
  connect(heaLosMea.u2, delT.y)
    annotation(Line(points={{158,-76}, {150,-76}, {150,-90}, {141,-90}}, color={0, 0, 127}));
...
</html>"), experiment(StopTime=875, Tolerance=1e-006), __Dymola_Commands(file="Resources/Scripts/Dymola/Fluid/FixedResistances/Validation/PlugFlowPipes/PlugFlowULg.mos"
    "Simulate and plot"), Diagram(coordinateSystem(extent={{-260,-120}, {260, 120}})));
end PlugFlowULg;
