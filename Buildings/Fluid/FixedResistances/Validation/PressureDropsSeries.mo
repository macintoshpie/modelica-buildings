...
equation
  for i in 1 : nRes-1 loop
    connect(res[i].port_b, res[i + 1].port_a)
      annotation(
        Line(
          points={{20, 30}, {26, 30}, {26, 10}, {-8, 10}, {-8, 30}, {-5.55112e-16, 30}},
          color={0, 127, 255}));
  end for;
  connect(P.y, sou.p_in)
    annotation(
      Line(
        points={{-59, 38}, {-52, 38}, {-42, 38}},
        color={0, 0, 127}));
  ...
  annotation(
    experiment(
      ...
      StopTime=1.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/FixedResistances/Validation/PressureDropsSeries.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
