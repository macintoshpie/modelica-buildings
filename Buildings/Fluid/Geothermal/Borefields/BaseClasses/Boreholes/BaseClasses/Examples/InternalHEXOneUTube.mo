...
model InternalHEXOneUTube
  ...
  Buildings.Fluid.Sources.MassFlowSource_T boundary(nPorts=1, redeclare package Medium=Medium, m_flow=borFieDat.conDat.mBor_flow_nominal, T=293.15)
    annotation(Placement(transformation(extent={{-48,-4}, {-28, 16}})));
  Buildings.Fluid.Sources.MassFlowSource_T boundary1(nPorts=1, redeclare package Medium=Medium, m_flow=borFieDat.conDat.mBor_flow_nominal, T=293.15)
  ...
  parameter Buildings.Fluid.Geothermal.Borefields.Data.Borefield.Example borFieDat(conDat=Buildings.Fluid.Geothermal.Borefields.Data.Configuration.Example(use_Rb=false))
    "Borefield data"
    annotation(Placement(transformation(extent={{-100,-100}, {-80,-80}})));
equation
  connect(boundary1.ports[1], intHex.port_a2)
    annotation(Line(points={{34,-6}, {10,-6}, {10,-7.6}}, color={0, 127, 255}));
  connect(boundary.ports[1], intHex.port_a1)
    annotation(Line(points={{-28, 6}, {-10, 6}, {-10, 5.6}}, color={0, 127, 255}));
  connect(bou.ports[1], senTem.port_b)
    annotation(Line(points={{-28,-22}, {70,-22}, {70, 6}, {28, 6}}, color={0, 127, 255}));
  connect(senTem.port_a, intHex.port_b1)
    annotation(Line(points={{16, 6}, {10, 6}, {10, 5.6}}, color={0, 127, 255}));
  connect(senTem1.port_a, bou.ports[2])
    annotation(Line(points={{-28,-6}, {-28,-26}, {-28,-26}}, color={0, 127, 255}));
  connect(senTem1.port_b, intHex.port_b2)
    annotation(Line(points={{-16,-6}, {-10,-6}, {-10,-7.6}}, color={0, 127, 255}));
  ...
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 100}})), experiment(StopTime=100000, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/BaseClasses/Boreholes/BaseClasses/Examples/InternalHEXOneUTube.mos"
