...
model InternalResistancesTwoUTube
  ...
  parameter Modelica.SIunits.Temperature T_start=298.15
    "Initial temperature";
  Buildings.HeatTransfer.Sources.FixedTemperature TWal(
    T=T_start)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-70,-4}, {-50, 16}})));
  Buildings.HeatTransfer.Sources.FixedTemperature TPip2(
    T=T_start + 15)
  ...
  Buildings.Fluid.Geothermal.Borefields.BaseClasses.Boreholes.BaseClasses.InternalResistancesTwoUTube intRes2UTub(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10,-10}, {10, 10}})));
  Buildings.HeatTransfer.Sources.FixedTemperature TPip3(
    T=T_start + 25)
    "Wall temperature of pipe 2"
    annotation(
      Placement(
        transformation(
          extent={{30,-50}, {10,-30}})));
  Buildings.HeatTransfer.Sources.FixedTemperature TPip4(
    T=T_start + 10)
    "Wall temperature of pipe 2"
    annotation(
      Placement(
...
equation
  ...
  annotation(
    experiment(
      ...
      Tolerance=1e-6),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/BaseClasses/Boreholes/BaseClasses/Examples/InternalResistancesTwoUTube.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
