...
model InternalResistancesTwoUTubeNegative
  ...
  parameter Real x(
    fixed=false)
    "Capacity location";
  parameter Modelica.SIunits.ThermalResistance Rgb(
    fixed=false)
    "Thermal resistance between grout zone and borehole wall";
  parameter Modelica.SIunits.ThermalResistance Rgg1(
    fixed=false)
    "Thermal resistance between the two adjacent grout zones";
  parameter Modelica.SIunits.ThermalResistance Rgg2(
    fixed=false)
    "Thermal resistance between the two opposite grout zones";
  parameter Modelica.SIunits.ThermalResistance RCondGro(
    fixed=false)
    "Thermal resistance between: pipe wall to capacity in grout";
initial equation
  (x, Rgb, Rgg1, Rgg2, RCondGro)=Buildings.Fluid.Geothermal.Borefields.BaseClasses.Boreholes.BaseClasses.Functions.internalResistancesTwoUTube(
    hSeg=hSeg,
    rBor=rBor,
    rTub=rTub,
    eTub=eTub,
  ...
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/BaseClasses/Boreholes/BaseClasses/Functions/Validation/InternalResistancesTwoUTubeNegative.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=1.0),
