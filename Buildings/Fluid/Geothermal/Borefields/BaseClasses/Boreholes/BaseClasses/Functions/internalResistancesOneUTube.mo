...
protected
  Real[2, 2] RDelta(each unit="(m.K)/W")
    "Delta-circuit thermal resistances";
  Real[2, 2] R(each unit="(m.K)/W")
    "Internal thermal resistances";
  Modelica.SIunits.Position[2] xPip={-sha, sha}
    "x-Coordinates of pipes";
  Modelica.SIunits.Position[2] yPip={0., 0.}
    "y-Coordinates of pipes";
  Modelica.SIunits.Radius[2] rPip={rTub, rTub}
    "Outer radius of pipes";
  Real[2] RFluPip(each unit="(m.K)/W")={RCondPipe + RConv, RCondPipe + RConv}
