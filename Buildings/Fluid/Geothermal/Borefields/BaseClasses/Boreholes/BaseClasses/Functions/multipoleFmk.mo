...
function multipoleFmk
  ...
  input Real QPip_flow[nPip](each unit="W/m")
    "Heat flow in pipes";
  input Real PRea[nPip, J]
    "Multipoles (Real part)";
  input Real PIma[nPip, J]
    "Multipoles (Imaginary part)";
  ...
  output Real FRea[nPip, J]
    "Multipole coefficients";
  output Real FIma[nPip, J]
    "Multipole coefficients";
protected
  Complex zPip_i
    "Position of pipe i";
  ...
  Complex f
    "Intermedia value of multipole coefficient";
  Integer j_pend
    "Maximum loop index in fourth term";
algorithm
    ...
    for k in 1 : J loop
      ...
      for n in 1 : nPip loop
        ...
        for j in 1 : J loop
          ...
          // Third term
          if m <> n then
            f := f + P_nj*Buildings.Utilities.Math.Functions.binomial(j + k-1, j-1)*rPip[n]^j*(-rPip[m])^k/(zPip_i-zPip_j)^(j + k);
          end if;
          //Fourth term
          j_pend := min(k, j);
          for jp in 0 : j_pend loop
            f := f + sigma*Modelica.ComplexMath.conj(P_nj)*Buildings.Utilities.Math.Functions.binomial(j, jp)*Buildings.Utilities.Math.Functions.binomial(j + k-jp-1, j-1)*rPip[n]^j*rPip[m]^k*zPip_i^(j-jp)*Modelica.ComplexMath.conj(zPip_j)^(k-jp)/(rBor^2-zPip_i*Modelica.ComplexMath.conj(zPip_j))^(k + j-jp);
          end for;
        end for;
      end for;
