...
model InternalResistancesOneUTube
  ...
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor capFil1(
    ...
    der_T(
      fixed=(energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyStateInitial))) if dynFil
...
equation
  connect(port_1, Rpg1.port_a)
    annotation(
      Line(
        points={{0, 100}, {0, 100}, {0, 80}},
        color={191, 0, 0}));
  connect(Rpg1.port_b, Rgb1.port_a)
    annotation(
      Line(
        points={{0, 60}, {0, 60}, {0, 50}, {-30, 50}, {-30, 40}},
        color={191, 0, 0}));
  connect(Rgb1.port_b, port_wall)
    annotation(
      Line(
        points={{-30, 20}, {-30, 20}, {-30, 0}, {0, 0}},
        color={191, 0, 0}));
  connect(capFil1.port, Rgb1.port_a)
    annotation(
      Line(
        points={{-60, 50}, {-30, 50}, {-30, 40}},
        color={191, 0, 0}));
  ...
  connect(Rgg.port_b, Rpg2.port_a)
    annotation(
      Line(
        points={{40, 50}, {46, 50}, {50, 50}, {50, 0}, {60, 0}},
        color={191, 0, 0}));
