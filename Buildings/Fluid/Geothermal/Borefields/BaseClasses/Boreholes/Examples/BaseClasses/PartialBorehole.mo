  ...
  parameter Buildings.Fluid.Geothermal.Borefields.Data.Borefield.Example borFieDat
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80,-80}, {-60,-60}})));
  replaceable Buildings.Fluid.Geothermal.Borefields.BaseClasses.Boreholes.BaseClasses.PartialBorehole borHol constrainedby Buildings.Fluid.Geothermal.Borefields.BaseClasses.Boreholes.BaseClasses.PartialBorehole(
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort TBorIn(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-50,-10}, {-30, 10}})));
  Buildings.Fluid.Sensors.TemperatureTwoPort TBorOut(
    m_flow_nominal=borFieDat.conDat.mBor_flow_nominal,
    redeclare package Medium=Medium,
    tau=0)
    ...
    annotation(
      Placement(
        transformation(
          extent={{30,-10}, {50, 10}})));
  Buildings.HeatTransfer.Sources.FixedTemperature preTem[nSeg](
    each T=T_start)
...
equation
  ...
  connect(TBorIn.port_b, borHol.port_a)
    annotation(
      Line(
        points={{-30, 0}, {-14, 0}, {-14, 1.77636e-015}},
        color={0, 127, 255}));
  connect(borHol.port_b, TBorOut.port_a)
    annotation(
      Line(
        points={{14, 1.77636e-015}, {14, 0}, {30, 0}},
        color={0, 127, 255}));
  connect(TBorOut.port_b, sin.ports[1])
    annotation(
      Line(
        points={{50, 0}, {70, 0}, {70,-2}},
        color={0, 127, 255}));
  connect(borHol.port_wall, preTem.port)
    annotation(
      Line(
        points={{1.77636e-15, 14}, {0, 14}, {0, 70}, {-40, 70}},
        color={191, 0, 0}));
  annotation(
    Documentation(
      info="<html>
