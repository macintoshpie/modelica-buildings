...
model AggregationCellTimes
  ...
  parameter Integer i=6
    "Number of aggregation cells";
  parameter Modelica.SIunits.Time tLoaAgg=10
    "Time resolution of load aggregation";
  final parameter Modelica.SIunits.Time[i] nu(each fixed=false)
    "Time vector for load aggregation";
  final parameter Modelica.SIunits.Time[i] rCel(each fixed=false)
    "Cell widths";
  Modelica.SIunits.Time nu_error, rCel_error
    "Error on chosen values";
initial equation
  (nu, rCel)=Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.LoadAggregation.aggregationCellTimes(i=i, lvlBas=2, nCel=2, tLoaAgg=tLoaAgg, timFin=12*tLoaAgg);
equation
  nu_error=100.0-nu[i-1];
  rCel_error=2.0-rCel[i];
  annotation(experiment(Tolerance=1e-6, StopTime=1.0), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/BaseClasses/HeatTransfer/LoadAggregation/Validation/AggregationCellTimes.mos"
