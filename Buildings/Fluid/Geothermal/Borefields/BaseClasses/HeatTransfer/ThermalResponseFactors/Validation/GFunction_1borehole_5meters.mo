...
model GFunction_1borehole_5meters
  ...
  final parameter Real[nTimTot] gFun(
    each fixed=false);
  final parameter Real[nTimTot] lntts(
    each fixed=false);
  final parameter Modelica.SIunits.Time[nTimTot] tGFun(
    each fixed=false);
  final parameter Real[nTimTot] dspline(
    each fixed=false);
  Real gFun_int
...
initial equation
  ...
  (tGFun, gFun)=Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.gFunction(
    nBor=nBor,
    cooBor=cooBor,
    hBor=hBor,
    dBor=dBor,
...
equation
  ...
  annotation(
    experiment(
      ...
      StopTime=412000000),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/BaseClasses/HeatTransfer/ThermalResponseFactors/Validation/GFunction_1borehole_5meters.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
