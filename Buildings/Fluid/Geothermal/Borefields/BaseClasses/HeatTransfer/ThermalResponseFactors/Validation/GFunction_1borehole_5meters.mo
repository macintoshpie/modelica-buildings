...
model GFunction_1borehole_5meters
  ...
  parameter Integer nBor=1
    "Number of boreholes";
  parameter Modelica.SIunits.Position cooBor[nBor, 2]={{5.*mod(i-1, 3), 5.*floor((i-1)/3)} for i in 1 : nBor}
    "Coordinates of boreholes";
  parameter Modelica.SIunits.Height hBor=5
    "Borehole length";
  parameter Modelica.SIunits.Height dBor=1
    "Borehole buried depth";
  parameter Modelica.SIunits.Radius rBor=0.075
    "Borehole radius";
  parameter Modelica.SIunits.ThermalDiffusivity aSoi=1e-6
  ...
  parameter Integer nTimSho=26
    "Number of time steps in short time region";
  parameter Integer nTimLon=50
  ...
  Real gFun_int
    "Interpolated value of g-function";
  Real lntts_int
    "Non-dimensional logarithmic time for interpolation";
  discrete Integer k
    "Current interpolation interval";
  discrete Modelica.SIunits.Time t1
    "Previous value of time for interpolation";
  discrete Modelica.SIunits.Time t2
    "Next value of time for interpolation";
  discrete Real gFun1
    "Previous g-function value for interpolation";
  discrete Real gFun2
    "Next g-function value for interpolation";
...
initial equation
  ...
  (tGFun, gFun)=Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.gFunction(nBor=nBor, cooBor=cooBor, hBor=hBor, dBor=dBor, rBor=rBor, aSoi=aSoi, nSeg=nSeg, nTimSho=nTimSho, nTimLon=nTimLon, ttsMax=ttsMax);
  lntts=log(tGFun/ts .+ Modelica.Constants.small);
  // Initialize parameters for interpolation
  dspline=Buildings.Utilities.Math.Functions.splineDerivatives(x=tGFun, y=gFun);
  k=1;
  t1=tGFun[1];
  t2=tGFun[2];
  gFun1=gFun[1];
...
equation
  ...
  gFun_int=Buildings.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(x=time, x1=t1, x2=t2, y1=gFun1, y2=gFun2, y1d=dspline[pre(k)], y2d=dspline[pre(k) + 1]);
  ...
  annotation(experiment(Tolerance=1e-6, StopTime=412000000), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/BaseClasses/HeatTransfer/ThermalResponseFactors/Validation/GFunction_1borehole_5meters.mos"
