...
model Analytic_20Years
  ...
  parameter Buildings.Fluid.Geothermal.Borefields.Data.Borefield.Template borFieDat(conDat(borCon=Types.BoreholeConfiguration.SingleUTube, nBor=1, cooBor={{0, 0}}, mBor_flow_nominal=0.3, dp_nominal=5e4, hBor=100, rBor=0.05, dBor=4, rTub=0.02, kTub=0.5, eTub=0.002, xC=0.05), soiDat(kSoi=1, cSoi=1, dSoi=1e6), filDat(kFil=0, cFil=Modelica.Constants.small, dFil=Modelica.Constants.small, steadyState=true))
    "Borefield parameters"
    annotation(Placement(transformation(extent={{-80,-80}, {-60,-60}})));
  Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.GroundTemperatureResponse groTemRes(tLoaAgg=3600, nCel=5, borFieDat=borFieDat, forceGFunCalc=true)
    "Ground temperature response of borehole"
    annotation(Placement(transformation(extent={{-40, 40}, {-20, 60}})));
  Modelica.Blocks.Sources.CombiTimeTable timTabQ(tableOnFile=true, tableName="tab1", columns={2}, smoothness=Modelica.Blocks.Types.Smoothness.ConstantSegments, fileName=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/Data/Fluid/Geothermal/Borefields/HeatTransfer/Validation/Analytic_20Years.txt"))
    "Table for heat injected, using constant segments"
    annotation(Placement(transformation(extent={{-80, 40}, {-60, 60}})));
  Modelica.Blocks.Math.Add add(y(unit="K"), k1=-1)
  ...
  Modelica.Blocks.Sources.CombiTimeTable timTabT(tableOnFile=true, tableName="tab1", columns={3}, smoothness=Modelica.Blocks.Types.Smoothness.LinearSegments, fileName=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/Data/Fluid/Geothermal/Borefields/HeatTransfer/Validation/Analytic_20Years.txt"), y(each unit="K", each displayUnit="degC"))
  ...
  Modelica.Blocks.Math.Add TBorWal(y(unit="K"))
...
equation
  connect(TBorWal.u2, const.y)
    annotation(Line(points={{18, 14}, {-14, 14}, {-14, 10}, {-39, 10}}, color={0, 0, 127}));
  connect(groTemRes.delTBor, TBorWal.u1)
    annotation(Line(points={{-19, 50}, {-6, 50}, {-6, 26}, {18, 26}}, color={0, 0, 127}));
  connect(add.u1, TBorWal.y)
    annotation(Line(points={{58,-4}, {50,-4}, {50, 20}, {41, 20}}, color={0, 0, 127}));
  connect(add.u2, timTabT.y[1])
    annotation(Line(points={{58,-16}, {0,-16}, {0,-20}, {-59,-20}}, color={0, 0, 127}));
  connect(groTemRes.QBor_flow, timTabQ.y[1])
    annotation(Line(points={{-41, 50}, {-59, 50}}, color={0, 0, 127}));
  annotation(experiment(StopTime=630720000, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/BaseClasses/HeatTransfer/Validation/Analytic_20Years.mos"
