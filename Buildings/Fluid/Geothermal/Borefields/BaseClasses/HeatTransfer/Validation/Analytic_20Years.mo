...
model Analytic_20Years
  ...
  Modelica.Blocks.Sources.CombiTimeTable timTabQ(
    ...
    fileName=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/Data/Fluid/Geothermal/Borefields/HeatTransfer/Validation/Analytic_20Years.txt"))
  ...
  Modelica.Blocks.Sources.Constant const(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60, 0}, {-40, 20}})));
  Modelica.Blocks.Math.Add TBorWal(
    y(
      unit="K"))
...
equation
  connect(TBorWal.u2, const.y)
    annotation(
      Line(
        points={{18, 14}, {-14, 14}, {-14, 10}, {-39, 10}},
        color={0, 0, 127}));
  connect(groTemRes.delTBor, TBorWal.u1)
    annotation(
      Line(
        points={{-19, 50}, {-6, 50}, {-6, 26}, {18, 26}},
        color={0, 0, 127}));
  connect(add.u1, TBorWal.y)
    annotation(
      Line(
        points={{58,-4}, {50,-4}, {50, 20}, {41, 20}},
        color={0, 0, 127}));
  connect(add.u2, timTabT.y[1])
    annotation(
      Line(
        points={{58,-16}, {0,-16}, {0,-20}, {-59,-20}},
        color={0, 0, 127}));
  ...
  annotation(
    experiment(
      ...
      Tolerance=1e-6),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/BaseClasses/HeatTransfer/Validation/Analytic_20Years.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
