  ...
  Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.GroundTemperatureResponse groTemRes(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-50,-10}, {-30, 10}})));
  Modelica.Blocks.Sources.Constant groTem(
    k=T_start)
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=0,
          origin={0, 20})));
  Modelica.Blocks.Math.Add TSoiAna(
    y(
      unit="K"))
...
equation
  ...
  connect(sine.y, preHeaFlo.Q_flow)
    annotation(
      Line(
        points={{-71, 0}, {-60, 0}, {-60, 60}, {-50, 60}},
        color={0, 0, 127}));
  connect(preTem.port, soi.port_b)
    annotation(
      Line(
        points={{28, 60}, {18, 60}, {8, 60}},
        color={191, 0, 0}));
  connect(preTem.T, groTem.y)
    annotation(
      Line(
        points={{50, 60}, {60, 60}, {60, 88}, {-66, 88}, {-66,-36}, {-73,-36}},
        color={0, 0, 127}));
  connect(temSenDis.T, deltaT.u1)
    annotation(
      Line(
        points={{10, 20}, {20, 20}, {20, 6}, {28, 6}},
        color={0, 0, 127}));
  connect(soi.port_a, temSenDis.port)
    annotation(
      Line(
        points={{-12, 60}, {-20, 60}, {-20, 20}, {-10, 20}},
        color={191, 0, 0}));
  connect(groTemRes.delTBor, TSoiAna.u1)
    annotation(
      Line(
        points={{-29, 0}, {-20, 0}, {-20,-24}, {-14,-24}},
        color={0, 0, 127}));
  ...
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/BaseClasses/HeatTransfer/Validation/FiniteDifference_1Week.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=604800.0),
