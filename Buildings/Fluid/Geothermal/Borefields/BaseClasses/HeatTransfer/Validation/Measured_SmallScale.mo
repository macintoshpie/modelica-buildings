...
model Measured_SmallScale
  ...
  Buildings.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.GroundTemperatureResponse groTemRes(nCel=5, borFieDat=borFieDat, forceGFunCalc=true, tLoaAgg=360000)
    "Ground temperature response of borehole"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}})));
  Modelica.Blocks.Sources.CombiTimeTable meaDat(tableOnFile=true, timeScale=sizFac^2, fileName=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/Data/Fluid/Geothermal/Borefields/HeatTransfer/Validation/Cimmino_Bernier_2015_SmallScale.txt"), columns={2, 3, 4, 5, 6, 7, 8, 9}, tableName="data", offset={0, 0, 0, 273.15, 273.15, 273.15, 273.15, 273.15})
    "Measurement data"
    annotation(Placement(transformation(extent={{-80,-10}, {-60, 10}})));
  Modelica.Blocks.Math.Add TBorHol(y(unit="K"))
    "Borehole temperature"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=0, origin={40,-30})));
...
equation
  ...
  connect(add.u1, meaDat.y[4])
    annotation(Line(points={{66,-54}, {-50,-54}, {-50, 0}, {-59, 0}}, color={0, 0, 127}));
  connect(add.u2, TBorHol.y)
    annotation(Line(points={{66,-66}, {60,-66}, {60,-30}, {51,-30}}, color={0, 0, 127}));
  connect(TBorHol.u2, meaDat.y[8])
    annotation(Line(points={{28,-36}, {-50,-36}, {-50, 0}, {-59, 0}}, color={0, 0, 127}));
  connect(TBorHol.u1, groTemRes.delTBor)
    annotation(Line(points={{28,-24}, {20,-24}, {20, 0}, {11, 0}}, color={0, 0, 127}));
  connect(groTemRes.QBor_flow, scaFac.y)
    annotation(Line(points={{-11, 0}, {-19, 0}}, color={0, 0, 127}));
  annotation(experiment(Tolerance=1e-6, StopTime=85050000000.0), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/BaseClasses/HeatTransfer/Validation/Measured_SmallScale.mos"
