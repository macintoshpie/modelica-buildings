  ...
  final parameter Buildings.Fluid.Geothermal.Borefields.Data.Configuration.Template conDat(
    ...
    annotation(
      Placement(
        transformation(
          extent={{80, 40}, {100, 60}})));
  final parameter Buildings.Fluid.Geothermal.Borefields.Data.Borefield.Template borFieDat(
    final filDat=filDat,
    final soiDat=soiDat,
    final conDat=conDat)
...
equation
  ...
  connect(borFie.port_b, hea.port_a)
    annotation(
      Line(
        points={{60, 0}, {70, 0}, {70,-20}, {-52,-20}, {-52, 0}, {-40, 0}},
        color={0, 127, 255}));
  connect(pum.port_a, bou.ports[1])
    annotation(
      Line(
        points={{0, 0}, {0,-60}, {-40,-60}},
        color={0, 127, 255}));
  annotation(
    Documentation(
      info="<html>
...
First implementation.
    ...
    __Dymola_Commands(
      file="Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/Examples/RectangularBorefield.mos"
        "Simulate and plot"),
    experiment(
      StopTime=2678400,
      Tolerance=1e-6));
