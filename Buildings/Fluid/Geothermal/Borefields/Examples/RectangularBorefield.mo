...
model RectangularBorefield
  ...
  final parameter Buildings.Fluid.Geothermal.Borefields.Data.Configuration.Template conDat(final borCon=Buildings.Fluid.Geothermal.Borefields.Types.BoreholeConfiguration.SingleUTube, final use_Rb=false, final mBor_flow_nominal=m_flow_nominal/(nXBorHol*nYBorHol), final mBorFie_flow_nominal=m_flow_nominal, final hBor=hBor, final dBor=1, final rBor=0.2, final rTub=rTub, final kTub=0.5, final eTub=0.002, final xC=0.05, final dp_nominal=dpBorFie_nominal, final cooBor={{dBorHol*mod((i-1), nXBorHol), dBorHol*floor((i-1)/nXBorHol)} for i in 1 : nBorHol})
    "Borefield configuration"
    annotation(Placement(transformation(extent={{80, 40}, {100, 60}})));
  final parameter Buildings.Fluid.Geothermal.Borefields.Data.Borefield.Template borFieDat(final filDat=filDat, final soiDat=soiDat, final conDat=conDat)
    "Borefield parameters"
    annotation(Placement(transformation(extent={{60, 74}, {80, 94}})));
  Buildings.Fluid.Geothermal.Borefields.OneUTube borFie(redeclare package Medium=Medium, borFieDat=borFieDat, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, TExt0_start=280.65, allowFlowReversal=false)
  ...
  Buildings.Fluid.Movers.FlowControlled_m_flow pum(redeclare package Medium=Medium, addPowerToMedium=false, use_inputFilter=false, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, massDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, inputType=Buildings.Fluid.Types.InputType.Constant, m_flow_nominal=borFieDat.conDat.mBorFie_flow_nominal, allowFlowReversal=false)
    annotation(Placement(transformation(extent={{0,-10}, {20, 10}})));
  Buildings.Fluid.HeatExchangers.Heater_T hea(redeclare package Medium=Medium, show_T=true, m_flow_nominal=borFieDat.conDat.mBorFie_flow_nominal, m_flow(start=borFieDat.conDat.mBorFie_flow_nominal), energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyStateInitial, dp_nominal=dpHex_nominal, allowFlowReversal=false)
    "Heater"
...
equation
  ...
  connect(TSou.y, hea.TSet)
    annotation(Line(points={{-59, 30}, {-52, 30}, {-52, 8}, {-42, 8}}, color={0, 0, 127}));
  connect(pum.port_b, borFie.port_a)
    annotation(Line(points={{20, 0}, {40, 0}}, color={0, 127, 255}));
  connect(borFie.port_b, hea.port_a)
    annotation(Line(points={{60, 0}, {70, 0}, {70,-20}, {-52,-20}, {-52, 0}, {-40, 0}}, color={0, 127, 255}));
...
</html>"), __Dymola_Commands(file="Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/Examples/RectangularBorefield.mos"
    "Simulate and plot"), experiment(StopTime=2678400, Tolerance=1e-6));
