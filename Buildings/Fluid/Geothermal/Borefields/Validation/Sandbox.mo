...
model Sandbox
  ...
  Modelica.Blocks.Sources.CombiTimeTable sandBoxMea(
    ...
    offset={0, 0, 0},
    columns={2, 3, 4},
    fileName=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/Data/Fluid/Geothermal/Borefields/HeatTransfer/Validation/Beier_Smith_Spitler_2011_SandBox.txt"))
    annotation(
      Placement(
        transformation(
          extent={{-90,-10}, {-70, 10}})));
  Buildings.Fluid.HeatExchangers.HeaterCooler_u hea(
    ...
    m_flow_nominal=borFieDat.conDat.mBorFie_flow_nominal,
    m_flow(
      start=borFieDat.conDat.mBorFie_flow_nominal),
    p_start=100000)
    "Heater"
    annotation(
      Placement(
...
equation
  ...
  connect(sin.ports[1], TBorFieOut.port_b)
    annotation(
      Line(
        points={{80, 10}, {100, 10}, {100,-20}, {90,-20}},
        color={0, 127, 255}));
  ...
  connect(hea.port_a, TBorFieOut.port_b)
    annotation(
      Line(
        points={{-50,-20}, {-100,-20}, {-100, 40}, {100, 40}, {100,-20}, {90,-20}},
        color={0, 127, 255}));
  connect(sandBoxMea.y[3], hea.u)
    annotation(
      Line(
        points={{-69, 0}, {-58, 0}, {-58,-14}, {-52,-14}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=186360),
    __Dymola_Commands(
      file="Resources/Scripts/Dymola/Fluid/Geothermal/Borefields/Validation/Sandbox.mos"
        "Simulate and Plot"),
    Documentation(
      info="<html>
<p>
