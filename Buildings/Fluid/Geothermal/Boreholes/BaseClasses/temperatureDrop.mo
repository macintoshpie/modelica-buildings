...
algorithm
  ...
  if iSam == 1 then
    // First call, at t=0
    dT := 0;
    QL_flow := Buildings.Fluid.Geothermal.Boreholes.BaseClasses.exchangeValues(table=table, iX=iSam, x=Q_flow, iY=iSam);
  else
    dT := 0;
    ...
    for i in 1 :(iSam-1) loop
      QL_flow := Buildings.Fluid.Geothermal.Boreholes.BaseClasses.exchangeValues(table=table, iX=iSam, x=Q_flow, iY=iSam + 1-i);
      QU_flow := Buildings.Fluid.Geothermal.Boreholes.BaseClasses.exchangeValues(table=table, iX=iSam, x=Q_flow, iY=iSam-i);
      ...
      dT := dT + 1/(4*Modelica.Constants.pi*k)*Buildings.Fluid.Geothermal.Boreholes.BaseClasses.powerSeries(u=c*d/(4*k*i*samplePeriod)*rExt^2, N=10)*(QL_flow-QU_flow)/hSeg;
    end for;
  end if;
