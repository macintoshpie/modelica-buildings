...
algorithm
  ...
  if iSam == 1 then
    ...
    for i in 1 :(iSam-1) loop
      ...
      dT := dT + 1/(4*Modelica.Constants.pi*k)*Buildings.Fluid.Geothermal.Boreholes.BaseClasses.powerSeries(
        u=c*d/(4*k*i*samplePeriod)*rExt^2,
        N=10)*(QL_flow-QU_flow)/hSeg;
    end for;
  end if;
  annotation(
    Documentation(
