...
model Cooling
  ...
  replaceable parameter Data.Generic perCoo
    ...
    annotation(
      ...
      Placement(
        transformation(
          extent={{102,-98}, {118,-82}})));
  parameter Integer nBeams(
    min=1)=1
  ...
  parameter MediumWat.Temperature TWatCoo_start=MediumWat.T_default
    ...
    annotation(
      Dialog(
        tab="Initialization",
        group="Cooling"));
  parameter MediumWat.MassFlowRate mWat_flow_small(
    min=0)=1E-4*abs(perCoo.mWat_flow_nominal)
    "Small mass flow rate for regularization of zero flow"
    annotation(
      Dialog(
        tab="Advanced"));
  parameter MediumAir.MassFlowRate mAir_flow_small(
    min=0)=1E-4*abs(perCoo.mAir_flow_nominal)
    "Small mass flow rate for regularization of zero flow"
    annotation(
      Dialog(
        tab="Advanced"));
  ...
  Modelica.Fluid.Interfaces.FluidPort_a watCoo_a(
    ...
    m_flow(
      min=if allowFlowReversalWat then-Modelica.Constants.inf else 0),
    h_outflow(
      start=MediumWat.h_default))
    "Fluid connector watCoo_a (positive design flow direction is from watCoo_a to watCoo_b)"
    annotation(
      Placement(
        transformation(
          extent={{-150, 50}, {-130, 70}})));
  Modelica.Fluid.Interfaces.FluidPort_b watCoo_b(
    redeclare final package Medium=MediumWat,
    m_flow(
      max=if allowFlowReversalWat then + Modelica.Constants.inf else 0),
    h_outflow(
      start=MediumWat.h_default))
  ...
  Modelica.Fluid.Interfaces.FluidPort_a air_a(
    ...
    m_flow(
      min=if allowFlowReversalAir then-Modelica.Constants.inf else 0),
    h_outflow(
      start=MediumAir.h_default))
    "Fluid connector air_a (positive design flow direction is from air_a to air_b)"
    annotation(
      Placement(
        transformation(
          extent={{130,-70}, {150,-50}})));
  Modelica.Fluid.Interfaces.FluidPort_b air_b(
    redeclare final package Medium=MediumAir,
    m_flow(
      max=if allowFlowReversalAir then + Modelica.Constants.inf else 0),
    h_outflow(
      start=MediumAir.h_default))
    ...
    annotation(
      Placement(
        transformation(
          ...
          origin={0,-36})));
  // Pressure drop
  Modelica.SIunits.PressureDifference dpWatCoo(
    displayUnit="Pa")=watCoo_a.p-watCoo_b.p
    "Pressure difference watCoo_a minus watCoo_b";
  Modelica.SIunits.PressureDifference dpAir(
    displayUnit="Pa")=air_a.p-air_b.p
...
equation
  ...
  connect(sum.y, gaiSig.u)
    annotation(
      Line(
        points={{61, 30}, {66, 30}, {70, 30}, {70,-20}, {62,-20}},
        color={0, 0, 127}));
  connect(gaiSig.y, heaToRoo.Q_flow)
    annotation(
      Line(
        points={{39,-20}, {0,-20}, {0,-26}},
        color={0, 0, 127}));
  connect(senTemRooAir.port, heaPor)
    annotation(
      Line(
        points={{-20,-40}, {-14,-40}, {-14,-52}, {0,-52}, {0,-120}},
        color={191, 0, 0}));
  ...
  connect(conCoo.Q_flow, sum.u[1])
    annotation(
      Line(
        points={{11, 67}, {20, 67}, {20, 30}, {38, 30}},
        color={0, 0, 127}));
  connect(senTemRooAir.T, conCoo.TRoo)
    annotation(
      Line(
        points={{-40,-40}, {-50,-40}, {-50, 54}, {-12, 54}},
        color={0, 0, 127}));
  connect(air_a, res.port_a)
    annotation(
      Line(
        points={{140,-60}, {90,-60}, {40,-60}},
        color={0, 127, 255}));
  connect(senFloAir.port_a, res.port_b)
    annotation(
      Line(
        points={{-80,-60}, {-30,-60}, {20,-60}},
        color={0, 127, 255}));
  connect(watCoo_a, conCoo.port_a)
    annotation(
      Line(
        points={{-140, 60}, {-76, 60}, {-10, 60}},
        color={0, 127, 255}));
  connect(senFloAir.m_flow, conCoo.mAir_flow)
    annotation(
      Line(
        points={{-90,-49}, {-90,-49}, {-90, 64}, {-12, 64}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
      ...
      graphics={Rectangle(
        ...
        points={{-114, 60}, {-68, 60}, {-84, 70}},
        color={0, 0, 255}), Line(
        points={{-68, 60}, {-84, 52}},
        color={0, 0, 255}), Line(
        points={{114,-60}, {70,-60}, {84,-52}},
        color={0, 127, 127}), Line(
        points={{70,-60}, {82,-68}},
        color={0, 127, 127})}),
