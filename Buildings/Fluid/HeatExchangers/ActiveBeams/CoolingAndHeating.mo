...
model CoolingAndHeating
  ...
  extends Buildings.Fluid.HeatExchangers.ActiveBeams.Cooling(
    sum(
      nin=2));
  ...
  Modelica.SIunits.PressureDifference dpWatHea(
    displayUnit="Pa")=watHea_a.p-watHea_b.p
...
equation
  ...
  connect(conHea.Q_flow, sum.u[2])
    annotation(
      Line(
        points={{11, 7}, {20, 7}, {20, 30}, {38, 30}},
        color={0, 0, 127}));
  connect(conHea.TRoo, senTemRooAir.T)
    annotation(
      Line(
        points={{-12,-6}, {-26,-6}, {-50,-6}, {-50,-40}, {-40,-40}},
        color={0, 0, 127}));
  connect(watHea_a, conHea.port_a)
    annotation(
      Line(
        points={{-140, 0}, {-76, 0}, {-10, 0}},
        color={0, 127, 255}));
  connect(conHea.mAir_flow, senFloAir.m_flow)
    annotation(
      Line(
        points={{-12, 4}, {-46, 4}, {-90, 4}, {-90,-49}},
        color={0, 0, 127}));
  annotation(
    Diagram(
      coordinateSystem(
    ...
    defaultComponentName="beaCooHea",
    Icon(
      coordinateSystem(
        extent={{-140,-120}, {140, 120}}),
      graphics={Rectangle(
