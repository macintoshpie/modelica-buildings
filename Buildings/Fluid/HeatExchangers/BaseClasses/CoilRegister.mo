...
model CoilRegister
  ...
  parameter Boolean allowFlowReversal2=true
    ...
    annotation(
      Dialog(
        tab="Assumptions"),
      Evaluate=true);
  replaceable Buildings.Fluid.HeatExchangers.BaseClasses.HexElementSensible ele[nPipPar, nPipSeg] constrainedby Buildings.Fluid.HeatExchangers.BaseClasses.PartialHexElement(
  ...
  Modelica.Fluid.Interfaces.FluidPort_a[nPipPar] port_a1(
    ...
    each m_flow(
      start=0,
      min=if allowFlowReversal1 then-Constants.inf else 0))
  ...
  Modelica.Fluid.Interfaces.FluidPort_b[nPipPar] port_b1(
    ...
    each m_flow(
      start=0,
      max=if allowFlowReversal1 then + Constants.inf else 0))
  ...
  Modelica.Fluid.Interfaces.FluidPort_a[nPipPar, nPipSeg] port_a2(
    ...
    each m_flow(
      start=0,
      min=if allowFlowReversal2 then-Constants.inf else 0))
  ...
  Modelica.Fluid.Interfaces.FluidPort_b[nPipPar, nPipSeg] port_b2(
    ...
    each m_flow(
      start=0,
      max=if allowFlowReversal2 then + Constants.inf else 0))
...
equation
  ...
  for i in 1 : nPipPar loop
    ...
    for j in 1 : nPipSeg loop
      connect(ele[i, j].port_a2, port_a2[i, j])
        annotation(
          Line(
            points={{10,-6}, {40,-6}, {40,-60}, {100,-60}},
            color={0, 127, 255}));
      connect(ele[i, j].port_b2, port_b2[i, j])
        annotation(
          Line(
            points={{-10,-6}, {-68,-6}, {-68,-62}, {-100,-62}},
            color={0, 127, 255}));
    end for;
  end for;
  connect(Gc_1, gai_1.u)
    annotation(
      Line(
        points={{-40, 100}, {-40, 25}, {-35.2, 25}},
        color={0, 0, 127}));
  connect(Gc_2, gai_2.u)
    annotation(
      Line(
        points={{40,-100}, {40,-69}, {37.2,-69}},
        color={0, 0, 127}));
  for i in 1 : nPipPar loop
    for j in 1 : nPipSeg loop
      connect(gai_1.y, ele[i, j].Gc_1)
        annotation(
          Line(
            points={{-21.4, 25}, {-4, 25}, {-4, 10}},
            color={0, 0, 127}));
      connect(gai_2.y, ele[i, j].Gc_2)
        annotation(
          Line(
            points={{23.4,-69}, {20,-69}, {20,-68}, {20,-18}, {4,-18}, {4,-10}},
            color={0, 0, 127}));
    end for;
  end for;
