...
model DuctManifoldFixedResistance
  ...
  Buildings.Fluid.FixedResistances.PressureDrop fixRes(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, dp_nominal=dp_nominal, from_dp=from_dp, deltaM=deltaM, linearized=linearized)
    "Fixed resistance for each duct"
    annotation(Placement(transformation(extent={{-40,-10}, {-20, 10}})));
protected
  DuctManifoldFlowDistributor floDis(redeclare package Medium=Medium, nPipPar=nPipPar, mStart_flow_a=mStart_flow_a, nPipSeg=nPipSeg, allowFlowReversal=allowFlowReversal, m_flow_nominal=m_flow_nominal)
    "Mass flow distributor to the individual segments of the coil"
    annotation(Placement(transformation(extent={{40,-10}, {60, 10}})));
equation
  connect(fixRes.port_a, port_a)
    annotation(Line(points={{-40, 0}, {-100, 0}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(fixRes.port_b, floDis.port_a)
    annotation(Line(points={{-20, 0}, {40, 0}}, color={0, 127, 255}, smooth=Smooth.None));
...
</html>"), Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 100}}), graphics={Rectangle(extent={{28, 68}, {72, 52}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 0}, fillPattern=FillPattern.Solid), Rectangle(extent={{30, 8}, {74,-8}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 0}, fillPattern=FillPattern.Solid), Rectangle(extent={{30,-52}, {74,-68}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 0}, fillPattern=FillPattern.Solid)}));
