  ...
  Buildings.Utilities.Psychrometrics.X_pW humRatPre(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10, 60}, {10, 80}})));
  Modelica.Blocks.Sources.Constant TWat(
    k=273.15 + 5)
  ...
  Buildings.Fluid.HeatExchangers.PrescribedOutlet hea(
    ...
    annotation(
      Placement(
        transformation(
          extent={{60,-82}, {80,-62}})));
  Modelica.Blocks.Sources.Constant TAir(
    k=273.15 + 20)
    ...
    annotation(
      Placement(
        transformation(
          extent={{20,-50}, {40,-30}})));
  Modelica.Blocks.Sources.RealExpression TSur(
    y=hex.masExc.TSur)
...
equation
  ...
  connect(hea.port_b, hex.port_a2)
    annotation(
      Line(
        points={{80,-72}, {88,-72}, {88,-4}, {30,-4}},
        color={0, 127, 255}));
  connect(TAir.y, hea.TSet)
    annotation(
      Line(
        points={{41,-40}, {48,-40}, {48,-64}, {58,-64}},
        color={0, 0, 127}));
  connect(TSur.y, TDewPoi.T)
    annotation(
      Line(
        points={{-59, 70}, {-41, 70}, {-41, 70}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=50),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/BaseClasses/Examples/HexElementLatentLoop.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
