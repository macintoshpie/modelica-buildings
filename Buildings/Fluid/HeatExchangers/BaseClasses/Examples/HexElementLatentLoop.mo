...
model HexElementLatentLoop
  ...
  parameter Modelica.SIunits.MassFlowRate mA_flow_nominal=0.14
    "Air mass flow rate";
  Buildings.Fluid.Sources.Boundary_pT sin_2(redeclare package Medium=Medium_A, use_p_in=false, use_T_in=false, nPorts=1)
    "Pressure boundary condition"
    annotation(Placement(transformation(extent={{-40,-82}, {-20,-62}})));
  Buildings.Fluid.Sources.Boundary_pT sin_1(redeclare package Medium=Medium_W, nPorts=1)
    "Pressure boundary condition"
    annotation(Placement(transformation(extent={{80,-2}, {60, 18}})));
  Sources.MassFlowSource_T sou(redeclare package Medium=Medium_W, m_flow=5, nPorts=1, use_T_in=true)
    "Mass flow source"
    annotation(Placement(transformation(extent={{-40,-2}, {-20, 18}})));
  Buildings.Fluid.HeatExchangers.BaseClasses.HexElementLatent hex(redeclare package Medium1=Medium_W, redeclare package Medium2=Medium_A, dp2_nominal=5, UA_nominal=1E3, m1_flow_nominal=mW_flow_nominal, m2_flow_nominal=mA_flow_nominal, dp1_nominal=0, show_T=true, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState)
    "Hex element"
    annotation(Placement(transformation(extent={{10,-8}, {30, 12}})));
  Modelica.Blocks.Sources.Constant hACon(k=70)
    "Convective heat transfer"
    annotation(Placement(transformation(extent={{-20, 20}, {0, 40}})));
  Movers.FlowControlled_m_flow fan(redeclare package Medium=Medium_A, m_flow_nominal=mA_flow_nominal, inputType=Buildings.Fluid.Types.InputType.Constant, addPowerToMedium=false, nominalValuesDefineDefaultPressureCurve=true, dp_nominal=5, use_inputFilter=false, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
    "Fan"
...
public
  ...
  Buildings.Fluid.HeatExchangers.PrescribedOutlet hea(redeclare package Medium=Medium_A, m_flow_nominal=mA_flow_nominal, dp_nominal=0, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, use_X_wSet=false)
    "Heater"
...
equation
  connect(hACon.y, hex.Gc_1)
    annotation(Line(points={{1, 30}, {16, 30}, {16, 12}}, color={0, 0, 127}));
  connect(hACon.y, hex.Gc_2)
    annotation(Line(points={{1, 30}, {8, 30}, {8,-16}, {24,-16}, {24,-8}}, color={0, 0, 127}));
  connect(sou.ports[1], hex.port_a1)
    annotation(Line(points={{-20, 8}, {-16, 8}, {10, 8}}, color={0, 127, 255}));
  connect(sin_1.ports[1], hex.port_b1)
    annotation(Line(points={{60, 8}, {46, 8}, {30, 8}}, color={0, 127, 255}));
  connect(fan.port_a, hex.port_b2)
    annotation(Line(points={{10,-72}, {-10,-72}, {-10,-4}, {10,-4}}, color={0, 127, 255}));
  connect(sin_2.ports[1], fan.port_a)
    annotation(Line(points={{-20,-72}, {-14,-72}, {10,-72}}, color={0, 127, 255}));
  ...
  connect(hea.port_b, hex.port_a2)
    annotation(Line(points={{80,-72}, {88,-72}, {88,-4}, {30,-4}}, color={0, 127, 255}));
  connect(TAir.y, hea.TSet)
    annotation(Line(points={{41,-40}, {48,-40}, {48,-64}, {58,-64}}, color={0, 0, 127}));
  connect(TSur.y, TDewPoi.T)
    annotation(Line(points={{-59, 70}, {-41, 70}, {-41, 70}}, color={0, 0, 127}));
  annotation(experiment(Tolerance=1e-6, StopTime=50), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/BaseClasses/Examples/HexElementLatentLoop.mos"
