...
model HANaturalCylinder
  ...
  parameter Modelica.SIunits.ThermalConductance hA_nominal(
    min=0)
  ...
  Modelica.Blocks.Interfaces.RealOutput hA(
    unit="W/K")
...
protected
  parameter Real Gr_nominal(
    fixed=false)
    "Grashof number";
  parameter Real B_nominal(
    unit="1/K",
    fixed=false)
    "isobaricExpansionCoefficient";
  parameter Real nu_nominal(
    unit="m2/s",
    fixed=false)
  ...
  parameter Modelica.SIunits.ThermalConductivity k_nominal(
    fixed=false)
    "Thermal conductivity of the fluid";
  parameter Real Ra_nominal(
    fixed=false)
    "Rayleigh number";
  parameter Real Pr_nominal(
    fixed=false)
    "Prandlt number";
  parameter Real Nusselt_nominal(
    fixed=false)
    "Nusselt number";
  parameter Real h_nominal(
    unit="W/(m2.K)",
    fixed=false)
    "Convection coefficient";
...
initial equation
  ...
  rho_nominal=Medium.density(Medium.setState_pTX(
    p=Medium.p_default,
    T=0.5*(TSur_nominal + TFlu_nominal),
    X=Medium.X_default));
  ...
  // Convection coefficient
  k_nominal=Medium.thermalConductivity(Medium.setState_pTX(
    p=Medium.p_default,
    T=0.5*(TFlu_nominal + TSur_nominal),
    X=Medium.X_default));
  Nusselt_nominal=nusselt(
    k=k_nominal,
    Pr=Pr_nominal,
    Ra=Ra_nominal);
