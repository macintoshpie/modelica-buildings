...
algorithm
  ...
  elseif(flowRegime == Integer(f.CounterFlow)) then
    ...
    a := smooth(1, if Z < 0.97 then Z else Buildings.Utilities.Math.Functions.smoothMin(
      x1=Z,
      x2=0.98,
      deltaX=0.01));
  ...
  elseif(flowRegime == Integer(f.CrossFlowUnmixed)) then
    ...
    NTU := Modelica.Math.Nonlinear.solveOneNonlinearEquation(
      f=function epsilon_ntuZ_crossFlowUnmixed(
        eps=eps,
        Z=Z),
      u_min=1.5*1e-10,
      u_max=1e6);
  elseif(flowRegime == Integer(f.CrossFlowCMinUnmixedCMaxMixed)) then
    ...
    a := smooth(1, if Z > 0.03 then Z else Buildings.Utilities.Math.Functions.smoothMin(
      x1=0.02,
      x2=Z,
      deltaX=0.01));
