...
function normalizedPower
  ...
  output Modelica.SIunits.Efficiency r_P(max=1)
    "Normalized power consumption";
protected
  Integer n=size(per.r_V, 1)
...
algorithm
  if n == 1 then
    ...
    r_P := Buildings.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(x=r_V, x1=per.r_V[i], x2=per.r_V[i + 1], y1=per.r_P[i], y2=per.r_P[i + 1], y1d=d[i], y2d=d[i + 1]);
  end if;
