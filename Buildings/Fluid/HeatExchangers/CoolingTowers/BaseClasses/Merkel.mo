...
block Merkel
  ...
  final parameter Modelica.SIunits.HeatFlowRate Q_flow_nominal(
    max=0)=-m_flow_nominal*cpWat_nominal*(TWatIn_nominal-TWatOut_nominal)
  ...
  final parameter Real NTU_nominal(
    min=0)=Buildings.Fluid.HeatExchangers.BaseClasses.ntu_epsilonZ(
  ...
  Modelica.SIunits.ThermalConductance UAe(
    min=0)
...
protected
  ...
  final parameter Medium.ThermodynamicState staWat_default=Medium.setState_pTX(
    T=TWatIn_nominal,
    p=Medium.p_default,
...
equation
  ...
  cpEqu=Buildings.Fluid.HeatExchangers.CoolingTowers.BaseClasses.Functions.equivalentHeatCapacity(
    TIn=TAir,
    TOut=TAirOut);
