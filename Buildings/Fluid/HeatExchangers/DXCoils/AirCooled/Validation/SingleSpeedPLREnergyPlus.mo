...
model SingleSpeedPLREnergyPlus
  ...
  Buildings.Fluid.Sources.Boundary_pT sin(redeclare package Medium=Medium, p(displayUnit="Pa")=101325, nPorts=1, T=303.15)
    "Sink"
    annotation(Placement(transformation(extent={{40,-20}, {20, 0}})));
  Buildings.Fluid.Sources.Boundary_pT sou(redeclare package Medium=Medium, p(displayUnit="Pa")=101325 + dp_nominal, use_T_in=true, nPorts=1, use_p_in=true, use_X_in=true, T=299.85)
    "Source"
    annotation(Placement(transformation(extent={{-40,-20}, {-20, 0}})));
  Buildings.Fluid.HeatExchangers.DXCoils.AirCooled.SingleSpeed sinSpeDX(redeclare package Medium=Medium, dp_nominal=dp_nominal, datCoi=datCoi, T_start=datCoi.sta[1].nomVal.TEvaIn_nominal, from_dp=true, computeReevaporation=true, eva(m(start=0)), energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
  ...
  Modelica.Blocks.Sources.Pulse p(nperiod=1, offset=101325, width=100, period=36000, startTime=25200, amplitude=1086)
    "Pressure"
  ...
  Modelica.Blocks.Sources.BooleanTable onOff(startValue=true, table={0, 25200, 26223.8694421458, 28800, 30457.5364262964, 32400, 34157.5093643325, 36000, 37915.2504828207, 39600, 41687.2390146012, 43200, 45363.3505825759, 46800, 49129.0477925565, 50400, 52764.5288886473, 54000, 56361.5689116996, 57600, 59868.3223307805})
  ...
  Modelica.Blocks.Sources.TimeTable TOutEPlu_ori(table=[
    0, 29.34948133;
    3600, 29.34948133;
    3600, 29.01814876;
    7200, 29.01814876;
  ...
  Modelica.Blocks.Sources.TimeTable PEPlu_ori(table=[
    0, 0;
    3600, 0;
    3600, 0;
    7200, 0;
  ...
  Modelica.Blocks.Sources.TimeTable XEvaOutEPlu_ori(table=[
    0, 0.009460927;
    3600, 0.009460927;
    3600, 0.009460927;
    7200, 0.009460927;
  ...
  Modelica.Blocks.Sources.TimeTable Q_flowSenEPlu_ori(table=[
    0, 0;
    3600, 0;
    3600, 0;
    7200, 0;
  ...
  Modelica.Blocks.Sources.TimeTable Q_flowEPlu_ori(table=[
    0, 0;
    3600, 0;
    3600, 0;
    7200, 0;
  ...
  Data.Generic.DXCoil datCoi(nSta=1, sta={Data.Generic.BaseClasses.Stage(spe=1800/60, nomVal=Data.Generic.BaseClasses.NominalValues(Q_flow_nominal=-25237.66, COP_nominal=3, SHR_nominal=0.775047, m_flow_nominal=1.72, tWet=1000, gamma=1.5), perCur=Examples.PerformanceCurves.Curve_II())})
    "Coil data"
...
protected
  block UnitDelay
    extends Modelica.Blocks.Discrete.UnitDelay(firstTrigger(start=false, fixed=true));
  end UnitDelay;
equation
  connect(sou.ports[1], sinSpeDX.port_a)
    annotation(Line(points={{-20,-10}, {-16,-10}, {-16, 10}, {-10, 10}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(sinSpeDX.port_b, sin.ports[1])
    annotation(Line(points={{10, 10}, {16, 10}, {16,-10}, {20,-10}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(mux.y, sou.X_in)
    annotation(Line(points={{-59,-50}, {-52,-50}, {-52,-14}, {-42,-14}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TCIn_K.Kelvin, sinSpeDX.TConIn)
    annotation(Line(points={{-79, 69.8}, {-66.5, 69.8}, {-66.5, 13}, {-11, 13}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TEvaIn_K.Kelvin, sou.T_in)
    annotation(Line(points={{-79,-10.2}, {-51.7,-10.2}, {-51.7,-6}, {-42,-6}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(add.y, mux.u2[1])
    annotation(Line(points={{-79,-90}, {-68,-90}, {-68,-68}, {-96,-68}, {-96,-56}, {-82,-56}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(sinSpeDX.P, PMea.u)
    annotation(Line(points={{11, 19}, {30, 19}, {30, 50}, {38, 50}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(p.y, sou.p_in)
    annotation(Line(points={{-119, 30}, {-74, 30}, {-74,-2}, {-42,-2}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(onOff.y, sinSpeDX.on)
    annotation(Line(points={{-99, 110}, {-54, 110}, {-54, 18}, {-11, 18}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(TCIn.y, TCIn_K.Celsius)
    annotation(Line(points={{-119, 70}, {-110.5, 70}, {-110.5, 69.6}, {-102, 69.6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TEvaIn.y, TEvaIn_K.Celsius)
    annotation(Line(points={{-119,-10}, {-110.5,-10}, {-110.5,-10.4}, {-102,-10.4}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(sinSpeDX.QSen_flow, Q_flowSenMea.u)
    annotation(Line(points={{11, 17}, {20, 17}, {20, 60}, {-12, 60}, {-12, 130}, {-2, 130}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(QCoo_flow.u1, sinSpeDX.QSen_flow)
    annotation(Line(points={{-42, 96}, {-50, 96}, {-50, 30}, {18, 30}, {18, 17}, {11, 17}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(sinSpeDX.QLat_flow, QCoo_flow.u2)
    annotation(Line(points={{11, 15}, {20, 15}, {20, 32}, {-48, 32}, {-48, 84}, {-42, 84}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(PEPlu_ori.y, PEPlu.u)
    annotation(Line(points={{-79,-130}, {-70,-130}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(Q_flowSenEPlu_ori.y, Q_flowSenEPlu.u)
    annotation(Line(points={{61,-50}, {78,-50}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(Q_flowSenEPlu.y, shrEPlu.u1)
    annotation(Line(points={{101,-50}, {108,-50}, {108,-84}, {138,-84}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(Q_flowEPlu_ori.y, Q_flowEPlu.u)
    annotation(Line(points={{61,-130}, {78,-130}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TOutEPlu_ori.y, TOutEPlu.u)
    annotation(Line(points={{-19,-70}, {-2,-70}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(XEvaOutEPlu_ori.y, XEvaOutEPlu.u)
    annotation(Line(points={{-19,-130}, {-2,-130}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(multiSum.y, shrEPlu.u2)
    annotation(Line(points={{131.02,-114}, {134,-114}, {134,-96}, {138,-96}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(small.y, multiSum.u[1])
    annotation(Line(points={{101,-90}, {110,-90}, {110,-111.9}, {118,-111.9}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(Q_flowEPlu.y, multiSum.u[2])
    annotation(Line(points={{101,-130}, {108,-130}, {108,-116.1}, {118,-116.1}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=true, extent={{-160,-140}, {160, 140}})), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/DXCoils/AirCooled/Validation/SingleSpeedPLREnergyPlus.mos"
