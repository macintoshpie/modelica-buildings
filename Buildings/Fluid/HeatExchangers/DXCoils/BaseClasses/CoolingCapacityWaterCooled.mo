...
block CoolingCapacityWaterCooled
  ...
  extends Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.PartialCoolingCapacity(final use_mCon_flow=true, redeclare Buildings.Fluid.HeatExchangers.DXCoils.WaterCooled.Data.Generic.BaseClasses.Stage sta[nSta]);
protected
...
initial algorithm
  ...
  for iSta in 1 : nSta loop
    Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.Functions.warnIfPerformanceOutOfBounds(Buildings.Fluid.Utilities.extendedPolynomial(x=1, c=sta[iSta].perCur.capFunFFCon, xMin=sta[iSta].perCur.ffConMin, xMax=sta[iSta].perCur.ffConMin), msg="Capacity as a function of normalized water mass flow rate at the condenser", curveName="sta[" + String(iSta) + "].perCur.capFunFFCon");
    Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.Functions.warnIfPerformanceOutOfBounds(Buildings.Fluid.Utilities.extendedPolynomial(x=1, c=sta[iSta].perCur.EIRFunFF, xMin=sta[iSta].perCur.ffConMin, xMax=sta[iSta].perCur.ffConMin), msg="EIR as a function of normalized water mass flow rate at the condenser", curveName="sta[" + String(iSta) + "].perCur.EIRFunFFCon");
  end for;
equation
  ...
  if stage > 0 then
    for iSta in 1 : nSta loop
      // Compute performance
      ffCon[iSta]=Buildings.Utilities.Math.Functions.smoothMax(x1=mCon_flow_internal, x2=m_flow_small, deltaX=m_flow_small/10)/sta[iSta].nomVal.mCon_flow_nominal;
      //-------------------------Cooling capacity modifiers----------------------------//
      cap_FFCon[iSta]=Buildings.Fluid.Utilities.extendedPolynomial(x=ff[iSta], c=sta[iSta].perCur.capFunFFCon, xMin=sta[iSta].perCur.ffConMin, xMax=sta[iSta].perCur.ffConMin);
      //-----------------------Energy Input Ratio modifiers--------------------------//
      EIR_FFCon[iSta]=Buildings.Fluid.Utilities.extendedPolynomial(x=ff[iSta], c=sta[iSta].perCur.EIRFunFFCon, xMin=sta[iSta].perCur.ffConMin, xMax=sta[iSta].perCur.ffConMin)
      ...
      corFacCon[iSta]=Buildings.Utilities.Math.Functions.smoothHeaviside(x=ffCon[iSta]-sta[iSta].perCur.ffConMin/4, delta=max(Modelica.Constants.eps, sta[iSta].perCur.ffConMin/4));
