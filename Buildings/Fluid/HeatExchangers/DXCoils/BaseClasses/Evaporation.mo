...
model Evaporation
  ...
  parameter Buildings.Fluid.HeatExchangers.DXCoils.AirCooled.Data.Generic.BaseClasses.NominalValues nomVal
    "Nominal values"
    annotation(Placement(transformation(extent={{60, 60}, {80, 80}})));
  parameter Modelica.SIunits.MassFlowRate mAir_flow_small(min=0)=0.1*abs(nomVal.m_flow_nominal)
  ...
  Modelica.Blocks.Interfaces.RealInput mWat_flow(final quantity="MassFlowRate", final unit="kg/s")
    "Water flow rate added into the medium"
    annotation(Placement(transformation(extent={{-140,-20}, {-100, 20}})));
  Modelica.Blocks.Interfaces.RealInput mAir_flow(final quantity="MassFlowRate", final unit="kg/s")
    "Air mass flow rate"
  ...
  Modelica.Blocks.Interfaces.RealInput TEvaOut(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC")
    "Air temperature"
    annotation(Placement(transformation(extent={{-20,-20}, {20, 20}}, rotation=90, origin={60,-120})));
  Modelica.Blocks.Interfaces.RealOutput mTotWat_flow(final quantity="MassFlowRate", final unit="kg/s")
...
protected
  ...
  final parameter Modelica.SIunits.MassFraction dX_nominal(min=-1, max=0, fixed=false)
  ...
  constant Modelica.SIunits.SpecificHeatCapacity cpAir_nominal=Buildings.Utilities.Psychrometrics.Constants.cpAir
    "Specific heat capacity of air";
  constant Modelica.SIunits.SpecificHeatCapacity cpSte_nominal=Buildings.Utilities.Psychrometrics.Constants.cpSte
    "Specific heat capacity of water vapor";
  constant Modelica.SIunits.SpecificHeatCapacity cpWatLiq_nominal=Buildings.Utilities.Psychrometrics.Constants.cpWatLiq
    "Specific heat capacity of liquid water";
  constant Modelica.SIunits.Temperature T_ref=Buildings.Utilities.Psychrometrics.Constants.T_ref
...
initial equation
  ...
  XEvaIn_nominal=Buildings.Utilities.Psychrometrics.Functions.X_pSatpphi(pSat=Medium.saturationPressure(nomVal.TEvaIn_nominal), p=nomVal.p_nominal, phi=nomVal.phiIn_nominal);
  ...
  // values.
  XEvaWetBulOut_nominal=Buildings.Utilities.Psychrometrics.Functions.X_pSatpphi(pSat=Buildings.Utilities.Psychrometrics.Functions.saturationPressureLiquid(TEvaWetBulOut_nominal), p=nomVal.p_nominal, phi=1);
  XiSatRefOut_nominal=(1-XEvaOut_nominal)*XEvaWetBulOut_nominal/(1-XEvaWetBulOut_nominal);
  (TEvaWetBulOut_nominal-T_ref)*((1-XEvaOut_nominal)*cpAir_nominal + XiSatRefOut_nominal*cpSte_nominal +(XEvaOut_nominal-XiSatRefOut_nominal)*cpWatLiq_nominal)=(TEvaOut_nominal-T_ref)*((1-XEvaOut_nominal)*cpAir_nominal + XEvaOut_nominal*cpSte_nominal) +(XEvaOut_nominal-XiSatRefOut_nominal)*Buildings.Utilities.Psychrometrics.Constants.h_fg;
  ...
  assert(dX_nominal > 1E-10, "*** Warning: In DX coil model, dX_nominal = " + String(dX_nominal) + "
    This means that the coil is not dehumidifying air at the nominal conditions.
    Check nominal parameters.
    " + Buildings.Fluid.HeatExchangers.DXCoils.AirCooled.Data.Generic.BaseClasses.nominalValuesToString(nomVal), AssertionLevel.warning);
  ...
  assert(nomVal.gamma <= gammaMax, "*** Warning: In DX coil model, gamma is too large for these coil conditions.
  ...
  assert(K > 0, "Require K>0 but received " + String(K) + "
   ...
   " + Buildings.Fluid.HeatExchangers.DXCoils.AirCooled.Data.Generic.BaseClasses.nominalValuesToString(nomVal) + "
...
equation
  ...
  if computeReevaporation then
    ...
    if on then
      ...
      XEvaWetBulOut=Buildings.Utilities.Psychrometrics.Functions.X_pSatpphi(pSat=Buildings.Utilities.Psychrometrics.Functions.saturationPressureLiquid(TEvaWetBulOut), p=nomVal.p_nominal, phi=1);
      (TEvaWetBulOut-T_ref)*((1-XEvaOut)*cpAir_nominal + XiSatRefOut*cpSte_nominal +(XEvaOut-XiSatRefOut)*cpWatLiq_nominal)=(TEvaOut-T_ref)*((1-XEvaOut)*cpAir_nominal + XEvaOut*cpSte_nominal) +(XEvaOut-XiSatRefOut)*Buildings.Utilities.Psychrometrics.Constants.h_fg;
      dX=smooth(1, noEvent(Buildings.Utilities.Math.Functions.spliceFunction(pos=XEvaOut, neg=XEvaWetBulOut, x=abs(mAir_flow)-nomVal.m_flow_nominal/2, deltax=nomVal.m_flow_nominal/3)))-XEvaWetBulOut;
      mEva_flow=-dX*smooth(1, noEvent(Buildings.Utilities.Math.Functions.spliceFunction(pos=if abs(mAir_flow) > mAir_flow_small/3 then abs(mAir_flow)*(1-Modelica.Math.exp(-K2*m*abs(mAir_flow)^(-0.2))) else 0, neg=K2*mAir_flow_small^(-0.2)*m*mAir_flow^2, x=abs(mAir_flow)-2*mAir_flow_small/3, deltax=2*mAir_flow_small/6)));
      der(m)=-mEva_flow;
      mTotWat_flow=mWat_flow + mEva_flow;
    end if;
  else
    // The model is configured to not compute reevaporation
    dX=0;
    mEva_flow=0;
    mTotWat_flow=mWat_flow;
    m=0;
  ...
  annotation(defaultComponentName="eva", Documentation(info="<html>
<p>
...
</html>"), Icon(graphics={Rectangle(extent={{-96, 94}, {96,-98}}, fillPattern=FillPattern.Sphere, pattern=LinePattern.None, lineColor={255, 255, 255}, fillColor={170, 213, 255})}));
end Evaporation;
