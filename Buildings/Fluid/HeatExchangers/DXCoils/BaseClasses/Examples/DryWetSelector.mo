...
model DryWetSelector
  ...
  Modelica.Blocks.Sources.Ramp XADP(duration=20, startTime=20, height=-0.002, offset=0.006)
    "Mass fraction at ADP"
    annotation(Placement(transformation(extent={{-90, 0}, {-70, 20}})));
  Modelica.Blocks.Sources.Ramp XEvaIn(startTime=20, height=0.002, offset=0.004, duration=20)
    "Inlet mass-fraction"
...
equation
  connect(XEvaIn.y, dryWet.XEvaIn)
    annotation(Line(points={{-39,-10}, {-28,-10}, {-28,-4}, {-1,-4}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(XADP.y, dryWet.XADP)
    annotation(Line(points={{-69, 10}, {-10, 10}, {-10, 4}, {-1, 4}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(mWetWat_flow.y, dryWet.mWetWat_flow)
    annotation(Line(points={{-67, 40}, {2, 40}, {2, 11}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(SHRWet.y, dryWet.SHRWet)
    annotation(Line(points={{-13, 72}, {10, 72}, {10, 11}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(QWet_flow.y, dryWet.QWet_flow)
    annotation(Line(points={{11, 90}, {14, 90}, {14, 11}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(EIRWet.y, dryWet.EIRWet)
    annotation(Line(points={{41, 90}, {52, 90}, {52, 40}, {18, 40}, {18, 11}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(QDry_flow.y, dryWet.QDry_flow)
    annotation(Line(points={{-27,-60}, {14,-60}, {14,-11}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(EIRDry.y, dryWet.EIRDry)
    annotation(Line(points={{1,-80}, {18,-80}, {18,-11}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(experiment(Tolerance=1e-6, StopTime=60), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/DXCoils/BaseClasses/Examples/DryWetSelector.mos"
