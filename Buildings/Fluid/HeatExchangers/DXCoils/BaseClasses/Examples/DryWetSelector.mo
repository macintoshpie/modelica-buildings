  ...
  Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.DryWetSelector dryWet
    ...
    annotation(
      Placement(
        transformation(
          extent={{0,-10}, {20, 10}})));
  Modelica.Blocks.Sources.Constant mWetWat_flow(
    k=0.01)
    annotation(
      Placement(
        transformation(
          extent={{-88, 30}, {-68, 50}})));
  Modelica.Blocks.Sources.Constant SHRWet(
    k=0.5)
    annotation(
      Placement(
        transformation(
          extent={{-34, 62}, {-14, 82}})));
  Modelica.Blocks.Sources.Constant QWet_flow(
    k=-3000)
    annotation(
      Placement(
        transformation(
          extent={{-10, 80}, {10, 100}})));
  Modelica.Blocks.Sources.Constant EIRWet(
    k=0.3)
    annotation(
      Placement(
        transformation(
          extent={{20, 80}, {40, 100}})));
  Modelica.Blocks.Sources.Constant QDry_flow(
    k=-2000)
    annotation(
      Placement(
        transformation(
          extent={{-48,-70}, {-28,-50}})));
  Modelica.Blocks.Sources.Constant EIRDry(
    k=0.2)
    annotation(
      Placement(
        transformation(
...
equation
  ...
  annotation(
    experiment(
      ...
      StopTime=60),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/DXCoils/BaseClasses/Examples/DryWetSelector.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
