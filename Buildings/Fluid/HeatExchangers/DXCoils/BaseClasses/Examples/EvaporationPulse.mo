...
model EvaporationPulse
  ...
  parameter Modelica.SIunits.MassFraction XEvaIn_nominal=Buildings.Utilities.Psychrometrics.Functions.X_pSatpphi(
  ...
  Modelica.Blocks.Sources.Pulse pulSho(
    period=30*60)
  ...
  Modelica.Blocks.Sources.Constant mAir_flow(
    k=nomVal.m_flow_nominal)
    "Air flow rate"
    annotation(
      Placement(
        transformation(
          extent={{-60, 80}, {-40, 100}})));
  Modelica.Blocks.Sources.Constant TIn(
    k=nomVal.TEvaIn_nominal)
    "Inlet temperature"
    annotation(
      Placement(
        transformation(
  ...
  Modelica.Blocks.Continuous.Integrator intNor
    ...
    annotation(
      Placement(
        transformation(
          extent={{80, 0}, {100, 20}})));
  Modelica.Blocks.Sources.Pulse pulNor(
    period=3600)
  ...
  Modelica.Blocks.Math.Gain mWat_flow(
    k=nomVal.Q_flow_nominal*(1-nomVal.SHR_nominal)/Medium.enthalpyOfVaporization(293.15))
    "Water mass flow rate from air to coil surface"
    annotation(
      Placement(
        transformation(
          extent={{-60,-50}, {-40,-30}})));
  Modelica.Blocks.Math.Gain mWat_flow1(
    k=nomVal.Q_flow_nominal*(1-nomVal.SHR_nominal)/Medium.enthalpyOfVaporization(293.15))
    "Water mass flow rate from air to coil surface"
    annotation(
      Placement(
...
equation
  ...
  annotation(
    ...
    experiment(
      ...
      StopTime=7200),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/DXCoils/BaseClasses/Examples/EvaporationPulse.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
