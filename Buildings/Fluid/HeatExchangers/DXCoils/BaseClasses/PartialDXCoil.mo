...
partial model PartialDXCoil
  ...
  extends Buildings.Fluid.Interfaces.TwoPortHeatMassExchanger(redeclare replaceable package Medium=Modelica.Media.Interfaces.PartialCondensingGases, redeclare final Buildings.Fluid.MixingVolumes.MixingVolumeMoistAir vol(prescribedHeatFlowRate=true), final m_flow_nominal=datCoi.sta[nSta].nomVal.m_flow_nominal);
  ...
  Modelica.Blocks.Interfaces.RealInput TConIn(unit="K", displayUnit="degC")
  ...
  Modelica.Blocks.Interfaces.RealInput mCon_flow(quantity="MassFlowRate", unit="kg/s") if use_mCon_flow
  ...
  Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.DXCooling dxCoo(redeclare final package Medium=Medium, datCoi=datCoi, use_mCon_flow=use_mCon_flow, wetCoi(redeclare replaceable Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.CoolingCapacityAirCooled cooCap), dryCoi(redeclare replaceable Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.CoolingCapacityAirCooled cooCap))
    "DX cooling coil operation"
    annotation(Placement(transformation(extent={{-20, 40}, {0, 60}})));
  Evaporation eva(redeclare final package Medium=Medium, nomVal=datCoi.sta[nSta].nomVal, final computeReevaporation=computeReevaporation)
...
protected
  parameter Integer i_x(fixed=false)
    "Index of substance";
  Modelica.SIunits.SpecificEnthalpy hEvaIn=inStream(port_a.h_outflow)
...
initial algorithm
  ...
  for i in 1 :(nSta-1) loop
    assert(datCoi.sta[i].nomVal.Q_flow_nominal >= datCoi.sta[nSta].nomVal.Q_flow_nominal, "Error in DX coil performance data: Q_flow_nominal of the highest stage must have
    the biggest value in magnitude. Obtained " + Modelica.Math.Vectors.toString({datCoi.sta[i].nomVal.Q_flow_nominal for i in 1 : nSta}, "Q_flow_nominal"));
  end for;
  // Compute index of species vector that carries the substance name
  i_x :=-1;
  for i in 1 : Medium.nXi loop
    if Modelica.Utilities.Strings.isEqual(string1=Medium.substanceNames[i], string2=substanceName, caseSensitive=false) then
      i_x := i;
    end if;
  end for;
...
equation
  connect(TConIn, dxCoo.TConIn)
    annotation(Line(points={{-110, 30}, {-94, 30}, {-94, 55}, {-21, 55}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(m.y, dxCoo.m_flow)
    annotation(Line(points={{-69, 44}, {-66, 44}, {-66, 52.4}, {-21, 52.4}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(T.y, dxCoo.TEvaIn)
    annotation(Line(points={{-69, 28}, {-62, 28}, {-62, 50}, {-21, 50}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(p.y, dxCoo.p)
    annotation(Line(points={{-69, 14}, {-58, 14}, {-58, 47.6}, {-21, 47.6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(X.y, dxCoo.XEvaIn)
    annotation(Line(points={{-35, 36}, {-30, 36}, {-30, 45}, {-21, 45}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(h.y, dxCoo.hEvaIn)
    annotation(Line(points={{-35, 22}, {-26, 22}, {-26, 42.3}, {-21, 42.3}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(dxCoo.EIR, pwr.EIR)
    annotation(Line(points={{1, 58}, {6, 58}, {6, 76}, {18, 76}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(dxCoo.Q_flow, pwr.Q_flow)
    annotation(Line(points={{1, 54}, {10, 54}, {10, 70}, {18, 70}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(dxCoo.SHR, pwr.SHR)
    annotation(Line(points={{1, 50}, {14, 50}, {14, 64}, {18, 64}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(q.port, vol.heatPort)
    annotation(Line(points={{62, 54}, {66, 54}, {66, 22}, {-12, 22}, {-12,-10}, {-9,-10}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(pwr.P, P)
    annotation(Line(points={{41, 76}, {50.5, 76}, {50.5, 90}, {110, 90}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(dxCoo.Q_flow, q.Q_flow)
    annotation(Line(points={{1, 54}, {42, 54}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(eva.mWat_flow, dxCoo.mWat_flow)
    annotation(Line(points={{-10,-70}, {-18,-70}, {-18, 8}, {8, 8}, {8, 42}, {1, 42}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(m.y, eva.mAir_flow)
    annotation(Line(points={{-69, 44}, {-66, 44}, {-66,-76}, {-10,-76}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TVol.port, q.port)
    annotation(Line(points={{66, 22}, {66, 54}, {62, 54}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(pwr.QSen_flow, QSen_flow)
    annotation(Line(points={{41, 70}, {110, 70}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(pwr.QLat_flow, QLat_flow)
    annotation(Line(points={{41, 64}, {94, 64}, {94, 50}, {110, 50}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(eva.mTotWat_flow, vol.mWat_flow)
    annotation(Line(points={{13,-70}, {20,-70}, {20,-32}, {-16,-32}, {-16,-18}, {-11,-18}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TVol.T, eva.TEvaOut)
    annotation(Line(points={{78, 22}, {88, 22}, {88,-94}, {8,-94}, {8,-82}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(vol.X_w, eva.XEvaOut)
    annotation(Line(points={{13,-6}, {40,-6}, {40,-90}, {-4,-90}, {-4,-82}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(mCon_flow, dxCoo.mCon_flow)
    annotation(Line(points={{-110,-30}, {-24,-30}, {-24, 40}, {-21, 40}}, color={0, 0, 127}));
...
</html>"), Icon(graphics={Text(extent={{-138, 64}, {-80, 46}}, lineColor={0, 0, 127}, textString="TConIn"), Text(extent={{58, 98}, {102, 78}}, lineColor={0, 0, 127}, textString="P"), Text(extent={{54, 60}, {98, 40}}, lineColor={0, 0, 127}, textString="QLat"), Text(extent={{54, 80}, {98, 60}}, lineColor={0, 0, 127}, textString="QSen")}));
end PartialDXCoil;
