...
partial block PartialSurfaceCondition
  ...
  final parameter Modelica.SIunits.AngularVelocity maxSpe(
    displayUnit="1/min")=datCoi.sta[nSta].spe
  ...
  output Real bypass(
    ...
    max=1.0)
    "Bypass factor";
  output Modelica.SIunits.AngularVelocity spe(
    displayUnit="1/min")
    "Rotational speed";
...
equation
  if variableSpeedCoil or stage <> 0 then
    ...
    spe=speRat*maxSpe;
    UAcp=if variableSpeedCoil then Buildings.Utilities.Math.Functions.smoothMax(
