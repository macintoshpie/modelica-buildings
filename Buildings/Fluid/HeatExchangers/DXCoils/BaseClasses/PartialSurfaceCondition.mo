...
partial block PartialSurfaceCondition
  ...
  Modelica.Blocks.Interfaces.RealInput Q_flow(quantity="Power", unit="W")
  ...
  Modelica.Blocks.Interfaces.RealInput p(quantity="Pressure", unit="Pa")
  ...
  Modelica.Blocks.Interfaces.RealInput hEvaIn(quantity="SpecificEnergy", unit="J/kg")
    "Evaporator air inlet specific enthalpy"
    annotation(Placement(transformation(extent={{-120,-90}, {-100,-70}})));
  Modelica.Blocks.Interfaces.IntegerInput stage
    "Stage of coil, or 0/1 for variable-speed coil"
    annotation(Placement(transformation(extent={{-120, 90}, {-100, 110}}), iconTransformation(extent={{-120, 90}, {-100, 110}})));
  output Real bypass(start=0.25, min=0, max=1.0)
    "Bypass factor";
  output Modelica.SIunits.AngularVelocity spe(displayUnit="1/min")
    "Rotational speed";
  Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.UACp uacp[nSta](per=datCoi.sta.nomVal, redeclare final package Medium=Medium)
...
protected
  ...
  output Modelica.SIunits.SpecificEnthalpy delta_h(start=40000, min=0.0)
    "Enthalpy required to be removed from the inlet air to attain apparatus dry point condition";
  output Real UAcp(min=0, fixed=false)
    "UA/Cp of coil";
equation
  if variableSpeedCoil or stage <> 0 then
    // Small mass flow rate to avoid division by zero
    m_flow_nonzero=Buildings.Utilities.Math.Functions.smoothMax(x1=m_flow, x2=m_flow_small, deltaX=0.1*m_flow_small);
    spe=speRat*maxSpe;
    UAcp=if variableSpeedCoil then Buildings.Utilities.Math.Functions.smoothMax(x1=Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.Functions.speedShift(spe=spe, speSet={datCoi.sta[iSpe].spe for iSpe in 1 : nSta}, u={uacp[iSpe].UAcp for iSpe in 1 : nSta}), x2=uacp[nSta].UAcp/1E3, deltaX=uacp[nSta].UAcp/1E4) else uacp[stage].UAcp;
    bypass=Buildings.Utilities.Math.Functions.smoothLimit(x=Modelica.Math.exp(-UAcp/m_flow_nonzero), l=0.01, u=0.99, deltaX=0.001);
    delta_h=Buildings.Utilities.Math.Functions.smoothMin(x1=-Q_flow/m_flow_nonzero/(1-bypass), x2=0.999*hEvaIn, deltaX=0.0001);
  else
    m_flow_nonzero=0;
    spe=0;
    UAcp=0;
