...
model PartialWaterCooledDXCoil
  ...
  Modelica.Fluid.Interfaces.FluidPort_a port_a(
    ...
    m_flow(
      min=if allowFlowReversalEva then-Modelica.Constants.inf else 0),
    h_outflow(
      start=MediumEva.h_default))
  ...
  Modelica.Fluid.Interfaces.FluidPort_b port_b(
    redeclare final package Medium=MediumEva,
    m_flow(
      max=if allowFlowReversalEva then + Modelica.Constants.inf else 0),
    h_outflow(
      start=MediumEva.h_default))
  ...
  Modelica.Fluid.Interfaces.FluidPort_a portCon_a(
    ...
    m_flow(
      min=if allowFlowReversalCon then-Modelica.Constants.inf else 0),
    h_outflow(
      start=MediumCon.h_default))
  ...
  Modelica.Fluid.Interfaces.FluidPort_b portCon_b(
    redeclare final package Medium=MediumCon,
    m_flow(
      max=if allowFlowReversalCon then + Modelica.Constants.inf else 0),
    h_outflow(
      start=MediumCon.h_default))
    ...
    annotation(
      Placement(
        transformation(
          extent={{-50,-110}, {-70,-90}})));
  // Components
  replaceable Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.PartialDXCoil eva constrainedby Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.PartialDXCoil(
    redeclare final package Medium=MediumEva,
    ...
    dxCoo(
      ...
      wetCoi(
        redeclare final Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.CoolingCapacityWaterCooled cooCap,
        redeclare final Buildings.Fluid.HeatExchangers.DXCoils.WaterCooled.Data.Generic.DXCoil datCoi=datCoi,
        appDewPt(
          redeclare final Buildings.Fluid.HeatExchangers.DXCoils.WaterCooled.Data.Generic.DXCoil datCoi=datCoi,
          uacp(
            redeclare final Buildings.Fluid.HeatExchangers.DXCoils.WaterCooled.Data.Generic.BaseClasses.NominalValues per))),
      dryCoi(
        redeclare final Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses.CoolingCapacityWaterCooled cooCap,
        redeclare final Buildings.Fluid.HeatExchangers.DXCoils.WaterCooled.Data.Generic.DXCoil datCoi=datCoi)),
    eva(
      final nomVal=Buildings.Fluid.HeatExchangers.DXCoils.AirCooled.Data.Generic.BaseClasses.NominalValues(
...
protected
  final parameter MediumEva.MassFlowRate mEva_flow_small(
    min=0)=datCoi.m_flow_small
    "Small mass flow rate for regularization of zero flow at evaporator"
    annotation(
      Dialog(
        tab="Advanced"));
  parameter MediumCon.MassFlowRate mCon_flow_small(
    min=0)=1E-4*abs(datCoi.sta[nSta].nomVal.mCon_flow_nominal)
    "Small mass flow rate for regularization of zero flow at condenser"
    annotation(
      Dialog(
        tab="Advanced"));
  Modelica.Blocks.Sources.RealExpression u(
    final y=(-eva.dxCoo.Q_flow + eva.P)/(-datCoi.sta[nSta].nomVal.Q_flow_nominal*(1 + 1/datCoi.sta[nSta].nomVal.COP_nominal)))
    ...
    annotation(
      Placement(
        transformation(
          extent={{-13,-10}, {13, 10}},
          origin={-29,-60})));
  Sensors.MassFlowRate senMasFloCon(
    redeclare final package Medium=MediumCon)
    ...
    annotation(
      Placement(
        transformation(
          extent={{40,-90}, {20,-70}})));
  Modelica.Blocks.Sources.RealExpression TConEntWat(
    final y=TConEnt)
...
equation
  connect(u.y, watCooCon.u)
    annotation(
      Line(
        points={{-14.7,-60}, {-8,-60}, {-8,-74}, {-18,-74}},
        color={0, 0, 127}));
  ...
  connect(watCooCon.port_b, portCon_b)
    annotation(
      Line(
        points={{-40,-80}, {-60,-80}, {-60,-100}},
        color={0, 127, 255}));
  connect(eva.P, P)
    annotation(
      Line(
        points={{11, 9}, {40, 9}, {40, 90}, {110, 90}},
        color={0, 0, 127}));
  connect(eva.QSen_flow, QEvaSen_flow)
    annotation(
      Line(
        points={{11, 7}, {44, 7}, {44, 60}, {110, 60}},
        color={0, 0, 127}));
  connect(eva.QLat_flow, QEvaLat_flow)
    annotation(
      Line(
        points={{11, 5}, {48, 5}, {48, 30}, {110, 30}},
        color={0, 0, 127}));
  ...
  connect(senMasFloCon.m_flow, eva.mCon_flow)
    annotation(
      Line(
        points={{30,-69}, {30,-69}, {30,-38}, {30,-30}, {-20,-30}, {-20,-3}, {-11,-3}},
        color={0, 0, 127}));
  connect(TConEntWat.y, eva.TConIn)
    annotation(
      Line(
        points={{-39, 11}, {-20, 11}, {-20, 4}, {-20, 4}, {-20, 3}, {-11, 3}},
        color={0, 0, 127}));
  connect(portCon_a, senMasFloCon.port_a)
    annotation(
      Line(
        points={{60,-100}, {60,-100}, {60,-80}, {40,-80}},
        color={0, 127, 255}));
  annotation(
    Icon(
      coordinateSystem(
