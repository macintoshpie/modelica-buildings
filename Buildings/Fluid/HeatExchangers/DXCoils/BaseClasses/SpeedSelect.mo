...
protected
  final parameter Real speRatNor[nSta](each min=0, each max=1)={speSet[i]/speSet[nSta] for i in 1 : nSta}
    "Array of normalized speed ratios for the compressor";
equation
  assert(stage >= 0 and stage <= nSta, "Compressor speed is out of range.
  Model has " + String(nSta) + " speeds, but received control signal " + String(stage));
...
</html>"), Icon(graphics={Line(points={{-86,-64}, {-26,-64}, {-26,-26}, {-10,-26}, {-10, 18}, {14, 18}, {14, 58}, {84, 58}}, color={0, 0, 255}, smooth=Smooth.None)}));
end SpeedSelect;
