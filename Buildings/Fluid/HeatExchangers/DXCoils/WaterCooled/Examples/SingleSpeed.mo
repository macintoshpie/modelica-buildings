...
model SingleSpeed
  ...
  Buildings.Fluid.HeatExchangers.DXCoils.WaterCooled.SingleSpeed sinSpeDX(redeclare package MediumEva=MediumAir, redeclare package MediumCon=MediumWater, datCoi=datCoi, dpEva_nominal=dpEva_nominal, dpCon_nominal=dpCon_nominal, show_T=true, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState)
...
equation
  connect(TEvaIn.y, souAir.T_in)
    annotation(Line(points={{-59, 8}, {-52, 8}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(onOff.y, sinSpeDX.on)
    annotation(Line(points={{-59, 50}, {-20, 50}, {-20, 12}, {-7, 12}}, color={255, 0, 255}, smooth=Smooth.None));
  connect(sinSpeDX.portCon_a, souWat.ports[1])
    annotation(Line(points={{10,-6}, {10,-30}, {32,-30}}, color={0, 127, 255}));
  connect(sinSpeDX.portCon_b, sinWat.ports[1])
    annotation(Line(points={{-2,-6}, {-2,-30}, {-30,-30}}, color={0, 127, 255}));
  connect(souAir.ports[1], sinSpeDX.port_a)
    annotation(Line(points={{-30, 4}, {-6, 4}}, color={0, 127, 255}));
  ...
  connect(mCon_flow.y, souWat.m_flow_in)
    annotation(Line(points={{71,-22}, {52,-22}}, color={0, 0, 127}));
  annotation(__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/DXCoils/WaterCooled/Examples/SingleSpeed.mos"
