...
protected
  ...
  Modelica.Blocks.Math.Gain gai_1(k=1/nEle)
    ...
    annotation(Placement(transformation(extent={{-18, 84}, {-6, 96}})));
  ...
  replaceable model HexElement=BaseClasses.HexElementSensible
    "Model for a heat exchanger element";
  HexElement ele[nEle](redeclare each package Medium1=Medium1, redeclare each package Medium2=Medium2, each allowFlowReversal1=allowFlowReversal1, each allowFlowReversal2=allowFlowReversal2, each tau1=tau1/nEle, each m1_flow_nominal=m1_flow_nominal, each tau2=tau2, each m2_flow_nominal=m2_flow_nominal, each tau_m=tau_m/nEle, each UA_nominal=UA_nominal/nEle, each energyDynamics=energyDynamics, initialize_p1={(i == 1 and(not Medium1.singleState)) for i in 1 : nEle}, initialize_p2={(i == 1 and(not Medium2.singleState)) for i in 1 : nEle}, each deltaM1=deltaM1, each deltaM2=deltaM2, each from_dp1=from_dp1, each from_dp2=from_dp2, dp1_nominal={if i == 1 then dp1_nominal else 0 for i in 1 : nEle}, dp2_nominal={if i == nEle then dp2_nominal else 0 for i in 1 : nEle})
    "Heat exchanger element"
    annotation(Placement(transformation(extent={{0, 0}, {20, 20}})));
  Modelica.Blocks.Routing.Replicator rep1(nout=nEle)
    "Signal replicator"
    annotation(Placement(transformation(extent={{4, 82}, {18, 98}})));
  Modelica.Blocks.Routing.Replicator rep2(nout=nEle)
    "Signal replicator"
  ...
  Modelica.Blocks.Sources.RealExpression THA2(y=if airSideTemperatureDependent then if allowFlowReversal1 then if energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState then temSen_2.T else ele[nEle].vol2.T else Medium2.temperature(state=Medium2.setState_phX(p=port_a2.p, h=inStream(port_a2.h_outflow), X=inStream(port_a2.Xi_outflow))) else Medium2.T_default)
...
equation
  connect(masFloSen_1.m_flow, hA.m1_flow)
    annotation(Line(points={{-74, 66.6}, {-74, 70}, {-84, 70}, {-84, 94}, {-64, 94}, {-64, 87}, {-61, 87}}, color={0, 0, 127}));
  ...
  connect(masFloSen_2.port_b, temSen_2.port_a)
    annotation(Line(points={{70,-60}, {64,-60}, {58,-60}}, color={0, 127, 255}));
  ...
  connect(hA.hA_1, gai_1.u)
    annotation(Line(points={{-39, 87}, {-28, 87}, {-28, 90}, {-19.2, 90}}, color={0, 0, 255}));
  connect(hA.hA_2, gai_2.u)
    annotation(Line(points={{-39, 73}, {-27.5, 73}, {-27.5, 68}, {-19.2, 68}}, color={0, 0, 255}));
  ...
  connect(ele[nEle].port_b1, port_b1)
    annotation(Line(points={{20, 16}, {40, 16}, {40, 60}, {100, 60}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(ele[1].port_b2, port_b2)
    annotation(Line(points={{0, 4}, {-30, 4}, {-30,-60}, {-100,-60}}, color={0, 127, 255}, smooth=Smooth.None));
  for i in 1 : nEle-1 loop
    connect(ele[i].port_b1, ele[i + 1].port_a1)
      annotation(Line(points={{20, 16}, {30, 16}, {30, 28}, {-10, 28}, {-10, 16}, {0, 16}}, color={0, 127, 255}, smooth=Smooth.None));
    connect(ele[i].port_a2, ele[i + 1].port_b2)
      annotation(Line(points={{20, 4}, {-12, 4}, {-12,-10}, {30,-10}, {30, 4}, {0, 4}}, color={0, 127, 255}, smooth=Smooth.None));
    connect(theCon1[i].port_a, ele[i].heaPor1)
      annotation(Line(points={{0, 44}, {-10, 44}, {-10, 32}, {10, 32}, {10, 20}}, color={191, 0, 0}));
    connect(theCon1[i].port_b, ele[i + 1].heaPor1)
      annotation(Line(points={{20, 44}, {28, 44}, {28, 32}, {10, 32}, {10, 20}}, color={191, 0, 0}));
    connect(theCon2[i].port_a, ele[i].heaPor2)
      annotation(Line(points={{20,-30}, {24,-30}, {24,-14}, {10,-14}, {10, 0}}, color={191, 0, 0}));
    connect(theCon2[i].port_b, ele[i + 1].heaPor2)
      annotation(Line(points={{0,-30}, {-8,-30}, {-8,-14}, {10,-14}, {10, 0}}, color={191, 0, 0}));
  end for;
  connect(gai_1.y, rep1.u)
    annotation(Line(points={{-5.4, 90}, {2.6, 90}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(rep1.y, ele.Gc_1)
    annotation(Line(points={{18.7, 90}, {44, 90}, {44, 26}, {6, 26}, {6, 20}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(gai_2.y, rep2.u)
    annotation(Line(points={{-5.4, 68}, {2.6, 68}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(rep2.y, ele.Gc_2)
    annotation(Line(points={{18.7, 68}, {48, 68}, {48,-6}, {14,-6}, {14, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(temSen_1.port_b, ele[1].port_a1)
    annotation(Line(points={{-46, 60}, {-30, 60}, {-30, 16}, {0, 16}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(temSen_2.port_b, ele[nEle].port_a2)
    annotation(Line(points={{44,-60}, {30,-60}, {30, 4}, {20, 4}}, color={0, 127, 255}, smooth=Smooth.None));
