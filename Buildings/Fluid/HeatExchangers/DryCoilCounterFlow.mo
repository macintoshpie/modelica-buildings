...
protected
  ...
  HexElement ele[nEle](
    ...
    annotation(
      Placement(
        transformation(
          extent={{0, 0}, {20, 20}})));
  Modelica.Blocks.Routing.Replicator rep1(
    nout=nEle)
    "Signal replicator"
    annotation(
      Placement(
        transformation(
          extent={{4, 82}, {18, 98}})));
  Modelica.Blocks.Routing.Replicator rep2(
    nout=nEle)
    "Signal replicator"
    annotation(
      Placement(
  ...
  Modelica.Blocks.Sources.RealExpression THA1(
    y=if waterSideTemperatureDependent then if allowFlowReversal1 then if energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState then temSen_1.T else ele[1].vol1.T else Medium1.temperature(
  ...
  Modelica.Blocks.Sources.RealExpression THA2(
    y=if airSideTemperatureDependent then if allowFlowReversal1 then if energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState then temSen_2.T else ele[nEle].vol2.T else Medium2.temperature(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 72}, {-66, 82}})));
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor theCon1[nEle-1](
    each final G=GDif)
    ...
    annotation(
      Placement(
        transformation(
          extent={{0, 34}, {20, 54}})));
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor theCon2[nEle-1](
    each final G=GDif)
...
equation
  connect(masFloSen_1.m_flow, hA.m1_flow)
    annotation(
      Line(
        points={{-74, 66.6}, {-74, 70}, {-84, 70}, {-84, 94}, {-64, 94}, {-64, 87}, {-61, 87}},
        color={0, 0, 127}));
  ...
  connect(hA.hA_1, gai_1.u)
    annotation(
      Line(
        points={{-39, 87}, {-28, 87}, {-28, 90}, {-19.2, 90}},
        color={0, 0, 255}));
  connect(hA.hA_2, gai_2.u)
    annotation(
      Line(
        points={{-39, 73}, {-27.5, 73}, {-27.5, 68}, {-19.2, 68}},
        color={0, 0, 255}));
  ...
  for i in 1 : nEle-1 loop
    ...
    connect(theCon1[i].port_a, ele[i].heaPor1)
      annotation(
        Line(
          points={{0, 44}, {-10, 44}, {-10, 32}, {10, 32}, {10, 20}},
          color={191, 0, 0}));
    connect(theCon1[i].port_b, ele[i + 1].heaPor1)
      annotation(
        Line(
          points={{20, 44}, {28, 44}, {28, 32}, {10, 32}, {10, 20}},
          color={191, 0, 0}));
    connect(theCon2[i].port_a, ele[i].heaPor2)
      annotation(
        Line(
          points={{20,-30}, {24,-30}, {24,-14}, {10,-14}, {10, 0}},
          color={191, 0, 0}));
    connect(theCon2[i].port_b, ele[i + 1].heaPor2)
      annotation(
        Line(
          points={{0,-30}, {-8,-30}, {-8,-14}, {10,-14}, {10, 0}},
          color={191, 0, 0}));
  end for;
