...
model DryCoilDiscretized
  ...
  parameter Boolean use_dh1=false
    ...
    annotation(
      Evaluate=true,
      Dialog(
        enable=not linearizeFlowResistance1,
        tab="Advanced"));
  ...
  parameter Real ReC_2=4000
    ...
    annotation(
      Dialog(
        enable=use_dh2 and not linearizeFlowResistance2,
        tab="Advanced"));
  ...
  Buildings.Fluid.HeatExchangers.BaseClasses.CoilRegister hexReg[nReg](
    ...
    each dp1_nominal=0,
    each dp2_nominal=0,
    each final UA_nominal=UA_nominal/nReg)
    "Heat exchanger register"
    annotation(
      Placement(
        transformation(
          extent={{-10, 0}, {10, 20}})));
  Buildings.Fluid.HeatExchangers.BaseClasses.PipeManifoldFixedResistance pipMan_a(
...
protected
  ...
  BaseClasses.CoilHeader hea1[div(nReg, 2)](
    ...
    each allowFlowReversal=allowFlowReversal1) if nReg > 1
  ...
  BaseClasses.CoilHeader hea2[div(nReg, 2)-1](
    ...
    each final nPipPar=nPipPar,
    each final mStart_flow_a=mStart_flow_a1,
    each allowFlowReversal=allowFlowReversal1) if nReg > 2
    "Pipe header to redirect flow into next register"
    annotation(
      Placement(
        transformation(
  ...
  Modelica.Blocks.Sources.RealExpression THA1(
    y=if waterSideTemperatureDependent then if allowFlowReversal1 then if energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState then temSen_1.T else hexReg[1].ele[1, 1].vol1.T else Medium1.temperature(
      state=Medium1.setState_phX(
        p=port_a1.p,
        h=inStream(port_a1.h_outflow),
        X=inStream(port_a1.Xi_outflow))) else Medium1.T_default)
  ...
  Modelica.Blocks.Sources.RealExpression THA2(
    y=if airSideTemperatureDependent then if allowFlowReversal1 then if energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState then temSen_2.T else hexReg[1].ele[1, 1].vol2.T else Medium2.temperature(
      state=Medium2.setState_phX(
        p=port_a2.p,
        h=inStream(port_a2.h_outflow),
        X=inStream(port_a2.Xi_outflow))) else Medium2.T_default)
...
equation
  ...
  for i in 1 : 2 : nReg loop
    ...
    connect(hexReg[i].port_b1, hea1[div((i + 1), 2)].port_a)
      annotation(
        Line(
          points={{10, 16}, {70, 16}, {70, 6}, {60, 6}},
          color={0, 127, 255}));
    connect(hea1[div((i + 1), 2)].port_b, hexReg[i + 1].port_b1)
      annotation(
        Line(
          points={{40, 6}, {36, 6}, {36, 10}, {10, 10}, {10, 16}},
          color={0, 127, 255}));
  end for;
  ...
  for i in 2 : 2 :(nReg-1) loop
    connect(hexReg[i].port_a1, hea2[div(i, 2)].port_a)
      annotation(
        Line(
          points={{-10, 16}, {-64, 16}, {-64, 8}, {-60, 8}},
          color={0, 127, 255}));
    connect(hea2[div(i, 2)].port_b, hexReg[i + 1].port_a1)
      annotation(
        Line(
          points={{-40, 8}, {-34, 8}, {-34, 12}, {-10, 12}, {-10, 16}},
          color={0, 127, 255}));
  end for;
  connect(masFloSen_1.m_flow, hA.m1_flow)
    annotation(
      Line(
        points={{-74, 66.6}, {-74, 70}, {-90, 70}, {-90, 92}, {-64, 92}, {-64, 87}, {-61, 87}},
        color={0, 0, 127}));
  ...
  connect(temSen_2.port_b, ducMan_a.port_a)
    annotation(
      Line(
        points={{44,-60}, {40,-60}, {40,-16}},
        color={0, 127, 255}));
  connect(masFloSen_2.m_flow, hA.m2_flow)
    annotation(
      Line(
        points={{76,-53.4}, {76,-44}, {-86,-44}, {-86, 73}, {-61, 73}},
        color={0, 0, 127}));
  connect(hA.hA_1, gai_1.u)
    annotation(
      Line(
        points={{-39, 87}, {-28, 87}, {-28, 91}, {-15.2, 91}},
        color={0, 0, 255}));
  connect(hA.hA_2, gai_2.u)
    annotation(
      Line(
        points={{-39, 73}, {-27.5, 73}, {-27.5, 67}, {-15.2, 67}},
        color={0, 0, 255}));
  for i in 1 : nReg loop
    connect(gai_1.y, hexReg[i].Gc_1)
      annotation(
        Line(
          points={{-1.4, 91}, {12, 91}, {12, 30}, {-4, 30}, {-4, 20}},
          color={0, 0, 127}));
    connect(gai_2.y, hexReg[i].Gc_2)
      annotation(
        Line(
          points={{-1.4, 67}, {14, 67}, {14,-6}, {4,-6}, {4, 0}},
          color={0, 0, 127}));
  end for;
  ...
  connect(THA1.y, hA.T_1)
    annotation(
      Line(
        points={{-65.3, 83}, {-65.3, 83}, {-61, 83}},
        color={0, 0, 127}));
  connect(THA2.y, hA.T_2)
    annotation(
      Line(
        points={{-65.3, 77}, {-65.3, 77}, {-61, 77}},
        color={0, 0, 127}));
