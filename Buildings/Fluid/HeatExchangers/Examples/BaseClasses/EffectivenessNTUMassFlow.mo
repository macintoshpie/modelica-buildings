...
partial model EffectivenessNTUMassFlow
  ...
  Buildings.Fluid.Sources.MassFlowSource_T sin_2(redeclare package Medium=Medium2, T=T_a2_nominal, use_m_flow_in=true)
    "Sink for air"
  ...
  Buildings.Fluid.Sources.MassFlowSource_T sin_1(redeclare package Medium=Medium1, T=T_a1_nominal, use_m_flow_in=true)
    "Sink for water"
    annotation(Placement(transformation(extent={{140, 50}, {120, 70}})));
  Buildings.Fluid.Sources.Boundary_pT sou_1(redeclare package Medium=Medium1, use_T_in=false, T=T_a1_nominal)
    "Source for water"
  ...
  Modelica.Blocks.Sources.TimeTable mWatGai(table=[
    ...
    0.1, 1;
    0.2, 0.01;
    0.3, 0.01], timeScale=3600)
    "Gain for water mass flow rate"
    annotation(Placement(transformation(extent={{-80, 100}, {-60, 120}})));
  Modelica.Blocks.Sources.TimeTable mAirGai(table=[
    0, 1;
    0.5, 1;
    0.6,-1;
    0.7, 0;
...
equation
  connect(x_pTphi.X, sou_2.X_in)
    annotation(Line(points={{171,-32}, {178,-32}, {178,-34}, {186,-34}, {186, 20}, {140, 20}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(temSou_2.y, sou_2.T_in)
    annotation(Line(points={{121,-28}, {134,-28}, {134, 0}, {160, 0}, {160, 28}, {140, 28}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(mWat_flow.y, sin_1.m_flow_in)
    annotation(Line(points={{-21, 110}, {160, 110}, {160, 68}, {140, 68}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(mAirGai.y, mAir_flow.u)
    annotation(Line(points={{-59, 32}, {-42, 32}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(mAir_flow.y, sin_2.m_flow_in)
    annotation(Line(points={{-19, 32}, {0, 32}}, color={0, 0, 127}, smooth=Smooth.None));
