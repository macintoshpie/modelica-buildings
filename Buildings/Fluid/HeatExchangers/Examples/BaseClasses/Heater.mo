...
partial model Heater
  ...
  Buildings.Fluid.MixingVolumes.MixingVolume vol(redeclare package Medium=Medium, T_start=289.15, V=V, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, m_flow_nominal=m_flow_nominal, mSenFac=2, nPorts=3)
  ...
  Buildings.Fluid.Movers.FlowControlled_m_flow mov(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, addPowerToMedium=false, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState)
    "Fan or pump"
    annotation(Placement(transformation(extent={{-70,-50}, {-50,-30}})));
  Modelica.Blocks.Sources.RealExpression TOut(y(final unit="K", displayUnit="degC")=273.15 + 16-5*cos(time/86400*2*Modelica.Constants.pi))
    "Outdoor temperature"
    annotation(Placement(transformation(extent={{-80, 60}, {-60, 80}})));
  Modelica.Blocks.Sources.Pulse TSet(amplitude=4, period=86400, offset=273.15 + 16, startTime=7*3600, y(final unit="K", displayUnit="degC"))
  ...
  Sensors.TemperatureTwoPort THeaOut(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, T_start=289.15)
    "Outlet temperature of the heater"
    annotation(Placement(transformation(extent={{20,-50}, {40,-30}})));
  FixedResistances.PressureDrop res(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, dp_nominal=100, linearized=true)
...
equation
  ...
  connect(vol.heatPort, theCon.port_b)
    annotation(Line(points={{40, 0}, {30, 0}, {30, 70}, {20, 70}}, color={191, 0, 0}));
  ...
  connect(TVol.T, conPI.u_m)
    annotation(Line(points={{0, 0}, {-50, 0}, {-50, 18}}, color={0, 0, 127}));
  connect(TSet.y, conPI.u_s)
    annotation(Line(points={{-69, 30}, {-62, 30}}, color={0, 0, 127}));
  connect(mFan_flow.y, mov.m_flow_in)
    annotation(Line(points={{-69,-10}, {-60,-10}, {-60,-28}}, color={0, 0, 127}));
  connect(THeaOut.port_b, vol.ports[1])
    annotation(Line(points={{40,-40}, {48,-40}, {48,-10}, {47.3333,-10}}, color={0, 127, 255}));
  connect(vol.ports[2], mov.port_a)
    annotation(Line(points={{50,-10}, {50,-70}, {-80,-70}, {-80,-40}, {-70,-40}}, color={0, 127, 255}));
  connect(res.port_b, bou.ports[1])
    annotation(Line(points={{80,-20}, {85,-20}, {90,-20}}, color={0, 127, 255}));
  connect(res.port_a, vol.ports[3])
    annotation(Line(points={{60,-20}, {52.6667,-20}, {52.6667,-10}}, color={0, 127, 255}));
...
</html>"), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {120, 100}})), Icon(coordinateSystem(extent={{-100,-100}, {120, 100}})));
end Heater;
