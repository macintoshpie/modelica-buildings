...
partial model Heater
  ...
  Modelica.Blocks.Sources.RealExpression TOut(
    y(
      final unit="K",
      displayUnit="degC")=273.15 + 16-5*cos(time/86400*2*Modelica.Constants.pi))
    "Outdoor temperature"
    annotation(
      Placement(
        transformation(
  ...
  Controls.Continuous.LimPID conPI(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60, 20}, {-40, 40}})));
  Modelica.Blocks.Sources.Constant mFan_flow(
    k=m_flow_nominal)
...
equation
  ...
  connect(res.port_b, bou.ports[1])
    annotation(
      Line(
        points={{80,-20}, {85,-20}, {90,-20}},
        color={0, 127, 255}));
  connect(res.port_a, vol.ports[3])
    annotation(
      Line(
        points={{60,-20}, {52.6667,-20}, {52.6667,-10}},
        color={0, 127, 255}));
  annotation(
    Documentation(
      info="<html>
