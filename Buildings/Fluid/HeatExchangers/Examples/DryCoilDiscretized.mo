...
model DryCoilDiscretized
  ...
  Modelica.Blocks.Sources.Ramp PIn(offset=101525, height=-199, duration=60, startTime=120)
    annotation(Placement(transformation(extent={{-20,-50}, {0,-30}})));
  Buildings.Fluid.Sources.Boundary_pT sou_2(redeclare package Medium=Medium2, use_p_in=true, use_T_in=true, T=283.15, nPorts=1)
    annotation(Placement(transformation(extent={{40,-70}, {60,-50}})));
  Modelica.Blocks.Sources.Ramp TWat(duration=60, startTime=60, height=5, offset=273.15 + 60)
    "Water temperature"
  ...
  Modelica.Blocks.Sources.Ramp PSin_1(startTime=240, offset=300000, height=4990, duration=60)
    annotation(Placement(transformation(extent={{40, 60}, {60, 80}})));
  Modelica.Blocks.Sources.Ramp m_flow_2(duration=60, startTime=120, height=28-0.124, offset=-28)
...
equation
  ...
  connect(TDb.y, sou_2.T_in)
    annotation(Line(points={{1,-80}, {20,-80}, {20,-56}, {38,-56}}, color={0, 0, 127}));
  connect(TWat.y, sou_1.T_in)
    annotation(Line(points={{-79, 54}, {-70.5, 54}, {-62, 54}}, color={0, 0, 127}));
  connect(PSin_1.y, sin_1.p_in)
    annotation(Line(points={{61, 70}, {100, 70}, {100, 20}, {86, 20}}, color={0, 0, 127}));
  connect(sou_1.ports[1], hex.port_a1)
    annotation(Line(points={{-40, 50}, {-16, 50}, {-16, 12}, {8, 12}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(sou_2.ports[1], hex.port_a2)
    annotation(Line(points={{60,-60}, {70,-60}, {70,-20}, {36,-20}, {36, 5.55112e-16}, {28, 5.55112e-16}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(sin_2.ports[1], hex.port_b2)
    annotation(Line(points={{-38, 6.66134e-16}, {-16, 6.66134e-16}, {-16, 5.55112e-16}, {8, 5.55112e-16}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(hex.port_b1, sin_1.ports[1])
    annotation(Line(points={{28, 12}, {64, 12}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(m_flow_2.y, sin_2.m_flow_in)
    annotation(Line(points={{-79, 8}, {-58, 8}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(experiment(Tolerance=1e-6, StopTime=360), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/Examples/DryCoilDiscretized.mos"
