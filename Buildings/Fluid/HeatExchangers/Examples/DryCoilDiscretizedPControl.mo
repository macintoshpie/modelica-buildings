...
model DryCoilDiscretizedPControl
  ...
  Sources.MassFlowSource_T sin_2(redeclare package Medium=Medium2, nPorts=1, m_flow=-10.5, T=303.15)
    annotation(Placement(transformation(extent={{-52, 10}, {-32, 30}})));
  Buildings.Fluid.Sources.Boundary_pT sou_2(redeclare package Medium=Medium2, nPorts=1, use_p_in=false, use_T_in=false, p(displayUnit="Pa")=101625, T=T_a2_nominal)
  ...
  Buildings.Fluid.Sources.Boundary_pT sou_1(redeclare package Medium=Medium1, p=300000 + 9000, nPorts=1, use_T_in=false, T=T_a1_nominal)
    annotation(Placement(transformation(extent={{-52, 50}, {-32, 70}})));
  Modelica.Blocks.Sources.Ramp PSin_1(duration=60, height=5000, startTime=240, offset=300000)
  ...
  Buildings.Fluid.Actuators.Valves.TwoWayEqualPercentage val(redeclare package Medium=Medium1, l=0.005, m_flow_nominal=m1_flow_nominal, use_inputFilter=false, dpFixed_nominal=2000 + 3000, dpValve_nominal=6000)
    "Valve model"
  ...
  Buildings.Controls.Continuous.LimPID con(k=1, Ti=60, controllerType=Modelica.Blocks.Types.SimpleController.P, Td=60)
    "Controller"
    annotation(Placement(transformation(extent={{-20, 90}, {0, 110}})));
equation
  connect(PSin_1.y, sin_1.p_in)
    annotation(Line(points={{161, 100}, {180, 100}, {180, 68}, {142, 68}}, color={0, 0, 127}));
  ...
  connect(sou_1.ports[1], val.port_a)
    annotation(Line(points={{-32, 60}, {30, 60}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(sou_2.ports[1], hex.port_a2)
    annotation(Line(points={{120, 20}, {80, 20}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(mot.y, val.y)
    annotation(Line(points={{33, 100}, {40, 100}, {40, 72}}, color={0, 0, 127}, smooth=Smooth.None));
  ...
  connect(TSet.y, con.u_s)
    annotation(Line(points={{-59, 100}, {-22, 100}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(temSen.T, con.u_m)
    annotation(Line(points={{30, 31}, {30, 40}, {-10, 40}, {-10, 88}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(con.y, mot.u)
    annotation(Line(points={{1, 100}, {10, 100}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(hex.port_b1, sin_1.ports[1])
    annotation(Line(points={{80, 32}, {100, 32}, {100, 60}, {120, 60}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(temSen.port_b, sin_2.ports[1])
    annotation(Line(points={{20, 20}, {-32, 20}}, color={0, 127, 255}, smooth=Smooth.None));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100}, {200, 200}})), experiment(Tolerance=1e-6, StopTime=3600), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/Examples/DryCoilDiscretizedPControl.mos"
