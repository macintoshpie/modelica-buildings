...
model WetCoilDiscretizedPControl
  ...
  Buildings.Fluid.Sources.Boundary_pT sin_2(redeclare package Medium=Medium2, use_p_in=false, p=101325, T=303.15, nPorts=1)
    annotation(Placement(transformation(extent={{-58,-26}, {-38,-6}})));
  Buildings.Fluid.Sources.Boundary_pT sou_2(redeclare package Medium=Medium2, nPorts=1, use_p_in=false, use_T_in=true, p(displayUnit="Pa")=101525, T=293.15)
    annotation(Placement(transformation(extent={{160, 8}, {140, 28}})));
  Buildings.Fluid.Sources.Boundary_pT sin_1(redeclare package Medium=Medium1, p=300000, T=293.15, use_p_in=true, nPorts=1)
    annotation(Placement(transformation(extent={{160, 40}, {140, 60}})));
  Buildings.Fluid.Sources.Boundary_pT sou_1(redeclare package Medium=Medium1, nPorts=1, use_T_in=true, p=300000 + 7000, T=278.15)
    annotation(Placement(transformation(extent={{-24, 38}, {-4, 58}})));
  Modelica.Blocks.Sources.Ramp PSin(duration=60, height=5000, startTime=240, offset=300000)
  ...
  Buildings.Fluid.Actuators.Valves.TwoWayLinear val(redeclare package Medium=Medium1, m_flow_nominal=m1_flow_nominal, use_inputFilter=false, dpFixed_nominal=2000, dpValve_nominal=5000)
...
equation
  connect(PSin.y, sin_1.p_in)
    annotation(Line(points={{161, 90}, {180, 90}, {180, 58}, {162, 58}}, color={0, 0, 127}));
  connect(sou_2.ports[1], hex.port_a2)
    annotation(Line(points={{140, 18}, {140, 20}, {80, 20}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(hex.port_b2, temSen.port_a)
    annotation(Line(points={{60, 20}, {48, 20}, {48,-16}, {40,-16}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(val.port_b, hex.port_a1)
    annotation(Line(points={{38, 48}, {42, 48}, {42, 32}, {60, 32}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(sou_1.ports[1], val.port_a)
    annotation(Line(points={{-4, 48}, {18, 48}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(TSou_2.y, sou_2.T_in)
    annotation(Line(points={{161,-30}, {178,-30}, {178, 22}, {162, 22}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TSou_1.y, sou_1.T_in)
    annotation(Line(points={{-39, 52}, {-26, 52}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TSet.y, con.u_s)
    annotation(Line(points={{-59, 130}, {-42, 130}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(temSen.T, con.u_m)
    annotation(Line(points={{30,-5}, {30, 8}, {10, 8}, {10, 100}, {-30, 100}, {-30, 118}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(con.y, mot.u)
    annotation(Line(points={{-19, 130}, {-2, 130}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(mot.y, val.y)
    annotation(Line(points={{21, 130}, {28, 130}, {28, 60}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(sin_2.ports[1], temSen.port_b)
    annotation(Line(points={{-38,-16}, {20,-16}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(hex.port_b1, sin_1.ports[1])
    annotation(Line(points={{80, 32}, {112, 32}, {112, 50}, {140, 50}}, color={0, 127, 255}, smooth=Smooth.None));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100}, {200, 200}}), graphics), experiment(Tolerance=1e-6, StopTime=3600), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/Examples/WetCoilDiscretizedPControl.mos"
