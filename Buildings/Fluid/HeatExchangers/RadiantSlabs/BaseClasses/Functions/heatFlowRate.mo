...
function heatFlowRate
  ...
  input Modelica.SIunits.Temperature T_s
    "Temperature of solid";
  input Modelica.SIunits.Temperature T_f
    "Temperature of fluid control volume";
  ...
  output Modelica.SIunits.HeatFlowRate Q_flow
    "Heat flow rate";
protected
  Modelica.SIunits.MassFlowRate m_abs_flow
...
algorithm
  ...
  elseif(m_abs_flow < 0.05*m_flow_nominal) then
    ...
    // above regimes.
    eps := 1-Modelica.Math.exp(-UA/m_abs_flow/c_p);
    Q_flow := Buildings.Utilities.Math.Functions.spliceFunction(pos=eps*(T_s-T_in)*m_abs_flow*c_p, neg=UA*(T_s-T_f), x=m_abs_flow/m_flow_nominal-0.1, deltax=0.05);
  end if;
