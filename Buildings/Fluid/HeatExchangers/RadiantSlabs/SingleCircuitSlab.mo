...
protected
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=180,
          origin={40, 76})));
  final parameter Modelica.SIunits.ThermalInsulance Rx=Buildings.Fluid.HeatExchangers.RadiantSlabs.BaseClasses.Functions.AverageResistance(
    disPip=disPip,
    dPipOut=pipe.dOut,
  ...
  BaseClasses.PipeToSlabConductance fluSlaCon[nSeg](
    ...
    each final RWal=Modelica.Math.log(pipe.dOut/pipe.dIn)/(2*Modelica.Constants.pi*pipe.k*(length/nSeg)),
    each final RFic=nSeg*Rx/A,
    each final m_flow_nominal=m_flow_nominal,
    each kc_IN_con=Modelica.Fluid.Dissipation.HeatTransfer.StraightPipe.kc_overall_IN_con(
      d_hyd=pipe.dIn,
      L=length/nSeg,
      K=pipe.roughness),
  ...
  Modelica.Blocks.Sources.RealExpression T_a(
    final y=Medium.temperature_phX(
      p=port_a.p,
  ...
  annotation(
    ...
    Icon(
      graphics={Rectangle(
        ...
        points={{-90, 0}, {-74, 0}, {-74, 72}, {60, 72}, {60, 34}, {-62, 34}, {-62,-6}, {60,-6}, {60,-44}, {-66,-44}, {-66,-74}, {74,-74}},
        color={0, 128, 255},
        thickness=1,
