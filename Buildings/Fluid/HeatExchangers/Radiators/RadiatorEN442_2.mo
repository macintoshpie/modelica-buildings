...
model RadiatorEN442_2
  ...
  extends Buildings.Fluid.Interfaces.PartialTwoPortInterface(show_T=true, m_flow_nominal=abs(Q_flow_nominal/cp_nominal/(T_a_nominal-T_b_nominal)));
  extends Buildings.Fluid.Interfaces.LumpedVolumeDeclarations(final X_start=Medium.X_default, final C_start=fill(0, Medium.nC), final C_nominal=fill(1E-2, Medium.nC), final mSenFac=1 + 500*mDry/(VWat*cp_nominal*Medium.density(Medium.setState_pTX(Medium.p_default, Medium.T_default, Medium.X_default))));
...
protected
  ...
  parameter Modelica.SIunits.HeatFlowRate QEle_flow_nominal[nEle](each fixed=false, each start=Q_flow_nominal/nEle)
    "Nominal heating power of each element";
  parameter Modelica.SIunits.Temperature TWat_nominal[nEle](each fixed=false, start={T_a_nominal-i/nEle*(T_a_nominal-T_b_nominal) for i in 1 : nEle})
    "Water temperature in each element at nominal conditions";
  parameter Modelica.SIunits.TemperatureDifference[nEle] dTRad_nominal(each fixed=false, start={T_a_nominal-i/nEle*(T_a_nominal-T_b_nominal)-TRad_nominal for i in 1 : nEle})
    "Temperature difference for radiative heat transfer at nominal conditions";
  parameter Modelica.SIunits.TemperatureDifference[nEle] dTCon_nominal(each fixed=false, start={T_a_nominal-i/nEle*(T_a_nominal-T_b_nominal)-TAir_nominal for i in 1 : nEle})
  ...
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow[nEle] preCon(each final alpha=0)
    "Heat input into radiator from convective heat transfer"
    annotation(Placement(transformation(extent={{-48,-48}, {-28,-28}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow[nEle] preRad(each final alpha=0)
  ...
  Modelica.Blocks.Sources.RealExpression QCon[nEle](y={if homotopyInitialization then homotopy(actual=(1-fraRad)*UAEle*(heatPortCon.T-vol[i].T)*Buildings.Utilities.Math.Functions.regNonZeroPower(x=(heatPortCon.T-vol[i].T), n=n-1, delta=0.05), simplified=(1-fraRad)*UAEle .* abs(dTCon_nominal[i])^(n-1)*(heatPortCon.T-vol[i].T)) else(1-fraRad)*UAEle*(heatPortCon.T-vol[i].T)*Buildings.Utilities.Math.Functions.regNonZeroPower(x=(heatPortCon.T-vol[i].T), n=n-1, delta=0.05) for i in 1 : nEle})
    "Convective heat flow rate"
    annotation(Placement(transformation(extent={{-100,-48}, {-80,-28}})));
  Modelica.Blocks.Sources.RealExpression QRad[nEle](y={if homotopyInitialization then homotopy(actual=fraRad*UAEle*(heatPortRad.T-vol[i].T)*Buildings.Utilities.Math.Functions.regNonZeroPower(x=(heatPortRad.T-vol[i].T), n=n-1, delta=0.05), simplified=fraRad*UAEle*abs(dTRad_nominal[i])^(n-1)*(heatPortRad.T-vol[i].T)) else fraRad*UAEle*(heatPortRad.T-vol[i].T)*Buildings.Utilities.Math.Functions.regNonZeroPower(x=(heatPortRad.T-vol[i].T), n=n-1, delta=0.05) for i in 1 : nEle})
    "Radiative heat flow rate"
    annotation(Placement(transformation(extent={{-100,-80}, {-80,-60}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow preSumCon(final alpha=0)
    "Heat input into radiator from convective heat transfer"
  ...
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow preSumRad(final alpha=0)
    "Heat input into radiator from radiative heat transfer"
    annotation(Placement(transformation(extent={{52,-90}, {72,-70}})));
  Buildings.Fluid.FixedResistances.PressureDrop res(redeclare final package Medium=Medium, final allowFlowReversal=allowFlowReversal, final m_flow_nominal=m_flow_nominal, final from_dp=from_dp, final dp_nominal=dp_nominal, final homotopyInitialization=homotopyInitialization, final linearized=linearized, final deltaM=deltaM, final show_T=false)
...
initial equation
  ...
  for i in 1 : nEle loop
    ...
    QEle_flow_nominal[i]=k*UAEle*(fraRad*Buildings.Utilities.Math.Functions.powerLinearized(x=k*TWat_nominal[i]-TRad_nominal, n=n, x0=0.1*k*(T_b_nominal-TRad_nominal)) +(1-fraRad)*Buildings.Utilities.Math.Functions.powerLinearized(x=k*TWat_nominal[i]-TAir_nominal, n=n, x0=0.1*k*(T_b_nominal-TAir_nominal)));
  end for;
...
equation
  ...
  annotation(Icon(graphics={Ellipse(extent={{-20, 22}, {20,-20}}, fillColor={127, 0, 0}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Rectangle(extent={{-95, 6}, {106,-4}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 255}, fillPattern=FillPattern.Solid), Rectangle(extent={{-102,-4}, {-2, 6}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={255, 0, 0}, fillPattern=FillPattern.Solid), Ellipse(extent={{-20, 22}, {20,-20}}, fillColor={127, 0, 0}, fillPattern=FillPattern.Solid, pattern=LinePattern.None), Rectangle(extent={{-80, 60}, {80,-60}}, lineColor={0, 0, 0}, fillColor=DynamicSelect({95, 95, 95}, max(0, min(1,-Q_flow/Q_flow_nominal))*{255, 0, 0} +(1-max(0, min(1,-Q_flow/Q_flow_nominal)))*{95, 95, 95}), fillPattern=FillPattern.Solid), Line(points={{-66, 30}, {66, 30}}), Line(points={{-66, 2}, {66, 2}}), Line(points={{-66,-30}, {66,-30}}), Line(points={{-66, 60}, {-66,-60}}), Line(points={{66, 60}, {66,-60}})}), defaultComponentName="rad", Documentation(info="<html>
<p>
...
</html>"), Diagram(graphics={Rectangle(extent={{-62, 78}, {-46, 50}}, lineColor={28, 108, 200})}));
end RadiatorEN442_2;
