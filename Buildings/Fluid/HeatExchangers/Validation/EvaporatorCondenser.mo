...
model EvaporatorCondenser
  ...
  Modelica.Fluid.Sources.MassFlowSource_T sou(nPorts=1, redeclare package Medium=Medium, m_flow=0.1, use_m_flow_in=true, T=323.15)
    "Flow source"
    annotation(Placement(transformation(extent={{-44,-10}, {-24, 10}})));
  Modelica.Fluid.Sources.FixedBoundary sin(redeclare package Medium=Medium, p=0, nPorts=1)
    "Sink"
    annotation(Placement(transformation(extent={{78,-10}, {58, 10}})));
  Buildings.Fluid.HeatExchangers.EvaporatorCondenser eva(redeclare package Medium=Medium, m_flow(start=0.1), dp(start=10), UA=100, massDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, dp_nominal=0, tau=5, energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, m_flow_nominal=m_flow_nominal)
    "Evaporator"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}})));
  Modelica.Blocks.Sources.Ramp m_flow(duration=100, height=9*m_flow_nominal, offset=m_flow_nominal)
    "Mass flow rate"
    annotation(Placement(transformation(extent={{-88,-2}, {-68, 18}})));
  Buildings.Fluid.Sensors.TemperatureTwoPort senTem(m_flow_nominal=m_flow_nominal, redeclare package Medium=Medium, tau=0.01, initType=Modelica.Blocks.Types.Init.SteadyState)
    "Temperature sensor"
...
equation
  ...
  annotation(__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/Validation/EvaporatorCondenser.mos"
    "Simulate and plot"), experiment(Tolerance=1e-6, StopTime=100), Documentation(info="<html>
<p>
