...
model EvaporatorCondenser
  ...
  Buildings.HeatTransfer.Sources.FixedTemperature ref(
    T=283.15)
...
equation
  connect(ref.port, heaFlo.port_a)
    annotation(
      Line(
        points={{-44,-50}, {-37,-50}, {-30,-50}},
        color={191, 0, 0}));
  connect(heaFlo.port_b, eva.port_ref)
    annotation(
      Line(
        points={{-10,-50}, {0,-50}, {0,-6}},
        color={191, 0, 0}));
  ...
  connect(eva.port_b, senTem.port_a)
    annotation(
      Line(
        points={{10, 0}, {18, 0}, {24, 0}},
        color={0, 127, 255}));
  connect(senTem.port_b, sin.ports[1])
    annotation(
      Line(
        points={{44, 0}, {52, 0}, {58, 0}},
        color={0, 127, 255}));
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatExchangers/Validation/EvaporatorCondenser.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=100),
