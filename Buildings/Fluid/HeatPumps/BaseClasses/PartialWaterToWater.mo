...
equation
  ...
  connect(con.port_b, port_b1)
    annotation(
      Line(
        points={{60, 60}, {60, 60}, {100, 60}},
        color={0, 127, 255}));
  connect(con.Q_flow, QCon_flow)
    annotation(
      Line(
        points={{61, 64}, {60, 64}, {60, 90}, {110, 90}},
        color={0, 0, 127}));
  ...
  connect(eva.Q_flow, QEva_flow)
    annotation(
      Line(
        points={{39,-64}, {34,-64}, {34,-90}, {110,-90}},
        color={0, 0, 127}));
  connect(com.port_b, con.port_ref)
    annotation(
      Line(
        points={{50, 4}, {50, 29}, {50, 54}},
        color={191, 0, 0}));
  connect(com.port_a, eva.port_ref)
    annotation(
      Line(
        points={{50,-16}, {50,-16}, {50,-54}},
        color={191, 0, 0}));
  connect(com.P, P)
    annotation(
      Line(
  ...
  if enable_variable_speed then
    if enable_temperature_protection then
      connect(y, temPro.u)
        annotation(
          Line(
            points={{-120, 30}, {-90, 30}, {-90, 0}, {-2, 0}},
            color={0, 0, 127}));
    else
      connect(y, com.y)
        annotation(
          Line(
            points={{-120, 30}, {32, 30}, {32, 14}, {32, 3.9968e-15}, {36, 3.9968e-15}, {39, 3.9968e-15}},
            color={0, 0, 127}));
    end if;
  else
    if enable_temperature_protection then
      connect(lim.y, temPro.u)
        annotation(
          Line(
            points={{-29,-30}, {-20,-30}, {-20, 0}, {-2, 0}},
            color={0, 0, 127}));
    else
      connect(lim.y, com.y)
        annotation(
          Line(
            points={{-29,-30}, {32,-30}, {32, 0}, {39, 0}, {39, 0}},
            color={0, 0, 127}));
    end if;
  end if;
  connect(stage, intToRea.u)
    annotation(
      Line(
        points={{-120, 30}, {-120, 30}, {-90, 30}, {-90,-16}, {-90,-30}, {-82,-30}},
        color={255, 127, 0}));
  ...
  connect(temPro.TCon, con.T)
    annotation(
      Line(
        points={{-2, 8}, {-2, 8}, {-20, 8}, {-20, 76}, {70, 76}, {70, 68}, {61, 68}},
        color={0, 0, 127}));
  connect(temPro.TEva, eva.T)
    annotation(
      Line(
        points={{-2,-8}, {-2,-8}, {-12,-8}, {-12,-68}, {39,-68}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
