  ...
  Modelica.Blocks.Math.RealToInteger reaToInt
    ...
    annotation(
      Placement(
        transformation(
          extent={{-20,-20}, {0, 0}})));
  Modelica.Blocks.Sources.Sine uMod(
    amplitude=1,
    freqHz=1/2600)
  ...
  Modelica.Blocks.Sources.Sine Q_flow_set(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60, 70}, {-40, 90}})));
  Controls.OBC.CDL.Continuous.Sources.Constant mLoa_flow(
    k=1.89)
  ...
  Controls.OBC.CDL.Continuous.Sources.Sine TLoaEnt(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60, 10}, {-40, 30}})));
  Controls.OBC.CDL.Continuous.Sources.Constant mSou_flow(
    k=1.89)
...
equation
  ...
  connect(Q_flow_set.y, equFit.Q_flow_set)
    annotation(
      Line(
        points={{-39, 80}, {40, 80}, {40,-1}, {45,-1}},
        color={0, 0, 127}));
  connect(mLoa_flow.y, equFit.mLoa_flow)
    annotation(
      Line(
        points={{-38, 50}, {34, 50}, {34,-4}, {45,-4}},
        color={0, 0, 127}));
  connect(TLoaEnt.y, equFit.TLoaEnt)
    annotation(
      Line(
        points={{-38, 20}, {24, 20}, {24,-7}, {45,-7}},
        color={0, 0, 127}));
  connect(mSou_flow.y, equFit.mSou_flow)
    annotation(
      Line(
        points={{-38,-70}, {36,-70}, {36,-18}, {45,-18}},
        color={0, 0, 127}));
  connect(TSouEnt.y, equFit.TSouEnt)
    annotation(
      Line(
        points={{-38,-40}, {30,-40}, {30,-14}, {45,-14}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatPumps/BaseClasses/Validation/EquationFitReversible.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=2600),
