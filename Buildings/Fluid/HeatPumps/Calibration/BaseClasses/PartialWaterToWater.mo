  ...
  Modelica.Fluid.Sources.MassFlowSource_T loa(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60,-2}, {-40, 18}})));
  replaceable Buildings.Fluid.HeatPumps.BaseClasses.PartialWaterToWater heaPum constrainedby Buildings.Fluid.HeatPumps.BaseClasses.PartialWaterToWater(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10,-10}, {10, 10}})));
  Modelica.Blocks.Sources.IntegerConstant isOn(
    k=1)
    "Control signal"
    annotation(
      Placement(
...
equation
  connect(calDat.y, splDat.u)
    annotation(
      Line(
        points={{-119, 10}, {-119, 10}, {-102, 10}},
        color={0, 0, 127}));
  connect(splDat.y1[1], Sou.T_in)
    annotation(
      Line(
        points={{-79, 19}, {-76, 19}, {-76, 84}, {92, 84}, {92,-2}, {62,-2}},
        color={0, 0, 127}));
  connect(splDat.y3[1], Sou.m_flow_in)
    annotation(
      Line(
        points={{-79, 7}, {-72, 7}, {-72, 80}, {88, 80}, {88, 2}, {60, 2}},
        color={0, 0, 127}));
  connect(splDat.y4[1], loa.m_flow_in)
    annotation(
      Line(
        points={{-79, 1}, {-75.5, 1}, {-75.5, 16}, {-60, 16}},
        color={0, 0, 127}));
  connect(loa.ports[1], heaPum.port_a1)
    annotation(
      Line(
        points={{-40, 8}, {-40, 8}, {-30, 8}, {-20, 8}, {-20, 6}, {-10, 6}},
        color={0, 127, 255}));
  connect(sin2.ports[1], heaPum.port_b2)
    annotation(
      Line(
        points={{-40,-40}, {-20,-40}, {-20,-6}, {-10,-6}},
        color={0, 127, 255}));
  connect(Sou.ports[1], heaPum.port_a2)
    annotation(
      Line(
        points={{40,-6}, {26,-6}, {10,-6}},
        color={0, 127, 255}));
  ...
  connect(splDat.y2[1], loa.T_in)
    annotation(
      Line(
        points={{-79, 13}, {-70.5, 13}, {-70.5, 12}, {-62, 12}},
        color={0, 0, 127}));
  connect(isOn.y, heaPum.stage)
    annotation(
      Line(
        points={{-37.4,-10}, {-24,-10}, {-24, 3}, {-12, 3}},
        color={255, 127, 0}));
  annotation(
    Icon(
      coordinateSystem(
