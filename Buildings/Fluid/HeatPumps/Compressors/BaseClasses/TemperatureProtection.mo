...
model TemperatureProtection
  ...
  parameter Real dTHys(final unit="K", min=Modelica.Constants.small)=5
    "Hysteresis interval width";
  parameter Boolean pre_y_start=false
    "Value of pre(y) of hysteresis at initial time";
  ...
  Modelica.Blocks.Interfaces.RealOutput y
    "Modified compressor control signal"
...
protected
  ...
  Modelica.Blocks.Logical.Hysteresis hysHig(uLow=0, uHigh=dTHys, final pre_y_start=pre_y_start)
  ...
  Modelica.Blocks.Logical.Hysteresis hysLow(uLow=0, uHigh=dTHys, final pre_y_start=pre_y_start)
  ...
  Modelica.Blocks.MathBoolean.And on(nu=3)
    "Compressor status"
    annotation(Placement(transformation(extent={{20,-10}, {40, 10}})));
  Modelica.Blocks.Logical.Hysteresis hysdTConEva(final uLow=0, final uHigh=dTHys, final pre_y_start=pre_y_start)
  ...
  Modelica.Blocks.MathBoolean.Not notEva
    "Negation of signal"
    annotation(Placement(transformation(extent={{64, 84}, {76, 96}})));
  Modelica.Blocks.MathBoolean.Not notdT
    "Negation of signal"
    annotation(Placement(transformation(extent={{64, 64}, {76, 76}})));
  Modelica.Blocks.MathBoolean.Not notCon
    "Negation of signal"
    annotation(Placement(transformation(extent={{64, 44}, {76, 56}})));
equation
  connect(zero.y, switch.u3)
    annotation(Line(points={{41,-30}, {50,-30}, {50,-8}, {58,-8}}, color={0, 0, 127}));
  connect(u, switch.u1)
    annotation(Line(points={{-120, 0}, {-96, 0}, {-96, 60}, {50, 60}, {50, 8}, {58, 8}}, color={0, 0, 127}));
  connect(switch.y, y)
    annotation(Line(points={{81, 0}, {110, 0}}, color={0, 0, 127}));
  connect(TMax.y, dTCon.u1)
    annotation(Line(points={{-79.4,-36}, {-79.4,-36}, {-62,-36}}, color={0, 0, 127}));
  connect(TMin.y, dTEva.u1)
    annotation(Line(points={{-79.4, 36}, {-62, 36}}, color={0, 0, 127}));
  connect(dTEva.u2, TEva)
    annotation(Line(points={{-62, 24}, {-70, 24}, {-70,-80}, {-120,-80}}, color={0, 0, 127}));
  connect(dTCon.u2, TCon)
    annotation(Line(points={{-62,-24}, {-76,-24}, {-76, 80}, {-120, 80}}, color={0, 0, 127}));
  ...
  connect(on.y, switch.u2)
    annotation(Line(points={{41.5, 0}, {41.5, 0}, {58, 0}}, color={255, 0, 255}));
  connect(dTEvaCon.y, hysdTConEva.u)
    annotation(Line(points={{-39, 0}, {-32, 0}, {-22, 0}}, color={0, 0, 127}));
  connect(dTEvaCon.u2, TCon)
    annotation(Line(points={{-62,-6}, {-76,-6}, {-76, 80}, {-120, 80}}, color={0, 0, 127}));
  connect(dTEvaCon.u1, TEva)
    annotation(Line(points={{-62, 6}, {-70, 6}, {-70,-80}, {-120,-80}}, color={0, 0, 127}));
  connect(hysLow.y, on.u[1])
    annotation(Line(points={{1, 30}, {10, 30}, {10, 4.66667}, {20, 4.66667}}, color={255, 0, 255}));
  connect(hysdTConEva.y, on.u[2])
    annotation(Line(points={{1, 0}, {20, 0}}, color={255, 0, 255}));
  connect(hysHig.y, on.u[3])
    annotation(Line(points={{1,-30}, {14,-30}, {14,-4}, {18,-4}, {18,-4.66667}, {20,-4.66667}}, color={255, 0, 255}));
  connect(hysLow.y, notEva.u)
    annotation(Line(points={{1, 30}, {10, 30}, {10, 90}, {61.6, 90}}, color={255, 0, 255}));
  connect(notEva.y, errLowPre)
    annotation(Line(points={{77.2, 90}, {94, 90}, {110, 90}}, color={255, 0, 255}));
  connect(notdT.u, hysdTConEva.y)
    annotation(Line(points={{61.6, 70}, {12, 70}, {12, 0}, {1, 0}}, color={255, 0, 255}));
  ...
  connect(notCon.u, hysHig.y)
    annotation(Line(points={{61.6, 50}, {14, 50}, {14,-30}, {1,-30}}, color={255, 0, 255}));
