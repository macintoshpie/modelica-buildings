  ...
  Buildings.Fluid.HeatPumps.Compressors.ReciprocatingCompressor com(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10,-10}, {10, 10}})));
  Buildings.HeatTransfer.Sources.FixedTemperature eva(
    T=253.15)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-82,-10}, {-62, 10}})));
  Buildings.HeatTransfer.Sources.FixedTemperature con(
    T=323.15)
...
equation
  ...
  connect(com.port_b, heaFloCon.port_a)
    annotation(
      Line(
        points={{10, 0}, {19, 0}, {28, 0}},
        color={191, 0, 0}));
  connect(heaFloCon.port_b, con.port)
    annotation(
      Line(
        points={{48, 0}, {56, 0}, {64, 0}},
        color={191, 0, 0}));
  connect(on.y, com.y)
    annotation(
      Line(
        points={{-5, 28}, {6, 28}, {6, 11}},
        color={0, 0, 127}));
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatPumps/Compressors/Validation/ReciprocatingCompressor_ZeroInput.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=100),
