  ...
  Modelica.Blocks.Interfaces.RealOutput P(
    ...
    annotation(
      Placement(
        ...
        iconTransformation(
          extent={{100,-12}, {120, 8}})));
  Modelica.Blocks.Interfaces.RealOutput QSou_flow(
    final unit="W")
    "Heat flow rate at the source heat exchanger"
    annotation(
      Placement(
        transformation(
          extent={{100,-98}, {120,-78}}),
        iconTransformation(
          extent={{100,-100}, {120,-80}})));
  Modelica.Blocks.Interfaces.RealOutput QLoa_flow(
    final unit="W")
...
protected
  ...
  Modelica.Blocks.Sources.RealExpression mLoa_flow(
    y=port_a1.m_flow)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 16}, {-60, 36}})));
  Modelica.Blocks.Sources.RealExpression mSou_flow(
    y=port_a2.m_flow)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80,-20}, {-60, 0}})));
  Modelica.SIunits.SpecificEnthalpy hSet=if uMod == 0 then h1_default else Medium1.specificEnthalpy_pTX(
    p=port_b1.p,
    T=TSet,
  ...
  Modelica.Blocks.Sources.RealExpression TSouEnt(
    final y=Medium2.temperature(Medium2.setState_phX(port_a2.p, inStream(port_a2.h_outflow), inStream(port_a2.Xi_outflow))))
  ...
  Modelica.Blocks.Sources.RealExpression TLoaEnt(
    final y=Medium1.temperature(Medium1.setState_phX(port_a1.p, inStream(port_a1.h_outflow), inStream(port_a1.Xi_outflow))))
  ...
  Modelica.Blocks.Sources.RealExpression Q_flow_set(
    final y=if(uMod == 0) then 0 else m1_flow*(hSet-inStream(port_a1.h_outflow)))
  ...
  Buildings.Controls.OBC.CDL.Integers.LessThreshold lesThr(
    final threshold=0) if not per.reverseCycle
  ...
  Buildings.Controls.OBC.CDL.Utilities.Assert aleMes(
    message="uMod cannot be -1 if reverseCycle is false.") if not per.reverseCycle
...
equation
  connect(equFit.QSou_flow, QSou_flow)
    annotation(
      Line(
        points={{11,-2}, {32,-2}, {32,-88}, {110,-88}},
        color={0, 0, 127}));
  connect(mSou_flow.y, equFit.mSou_flow)
    annotation(
      Line(
        points={{-59,-10}, {-54,-10}, {-54,-8}, {-11,-8}},
        color={0, 0, 127}));
  connect(mLoa_flow.y, equFit.mLoa_flow)
    annotation(
      Line(
        points={{-59, 26}, {-48, 26}, {-48, 6}, {-11, 6}},
        color={0, 0, 127}));
  connect(equFit.QLoa_flow, QLoa_flow)
    annotation(
      Line(
        points={{11, 6}, {80, 6}, {80, 88}, {110, 88}},
        color={0, 0, 127}));
  connect(equFit.QLoa_flow, preHeaFloLoa.Q_flow)
    annotation(
      Line(
        points={{11, 6}, {80, 6}, {80, 20}, {59, 20}},
        color={0, 0, 127}));
  connect(TSouEnt.y, equFit.TSouEnt)
    annotation(
      Line(
        points={{-59,-26}, {-50,-26}, {-50,-4}, {-11,-4}},
        color={0, 0, 127}));
  connect(TLoaEnt.y, equFit.TLoaEnt)
    annotation(
      Line(
        points={{-59, 8}, {-54, 8}, {-54, 3}, {-11, 3}},
        color={0, 0, 127}));
  connect(equFit.P, P)
    annotation(
      Line(
        points={{11, 2}, {60, 2}, {60, 0}, {110, 0}},
        color={0, 0, 127}));
  ...
  connect(equFit.QSou_flow, preHeaFloSou.Q_flow)
    annotation(
      Line(
        points={{11,-2}, {32,-2}, {32,-44}, {74,-44}, {74,-60}, {59,-60}},
        color={0, 0, 127}));
  connect(vol1.heatPort, preHeaFloLoa.port)
    annotation(
      Line(
        points={{-10, 60}, {-14, 60}, {-14, 20}, {39, 20}},
        color={191, 0, 0}));
  ...
  connect(lesThr.u, uMod)
    annotation(
      Line(
        points={{-82,-80}, {-88,-80}, {-88, 0}, {-112, 0}},
        color={255, 127, 0}));
  connect(equFit.Q_flow_set, Q_flow_set.y)
    annotation(
      Line(
        points={{-11, 9}, {-44, 9}, {-44, 40}, {-59, 40}},
        color={0, 0, 127}));
  connect(equFit.COP, COP)
    annotation(
      Line(
        points={{11,-6}, {36,-6}, {36,-40}, {110,-40}},
        color={0, 0, 127}));
  annotation(
    Icon(
      coordinateSystem(
      ...
      graphics={Ellipse(
        ...
        points={{0, 68}, {0, 90}, {-100, 90}},
        color={0, 0, 255}), Line(
        points={{112,-6}},
        color={28, 108, 200}), Line(
        points={{2, 68}, {2, 90}, {100, 90}, {102, 86}},
        color={28, 108, 200}), Line(
        points={{70, 0}, {108, 0}},
        color={28, 108, 200}), Line(
        points={{2,-90}},
        color={28, 108, 200}), Line(
        points={{2,-70}, {2,-90}, {106,-90}},
        color={28, 108, 200}), Line(
        points={{24,-18}, {6,-18}, {6,-52}},
        color={238, 46, 47}), Line(
        points={{24, 6}, {6, 6}, {6, 50}},
        color={238, 46, 47}), Rectangle(
        extent={{24, 18}, {26,-24}},
        lineColor={0, 0, 0},
        fillColor={135, 135, 135},
        fillPattern=FillPattern.Solid), Polygon(
        ...
        extent={{22,-22}, {28,-14}},
        lineColor={255, 0, 0}), Line(
        points={{32,-4}, {26,-4}, {26,-4}},
        color={255, 0, 0}), Line(
        points={{70,-30}, {108,-30}},
        color={28, 108, 200})}),
    Diagram(
