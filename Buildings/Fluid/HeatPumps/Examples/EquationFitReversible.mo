  ...
  Controls.OBC.CDL.Continuous.Sources.Ramp TCooLoaSet(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60,-40}, {-40,-20}})));
  Controls.OBC.CDL.Integers.GreaterThreshold intGreThr(
    threshold=-1)
    "Integer threshold"
    annotation(
      Placement(
...
equation
  connect(souPum.ports[1], heaPum.port_a2)
    annotation(
      Line(
        points={{80,-8}, {70,-8}, {70, 4}, {60, 4}},
        color={0, 127, 255}));
  connect(heaPum.port_b1, loaVol.ports[1])
    annotation(
      Line(
        points={{60, 16}, {72, 16}, {72, 30}, {80, 30}},
        color={0, 127, 255}));
  connect(heaPum.port_b2, souVol.ports[1])
    annotation(
      Line(
        points={{40, 4}, {24, 4}, {24,-70}, {-40,-70}},
        color={0, 127, 255}));
  ...
  connect(swi2.u3, TSouEntCoo.y)
    annotation(
      Line(
        points={{78,-58}, {66,-58}, {66,-70}, {62,-70}},
        color={0, 0, 127}));
  connect(TSouEntHea.y, swi2.u1)
    annotation(
      Line(
        points={{62,-30}, {64,-30}, {64,-42}, {78,-42}},
        color={0, 0, 127}));
  connect(intGreThr.y, swi2.u2)
    annotation(
      Line(
        points={{-38, 10}, {-30, 10}, {-30,-50}, {78,-50}},
        color={255, 0, 255}));
  connect(swi1.y, loaPum.T_in)
    annotation(
      Line(
        points={{42, 70}, {50, 70}, {50, 66}, {58, 66}},
        color={0, 0, 127}));
  connect(intGreThr.y, swi1.u2)
    annotation(
      Line(
        points={{-38, 10}, {-30, 10}, {-30, 70}, {18, 70}},
        color={255, 0, 255}));
  connect(TLoaEntCoo.y, swi1.u3)
    annotation(
      Line(
        points={{2, 50}, {8, 50}, {8, 62}, {18, 62}},
        color={0, 0, 127}));
  connect(TLoaEntHea.y, swi1.u1)
    annotation(
      Line(
        points={{2, 90}, {12, 90}, {12, 78}, {18, 78}},
        color={0, 0, 127}));
  connect(swi.y, heaPum.TSet)
    annotation(
      Line(
        points={{2, 10}, {16, 10}, {16, 19}, {38.6, 19}},
        color={0, 0, 127}));
  connect(swi2.y, souPum.T_in)
    annotation(
      Line(
        points={{102,-50}, {108,-50}, {108,-12}, {102,-12}},
        color={0, 0, 127}));
