...
model EquationFitReversible
  ...
  Buildings.Fluid.HeatPumps.EquationFitReversible heaPum(redeclare package Medium1=Medium, redeclare package Medium2=Medium, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, massDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, T1_start=281.4, per=per)
    "Water to Water heat pump"
    annotation(Placement(transformation(extent={{40, 0}, {60, 20}})));
  Modelica.Blocks.Math.RealToInteger reaToInt
    "Real to integer conversion"
    annotation(Placement(transformation(extent={{-88, 0}, {-68, 20}})));
  Sources.MassFlowSource_T loaPum(redeclare package Medium=Medium, use_m_flow_in=false, m_flow=mLoa_flow_nominal, use_T_in=true, nPorts=1)
    "Load Side water pump"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, rotation=180, origin={70, 70})));
  Sources.MassFlowSource_T souPum(redeclare package Medium=Medium, m_flow=mSou_flow_nominal, nPorts=1, use_T_in=true)
    "Source side water pump"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=180, origin={90,-8})));
  Modelica.Fluid.Sources.FixedBoundary loaVol(redeclare package Medium=Medium, nPorts=1)
    "Volume for the load side"
    annotation(Placement(transformation(extent={{100, 20}, {80, 40}})));
  Modelica.Fluid.Sources.FixedBoundary souVol(redeclare package Medium=Medium, nPorts=1)
    "Volume for source side"
    annotation(Placement(transformation(extent={{-60,-80}, {-40,-60}})));
  Controls.OBC.CDL.Continuous.Sources.Ramp THeaLoaSet(height=5, duration(displayUnit="h")=14400, offset=55 + 273.15)
    "Heating load side setpoint water temperature"
    annotation(Placement(transformation(extent={{-60, 40}, {-40, 60}})));
  Controls.OBC.CDL.Continuous.Sources.Ramp TCooLoaSet(height=1, duration(displayUnit="h")=14400, offset=6 + 273.15)
  ...
  Controls.OBC.CDL.Continuous.Sources.Ramp TSouEntCoo(height=5, duration(displayUnit="h")=14400, offset=28 + 273.15)
    "Source side entering water temperature in cooling mode"
    annotation(Placement(transformation(extent={{40,-80}, {60,-60}})));
  Modelica.Blocks.Sources.Ramp uMod1(height=2, duration(displayUnit="h")=14400, offset=-1)
  ...
  Controls.OBC.CDL.Continuous.Sources.Ramp TSouEntHea(height=2, duration(displayUnit="h")=14400, offset=12 + 273.15)
    "Source side entering water temperature in heating mode"
    annotation(Placement(transformation(extent={{40,-40}, {60,-20}})));
  Controls.OBC.CDL.Continuous.Sources.Ramp TLoaEntHea(height=5, duration(displayUnit="h")=14400, offset=50 + 273.15)
    "Load side entering water temperature  in heating mode"
    annotation(Placement(transformation(extent={{-20, 80}, {0, 100}})));
  Controls.OBC.CDL.Continuous.Sources.Ramp TLoaEntCoo(height=5, duration(displayUnit="h")=14400, offset=10 + 273.15)
    "Load side entering water temperature in cooling mode"
    annotation(Placement(transformation(extent={{-20, 40}, {0, 60}})));
equation
  connect(souPum.ports[1], heaPum.port_a2)
    annotation(Line(points={{80,-8}, {70,-8}, {70, 4}, {60, 4}}, color={0, 127, 255}));
  connect(heaPum.port_b1, loaVol.ports[1])
    annotation(Line(points={{60, 16}, {72, 16}, {72, 30}, {80, 30}}, color={0, 127, 255}));
  ...
  connect(intGreThr.y, swi1.u2)
    annotation(Line(points={{-38, 10}, {-30, 10}, {-30, 70}, {18, 70}}, color={255, 0, 255}));
  ...
  connect(loaPum.ports[1], heaPum.port_a1)
    annotation(Line(points={{80, 70}, {80, 40}, {24, 40}, {24, 16}, {40, 16}}, color={0, 127, 255}, pattern=LinePattern.Dash));
  connect(THeaLoaSet.y, swi.u1)
    annotation(Line(points={{-38, 50}, {-34, 50}, {-34, 18}, {-22, 18}}, color={0, 0, 127}, pattern=LinePattern.Dot));
  connect(TCooLoaSet.y, swi.u3)
    annotation(Line(points={{-38,-30}, {-34,-30}, {-34, 2}, {-22, 2}}, color={0, 0, 127}, pattern=LinePattern.Dot));
  connect(reaToInt.y, heaPum.uMod)
    annotation(Line(points={{-67, 10}, {-64, 10}, {-64,-8}, {20,-8}, {20, 10}, {39, 10}}, color={255, 127, 0}));
  annotation(Icon(coordinateSystem(preserveAspectRatio=false), graphics={Ellipse(lineColor={75, 138, 73}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid, extent={{-98,-100}, {98, 98}}), Polygon(lineColor={0, 0, 255}, fillColor={75, 138, 73}, pattern=LinePattern.None, fillPattern=FillPattern.Solid, points={{-30, 64}, {70, 4}, {-30,-56}, {-30, 64}})}), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,-100}, {120, 120}}), graphics={Line(points={{-14,-8}}, color={28, 108, 200})}), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatPumps/Examples/EquationFitReversible.mos"
    "Simulate and plot"), experiment(Tolerance=1e-6, StopTime=14400), Documentation(info="<html>
<p>
