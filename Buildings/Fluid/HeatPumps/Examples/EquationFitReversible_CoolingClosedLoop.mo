...
equation
  ...
  connect(heaFlo.port, vol.heatPort)
    annotation(
      Line(
        points={{70, 80}, {80, 80}, {80,-2}},
        color={191, 0, 0}));
  connect(pum.port_a, vol.ports[1])
    annotation(
      Line(
        points={{20, 50}, {58, 50}, {58,-10}, {70,-10}},
        color={0, 127, 255}));
  ...
  connect(pum.port_b, heaPum.port_a1)
    annotation(
      Line(
        points={{0, 50}, {-20, 50}, {-20,-14}, {0,-14}},
        color={0, 127, 255}));
  connect(m_flow.y, pum.m_flow_in)
    annotation(
      Line(
        points={{-19, 80}, {10, 80}, {10, 62}},
        color={0, 0, 127}));
  ...
  connect(souPum.T_in, TSouEnt.y)
    annotation(
      Line(
        points={{62,-54}, {68,-54}, {68,-80}, {42,-80}},
        color={0, 0, 127}));
  connect(TLoaSet.y, heaPum.TSet)
    annotation(
      Line(
        points={{-68, 40}, {-26, 40}, {-26,-11}, {-1.4,-11}},
        color={0, 0, 127}));
  connect(heaPum.port_a1, pre.ports[1])
    annotation(
      Line(
        points={{0,-14}, {-32,-14}, {-32, 10}, {-50, 10}},
        color={0, 127, 255}));
  annotation(
    Icon(
      coordinateSystem(
