...
model Carnot_TCon_reverseFlow
  ...
  Modelica.Blocks.Sources.Constant TConLvg(
    k=273.15 + 40)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10,-10}, {10, 10}},
          origin={-50,-30})));
  Modelica.Blocks.Math.Gain mEva_flow(
    k=-1/cp2_default/dTEva_nominal)
...
equation
  ...
  connect(QEva_flow.y, mEva_flow.u)
    annotation(
      Line(
        points={{61,-40}, {76,-40}, {76,-60}, {20,-60}, {20,-80}, {38,-80}},
        color={0, 0, 127}));
  connect(heaPum.port_b1, sin1.ports[1])
    annotation(
      Line(
        points={{10, 6}, {30, 6}, {30, 30}, {40, 30}},
        color={0, 127, 255}));
  connect(mEva_flow.y, sou2.m_flow_in)
    annotation(
      Line(
        points={{61,-80}, {80,-80}, {80, 2}, {62, 2}},
        color={0, 0, 127}));
  connect(TConLvg.y, heaPum.TSet)
    annotation(
      Line(
        points={{-29, 40}, {-20, 40}, {-20, 9}, {-12, 9}},
        color={0, 0, 127}));
  connect(mCon_flow.y, sou1.m_flow_in)
    annotation(
      Line(
        points={{-69, 10}, {-60, 10}, {-60, 14}, {-54, 14}},
        color={0, 0, 127}));
  connect(heaPum.QCon_flow, QEva_flow.u1)
    annotation(
      Line(
        points={{11, 9}, {20, 9}, {20,-34}, {38,-34}},
        color={0, 0, 127}));
  connect(QEva_flow.u2, heaPum.P)
    annotation(
      Line(
        points={{38,-46}, {16,-46}, {16, 0}, {11, 0}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=3600),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatPumps/Validation/Carnot_TCon_reverseFlow.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
