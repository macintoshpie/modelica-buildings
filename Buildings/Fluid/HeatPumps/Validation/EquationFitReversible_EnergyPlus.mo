...
model EquationFitReversible_EnergyPlus
  ...
  Modelica.Blocks.Sources.CombiTimeTable datRea(
    tableOnFile=true,
    fileName=ModelicaServices.ExternalReferences.loadResource("modelica://Buildings//Resources/Data/Fluid/HeatPumps/Validation/EquationFitReversible_EnergyPlus/GSHPSimple-GLHE-ReverseHeatPump.dat"),
    verboseRead=false,
    ...
    annotation(
      Placement(
        transformation(
          extent={{-100, 60}, {-80, 80}})));
  Controls.OBC.CDL.Integers.Sources.Constant uMod(
    k=1)
  ...
  Buildings.Controls.OBC.UnitConversions.From_degC TSetHea
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60, 30}, {-40, 50}})));
  Modelica.Blocks.Sources.RealExpression QSou_flow_EP(
    y=datRea.y[2])
    ...
    annotation(
      Placement(
        transformation(
          extent={{-90,-62}, {-70,-42}})));
  Modelica.Blocks.Sources.RealExpression QLoa_flow_EP(
    y=-datRea.y[3])
    ...
    annotation(
      Placement(
        transformation(
          extent={{-90,-82}, {-70,-62}})));
  Modelica.Blocks.Sources.RealExpression P_EP(
    y=datRea.y[1])
    "EnergyPlus results: load side heat flow rate"
    annotation(
      Placement(
...
equation
  connect(heaPum.port_a1, loaPum.ports[1])
    annotation(
      Line(
        points={{30, 6}, {20, 6}, {20, 74}, {-1.77636e-15, 74}},
        color={0, 127, 255}));
  connect(souPum.ports[1], heaPum.port_a2)
    annotation(
      Line(
        points={{60,-20}, {50,-20}, {50,-6}},
        color={0, 127, 255}));
  connect(cooVol.ports[1], heaPum.port_b2)
    annotation(
      Line(
        points={{0,-20}, {20,-20}, {20,-6}, {30,-6}},
        color={0, 127, 255}));
  ...
  connect(TSouEnt.y, souPum.T_in)
    annotation(
      Line(
        points={{-38,-40}, {92,-40}, {92,-24}, {82,-24}},
        color={0, 0, 127}));
  connect(TSetHea.y, heaPum.TSet)
    annotation(
      Line(
        points={{-38, 40}, {8, 40}, {8, 9}, {28.6, 9}},
        color={0, 0, 127}));
  ...
  connect(datRea.y[4], TSetHea.u)
    annotation(
      Line(
        points={{-79, 70}, {-70, 70}, {-70, 40}, {-62, 40}},
        color={0, 0, 127}));
  ...
  annotation(
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatPumps/Validation/EquationFitReversible_EnergyPlus.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=86400),
