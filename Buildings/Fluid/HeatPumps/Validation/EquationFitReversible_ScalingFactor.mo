...
model EquationFitReversible_ScalingFactor
  ...
  Modelica.Blocks.Sources.RealExpression capErr(
    y=(heaPum1.QLoa_flow-scaling_factor*heaPum.QLoa_flow))
  ...
  Modelica.Blocks.Sources.RealExpression mSou1(
    y=mSou_flow_nominal*scaling_factor)
    "Source side mass flow rate"
    annotation(
      Placement(
        transformation(
  ...
  Modelica.Blocks.Sources.RealExpression mLoa1(
    y=mLoa_flow_nominal*scaling_factor)
    "Load side mass flwo rate"
    annotation(
      Placement(
        transformation(
...
equation
  ...
  connect(heaPum.port_b1, sin1.ports[1])
    annotation(
      Line(
        points={{10, 56}, {20, 56}, {20, 2}, {30, 2}},
        color={0, 127, 255}));
  connect(heaPum.port_a1, loa.ports[1])
    annotation(
      Line(
        points={{-10, 56}, {-24, 56}, {-24, 42}, {-40, 42}},
        color={0, 127, 255}));
  connect(heaPum.port_b2, sin2.ports[1])
    annotation(
      Line(
        points={{-10, 44}, {-20, 44}, {-20, 2}, {-40, 2}},
        color={0, 127, 255}));
  ...
  connect(TSou.y, sou1.T_in)
    annotation(
      Line(
        points={{71,-80}, {60,-80}, {60,-56}, {52,-56}},
        color={0, 0, 127}));
  connect(sou.T_in, sou1.T_in)
    annotation(
      Line(
        points={{48, 48}, {60, 48}, {60,-56}, {52,-56}},
        color={0, 0, 127}));
  connect(TLoa.y, loa1.T_in)
    annotation(
      Line(
        points={{-79, 0}, {-72, 0}, {-72,-44}, {-62,-44}},
        color={0, 0, 127}));
  connect(TLoa.y, loa.T_in)
    annotation(
      Line(
        points={{-79, 0}, {-72, 0}, {-72, 46}, {-62, 46}},
        color={0, 0, 127}));
  ...
  connect(TLoaEntHea.y, heaPum.TSet)
    annotation(
      Line(
        points={{-38, 80}, {-30, 80}, {-30, 60}, {-11.4, 60}, {-11.4, 59}},
        color={0, 0, 127}));
  connect(reaToInt.y, heaPum.uMod)
    annotation(
      Line(
        points={{-37,-80}, {-16,-80}, {-16, 50}, {-11, 50}},
        color={255, 127, 0}));
  connect(reaToInt.y, heaPum1.uMod)
    annotation(
      Line(
        points={{-37,-80}, {-16,-80}, {-16,-54}, {-11,-54}},
        color={255, 127, 0}));
  ...
  connect(TLoaEntHea.y, heaPum1.TSet)
    annotation(
      Line(
        points={{-38, 80}, {-30, 80}, {-30,-45}, {-11.4,-45}},
        color={0, 0, 127}));
  ...
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatPumps/Validation/EquationFitReversible_ScalingFactor.mos"
        "Simulate and plot"),
    experiment(
      StopTime=5000,
      Tolerance=1e-06),
