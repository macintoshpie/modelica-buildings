...
model EquationFitReversible_ScalingFactor
  ...
  Buildings.Fluid.Sources.Boundary_pT sin2(redeclare package Medium=Medium, nPorts=2)
    "Source side sink"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, origin={-50, 0})));
  Buildings.Fluid.Sources.Boundary_pT sin1(redeclare package Medium=Medium, nPorts=2)
    "Load side sink"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, origin={40, 0})));
  Modelica.Fluid.Sources.MassFlowSource_T loa(redeclare package Medium=Medium, m_flow=mLoa_flow_nominal, use_m_flow_in=true, use_T_in=true, nPorts=1)
    "Load side flow source"
    annotation(Placement(transformation(extent={{-60, 32}, {-40, 52}})));
  Modelica.Fluid.Sources.MassFlowSource_T sou(redeclare package Medium=Medium, m_flow=mSou_flow_nominal, use_m_flow_in=true, use_T_in=true, nPorts=1)
    "Source side flow source"
    annotation(Placement(transformation(extent={{46, 34}, {26, 54}})));
  EquationFitReversible heaPum(redeclare package Medium1=Medium, redeclare package Medium2=Medium, per=per, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
    "Equation fit reverse heat pump"
    annotation(Placement(transformation(extent={{-10, 40}, {10, 60}})));
  EquationFitReversible heaPum1(redeclare package Medium1=Medium, redeclare package Medium2=Medium, per=per, scaling_factor=scaling_factor, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
    "Scaled equation fit reverse heat pump"
    annotation(Placement(transformation(extent={{-10,-64}, {10,-44}})));
  Modelica.Fluid.Sources.MassFlowSource_T loa1(redeclare package Medium=Medium, m_flow=mLoa_flow_nominal, use_m_flow_in=true, use_T_in=true, nPorts=1)
    "Load side flow source"
    annotation(Placement(transformation(extent={{-60,-58}, {-40,-38}})));
  Modelica.Fluid.Sources.MassFlowSource_T sou1(redeclare package Medium=Medium, m_flow=mSou_flow_nominal, use_m_flow_in=true, use_T_in=true, nPorts=1)
    "Source side flow source"
  ...
  Modelica.Blocks.Sources.RealExpression capErr(y=(heaPum1.QLoa_flow-scaling_factor*heaPum.QLoa_flow))
  ...
  Controls.OBC.CDL.Continuous.Sources.Ramp TLoaEntHea(height=4, duration(displayUnit="h")=14400, offset=50 + 273.15, startTime=0)
    "Load side entering water temperature in heating mode"
    annotation(Placement(transformation(extent={{-60, 70}, {-40, 90}})));
  Modelica.Blocks.Sources.Pulse uMod(amplitude=1, width=70, period=500, offset=0, startTime=0)
    "Heat pump signal to operate in heating mode"
    annotation(Placement(transformation(extent={{-100,-90}, {-80,-70}})));
  Modelica.Blocks.Sources.RealExpression mSou1(y=mSou_flow_nominal*scaling_factor)
    "Source side mass flow rate"
    annotation(Placement(transformation(extent={{90,-62}, {70,-42}})));
  Modelica.Blocks.Sources.RealExpression mLoa1(y=mLoa_flow_nominal*scaling_factor)
    "Load side mass flwo rate"
...
equation
  ...
  connect(TLoaEntHea.y, heaPum.TSet)
    annotation(Line(points={{-38, 80}, {-30, 80}, {-30, 60}, {-11.4, 60}, {-11.4, 59}}, color={0, 0, 127}));
  ...
  connect(TLoaEntHea.y, heaPum1.TSet)
    annotation(Line(points={{-38, 80}, {-30, 80}, {-30,-45}, {-11.4,-45}}, color={0, 0, 127}));
  ...
  annotation(__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatPumps/Validation/EquationFitReversible_ScalingFactor.mos"
    "Simulate and plot"), experiment(StopTime=5000, Tolerance=1e-06), Documentation(info="<html>
<p>
...
</html>"), Diagram(coordinateSystem(extent={{-120,-120}, {120, 120}})), Icon(coordinateSystem(extent={{-100,-100}, {100, 100}})));
