  ...
  Modelica.Fluid.Sources.MassFlowSource_T sou(
    ...
    annotation(
      Placement(
        transformation(
          extent={{60, 36}, {40, 56}})));
  Modelica.Blocks.Sources.RealExpression mLoa(
    y=flowLoad*scaling_factor)
    "Load side mass flwo rate"
    annotation(
      Placement(
        transformation(
          extent={{-100,-50}, {-80,-30}})));
  Modelica.Blocks.Sources.RealExpression mSou(
    y=flowSource*scaling_factor)
  ...
  Buildings.Fluid.HeatPumps.ReciprocatingWaterToWater heaPum1(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10,-64}, {10,-44}})));
  Modelica.Blocks.Sources.Pulse N(
    width=60,
    period=500)
  ...
  Modelica.Fluid.Sources.MassFlowSource_T sou1(
    ...
    annotation(
      Placement(
        transformation(
          extent={{60,-70}, {40,-50}})));
  Modelica.Blocks.Sources.Constant TLoa(
    k=285.15)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-102,-68}, {-82,-48}})));
  Modelica.Blocks.Sources.Constant TSou(
    k=283.15)
    ...
    annotation(
      Placement(
        transformation(
          extent={{100,-90}, {80,-70}})));
  Modelica.Blocks.Sources.RealExpression capErr(
    y=(heaPum1.QCon_flow-scaling_factor*heaPum.QCon_flow))
...
equation
  ...
  connect(mSou.y, sou1.m_flow_in)
    annotation(
      Line(
        points={{79,-52}, {74,-52}, {60,-52}},
        color={0, 0, 127}));
  connect(TSou.y, sou1.T_in)
    annotation(
      Line(
        points={{79,-80}, {70,-80}, {70,-56}, {62,-56}},
        color={0, 0, 127}));
  connect(sou.T_in, sou1.T_in)
    annotation(
      Line(
        points={{62, 50}, {70, 50}, {70,-56}, {62,-56}},
        color={0, 0, 127}));
  connect(TLoa.y, loa1.T_in)
    annotation(
      Line(
        points={{-81,-58}, {-70,-58}, {-70,-44}, {-62,-44}},
        color={0, 0, 127}));
  connect(TLoa.y, loa.T_in)
    annotation(
      Line(
        points={{-81,-58}, {-70,-58}, {-70, 62}, {-62, 62}},
        color={0, 0, 127}));
  connect(loa1.ports[1], heaPum1.port_a1)
    annotation(
      Line(
        points={{-40,-48}, {-26,-48}, {-10,-48}},
        color={0, 127, 255}));
  connect(heaPum1.port_b1, sin1.ports[2])
    annotation(
      Line(
        points={{10,-48}, {20,-48}, {20,-2}, {40,-2}},
        color={0, 127, 255}));
  ...
  connect(realToInteger.y, heaPum.stage)
    annotation(
      Line(
        points={{-19, 80}, {-16, 80}, {-16, 55}, {-12, 55}},
        color={255, 127, 0}));
  connect(realToInteger.y, heaPum1.stage)
    annotation(
      Line(
        points={{-19, 80}, {-16, 80}, {-16,-51}, {-12,-51}},
        color={255, 127, 0}));
  connect(mLoa1.y, loa.m_flow_in)
    annotation(
      Line(
        points={{-79, 40}, {-74, 40}, {-74, 66}, {-60, 66}},
        color={0, 0, 127}));
  connect(mSou1.y, sou.m_flow_in)
    annotation(
      Line(
        points={{79, 54}, {69.5, 54}, {60, 54}},
        color={0, 0, 127}));
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatPumps/Validation/ReciprocatingWaterToWater_ScalingFactor.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=1000),
