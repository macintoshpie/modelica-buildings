...
equation
  connect(mSou.y, sou.m_flow_in)
    annotation(
      Line(
        points={{79, 2}, {79, 2}, {68, 2}},
        color={0, 0, 127}));
  connect(mLoa.y, loa.m_flow_in)
    annotation(
      Line(
        points={{-79, 28}, {-79, 28}, {-66, 28}},
        color={0, 0, 127}));
  connect(N.y, heaPum.y)
    annotation(
      Line(
        points={{-39.4,-20}, {-32,-20}, {-32, 3}, {-12, 3}},
        color={0, 0, 127}));
  connect(TSou.y, sou.T_in)
    annotation(
      Line(
        points={{79,-28}, {76,-28}, {72,-28}, {72,-2}, {70,-2}},
        color={0, 0, 127}));
  connect(TLoa.y, loa.T_in)
    annotation(
      Line(
        points={{-79, 0}, {-79, 0}, {-74, 0}, {-74, 24}, {-68, 24}},
        color={0, 0, 127}));
  connect(sin2.ports[1], heaPum.port_b2)
    annotation(
      Line(
        points={{-60,-40}, {-20,-40}, {-20,-6}, {-10,-6}},
        color={0, 127, 255}));
  connect(heaPum.port_a1, loa.ports[1])
    annotation(
      Line(
        points={{-10, 6}, {-20, 6}, {-20, 20}, {-46, 20}},
        color={0, 127, 255}));
  connect(heaPum.port_b1, sin1.ports[1])
    annotation(
      Line(
        points={{10, 6}, {20, 6}, {20, 20}, {48, 20}},
        color={0, 127, 255}));
  ...
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/HeatPumps/Validation/ReciprocatingWaterToWater_VariableSpeed.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=2000),
