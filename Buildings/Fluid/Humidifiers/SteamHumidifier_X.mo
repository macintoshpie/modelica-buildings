...
model SteamHumidifier_X
  ...
  extends Buildings.Fluid.HeatExchangers.BaseClasses.PartialPrescribedOutlet(outCon(final T_start=293.15, final X_start=X_start, final use_TSet=true, final use_X_wSet=true, final QMax_flow=Modelica.Constants.inf, final QMin_flow=-Modelica.Constants.inf, final mWatMax_flow=mWatMax_flow, final mWatMin_flow=0, final energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, final massDynamics=massDynamics));
...
protected
  ...
  Modelica.SIunits.SpecificEnthalpy hLea=inStream(port_a.h_outflow) + {hSte}*(port_b.Xi_outflow-inStream(port_a.Xi_outflow))
    "Approximation of leaving enthalpy, based on dh/dx=h_fg";
  Modelica.Blocks.Sources.RealExpression TLea(y=Medium.temperature_phX(p=port_b.p, h=hLea, X=port_b.Xi_outflow))
