...
protected
  parameter Modelica.SIunits.SpecificHeatCapacity cp_default=Medium.specificHeatCapacityCp(Medium.setState_pTX(p=Medium.p_default, T=Medium.T_default, X=Medium.X_default))
...
initial equation
  ...
  assert((energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState) or tau > Modelica.Constants.eps, "The parameter tau, or the volume of the model from which tau may be derived, is unreasonably small.
 You need to set energyDynamics == Modelica.Fluid.Types.Dynamics.SteadyState to model steady-state.
 Received tau = " + String(tau) + "\n");
  assert((massDynamics == Modelica.Fluid.Types.Dynamics.SteadyState) or tau > Modelica.Constants.eps, "The parameter tau, or the volume of the model from which tau may be derived, is unreasonably small.
...
equation
  ...
  m_flow_pos=Buildings.Utilities.Math.Functions.smoothMax(x1=m_flow, x2=0, deltaX=m_flow_small);
  if not restrictHeat and not restrictCool and not restrictHumi and not restrictDehu then
    ...
    m_flow_non_zero=Buildings.Utilities.Math.Functions.smoothMax(x1=port_a.m_flow, x2=m_flow_small, deltaX=m_flow_small/2);
  end if;
  ...
  if use_X_wSet then
    if not restrictHumi and not restrictDehu then
      ...
      if restrictHumi and restrictDehu then
        ...
        dXiAct=Buildings.Utilities.Math.Functions.smoothLimit(x=Xi-sum(Xi_instream), l=mWatMin_flow/m_flow_non_zero, u=mWatMax_flow/m_flow_non_zero, deltaX=deltaXi);
      elseif restrictHumi then
        // Capacity limit for heating only
        dXiAct=Buildings.Utilities.Math.Functions.smoothMin(x1=Xi-sum(Xi_instream), x2=mWatMax_flow/m_flow_non_zero, deltaX=deltaXi);
      else
        // Capacity limit for cooling only
        dXiAct=Buildings.Utilities.Math.Functions.smoothMax(x1=Xi-sum(Xi_instream), x2=mWatMin_flow/m_flow_non_zero, deltaX=deltaXi);
      end if;
  ...
  if use_TSet then
    if not restrictHeat and not restrictCool then
      ...
      if restrictHeat and restrictCool then
        // Capacity limits for heating and cooling
        dhAct=Buildings.Utilities.Math.Functions.smoothLimit(x=hSet-inStream(port_a.h_outflow), l=QMin_flow/m_flow_non_zero, u=QMax_flow/m_flow_non_zero, deltaX=deltaH);
      elseif restrictHeat then
        // Capacity limit for heating only
        dhAct=Buildings.Utilities.Math.Functions.smoothMin(x1=hSet-inStream(port_a.h_outflow), x2=QMax_flow/m_flow_non_zero, deltaX=deltaH);
      else
        // Capacity limit for cooling only
        dhAct=Buildings.Utilities.Math.Functions.smoothMax(x1=hSet-inStream(port_a.h_outflow), x2=QMin_flow/m_flow_non_zero, deltaX=deltaH);
      end if;
  ...
  if not allowFlowReversal then
    assert(m_flow >-m_flow_small, "Reverting flow occurs even though allowFlowReversal is false");
  end if;
  annotation(defaultComponentName="heaCoo", Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 100}}), graphics={Rectangle(extent={{-99, 6}, {102,-4}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 255}, fillPattern=FillPattern.Solid), Rectangle(extent={{2,-4}, {102, 6}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={255, 0, 0}, fillPattern=FillPattern.Solid), Text(extent={{-98, 64}, {-76, 42}}, lineColor={0, 0, 127}, visible=use_X_wSet, textString="X_w"), Text(extent={{74, 72}, {120, 44}}, lineColor={0, 0, 127}, textString="mWat_flow"), Rectangle(extent={{-70, 60}, {70,-60}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={95, 95, 95}, fillPattern=FillPattern.Solid), Rectangle(extent={{-64, 34}, {-34, 54}}, lineColor={0, 0, 0}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Line(points={{-64, 34}, {-52, 44}, {-64, 54}}, color={0, 0, 0}), Rectangle(extent={{-70, 60}, {-66, 82}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 127}, visible=use_TSet, fillPattern=FillPattern.Solid), Rectangle(extent={{66, 60}, {70, 82}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 127}, fillPattern=FillPattern.Solid), Text(extent={{-106, 102}, {-74, 88}}, lineColor={0, 0, 127}, visible=use_TSet, textString="T"), Rectangle(extent={{-100, 82}, {-70, 78}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 127}, visible=use_TSet, fillPattern=FillPattern.Solid), Text(extent={{72, 108}, {120, 92}}, lineColor={0, 0, 127}, textString="Q_flow"), Rectangle(extent={{70, 82}, {100, 78}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 127}, fillPattern=FillPattern.Solid), Rectangle(extent={{-100, 41}, {-70, 38}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 127}, visible=use_X_wSet, fillPattern=FillPattern.Solid), Rectangle(extent={{70, 41}, {100, 38}}, lineColor={0, 0, 255}, pattern=LinePattern.None, fillColor={0, 0, 127}, fillPattern=FillPattern.Solid)}), Documentation(info="<html>
<p>
