...
model PrescribedOutlet
  ...
  parameter Modelica.SIunits.MassFraction X_start[Medium.nX]=Medium.X_default
    ...
    annotation(
      Dialog(
        tab="Initialization",
        enable=use_X_wSet and Medium.nXi > 0));
  // Dynamics
...
protected
  parameter Modelica.SIunits.SpecificHeatCapacity cp_default=Medium.specificHeatCapacityCp(Medium.setState_pTX(
    p=Medium.p_default,
    T=Medium.T_default,
    X=Medium.X_default))
...
equation
  ...
  hSet=if use_TSet then Medium.specificEnthalpy(Medium.setState_pTX(
    p=port_a.p,
    T=T,
    X=Xi_instream + fill(dXiAct, Medium.nXi))) else Medium.h_default;
  ...
  annotation(
    ...
    Icon(
      ...
      graphics={Rectangle(
        ...
        extent={{74, 72}, {120, 44}},
        lineColor={0, 0, 127},
        textString="mWat_flow"), Rectangle(
        extent={{-70, 60}, {70,-60}},
        lineColor={0, 0, 255},
        pattern=LinePattern.None,
        fillColor={95, 95, 95},
