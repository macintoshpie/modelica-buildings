...
model ConstantEffectiveness
  ...
  parameter Modelica.SIunits.Efficiency epsS(
    max=1)=0.8
    "Sensible heat exchanger effectiveness";
  parameter Modelica.SIunits.Efficiency epsL(
    max=1)=0.8
...
protected
  parameter Integer i1_w(
    min=1,
    fixed=false)
    "Index for water substance";
  parameter Integer i2_w(
    min=1,
    fixed=false)
    "Index for water substance";
  Real gai1(
...
initial algorithm
  ...
  for i in 1 : Medium1.nXi loop
    if Modelica.Utilities.Strings.isEqual(
      string1=Medium1.substanceNames[i],
      string2="Water",
  ...
  for i in 1 : Medium2.nXi loop
    if Modelica.Utilities.Strings.isEqual(
      string1=Medium2.substanceNames[i],
      string2="Water",
      caseSensitive=false) then
      i2_w := i;
    end if;
