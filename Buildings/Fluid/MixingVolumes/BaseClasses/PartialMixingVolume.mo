...
protected
  Buildings.Fluid.Interfaces.StaticTwoPortConservationEquation steBal(
    ...
    final prescribedHeatFlowRate=prescribedHeatFlowRate) if useSteadyStateTwoPort
...
");
  ...
  if useSteadyStateTwoPort then
    ...
    connect(dynBal.ports, ports)
      annotation(
        Line(
          points={{70, 0}, {70,-80}, {62,-80}, {2.22045e-15,-80}, {2.22045e-15,-90}, {2.22045e-15,-100}},
          color={0, 127, 255}));
  ...
  connect(heaFloSen.port_b, preTem.port)
    annotation(
      Line(
        points={{-70, 0}, {-65, 0}, {-60, 0}},
        color={191, 0, 0}));
  connect(heaFloSen.Q_flow, steBal.Q_flow)
    annotation(
      Line(
        points={{-80,-10}, {-80,-16}, {6,-16}, {6, 18}, {18, 18}},
        color={0, 0, 127}));
  connect(heaFloSen.Q_flow, dynBal.Q_flow)
    annotation(
      Line(
        points={{-80,-10}, {-80,-10}, {-80,-16}, {6,-16}, {6, 24}, {50, 24}, {50, 16}, {58, 16}},
        color={0, 0, 127}));
  annotation(
...
Buildings.Fluid.MixingVolumes.BaseClasses.ClosedVolume</a>.
    ...
    Icon(
      ...
      graphics={Text(
        ...
        fillColor=DynamicSelect({170, 213, 255}, min(1, max(0,(1-(T-273.15)/50)))*{28, 108, 200} + min(1, max(0,(T-273.15)/50))*{255, 0, 0})), Text(
        extent={{62, 28}, {-58,-22}},
        lineColor={255, 255, 255},
