...
model MixingVolumeHeatPortWater
  ...
  replaceable package Medium=Buildings.Media.Water constrainedby Modelica.Media.Interfaces.PartialMedium
    "Medium model";
  parameter Integer nEle(min=2)=3
    "Number of volumes"
    annotation(Evaluate=true);
  replaceable Buildings.Fluid.MixingVolumes.BaseClasses.MixingVolumeHeatPort vol[nEle] constrainedby Buildings.Fluid.MixingVolumes.BaseClasses.MixingVolumeHeatPort(redeclare each package Medium=Medium, each energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, final initialize_p={(i == 1 and not Medium.singleState) for i in 1 : nEle}, each m_flow_nominal=1, each V=1, each nPorts=2)
    "Mixing volume"
    annotation(Placement(transformation(extent={{-10, 20}, {10, 40}})));
  Buildings.Fluid.FixedResistances.PressureDrop res(redeclare package Medium=Medium, m_flow_nominal=1, dp_nominal=1000)
    "Pressure drop"
  ...
  Buildings.Fluid.FixedResistances.PressureDrop res1(redeclare package Medium=Medium, m_flow_nominal=1, dp_nominal=1000)
    "Pressure drop"
    annotation(Placement(transformation(extent={{40,-10}, {20, 10}})));
  Buildings.Fluid.Sources.Boundary_pT bou1(redeclare package Medium=Medium, nPorts=1)
    "Boundary condition"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=180, origin={70, 0})));
equation
  ...
  connect(res1.port_b, vol[nEle].ports[2])
    annotation(Line(points={{20, 0}, {0, 0}, {0, 20}}, color={0, 127, 255}));
  connect(bou1.ports[1], res1.port_a)
    annotation(Line(points={{60, 1.33227e-15}, {46, 1.33227e-15}, {46, 0}, {40, 0}}, color={0, 127, 255}));
...
</html>"), experiment(Tolerance=1E-6, StopTime=1.0), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/MixingVolumes/BaseClasses/Validation/MixingVolumeHeatPortWater.mos"
    "Simulate and plot"));
