...
model MixingVolumeMoistAir
  ...
  Buildings.Utilities.Psychrometrics.TDewPoi_pW dewPoi
    ...
    annotation(
      Placement(
        transformation(
          extent={{12,-120}, {32,-100}})));
  Modelica.Thermal.HeatTransfer.Sensors.HeatFlowSensor heatFlowSensor
    annotation(
      Placement(
        transformation(
          extent={{64, 120}, {84, 140}})));
  Modelica.Blocks.Continuous.Integrator QSen
    "Sensible heat transfer"
    annotation(
      Placement(
        transformation(
          extent={{140, 100}, {160, 120}})));
  ...
  Buildings.Fluid.Sensors.MassFlowRate mOut_flow(
    ...
    annotation(
      Placement(
        transformation(
          extent={{84,-10}, {104, 10}})));
  Modelica.Blocks.Math.Add dM_flow(
    k2=-1)
    annotation(
      Placement(
        transformation(
...
equation
  connect(preHeaFlo.port, heatFlowSensor.port_a)
    annotation(
      Line(
        points={{56, 130}, {64, 130}},
        color={191, 0, 0}));
  connect(heatFlowSensor.Q_flow, QSen.u)
    annotation(
      Line(
        points={{74, 120}, {74, 110}, {138, 110}},
        color={0, 0, 127}));
  ...
  connect(mOut_flow.m_flow, dM_flow.u1)
    annotation(
      Line(
        points={{94, 11}, {94, 36}, {138, 36}},
        color={0, 0, 127}));
  connect(mIn_flow.m_flow, dM_flow.u2)
    annotation(
      Line(
        points={{-30, 11}, {-30, 12}, {-30, 12}, {-30, 12}, {-30, 24}, {138, 24}},
        color={0, 0, 127}));
  connect(gai.y, preHeaFlo.Q_flow)
    annotation(
      Line(
        points={{23, 130}, {36, 130}},
        color={0, 0, 127}));
  ...
  connect(gai1.y, vol1.mWat_flow)
    annotation(
      Line(
        points={{1,-50}, {14,-50}, {14, 18}, {18, 18}},
        color={0, 0, 127}));
  connect(vol1.X_w, PI1.u_m)
    annotation(
      Line(
        points={{42, 6}, {46, 6}, {46,-134}, {-40,-134}, {-40,-62}},
        color={0, 0, 127}));
  connect(vol1.X_w, humRat.X_w)
    annotation(
      Line(
        points={{42, 6}, {46, 6}, {46,-134}, {-40,-134}, {-40,-110}, {-21,-110}},
        color={0, 0, 127}));
  ...
  connect(vol1.ports[2], senLatEnt1.port_a)
    annotation(
      Line(
        points={{32, 0}, {32, 0}, {50, 0}},
        color={0, 127, 255}));
  connect(senLatEnt1.port_b, mOut_flow.port_a)
    annotation(
      Line(
        points={{70, 0}, {70, 0}, {84, 0}},
        color={0, 127, 255}));
  connect(add.y, QLat.u)
    annotation(
      Line(
        points={{121, 70}, {130, 70}, {138, 70}},
        color={0, 0, 127}));
  connect(add.u1, senLatEnt.H_flow)
    annotation(
      Line(
        points={{98, 76}, {52, 76}, {0, 76}, {0, 11}},
        color={0, 0, 127}));
  connect(senLatEnt1.H_flow, add.u2)
    annotation(
      Line(
        points={{60, 11}, {60, 11}, {60, 62}, {60, 64}, {98, 64}},
        color={0, 0, 127}));
  annotation(
    Diagram(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-100,-160}, {180, 160}})),
    experiment(
      StopTime=600,
      Tolerance=1E-6),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/MixingVolumes/Examples/MixingVolumeMoistAir.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
