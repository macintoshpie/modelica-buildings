    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=180,
          origin={70,-20})));
  replaceable Buildings.Fluid.MixingVolumes.MixingVolume volDyn constrainedby Buildings.Fluid.MixingVolumes.BaseClasses.MixingVolumeHeatPort(
    redeclare package Medium=Medium,
    V=1,
    nPorts=2,
    ...
    annotation(
      Placement(
        transformation(
          extent={{10, 0}, {30, 20}})));
  replaceable Buildings.Fluid.MixingVolumes.MixingVolume volSte constrainedby Buildings.Fluid.MixingVolumes.BaseClasses.MixingVolumeHeatPort(
    redeclare package Medium=Medium,
...
equation
  connect(sou.ports[1], volDyn.ports[1])
    annotation(
      Line(
        points={{-20, 6.66134e-16}, {-6, 6.66134e-16}, {-6,-5.55112e-16}, {20,-5.55112e-16}},
        color={0, 127, 255}));
  ...
  connect(volSte.ports[2], bou.ports[1])
    annotation(
      Line(
        points={{20,-50}, {20,-60}, {40,-60}, {40,-22}, {60,-22}},
        color={0, 127, 255}));
  connect(volDyn.ports[2], bou.ports[2])
    annotation(
      Line(
        points={{20, 0}, {20,-10}, {40,-10}, {40,-18}, {60,-18}},
        color={0, 127, 255}));
  connect(sou.m_flow_in, m_flow.y)
    annotation(
      Line(
        points={{-40, 8}, {-48, 8}, {-48, 0}, {-59, 0}},
        color={0, 0, 127}));
  connect(m_flow.y, sou1.m_flow_in)
    annotation(
      Line(
        points={{-59, 0}, {-48, 0}, {-48,-42}, {-40,-42}},
        color={0, 0, 127}));
  connect(m_flow.y, gain.u)
    annotation(
      Line(
        points={{-59, 0}, {-48, 0}, {-48, 40}, {-42, 40}},
        color={0, 0, 127}));
  annotation(
    Documentation(
      info="<html>
