...
model MixingVolumeAdiabaticCooling
  ...
  MixingVolumeMoistAir volExa(
    ...
    annotation(
      Placement(
        transformation(
          extent={{20,-38}, {40,-18}})));
  Modelica.Blocks.Sources.Constant mWatFlo(
    k=0.001)
  ...
  Modelica.Blocks.Sources.RealExpression relErr(
    y=(volSim.heatPort.T-20)/(if abs(volExa.heatPort.T-20) < 1E-5 then 1 else(volExa.heatPort.T-20)))
...
equation
  connect(volSim.mWat_flow, mWatFlo.y)
    annotation(
      Line(
        points={{18, 28}, {18, 28}, {-16, 28}, {-16, 80}, {-39, 80}},
        color={0, 0, 127}));
  connect(volExa.mWat_flow, mWatFlo.y)
    annotation(
      Line(
        points={{18,-20}, {18,-20}, {-16,-20}, {-16, 80}, {-39, 80}},
        color={0, 0, 127}));
  connect(bou.ports[1], volSim.ports[1])
    annotation(
      Line(
        points={{70, 2}, {30, 2}, {30, 10}},
        color={0, 127, 255}));
  connect(bou.ports[2], volExa.ports[1])
    annotation(
      Line(
        points={{70,-2}, {66,-2}, {66,-2}, {58,-2}, {58,-48}, {30,-48}, {30,-38}},
        color={0, 127, 255}));
  annotation(
    Documentation(
      info="<html>
...
First implementation.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/MixingVolumes/Validation/MixingVolumeAdiabaticCooling.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=10));
