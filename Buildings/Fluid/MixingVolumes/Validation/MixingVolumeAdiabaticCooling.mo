...
model MixingVolumeAdiabaticCooling
  ...
  package Medium=Buildings.Media.Air
    "Medium model";
  MixingVolumeMoistAir volSim(redeclare package Medium=Medium, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, m_flow_nominal=1, V=1, final simplify_mWat_flow=true, nPorts=1)
    "Mixing volume with water vapor input and simplified water balance"
    annotation(Placement(transformation(extent={{20, 10}, {40, 30}})));
  MixingVolumeMoistAir volExa(redeclare package Medium=Medium, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, m_flow_nominal=1, V=1, final simplify_mWat_flow=false, nPorts=1)
    "Mixing volume with water vapor input and exact water balance"
    annotation(Placement(transformation(extent={{20,-38}, {40,-18}})));
  Modelica.Blocks.Sources.Constant mWatFlo(k=0.001)
    "Water mass flow rate "
    annotation(Placement(transformation(extent={{-60, 70}, {-40, 90}})));
  Modelica.Blocks.Sources.RealExpression relErr(y=(volSim.heatPort.T-20)/(if abs(volExa.heatPort.T-20) < 1E-5 then 1 else(volExa.heatPort.T-20)))
...
equation
  connect(volSim.mWat_flow, mWatFlo.y)
    annotation(Line(points={{18, 28}, {18, 28}, {-16, 28}, {-16, 80}, {-39, 80}}, color={0, 0, 127}));
  connect(volExa.mWat_flow, mWatFlo.y)
    annotation(Line(points={{18,-20}, {18,-20}, {-16,-20}, {-16, 80}, {-39, 80}}, color={0, 0, 127}));
  ...
  connect(bou.ports[2], volExa.ports[1])
    annotation(Line(points={{70,-2}, {66,-2}, {66,-2}, {58,-2}, {58,-48}, {30,-48}, {30,-38}}, color={0, 127, 255}));
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/MixingVolumes/Validation/MixingVolumeAdiabaticCooling.mos"
