...
model MixingVolumeHeatReverseFlow
  ...
  extends Buildings.Fluid.MixingVolumes.Validation.BaseClasses.MixingVolumeReverseFlow(
    gain(
      k=10));
...
equation
  connect(gain.y, preHeaDyn.Q_flow)
    annotation(
      Line(
        points={{-19, 40}, {-12, 40}, {-12, 60}, {0, 60}},
        color={0, 0, 127}));
  connect(gain.y, preHeaSte.Q_flow)
    annotation(
      Line(
        points={{-19, 40}, {-9.5, 40}, {0, 40}},
        color={0, 0, 127}));
  connect(preHeaDyn.port, volDyn.heatPort)
    annotation(
      Line(
        points={{20, 60}, {30, 60}, {30, 26}, {0, 26}, {0, 10}, {10, 10}},
        color={191, 0, 0}));
  connect(preHeaSte.port, volSte.heatPort)
    annotation(
      Line(
        points={{20, 40}, {26, 40}, {26, 28}, {-2, 28}, {-2,-40}, {10,-40}},
        color={191, 0, 0}));
  annotation(
    Documentation(
      info="<html>
...
First implementation.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/MixingVolumes/Validation/MixingVolumeHeatReverseFlow.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-08,
      StopTime=10));
