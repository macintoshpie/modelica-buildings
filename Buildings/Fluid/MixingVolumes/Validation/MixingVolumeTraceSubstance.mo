  ...
  Buildings.Fluid.MixingVolumes.MixingVolume volDyn(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10, 0}, {10, 20}})));
  Modelica.Blocks.Sources.Constant C_flow(
...
equation
  ...
  connect(C_flow.y, volSte.C_flow[1])
    annotation(
      Line(
        points={{-39, 40}, {-30, 40}, {-30,-46}, {-12,-46}},
        color={0, 0, 127}));
  connect(volSte.ports[2], bou.ports[1])
    annotation(
      Line(
        points={{2,-50}, {2,-60}, {20,-60}, {20,-22}, {40,-22}},
        color={0, 127, 255}));
  connect(volDyn.ports[2], bou.ports[2])
    annotation(
      Line(
        points={{2, 0}, {2,-10}, {20,-10}, {20,-18}, {40,-18}},
        color={0, 127, 255}));
  annotation(
    Documentation(
      info="<html>
...
First implementation.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/MixingVolumes/Validation/MixingVolumeTraceSubstance.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=10));
