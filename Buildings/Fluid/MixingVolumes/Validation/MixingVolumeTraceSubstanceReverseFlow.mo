...
model MixingVolumeTraceSubstanceReverseFlow
  ...
  extends Buildings.Fluid.MixingVolumes.Validation.BaseClasses.MixingVolumeReverseFlow(Medium(extraPropertiesNames={"CO2"}), volDyn(use_C_flow=true), volSte(use_C_flow=true), gain(k=1/1000), bou(C={0.003}));
equation
  connect(gain.y, volSte.C_flow[1])
    annotation(Line(points={{-19, 40}, {-10, 40}, {-10,-46}, {8,-46}}, color={0, 0, 127}));
  connect(gain.y, volDyn.C_flow[1])
    annotation(Line(points={{-19, 40}, {-10, 40}, {-10, 4}, {8, 4}}, color={0, 0, 127}));
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/MixingVolumes/Validation/MixingVolumeTraceSubstanceReverseFlow.mos"
