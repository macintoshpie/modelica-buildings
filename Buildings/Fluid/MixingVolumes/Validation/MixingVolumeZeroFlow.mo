...
model MixingVolumeZeroFlow
  ...
  Modelica.Blocks.Sources.RealExpression reaExp(
    y=(290-volNonLinSys.heatPort.T)/100*volNonLinSys.heatPort.T^(3/5)*time)
  ...
  Buildings.Fluid.Sources.MassFlowSource_T sou4(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-60,-100}, {-40,-80}})));
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor theRes(
    R=0.001)
...
equation
  ...
  connect(sou1.ports[1], volQflow.ports[1])
    annotation(
      Line(
        points={{-40, 10}, {-28, 10}, {-2, 10}},
        color={0, 127, 255}));
  connect(volQflow.ports[2], sin.ports[4])
    annotation(
      Line(
        points={{2, 10}, {20, 10}, {20,-37}},
        color={0, 127, 255}));
  ...
  connect(ramp_m_flow.y, sou2.m_flow_in)
    annotation(
      Line(
        points={{-79,-10}, {-72,-10}, {-72,-14}, {-62,-14}},
        color={0, 0, 127}));
  connect(ramp_m_flow.y, sou3.m_flow_in)
    annotation(
      Line(
        points={{-79,-10}, {-72,-10}, {-72,-42}, {-62,-42}},
        color={0, 0, 127}));
  connect(ramp_m_flow.y, sou4.m_flow_in)
    annotation(
      Line(
        points={{-79,-10}, {-72,-10}, {-72,-82}, {-62,-82}},
        color={0, 0, 127}));
  connect(ramp_T.y, sou1.T_in)
    annotation(
      Line(
        points={{-79,-50}, {-74,-50}, {-68,-50}, {-68, 14}, {-62, 14}},
        color={0, 0, 127}));
  connect(ramp_T.y, sou2.T_in)
    annotation(
      Line(
        points={{-79,-50}, {-68,-50}, {-68,-18}, {-62,-18}},
        color={0, 0, 127}));
  connect(ramp_T.y, sou3.T_in)
    annotation(
      Line(
        points={{-79,-50}, {-68,-50}, {-68,-46}, {-62,-46}},
        color={0, 0, 127}));
  connect(ramp_T.y, sou4.T_in)
    annotation(
      Line(
        points={{-79,-50}, {-68,-50}, {-68,-86}, {-62,-86}},
        color={0, 0, 127}));
  connect(reaPasThr.y, preHea1.Q_flow)
    annotation(
      Line(
        points={{-53.4, 80}, {-40, 80}},
        color={0, 0, 127}));
  connect(reaPasThr.u, ramp_m_flow.y)
    annotation(
      Line(
        points={{-67.2, 80}, {-79, 80}, {-79,-10}},
        color={0, 0, 127}));
  annotation(
    Diagram(
      coordinateSystem(
...
increase the chance of the error being produced for this simple example.
    ...
    __Dymola_Commands(
      file="Resources/Scripts/Dymola/Fluid/MixingVolumes/Validation/MixingVolumeZeroFlow.mos"
        "Simulate and plot"));
