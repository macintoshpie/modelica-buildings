...
model MoistureMixingConservationDynamicBalance
  ...
  extends Buildings.Fluid.MixingVolumes.Validation.BaseClasses.MoistureMixingConservation(
    mWatFloSol(
      k={vol.X_start[1], vol1.X_start[1], vol2.X_start[1]}*m_start),
    vol(
  ...
  Modelica.Blocks.Math.Add3 add
    ...
    annotation(
      Placement(
        transformation(
          extent={{-40,-60}, {-20,-80}})));
  Modelica.Blocks.Sources.RealExpression masVapVol(
    y=vol.mXi[1] + vol1.mXi[1] + vol2.mXi[1])
    ...
    annotation(
      Placement(
        transformation(
          extent={{-32,-10}, {32, 10}},
          origin={-32,-50})));
  Modelica.Blocks.Math.Add3 add3Vap(
    k3=-1)
    ...
    annotation(
      Placement(
        transformation(
          extent={{60,-60}, {80,-40}})));
  Modelica.Blocks.Sources.RealExpression masVol(
    y=vol1.m + vol.m + vol2.m)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10, 10}, {10,-10}},
          origin={60,-100})));
  Modelica.Blocks.Math.Add3 add3Mass(
    k3=-1)
    "Adding 3 mass streams"
    annotation(
      Placement(
        transformation(
          extent={{86,-140}, {106,-120}})));
  Modelica.Blocks.Continuous.Integrator intMasFloIn(
    ...
    annotation(
      Placement(
        transformation(
          extent={{0,-160}, {20,-140}})));
  Modelica.Blocks.Sources.Constant masFloIn(
    k=sou1.m_flow + sou2.m_flow)
  ...
  Modelica.Blocks.Continuous.Integrator intEntIn(
    ...
    annotation(
      Placement(
        transformation(
          extent={{0,-242}, {20,-222}})));
  Modelica.Blocks.Sources.RealExpression entVol(
    y=vol1.U + vol.U + vol2.U)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-40,-10}, {40, 10}},
          origin={-40,-212})));
  Modelica.Blocks.Math.Add3 add3Ent(
    k3=-1)
    "Adding 3 enthalpy streams"
    annotation(
      Placement(
        transformation(
          extent={{80,-214}, {100,-194}})));
...
equation
  ...
  connect(add3Ent.u1, intEntOut.y)
    annotation(
      Line(
        points={{78,-196}, {72,-196}, {72,-180}, {64,-180}, {62,-180}, {62,-180}, {61,-180}},
        color={0, 0, 127},
        smooth=Smooth.None));
  ...
  connect(intMasVapOut.u, pro.y)
    annotation(
      Line(
        points={{102,-30}, {110,-30}, {110,-21}},
        color={0, 0, 127}));
  connect(add3Mass.u1, intMasFloOut.y)
    annotation(
      Line(
        points={{84,-122}, {80,-122}, {80,-100}, {71,-100}},
        color={0, 0, 127}));
  annotation(
    Diagram(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-120,-280}, {180, 120}})),
    Documentation(
      info="<html>
<p>
...
First implementation.
    ...
    experiment(
      ...
      StopTime=1),
    __Dymola_Commands(
      file="Resources/Scripts/Dymola/Fluid/MixingVolumes/Validation/MoistureMixingConservationDynamicBalance.mos"
        "Simulate and plot"));
