...
model MoistureMixingConservationSteadyState
  ...
  extends Buildings.Fluid.MixingVolumes.Validation.BaseClasses.MoistureMixingConservation(mWatFloSol(k=0), mFloSol(k=sou1.m_flow + sou2.m_flow), hSol(k=Medium.h_default*(sou1.m_flow + sou2.m_flow)));
equation
  connect(cheMasFra.u2, senMasFra.X)
    annotation(Line(points={{138,-44}, {110,-44}, {110, 9}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(cheMasFlo.u2, senMasFlo.m_flow)
    annotation(Line(points={{138,-124}, {70,-124}, {70, 9}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(cheSpeEnt.u2, senSpeEnt.H_flow)
    annotation(Line(points={{138,-204}, {30,-204}, {30, 9}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,-280}, {180, 120}}), graphics), experiment(Tolerance=1e-6, StopTime=1), Documentation(info="<html>
<p>
...
</html>"), __Dymola_Commands(file="Resources/Scripts/Dymola/Fluid/MixingVolumes/Validation/MoistureMixingConservationSteadyState.mos"
    "Simulate and plot"));
