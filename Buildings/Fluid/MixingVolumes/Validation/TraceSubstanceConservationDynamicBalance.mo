...
model TraceSubstanceConservationDynamicBalance
  ...
  extends Buildings.Fluid.MixingVolumes.Validation.BaseClasses.TraceSubstanceConservation(vol(massDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial));
  Modelica.Blocks.Continuous.Integrator intTraSubIn(k=1, initType=Modelica.Blocks.Types.Init.InitialState, y_start=0)
    "Integrator for trace substance inlet"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, origin={-30,-60})));
  Modelica.Blocks.Continuous.Integrator intTraSubOut(k=1, initType=Modelica.Blocks.Types.Init.InitialState, y_start=0)
    "Integrator for trace substance outlet"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, origin={10,-40})));
  ...
  Modelica.Blocks.Math.Add3 add3(k1=-1, k2=+ 1, k3=-1)
...
equation
  connect(reaExp.y, add3.u3)
    annotation(Line(points={{31.9,-68}, {38,-68}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(intTraSubIn.y, add3.u2)
    annotation(Line(points={{-19,-60}, {38,-60}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(intTraSubOut.y, add3.u1)
    annotation(Line(points={{21,-40}, {21,-52}, {38,-52}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(add3.y, cheConMas.u1)
    annotation(Line(points={{61,-60}, {70,-60}, {70,-72}, {78,-72}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(zero.y, cheConMas.u2)
    annotation(Line(points={{61,-90}, {70,-90}, {70,-84}, {78,-84}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(intTraSubIn.u, CfloIn.y)
    annotation(Line(points={{-42,-60}, {-46,-60}, {-46,-41}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(CfloOut.y, intTraSubOut.u)
    annotation(Line(points={{46,-41}, {46,-41}, {46,-44}, {30,-44}, {30,-22}, {-12,-22}, {-12,-40}, {-2,-40}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(experiment(Tolerance=1e-08, StopTime=1), Documentation(info="<html>
<p>
...
</html>"), __Dymola_Commands(file="Resources/Scripts/Dymola/Fluid/MixingVolumes/Validation/TraceSubstanceConservationDynamicBalance.mos"
    "Simulate and plot"));
