...
model MoverContinuous
  ...
  extends MoverParameter(pump_Nrpm(inputType=Buildings.Fluid.Types.InputType.Continuous), pump_m_flow(inputType=Buildings.Fluid.Types.InputType.Continuous), pump_y(inputType=Buildings.Fluid.Types.InputType.Continuous), pump_dp(inputType=Buildings.Fluid.Types.InputType.Continuous));
  Modelica.Blocks.Sources.Ramp ramp(duration=1)
    "Ramp input for all movers"
...
equation
  ...
  connect(gai_dp.y, pump_dp.dp_in)
    annotation(Line(points={{-19,-60}, {-10,-60}, {-0.2,-60}, {-0.2,-68}}, color={0, 0, 127}));
  ...
  connect(gai_m_flow.u, ramp.y)
    annotation(Line(points={{-42, 20}, {-50, 20}, {-50, 70}, {-59, 70}}, color={0, 0, 127}));
  connect(gai_dp.u, ramp.y)
    annotation(Line(points={{-42,-60}, {-50,-60}, {-50, 70}, {-59, 70}}, color={0, 0, 127}));
  connect(pump_y.y, ramp.y)
    annotation(Line(points={{-0.2,-28}, {-0.2,-20}, {-50,-20}, {-50, 70}, {-59, 70}}, color={0, 0, 127}));
...
</html>"), experiment(Tolerance=1e-06, StopTime=1), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Movers/Examples/MoverContinuous.mos"
    "Simulate and plot"));
end MoverContinuous;
