...
model FlowControlled_dp
  ...
  parameter Modelica.SIunits.PressureDifference dp_nominal(min=0, displayUnit="Pa")=if rho_default < 500 then 500 else 10000
  ...
  parameter Modelica.SIunits.PressureDifference constantHead(min=0, displayUnit="Pa")=dp_nominal
  ...
  parameter Modelica.SIunits.PressureDifference[:] heads(each min=0, each displayUnit="Pa")=dp_nominal*{(per.speeds[i]/per.speeds[end])^2 for i in 1 : size(per.speeds, 1)}
  ...
  Modelica.Blocks.Interfaces.RealInput dpMea(final quantity="PressureDifference", final displayUnit="Pa", final unit="Pa")=gain.u if prescribeSystemPressure
  ...
  Modelica.Blocks.Interfaces.RealInput dp_in(final unit="Pa") if inputType == Buildings.Fluid.Types.InputType.Continuous
...
equation
  assert(inputSwitch.u >=-1E-3, "Pressure set point for mover cannot be negative. Obtained dp = " + String(inputSwitch.u));
  ...
  annotation(Icon(graphics={Text(extent={{-40, 126}, {-160, 76}}, lineColor={0, 0, 127}, visible=inputType == Buildings.Fluid.Types.InputType.Continuous or inputType == Buildings.Fluid.Types.InputType.Stages, textString=DynamicSelect("dp", if inputType == Buildings.Fluid.Types.InputType.Continuous then String(dp_in, format=".0f") else String(stage)))}), defaultComponentName="fan", Documentation(info="<html>
<p>
