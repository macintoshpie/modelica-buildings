...
model FlowControlled_m_flow
  ...
  extends Buildings.Fluid.Movers.BaseClasses.PartialFlowMachine(
    ...
    final stageInputs(
      each final unit="kg/s")=massFlowRates,
    final constInput(
      final unit="kg/s")=constantMassFlowRate,
    filter(
    ...
    eff(
      per(
        final pressure=if per.havePressureCurve then per.pressure else Buildings.Fluid.Movers.BaseClasses.Characteristics.flowParameters(
  ...
  Modelica.Blocks.Interfaces.RealInput m_flow_in(
    final unit="kg/s",
    nominal=m_flow_nominal) if inputType == Buildings.Fluid.Types.InputType.Continuous
  ...
  annotation(
    Icon(
      graphics={Text(
        ...
        textString=DynamicSelect("m_flow", if inputType == Buildings.Fluid.Types.InputType.Continuous then String(m_flow_in,
          leftJustified=false,
          significantDigits=3) else String(stage)))}),
