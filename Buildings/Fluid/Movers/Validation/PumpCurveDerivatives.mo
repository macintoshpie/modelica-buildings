  ...
  Buildings.Fluid.Movers.FlowControlled_m_flow forcedPump1(
    ...
    annotation(
      Placement(
        transformation(
          extent={{38, 30}, {58, 50}})));
  Modelica.Blocks.Sources.Constant rpm1(
    k=1000)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-5,-5}, {5, 5}},
          origin={35, 63})));
  Modelica.Blocks.Sources.Constant mMax_flow(
    k=40/3.6)
...
equation
  ...
  connect(mMax_flow.y, min1.u2)
    annotation(
      Line(
        points={{12.6, 52}, {18, 52}, {18, 60}, {29, 60}},
        color={0, 0, 127}));
  ...
  annotation(
    experiment(
      ...
      StopTime=1.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Movers/Validation/PumpCurveDerivatives.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
