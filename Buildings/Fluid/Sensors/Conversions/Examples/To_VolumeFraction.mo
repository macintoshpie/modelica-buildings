...
model To_VolumeFraction
  ...
  Buildings.Fluid.Sensors.Conversions.To_VolumeFraction conMasVolFra(MMMea=Modelica.Media.IdealGases.Common.SingleGasesData.CO2.MM)
  ...
  Buildings.Controls.Continuous.LimPID limPID(controllerType=Modelica.Blocks.Types.SimpleController.PI, reverseAction=true, Ti=600, k=2, Td=1)
    annotation(Placement(transformation(extent={{-100,-20}, {-80, 0}})));
  Buildings.Fluid.Sensors.TraceSubstances senCO2(redeclare package Medium=Medium, substanceName="CO2")
    "CO2 sensor"
    annotation(Placement(transformation(extent={{120, 0}, {140, 20}})));
  Buildings.Fluid.MixingVolumes.MixingVolume vol(nPorts=4, redeclare package Medium=Medium, V=4*4*2.7, C_start={300E-6}*44.009544/28.9651159, m_flow_nominal=0.1, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
    "Volume of air"
    annotation(Placement(transformation(extent={{90, 60}, {110, 80}})));
  Buildings.Fluid.Sources.TraceSubstancesFlowSource souCO2(use_m_flow_in=true, redeclare package Medium=Medium, nPorts=1)
    "CO2 source"
    annotation(Placement(transformation(extent={{-20, 60}, {0, 80}})));
  Modelica.Blocks.Math.Gain CO2Per(k=15/1000/3600*1.977)
    "CO2 emission per person"
    annotation(Placement(transformation(extent={{-60, 60}, {-40, 80}})));
  Buildings.Fluid.Sources.MassFlowSource_T sou(use_m_flow_in=true, redeclare package Medium=Medium, C={300E-6}*44.009544/28.9651159, nPorts=1)
    "Source of fresh air with 300 PPM CO2"
  ...
  Buildings.Fluid.Sensors.VolumeFlowRate senVolFlo(redeclare package Medium=Medium, m_flow_nominal=0.1)
  ...
  Buildings.Fluid.Sensors.TraceSubstancesTwoPort senTraSubPeo(m_flow_nominal=0.1, redeclare package Medium=Medium, C_start=0, initType=Modelica.Blocks.Types.Init.InitialState)
    "CO2 concentration in absorptance from people"
    annotation(Placement(transformation(extent={{40, 60}, {60, 80}})));
  Buildings.Fluid.Sensors.TraceSubstancesTwoPort senTraSubFre(m_flow_nominal=0.1, redeclare package Medium=Medium, C_start=0, initType=Modelica.Blocks.Types.Init.InitialState)
    "CO2 concentration in fresh air supply"
    annotation(Placement(transformation(extent={{60,-20}, {80, 0}})));
  Buildings.Fluid.FixedResistances.PressureDrop res(redeclare package Medium=Medium, dp_nominal=10, m_flow_nominal=50/3600)
  ...
  Buildings.Fluid.Sources.Boundary_pT sin(redeclare package Medium=Medium, C={300E-6}*44.009544/28.9651159, p=100000, nPorts=1)
    "Sink for exhaust air"
    annotation(Placement(transformation(extent={{180, 30}, {160, 50}})));
equation
  connect(souCO2.m_flow_in, CO2Per.y)
    annotation(Line(points={{-22.1, 70}, {-39, 70}}, color={0, 0, 127}));
  connect(gai.y, sou.m_flow_in)
    annotation(Line(points={{-39,-10}, {-30,-10}, {-30,-2}, {-22,-2}}, color={0, 0, 127}));
  ...
  connect(senCO2.C, conMasVolFra.m)
    annotation(Line(points={{141, 10}, {147, 10}}, color={0, 0, 127}));
  connect(conMasVolFra.V, norMea.u)
    annotation(Line(points={{169, 10}, {180, 10}, {180,-80}, {-152,-80}, {-152,-50}, {-142,-50}}, color={0, 0, 127}));
  connect(norMea.y, limPID.u_m)
    annotation(Line(points={{-119,-50}, {-90,-50}, {-90,-22}}, color={0, 0, 127}));
  ...
  connect(conVolFlo.u, senVolFlo.V_flow)
    annotation(Line(points={{38, 30}, {30, 30}, {30, 1}}, color={0, 0, 127}));
  connect(souCO2.ports[1], senTraSubPeo.port_a)
    annotation(Line(points={{5.55112e-16, 70}, {40, 70}}, color={0, 127, 255}));
  connect(senTraSubPeo.port_b, vol.ports[1])
    annotation(Line(points={{60, 70}, {82, 70}, {82, 50}, {97, 50}, {97, 60}}, color={0, 127, 255}));
  ...
  connect(senTraSubFre.port_b, vol.ports[2])
    annotation(Line(points={{80,-10}, {99,-10}, {99, 60}}, color={0, 127, 255}));
  connect(vol.ports[3], senCO2.port)
    annotation(Line(points={{101, 60}, {101,-10}, {130,-10}, {130,-5.55112e-16}}, color={0, 127, 255}));
  connect(vol.ports[4], res.port_a)
    annotation(Line(points={{103, 60}, {102, 60}, {102, 38}, {122, 38}, {122, 40}}, color={0, 127, 255}));
  connect(sin.ports[1], res.port_b)
    annotation(Line(points={{160, 40}, {142, 40}}, color={0, 127, 255}));
  annotation(experiment(Tolerance=1e-8, StopTime=36000), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Sensors/Conversions/Examples/To_VolumeFraction.mos"
    "Simulate and plot"), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-200,-100}, {200, 100}})), Documentation(info="<html>
