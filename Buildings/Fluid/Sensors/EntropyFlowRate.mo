...
model EntropyFlowRate
  ...
  parameter Modelica.SIunits.SpecificEntropy s_out_start=Medium.specificEntropy_pTX(p=Medium.p_default, T=Medium.T_default, X=Medium.X_default)
    "Initial or guess value of measured specific entropy"
...
equation
  port_b_s_outflow=Medium.specificEntropy(Medium.setState_phX(p=port_b.p, h=port_b.h_outflow, X=port_b.Xi_outflow));
  if allowFlowReversal then
    sMed_out=Modelica.Fluid.Utilities.regStep(x=port_a.m_flow, y1=port_b_s_outflow, y2=Medium.specificEntropy(Medium.setState_phX(p=port_a.p, h=port_a.h_outflow, X=port_a.Xi_outflow)), x_small=m_flow_small);
  else
  ...
  annotation(defaultComponentName="senS_flow", Icon(graphics={Line(points={{-100, 0}, {-70, 0}}, color={0, 128, 255}), Line(points={{70, 0}, {100, 0}}, color={0, 128, 255}), Line(points={{0, 100}, {0, 70}}, color={0, 0, 127}), Text(extent={{180, 151}, {20, 99}}, lineColor={0, 0, 0}, textString="S_flow"), Ellipse(extent={{-70, 70}, {70,-70}}, lineColor={255, 0, 0}), Text(extent={{-20, 120}, {-140, 70}}, lineColor={0, 0, 0}, textString=DynamicSelect("", String(S_flow, leftjustified=false, significantDigits=3)))}), Documentation(info="<html>
<p>
