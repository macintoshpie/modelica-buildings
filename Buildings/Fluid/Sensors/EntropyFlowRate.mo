...
model EntropyFlowRate
  ...
  Modelica.Blocks.Interfaces.RealOutput S_flow(
    final unit="W/K")
    ...
    annotation(
      Placement(
        transformation(
          ...
          extent={{-10,-10}, {10, 10}},
          rotation=90)));
  parameter Modelica.SIunits.SpecificEntropy s_out_start=Medium.specificEntropy_pTX(
    p=Medium.p_default,
    T=Medium.T_default,
    X=Medium.X_default)
...
protected
  Modelica.SIunits.SpecificEntropy sMed_out(
    start=s_out_start)
    "Medium entropy to which the sensor is exposed";
  Modelica.SIunits.SpecificEntropy s_out(
    start=s_out_start)
    "Medium entropy that is used to compute the entropy flow rate";
  Modelica.SIunits.SpecificEntropy port_b_s_outflow(
    start=s_out_start)
...
equation
  port_b_s_outflow=Medium.specificEntropy(Medium.setState_phX(
    p=port_b.p,
    h=port_b.h_outflow,
    X=port_b.Xi_outflow));
  if allowFlowReversal then
    sMed_out=Modelica.Fluid.Utilities.regStep(
      x=port_a.m_flow,
      y1=port_b_s_outflow,
      y2=Medium.specificEntropy(Medium.setState_phX(
        p=port_a.p,
        h=port_a.h_outflow,
        X=port_a.Xi_outflow)),
  ...
  annotation(
    defaultComponentName="senS_flow",
    Icon(
      graphics={Line(
        points={{-100, 0}, {-70, 0}},
        ...
        lineColor={0, 0, 0},
        textString="S_flow"), Ellipse(
        extent={{-70, 70}, {70,-70}},
        lineColor={255, 0, 0}), Text(
        extent={{-20, 120}, {-140, 70}},
        lineColor={0, 0, 0},
        textString=DynamicSelect("", String(S_flow,
          leftjustified=false,
          significantDigits=3)))}),
    Documentation(
      info="<html>
