...
equation
  ...
  connect(masFloRat.m_flow_in, add_m_flow.y)
    annotation(
      Line(
        points={{-32,-4}, {-32,-4}, {-40,-4}, {-40, 50}, {-47, 50}},
        color={0, 0, 127}));
  connect(add_m_flow.u1, m_flow1.y)
    annotation(
      Line(
        points={{-70, 56}, {-70, 56}, {-74, 56}, {-74, 70}, {-79, 70}},
        color={0, 0, 127}));
  connect(m_flow2.y, add_m_flow.u2)
    annotation(
      Line(
        points={{-79, 40}, {-76, 40}, {-76, 44}, {-70, 44}},
        color={0, 0, 127}));
  connect(sin.ports[1], temDynLoss.port_b)
    annotation(
      Line(
        points={{90,-12}, {84,-12}, {76,-12}},
        color={0, 127, 255}));
  connect(sin.ports[2], senRelTem.port_b)
    annotation(
      Line(
        points={{90,-16}, {80,-16}, {80,-60}, {50,-60}},
        color={0, 127, 255}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=60),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Sensors/Examples/TemperatureDryBulb.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
