...
model TraceSubstances
  ...
  MixingVolumes.MixingVolume vol(redeclare package Medium=Medium, V=2*3*3, m_flow_nominal=1E-6, nPorts=5, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
    "Mixing volume"
    annotation(Placement(transformation(extent={{74, 50}, {94, 70}})));
  Sources.TraceSubstancesFlowSource sou(redeclare package Medium=Medium, nPorts=2, use_m_flow_in=true)
  ...
  Buildings.Fluid.Sensors.TraceSubstances senVol(redeclare package Medium=Medium)
    "Sensor at volume"
    annotation(Placement(transformation(extent={{100, 50}, {120, 70}})));
  Buildings.Fluid.Sensors.TraceSubstances senSou(redeclare package Medium=Medium, substanceName="CO2")
    "Sensor at source"
  ...
  Buildings.Fluid.Sources.MassFlowSource_T mSou(redeclare package Medium=Medium, use_m_flow_in=true, nPorts=1)
    "Fresh air supply"
    annotation(Placement(transformation(extent={{0,-22}, {20,-2}})));
  Buildings.Fluid.Sources.Boundary_pT mSin(redeclare package Medium=Medium, nPorts=1)
    "Exhaust air"
    annotation(Placement(transformation(extent={{-42,-62}, {-22,-42}})));
  Buildings.Fluid.Sensors.Conversions.To_VolumeFraction masFraSou(MMMea=Modelica.Media.IdealGases.Common.SingleGasesData.CO2.MM)
    "Conversion of mass ratio to volume ratio"
    annotation(Placement(transformation(extent={{140, 90}, {160, 110}})));
  Buildings.Fluid.Sensors.Conversions.To_VolumeFraction masFraVol(MMMea=Modelica.Media.IdealGases.Common.SingleGasesData.CO2.MM)
    "Conversion of mass ratio to volume ratio"
    annotation(Placement(transformation(extent={{140, 50}, {160, 70}})));
  Buildings.Fluid.Sensors.TraceSubstancesTwoPort senTraSub(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, tau=0)
    "Sensor at exhaust air"
    annotation(Placement(transformation(extent={{50,-62}, {30,-42}})));
  FixedResistances.PressureDrop res(redeclare package Medium=Medium, dp_nominal=10, m_flow_nominal=0.005, linearized=true)
    annotation(Placement(transformation(extent={{60,-62}, {80,-42}})));
  Buildings.Fluid.Sensors.TraceSubstancesTwoPort senTraSubNoFlorRev(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, allowFlowReversal=false, tau=0)
...
equation
  connect(m_flow.y, mSou.m_flow_in)
    annotation(Line(points={{-59,-4}, {-2,-4}}, color={0, 0, 127}));
  connect(senSou.C, masFraSou.m)
    annotation(Line(points={{45, 100}, {45, 100}, {139, 100}}, color={0, 0, 127}));
  connect(senVol.C, masFraVol.m)
    annotation(Line(points={{121, 60}, {139, 60}}, color={0, 0, 127}));
  connect(sou.ports[1], senSou.port)
    annotation(Line(points={{18, 42}, {34, 42}, {34, 90}}, color={0, 127, 255}));
  connect(step.y, sou.m_flow_in)
    annotation(Line(points={{-59, 40}, {-4.1, 40}}, color={0, 0, 127}));
  connect(sou.ports[2], vol.ports[1])
    annotation(Line(points={{18, 38}, {80.8, 38}, {80.8, 50}}, color={0, 127, 255}));
  connect(mSou.ports[1], vol.ports[2])
    annotation(Line(points={{20,-12}, {82.4,-12}, {82.4, 50}}, color={0, 127, 255}));
  connect(res.port_a, senTraSub.port_a)
    annotation(Line(points={{60,-52}, {50,-52}}, color={0, 127, 255}));
  connect(res.port_b, vol.ports[3])
    annotation(Line(points={{80,-52}, {84,-52}, {84, 50}}, color={0, 127, 255}));
  connect(senVol.port, vol.ports[4])
    annotation(Line(points={{110, 50}, {110, 40}, {85.6, 40}, {85.6, 50}}, color={0, 127, 255}));
  connect(senTraSubNoFlorRev.port_a, senTraSub.port_b)
    annotation(Line(points={{18,-52}, {30,-52}}, color={0, 127, 255}));
  connect(senTraSubNoFlorRev.port_b, mSin.ports[1])
    annotation(Line(points={{-2,-52}, {-22,-52}}, color={0, 127, 255}));
  connect(senPPM.port, vol.ports[5])
    annotation(Line(points={{110, 10}, {110, 4}, {86, 4}, {86, 50}, {87.2, 50}}, color={0, 127, 255}));
  annotation(experiment(Tolerance=1e-6, StopTime=7200), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Sensors/Examples/TraceSubstances.mos"
