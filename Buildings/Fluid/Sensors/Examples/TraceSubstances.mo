  ...
  Sources.TraceSubstancesFlowSource sou(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-2, 30}, {18, 50}})));
  Modelica.Blocks.Sources.Constant step(
    k=8.18E-6)
    "CO2 mass flow rate"
    annotation(
      Placement(
...
equation
  ...
  connect(senSou.C, masFraSou.m)
    annotation(
      Line(
        points={{45, 100}, {45, 100}, {139, 100}},
        color={0, 0, 127}));
  connect(senVol.C, masFraVol.m)
    annotation(
      Line(
        points={{121, 60}, {139, 60}},
        color={0, 0, 127}));
  ...
  connect(senPPM.port, vol.ports[5])
    annotation(
      Line(
        points={{110, 10}, {110, 4}, {86, 4}, {86, 50}, {87.2, 50}},
        color={0, 127, 255}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=7200),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Sensors/Examples/TraceSubstances.mos"
        "Simulate and plot"),
    Diagram(
      coordinateSystem(
