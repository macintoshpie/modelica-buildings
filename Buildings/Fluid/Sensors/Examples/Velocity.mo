...
model Velocity
  ...
  Modelica.Blocks.Sources.Ramp ramp(duration=60, height=-0.2, offset=0.1)
    "Signal for mass flow rate"
    annotation(Placement(transformation(extent={{-90,-2}, {-70, 18}})));
  Buildings.Fluid.Sensors.Velocity senDyn(redeclare package Medium=Medium, m_flow_nominal=10, A=A)
    "Sensor configured to use a dynamic model for the density"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}})));
  Buildings.Fluid.Sensors.Velocity senSteSta(redeclare package Medium=Medium, m_flow_nominal=10, tau=0, A=A)
...
equation
  ...
  connect(masFloRat.ports[1], senDyn.port_a)
    annotation(Line(points={{-30, 4.44089e-16}, {-20, 4.44089e-16}, {-20, 0}, {-10, 0}}, color={0, 127, 255}));
  ...
  annotation(experiment(Tolerance=1e-6, StopTime=60), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Sensors/Examples/Velocity.mos"
