    ...
    annotation(
      Placement(
        ...
        iconTransformation(
          ...
          origin={0, 110})));
protected
  parameter Real s[:]={if(Modelica.Utilities.Strings.isEqual(
    string1=Medium.extraPropertiesNames[i],
    string2=substanceName,
    caseSensitive=false)) then 1 else 0 for i in 1 : Medium.nC}
    "Vector with zero everywhere except where species is";
  final parameter Modelica.SIunits.MolarMass MMBul=Medium.molarMass(Medium.setState_phX(
    p=Medium.p_default,
    h=Medium.h_default,
    X=Medium.X_default))
  ...
  Real CMed(
    min=0,
    start=C_start,
    nominal=sum(Medium.C_nominal))=if allowFlowReversal then Modelica.Fluid.Utilities.regStep(
    x=port_a.m_flow,
    y1=s*port_b.C_outflow,
    y2=s*port_a.C_outflow,
    x_small=m_flow_small) else s*port_b.C_outflow
  ...
  annotation(
    ...
    Icon(
      ...
      graphics={Text(
        ...
        lineColor={0, 0, 0},
        textString=DynamicSelect("", String(ppm,
          leftjustified=false,
          significantDigits=3)))}),
    Documentation(
      info="<html>
