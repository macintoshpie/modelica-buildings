...
model RelativeHumidityTwoPort
  ...
  Modelica.Blocks.Interfaces.RealOutput phi(final unit="1", min=0, start=phi_start)
    "Relative humidity of the passing fluid"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=90, origin={1, 110})));
...
protected
  ...
  Medium.MassFraction Xi_a[Medium.nXi](quantity=Medium.substanceNames[1 : Medium.nXi])
  ...
  Medium.MassFraction Xi_b[Medium.nXi](quantity=Medium.substanceNames[1 : Medium.nXi])
...
equation
  ...
  T_a=Medium.temperature_phX(p=port_a.p, h=port_b.h_outflow, X=Xi_a);
  phi_a=Buildings.Utilities.Psychrometrics.Functions.phi_pTX(p=port_a.p, T=T_a, X_w=Xi_a[1]);
  if allowFlowReversal then
    phi_b=Buildings.Utilities.Psychrometrics.Functions.phi_pTX(p=port_b.p, T=T_b, X_w=Xi_b[1]);
    T_b=Medium.temperature_phX(p=port_b.p, h=port_a.h_outflow, X=Xi_b);
    Xi_b=port_a.Xi_outflow;
    phiMed=Modelica.Fluid.Utilities.regStep(x=port_a.m_flow, y1=phi_a, y2=phi_b, x_small=m_flow_small);
  else
    phi_b=0;
    T_b=273.15;
  ...
  annotation(defaultComponentName="senRelHum", Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 100}}, grid={1, 1}), graphics={Text(extent={{102, 124}, {6, 95}}, lineColor={0, 0, 0}, textString="phi"), Line(points={{0, 100}, {0, 70}}, color={0, 0, 127}), Line(points={{-100, 0}, {-70, 0}}, color={0, 128, 255}), Line(points={{70, 0}, {100, 0}}, color={0, 128, 255}), Text(extent={{-20, 120}, {-140, 70}}, lineColor={0, 0, 0}, textString=DynamicSelect("", String(phi, leftjustified=false, significantDigits=2)))}), Documentation(info="<html>
<p>
