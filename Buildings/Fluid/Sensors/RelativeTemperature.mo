...
equation
  ...
  T_rel=Medium.temperature(
    state=Medium.setState_phX(
      p=port_a.p,
      h=inStream(port_a.h_outflow),
      X=inStream(port_a.Xi_outflow)))-Medium.temperature(
    state=Medium.setState_phX(
      p=port_b.p,
      h=inStream(port_b.h_outflow),
      X=inStream(port_b.Xi_outflow)));
  annotation(
    defaultComponentName="senRelTem",
    Icon(
      graphics={Line(
        points={{-100, 0}, {-70, 0}},
        ...
        lineColor={0, 0, 0},
        textString="T_rel"), Line(
        points={{34, 3}, {-56, 3}},
        color={0, 128, 255}), Polygon(
        ...
        lineColor={0, 0, 0},
        textString=DynamicSelect("", String(T_rel,
          leftjustified=false,
          significantDigits=3)))}),
    Documentation(
      info="<html>
