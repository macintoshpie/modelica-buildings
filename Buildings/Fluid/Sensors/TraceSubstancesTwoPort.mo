...
model TraceSubstancesTwoPort
  ...
  Modelica.Blocks.Interfaces.RealOutput C(min=0, start=C_start)
...
protected
  constant Real sumC_nominal=sum(Medium.C_nominal)
    "Sum of Medium.C_nominal";
  Real CMed(min=0, start=C_start, nominal=if sumC_nominal > Modelica.Constants.eps then sumC_nominal else 1)
...
equation
  if allowFlowReversal then
    CMed=Modelica.Fluid.Utilities.regStep(x=port_a.m_flow, y1=s*port_b.C_outflow, y2=s*port_a.C_outflow, x_small=m_flow_small);
  else
  ...
  annotation(defaultComponentName="senTraSub", Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {100, 100}}), graphics={Text(extent={{82, 122}, {0, 92}}, lineColor={0, 0, 0}, textString="C"), Line(points={{0, 100}, {0, 70}}, color={0, 0, 127}), Line(points={{-100, 0}, {-70, 0}}, color={0, 128, 255}), Line(points={{70, 0}, {100, 0}}, color={0, 128, 255}), Text(extent={{-20, 120}, {-140, 70}}, lineColor={0, 0, 0}, textString=DynamicSelect("", String(C, leftjustified=false, significantDigits=3)))}), Documentation(info="<html>
<p>
