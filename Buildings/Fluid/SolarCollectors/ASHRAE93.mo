...
model ASHRAE93
  ...
  BaseClasses.ASHRAESolarGain solGai(final B0=per.B0, final B1=per.B1, final shaCoe=shaCoe, final til=til, final nSeg=nSeg, final y_intercept=per.y_intercept, final use_shaCoe_in=use_shaCoe_in, final A_c=TotalArea_internal, redeclare package Medium=Medium)
  ...
  BaseClasses.ASHRAEHeatLoss heaLos(final nSeg=nSeg, final slope=per.slope, final y_intercept=per.y_intercept, redeclare package Medium=Medium, final G_nominal=per.G_nominal, dT_nominal=per.dT_nominal, final A_c=TotalArea_internal, m_flow_nominal=per.mperA_flow_nominal*per.A*nPanels_internal, final cp_default=cp_default)
...
equation
  ...
  assert(per.slope < 0, "The heat loss coefficient from the ASHRAE ratings data must be strictly negative. Obtained slope = " + String(per.slope));
  ...
  connect(HDirTil.inc, solGai.incAng)
    annotation(Line(points={{-59, 48}, {-54, 48}, {-54, 46}, {-22, 46}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(HDirTil.H, solGai.HDirTil)
    annotation(Line(points={{-59, 52}, {-52, 52}, {-52, 50}, {-22, 50}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(HDifTilIso.HGroDifTil, solGai.HGroDifTil)
    annotation(Line(points={{-59, 74}, {-50, 74}, {-50, 52.8}, {-22, 52.8}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(HDifTilIso.HSkyDifTil, solGai.HSkyDifTil)
    annotation(Line(points={{-59, 86}, {-48, 86}, {-48, 56}, {-22, 56}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(shaCoe_in, solGai.shaCoe_in)
    annotation(Line(points={{-120, 26}, {-54, 26}, {-54, 43}, {-22, 43}}, color={0, 0, 127}, smooth=Smooth.None));
