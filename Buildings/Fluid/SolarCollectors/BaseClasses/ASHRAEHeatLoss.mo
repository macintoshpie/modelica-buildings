...
block ASHRAEHeatLoss
  ...
  extends Buildings.Fluid.SolarCollectors.BaseClasses.PartialHeatLoss(final QLos_nominal=slope*A_c*dT_nominal
    "Heat loss at nominal condition, for reporting only", QLos_internal=-slope*A_c/nSeg*{dT[i] for i in 1 : nSeg});
...
with ASHRAE Standard 93. A negative heat loss indicates that heat
...
Q<sub>los,i</sub> = -slope A<sub>c</sub> &frasl; n<sub>seg</sub> (T<sub>env</sub>-T<sub>flu,i</sub>)
</p>
<p>
where
<i>slope &lt; 0</i> is the slope for the heat loss, as specified in the ratings data,
...
However, unless the environment temperature which was present during the collector rating is known,
which is not the case, one cannot compute
