...
protected
  ...
  constant Real delta=0.0001
    "Width of the smoothing function";
  constant Real cosIncAngMin=Modelica.Math.cos(incAngMin)
    "Cosine of minimum incidence angle";
  Real k=1/Buildings.Utilities.Math.Functions.smoothMax(Modelica.Math.cos(incAng), cosIncAngMin, delta)-1
...
algorithm
  ...
  incAngMod := Buildings.Utilities.Math.Functions.smoothLimit(x=(1 + B0*k + B1*k^2), l=0, u=1, deltaX=delta);
