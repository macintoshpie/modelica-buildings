...
block PartialHeatLoss
  ...
  Modelica.Blocks.Interfaces.RealOutput QLos[nSeg](
    ...
    each unit="W",
    each displayUnit="W")={QLos_internal[i]*smooth(1, if TFlu[i] > TMedMin2 then 1 else Buildings.Utilities.Math.Functions.smoothHeaviside(TFlu[i]-TMedMin, dTMin)) for i in 1 : nSeg}
