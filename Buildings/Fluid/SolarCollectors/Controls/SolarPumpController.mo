...
model SolarPumpController
  ...
  Modelica.Blocks.Interfaces.RealInput TIn(final unit="K", final displayUnit="degC", quantity="ThermodynamicTemperature")
  ...
  Buildings.Fluid.SolarCollectors.Controls.BaseClasses.GCritCalc criSol(final slope=per.slope, final y_intercept=per.y_intercept)
...
equation
  connect(TIn, criSol.TIn)
    annotation(Line(points={{-120,-40}, {-84,-40}, {-84,-16}, {-60,-16}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(weaBus.TDryBul, criSol.TEnv)
    annotation(Line(points={{-102, 60}, {-84, 60}, {-84,-4}, {-60,-4}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None), Text(textString="%first", index=-1, extent={{-6, 3}, {-6, 3}}));
  connect(weaBus.HDirNor, add.u1)
    annotation(Line(points={{-102, 60}, {-34, 60}, {-34, 6}, {-22, 6}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None), Text(textString="%first", index=-1, extent={{-6, 3}, {-6, 3}}));
  connect(smoHea.y, y)
    annotation(Line(points={{49, 0}, {118, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(add.y, smoHea.u)
    annotation(Line(points={{1, 6.66134e-16}, {14, 6.66134e-16}, {14, 0}, {26, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(criSol.G_TC, add.u2)
    annotation(Line(points={{-36.4,-10}, {-30,-10}, {-30,-6}, {-22,-6}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(defaultComponentName="pumCon", Documentation(info="<html>
      <p>
