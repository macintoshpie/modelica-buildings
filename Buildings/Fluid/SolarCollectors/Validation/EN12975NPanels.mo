  ...
  Buildings.Fluid.Sources.MassFlowSource_T bou1(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-12,-40}, {8,-20}})));
  Modelica.Blocks.Math.Gain gaiNPan(
    k=nPanels)
  ...
  Modelica.Blocks.Sources.RealExpression difHeaLos(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-68,-92}, {-48,-72}})));
  Modelica.Blocks.Sources.Constant m_flow_nominal(
    k=datSolCol.A*datSolCol.mperA_flow_nominal)
...
equation
  ...
  connect(m_flow_nominal.y, bou.m_flow_in)
    annotation(
      Line(
        points={{-67, 40}, {-22, 40}, {-22, 38}, {-12, 38}},
        color={0, 0, 127}));
  connect(gaiNPan.u, m_flow_nominal.y)
    annotation(
      Line(
        points={{-54,-22}, {-60,-22}, {-60, 40}, {-67, 40}},
        color={0, 0, 127}));
  annotation(
    Documentation(
      info="<html>
...
<a href=\"https://github.com/lbl-srg/modelica-buildings/issues/1073\">#1073</a>.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/SolarCollectors/Validation/EN12975NPanels.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=86400));
