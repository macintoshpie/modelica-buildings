...
model EN12975NPanels
  ...
  parameter Integer nPanels=10
    "Number of panels";
  Buildings.Fluid.SolarCollectors.EN12975 solCol(redeclare package Medium=Medium, per=datSolCol, shaCoe=0, azi=0, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, rho=0.2, nColType=Buildings.Fluid.SolarCollectors.Types.NumberSelection.Number, nPanels=1, nSeg=30, lat=0.6457718232379, til=0.78539816339745)
  ...
  Buildings.Fluid.Sources.MassFlowSource_T bou(nPorts=1, redeclare package Medium=Medium, use_m_flow_in=true, use_T_in=false, T=303.15)
    "Inlet for water flow, at a prescribed flow rate and temperature"
    annotation(Placement(transformation(extent={{-12, 20}, {8, 40}})));
  Buildings.Fluid.SolarCollectors.EN12975 solCol1(redeclare package Medium=Medium, per=datSolCol, shaCoe=0, azi=0, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, rho=0.2, nColType=Buildings.Fluid.SolarCollectors.Types.NumberSelection.Number, nSeg=30, lat=0.6457718232379, til=0.78539816339745, nPanels=nPanels)
    "Flat plate solar collector model, has been modified for validation purposes"
  ...
  Buildings.Fluid.Sources.MassFlowSource_T bou1(nPorts=1, redeclare package Medium=Medium, use_m_flow_in=true, use_T_in=false, T=303.15)
    "Inlet for water flow, at a prescribed flow rate and temperature"
    annotation(Placement(transformation(extent={{-12,-40}, {8,-20}})));
  Modelica.Blocks.Math.Gain gaiNPan(k=nPanels)
    "Gain for number of panels"
    annotation(Placement(transformation(extent={{-52,-32}, {-32,-12}})));
  Modelica.Blocks.Sources.RealExpression difHeaGai(y=solCol.heaGai[30].Q_flow-solCol1.heaGai[30].Q_flow/nPanels)
  ...
  Modelica.Blocks.Sources.RealExpression difHeaLos(y=solCol.QLos[30].Q_flow-solCol1.QLos[30].Q_flow/nPanels)
...
equation
  connect(weaDat.weaBus, solCol1.weaBus)
    annotation(Line(points={{-20, 70}, {14, 70}, {14,-20}, {20,-20}, {20,-20.4}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  ...
  connect(solCol.weaBus, weaDat.weaBus)
    annotation(Line(points={{20, 39.6}, {18, 39.6}, {18, 40}, {14, 40}, {14, 70}, {-20, 70}}, color={255, 204, 51}, thickness=0.5));
  connect(m_flow_nominal.y, bou.m_flow_in)
    annotation(Line(points={{-67, 40}, {-22, 40}, {-22, 38}, {-12, 38}}, color={0, 0, 127}));
  connect(gaiNPan.u, m_flow_nominal.y)
    annotation(Line(points={{-54,-22}, {-60,-22}, {-60, 40}, {-67, 40}}, color={0, 0, 127}));
...
</html>"), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/SolarCollectors/Validation/EN12975NPanels.mos"
