  ...
  Modelica.Blocks.Sources.Constant const(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 2}, {-60, 22}})));
  Modelica.Blocks.Sources.Constant solTim(
    k=12*3600)
    "Solar time"
    annotation(
      Placement(
...
equation
  connect(sou.ports[1], solAsh.port_a)
    annotation(
      Line(
        points={{-20,-40}, {-20,-40}, {20,-40}},
        color={0, 127, 255}));
  connect(sou2.ports[1], solEn.port_a)
    annotation(
      Line(
        points={{-20,-80}, {0,-80}, {20,-80}},
        color={0, 127, 255}));
  connect(solAsh.port_b, sou1.ports[1])
    annotation(
      Line(
        points={{40,-40}, {70,-40}, {70,-38}},
        color={0, 127, 255}));
  connect(solEn.port_b, sou1.ports[2])
    annotation(
      Line(
        points={{40,-80}, {56,-80}, {56,-42}, {70,-42}},
        color={0, 127, 255}));
...
First implementation.
    ...
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/SolarCollectors/Validation/ExtremeAmbientConditions.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-06,
      StopTime=86400));
