...
Implementation is based on <code>Modelica.Fluid</code>.
    ...
    Icon(
      graphics={Text(
        ...
        fillColor=DynamicSelect({0, 127, 255}, min(1, max(0,(1-((if use_T_in then T_in else T)-273.15)/50)))*{28, 108, 200} + min(1, max(0,((if use_T_in then T_in else T)-273.15)/50))*{255, 0, 0})), Text(
        extent={{62, 28}, {-58,-22}},
        lineColor={255, 255, 255},
        textString=DynamicSelect("", String((if use_T_in then T_in else T)-273.15,
          format=".1f")))}));
end Boundary_pT;
