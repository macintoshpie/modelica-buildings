...
model MassFlowSource_WeatherData
  ...
  Buildings.Fluid.Sources.MassFlowSource_WeatherData sin_with_h(redeclare package Medium=Medium, m_flow=-1, nPorts=1)
  ...
  Buildings.Fluid.Sensors.RelativeHumidityTwoPort senRelHum(redeclare package Medium=Medium, m_flow_nominal=1, tau=0)
    "Sensor for relative humidity"
    annotation(Placement(transformation(extent={{-30,-10}, {-10, 10}})));
  Buildings.Fluid.Sensors.MassFractionTwoPort senMasFra(redeclare package Medium=Medium, m_flow_nominal=1, tau=0)
    "Sensor for mass fraction of water"
    annotation(Placement(transformation(extent={{10,-10}, {30, 10}})));
  Sensors.TemperatureTwoPort senTem(redeclare package Medium=Medium, m_flow_nominal=1, tau=0)
    "Temperature sensor"
...
equation
  ...
  connect(weaDat.weaBus, bou.weaBus)
    annotation(Line(points={{-80, 0}, {-72, 0}, {-72, 0.2}, {-64, 0.2}}, color={255, 204, 51}, thickness=0.5));
  connect(weaDat.weaBus, sin_with_h.weaBus)
    annotation(Line(points={{-80, 0}, {-72, 0}, {-72, 40}, {96, 40}, {96, 0.2}}, color={255, 204, 51}, thickness=0.5));
  annotation(experiment(Tolerance=1e-6, StopTime=3.1536e+07), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Sources/Examples/MassFlowSource_WeatherData.mos"
