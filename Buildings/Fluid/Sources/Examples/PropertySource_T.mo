...
equation
  ...
  connect(Xi.y, proSouXi.Xi_in[1])
    annotation(
      Line(
        points={{-19, 20}, {0, 20}, {0, 12}},
        color={0, 0, 127}));
  connect(C.y, proSouC.C_in[1])
    annotation(
      Line(
        points={{-19,-20}, {4,-20}, {4,-28}},
        color={0, 0, 127}));
  connect(bouT.m_flow_in, ramp.y)
    annotation(
      Line(
        points={{-62, 48}, {-72, 48}, {-72, 70}, {-79, 70}},
        color={0, 0, 127}));
  connect(ramp.y, bouXi.m_flow_in)
    annotation(
      Line(
        points={{-79, 70}, {-72, 70}, {-72, 8}, {-62, 8}},
        color={0, 0, 127}));
  connect(ramp.y, bouC.m_flow_in)
    annotation(
      Line(
        points={{-79, 70}, {-72, 70}, {-72,-32}, {-62,-32}},
        color={0, 0, 127}));
  connect(T.y, proSouT.T_in)
    annotation(
      Line(
        points={{-19, 60}, {-4, 60}, {-4, 52}},
        color={0, 0, 127}));
  ...
  connect(senTemC.port_b, sin.ports[1])
    annotation(
      Line(
        points={{40,-40}, {60,-40}, {60, 2.66667}},
        color={0, 127, 255}));
  connect(senTemXi.port_b, sin.ports[2])
    annotation(
      Line(
        points={{40, 0}, {50, 0}, {50,-2.22045e-16}, {60,-2.22045e-16}},
        color={0, 127, 255}));
...
See <a href=\"https://github.com/ibpsa/modelica-ibpsa/issues/881\">#881</a>.
    ...
    __Dymola_Commands(
      file="Resources/Scripts/Dymola/Fluid/Sources/Examples/PropertySource_T.mos"
        "Simulate and plot"),
    experiment(
      StopTime=1,
      Tolerance=1e-06));
