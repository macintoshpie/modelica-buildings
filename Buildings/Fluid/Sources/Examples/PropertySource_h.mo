...
model PropertySource_h
  ...
  Buildings.Fluid.Sources.PropertySource_h proSouXi(redeclare package Medium=Medium, use_Xi_in=true)
    "Property source that prescribes Xi"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}})));
  Buildings.Fluid.Sources.MassFlowSource_h bouXi(redeclare package Medium=Medium, nPorts=1, use_m_flow_in=true)
    "Boundary for Xi test"
    annotation(Placement(transformation(extent={{-60,-10}, {-40, 10}})));
  Buildings.Fluid.Sources.Boundary_ph sin(redeclare package Medium=Medium, nPorts=3)
    "Sink model"
    annotation(Placement(transformation(extent={{80,-10}, {60, 10}})));
  Buildings.Fluid.Sources.PropertySource_h proSouH(redeclare package Medium=Medium, use_h_in=true)
    "Property source that prescribes the specific enthalpy"
    annotation(Placement(transformation(extent={{-10, 30}, {10, 50}})));
  Buildings.Fluid.Sources.PropertySource_h proSouC(redeclare package Medium=Medium, use_C_in=true)
    "Property source that prescribes C"
    annotation(Placement(transformation(extent={{-10,-50}, {10,-30}})));
  Buildings.Fluid.Sources.MassFlowSource_h bouH(redeclare package Medium=Medium, nPorts=1, use_m_flow_in=true)
    "Boundary for specific enthalpy test"
    annotation(Placement(transformation(extent={{-60, 30}, {-40, 50}})));
  Buildings.Fluid.Sources.MassFlowSource_h bouC(redeclare package Medium=Medium, nPorts=1, use_m_flow_in=true)
    "Boundary for C test"
...
equation
  ...
  connect(proSouH.port_b, sin.ports[1])
    annotation(Line(points={{10, 40}, {60, 40}, {60, 2.66667}}, color={0, 127, 255}));
  connect(proSouXi.port_b, sin.ports[2])
    annotation(Line(points={{10, 0}, {36, 0}, {36,-2.22045e-16}, {60,-2.22045e-16}}, color={0, 127, 255}));
...
</html>"), __Dymola_Commands(file="Resources/Scripts/Dymola/Fluid/Sources/Examples/PropertySource_h.mos"
