...
model Outside_Cp
  ...
  parameter Real Cp=0.6
    ...
    annotation(
      Dialog(
        enable=not use_Cp_in));
  Modelica.Blocks.Interfaces.RealInput Cp_in(
    unit="1") if use_Cp_in
  ...
  Modelica.SIunits.Pressure pWin(
    displayUnit="Pa")
    "Change in pressure due to wind force";
protected
  Modelica.Blocks.Interfaces.RealInput Cp_in_internal(
    unit="1")
    "Needed to connect to conditional connector";
  Modelica.Blocks.Interfaces.RealInput pWea(
    min=0,
    nominal=1E5,
    unit="Pa")
...
equation
  ...
  pWin=0.5*Cp_in_internal*vWin*vWin*Medium.density(Medium.setState_pTX(
    p=pWea,
    T=T_in_internal,
    X=Medium.X_default));
  pTot=pWea + pWin;
