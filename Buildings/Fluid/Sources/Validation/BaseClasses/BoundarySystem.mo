...
model BoundarySystem
  ...
  Boundary_pT sou(redeclare final package Medium=Medium, nPorts=1)
    "Boundary"
    annotation(Placement(transformation(extent={{-60,-10}, {-40, 10}})));
  MassFlowSource_T sin(redeclare final package Medium=Medium, m_flow=-1, nPorts=1)
    "Mass flow source"
    annotation(Placement(transformation(extent={{60,-10}, {40, 10}})));
  Modelica.SIunits.Temperature T=senTem.T
  ...
  Modelica.SIunits.Pressure p=sou.ports[1].p
    "Pressure in the source";
  Modelica.Fluid.Sensors.TemperatureTwoPort senTem(redeclare package Medium=Medium, allowFlowReversal=false, m_flow_nominal=1, m_flow_small=1E-4)
    "Temperature sensor"
