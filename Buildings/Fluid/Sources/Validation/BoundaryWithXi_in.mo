  ...
  Buildings.Fluid.Sources.Validation.BaseClasses.BoundarySystemWithXi_in bouNatGas(
    ...
    annotation(
      Placement(
        transformation(
          extent={{20,-50}, {40,-30}})));
  Modelica.Blocks.Sources.Constant Xi_1[1](
    k={0.015})
    ...
    annotation(
      Placement(
        transformation(
          extent={{-20, 66}, {0, 86}})));
  Modelica.Blocks.Sources.Constant Xi_6[6](
    k={0.5, 0.2, 0.0, 0.3, 0.0, 0.0})
    "Prescribed mass fraction"
    annotation(
      Placement(
        transformation(
...
equation
  connect(Xi_6.y, bouProFluGas.Xi_in)
    annotation(
      Line(
        points={{1,-4}, {10,-4}, {10, 0}, {18, 0}},
        color={0, 0, 127}));
  connect(Xi_1.y, bouMoiAir.Xi_in)
    annotation(
      Line(
        points={{1, 76}, {10, 76}, {10, 80}, {18, 80}},
        color={0, 0, 127}));
  connect(Xi_6.y, bouNatGas.Xi_in)
    annotation(
      Line(
        points={{1,-4}, {10,-4}, {10,-40}, {18,-40}},
        color={0, 0, 127}));
  connect(Xi_1.y, bouMoiAirCO2.Xi_in)
    annotation(
      Line(
        points={{1, 76}, {10, 76}, {10, 40}, {18, 40}},
        color={0, 0, 127}));
  annotation(
    Documentation(
      info="<html>
...
<a href=\"https://github.com/ibpsa/modelica-ibpsa/issues/1205\">Buildings, #1205</a>.
    ...
    __Dymola_Commands(
      file="Resources/Scripts/Dymola/Fluid/Sources/Validation/BoundaryWithXi_in.mos"
        "Simulate and plot"),
    experiment(
      StopTime=1,
      Tolerance=1e-06));
