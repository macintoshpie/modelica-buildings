...
equation
  ...
  connect(senTemSte.port_b, toSolCol.ports[1])
    annotation(
      Line(
        points={{-40, 10}, {-40, 10}, {-50, 10}, {-50,-17.3333}, {-62,-17.3333}},
        color={0, 127, 255}));
  connect(senTemSte.port_a, tanSte.portHex_b)
    annotation(
      Line(
        points={{-20, 10}, {0, 10}, {0, 59.2}, {6, 59.2}},
        color={0, 127, 255}));
  connect(senTemDyn.port_a, tanDyn.portHex_b)
    annotation(
      Line(
        points={{-22,-20}, {0,-20}, {0,-24.8}, {4,-24.8}},
        color={0, 127, 255}));
  connect(senTemDyn.port_b, toSolCol.ports[2])
    annotation(
      Line(
        points={{-42,-20}, {-52,-20}, {-62,-20}},
        color={0, 127, 255}));
  connect(tanDynSol.portHex_b, senTemDynSol.port_a)
    annotation(
      Line(
        points={{6,-72.8}, {-8,-72.8}, {-8,-70}, {-22,-70}},
        color={0, 127, 255}));
  connect(senTemDynSol.port_b, toSolCol.ports[3])
    annotation(
      Line(
        points={{-42,-70}, {-42,-70}, {-50,-70}, {-50,-22.6667}, {-62,-22.6667}},
        color={0, 127, 255}));
  connect(solColSup.ports[2], tanDyn.portHex_a)
    annotation(
      Line(
        points={{-20, 40}, {-14, 40}, {-4, 40}, {-4,-18.08}, {4,-18.08}},
        color={0, 127, 255}));
  connect(solColSup.ports[3], tanDynSol.portHex_a)
    annotation(
      Line(
        points={{-20, 37.3333}, {-12, 37.3333}, {-4, 37.3333}, {-4,-66.08}, {6,-66.08}},
        color={0, 127, 255}));
  connect(bouWat.ports[1], tanSte.port_b)
    annotation(
      Line(
        points={{60,-9.33333}, {52,-9.33333}, {52, 72}, {40, 72}},
        color={0, 127, 255}));
  connect(bouWat.ports[2], tanDyn.port_b)
    annotation(
      Line(
        points={{60,-12}, {52,-12}, {38,-12}},
        color={0, 127, 255}));
  connect(bouWat.ports[3], tanDynSol.port_b)
    annotation(
      Line(
        points={{60,-14.6667}, {52,-14.6667}, {52,-60}, {40,-60}},
        color={0, 127, 255}));
  connect(step.y, solColSup.p_in)
    annotation(
      Line(
        points={{-59, 48}, {-52, 48}, {-42, 48}},
        color={0, 0, 127}));
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Storage/Examples/StratifiedEnhancedInternalHex.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=1200),
