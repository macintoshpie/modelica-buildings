...
model StratifiedUnloadAtMinimumTemperature
  ...
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor TBot
    ...
    annotation(
      Placement(
        transformation(
          extent={{-100, 30}, {-80, 50}})));
  Modelica.Blocks.Logical.Hysteresis onOffBot(
    uLow=273.15 + 40-0.05,
    uHigh=273.15 + 40 + 0.05)
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort senTem(
    ...
    annotation(
      Placement(
        transformation(
          extent={{190,-70}, {210,-50}})));
  Modelica.Blocks.Logical.Hysteresis onOffMid(
    uLow=273.15 + 40-0.05,
    uHigh=273.15 + 40 + 0.05)
  ...
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow hea
    ...
    annotation(
      Placement(
        transformation(
          extent={{-150,-134}, {-130,-114}})));
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor TTop
    "Temperature tank top"
    annotation(
      Placement(
        transformation(
          extent={{-160,-100}, {-180,-80}})));
  Modelica.Blocks.Logical.Hysteresis onOffHea(
    uLow=273.15 + 50-0.05,
    uHigh=273.15 + 50 + 0.05)
    "Controller for heater at bottom"
    annotation(
      Placement(
        transformation(
          extent={{-210,-134}, {-190,-114}})));
  Modelica.Blocks.Math.BooleanToReal yHea(
    realFalse=150000)
    "Boolean to real for valve at bottom"
    annotation(
      Placement(
        transformation(
          extent={{-180,-134}, {-160,-114}})));
...
equation
  ...
  connect(yTop.u, nor.y)
    annotation(
      Line(
        points={{78, 120}, {71, 120}},
        color={255, 0, 255}));
  connect(and2.y, nor.u1)
    annotation(
      Line(
        points={{31, 80}, {40, 80}, {40, 120}, {48, 120}},
        color={255, 0, 255}));
  connect(onOffBot.y, nor.u2)
    annotation(
      Line(
        points={{-29, 40}, {46, 40}, {46, 112}, {48, 112}},
        color={255, 0, 255}));
  connect(yTop.y, valTop.y)
    annotation(
      Line(
        points={{101, 120}, {122, 120}, {122,-8}},
        color={0, 0, 127}));
  connect(yMid.y, valMed.y)
    annotation(
      Line(
        points={{101, 80}, {142, 80}, {142,-48}},
        color={0, 0, 127}));
  connect(not1.u, onOffBot.y)
    annotation(
      Line(
        points={{-22, 60}, {-26, 60}, {-26, 40}, {-29, 40}},
        color={255, 0, 255}));
  connect(not1.y, and2.u2)
    annotation(
      Line(
        points={{1, 60}, {4, 60}, {4, 72}, {8, 72}},
        color={255, 0, 255}));
  connect(hea.port, tan.heaPorVol[5])
    annotation(
      Line(
        points={{-130,-124}, {-110,-124}, {-110,-120}},
        color={191, 0, 0}));
  connect(TTop.port, tan.heaPorVol[1])
    annotation(
      Line(
        points={{-160,-90}, {-110,-90}, {-110,-120}},
        color={191, 0, 0}));
  connect(onOffHea.u, TTop.T)
    annotation(
      Line(
        points={{-212,-124}, {-230,-124}, {-230,-90}, {-180,-90}},
        color={0, 0, 127}));
  connect(onOffHea.y, yHea.u)
    annotation(
      Line(
        points={{-189,-124}, {-182,-124}},
        color={255, 0, 255}));
  connect(hea.Q_flow, yHea.y)
    annotation(
      Line(
        points={{-150,-124}, {-159,-124}},
        color={0, 0, 127}));
  ...
  annotation(
    Diagram(
      coordinateSystem(
        ...
        extent={{-300,-140}, {260, 140}})),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Storage/Examples/StratifiedUnloadAtMinimumTemperature.mos"
        "Simulate and plot"),
    experiment(
      StopTime=21600,
      Tolerance=1e-06),
