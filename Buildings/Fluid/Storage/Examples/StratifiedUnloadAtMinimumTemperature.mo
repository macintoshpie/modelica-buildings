...
model StratifiedUnloadAtMinimumTemperature
  ...
  package Medium=Buildings.Media.Water
    "Medium model";
  parameter Modelica.SIunits.Volume VTan=3
    "Tank volume";
  parameter Modelica.SIunits.MassFlowRate m_flow_nominal=3*1000/3600
    "Nominal mass flow rate";
  constant Integer nSeg=5
    "Number of volume segments";
  Buildings.Fluid.Storage.Stratified tan(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, VTan=VTan, hTan=2, dIns=0.2, energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, nSeg=nSeg, T_start=353.15)
    "Hot water storage tank"
    annotation(Placement(transformation(extent={{-120,-130}, {-100,-110}})));
  Buildings.Fluid.Sources.Boundary_pT loa(redeclare package Medium=Medium, nPorts=1)
  ...
  Buildings.Fluid.Sources.MassFlowSource_T masSou(nPorts=1, redeclare package Medium=Medium, m_flow=m_flow_nominal)
    "Mass flow rate into the tank"
    annotation(Placement(transformation(extent={{242,-130}, {222,-110}})));
  Buildings.Fluid.Actuators.Valves.TwoWayLinear valTop(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, dpValve_nominal=3000, use_inputFilter=false)
    "Control valve at top"
    annotation(Placement(transformation(extent={{112,-30}, {132,-10}})));
  Buildings.Fluid.Actuators.Valves.TwoWayLinear valMed(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, dpValve_nominal=3000, use_inputFilter=false)
    "Control valve at top"
    annotation(Placement(transformation(extent={{132,-70}, {152,-50}})));
  Buildings.Fluid.Actuators.Valves.TwoWayLinear valBot(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, dpValve_nominal=3000, use_inputFilter=false)
    "Control valve at top"
  ...
  Modelica.Blocks.Logical.Hysteresis onOffBot(uLow=273.15 + 40-0.05, uHigh=273.15 + 40 + 0.05)
    "Controller for valve at bottom"
    annotation(Placement(transformation(extent={{-50, 30}, {-30, 50}})));
  Buildings.Fluid.Sensors.TemperatureTwoPort senTem(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, tau=0)
    "Outflowing temperature"
    annotation(Placement(transformation(extent={{190,-70}, {210,-50}})));
  Modelica.Blocks.Logical.Hysteresis onOffMid(uLow=273.15 + 40-0.05, uHigh=273.15 + 40 + 0.05)
  ...
  Modelica.Blocks.Logical.Hysteresis onOffHea(uLow=273.15 + 50-0.05, uHigh=273.15 + 50 + 0.05)
...
equation
  connect(masSou.ports[1], tan.port_b)
    annotation(Line(points={{222,-120}, {-100,-120}}, color={0, 127, 255}));
  connect(TMid.port, tan.heaPorVol[3])
    annotation(Line(points={{-100, 80}, {-110, 80}, {-110,-120}}, color={191, 0, 0}));
  connect(TBot.port, tan.heaPorVol[5])
    annotation(Line(points={{-100, 40}, {-110, 40}, {-110,-120}}, color={191, 0, 0}));
  ...
  connect(valTop.port_a, tan.fluPorVol[1])
    annotation(Line(points={{112,-20}, {-112.6,-20}, {-112.6,-120}}, color={0, 127, 255}));
  connect(valMed.port_a, tan.fluPorVol[3])
    annotation(Line(points={{132,-60}, {-112.6,-60}, {-112.6,-120}}, color={0, 127, 255}));
  connect(valBot.port_a, tan.fluPorVol[5])
    annotation(Line(points={{150,-100}, {-112.6,-100}, {-112.6,-120}}, color={0, 127, 255}));
  ...
  connect(hea.port, tan.heaPorVol[5])
    annotation(Line(points={{-130,-124}, {-110,-124}, {-110,-120}}, color={191, 0, 0}));
  connect(TTop.port, tan.heaPorVol[1])
    annotation(Line(points={{-160,-90}, {-110,-90}, {-110,-120}}, color={191, 0, 0}));
  connect(onOffHea.u, TTop.T)
    annotation(Line(points={{-212,-124}, {-230,-124}, {-230,-90}, {-180,-90}}, color={0, 0, 127}));
  ...
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-300,-140}, {260, 140}})), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Storage/Examples/StratifiedUnloadAtMinimumTemperature.mos"
    "Simulate and plot"), experiment(StopTime=21600, Tolerance=1e-06), Documentation(info="<html>
<p>
