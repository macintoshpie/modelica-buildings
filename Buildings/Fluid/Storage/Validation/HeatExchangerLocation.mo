...
model HeatExchangerLocation
  ...
  Buildings.Fluid.Sources.MassFlowSource_T mHex_flow1(redeclare package Medium=Medium, nPorts=2, m_flow=m_flow_nominal/100, T=273.15 + 60)
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort senTan_aTop(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, tau=0)
    "Temperature sensor at tank outlet"
    annotation(Placement(transformation(extent={{10,-30}, {-10,-10}})));
  Buildings.Fluid.Sensors.TemperatureTwoPort senTan_bTop(redeclare package Medium=Medium, m_flow_nominal=m_flow_nominal, tau=0)
    "Temperature sensor at tank outlet"
...
equation
  connect(mHex_flow1.ports[1], tan_aTop.portHex_a)
    annotation(Line(points={{-20, 22}, {-20, 20.2}, {42, 20.2}}, color={0, 127, 255}));
  connect(senTan_aTop.port_a, tan_aTop.portHex_b)
    annotation(Line(points={{10,-20}, {10,-20}, {20,-20}, {20, 16}, {42, 16}}, color={0, 127, 255}));
  connect(senTan_aTop.port_b, sin.ports[1])
    annotation(Line(points={{-10,-20}, {-26,-20}, {-40,-20}}, color={0, 127, 255}));
  connect(senTan_bTop.port_b, sin.ports[2])
    annotation(Line(points={{-10,-70}, {-20,-70}, {-20,-24}, {-40,-24}}, color={0, 127, 255}));
  connect(senTan_bTop.port_a, tan_bTop.portHex_b)
    annotation(Line(points={{10,-70}, {20,-70}, {20,-56}, {40,-56}}, color={0, 127, 255}));
  connect(tan_bTop.portHex_a, mHex_flow1.ports[2])
    annotation(Line(points={{40,-51.8}, {30,-51.8}, {30, 18}, {-20, 18}}, color={0, 127, 255}));
  connect(tan_aTop.port_a, watInTan.ports[1])
    annotation(Line(points={{42, 24}, {38, 24}, {38, 62}, {-40, 62}}, color={0, 127, 255}));
  connect(tan_bTop.port_a, watInTan.ports[2])
    annotation(Line(points={{40,-48}, {40,-48}, {34,-48}, {34, 58}, {-40, 58}}, color={0, 127, 255}));
  ...
  annotation(__Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Fluid/Storage/Validation/HeatExchangerLocation.mos"
