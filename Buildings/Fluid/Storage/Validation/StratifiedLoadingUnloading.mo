...
model StratifiedLoadingUnloading
  ...
  Buildings.Fluid.Sources.MassFlowSource_T sin_1(redeclare package Medium=Medium, T=273.15 + 20, m_flow=-0.028, use_m_flow_in=true, nPorts=1)
    annotation(Placement(transformation(extent={{78,-2}, {58, 18}})));
  Buildings.Fluid.Storage.StratifiedEnhanced tanEnh(redeclare package Medium=Medium, hTan=3, dIns=0.3, VTan=0.1, nSeg=nSeg, show_T=true, m_flow_nominal=m_flow_nominal)
    "Tank"
    annotation(Placement(transformation(extent={{-30,-2}, {-10, 18}})));
  Buildings.Fluid.Sources.MassFlowSource_T sin_2(redeclare package Medium=Medium, T=273.15 + 20, m_flow=-0.028, use_m_flow_in=true, nPorts=1)
    annotation(Placement(transformation(extent={{78,-40}, {58,-20}})));
  Buildings.Fluid.Storage.Stratified tan(redeclare package Medium=Medium, hTan=3, dIns=0.3, VTan=0.1, nSeg=nSeg, show_T=true, m_flow_nominal=m_flow_nominal)
    "Tank"
    annotation(Placement(transformation(extent={{-26,-40}, {-6,-20}})));
  Modelica.Blocks.Sources.Pulse pulse(amplitude=2*m_flow_nominal, offset=-m_flow_nominal, period=7200)
...
equation
  connect(sou_1.ports[1], HIn_flow.port_a)
    annotation(Line(points={{-80,-8}, {-70,-8}, {-70,-30}, {-60,-30}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(HIn_flow.port_b, tan.port_a)
    annotation(Line(points={{-44,-30}, {-26,-30}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(tan.port_b, HOut_flow.port_a)
    annotation(Line(points={{-6,-30}, {22,-30}}, color={0, 127, 255}, smooth=Smooth.None));
  ...
  connect(sou_1.ports[2], HInEnh_flow.port_a)
    annotation(Line(points={{-80,-12}, {-76,-12}, {-76,-6}, {-72,-6}, {-72, 8}, {-60, 8}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(HInEnh_flow.port_b, tanEnh.port_a)
    annotation(Line(points={{-44, 8}, {-30, 8}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(tanEnh.port_b, HOutEnh_flow.port_a)
    annotation(Line(points={{-10, 8}, {2, 8}}, color={0, 127, 255}, smooth=Smooth.None));
