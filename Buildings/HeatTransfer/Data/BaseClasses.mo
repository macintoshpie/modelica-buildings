  ...
  record Material
    ...
    parameter Real piMat=if steadyState then piRef else x*sqrt(c*d/k)
      ...
      annotation(
        Dialog(
          tab="Advanced"));
    parameter Real nStaReal(
      min=0)=nStaRef*piMat/piRef
    ...
    parameter Modelica.SIunits.Temperature TSol
      ...
      annotation(
        Dialog(
          group="Properties for phase change material"));
