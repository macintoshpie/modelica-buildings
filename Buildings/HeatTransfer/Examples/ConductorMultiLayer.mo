...
model ConductorMultiLayer
  ...
  Buildings.HeatTransfer.Conduction.MultiLayer con(steadyStateInitial=false, redeclare Buildings.HeatTransfer.Data.OpaqueConstructions.Insulation100Concrete200 layers, A=0.1, stateAtSurface_b=false)
...
equation
  ...
  connect(conv.fluid, TA.port)
    annotation(Line(points={{-20, 10}, {-40, 10}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(conv.solid, con.port_a)
    annotation(Line(points={{5.55112e-16, 10}, {20, 10}}, color={191, 0, 0}, smooth=Smooth.None));
  annotation(experiment(Tolerance=1e-6, StopTime=86400), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/HeatTransfer/Examples/ConductorMultiLayer.mos"
