  ...
  Modelica.Thermal.HeatTransfer.Sensors.HeatFlowSensor heaFlo1
    annotation(
      Placement(
        transformation(
          extent={{-6, 4}, {6, 16}})));
  parameter Buildings.HeatTransfer.Data.Solids.Concrete concrete200(
    x=0.2,
    nSta=4)
    annotation(
      Placement(
        transformation(
          extent={{20, 60}, {40, 80}})));
  parameter Buildings.HeatTransfer.Data.Solids.Concrete concrete100(
    x=0.1,
    nSta=2)
    annotation(
      Placement(
...
equation
  ...
  connect(heaFlo1.Q_flow, cheEqu.u1)
    annotation(
      Line(
        points={{0, 4}, {0, 4}, {0,-64}, {18,-64}},
        color={0, 0, 127}));
  connect(heaFlo2.Q_flow, cheEqu.u2)
    annotation(
      Line(
        points={{8,-36}, {8,-36}, {8,-76}, {18,-76}},
        color={0, 0, 127}));
  annotation(
    experiment(
      StopTime=86400,
      Tolerance=1e-6),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/HeatTransfer/Examples/ConductorSingleLayer.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
