...
model ConductorSingleLayer
  ...
  Buildings.HeatTransfer.Conduction.SingleLayer con(A=1, material=concrete200, stateAtSurface_b=true)
...
equation
  connect(con.port_b, TB.port)
    annotation(Line(points={{40, 10}, {60, 10}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(step.y, TA.T)
  ...
  connect(con1.port_b, con2.port_a)
    annotation(Line(points={{40,-30}, {50,-30}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(con2.port_b, TB1.port)
    annotation(Line(points={{70,-30}, {80,-30}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(heaFlo2.port_b, con1.port_a)
    annotation(Line(points={{14,-30}, {20,-30}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(heaFlo1.port_b, con.port_a)
    annotation(Line(points={{6, 10}, {20, 10}}, color={191, 0, 0}, smooth=Smooth.None));
  ...
  connect(heaFlo2.Q_flow, cheEqu.u2)
    annotation(Line(points={{8,-36}, {8,-36}, {8,-76}, {18,-76}}, color={0, 0, 127}));
  annotation(experiment(StopTime=86400, Tolerance=1e-6), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/HeatTransfer/Examples/ConductorSingleLayer.mos"
