...
model ConductorSingleLayerPCM
  ...
  Modelica.Blocks.Sources.Step step(height=10, offset=293.15, startTime=360)
  ...
  Modelica.Blocks.Math.Add cheEqu(k2=-1)
    "Checks for equality of the results"
  ...
  Buildings.HeatTransfer.Conduction.SingleLayer conPCM(A=1, material=matPCM, stateAtSurface_b=true)
    "Construction with phase change around 40 degC"
    annotation(Placement(transformation(extent={{24, 0}, {44, 20}})));
  Buildings.HeatTransfer.Conduction.SingleLayer con1(A=1, material=concrete100, stateAtSurface_b=false)
    "Construction without PCM"
    annotation(Placement(transformation(extent={{22,-40}, {42,-20}})));
  Buildings.HeatTransfer.Conduction.SingleLayer con2(A=1, material=concrete100, stateAtSurface_a=false, stateAtSurface_b=true)
    "Construction without PCM"
    annotation(Placement(transformation(extent={{50,-40}, {70,-20}})));
  parameter Buildings.HeatTransfer.Data.SolidsPCM.Generic matPCM(x=0.2, k=1.4, c=840, d=2240, nSta=4, TSol=273.15 + 40.49, TLiq=273.15 + 40.51, LHea=100000)
  ...
  Buildings.HeatTransfer.Conduction.SingleLayer conPCM2(A=1, material=matPCM2, stateAtSurface_b=false)
    "Construction with phase change near room temperature"
    annotation(Placement(transformation(extent={{24, 34}, {44, 54}})));
  parameter Buildings.HeatTransfer.Data.SolidsPCM.Generic matPCM2(x=0.2, k=1.4, c=840, d=2240, nSta=4, TSol=273.15 + 20.49, TLiq=273.15 + 20.51, LHea=100000)
    "PCM material with phase change near room temperature"
    annotation(Placement(transformation(extent={{-12, 70}, {8, 90}})));
equation
  connect(step.y, TA.T)
    annotation(Line(points={{-79, 10}, {-62, 10}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(step.y, TA1.T)
    annotation(Line(points={{-79, 10}, {-72, 10}, {-72,-30}, {-62,-30}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(cheEqu.u1, heaFlo2.Q_flow)
    annotation(Line(points={{18,-64}, {8,-64}, {8,-36}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(cheEqu.u2, heaFlo1.Q_flow)
    annotation(Line(points={{18,-76}, {0,-76}, {0, 4}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TA.port, conv1.fluid)
    annotation(Line(points={{-40, 10}, {-32, 10}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(TA1.port, conv2.fluid)
    annotation(Line(points={{-40,-30}, {-32,-30}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(conv2.solid, heaFlo2.port_a)
    annotation(Line(points={{-12,-30}, {2,-30}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(conv1.solid, heaFlo1.port_a)
    annotation(Line(points={{-12, 10}, {-6, 10}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(heaFlo1.port_b, conPCM.port_a)
    annotation(Line(points={{6, 10}, {24, 10}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(heaFlo2.port_b, con1.port_a)
    annotation(Line(points={{14,-30}, {22,-30}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(con1.port_b, con2.port_a)
    annotation(Line(points={{42,-30}, {50,-30}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(step.y, TA2.T)
    annotation(Line(points={{-79, 10}, {-72, 10}, {-72, 44}, {-62, 44}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(TA2.port, conv3.fluid)
    annotation(Line(points={{-40, 44}, {-32, 44}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(conv3.solid, conPCM2.port_a)
    annotation(Line(points={{-12, 44}, {24, 44}}, color={191, 0, 0}, smooth=Smooth.None));
...
</html>"), experiment(StopTime=7200, Tolerance=1e-6));
