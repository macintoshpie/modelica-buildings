...
block AbsorbedRadiation
  ...
  Modelica.Blocks.Interfaces.RealInput HRoo(quantity="RadiantEnergyFluenceRate", final unit="W/m2")
  ...
  Modelica.Blocks.Interfaces.RealOutput QAbsExtSha_flow[NSta](each final quantity="Power", each final unit="W")
  ...
  Modelica.Blocks.Interfaces.RealOutput QAbsIntSha_flow[NSta](each final quantity="Power", each final unit="W")
  ...
  Modelica.Blocks.Interfaces.RealOutput QAbsGlaUns_flow[N, NSta](each quantity="Power", each final unit="W")
  ...
  Modelica.Blocks.Interfaces.RealOutput QAbsGlaSha_flow[N, NSta](each quantity="Power", each final unit="W")
...
protected
  ...
  constant Integer NoShade=1
    "Index for data for no shade";
  constant Integer Shade=2
    "Index for data with shade";
  ...
  final parameter Real coeAbsDevExtIrrExtSha=1-radDat.traRefShaDev[1, 1]-radDat.traRefShaDev[2, 1]
  ...
  final parameter Real coeAbsDevIntIrrExtSha[NSta]={1-radDat.winTraRefIntIrrExtSha[1, iSta]-radDat.winTraRefIntIrrExtSha[2, iSta] for iSta in 1 : NSta}
    "Absorptivity of exterior shading device for interior radiation";
  Real x
    "Intermediate variable, x=(index-1)*incAng/(0.5pi)+2, 0<=x<=NDIR";
  Real incAng2;
initial equation
  //**************************************************************
  // Assign coefficients.
...
algorithm
  ...
  // **************************************************************
  for i in 1 : N loop
    absRad[NoShade, i + 1, 1 : NSta] := AWin*(1-uSha_internal)*(HDif*coeAbsEx[NoShade, i, HEM + 1, 1 : NSta] + HRoo*coeAbsIn[NoShade, i, 1 : NSta]);
    absRad[Shade, i + 1, 1 : NSta] := AWin*uSha_internal*(HDif*coeAbsEx[Shade, i, HEM + 1, 1 : NSta] + HRoo*coeAbsIn[Shade, i, 1 : NSta]);
  end for;
  // **************************************************************
  ...
  for iSta in 1 : NSta loop
    if haveExteriorShade then
      absRad[Shade, 1, iSta] := AWin*uSha_internal*coeAbsDevExtIrrExtSha*(HDif + HDir + HDif*radDat.traRefShaDev[1, 1]*radDat.traRef[2, 1, N, HEM, iSta]);
    // Interior Shading Device: diffusive radiation from both interior and exterior
    elseif haveInteriorShade then
      absRad[Shade, N + 2, iSta] := AWin*uSha_internal*(HDif*radDat.devAbsExtIrrIntShaDev[HEM, iSta] + HRoo*coeAbsDevIntIrrIntSha[iSta]);
    end if;
  end for;
  // **************************************************************
  ...
  for i in 1 : N loop
    ...
    for iSta in 1 : NSta loop
      absRad[NoShade, i + 1, iSta] := absRad[NoShade, i + 1, iSta] + AWin*HDir*(1-uSha_internal)*Buildings.HeatTransfer.Windows.BaseClasses.smoothInterpolation({coeAbsEx[NoShade, i, k, iSta] for k in 1 :(HEM + 2)}, x);
      // Glass with shading: add absorbed direct radiation
      absRad[Shade, i + 1, iSta] := absRad[Shade, i + 1, iSta] + AWin*HDir*uSha_internal*Buildings.HeatTransfer.Windows.BaseClasses.smoothInterpolation({coeAbsEx[Shade, i, k, iSta] for k in 1 :(HEM + 2)}, x);
    end for;
  end for;
  // Interior shading device: add absorbed direct radiation
  if haveInteriorShade then
    for iSta in 1 : NSta loop
      absRad[Shade, N + 2, iSta] := absRad[Shade, N + 2, iSta] + AWin*HDir*uSha_internal*Buildings.HeatTransfer.Windows.BaseClasses.smoothInterpolation({coeAbsDevExtIrrIntSha[k, iSta] for k in 1 :(HEM + 2)}, x);
    end for;
  end if;
  // Exterior shading device: add absorbed reflection of direct radiation from exterior source
  if haveExteriorShade then
    for iSta in 1 : NSta loop
      absRad[Shade, 1, iSta] := absRad[Shade, 1, iSta] + AWin*HDir*uSha_internal*coeAbsDevExtIrrExtSha*Buildings.HeatTransfer.Windows.BaseClasses.smoothInterpolation({coeRefExtPan1[k, iSta] for k in 1 :(HEM + 2)}, x);
    end for;
  end if;
...
</html>"), Icon(graphics={Text(extent={{26, 86}, {102, 74}}, lineColor={0, 0, 127}, textString="QAbsExtSha"), Text(extent={{-101,-73}, {-54,-82}}, lineColor={0, 0, 127}, textString="HRoo"), Text(extent={{24,-68}, {102,-80}}, lineColor={0, 0, 127}, textString="QAbsIntSha"), Text(extent={{34, 46}, {94, 32}}, lineColor={0, 0, 127}, textString="QAbsGlaUns"), Text(extent={{30,-32}, {104,-44}}, lineColor={0, 0, 127}, textString="QAbsGlaSha")}));
