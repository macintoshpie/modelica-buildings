...
model Overhang
  ...
  Buildings.BoundaryConditions.SolarGeometry.IncidenceAngle incAng(lat=weaDat.lat, azi=Buildings.Types.Azimuth.S, til=Buildings.Types.Tilt.Wall)
  ...
  Buildings.HeatTransfer.Windows.BaseClasses.Overhang ove(gap=0.1, azi=Buildings.Types.Azimuth.S, lat=weaDat.lat, wL=0, wR=0.95, dep=0.5, hWin=2, wWin=0.1)
...
equation
  connect(weaDat.weaBus, incAng.weaBus)
    annotation(Line(points={{-80, 30}, {-70, 30}, {-70, 10.4}, {-40, 10.4}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  connect(incAng.y, walSolAzi.incAng)
    annotation(Line(points={{-19, 10}, {-10, 10}, {-10, 25.2}, {-2, 25.2}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(walSolAzi.verAzi, ove.verAzi)
    annotation(Line(points={{21, 30}, {48, 30}, {48, 20}, {66, 20}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(weaDat.weaBus, ove.weaBus)
    annotation(Line(points={{-80, 30}, {-70, 30}, {-70,-20}, {50,-20}, {50, 16}, {67.8, 16}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  connect(weaDat.weaBus, weaBus)
    annotation(Line(points={{-80, 30}, {-70, 30}, {-70, 34}, {-34, 34}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None), Text(textString="%second", index=1, extent={{6, 3}, {6, 3}}));
  connect(weaBus.solAlt, walSolAzi.alt)
    annotation(Line(points={{-34, 34}, {-32, 34}, {-32, 34.8}, {-2, 34.8}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None), Text(textString="%first", index=-1, extent={{-6, 3}, {-6, 3}}));
  connect(weaBus.solAlt, ove.alt)
    annotation(Line(points={{-34, 34}, {-70, 34}, {-70,-20}, {60,-20}, {60, 12}, {66, 12}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None), Text(textString="%first", index=-1, extent={{-6, 3}, {-6, 3}}));
  annotation(experiment(Tolerance=1e-6, StopTime=86400), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/HeatTransfer/Windows/BaseClasses/Examples/Overhang.mos"
