...
model SideFins
  ...
  Buildings.BoundaryConditions.SolarGeometry.IncidenceAngle incAng(azi=0, lat=0.73129295658562, til=1.5707963267949)
  ...
  Buildings.HeatTransfer.Windows.BaseClasses.SideFins fin(gap=0.1, h=0.7, dep=1.0, hWin=1.5, wWin=2.0)
...
equation
  connect(weaDat.weaBus, zen.weaBus)
    annotation(Line(points={{-80, 30}, {-70, 30}, {-70, 50}, {-60.2, 50}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  ...
  connect(altAng.alt, fin.alt)
    annotation(Line(points={{1, 50}, {8, 50}, {8, 6}, {58, 6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(walSolAzi.verAzi, fin.verAzi)
    annotation(Line(points={{41, 30}, {50, 30}, {50, 14}, {58, 14}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(altAng.alt, walSolAzi.alt)
    annotation(Line(points={{1, 50}, {8, 50}, {8, 34.8}, {18, 34.8}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(incAng.y, walSolAzi.incAng)
    annotation(Line(points={{-19, 10}, {0, 10}, {0, 25.2}, {18, 25.2}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(experiment(Tolerance=1e-6, StopTime=86400), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/HeatTransfer/Windows/BaseClasses/Examples/SideFins.mos"
