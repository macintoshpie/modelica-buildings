...
model HeatCapacityVariableShade
  ...
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor heaCapSha(C=1000, T(start=293.15, fixed=true))
    "Thermal capacity connected to shaded part of window"
    annotation(Placement(transformation(extent={{50,-20}, {70, 0}})));
  Modelica.Blocks.Sources.Ramp yRamp(height=0.5, offset=0.25, duration=300, startTime=150)
...
equation
  connect(yRamp.y, heaCapGla.ySha)
    annotation(Line(points={{-59,-10}, {-26,-10}, {-26,-36}, {-12,-36}}, color={0, 0, 127}));
  connect(heaCapGla.portSha, theConSha.solid)
    annotation(Line(points={{10,-36}, {16,-36}, {16,-20}, {22,-20}}, color={191, 0, 0}));
  ...
  connect(heaCapGla.portUns, theConUns.solid)
    annotation(Line(points={{10,-44}, {16,-44}, {16,-60}, {20,-60}}, color={191, 0, 0}));
  connect(yCom.y, heaCapGla.yCom)
    annotation(Line(points={{-59,-44}, {-59,-44}, {-12,-44}}, color={0, 0, 127}));
  connect(yRamp.y, hASha.u)
    annotation(Line(points={{-59,-10}, {-26,-10}, {-26, 50}, {-10, 50}}, color={0, 0, 127}));
  connect(hASha.y, theConSha.Gc)
    annotation(Line(points={{13, 50}, {32, 50}, {32,-10}}, color={0, 0, 127}));
  connect(hAUns.y, theConUns.Gc)
    annotation(Line(points={{13, 20}, {18, 20}, {18,-40}, {30,-40}, {30,-50}}, color={0, 0, 127}));
  connect(yCom.y, hAUns.u)
    annotation(Line(points={{-59,-44}, {-44,-44}, {-20,-44}, {-20, 20}, {-10, 20}}, color={0, 0, 127}));
  annotation(experiment(Tolerance=1e-6, StopTime=600), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/HeatTransfer/Windows/BaseClasses/Validation/HeatCapacityVariableShade.mos"
