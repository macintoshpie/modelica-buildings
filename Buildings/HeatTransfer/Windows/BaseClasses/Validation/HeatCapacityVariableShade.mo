...
model HeatCapacityVariableShade
  ...
  parameter Modelica.SIunits.Area AGla=2
    "Glass area";
  parameter Buildings.HeatTransfer.Data.Solids.Glass datGla(
    x=0.005)
  ...
  Modelica.Thermal.HeatTransfer.Components.Convection theConUns
    ...
    annotation(
      Placement(
        transformation(
          extent={{20,-70}, {40,-50}})));
  Modelica.Blocks.Sources.RealExpression yCom(
    y=1-yRamp.y)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80,-54}, {-60,-34}})));
  Modelica.Blocks.Math.Gain hASha(
    k=AGla*10)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-8, 40}, {12, 60}})));
  Modelica.Blocks.Math.Gain hAUns(
    k=AGla*10)
...
equation
  connect(yRamp.y, heaCapGla.ySha)
    annotation(
      Line(
        points={{-59,-10}, {-26,-10}, {-26,-36}, {-12,-36}},
        color={0, 0, 127}));
  connect(heaCapGla.portSha, theConSha.solid)
    annotation(
      Line(
        points={{10,-36}, {16,-36}, {16,-20}, {22,-20}},
        color={191, 0, 0}));
  connect(theConSha.fluid, heaCapSha.port)
    annotation(
      Line(
        points={{42,-20}, {52,-20}, {60,-20}},
        color={191, 0, 0}));
  connect(theConUns.fluid, heaCapSha.port)
    annotation(
      Line(
        points={{40,-60}, {60,-60}, {60,-20}},
        color={191, 0, 0}));
  connect(heaCapGla.portUns, theConUns.solid)
    annotation(
      Line(
        points={{10,-44}, {16,-44}, {16,-60}, {20,-60}},
        color={191, 0, 0}));
  connect(yCom.y, heaCapGla.yCom)
    annotation(
      Line(
        points={{-59,-44}, {-59,-44}, {-12,-44}},
        color={0, 0, 127}));
  connect(yRamp.y, hASha.u)
    annotation(
      Line(
        points={{-59,-10}, {-26,-10}, {-26, 50}, {-10, 50}},
        color={0, 0, 127}));
  connect(hASha.y, theConSha.Gc)
    annotation(
      Line(
        points={{13, 50}, {32, 50}, {32,-10}},
        color={0, 0, 127}));
  connect(hAUns.y, theConUns.Gc)
    annotation(
      Line(
        points={{13, 20}, {18, 20}, {18,-40}, {30,-40}, {30,-50}},
        color={0, 0, 127}));
  connect(yCom.y, hAUns.u)
    annotation(
      Line(
        points={{-59,-44}, {-44,-44}, {-20,-44}, {-20, 20}, {-10, 20}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=600),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/HeatTransfer/Windows/BaseClasses/Validation/HeatCapacityVariableShade.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
