...
model FixedShade
  ...
  Buildings.HeatTransfer.Windows.FixedShade sha[4](final conPar=conPar, azi=conPar.azi, each lat=weaDat.lat)
    "Shade model"
  ...
  Buildings.BoundaryConditions.SolarIrradiation.DirectTiltedSurface HDirTil(lat=weaDat.lat, til=Buildings.Types.Tilt.Wall, azi=Buildings.Types.Azimuth.S)
  ...
  parameter Buildings.ThermalZones.Detailed.BaseClasses.ParameterConstructionWithWindow conPar[4](each til=Buildings.Types.Tilt.Wall, each azi=Buildings.Types.Azimuth.S, each A=20, each hWin=1.5, each wWin=2, each glaSys=glaSys, each layers=insCon, ove(wR={0.1, 0.1, 0, 0}, wL={0.1, 0.1, 0, 0}, gap={0.1, 0.1, 0, 0}, dep={1, 1, 0, 0}), sidFin(dep={0, 1, 1, 0}, gap={0, 0.1, 0.1, 0}, h={0, 0.1, 0.1, 0}))
...
equation
  connect(weaDat.weaBus, sha[1].weaBus)
    annotation(Line(points={{-40, 0}, {60, 0}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  connect(weaDat.weaBus, sha[2].weaBus)
    annotation(Line(points={{-40, 0}, {60, 0}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  connect(weaDat.weaBus, sha[3].weaBus)
    annotation(Line(points={{-40, 0}, {60, 0}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  connect(HDirTil.weaBus, weaDat.weaBus)
    annotation(Line(points={{-20, 70}, {-30, 70}, {-30, 0}, {-40, 0}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  connect(HDirTil.H, H.u)
    annotation(Line(points={{1, 70}, {18, 70}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(HDirTil.inc, incAng.u)
    annotation(Line(points={{1, 66}, {10, 66}, {10, 30}, {18, 30}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(incAng.y, sha.incAng)
    annotation(Line(points={{41, 30}, {48, 30}, {48,-6}, {58,-6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(H.y, sha.HDirTilUns)
    annotation(Line(points={{41, 70}, {50, 70}, {50, 6}, {58, 6}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(weaDat.weaBus, sha[4].weaBus)
    annotation(Line(points={{-40, 0}, {60, 0}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  annotation(experiment(Tolerance=1e-6, StopTime=86400), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/HeatTransfer/Windows/Examples/FixedShade.mos"
