...
model Overhang
  ...
  Buildings.HeatTransfer.Windows.Overhang ove(dep=1.2, gap=0.1, hWin=1.0, wWin=1.0, azi=Buildings.Types.Azimuth.S, wR=0.1, wL=0.1, lat=weaDat.lat)
...
equation
  connect(weaDat.weaBus, ove.weaBus)
    annotation(Line(points={{-40, 10}, {20, 10}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  connect(HDirTil.weaBus, weaDat.weaBus)
    annotation(Line(points={{-20, 30}, {-30, 30}, {-30, 10}, {-40, 10}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  connect(HDirTil.inc, ove.incAng)
    annotation(Line(points={{1, 26}, {6, 26}, {6, 4}, {18, 4}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(HDirTil.H, ove.HDirTilUns)
    annotation(Line(points={{1, 30}, {10, 30}, {10, 16}, {18, 16}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(experiment(Tolerance=1e-6, StopTime=86400), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/HeatTransfer/Windows/Examples/Overhang.mos"
