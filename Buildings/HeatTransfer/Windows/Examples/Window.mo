...
model Window
  ...
  Buildings.HeatTransfer.Sources.PrescribedTemperature TOuts
    ...
    annotation(
      Placement(
        transformation(
          extent={{-20, 40}, {0, 60}})));
  Buildings.HeatTransfer.Sources.FixedTemperature TRAir(
    T=293.15)
    "Room air temperature"
    annotation(
      Placement(
        transformation(
          extent={{300, 20}, {280, 40}})));
  Buildings.HeatTransfer.Radiosity.IndoorRadiosity indRad(
    A=A)
    "Model for indoor radiosity"
    annotation(
      Placement(
        transformation(
          extent={{322, 100}, {302, 120}})));
  Buildings.HeatTransfer.Sources.FixedHeatFlow fixedHeatFlow(
    Q_flow=0)
    annotation(
      Placement(
        transformation(
          ...
          rotation=90,
          origin={312, 68})));
  replaceable parameter Buildings.HeatTransfer.Data.GlazingSystems.DoubleClearAir13Clear glaSys(
  ...
  Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-20,-20}, {0, 0}})));
  Modelica.Blocks.Math.Gain HRoo(
    k=0.1)
  ...
  Buildings.HeatTransfer.Windows.BaseClasses.ShadeRadiation intShaRad(
    ...
    A=AGla) if glaSys.haveShade
    "Interior shade radiation model"
    annotation(
      Placement(
        transformation(
          extent={{240, 106}, {220, 126}})));
  ...
  annotation(
    Diagram(
      coordinateSystem(
        ...
        extent={{-100,-100}, {340, 200}}),
      graphics),
    experiment(
      Tolerance=1e-6,
      StopTime=1.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/HeatTransfer/Windows/Examples/Window.mos"
        "Simulate and plot"),
    Documentation(
      revisions="<html>
<ul>
