...
model ExteriorHeatTransfer
  ...
  extends BaseClasses.PartialWindowBoundaryCondition(
    final thisSideHasShade=haveExteriorShade);
  ...
  parameter Real vieFacSky(
    final min=0,
    final max=1,
    final unit="1")
    "View factor from receiving surface to sky";
  Modelica.Blocks.Interfaces.RealInput vWin(
    final unit="m/s")
    "Wind speed"
    annotation(
      Placement(
        ...
        iconTransformation(
          extent={{-116, 32}, {-100, 48}})));
  Buildings.HeatTransfer.Windows.BaseClasses.ExteriorConvectionCoefficient conCoeGla(
    final A=AGla)
    ...
    annotation(
      Placement(
        transformation(
          extent={{-84, 40}, {-64, 60}})));
  Buildings.HeatTransfer.Windows.BaseClasses.ExteriorConvectionCoefficient conCoeFra(
    final A=AFra)
  ...
  Modelica.Blocks.Interfaces.RealInput QSolAbs_flow(
    unit="W",
    quantity="Power") if haveShade
  ...
  BaseClasses.ShadeRadiation shaRad(
    ...
    final tauIR_glass=if thisSideHasShade then tauIRSha_glass else 1) if haveShade
  ...
  Radiosity.RadiositySplitter radShaOut
    ...
    annotation(
      Placement(
        transformation(
          extent={{-40,-40}, {-20,-20}})));
  BaseClasses.ShadeConvection shaCon(
    final thisSideHasShade=thisSideHasShade,
    final A=AGla) if haveShade
...
equation
  ...
  connect(shaCon.air, air)
    annotation(
      Line(
        points={{0, 30}, {-20, 30}, {-20, 10}, {-80, 10}, {-80, 5.55112e-16}, {-100, 5.55112e-16}, {-100, 0}},
        color={191, 0, 0},
        smooth=Smooth.None));
