...
algorithm
  traRef := Buildings.HeatTransfer.Windows.Functions.glassTRExteriorIrradiationNoShading(N=N, NSta=NSta, HEM=HEM, layer=layer)
    "property for exterior irradiation";
  traRefIntIrr := Buildings.HeatTransfer.Windows.Functions.glassTRInteriorIrradiationNoShading(N=N, NSta=NSta, HEM=HEM, layer=layer)
    ...
    for i in 1 : N-1 loop
      for j in i + 1 : N loop
        for iD in 1 : HEM loop
          traRef[k, N + 1-i, N + 1-j, iD, 1 : NSta] := traRefIntIrr[k, N + 1-i, N + 1-j, iD, 1 : NSta];
        end for;
      end for;
    end for;
  end for;
