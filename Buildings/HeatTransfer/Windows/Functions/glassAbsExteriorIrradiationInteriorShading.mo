...
function glassAbsExteriorIrradiationInteriorShading
  ...
  input Real absExtIrrNoSha[N, HEM, NSta](
    each min=0,
    each max=1)
  ...
  output Real absExtIrrNoShaIntSha[N, HEM, NSta](
    each min=0,
    each max=1)
    "Angular and hemispherical absorptance of each glass pane for exterior irradiation with interior shading";
protected
...
algorithm
  for iSta in 1 : NSta loop
    for iD in 1 : HEM loop
      i := 1;
      fac := traRef[TRA, 1, N, iD, iSta]*refIntShaDev/(1-traRef[Rb, N, 1, HEM, iSta]*refIntShaDev)
        "Equation (A.4.90)";
      absBac := 1-traRef[TRA, i, i, iD, iSta]-traRef[Rb, i, i, iD, iSta]
        "Equation (A.4.81b)";
      if N >= 2 then
        absExtIrrNoShaIntSha[i, iD, iSta] := absExtIrrNoSha[i, iD, iSta] + fac*traRef[TRA, N, i + 1, HEM, iSta]*absBac
          "Equation (A.4.90)";
        for i in 2 : N-1 loop
          fac := traRef[TRA, 1, N, iD, iSta]*refIntShaDev/(1-traRef[Rb, N, 1, HEM, iSta]*refIntShaDev)
            "Equation (A.4.90)";
          ...
          absBac := 1-traRef[TRA, i, i, iD, iSta]-traRef[Rb, i, i, iD, iSta]
            "Equation (A.4.81b)";
          absExtIrrNoShaIntSha[i, iD, iSta] := absExtIrrNoSha[i, iD, iSta] + fac*(traRef[TRA, N, i, HEM, iSta]*traRef[Rb, i-1, 1, HEM, iSta]*absFro + traRef[TRA, N, i + 1, HEM, iSta]*absBac)
            "Equation (A.4.90)";
        end for;
        i := N;
        fac := traRef[TRA, 1, N, iD, iSta]*refIntShaDev/(1-traRef[Rb, N, 1, HEM, iSta]*refIntShaDev)
        ...
        absBac := 1-traRef[TRA, i, i, iD, iSta]-traRef[Rb, i, i, iD, iSta]
          "Equation (A.4.81b)";
        absExtIrrNoShaIntSha[i, iD, iSta] := absExtIrrNoSha[i, iD, iSta] + fac*(traRef[TRA, N, i, HEM, iSta]*traRef[Rb, i-1, 1, HEM, iSta]*absFro + absBac)
          "Equation (A.4.90)";
      else
