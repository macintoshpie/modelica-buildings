...
function glassAbsExteriorIrradiationNoShading
  ...
  output Real[N, HEM, NSta] abs(each min=0, each max=1)
    "Angular and hemispherical absorptance of each glass pane for exterior irradiation without shading.
    ...
    for iSta in 1 : NSta loop
      for iD in 1 : HEM loop
        ...
        if deno2 < SMALL then
          ...
          abs[j, iD, iSta] := af + ab*traRef[TRA, 1, j, iD, iSta]*traRef[Ra, j + 1, N, iD, iSta]/deno2
        ...
        for j in 2 : N-1 loop
          ...
          if deno1 < SMALL or deno2 < SMALL then
            ...
            abs[j, iD, iSta] := af*traRef[TRA, 1, j-1, iD, iSta]/deno1 + ab*traRef[TRA, 1, j, iD, iSta]*traRef[Ra, j + 1, N, iD, iSta]/deno2
              "Equation (A.4.83b)";
          end if;
        end for;
        j := N;
