...
function glassAbsInteriorIrradiationExteriorShading
  ...
  output Real absIntIrrExtSha[N, NSta](
    each min=0,
    each max=1)
    "Hemispherical absorptance of each glass pane for interior irradiation with exterior shading";
protected
...
algorithm
  for iSta in 1 : NSta loop
    fac := traRef[TRA, N, 1, HEM, iSta]*refExtShaDev/(1-traRef[Ra, 1, N, HEM, iSta]*refExtShaDev);
    i := 1;
    ...
    absBac := 1-traRef[TRA, i, i, HEM, iSta]-traRef[Rb, i, i, HEM, iSta]
      "Equation (A.4.81b)";
    if N >= 2 then
      absIntIrrExtSha[i, iSta] := absIntIrrNoSha[i, iSta] + fac*absFro + fac*traRef[TRA, 1, i, HEM, iSta]*traRef[Ra, i + 1, N, HEM, iSta]*absBac
        "Equation (A.4.94)";
      for i in 2 : N-1 loop
        absFro := 1-traRef[TRA, i, i, HEM, iSta]-traRef[Ra, i, i, HEM, iSta]
          "Equaiton (A.4.81a)";
        absBac := 1-traRef[TRA, i, i, HEM, iSta]-traRef[Rb, i, i, HEM, iSta]
          "Equation (A.4.81b)";
        absIntIrrExtSha[i, iSta] := absIntIrrNoSha[i, iSta] + fac*traRef[TRA, 1, i-1, HEM, iSta]*absFro + fac*traRef[TRA, 1, i, HEM, iSta]*traRef[Ra, i + 1, N, HEM, iSta]*absBac
          "Equation (A.4.94)";
      end for;
      i := N;
      absFro := 1-traRef[TRA, i, i, HEM, iSta]-traRef[Ra, i, i, HEM, iSta]
        "Equaiton (A.4.81a)";
      absIntIrrExtSha[i, iSta] := absIntIrrNoSha[i, iSta] + fac*traRef[TRA, 1, i-1, HEM, iSta]*absFro
        "Equation (A.4.94)";
    else
