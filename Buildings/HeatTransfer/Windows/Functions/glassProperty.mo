  ...
  for i in 1 : N loop
    ...
    for iSta in 1 : NSta loop
      ...
      if(abs(glass[Ra, i, iSta]-glass[Rb, i, iSta]) < tol) then
        oneLay[:, :, iSta] := Buildings.HeatTransfer.Windows.Functions.glassPropertyUncoated(HEM, oneGla[:, iSta], xGla[i], psi);
      else
        //coated glass
        oneLay[:, :, iSta] := Buildings.HeatTransfer.Windows.Functions.glassPropertyCoated(HEM, oneGla[:, iSta], psi);
      end if;
    end for;
    // iSta
    for j in 1 : 3 loop
