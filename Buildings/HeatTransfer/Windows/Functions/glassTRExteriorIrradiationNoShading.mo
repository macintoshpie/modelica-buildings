...
function glassTRExteriorIrradiationNoShading
  ...
  input Real layer[3, N, HEM, NSta]
    "Angular data of glass pane";
  output Real traRef[3, N, N, HEM, NSta](
    each min=0,
    each max=1)
        ...
        for j in i + 1 : N loop
          ...
          for iSta in 1 : NSta loop
            aij := 1-traRef[Ra, j, j, iD, iSta]*traRef[Rb, j-1, i, iD, iSta]
              "Equation (A.4.77)";
            assert(aij >-SMALL, "Glass transmittance and reflectance data was not correct.\n");
            if aij < SMALL then
              ...
              aij := 1/aij;
              traRef[TRA, i, j, iD, iSta] := aij*traRef[TRA, i, j-1, iD, iSta]*traRef[TRA, j, j, iD, iSta]
                "Equation (A.4.78a)";
              traRef[Ra, i, j, iD, iSta] := traRef[Ra, i, j-1, iD, iSta] + aij*traRef[TRA, i, j-1, iD, iSta]*traRef[TRA, i, j-1, iD, iSta]*traRef[Ra, j, j, iD, iSta]
                "Equation (A.4.78b)";
              traRef[Rb, j, i, iD, iSta] := traRef[Rb, j, j, iD, iSta] + aij*traRef[TRA, j, j, iD, iSta]*traRef[TRA, j, j, iD, iSta]*traRef[Rb, j-1, i, iD, iSta]
                "Equation (A.4.78c)";
            end if;
          end for;
        // iSta in 1:NSta
