...
function winTRInteriorIrradiationExteriorShading
  ...
  output Real traRefIntIrrExtSha[3, NSta](
    each min=0,
    each max=1)
    "Hemispherical transmittance and back reflectance of a window system (glass and shading device) for interior irradiation with exterior shading";
algorithm
  for iSta in 1 : NSta loop
    traRefIntIrrExtSha[TRA, iSta] := traRef[TRA, N, 1, HEM, iSta]*traExtShaDev/(1-refExtShaDev*traRef[Ra, 1, N, HEM, iSta])
      "Equation (A.4.95)";
    traRefIntIrrExtSha[Rb, iSta] := traRef[Rb, N, 1, HEM, iSta] + traRef[TRA, N, 1, HEM, iSta]*refExtShaDev*traRef[1, 1, N, HEM, iSta]/(1-traRef[Ra, 1, N, HEM, iSta]*refExtShaDev)
      "Equation (A.4.97)";
    traRefIntIrrExtSha[Ra, iSta] := 0
      "Dummy value";
  end for;
  annotation(
    Documentation(
