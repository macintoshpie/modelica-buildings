...
function winTRInteriorIrradiationInteriorShading
  ...
  output Real traRefIntIrrIntSha[3, NSta](
    each min=0,
    each max=1)
    "Hemispherical transmittance and back reflectance of a window system (glass and shading device) for interior irradiation with interior shading";
protected
...
algorithm
  for iSta in 1 : NSta loop
    rRho := traRef[Rb, N, 1, HEM, iSta]*refIntShaDev
      "Part of Equation (A.4.99)";
    rTau := traRef[Rb, N, 1, HEM, iSta]*traIntShaDev
      "Part of Equation (A.4.105)";
    c := traIntShaDev*(1 + rRho/(1-rRho))
      "Equation (A.4.99)";
    traRefIntIrrIntSha[TRA, iSta] := c*traRef[TRA, N, 1, HEM, iSta]
      "Equation (A.4.100b)";
    traRefIntIrrIntSha[Rb, iSta] := refIntShaDev + c*rTau
      "Equation (A.4.105)";
    traRefIntIrrIntSha[Ra, iSta] := 0
      "Dummy value";
  end for;
  annotation(
    Documentation(
