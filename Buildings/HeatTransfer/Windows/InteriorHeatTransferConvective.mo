...
model InteriorHeatTransferConvective
  ...
  parameter Modelica.SIunits.Angle til
    "Surface tilt";
  Modelica.Blocks.Interfaces.RealInput QRadAbs_flow(
    final unit="W") if haveShade
  ...
  Modelica.Blocks.Interfaces.RealOutput TSha(
    final unit="K",
    final quantity="ThermodynamicTemperature") if haveShade
    "Shade temperature"
    annotation(
      Placement(
        transformation(
        ...
        iconTransformation(
          ...
          rotation=270,
          origin={0,-110})));
  Buildings.HeatTransfer.Windows.BaseClasses.InteriorConvectionCoefficient conCoeGla(
    final A=AGla)
  ...
  BaseClasses.ShadeConvection conSha(
    final A=AGla,
    final thisSideHasShade=thisSideHasShade) if haveShade
