...
model Window
  ...
  Buildings.HeatTransfer.Windows.BaseClasses.HeatCapacity capGla(
    ...
    der_TUns(
      fixed=true),
    der_TSha(
      fixed=glaSys.haveExteriorShade or glaSys.haveInteriorShade)) if not steadyState
    "Heat capacity of glass on room-side, used to reduce nonlinear system of equations"
    annotation(
      Placement(
        transformation(
          extent={{130, 38}, {150, 58}})));
...
equation
  ...
  connect(capFra.port, fra_b)
    annotation(
      Line(
        points={{140,-142}, {140,-160}, {202,-160}},
        color={191, 0, 0}));
  connect(capGla.ySha, shaSig.y)
    annotation(
      Line(
        points={{128, 52}, {80, 52}, {-24, 52}, {-24, 160}, {-39, 160}},
        color={0, 0, 127}));
  connect(capGla.yCom, shaSig.yCom)
    annotation(
      Line(
        points={{128, 44}, {60, 44}, {-20, 44}, {-20, 154}, {-39, 154}},
        color={0, 0, 127}));
  connect(capGla.portSha, glaSha_b)
    annotation(
      Line(
        points={{150, 52}, {160, 52}, {160,-20}, {200,-20}},
        color={191, 0, 0}));
  connect(capGla.portUns, glaUns_b)
    annotation(
      Line(
        points={{150, 44}, {156, 44}, {156, 20}, {200, 20}},
        color={191, 0, 0}));
  annotation(
    Diagram(
      coordinateSystem(
