...
partial model FluidProperties
  ...
  replaceable package Medium=Buildings.Media.Antifreeze.Validation.BaseClasses.PropyleneGlycolWater
    "Medium package";
  parameter Integer nX_a
    "Number of mass fractions to evaluate fluid properties";
  parameter Modelica.SIunits.MassFraction X_a[nX_a]
  ...
  Modelica.SIunits.Temperature Tf[nX_a]
    "Fluid temperature";
  Modelica.SIunits.Density d[nX_a]
    "Density of fluid mixture";
  Modelica.SIunits.SpecificHeatCapacity cp[nX_a]
    "Specific heat capacity of fluid mixture";
  Modelica.SIunits.ThermalConductivity lambda[nX_a]
    "Density of fluid mixture";
  Modelica.SIunits.DynamicViscosity eta[nX_a]
    "Dynamic viscosity of fluid mixture";
  Modelica.SIunits.Temperature T
    "Temperature of fluid mixture";
  Modelica.SIunits.Conversions.NonSIunits.Temperature_degC T_degC
    "Celsius temperature";
protected
  parameter Modelica.SIunits.Time dt=1
    "Simulation length";
  parameter Real convT(
    unit="K/s")=(T_max-T_min)/dt
    "Rate of temperature change";
equation
  T=T_min + convT*time;
  T_degC=Modelica.SIunits.Conversions.to_degC(T);
  for i in 1 : nX_a loop
    Tf[i]=Medium.testFusionTemperature_TX_a(
      T=T,
      X_a=X_a[i]);
    d[i]=if T >= Tf[i] then Medium.testDensity_TX_a(
      T=T,
      X_a=X_a[i]) else 0.;
    cp[i]=if T >= Tf[i] then Medium.testSpecificHeatCapacityCp_TX_a(
      T=T,
      X_a=X_a[i]) else 0.;
    lambda[i]=if T >= Tf[i] then Medium.testThermalConductivity_TX_a(
      T=T,
      X_a=X_a[i]) else 0.;
    eta[i]=if T >= Tf[i] then Medium.testDynamicViscosity_TX_a(
      T=T,
      X_a=X_a[i]) else 0.;
  end for;
  annotation(
