...
algorithm
  ...
  while abs(dv/v) > 1e-10 loop
    assert(m < 1E3, "Failed to converge in Buildings.Media.Refrigerants.R410A.specificVolumeVap_pT");
    m := m + 1;
  ...
  annotation(derivative=Buildings.Media.Refrigerants.R410A.dSpecificVolumeVap_pT, preferredView="info", Documentation(info="<HTML>
<p>
