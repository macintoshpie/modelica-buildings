...
package PerfectGas
  ...
  extends Modelica.Media.Interfaces.PartialCondensingGases(mediumName="Moist air unsaturated perfect gas", substanceNames={"water", "air"}, final reducedX=true, final singleState=false, reference_X={0.01, 0.99}, reference_T=273.15, reference_p=101325, fluidConstants={Modelica.Media.IdealGases.Common.FluidData.H2O, Modelica.Media.IdealGases.Common.FluidData.N2}, AbsolutePressure(start=p_default), Temperature(start=T_default));
  extends Modelica.Icons.Package;
  constant Integer Water=1
    "Index of water (in substanceNames, massFractions X, etc.)";
  constant Integer Air=2
    "Index of air (in substanceNames, massFractions X, etc.)";
  redeclare record extends ThermodynamicState(p(start=p_default), T(start=T_default), X(start=X_default))
    "ThermodynamicState record for moist air"
  end ThermodynamicState;
  redeclare replaceable model extends BaseProperties(p(stateSelect=if preferredMediumStates then StateSelect.prefer else StateSelect.default), Xi(each stateSelect=if preferredMediumStates then StateSelect.prefer else StateSelect.default), final standardOrderComponents=true)
...
as required from medium model \"" + mediumName + "\".");
    ...
    X_steam=Xi[Water];
    X_air=1-Xi[Water];
    h=(T-reference_T)*dryair.cp*(1-Xi[Water]) +((T-reference_T)*steam.cp + h_fg)*Xi[Water];
...
Function to set the state for given pressure, enthalpy and species concentration.
  ...
  algorithm
    ...
    annotation(smoothOrder=2, Inline=true, Documentation(info="<html>
  ...
  algorithm
    ...
    annotation(smoothOrder=5, Inline=true, derivative=Buildings.Media.Specialized.Air.PerfectGas.saturationPressureLiquid_der, Documentation(info="<html>
...
Buildings.Media.Specialized.Air.PerfectGas.saturationPressureLiquid</a>
  ...
  function sublimationPressureIce=Modelica.Media.Air.MoistAir.sublimationPressureIce
    ...
    annotation(Inline=true);
  function sublimationPressureIce_der=Modelica.Media.Air.MoistAir.sublimationPressureIce_der
    "Derivative function for 'sublimationPressureIce'"
    annotation(Inline=true);
  redeclare function extends saturationPressure
    "Saturation curve valid for 223.16 <= T <= 373.16 (and slightly outside with less accuracy)"
  algorithm
    psat := Buildings.Utilities.Math.Functions.regStep(y1=saturationPressureLiquid(Tsat), y2=sublimationPressureIce(Tsat), x=Tsat-273.16, x_small=1.0);
...
Specific entropy is calculated from the thermodynamic state record, assuming ideal gas behavior and including entropy of mixing. Liquid or solid water is not taken into account, the entire water content X[1] is assumed to be in the vapor state (relative humidity below 1.0).
  ...
  algorithm
    ...
    annotation(smoothOrder=5, Inline=true, derivative=der_enthalpyOfNonCondensingGas);
  ...
  replaceable function der_enthalpyOfDryAir
    ...
    input Real der_T
      "temperature derivative";
    output Real der_h
      "derivative of dry air enthalpy";
  algorithm
  ...
  algorithm
    ...
    annotation(Inline=true, derivative=der_specificHeatCapacityCv);
  ...
  redeclare replaceable function specificEnthalpy_pTX
    ...
    input Modelica.SIunits.MassFraction X[:]
      "Mass fractions of moist air";
    output Modelica.SIunits.SpecificEnthalpy h
      "Specific enthalpy at p, T, X";
  protected
    Modelica.SIunits.SpecificEnthalpy hDryAir
      "Enthalpy of dry air";
  algorithm
    hDryAir :=(T-reference_T)*dryair.cp;
    h := hDryAir*(1-X[Water]) +((T-reference_T)*steam.cp + h_fg)*X[Water];
  ...
  algorithm
    ...
    annotation(smoothOrder=5, Inline=true, inverse(h=specificEnthalpy_pTX(p, T, X)), Documentation(info="<html>
...
First implementation.
  ...
  constant Modelica.SIunits.SpecificEnergy h_fg=Buildings.Utilities.Psychrometrics.Constants.h_fg
    "Latent heat of evaporation of water";
  constant Modelica.SIunits.SpecificHeatCapacity cpWatLiq=Buildings.Utilities.Psychrometrics.Constants.cpWatLiq
...
</html>"), Icon(graphics={Ellipse(extent={{-78, 78}, {-34, 34}}, lineColor={0, 0, 0}, fillPattern=FillPattern.Sphere, fillColor={120, 120, 120}), Ellipse(extent={{-18, 86}, {26, 42}}, lineColor={0, 0, 0}, fillPattern=FillPattern.Sphere, fillColor={120, 120, 120}), Ellipse(extent={{48, 58}, {92, 14}}, lineColor={0, 0, 0}, fillPattern=FillPattern.Sphere, fillColor={120, 120, 120}), Ellipse(extent={{-22, 32}, {22,-12}}, lineColor={0, 0, 0}, fillPattern=FillPattern.Sphere, fillColor={120, 120, 120}), Ellipse(extent={{36,-32}, {80,-76}}, lineColor={0, 0, 0}, fillPattern=FillPattern.Sphere, fillColor={120, 120, 120}), Ellipse(extent={{-36,-30}, {8,-74}}, lineColor={0, 0, 0}, fillPattern=FillPattern.Sphere, fillColor={120, 120, 120}), Ellipse(extent={{-90,-6}, {-46,-50}}, lineColor={0, 0, 0}, fillPattern=FillPattern.Sphere, fillColor={120, 120, 120})}));
end PerfectGas;
