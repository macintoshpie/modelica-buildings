  ...
  Real_Real pyt(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-10, 0}, {10, 20}})));
  Modelica.Blocks.Sources.Constant T0(
    k=293.15)
    "Initial temperature"
    annotation(
      Placement(
  ...
  Buildings.Controls.OBC.CDL.Logical.OnOffController onOffCon(
    ...
    annotation(
      Placement(
        transformation(
          extent={{30, 10}, {50, 30}})));
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal onOff(
    realTrue=1.2)
    "On off control"
    annotation(
      Placement(
...
equation
  connect(mul.u1[1], T0.y)
    annotation(
      Line(
        points={{-42, 19}, {-46, 19}, {-46, 80}, {-59, 80}},
        color={0, 0, 127}));
  connect(clock.y, mul.u4[1])
    annotation(
      Line(
        points={{-59, 0}, {-50, 0}, {-50, 1}, {-42, 1}},
        color={0, 0, 127}));
  ...
  connect(pyt.yR[1], onOffCon.u)
    annotation(
      Line(
        points={{11, 9.5}, {18, 9.5}, {18, 14}, {28, 14}},
        color={0, 0, 127}));
  connect(onOffCon.reference, T0.y)
    annotation(
      Line(
        points={{28, 26}, {0, 26}, {0, 80}, {-59, 80}},
        color={0, 0, 127}));
  ...
  connect(onOff.y, mul.u3[1])
    annotation(
      Line(
        points={{82, 20}, {90, 20}, {90,-10}, {-48,-10}, {-48, 7}, {-42, 7}},
        color={0, 0, 127}));
  connect(TAmb.y, mul.u2[1])
    annotation(
      Line(
        points={{-59, 40}, {-52, 40}, {-52, 13}, {-42, 13}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=86400),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Obsolete/Utilities/IO/Python27/Examples/SimpleRoom.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
