within Buildings.Obsolete.Utilities.IO.Python27.Functions;
function exchange
  ...
  input BaseClasses.PythonObject pytObj
    "Memory that holds the Python object";
  input Boolean passPythonObject
    "Set to true if the Python function returns and receives an object, see User's Guide";
  input Real dblWri[max(1, nDblWri)]
    "Double values to write";
  input Integer intWri[max(1, nIntWri)]
    "Integer values to write";
  input String strWri[max(1, nStrWri)]
    "String values to write";
  input Integer nDblWri(min=0)
    "Number of double values to write";
  input Integer nDblRea(min=0)
    "Number of double values to read";
  input Integer nIntWri(min=0)
    "Number of integer values to write";
  input Integer nIntRea(min=0)
    "Number of integer values to read";
  input Integer nStrWri(min=0)
  ...
  output Real dblRea[max(1, nDblRea)]
    "Double values returned by Python";
  output Integer intRea[max(1, nIntRea)]
    "Integer values returned by Python";
protected
  String pytPatOld
    "Old value of PYTHONPATH environment variable";
  String pytPatBuildings
    "PYTHONPATH of Buildings library";
  String pytPat
    "Value of PYTHONPATH environment variable";
  Boolean havePytPat
    "true if PYTHONPATH is already set by the user";
  package P
    constant String filNam="modelica://Buildings/legal.html"
      "Name to a file of the Buildings library";
    constant String buiLibFil=Modelica.Utilities.Files.loadResource(uri=filNam)
      "Absolute path to filNam";
  end P;
  String searchString="legal.html"
    "String to be replaced";
algorithm
  // Get the directory to Buildings/Resources/Python-Sources
  pytPatBuildings := Modelica.Utilities.Strings.replace(string=P.buiLibFil, searchString=searchString, replaceString="Resources/Python-Sources");
  ...
  (dblRea, intRea) := BaseClasses.exchange(moduleName=moduleName, functionName=functionName, pytObj=pytObj, passPythonObject=passPythonObject, pythonPath=pytPat, dblWri=dblWri, intWri=intWri, strWri=strWri, nDblWri=nDblWri, nDblRea=nDblRea, nIntWri=nIntWri, nIntRea=nIntRea, nStrWri=nStrWri);
  annotation(Documentation(info="<html>
<p>
This function is a wrapper for
<a href=\"modelica://Buildings.Obsolete.Utilities.IO.Python27.Functions.BaseClasses.exchange\">
Buildings.Obsolete.Utilities.IO.Python27.Functions.BaseClasses.exchange</a>.
...
<a href=\"modelica://Buildings.Obsolete.Utilities.IO.Python27.Functions.Examples\">
Buildings.Obsolete.Utilities.IO.Python27.Functions.Examples</a>
for examples.
</p>
</html>", revisions="<html>
<ul>
