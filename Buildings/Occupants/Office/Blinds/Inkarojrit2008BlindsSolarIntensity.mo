...
model Inkarojrit2008BlindsSolarIntensity
  ...
  parameter Real A1=3.22
    "Slope of solar intensity at window";
  parameter Real A2=1.22
    "Slope of Occupants' brightness sensitivity";
  parameter Real B=-8.94
    "Intercept";
  parameter Real LSen=4
    "Self-reported sensitivity to brightness,
  ...
  Real p(
    ...
    final min=0,
    final max=1)
    "The probability of keeping the blinds on";
protected
...
equation
  ...
  when sampleTrigger then
    curSeed=seed*time;
    if occ then
      p=1-Modelica.Math.exp(A1*Modelica.Math.log10(H) + A2*LSen + B)/(Modelica.Math.exp(A1*Modelica.Math.log10(H) + A2*LSen + B) + 1);
      if Buildings.Occupants.BaseClasses.binaryVariableGeneration(p,
