...
model Zhang2012BlindsSolarAltitude
  ...
  parameter Real AUp=1.089
    "Slope of solar altitude for blinds up";
  parameter Real ADown=1.031
    "Slope of solar altitude for blinds down";
  parameter Real BUp=-3.446
    "Intercept for blinds up";
  parameter Real BDown=-3.424
    "Intercept for blinds down";
  parameter Integer seed=10
    "Seed for the random number generator";
  parameter Modelica.SIunits.Time samplePeriod=120
    "Sample period";
  Modelica.Blocks.Interfaces.RealInput solarAltitude(unit="rad")
    "Solar Altitude"
  ...
  Real pUp(final unit="1", final min=0, final max=1)
    "The probability of blinds up";
  Real pDown(final unit="1", final min=0, final max=1)
    "The probability of blinds down";
protected
...
initial equation
  ...
  curSeed=t0*seed;
  isOpen=false
    "Initial state of blinds is deployed";
...
equation
  ...
  when sampleTrigger then
    curSeed=seed*time;
    if occ then
      if not pre(isOpen) then
        // blindState is either 0 or 1.
        pUp=0;
  ...
  annotation(Icon(graphics={Rectangle(extent={{-60, 40}, {60,-40}}, lineColor={28, 108, 200}), Text(extent={{-40, 20}, {40,-20}}, lineColor={28, 108, 200}, fillColor={0, 0, 255}, fillPattern=FillPattern.Solid, textStyle={TextStyle.Bold}, textString="Blinds_SA")}), defaultComponentName="bli", Documentation(info="<html>
<p>
