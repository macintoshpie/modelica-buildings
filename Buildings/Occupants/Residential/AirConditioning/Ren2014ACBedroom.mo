...
model Ren2014ACBedroom
  ...
  parameter Real u1=304.40
    "Threshold for turning off the AC of the Weibull Distribution";
  parameter Real u2=301.90
    "Threshold for turning on the AC of the Weibull Distribution";
  parameter Real L1=13.34
    "Normalization factor for turning off the AC of the Weibull Distribution";
  parameter Real L2=42.86
    "Normalization factor for turning on the AC of the Weibull Distribution";
  parameter Real k1=1.73
    "Shape factor for turning off the AC of the Weibull Distribution";
  parameter Real k2=1.80
    "Shape factor for turning on the AC of the Weibull Distribution";
  ...
  Real pOn(final unit="1", final min=0, final max=1)
    "Probability of turning on the AC";
  Real pOff(final unit="1", final min=0, final max=1)
    "Probability of turning off the AC";
protected
...
initial equation
  ...
  curSeed=t0*seed;
  on=false
    "The initial state of AC is off";
  pOn=0;
  pOff=0;
equation
  ...
  when sampleTrigger then
    ...
    if occ then
      if pre(on) then
        on=not Buildings.Occupants.BaseClasses.weibull1DOFF(x=TIn, u=u1, L=L1, k=k1, dt=samplePeriod, globalSeed=integer(curSeed));
      else
        on=Buildings.Occupants.BaseClasses.weibull1DON(x=TIn, u=u2, L=L2, k=k2, dt=samplePeriod, globalSeed=integer(curSeed));
      end if;
    else
      on=false;
    end if;
  end when;
  annotation(Icon(graphics={Rectangle(extent={{-60, 40}, {60,-40}}, lineColor={28, 108, 200}), Text(extent={{-40, 20}, {40,-20}}, lineColor={28, 108, 200}, fillColor={0, 0, 255}, fillPattern=FillPattern.Solid, textStyle={TextStyle.Bold}, textString="AC_Tin")}), defaultComponentName="ac", Documentation(info="<html>
<p>
