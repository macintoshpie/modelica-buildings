...
model CFDAirHeatMassBalance
  ...
  Modelica.Blocks.Interfaces.RealOutput yCFD[nSen] if haveSensor
    ...
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=270, origin={180,-250})));
protected
  ...
  parameter Modelica.SIunits.Temperature TRooAve_fixed=Medium.T_default
    ...
    annotation(Dialog(group="Outputs if activateInterface=false"));
  parameter Modelica.SIunits.Temperature TSha_fixed[NConExtWin]=fill(Medium.T_default, NConExtWin)
    "Shade temperature used for yFixed"
    annotation(Dialog(group="Outputs if activateInterface=false"));
  parameter Modelica.SIunits.Temperature T_outflow_fixed[nPorts]=fill(Medium.T_default, nPorts)
  ...
  parameter Real C_outflow_fixed[nPorts*Medium.nC](each fixed=false)
    ...
    annotation(Dialog(group="Outputs if activateInterface=false"));
  parameter Modelica.SIunits.Density rho_start=Medium.density(Medium.setState_pTX(T=Medium.T_default, p=p_start, X=Medium.X_default))
    "Density, used to compute fluid mass";
  final parameter CFDSurfaceIdentifier surIde[kSurBou + nSurBou]=assignSurfaceIdentifier(nConExt=nConExt, nConExtWin=nConExtWin, nConPar=nConPar, nConBou=nConBou, nSurBou=nSurBou, nSur=nSur, haveShade=haveShade, nameConExt=datConExt.name, AConExt=datConExt.A, tilConExt=datConExt.til, bouConConExt=datConExt.boundaryCondition, nameConExtWin=datConExtWin.name, AConExtWin=datConExtWin.AOpa, tilConExtWin=datConExtWin.til, bouConConExtWin=datConExtWin.boundaryCondition, AGla=datConExtWin.AGla, AFra=datConExtWin.AFra, uSha=uSha_fixed, nameConPar=datConPar.name, AConPar=datConPar.A, tilConPar=datConPar.til, bouConConPar=datConPar.boundaryCondition, nameConBou=datConBou.name, AConBou=datConBou.A, tilConBou=datConBou.til, bouConConBou=datConBou.boundaryCondition, nameSurBou=surBou.name, ASurBou=surBou.A, tilSurBou=surBou.til, bouConSurBou=surBou.boundaryCondition)
  ...
  CFDSurfaceInterface cfdConExt[NConExt](final bouCon=datConExt[:].boundaryCondition) if haveConExt
    "Interface to heat port of exterior constructions"
    annotation(Placement(transformation(extent={{180, 210}, {200, 230}})));
  CFDSurfaceInterface cfdConExtWin[NConExtWin](final bouCon=datConExtWin[:].boundaryCondition) if haveConExtWin
  ...
  CFDSurfaceInterface cfdGlaUns[NConExtWin](final bouCon=datConExtWin[:].boundaryCondition) if haveConExtWin
    "Interface to heat port of unshaded part of glass"
    annotation(Placement(transformation(extent={{180, 110}, {200, 130}})));
  CFDSurfaceInterface cfdGlaSha[NConExtWin](final bouCon=datConExtWin[:].boundaryCondition) if haveShade
    "Interface to heat port of shaded part of glass"
    annotation(Placement(transformation(extent={{180, 70}, {200, 90}})));
  CFDSurfaceInterface cfdConExtWinFra[NConExtWin](final bouCon=datConExtWin[:].boundaryCondition) if haveConExtWin
    "Interface to heat port of window frame"
    annotation(Placement(transformation(extent={{180,-10}, {200, 10}})));
  CFDSurfaceInterface cfdConPar_a[NConPar](final bouCon=datConPar[:].boundaryCondition) if haveConPar
    "Interface to heat port of surface a of partition constructions"
    annotation(Placement(transformation(extent={{180,-70}, {200,-50}})));
  CFDSurfaceInterface cfdConPar_b[NConPar](final bouCon=datConPar[:].boundaryCondition) if haveConPar
    "Interface to heat port of surface b of partition constructions"
    annotation(Placement(transformation(extent={{180,-110}, {200,-90}})));
  CFDSurfaceInterface cfdConBou[NConBou](final bouCon=datConBou[:].boundaryCondition) if haveConBou
  ...
  CFDSurfaceInterface cfdSurBou[NSurBou](final bouCon=surBou[:].boundaryCondition) if haveSurBou
  ...
  CFDFluidInterface fluInt(redeclare final package Medium=Medium, final massDynamics=massDynamics, final nPorts=nPorts, final V=V, final p_start=p_start, rho_start=rho_start)
    "Fluid interface"
...
*/// Declaration of construction data
    ...
    input Modelica.SIunits.Angle tilConExtWin[nConExtWin]
      "Surface tilt";
    input Buildings.ThermalZones.Detailed.Types.CFDBoundaryConditions bouConConExtWin[nConExtWin]
      "Boundary condition";
  ...
  algorithm
    id := cat(1, {CFDSurfaceIdentifier(name=nameConExt[i], A=AConExt[i], til=tilConExt[i], bouCon=bouConConExt[i]) for i in 1 : nConExt}, {CFDSurfaceIdentifier(name=nameConExtWin[i], A=AConExtWin[i], til=tilConExtWin[i], bouCon=bouConConExtWin[i]) for i in 1 : nConExtWin}, {CFDSurfaceIdentifier(name=nameConExtWin[i] + " (glass, unshaded)", A=AGla[i]*(1-uSha[i]), til=tilConExtWin[i], bouCon=bouConConExtWin[i]) for i in 1 : nConExtWin}, {CFDSurfaceIdentifier(name=nameConExtWin[i] + " (glass, shaded)", A=AGla[i]*uSha[i], til=tilConExtWin[i], bouCon=bouConConExtWin[i]) for i in 1 :(if haveShade then nConExtWin else 0)}, {CFDSurfaceIdentifier(name=nameConExtWin[i] + " (frame)", A=AFra[i], til=tilConExtWin[i], bouCon=bouConConExtWin[i]) for i in 1 : nConExtWin}, {CFDSurfaceIdentifier(name=nameConPar[i] + " (surface a)", A=AConPar[i], til=tilConPar[i], bouCon=bouConConPar[i]) for i in 1 : nConPar}, {CFDSurfaceIdentifier(name=nameConPar[i] + " (surface b)", A=AConPar[i], til=tilConPar[i] + Modelica.Constants.pi/180, bouCon=bouConConPar[i]) for i in 1 : nConPar}, {CFDSurfaceIdentifier(name=nameConBou[i], A=AConBou[i], til=tilConBou[i], bouCon=bouConConBou[i]) for i in 1 : nConBou}, {CFDSurfaceIdentifier(name=nameSurBou[i], A=ASurBou[i], til=tilSurBou[i], bouCon=bouConSurBou[i]) for i in 1 : nSurBou});
...
equation
    ...
    for i in 1 : nConExt loop
      if datConExt[i].boundaryCondition == Buildings.ThermalZones.Detailed.Types.CFDBoundaryConditions.Temperature then
        connect(cfd.u[kConExt + i], cfdConExt[i].T_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 224}, {179, 224}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConExt + i], cfdConExt[i].Q_flow_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60, 228}, {178, 228}}, color={0, 0, 127}, smooth=Smooth.None));
      else
        connect(cfd.u[kConExt + i], cfdConExt[i].Q_flow_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 216}, {179, 216}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConExt + i], cfdConExt[i].T_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60, 212}, {179, 212}}, color={0, 0, 127}, smooth=Smooth.None));
      end if;
    end for;
  end if;
    ...
    for i in 1 : nConExtWin loop
      if datConExtWin[i].boundaryCondition == Buildings.ThermalZones.Detailed.Types.CFDBoundaryConditions.Temperature then
        connect(cfd.u[kConExtWin + i], cfdConExtWin[i].T_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 172}, {60, 172}, {60, 184}, {179, 184}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConExtWin + i], cfdConExtWin[i].Q_flow_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60, 188}, {178, 188}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.u[kGlaUns + i], cfdGlaUns[i].T_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 124}, {179, 124}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kGlaUns + i], cfdGlaUns[i].Q_flow_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60, 128}, {178, 128}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.u[kConExtWinFra + i], cfdConExtWinFra[i].T_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 4}, {179, 4}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConExtWinFra + i], cfdConExtWinFra[i].Q_flow_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60, 8}, {178, 8}}, color={0, 0, 127}, smooth=Smooth.None));
      else
        connect(cfd.u[kConExtWin + i], cfdConExtWin[i].Q_flow_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 176}, {179, 176}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConExtWin + i], cfdConExtWin[i].T_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60, 172}, {179, 172}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.u[kGlaUns + i], cfdGlaUns[i].Q_flow_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 116}, {179, 116}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kGlaUns + i], cfdGlaUns[i].T_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60, 112}, {179, 112}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.u[kConExtWinFra + i], cfdConExtWinFra[i].Q_flow_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-4}, {179,-4}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConExtWinFra + i], cfdConExtWinFra[i].T_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60,-8}, {179,-8}}, color={0, 0, 127}, smooth=Smooth.None));
      end if;
    end for;
  end if;
  if haveShade then
    for i in 1 : nConExtWin loop
      if datConExtWin[i].boundaryCondition == Buildings.ThermalZones.Detailed.Types.CFDBoundaryConditions.Temperature then
        connect(cfd.u[kGlaSha + i], cfdGlaSha[i].T_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 84}, {179, 84}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kGlaSha + i], cfdGlaSha[i].Q_flow_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60, 88}, {178, 88}}, color={0, 0, 127}, smooth=Smooth.None));
      else
        connect(cfd.u[kGlaSha + i], cfdGlaSha[i].Q_flow_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 76}, {179, 76}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kGlaSha + i], cfdGlaSha[i].T_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60, 72}, {179, 72}}, color={0, 0, 127}, smooth=Smooth.None));
      end if;
    end for;
  end if;
  if haveConPar then
    for i in 1 : nConPar loop
      if datConPar[i].boundaryCondition == Buildings.ThermalZones.Detailed.Types.CFDBoundaryConditions.Temperature then
        connect(cfd.u[kConPar_a + i], cfdConPar_a[i].T_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-56}, {179,-56}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConPar_a + i], cfdConPar_a[i].Q_flow_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60,-52}, {178,-52}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.u[kConPar_b + i], cfdConPar_b[i].T_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-96}, {179,-96}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConPar_b + i], cfdConPar_b[i].Q_flow_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60,-92}, {178,-92}}, color={0, 0, 127}, smooth=Smooth.None));
      else
        connect(cfd.u[kConPar_a + i], cfdConPar_a[i].Q_flow_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-64}, {179,-64}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConPar_a + i], cfdConPar_a[i].T_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60,-68}, {179,-68}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.u[kConPar_b + i], cfdConPar_b[i].Q_flow_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-104}, {179,-104}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConPar_b + i], cfdConPar_b[i].T_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60,-108}, {179,-108}}, color={0, 0, 127}, smooth=Smooth.None));
      end if;
    end for;
  end if;
  if haveConBou then
    for i in 1 : nConBou loop
      if datConBou[i].boundaryCondition == Buildings.ThermalZones.Detailed.Types.CFDBoundaryConditions.Temperature then
        connect(cfd.u[kConBou + i], cfdConBou[i].T_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-156}, {179,-156}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConBou + i], cfdConBou[i].Q_flow_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60,-152}, {178,-152}}, color={0, 0, 127}, smooth=Smooth.None));
      else
        connect(cfd.u[kConBou + i], cfdConBou[i].Q_flow_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-164}, {179,-164}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kConBou + i], cfdConBou[i].T_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60,-168}, {179,-168}}, color={0, 0, 127}, smooth=Smooth.None));
      end if;
    end for;
  end if;
  if haveSurBou then
    for i in 1 : nSurBou loop
      if surBou[i].boundaryCondition == Buildings.ThermalZones.Detailed.Types.CFDBoundaryConditions.Temperature then
        ...
        connect(cfd.y[kSurBou + i], cfdSurBou[i].Q_flow_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60,-212}, {178,-212}}, color={0, 0, 127}, smooth=Smooth.None));
      else
        connect(cfd.u[kSurBou + i], cfdSurBou[i].Q_flow_out)
          annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-224}, {179,-224}}, color={0, 0, 127}, smooth=Smooth.None));
        connect(cfd.y[kSurBou + i], cfdSurBou[i].T_in)
          annotation(Line(points={{-19, 190}, {60, 190}, {60,-228}, {179,-228}}, color={0, 0, 127}, smooth=Smooth.None));
      end if;
    end for;
  end if;
  connect(cfdConExt.port, conExt)
    annotation(Line(points={{200, 220}, {240, 220}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(cfdConExtWin.port, conExtWin)
    annotation(Line(points={{200, 180}, {240, 180}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(cfdGlaUns.port, glaUns)
    annotation(Line(points={{200, 120}, {220, 120}, {220, 120}, {240, 120}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(cfdGlaSha.port, glaSha)
    annotation(Line(points={{200, 80}, {240, 80}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(cfdConExtWinFra.port, conExtWinFra)
    annotation(Line(points={{200, 0}, {242, 0}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(cfdConPar_a.port, conPar_a)
    annotation(Line(points={{200,-60}, {242,-60}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(cfdConPar_b.port, conPar_b)
    annotation(Line(points={{200,-100}, {242,-100}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(cfdConBou.port, conBou)
    annotation(Line(points={{200,-160}, {222,-160}, {222,-160}, {242,-160}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(cfdSurBou.port, conSurBou)
    annotation(Line(points={{200,-220}, {241,-220}}, color={191, 0, 0}, smooth=Smooth.None));
  // Connections to heat port of air volume
  connect(cfd.y[kHeaPorAir + 1], cfdHeaPorAir.T)
    annotation(Line(points={{-19, 190}, {60, 190}, {60, 8.88178e-16}, {-138, 8.88178e-16}}, color={0, 0, 127}, smooth=Smooth.None));
  // Connections to shade
  if haveShade then
    for i in 1 : nConExtWin loop
      connect(cfd.u[kUSha + i], uSha[i])
        annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 200}, {-260, 200}}, color={0, 0, 127}, smooth=Smooth.None));
      connect(cfd.u[kQRadAbs_flow + i], QRadAbs_flow[i])
        annotation(Line(points={{-42, 190}, {-60, 190}, {-60, 90}, {-260, 90}}, color={0, 0, 127}, smooth=Smooth.None));
      connect(cfd.y[kTSha + i], TSha[i])
        annotation(Line(points={{-19, 190}, {60, 190}, {60, 60}, {-250, 60}}, color={0, 0, 127}, smooth=Smooth.None));
    end for;
  end if;
  ...
  connect(QTotCon_flow_W.y, cfd.u[kQConGai_flow + 1])
    annotation(Line(points={{-119,-50}, {-60,-50}, {-60, 190}, {-42, 190}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(QLat_flow, cfd.u[kQLatGai_flow + 1])
    annotation(Line(points={{-260,-160}, {-190,-160}, {-190,-72}, {-60,-72}, {-60, 190}, {-42, 190}}, color={0, 0, 127}, smooth=Smooth.None));
  // Connections to fluid port
  connect(ports, fluInt.ports)
    annotation(Line(points={{0,-238}, {0,-198}}, color={0, 127, 255}, smooth=Smooth.None));
  ...
  connect(cfd.u[kFluIntP + 1], fluInt.p)
    annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-180}, {-11,-180}}, color={0, 0, 127}, smooth=Smooth.None));
  for i in 1 : nPorts loop
    connect(cfd.u[kFluIntM_flow + i], fluInt.m_flow[i])
      annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-184}, {-11,-184}}, color={0, 0, 127}, smooth=Smooth.None));
    connect(cfd.u[kFluIntT_inflow + i], fluInt.T_inflow[i])
      annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-188}, {-11,-188}}, color={0, 0, 127}, smooth=Smooth.None));
    for j in 1 : Medium.nXi loop
      connect(cfd.u[kFluIntXi_inflow +(i-1)*Medium.nXi + j], fluInt.Xi_inflow[(i-1)*Medium.nXi + j])
        annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-192}, {-11,-192}}, color={0, 0, 127}, smooth=Smooth.None));
    end for;
    for j in 1 : Medium.nC loop
      connect(cfd.u[kFluIntC_inflow +(i-1)*Medium.nC + j], fluInt.C_inflow[(i-1)*Medium.nC + j])
        annotation(Line(points={{-42, 190}, {-60, 190}, {-60,-196}, {-11,-196}}, color={0, 0, 127}, smooth=Smooth.None));
    end for;
  end for;
  // Input signals to fluInt block
  ...
  for i in 1 : nPorts loop
    connect(cfd.y[kFluIntT_outflow + i], fluInt.T_outflow[i])
      annotation(Line(points={{-19, 190}, {60, 190}, {60,-188}, {12,-188}}, color={0, 0, 127}, smooth=Smooth.None));
    for j in 1 : Medium.nXi loop
      connect(cfd.y[kFluIntXi_outflow +(i-1)*Medium.nXi + j], fluInt.Xi_outflow[(i-1)*Medium.nXi + j])
        annotation(Line(points={{-19, 190}, {60, 190}, {60,-192}, {12,-192}}, color={0, 0, 127}, smooth=Smooth.None));
    end for;
    for j in 1 : Medium.nC loop
      connect(cfd.y[kFluIntC_outflow +(i-1)*Medium.nC + j], fluInt.C_outflow[(i-1)*Medium.nC + j])
        annotation(Line(points={{-19, 190}, {60, 190}, {60,-196}, {12,-196}}, color={0, 0, 127}, smooth=Smooth.None));
    end for;
  end for;
  ...
  if haveSensor then
    for i in 1 : nSen loop
      connect(cfd.y[kSen + i], yCFD[i])
        annotation(Line(points={{-19, 190}, {60, 190}, {60,-234}, {180,-234}, {180,-250}}, color={0, 0, 127}, smooth=Smooth.None));
    end for;
  end if;
  connect(heaPorAir, senHeaFlo.port_a)
    annotation(Line(points={{-240, 0}, {-210, 0}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(senHeaFlo.port_b, cfdHeaPorAir.port)
    annotation(Line(points={{-190, 0}, {-160, 0}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(senHeaFlo.Q_flow, QTotCon_flow.u1)
    annotation(Line(points={{-200,-10}, {-200,-44}, {-182,-44}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(QCon_flow, QTotCon_flow.u2)
    annotation(Line(points={{-260,-100}, {-200,-100}, {-200,-56}, {-182,-56}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(QTotCon_flow.y, QTotCon_flow_W.u)
    annotation(Line(points={{-159,-50}, {-142,-50}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(preferredView="info", Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-240,-240}, {240, 240}}), graphics), Icon(coordinateSystem(preserveAspectRatio=false, extent={{-240,-240}, {240, 240}}), graphics={Rectangle(extent={{-144, 184}, {148,-200}}, pattern=LinePattern.None, lineColor={0, 0, 0}, fillColor={170, 213, 255}, fillPattern=FillPattern.Sphere)}), Documentation(info="<html>
<p>
