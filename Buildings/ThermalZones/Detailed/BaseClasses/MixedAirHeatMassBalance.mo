...
model MixedAirHeatMassBalance
  ...
  HeatTransfer.Windows.InteriorHeatTransferConvective convConWin[NConExtWin](
    ...
    each conMod=conMod,
    each hFixed=hFixed,
    final A=AConExtWinGla + AConExtWinFra) if haveConExtWin
    "Model for convective heat transfer at window"
    annotation(
      Placement(
        transformation(
          extent={{98, 110}, {118, 130}})));
...
protected
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector theConConExtWin(
    final m=nConExtWin) if haveConExtWin
    "Thermal collector to convert from vector to scalar connector"
    annotation(
      Placement(
        transformation(
...
equation
  ...
  connect(conQCon_flow.port, vol.heatPort)
    annotation(
      Line(
        points={{-200,-100}, {-118,-100}, {20,-100}, {20,-200}, {10,-200}},
        color={191, 0, 0}));
  connect(QCon_flow, conQCon_flow.Q_flow)
    annotation(
      Line(
        points={{-260,-100}, {-240,-100}, {-220,-100}},
        color={0, 0, 127}));
  connect(QLat_flow, mWat_flow.u)
    annotation(
      Line(
        points={{-260,-160}, {-222,-160}},
        color={0, 0, 127}));
  connect(mWat_flow.y, vol.mWat_flow)
    annotation(
      Line(
        points={{-199,-160}, {-168,-160}, {-168,-212}, {-30,-212}, {-30,-180}, {16,-180}, {16,-192}, {12,-192}},
        color={0, 0, 127}));
  connect(conQLat_flow.Q_flow, QLat_flow)
    annotation(
      Line(
        points={{-220,-80}, {-230,-80}, {-230,-160}, {-260,-160}},
        color={0, 0, 127}));
  connect(conQLat_flow.port, vol.heatPort)
    annotation(
      Line(
        points={{-200,-80}, {-96,-80}, {20,-80}, {20,-200}, {10,-200}},
        color={191, 0, 0}));
  connect(vol.C_flow, C_flow)
    annotation(
      Line(
        points={{12,-206}, {16,-206}, {16,-220}, {-260,-220}},
        color={0, 0, 127}));
  annotation(
    Diagram(
      coordinateSystem(
