...
model ExteriorWall
  ...
  parameter Buildings.ThermalZones.Detailed.BaseClasses.ParameterConstruction conPar[1](each til=Buildings.Types.Tilt.Wall, each azi=0, each A=3*10, layers={extConMat})
    "Data for construction"
    annotation(Placement(transformation(extent={{-120, 60}, {-100, 80}})));
  Buildings.ThermalZones.Detailed.Constructions.Construction conExt[1](each stateAtSurface_a=false, each stateAtSurface_b=false, A=conPar[:].A, layers=conPar.layers, til={Buildings.Types.Tilt.Wall})
    "Construction of an exterior wall without a window"
    annotation(Placement(transformation(extent={{0,-64}, {60,-4}})));
  Buildings.ThermalZones.Detailed.BaseClasses.ExteriorBoundaryConditions bouConExt(nCon=1, linearizeRadiation=false, conMod=Buildings.HeatTransfer.Types.ExteriorConvection.TemperatureWind, lat=0.73268921998722, conPar=conPar)
  ...
  Buildings.HeatTransfer.Convection.Interior con[1](A={3*10}, til={Buildings.Types.Tilt.Wall})
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalCollector theCol(m=1)
    ...
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=270, origin={-70, 20})));
...
equation
  connect(prescribedTemperature.port, theCol.port_b)
    annotation(Line(points={{-100, 20}, {-90, 20}, {-90, 20}, {-80, 20}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(theCol.port_a, con.fluid)
    annotation(Line(points={{-60, 20}, {-40, 20}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(weaDat.weaBus, bouConExt.weaBus)
    annotation(Line(points={{120, 70}, {140, 70}, {140,-58.6}, {110.867,-58.6}}, color={255, 204, 51}, thickness=0.5, smooth=Smooth.None));
  connect(TRoo.y, prescribedTemperature.T)
    annotation(Line(points={{-159, 20}, {-122, 20}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(con.solid, conExt.opa_b)
    annotation(Line(points={{-20, 20}, {60.2, 20}, {60.2,-14}}, color={191, 0, 0}, smooth=Smooth.None));
  connect(bouConExt.opa_a, conExt.opa_a)
    annotation(Line(points={{76,-46.6667}, {44,-46.6667}, {44,-46}, {-1.66533e-15,-46}, {-1.66533e-15,-14}}, color={191, 0, 0}, smooth=Smooth.None));
  annotation(experiment(Tolerance=1e-6, StopTime=1209600), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/Detailed/Constructions/Examples/ExteriorWall.mos"
    "Simulate and plot"), Diagram(coordinateSystem(preserveAspectRatio=true, extent={{-200,-100}, {200, 100}})), Documentation(info="<html>
<p>
