...
protected
  Modelica.Blocks.Sources.Constant TLow(
    final k=TClear)
    "Lower temperature"
    annotation(
      Placement(
  ...
  Modelica.Blocks.Math.Feedback feeT
    ...
    annotation(
      Placement(
        transformation(
          extent={{-50, 30}, {-30, 50}})));
  Modelica.Blocks.Math.Gain gaiT(
    final k=1/(TDark-TClear))
    "Gain for temperature"
    annotation(
      Placement(
        transformation(
          extent={{-8, 30}, {12, 50}})));
  Modelica.Blocks.Sources.Constant HLow(
    final k=HClear)
    "Lower irradiation"
    annotation(
      Placement(
  ...
  Modelica.Blocks.Math.Feedback feeH
    ...
    annotation(
      Placement(
        transformation(
          extent={{-50,-50}, {-30,-30}})));
  Modelica.Blocks.Math.Gain gaiH(
    final k=1/(HDark-HClear))
...
equation
  connect(TLow.y, feeT.u2)
    annotation(
      Line(
        points={{-59, 20}, {-40, 20}, {-40, 32}},
        color={0, 0, 127}));
  connect(feeT.u1, T)
    annotation(
      Line(
        points={{-48, 40}, {-76, 40}, {-120, 40}},
        color={0, 0, 127}));
  ...
  connect(HLow.y, feeH.u2)
    annotation(
      Line(
        points={{-59,-60}, {-40,-60}, {-40,-48}},
        color={0, 0, 127}));
  connect(feeH.y, gaiH.u)
    annotation(
      Line(
        points={{-31,-40}, {-20,-40}, {-10,-40}},
        color={0, 0, 127}));
  connect(gaiH.y, limH.u)
    annotation(
      Line(
        points={{13,-40}, {28,-40}, {28,-40}},
        color={0, 0, 127}));
  connect(limT.y, product.u1)
    annotation(
      Line(
        points={{51, 40}, {58, 40}, {58, 6}, {68, 6}},
        color={0, 0, 127}));
