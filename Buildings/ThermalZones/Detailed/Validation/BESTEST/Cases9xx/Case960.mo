...
model Case960
  ...
  extends Buildings.ThermalZones.Detailed.Validation.BESTEST.Cases6xx.Case600(
    ...
    staRes(
      ...
      peakCoo(
        ...
        Max=-1.404*1000,
        Mean=-1.212*1000)));
  Buildings.HeatTransfer.Conduction.MultiLayer parWal(
    layers=matLayPar,
    A=8*2.7,
  ...
  Buildings.ThermalZones.Detailed.MixedAir sunSpa(
    ...
    annotation(
      Placement(
        transformation(
          extent={{154,-30}, {184, 0}})));
  Modelica.Blocks.Sources.Constant qConGai_flow1(
    k=0)
    ...
    annotation(
      Placement(
        transformation(
          extent={{80, 16}, {88, 24}})));
  Modelica.Blocks.Sources.Constant qRadGai_flow1(
    k=0)
    "Radiative heat gain"
    annotation(
      Placement(
  ...
  Modelica.Blocks.Routing.Multiplex3 multiplex3_2
    annotation(
      Placement(
        transformation(
          extent={{108, 16}, {116, 24}})));
  Modelica.Blocks.Sources.Constant qLatGai_flow1(
    k=0)
    "Latent heat gain"
    annotation(
      Placement(
  ...
  Buildings.Fluid.Sources.MassFlowSource_T sinInf2(
    ...
    annotation(
      Placement(
        transformation(
          extent={{124,-144}, {136,-132}})));
  Modelica.Blocks.Sources.Constant InfiltrationRate1(
    k=-16*2.7*0.5/3600)
    "0.41 ACH adjusted for the altitude (0.5 at sea level)"
    annotation(
      Placement(
        transformation(
          extent={{54,-134}, {62,-126}})));
  Modelica.Blocks.Math.Product product1
    annotation(
      Placement(
        transformation(
          extent={{74,-138}, {84,-128}})));
  Buildings.Fluid.Sensors.Density density1(
    redeclare package Medium=MediumA)
    "Air density inside the building"
    annotation(
      Placement(
        transformation(
          extent={{84,-162}, {74,-152}})));
...
equation
  ...
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/Detailed/Validation/BESTEST/Cases9xx/Case960.mos"
        "Simulate and plot"),
    experiment(
      StopTime=3.1536e+07,
      Interval=3600,
