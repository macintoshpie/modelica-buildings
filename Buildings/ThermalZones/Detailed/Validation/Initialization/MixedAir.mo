...
model MixedAir
  ...
  HeatTransfer.Conduction.MultiLayer conOut[nSurBou](each A=6*4, each layers=matLayPar, each steadyStateInitial=true)
    "Construction that is modeled outside of room"
    annotation(Placement(transformation(extent={{100,-60}, {120,-40}})));
  Buildings.Fluid.Sources.Boundary_pT boundary(nPorts=1, redeclare package Medium=MediumA, T=T_start)
    "Boundary condition"
...
equation
  ...
  connect(TSoi.port, roo.surf_conBou)
    annotation(Line(points={{100,-10}, {72,-10}, {72, 24}}, color={191, 0, 0}, smooth=Smooth.None));
  ...
  connect(roo.ports[1], boundary.ports[1])
    annotation(Line(points={{51, 30}, {34, 30}, {34, 10}, {20, 10}}, color={0, 127, 255}, smooth=Smooth.None));
  connect(HSol.y, weaDat.HGloHor_in)
    annotation(Line(points={{151, 120}, {154, 120}, {154, 137}, {159, 137}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(HSol.y, weaDat.HDifHor_in)
    annotation(Line(points={{151, 120}, {154, 120}, {154, 140.5}, {159, 140.5}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(T.y, weaDat.TDryBul_in)
    annotation(Line(points={{151, 160}, {154, 160}, {154, 159}, {159, 159}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(weaDat.TBlaSky_in, T.y)
    annotation(Line(points={{159, 157}, {155.5, 157}, {155.5, 160}, {151, 160}}, color={0, 0, 127}, smooth=Smooth.None));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100}, {200, 200}})), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/Detailed/Validation/Initialization/MixedAir.mos"
...
</html>"), experiment(Tolerance=1e-06, StopTime=172800));
end MixedAir;
