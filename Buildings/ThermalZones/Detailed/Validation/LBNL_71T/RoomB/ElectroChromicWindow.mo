...
model ElectroChromicWindow
  ...
  BoundaryConditions.WeatherData.Bus weaBus
    annotation(Placement(transformation(extent={{130, 10}, {154, 34}}), iconTransformation(extent={{-116, 36}, {-96, 56}})));
  ...
  Modelica.Blocks.Sources.CombiTimeTable refRes(tableOnFile=true, tableName="EnergyPlus", fileName=Modelica.Utilities.Files.loadResource("modelica://Buildings/Resources/Data/ThermalZones/Detailed/Validation/LBNL_71T/RoomB/EnergyPlusHeatingCoolingPower.txt"), columns=2 : 2)
  ...
  Controls.Continuous.LimPID conHea(Td=60, initType=Modelica.Blocks.Types.InitPID.InitialState, controllerType=Modelica.Blocks.Types.SimpleController.PI, yMin=-1, k=5, Ti=30)
    "Controller for heating"
    annotation(Placement(transformation(extent={{-40,-100}, {-20,-80}})));
  Modelica.Blocks.Math.Gain gaiHea(k=1E4)
    "Gain for heating"
    annotation(Placement(transformation(extent={{0,-100}, {20,-80}})));
  Modelica.Blocks.Sources.Constant TRooSet(k=297.15)
    "Set point for room air"
...
equation
  connect(uSha.y, replicator.u)
    annotation(Line(points={{-23, 64}, {-6, 64}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(roo.uSha, replicator.y)
    annotation(Line(points={{60.4, 22}, {40, 22}, {40, 64}, {17, 64}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(weaBus, roo.weaBus)
    annotation(Line(points={{142, 22}, {136, 22}, {136, 21.9}, {99.9, 21.9}}, color={255, 204, 51}, thickness=0.5), Text(textString="%first", index=-1, extent={{-6, 3}, {-6, 3}}));
  connect(weaBus, weaDat1.weaBus)
    annotation(Line(points={{142, 22}, {142, 20}, {196, 20}, {196, 114}, {160, 114}}, color={255, 204, 51}, thickness=0.5), Text(textString="%first", index=-1, extent={{-6, 3}, {-6, 3}}));
  connect(qRadGai_flow.y, multiplex3_1.u1[1])
    annotation(Line(points={{-73, 26}, {-68, 26}, {-68,-9}, {-58,-9}}, color={0, 0, 127}));
  connect(qConGai_flow.y, multiplex3_1.u2[1])
    annotation(Line(points={{-75,-16}, {-66.5,-16}, {-58,-16}}, color={0, 0, 127}));
  connect(qLatGai_flow.y, multiplex3_1.u3[1])
    annotation(Line(points={{-75,-56}, {-72,-56}, {-72,-26}, {-58,-26}, {-58,-23}}, color={0, 0, 127}));
  connect(multiplex3_1.y, roo.qGai_flow)
    annotation(Line(points={{-35,-16}, {-35,-16}, {38,-16}, {38, 12}, {60.4, 12}}, color={0, 0, 127}));
  connect(uWin.y, roo.uWin[1])
    annotation(Line(points={{11, 16}, {34, 16}, {34, 17}, {60.4, 17}}, color={0, 0, 127}));
  connect(preHea.port, roo.heaPorAir)
    annotation(Line(points={{60,-90}, {72,-90}, {72,-58}, {72, 4}, {81, 4}}, color={191, 0, 0}));
  ...
  connect(TRooAir.T, conHea.u_m)
    annotation(Line(points={{100,-90}, {110,-90}, {110,-112}, {-30,-112}, {-30,-102}}, color={0, 0, 127}));
  annotation(Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-140,-140}, {220, 160}})), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/Detailed/Validation/LBNL_71T/RoomB/ElectroChromicWindow.mos"
