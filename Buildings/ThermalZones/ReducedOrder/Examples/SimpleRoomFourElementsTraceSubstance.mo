...
model SimpleRoomFourElementsTraceSubstance
  ...
  extends Buildings.ThermalZones.ReducedOrder.Examples.SimpleRoomFourElements(thermalZoneFourElements(redeclare package Medium=Medium, use_C_flow=true, nPorts=2));
  replaceable package Medium=Buildings.Media.Air(extraPropertiesNames={"C_flow"})
    "Medium model"
    annotation(choicesAllMatching=true);
  parameter Real airChaRat(final unit="1/s")=2/3600
    "Air change rate";
  Modelica.Blocks.Sources.Pulse traSub(amplitude=10.4*2*(28.949/44.01), width=50, period=86400, offset=0)
    "Source of trace substance (for example CO2)"
    annotation(Placement(transformation(extent={{-88,-58}, {-68,-38}})));
  Buildings.Fluid.Sources.MassFlowSource_T souAir(redeclare package Medium=Medium, use_C_in=true, m_flow=airChaRat*rho_default*thermalZoneFourElements.VAir, nPorts=1)
    "Source of air"
    annotation(Placement(transformation(extent={{-10,-10}, {10, 10}}, rotation=0, origin={-30,-70})));
  Buildings.Fluid.Sources.Boundary_pT sinAir(redeclare package Medium=Medium, C={400}, nPorts=1)
    "Sink of air"
    annotation(Placement(transformation(extent={{10,-10}, {-10, 10}}, rotation=180, origin={-28,-90})));
  Modelica.Blocks.Sources.Ramp traSubAmb(height=200, duration(displayUnit="d")=259200, offset=200)
    "Source for CO2 concentration in fresh air"
...
equation
  ...
  connect(souAir.ports[1], thermalZoneFourElements.ports[1])
    annotation(Line(points={{-20,-70}, {2,-70}, {2,-24}, {44,-24}, {44,-4}, {84,-4}, {84,-1.95}, {83,-1.95}}, color={0, 127, 255}));
  connect(sinAir.ports[1], thermalZoneFourElements.ports[2])
    annotation(Line(points={{-18,-90}, {4,-90}, {4,-26}, {46,-26}, {46,-4}, {86,-4}, {86,-1.95}, {83,-1.95}}, color={0, 127, 255}));
...
</html>"), experiment(Tolerance=1e-6, StopTime=604800), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/ReducedOrder/Examples/SimpleRoomFourElementsTraceSubstance.mos"
    "Simulate and plot"), Diagram(coordinateSystem(extent={{-100,-120}, {100, 100}})), Icon(coordinateSystem(extent={{-100,-100}, {100, 100}})));
