...
function splitFacVal
  ...
  input Modelica.SIunits.Area[:] AArray
    "Vector of areas";
  input Modelica.SIunits.Area[nCol] AExt
    "Vector of exterior wall areas";
  input Modelica.SIunits.Area[nCol] AWin
    "Vector of window areas";
  output Real[nRow, nCol] splitFacValues
    "Split factor values for ThermSplitter";
protected
  Modelica.SIunits.Area ATot=sum(AArray)
    "Total area";
  Integer j=1
    "Row counter";
  Integer k=1
