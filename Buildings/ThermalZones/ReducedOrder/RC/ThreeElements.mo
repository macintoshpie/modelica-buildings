...
model ThreeElements
  ...
  extends TwoElements(
    AArray={ATotExt, ATotWin, AInt, AFloor});
  ...
  BaseClasses.ExteriorWall floorRC(
    ...
    final CExt=CFloor,
    final T_start=T_start) if AFloor > 0
    ...
    annotation(
      Placement(
        transformation(
          ...
          rotation=90,
          origin={-12,-116})));
  Modelica.Blocks.Sources.Constant hConFloor_const(
    final k=AFloor*hConFloor) if AFloor > 0
...
equation
  ...
  elseif ATotExt > 0 and ATotWin > 0 and AInt > 0 and AFloor > 0 then
    connect(thermSplitterIntGains.portOut[4], floorRC.port_a)
      annotation(
        Line(
          points={{190, 86}, {190, 80}, {-38, 80}, {-38,-132}, {-10.9091,-132}, {-10.9091,-136}},
          color={191, 0, 0},
          smooth=Smooth.None));
  ...
  connect(intWallRC.port_a, resIntWallFloor.port_a)
    annotation(
      Line(
        points={{182,-40}, {182,-40}, {168,-40}, {168,-90}, {168,-106}, {194,-106}},
        color={191, 0, 0}));
  connect(resFloorWin.port_b, resExtWallFloor.port_b)
    annotation(
      Line(
        points={{-80,-120}, {-80,-120}, {-80,-132}, {-144,-132}, {-144,-121}},
        color={191, 0, 0}));
  connect(resExtWallFloor.port_a, convExtWall.solid)
    annotation(
      Line(
        points={{-144,-101}, {-144,-40}, {-114,-40}},
        color={191, 0, 0}));
  connect(hConFloor_const.y, convFloor.Gc)
    annotation(
      Line(
        points={{6.5,-116}, {-4,-116}, {-4,-116}},
        color={0, 0, 127}));
  connect(convFloor.fluid, senTAir.port)
    annotation(
      Line(
        points={{-12,-108}, {-12,-40}, {66,-40}, {66, 0}, {80, 0}},
        color={191, 0, 0}));
  connect(floorRC.port_a, floorIndoorSurface)
    annotation(
      Line(
        points={{-10.9091,-136}, {-10.9091,-136}, {-10.9091,-132}, {-80,-132}, {-80,-180}},
        color={191, 0, 0}));
  annotation(
