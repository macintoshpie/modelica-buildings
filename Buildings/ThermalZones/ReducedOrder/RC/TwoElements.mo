...
model TwoElements
  ...
  BaseClasses.InteriorWall intWallRC(
    ...
    annotation(
      Placement(
        transformation(
          extent={{182,-50}, {202,-28}})));
protected
  Modelica.Thermal.HeatTransfer.Components.Convection convIntWall(
    dT(
      start=0)) if AInt > 0
    ...
    annotation(
      Placement(
        transformation(
          extent={{148,-30}, {128,-50}})));
  Modelica.Blocks.Sources.Constant hConIntWall(
    k=AInt*hConInt) if AInt > 0
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resExtWallIntWall(
    final G=min(ATotExt, AInt)*hRad,
    dT(
      start=0)) if ATotExt > 0 and AInt > 0
    "Resistor between exterior walls and interior walls"
    annotation(
      Placement(
        transformation(
          extent={{138,-116}, {158,-96}})));
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resIntWallWin(
    final G=min(ATotWin, AInt)*hRad,
    dT(
      start=0)) if ATotWin > 0 and AInt > 0
...
equation
  ...
  connect(resIntWallWin.port_a, convWin.solid)
    annotation(
      Line(
        points={{74,-108}, {68,-108}, {68,-94}, {-46,-94}, {-46, 20}, {-146, 20}, {-146, 40}, {-116, 40}},
        color={191, 0, 0},
        smooth=Smooth.None));
  connect(hConIntWall.y, convIntWall.Gc)
    annotation(
      Line(
        points={{138,-55.5}, {138,-50}, {138,-50}},
        color={0, 0, 127}));
  connect(intWallRC.port_a, intWallIndoorSurface)
    annotation(
      Line(
        points={{182,-40}, {168,-40}, {168,-82}, {-120,-82}, {-120,-180}},
        color={191, 0, 0}));
  connect(convIntWall.fluid, senTAir.port)
    annotation(
      Line(
        points={{128,-40}, {66,-40}, {66, 0}, {80, 0}},
        color={191, 0, 0}));
  annotation(
    ...
    Diagram(
      ...
      graphics={Polygon(
        points={{116,-18}, {230,-18}, {230,-80}, {140,-80}, {138,-80}, {116,-80}, {116,-18}},
        lineColor={0, 0, 255},
        smooth=Smooth.None,
