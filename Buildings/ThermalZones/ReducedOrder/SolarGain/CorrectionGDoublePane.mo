...
protected
  ...
  parameter Modelica.SIunits.ReflectionCoefficient rho_1_diff=rho_11_diff +(((1-rho_11_diff)*Tai_diff)^2*rho_11_diff)/(1-(rho_11_diff*Tai_diff)^2)
  ...
  parameter Modelica.SIunits.CoefficientOfHeatTransfer Q21_diff=a1_diff*(1 +(Ta1_diff*rho_1_diff/XN2_diff))*UWin/25
    "Coefficient of heat transfer for exterior pane of double pane window";
  parameter Modelica.SIunits.CoefficientOfHeatTransfer Q22_diff=a1_diff*(Ta1_diff/XN2_diff)*(1-(UWin/7.7))
    "Coefficient of heat transfer for interior pane of double pane window";
  parameter Modelica.SIunits.CoefficientOfHeatTransfer Qsek2_diff=Q21_diff + Q22_diff
    "Overall coefficient of heat transfer for double pane window";
  parameter Modelica.SIunits.TransmissionCoefficient CorG_diff=(Ta2_diff + Qsek2_diff)/g_dir0
    "Transmission coefficient correction factor for diffuse radiation";
  parameter Modelica.SIunits.TransmissionCoefficient CorG_gr=(Ta2_diff + Qsek2_diff)/g_dir0
...
equation
  for i in 1 : n loop
    Ta_dir[i]=(((((A6*con.to_deg(inc[i]) + A5)*con.to_deg(inc[i]) + A4)*con.to_deg(inc[i]) + A3)*con.to_deg(inc[i]) + A2)*con.to_deg(inc[i]) + A1)*con.to_deg(inc[i]) + A0;
  ...
  annotation(defaultComponentName="corG", Documentation(info="<html>
