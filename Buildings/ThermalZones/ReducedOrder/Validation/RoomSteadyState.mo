...
model RoomSteadyState
  ...
  Modelica.Blocks.Sources.Constant zer(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-134, 44}, {-114, 64}})));
equation
  connect(zer.y, weaDat.HDifHor_in)
    annotation(
      Line(
        points={{-113, 54}, {-106, 54}, {-106, 52.5}, {-99, 52.5}},
        color={0, 0, 127}));
  connect(zer.y, weaDat.HGloHor_in)
    annotation(
      Line(
        points={{-113, 54}, {-106, 54}, {-106, 49}, {-99, 49}},
        color={0, 0, 127}));
  annotation(
    experiment(
      StopTime=604800,
      Tolerance=1e-06),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/ReducedOrder/Validation/RoomSteadyState.mos"
        "Simulate and plot"),
    Diagram(
      coordinateSystem(
