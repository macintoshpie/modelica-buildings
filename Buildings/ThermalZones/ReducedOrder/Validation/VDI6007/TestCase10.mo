  ...
  Modelica.Blocks.Sources.Constant HSol(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-84, 22}, {-78, 28}})));
  Modelica.Blocks.Sources.Constant sunblind(
    k=0)
...
equation
  ...
  connect(tableSolRadWindow.y[1], greaterThreshold1.u)
    annotation(
      Line(
        points={{-69.3, 73}, {-59, 73}, {-59, 65}},
        color={0, 0, 127}));
  connect(sunblind_open.y, switch1.u3)
    annotation(
      Line(
        points={{-33, 53.7}, {-33, 48}, {-30.8, 48}, {-30.8, 45.2}},
        color={0, 0, 127}));
  connect(g_sunblind.y, switch1.u1)
    annotation(
      Line(
        points={{-19, 53.7}, {-19, 48}, {-21.2, 48}, {-21.2, 45.2}},
        color={0, 0, 127}));
  connect(tableSolRadWindow.y[1], product1.u1)
    annotation(
      Line(
        points={{-69.3, 73}, {-37.65, 73}, {-7, 73}},
        color={0, 0, 127}));
  connect(greaterThreshold1.y, switch1.u2)
    annotation(
      Line(
        points={{-59, 53.5}, {-59, 46}, {-40, 46}, {-40, 64}, {-26, 64}, {-26, 45.2}},
        color={255, 0, 255}));
  connect(switch1.y, product1.u2)
    annotation(
      Line(
        points={{-26, 31.4}, {-26, 28}, {-10, 28}, {-10, 67}, {-7, 67}},
        color={0, 0, 127}));
  connect(outdoorTemp.y[1], eqAirTemp.TDryBul)
    annotation(
      Line(
        points={{-73.2, 0}, {-26, 0}, {-26,-0.4}},
        color={0, 0, 127}));
  connect(const.y, eqAirTemp.TBlaSky)
    annotation(
      Line(
        points={{-49.7, 7}, {-28.85, 7}, {-28.85, 5}, {-26, 5}},
        color={0, 0, 127}));
  connect(eqAirTemp.TEqAir, preTem.T)
    annotation(
      Line(
        points={{-3, 5}, {0.9, 5}, {0.9, 0}, {6.8, 0}},
        color={0, 0, 127}));
  connect(HSol.y, eqAirTemp.HSol[1])
    annotation(
      Line(
        points={{-77.7, 25}, {-32, 25}, {-32, 10.4}, {-26, 10.4}},
        color={0, 0, 127}));
  connect(sunblind.y, eqAirTemp.sunblind[1])
    annotation(
      Line(
        points={{-15, 19.7}, {-15, 15.85}, {-14, 15.85}, {-14, 15.8}},
        color={0, 0, 127}));
  connect(hConWall.y, theConWall.Gc)
    annotation(
      Line(
        points={{28,-14.6}, {29,-14.6}, {29,-5}},
        color={0, 0, 127}));
  ...
  connect(product1.y, thermalZoneTwoElements.solRad[1])
    annotation(
      Line(
        points={{4.5, 70}, {14, 70}, {28, 70}, {28, 31}, {43, 31}},
        color={0, 0, 127}));
  ...
  Implemented.
    ...
    experiment(
      ...
      Interval=60),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/ReducedOrder/Validation/VDI6007/TestCase10.mos"
        "Simulate and plot"));
end TestCase10;
