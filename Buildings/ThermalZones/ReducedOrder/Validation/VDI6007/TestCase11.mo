  ...
  Modelica.Blocks.Math.UnitConversions.From_degC from_degC
    ...
    annotation(
      Placement(
        transformation(
          extent={{-74, 18}, {-62, 30}})));
  Modelica.Blocks.Math.Gain gainHea(
    k=500)
    "Gain for heating"
    annotation(
      Placement(
  ...
  Controls.Continuous.LimPID conHeaCoo(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-50, 16}, {-34, 32}})));
  Modelica.Blocks.Math.Gain gainCoo(
    k=500)
    "Gain for cooling"
    annotation(
      Placement(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-5,-5}, {5, 5}},
          origin={-67, 51})));
  Modelica.Blocks.Sources.Constant DefPow(
    k=0)
    "Default power"
    annotation(
      Placement(
...
equation
  ...
  connect(switchHea.y, gainHea.u)
    annotation(
      Line(
        points={{-33.5,-34}, {-33.5,-34}, {-17.2,-34}},
        color={0, 0, 127}));
  connect(DefPow.y, switchHea.u3)
    annotation(
      Line(
        points={{-81.6, 0}, {-81.6, 0}, {-56, 0}, {-56,-38}, {-45,-38}},
        color={0, 0, 127}));
  connect(from_degC.y, conHeaCoo.u_s)
    annotation(
      Line(
        points={{-61.4, 24}, {-51.6, 24}},
        color={0, 0, 127}));
  connect(switchCoo.y, gainCoo.u)
    annotation(
      Line(
        points={{-35.5, 86}, {-26.75, 86}, {-17.2, 86}},
        color={0, 0, 127}));
  connect(hysteresis.y, switchCoo.u2)
    annotation(
      Line(
        points={{-61.5, 51}, {-54, 51}, {-54, 86}, {-47, 86}},
        color={255, 0, 255}));
  connect(DefPow.y, switchCoo.u1)
    annotation(
      Line(
        points={{-81.6, 0}, {-76, 0}, {-76, 14}, {-98, 14}, {-98, 90}, {-47, 90}},
        color={0, 0, 127}));
  connect(hysteresis.y, switchHea.u2)
    annotation(
      Line(
        points={{-61.5, 51}, {-54, 51}, {-54,-34}, {-45,-34}},
        color={255, 0, 255}));
  connect(conHeaCoo.y, switchHea.u1)
    annotation(
      Line(
        points={{-33.2, 24}, {-24, 24}, {-24,-24}, {-50,-24}, {-50,-30}, {-45,-30}},
        color={0, 0, 127}));
  ...
  connect(heatFlowSensor.port_a, thermalZoneTwoElements.intGainsConv)
    annotation(
      Line(
        points={{88,-34}, {96,-34}, {96, 20}, {92, 20}},
        color={191, 0, 0}));
  connect(cool.port, coolFlowSensor.port_a)
    annotation(
      Line(
        points={{22, 86}, {26, 86}, {26, 70}, {20, 70}},
        color={191, 0, 0}));
  connect(coolFlowSensor.port_b, thermalZoneTwoElements.intWallIndoorSurface)
    annotation(
      Line(
        points={{8, 70}, {4, 70}, {4,-24}, {44,-24}, {44,-6}, {56,-6}, {56,-2}},
        color={191, 0, 0}));
  connect(coolFlowSensor.Q_flow, add.u1)
    annotation(
      Line(
        points={{14, 64}, {14, 62}, {14, 62}, {14, 60}, {30, 60}, {30, 76}, {38, 76}},
        color={0, 0, 127}));
  connect(heatFlowSensor.Q_flow, add.u2)
    annotation(
      Line(
        points={{82,-40}, {82,-58}, {0,-58}, {0, 52}, {32, 52}, {32, 64}, {38, 64}},
        color={0, 0, 127}));
  connect(hysteresis.u, switchHea.u1)
    annotation(
      Line(
        points={{-73, 51}, {-76, 51}, {-76, 40}, {-24, 40}, {-24,-24}, {-50,-24}, {-50,-30}, {-45,-30}},
        color={0, 0, 127}));
  connect(conHeaCoo.y, switchCoo.u3)
    annotation(
      Line(
        points={{-33.2, 24}, {-24, 24}, {-24, 68}, {-52, 68}, {-52, 82}, {-47, 82}},
        color={0, 0, 127}));
  connect(add.y, switchMea.u3)
    annotation(
      Line(
        points={{61, 70}, {69, 70}, {69, 73}, {69, 110}, {126, 110}, {126, 121.1}, {138, 121.1}},
        color={0, 0, 127}));
  connect(reference.y[3], switchMea.u1)
    annotation(
      Line(
        points={{101, 90}, {130, 90}, {130, 108}, {130, 137.9}, {138, 137.9}},
        color={0, 0, 127}));
  connect(timer.y, thr.u)
    annotation(
      Line(
        points={{81, 130}, {86, 130}, {98, 130}},
        color={0, 0, 127}));
  connect(cha.y, not1.u)
    annotation(
      Line(
        points={{1, 130}, {1, 130}, {18, 130}},
        color={255, 0, 255}));
  connect(not1.y, timer.u)
    annotation(
      Line(
        points={{41, 130}, {44, 130}, {58, 130}},
        color={255, 0, 255}));
  connect(thr.y, switchMea.u2)
    annotation(
      Line(
        points={{121, 130}, {130, 130}, {134, 130}, {136, 130}, {136, 129.5}, {138, 129.5}},
        color={255, 0, 255}));
  connect(cha.u, hysteresis.y)
    annotation(
      Line(
        points={{-22, 130}, {-54, 130}, {-54, 51}, {-61.5, 51}},
        color={255, 0, 255}));
  connect(switchMea.y, mean.u)
    annotation(
      Line(
        points={{161, 129.5}, {170, 129.5}, {170, 100}, {140, 100}, {140, 80}, {148, 80}},
        color={0, 0, 127}));
  connect(reference.y[2], assEqu.u2)
    annotation(
      Line(
        points={{101, 90}, {114, 90}, {120, 90}, {120, 64}, {198, 64}},
        color={0, 0, 127}));
  connect(mean.y, assEqu.u1)
    annotation(
      Line(
        points={{171, 80}, {180, 80}, {180, 76}, {198, 76}},
        color={0, 0, 127}));
  annotation(
    Diagram(
      coordinateSystem(
        preserveAspectRatio=false,
        extent={{-100,-100}, {240, 160}})),
    Documentation(
      info="<html>
  ...
  Implemented.
    ...
    experiment(
      ...
      Interval=60),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/ReducedOrder/Validation/VDI6007/TestCase11.mos"
        "Simulate and plot"));
end TestCase11;
