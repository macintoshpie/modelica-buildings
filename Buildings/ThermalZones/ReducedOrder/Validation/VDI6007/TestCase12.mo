...
model TestCase12
  ...
  replaceable package Medium=Modelica.Media.Air.SimpleAir constrainedby Modelica.Media.Interfaces.PartialMedium
    "Medium model";
  RC.TwoElements thermalZoneTwoElements(redeclare final package Medium=Medium, hConExt=2.7, hConWin=2.7, gWin=1, nExt=1, hRad=5, nInt=1, AInt=75.5, hConInt=2.24, RWin=0.00000001, RExt={0.00436791293674}, RExtRem=0.03895919557, CExt={1600848.94}, RInt={0.000595693407511}, CInt={14836354.6282}, ratioWinConRad=0.09, nPorts=2, nOrientations=1, VAir=0.1, AWin={0}, ATransparent={7}, AExt={10.5}, extWallRC(thermCapExt(each T(fixed=true))), intWallRC(thermCapInt(each T(fixed=true))), energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial, T_start=295.15)
    "Thermal zone"
    annotation(Placement(transformation(extent={{44,-2}, {92, 34}})));
  Buildings.HeatTransfer.Sources.PrescribedTemperature preTem(port(T(start=300)))
  ...
  Modelica.Blocks.Sources.Constant hConWall(k=25*10.5)
    ...
    annotation(Placement(transformation(extent={{-4,-4}, {4, 4}}, rotation=90, origin={-2,-4})));
  ...
  Modelica.Blocks.Sources.Constant g_sunblind(k=0.15)
    ...
    annotation(Placement(transformation(extent={{-3,-3}, {3, 3}}, rotation=-90, origin={-19, 57})));
  Modelica.Blocks.Sources.Constant sunblind_open(k=1)
    "g value for sunblind open"
    annotation(Placement(transformation(extent={{-3,-3}, {3, 3}}, rotation=-90, origin={-33, 57})));
  Modelica.Blocks.Logical.GreaterThreshold greaterThreshold1(threshold=100)
    "Threshold for sunblind for one direction"
    annotation(Placement(transformation(extent={{-5,-5}, {5, 5}}, rotation=-90, origin={-59, 59})));
  ...
  Modelica.Blocks.Logical.Switch switch1
    ...
    annotation(Placement(transformation(extent={{-6,-6}, {6, 6}}, rotation=-90, origin={-26, 38})));
  Modelica.Blocks.Sources.CombiTimeTable ventRate(tableOnFile=false, columns={2}, extrapolation=Modelica.Blocks.Types.Extrapolation.Periodic, table=[
    0, 100;
    3600, 100;
    7200, 100;
    10800, 100;
  ...
  Fluid.Sources.MassFlowSource_T ventilationIn(use_m_flow_in=true, use_T_in=true, nPorts=1, redeclare package Medium=Medium)
    "Fan"
    annotation(Placement(transformation(extent={{-30,-40}, {-10,-20}})));
  Fluid.Sources.MassFlowSource_T ventilationOut(redeclare package Medium=Medium, use_m_flow_in=true, nPorts=1, use_T_in=false)
    "Fan"
...
equation
  ...
  connect(outdoorTemp.y[1], preTem.T)
    annotation(Line(points={{-43.2, 12}, {-31.2, 12}}, color={0, 0, 127}));
  ...
  connect(sunblind_open.y, switch1.u3)
    annotation(Line(points={{-33, 53.7}, {-33, 48}, {-30.8, 48}, {-30.8, 45.2}}, color={0, 0, 127}));
  connect(g_sunblind.y, switch1.u1)
    annotation(Line(points={{-19, 53.7}, {-19, 48}, {-21.2, 48}, {-21.2, 45.2}}, color={0, 0, 127}));
  connect(tableSolRadWindow.y[1], product1.u1)
    annotation(Line(points={{-69.3, 73}, {-37.65, 73}, {-7, 73}}, color={0, 0, 127}));
  connect(greaterThreshold1.y, switch1.u2)
    annotation(Line(points={{-59, 53.5}, {-59, 46}, {-40, 46}, {-40, 64}, {-26, 64}, {-26, 45.2}}, color={255, 0, 255}));
  connect(switch1.y, product1.u2)
    annotation(Line(points={{-26, 31.4}, {-26, 28}, {-10, 28}, {-10, 67}, {-7, 67}}, color={0, 0, 127}));
  connect(ventRate.y[1], gain.u)
    annotation(Line(points={{-71.3,-22}, {-68,-22}, {-63.4,-22}}, color={0, 0, 127}));
  connect(ventilationIn.ports[1], thermalZoneTwoElements.ports[1])
    annotation(Line(points={{-10,-30}, {-10,-30}, {82,-30}, {82,-1.95}, {81.475,-1.95}}, color={0, 127, 255}));
  ...
  connect(outdoorTemp.y[1], ventilationIn.T_in)
    annotation(Line(points={{-43.2, 12}, {-40, 12}, {-40,-26}, {-32,-26}}, color={0, 0, 127}));
  connect(gain.y, gain1.u)
    annotation(Line(points={{-47.3,-22}, {-44,-22}, {-44,-38}, {-70,-38}, {-70,-54}, {-63.4,-54}}, color={0, 0, 127}));
  ...
  connect(ventilationOut.ports[1], thermalZoneTwoElements.ports[2])
    annotation(Line(points={{-12,-62}, {-6,-62}, {0,-62}, {0,-36}, {84.525,-36}, {84.525,-1.95}}, color={0, 127, 255}));
  connect(perCon.port, thermalZoneTwoElements.intGainsConv)
    annotation(Line(points={{68,-74}, {96,-74}, {96, 20}, {92, 20}}, color={191, 0, 0}));
  ...
  connect(product1.y, thermalZoneTwoElements.solRad[1])
    annotation(Line(points={{4.5, 70}, {12, 70}, {22, 70}, {22, 31}, {43, 31}}, color={0, 0, 127}));
  ...
  </html>"), experiment(Tolerance=1e-6, StopTime=5.184e+006, Interval=60), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/ThermalZones/ReducedOrder/Validation/VDI6007/TestCase12.mos"
    "Simulate and plot"));
end TestCase12;
