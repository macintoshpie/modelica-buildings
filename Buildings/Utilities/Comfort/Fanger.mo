...
model Fanger
  ...
  parameter Modelica.SIunits.CoefficientOfHeatTransfer hRad(min=0, max=10)=0.8*4.7
  ...
  Modelica.Blocks.Interfaces.RealInput TAir(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC")
    "Air temperature"
    annotation(Placement(transformation(extent={{-120, 90}, {-100, 110}})));
  Modelica.Blocks.Interfaces.RealInput TRad(final quantity="ThermodynamicTemperature", final unit="K", displayUnit="degC")
    "Radiation temperature"
    annotation(Placement(transformation(extent={{-120, 50}, {-100, 70}}), iconTransformation(extent={{-120, 50}, {-100, 70}})));
  Modelica.SIunits.Temperature TOpe
    "Operative temperature";
  Modelica.SIunits.Temperature TClo(start=273.15 + 40)
    "Surface temperature of clothing";
  Modelica.SIunits.Temperature TSki(min=273.15 + 10, max=273.15 + 42)
    "Skin temperature";
  Modelica.Blocks.Interfaces.RealInput phi(min=0, max=1)
    "Relative humidity"
    annotation(Placement(transformation(extent={{-120, 10}, {-100, 30}}), iconTransformation(extent={{-120, 10}, {-100, 30}})));
  Modelica.Blocks.Interfaces.RealInput pAir_in(final quantity="Pressure", final unit="Pa", min=0) if use_pAir_in
    "Air pressure"
    annotation(Placement(transformation(extent={{-120,-110}, {-100,-90}}), iconTransformation(extent={{-120,-110}, {-100,-90}})));
  ...
  Modelica.Blocks.Interfaces.RealInput M_in(min=40, max=600, final quantity="HeatFlux", final unit="W/m2") if use_M_in
  ...
  Modelica.SIunits.CoefficientOfHeatTransfer hCom(min=0, max=10)
    "Combined heat transfer coefficient";
  Modelica.SIunits.CoefficientOfHeatTransfer hCon(min=0, max=10)
    "Convective heat transfer coefficient";
  Modelica.SIunits.Pressure pSte(min=0, max=3000)
...
protected
  ...
  Modelica.Blocks.Interfaces.RealInput M_in_internal(min=40, max=600, final quantity="HeatFlux", final unit="W/m2")
    "Needed to connect to conditional connector";
  Modelica.Blocks.Interfaces.RealInput ICl_in_internal
    "Needed to connect to conditional connector";
  Modelica.Blocks.Interfaces.RealInput pAir_in_internal(final quantity="Pressure", final unit="Pa", min=0)
    "Needed to connect to conditional connector";
initial equation
...
equation
  ...
  // fcl eq (61)
  fCl=fCl1 +(fCl2-fCl1)*Buildings.Utilities.Math.Functions.smoothHeaviside(x=(ICl_in_internal-0.5), delta=0.01);
  hCon=Buildings.Utilities.Math.Functions.smoothMax(x1=12.1*sqrt(abs(vAir_in_internal)), x2=2.38*abs(TClo-TAir)^0.25, deltaX=0.0001);
  hCom=hRad + hCon;
  ...
  annotation(defaultComponentName="com", Documentation(info="<html>
<p>
...
</html>"), Icon(graphics={Rectangle(extent={{100, 100}, {-100,-100}}, lineColor={0, 0, 0}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Text(extent={{-108, 100}, {-46, 80}}, lineColor={0, 0, 255}, textString="TAir"), Text(extent={{-100, 68}, {-38, 48}}, lineColor={0, 0, 255}, textString="TRad"), Text(visible=use_vAir_in, extent={{-106, 0}, {-44,-20}}, lineColor={0, 0, 255}, textString="vAir"), Text(visible=use_M_in, extent={{-114,-30}, {-52,-50}}, lineColor={0, 0, 255}, textString="M"), Text(extent={{-100, 32}, {-50, 10}}, lineColor={0, 0, 255}, textString="phi"), Text(visible=use_pAir_in, extent={{-104,-84}, {-48,-100}}, lineColor={0, 0, 255}, textString="pAir"), Text(visible=use_ICl_in, extent={{-108,-62}, {-52,-78}}, lineColor={0, 0, 255}, textString="ICl"), Text(extent={{40, 48}, {102, 28}}, lineColor={0, 0, 255}, textString="PMV"), Text(extent={{44,-34}, {106,-54}}, lineColor={0, 0, 255}, textString="PPD"), Line(points={{-50, 42}, {-50,-38}}, color={0, 0, 0}, smooth=Smooth.None), Line(points={{50,-38}, {-50,-38}}, color={0, 0, 0}, smooth=Smooth.None), Line(points={{-44, 30}, {-30, 24}, {-22, 10}, {-18,-14}, {-6,-20}, {2,-20}, {8,-16}, {12,-12}, {16, 6}, {24, 24}, {38, 30}}, color={255, 0, 0}, smooth=Smooth.None)}));
end Fanger;
