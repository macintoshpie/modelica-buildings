...
partial model FluidInterface
  ...
  Modelica.Fluid.Interfaces.FluidPorts_b ports[nPorts](
    ...
    m_flow(
      each max=if flowDirection == Modelica.Fluid.Types.PortFlowDirection.Leaving then 0 else + Modelica.Constants.inf,
      each min=if flowDirection == Modelica.Fluid.Types.PortFlowDirection.Entering then 0 else-Modelica.Constants.inf))
    annotation(
      Placement(
        transformation(
          extent={{88, 40}, {108,-40}})));
  Modelica.Blocks.Math.Sum sumHTot_flow(
    nin=nPorts)
    ...
    annotation(
      Placement(
        transformation(
          extent={{0, 70}, {20, 90}})));
  Modelica.Blocks.Interfaces.RealOutput HSen_flow(
    unit="W")
...
protected
  parameter Modelica.Fluid.Types.PortFlowDirection flowDirection=Modelica.Fluid.Types.PortFlowDirection.Bidirectional
    ...
    annotation(
      ...
      Dialog(
        tab="Advanced"));
  Modelica.Blocks.Sources.Constant m_flow_par(
    final k=m_flow)
...
equation
  if use_m_flow_in then
    connect(m_flow_in, bou.m_flow_in)
      annotation(
        Line(
          points={{-100, 80}, {-86, 80}, {-74, 80}, {-74, 8}, {-52, 8}},
          color={0, 0, 127}));
  else
    connect(m_flow_par.y, bou.m_flow_in)
      annotation(
        Line(
          points={{-67,-20}, {-60,-20}, {-60, 8}, {-52, 8}},
          color={0, 0, 127}));
  end if;
