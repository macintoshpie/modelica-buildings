...
model CalBayComm
  ...
  Modelica.Blocks.Sources.CombiTimeTable setPoint(tableOnFile=false, extrapolation=Modelica.Blocks.Types.Extrapolation.Periodic, smoothness=Modelica.Blocks.Types.Smoothness.ConstantSegments, table=[
    -1, 12;
    119, 6;
    239, 8;
    359, 9;
  ...
  Utilities.IO.Python36.Real_Real calBay(functionName="CalBayComm", nDblWri=1, nDblRea=2, moduleName="CalBayComm", final samplePeriod=samplePeriod)
  ...
  block lightingController
    ...
    Modelica.Blocks.Discrete.ZeroOrderHold zeroOrderHold(final samplePeriod=samplePeriod)
      "Zero order hold for feedback"
      annotation(Placement(transformation(extent={{10,-50}, {-10,-30}})));
  equation
    connect(limiter.y, y)
      annotation(Line(points={{61, 0}, {110, 0}}, color={0, 0, 127}, smooth=Smooth.None));
    connect(add.y, limiter.u)
      annotation(Line(points={{11, 4.44089e-16}, {26, 4.44089e-16}, {26, 0}, {38, 0}}, color={0, 0, 127}, smooth=Smooth.None));
    connect(limiter.y, zeroOrderHold.u)
      annotation(Line(points={{61, 0}, {70, 0}, {70,-40}, {12,-40}}, color={0, 0, 127}, smooth=Smooth.None));
    connect(zeroOrderHold.y, add.u2)
      annotation(Line(points={{-11,-40}, {-30,-40}, {-30,-6}, {-12,-6}}, color={0, 0, 127}, smooth=Smooth.None));
    connect(gain.y, add.u1)
      annotation(Line(points={{-47, 0}, {-30, 0}, {-30, 6}, {-12, 6}}, color={0, 0, 127}, smooth=Smooth.None));
    connect(u, gain.u)
      annotation(Line(points={{-120, 8.88178e-16}, {-95, 8.88178e-16}, {-95, 0}, {-70, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  end lightingController;
equation
  connect(setPoint.y[1], feedback.u1)
    annotation(Line(points={{-59, 0}, {-38, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(calBay.yR[1], feedback.u2)
    annotation(Line(points={{61,-0.5}, {70,-0.5}, {70,-20}, {-30,-20}, {-30,-8}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(controller.y, calBay.uR[1])
    annotation(Line(points={{21, 0}, {38, 0}}, color={0, 0, 127}, smooth=Smooth.None));
  connect(feedback.y, controller.u)
    annotation(Line(points={{-21, 0}, {-2, 0}}, color={0, 0, 127}, smooth=Smooth.None));
