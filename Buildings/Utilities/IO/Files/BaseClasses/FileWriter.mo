...
protected
  ...
  parameter String insNam=getInstanceName()
    "Instance name";
  Buildings.Utilities.IO.Files.BaseClasses.FileWriterObject filWri=Buildings.Utilities.IO.Files.BaseClasses.FileWriterObject(insNam, fileName, nin + 1, isCombiTimeTable)
    "File writer object";
  discrete String str
    "Intermediate variable for constructing a single line";
  ...
  function writeLine
    ...
    input String string
      "Written string";
    input Integer isMetaData
      "=1, if line should not be included for row count of combiTimeTable";
  external "C" writeLine(
    id, string, isMetaData)
    annotation(Include="#include \"fileWriterStructure.h\"", IncludeDirectory="modelica://Buildings/Resources/C-Sources");
  end writeLine;
initial equation
  t0=time;
equation
...
algorithm
  when sampleTrigger then
    ...
    for i in 1 : nin-1 loop
      ...
      if mod(i + 1, 10) == 0 then
        ...
        // not adding this one to row count
        str := "";
      end if;
    end for;
  ...
  annotation(defaultComponentName="csvWri", Icon(coordinateSystem(preserveAspectRatio=false), graphics={Text(extent={{-86,-54}, {90,-96}}, lineColor={0, 0, 127}, horizontalAlignment=TextAlignment.Right, textString="%fileName"), Text(extent={{-86,-54}, {90,-96}}, lineColor={0, 0, 127}, horizontalAlignment=TextAlignment.Right, textString="%fileName"), Text(extent={{-86,-16}, {90,-58}}, lineColor={0, 0, 127}, horizontalAlignment=TextAlignment.Right, textString="%samplePeriod")}), Diagram(coordinateSystem(preserveAspectRatio=false)), Documentation(revisions="<html>
<ul>
<li>
