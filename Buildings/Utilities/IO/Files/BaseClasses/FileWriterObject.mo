...
class FileWriterObject
  ...
  function constructor
    ...
    input Integer numColumns
      "Number of columns that are written to file";
    input Boolean isCombiTimeTable
      "Flag to indicate whether combiTimeTable header should be prepended upon destruction";
    output FileWriterObject fileWriter
      "Pointer to the file writer";
  external "C" fileWriter=fileWriterInit(
    instanceName, fileName, numColumns, isCombiTimeTable)
    annotation(Include="#include <fileWriterInit.c>", IncludeDirectory="modelica://Buildings/Resources/C-Sources");
...
c
  ...
  function destructor
    ...
    input FileWriterObject fileWriter
      "Pointer to file writer object";
  external "C" fileWriterFree(
    fileWriter)
    annotation(Include=" #include <fileWriterFree.c>", IncludeDirectory="modelica://Buildings/Resources/C-Sources");
    annotation(Documentation(info="<html>
<p>
