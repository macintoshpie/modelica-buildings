...
model OriginalModel
  ...
  Modelica.Blocks.Continuous.LimPID conPI(controllerType=Modelica.Blocks.Types.SimpleController.PI, yMax=10)
    "Controller"
    annotation(Placement(transformation(extent={{-10, 20}, {10, 40}})));
  Modelica.Blocks.Continuous.FirstOrder firOrd(T=1, initType=Modelica.Blocks.Types.Init.InitialOutput)
    "First order element"
    annotation(Placement(transformation(extent={{50, 20}, {70, 40}})));
  Overwrite oveWriSet(description="First order system control setpoint", u(min=-10, max=10, unit="1"))
    "Overwrite block for setpoint"
    annotation(Placement(transformation(extent={{-40, 20}, {-20, 40}})));
  Overwrite oveWriAct(description="First order system input", u(min=-10, max=10, unit="1"))
    "Overwrite block for actuator signal"
    annotation(Placement(transformation(extent={{20, 20}, {40, 40}})));
  Read rea(description="First order system output", KPIs=Buildings.Utilities.IO.SignalExchange.SignalTypes.SignalsForKPIs.None)
...
equation
  ...
  connect(firOrd.y, rea.u)
    annotation(Line(points={{71, 30}, {80, 30}, {80,-20}, {52,-20}}, color={0, 0, 127}));
