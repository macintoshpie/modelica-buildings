  ...
  Buildings.Utilities.Math.FallingFactorial falFac
    ...
    annotation(
      Placement(
        transformation(
          extent={{40,-4}, {60, 16}})));
  Modelica.Blocks.Sources.IntegerConstant n(
    k=13)
...
equation
  ...
  connect(rea2Int.y, falFac.k)
    annotation(
      Line(
        points={{11, 0}, {24, 0}, {38, 0}},
        color={255, 127, 0}));
  connect(n.y, falFac.n)
    annotation(
      Line(
        points={{11, 40}, {24, 40}, {24, 12}, {38, 12}},
        color={255, 127, 0}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=11.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Utilities/Math/Examples/FallingFactorial.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
