...
model IntegratorWithReset
  ...
  Buildings.Utilities.Math.IntegratorWithReset intWitRes1(y_start=5, reset=Buildings.Types.Reset.Parameter, k=0.5, y_reset=2)
    "Integrator with reset"
    annotation(Placement(transformation(extent={{-10, 20}, {10, 40}})));
  Buildings.Utilities.Math.IntegratorWithReset intWitRes2(y_reset=10, y_start=-5, reset=Buildings.Types.Reset.Input, k=0.5)
  ...
  Buildings.Utilities.Math.IntegratorWithReset intNoReset(reset=Buildings.Types.Reset.Disabled, k=0.5, y_start=1)
    "Integrator without reset"
    annotation(Placement(transformation(extent={{-10, 60}, {10, 80}})));
  Modelica.Blocks.Sources.SampleTrigger sampleTrigger(period=0.2)
    "Sample trigger"
    annotation(Placement(transformation(extent={{-60, 0}, {-40, 20}})));
  Modelica.Blocks.Sources.Ramp ramp(height=-1, duration=1, offset=-2)
...
equation
  ...
  connect(ramp.y, intWitRes2.y_reset_in)
    annotation(Line(points={{-39,-20}, {-39,-20}, {-34,-20}, {-34,-28}, {-12,-28}}, color={0, 0, 127}));
  connect(cons.y, intDef.u)
    annotation(Line(points={{-39, 70}, {-26, 70}, {-26,-80}, {-14,-80}}, color={0, 0, 127}));
  annotation(experiment(Tolerance=1e-6, StopTime=1.0), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Utilities/Math/Examples/IntegratorWithReset.mos"
