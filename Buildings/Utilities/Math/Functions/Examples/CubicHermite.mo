...
model CubicHermite
  ...
  parameter Real[size(xd, 1)] dMonotone(
    each fixed=false)
    "Derivatives at the support points";
...
algorithm
  ...
  y := Buildings.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(
    ...
    y1d=d[i],
    y2d=d[i + 1]);
  yMonotone := Buildings.Utilities.Math.Functions.cubicHermiteLinearExtrapolation(
    x=x,
    x1=xd[i],
    x2=xd[i + 1],
    y1=yd[i],
  ...
  annotation(
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Utilities/Math/Functions/Examples/CubicHermite.mos"
        "Simulate and plot"),
    experiment(
      Tolerance=1e-6,
      StopTime=1.0),
