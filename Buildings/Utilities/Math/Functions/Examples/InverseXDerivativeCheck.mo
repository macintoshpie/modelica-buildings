...
equation
  ...
  y=Buildings.Utilities.Math.Functions.inverseXRegularized(
    x=x,
    delta=delta);
  der(y_comp)=Buildings.Utilities.Math.Functions.BaseClasses.der_inverseXRegularized(
    x=x,
    delta=delta,
    x_der=der(x));
  err=y-y_comp;
  ...
  annotation(
    experiment(
      ...
      Tolerance=1e-08),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Utilities/Math/Functions/Examples/InverseXDerivativeCheck.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
