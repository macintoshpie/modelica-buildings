...
algorithm
  E1 := if x < 1 then Buildings.Utilities.Math.Functions.polynomial(x, a1)-log(x) else Buildings.Utilities.Math.Functions.polynomial(x, a2)/(Buildings.Utilities.Math.Functions.polynomial(x, b2)*x*exp(x));
  annotation(
    Inline=true,
    Documentation(
      info="<html>
