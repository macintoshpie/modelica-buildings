...
block CoolingCoilValve
  ...
  parameter Real Ti(final unit="s")=k*interval/(alc_int_k*alc_k_unit_conv)
    ...
    annotation(Evaluate=true, Dialog(tab="Advanced", group="Controller"));
  parameter Real uMax(final min=0, final max=1, final unit="1")=1
    "Maximum controller signal"
    annotation(Evaluate=true, Dialog(tab="Advanced", group="Controller"));
  parameter Real uMin(final min=0, final max=1, final unit="1")=0
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput uFanFee
    "Supply fan feedback"
    annotation(Placement(transformation(extent={{-160,-80}, {-120,-40}}), iconTransformation(extent={{-120,-50}, {-100,-30}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TSup(final unit="K", final quantity="ThermodynamicTemperature")
    "Measured supply air temperature (SAT)"
    annotation(Placement(transformation(extent={{-160, 20}, {-120, 60}}), iconTransformation(extent={{-120, 70}, {-100, 90}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TSupSet(final unit="K", final quantity="ThermodynamicTemperature")
    "Supply air temperature setpoint"
    annotation(Placement(transformation(extent={{-160, 70}, {-120, 110}}), iconTransformation(extent={{-120, 30}, {-100, 50}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput TOut(final unit="K", final quantity="ThermodynamicTemperature")
    "Measured outdoor air temperature"
    annotation(Placement(transformation(extent={{-160,-40}, {-120, 0}}), iconTransformation(extent={{-120,-10}, {-100, 10}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput yCooVal(final min=0, final max=1, final unit="1")
  ...
  Buildings.Controls.OBC.CDL.Continuous.LimPID limPI(final reverseAction=reverseAction, final controllerType=Modelica.Blocks.Types.SimpleController.PI, final k=k, final Ti=Ti, final yMax=1, final yMin=0, final y_reset=0, final reset=Buildings.Controls.OBC.CDL.Types.Reset.Parameter)
    "Custom PI controller"
    annotation(Placement(transformation(extent={{-40, 80}, {-20, 100}})));
  Buildings.Controls.OBC.CDL.Continuous.Line higLim(final limitBelow=true, final limitAbove=true)
...
protected
  ...
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis TOutThr(final uLow=TOutCooCut-TOutDelta, final uHigh=TOutCooCut)
    "Determines whether the outdoor air temperature is below a treashold"
    annotation(Placement(transformation(extent={{-110,-30}, {-90,-10}})));
  Buildings.Controls.OBC.CDL.Continuous.Hysteresis uFanFeeThr(final uLow=FanFeeCut-FanFeeDelta, final uHigh=FanFeeCut)
...
equation
  ...
  connect(TSupMax.y, higLim.x2)
    annotation(Line(points={{61,-50}, {64,-50}, {64,-24}, {68,-24}, {68,-24}, {78,-24}}, color={0, 0, 127}));
  ...
  connect(cha.y, limPI.trigger)
    annotation(Line(points={{-19,-60}, {-16,-60}, {-16, 20}, {-38, 20}, {-38, 78}}, color={255, 0, 255}));
  connect(TSupMin.y, higLim.x1)
    annotation(Line(points={{21,-50}, {26,-50}, {26,-12}, {78,-12}}, color={0, 0, 127}));
  connect(yCooValMin.y, higLim.f1)
    annotation(Line(points={{21,-90}, {30,-90}, {30,-16}, {78,-16}}, color={0, 0, 127}));
  connect(uFanSta, andIntErr.u3)
    annotation(Line(points={{-140,-100}, {-82,-100}, {-82,-68}, {-72,-68}}, color={255, 0, 255}));
  connect(andIntErr.y, booToRea.u)
    annotation(Line(points={{-49,-60}, {-46,-60}, {-46,-20}, {-42,-20}}, color={255, 0, 255}));
  connect(limPI.y, pro.u1)
    annotation(Line(points={{-19, 90}, {38, 90}}, color={0, 0, 127}));
  connect(booToRea.y, pro.u2)
    annotation(Line(points={{-19,-20}, {0,-20}, {0, 78}, {38, 78}}, color={0, 0, 127}));
  connect(min.u1, pro.y)
    annotation(Line(points={{78, 36}, {72, 36}, {72, 84}, {61, 84}}, color={0, 0, 127}));
  annotation(defaultComponentName="cooVal", Icon(graphics={Rectangle(extent={{-100,-100}, {100, 100}}, lineColor={0, 0, 127}, fillColor={255, 255, 255}, fillPattern=FillPattern.Solid), Line(points={{20, 58}}, color={28, 108, 200}), Text(extent={{-108, 138}, {102, 110}}, lineColor={0, 0, 127}, textString="%name"), Text(extent={{-64,-132}, {60,-18}}, lineColor={0, 0, 127}, textString="Cooling coil valve")}), Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-120,-120}, {120, 120}}), graphics={Rectangle(extent={{-10, 120}, {120,-120}}, lineColor={217, 217, 217}, fillColor={217, 217, 217}, fillPattern=FillPattern.Solid), Text(extent={{80,-102}, {120,-110}}, lineColor={0, 0, 127}, textString="Limiter for
low TSup"), Rectangle(extent={{-120, 0}, {-12,-120}}, lineColor={217, 217, 217}, fillColor={217, 217, 217}, fillPattern=FillPattern.Solid), Text(extent={{-48,-106}, {-20,-112}}, lineColor={0, 0, 127}, textString="Enbale/Disable"), Rectangle(extent={{-120, 120}, {-12, 2}}, lineColor={217, 217, 217}, fillColor={217, 217, 217}, fillPattern=FillPattern.Solid), Text(extent={{-112, 12}, {-92, 6}}, lineColor={0, 0, 127}, textString="Controller")}), Documentation(info="<html>
<p>
