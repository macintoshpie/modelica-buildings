...
block CoolingCoilValve
  ...
  parameter Real alc_prop_k(
    final unit="1/F")=1/100
    "Recorded proportional controller gain"
    annotation(
      Evaluate=true,
      Dialog(
        group="Controller"));
  parameter Real alc_int_k(
    final unit="1/F")=0.5/100
  ...
  parameter Boolean reverseAction=true
    ...
    annotation(
      ...
      Dialog(
        ...
        group="Controller"));
  parameter Real k(
    final unit="1/K")=alc_prop_k*alc_k_unit_conv
    ...
    annotation(
      ...
      Dialog(
        ...
        group="Controller"));
  parameter Real Ti(
    final unit="s")=k*interval/(alc_int_k*alc_k_unit_conv)
...
equation
  connect(TOut, TOutThr.u)
    annotation(
      Line(
        points={{-140,-20}, {-112,-20}},
        color={0, 0, 127}));
  connect(TOutThr.y, andIntErr.u1)
    annotation(
      Line(
        points={{-89,-20}, {-86,-20}, {-86,-52}, {-72,-52}},
        color={255, 0, 255}));
  connect(TSup, higLim.u)
    annotation(
      Line(
        points={{-140, 40}, {20, 40}, {20,-20}, {78,-20}},
        color={0, 0, 127}));
  connect(yCooVal, min.y)
    annotation(
      Line(
        points={{130, 0}, {114, 0}, {114, 30}, {101, 30}},
        color={0, 0, 127}));
  connect(TSupMax.y, higLim.x2)
    annotation(
      Line(
        points={{61,-50}, {64,-50}, {64,-24}, {68,-24}, {68,-24}, {78,-24}},
        color={0, 0, 127}));
  connect(yCooValMax.y, higLim.f2)
    annotation(
      Line(
        points={{61,-90}, {70,-90}, {70,-28}, {78,-28}},
        color={0, 0, 127}));
  connect(higLim.y, min.u2)
    annotation(
      Line(
        points={{101,-20}, {106,-20}, {106, 0}, {70, 0}, {70, 24}, {78, 24}},
        color={0, 0, 127}));
  connect(TSupSet, limPI.u_s)
    annotation(
      Line(
        points={{-140, 90}, {-92, 90}, {-92, 90}, {-42, 90}},
        color={0, 0, 127}));
  connect(TSup, limPI.u_m)
    annotation(
      Line(
        points={{-140, 40}, {-30, 40}, {-30, 78}},
        color={0, 0, 127}));
  ...
  connect(cha.y, limPI.trigger)
    annotation(
      Line(
        points={{-19,-60}, {-16,-60}, {-16, 20}, {-38, 20}, {-38, 78}},
        color={255, 0, 255}));
  connect(TSupMin.y, higLim.x1)
    annotation(
      Line(
        points={{21,-50}, {26,-50}, {26,-12}, {78,-12}},
        color={0, 0, 127}));
  connect(yCooValMin.y, higLim.f1)
    annotation(
      Line(
        points={{21,-90}, {30,-90}, {30,-16}, {78,-16}},
        color={0, 0, 127}));
  connect(uFanSta, andIntErr.u3)
    annotation(
      Line(
        points={{-140,-100}, {-82,-100}, {-82,-68}, {-72,-68}},
        color={255, 0, 255}));
  connect(andIntErr.y, booToRea.u)
    annotation(
      Line(
        points={{-49,-60}, {-46,-60}, {-46,-20}, {-42,-20}},
        color={255, 0, 255}));
  ...
  connect(booToRea.y, pro.u2)
    annotation(
      Line(
        points={{-19,-20}, {0,-20}, {0, 78}, {38, 78}},
        color={0, 0, 127}));
  connect(min.u1, pro.y)
    annotation(
      Line(
        points={{78, 36}, {72, 36}, {72, 84}, {61, 84}},
        color={0, 0, 127}));
  annotation(
...
low TSup"), Rectangle(
        ...
        lineColor={0, 0, 127},
        textString="Enbale/Disable"), Rectangle(
        extent={{-120, 120}, {-12, 2}},
        lineColor={217, 217, 217},
        fillColor={217, 217, 217},
        fillPattern=FillPattern.Solid), Text(
