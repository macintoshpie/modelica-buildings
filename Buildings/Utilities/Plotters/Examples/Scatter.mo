...
model Scatter
  ...
  Buildings.Utilities.Plotters.Scatter sca1(
    ...
    xlabel="sine",
    introduction="This plot shows a scatter plot of sine vs cosine and of sine vs cosine^2.")
    "Scatter plot"
    annotation(
      Placement(
        transformation(
...
equation
  ...
  connect(sca1.x, sine.y)
    annotation(
      Line(
        points={{58, 32}, {52, 32}, {52,-20}, {-19,-20}},
        color={0, 0, 127}));
  connect(cosine.y, sca1.y[1])
    annotation(
      Line(
        points={{-19, 0}, {0, 0}, {0, 41}, {58, 41}},
        color={0, 0, 127}));
  connect(cosine.y, product.u1)
    annotation(
      Line(
        points={{-19, 0}, {0, 0}, {0, 26}, {18, 26}},
        color={0, 0, 127}));
  connect(cosine.y, product.u2)
    annotation(
      Line(
        points={{-19, 0}, {0, 0}, {0, 14}, {18, 14}},
        color={0, 0, 127}));
  connect(product.y, sca1.y[2])
    annotation(
      Line(
        points={{41, 20}, {50, 20}, {50, 39}, {58, 39}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=10.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Utilities/Plotters/Examples/Scatter.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
