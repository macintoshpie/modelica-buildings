...
equation
  connect(timeSignal.y, sca.x)
    annotation(
      Line(
        points={{-39, 10}, {-28, 10}, {-28, 22}, {-2, 22}},
        color={0, 0, 127}));
  connect(timeSignal.y, scaDel.x)
    annotation(
      Line(
        points={{-39, 10}, {-28, 10}, {-28,-18}, {-2,-18}},
        color={0, 0, 127}));
  connect(timeSignal.y, sca.y[1])
    annotation(
      Line(
        points={{-39, 10}, {-20, 10}, {-20, 30}, {-2, 30}},
        color={0, 0, 127}));
  connect(timeSignal.y, scaDel.y[1])
    annotation(
      Line(
        points={{-39, 10}, {-20, 10}, {-20,-10}, {-2,-10}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=10.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Utilities/Plotters/Validation/PlotterActivationAlwaysOn.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
