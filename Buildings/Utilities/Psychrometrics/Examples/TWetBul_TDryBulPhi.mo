  ...
  Modelica.Blocks.Sources.Ramp phi(
    ...
    annotation(
      Placement(
        transformation(
          extent={{-80, 20}, {-60, 40}})));
  Modelica.Blocks.Sources.Constant TDryBul(
    k=273.15 + 29.4)
...
equation
  ...
  connect(wetBulXi.TDryBul, TDryBul.y)
    annotation(
      Line(
        points={{-1,-22}, {-6,-22}, {-6,-10}, {-10,-10}, {-32,-10}, {-32, 70}, {-59, 70}},
        color={0, 0, 127}));
  connect(wetBulXi.p, p.y)
    annotation(
      Line(
        points={{-1,-38}, {-40,-38}, {-40,-10}, {-59,-10}},
        color={0, 0, 127}));
  connect(x_pTphi.p_in, p.y)
    annotation(
      Line(
        points={{-23.2,-26.4}, {-40,-26.4}, {-40,-10}, {-59,-10}},
        color={0, 0, 127}));
  connect(x_pTphi.T, TDryBul.y)
    annotation(
      Line(
        points={{-23.2,-30}, {-32,-30}, {-32, 70}, {-59, 70}},
        color={0, 0, 127}));
  connect(x_pTphi.phi, phi.y)
    annotation(
      Line(
        points={{-23.2,-33.6}, {-46,-33.6}, {-46, 30}, {-59, 30}},
        color={0, 0, 127}));
  ...
  connect(wetBulPhi.TWetBul, assertions.wetBulPhi_TWetBul)
    annotation(
      Line(
        points={{21, 50}, {48, 50}, {48, 10}, {58, 10}},
        color={0, 0, 127}));
  connect(wetBulXi.TWetBul, assertions.wetBulXi_TWetBul)
    annotation(
      Line(
        points={{21,-30}, {36,-30}, {48,-30}, {48, 4}, {58, 4}},
        color={0, 0, 127}));
  connect(assertions.phi, phi.y)
    annotation(
      Line(
        points={{58, 16}, {50, 16}, {40, 16}, {40, 30}, {-59, 30}},
        color={0, 0, 127}));
  annotation(
    experiment(
      Tolerance=1e-6,
      StopTime=1.0),
    __Dymola_Commands(
      file="modelica://Buildings/Resources/Scripts/Dymola/Utilities/Psychrometrics/Examples/TWetBul_TDryBulPhi.mos"
        "Simulate and plot"),
    Documentation(
      info="<html>
<p>
