...
model TotalAirDryAir
  ...
  Modelica.Blocks.Sources.Ramp XHum(duration=1, height=(0.01-0.1), offset=0.1)
...
equation
  connect(toTotalAir.XiDry, XHum.y)
    annotation(Line(points={{-41, 10}, {-59, 10}}, color={0, 0, 127}));
  ...
  connect(toDryAir.XiDry, assertEquality.u1)
    annotation(Line(points={{21, 10}, {28, 10}, {28,-4}, {38,-4}}, color={0, 0, 127}));
  connect(assertEquality.u2, XHum.y)
    annotation(Line(points={{38,-16}, {-50,-16}, {-50, 10}, {-59, 10}}, color={0, 0, 127}));
  annotation(experiment(Tolerance=1e-6, StopTime=1.0), __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Utilities/Psychrometrics/Examples/TotalAirDryAir.mos"
    "Simulate and plot"));
end TotalAirDryAir;
