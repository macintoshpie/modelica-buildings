...
model CalendarTime
  ...
  Modelica.Blocks.Interfaces.RealOutput unixTimeStamp(
    final unit="s")=unixTimeStampLocal-timZon if outputUnixTimeStamp
    "Unix time stamp"
    annotation(
      Placement(
        transformation(
  ...
  discrete Modelica.Blocks.Interfaces.IntegerOutput month
    ...
    annotation(
      Placement(
        ...
        iconTransformation(
          extent={{100, 2}, {120, 22}})));
  Modelica.Blocks.Interfaces.IntegerOutput day(
    fixed=false)
    "Day of the month"
    annotation(
      Placement(
        transformation(
          extent={{100, 28}, {120, 48}}),
        iconTransformation(
          extent={{100, 28}, {120, 48}})));
  Modelica.Blocks.Interfaces.IntegerOutput hour(
    fixed=false)
    "Hour of the day"
    annotation(
      Placement(
...
protected
  ...
  final parameter Modelica.SIunits.Time hourSampleStart(
    fixed=false)
    "Time when the sampling every hour starts";
  final parameter Modelica.SIunits.Time daySampleStart(
    fixed=false)
  ...
  Boolean firstHourSampling(
    fixed=true,
    start=true)
    "=true if the hour is sampled the first time";
  Boolean firstDaySampling(
    fixed=true,
    start=true)
...
that it changes the time based on which the solar position is computed and TMY3 data are read.
    ...
    Icon(
      graphics={Text(
        ...
        lineColor={28, 108, 200},
        horizontalAlignment=TextAlignment.Right,
        textString="Weekday"), Text(
        extent={{-102,-60}, {94,-72}},
        lineColor={28, 108, 200},
        horizontalAlignment=TextAlignment.Right,
        textString="Unix time stamp (local)"), Ellipse(
        extent={{-94, 94}, {16,-16}},
