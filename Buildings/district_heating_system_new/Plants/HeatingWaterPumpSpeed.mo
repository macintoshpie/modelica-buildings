within Buildings.district_heating_system_new.Plants;
model HeatingWaterPumpSpeed
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput masFloPum(
    final unit="kg/s") "Total mass flowrate of chilled/heating water pumps"
    annotation (Placement(transformation(extent={{-120,30},{-100,50}}),iconTransformation(extent={{-120,30},
            {-100,50}})));
  ...
  Controls.OBC.CDL.Interfaces.BooleanInput resPI[numPum] annotation (Placement(
        transformation(extent={{-120,70},{-100,90}}), iconTransformation(extent
          ={{-120,56},{-100,76}})));
equation
  connect(pumStaCon.masFloPum,masFloPum)
    annotation (Line(points={{-12,8},{-22,8},{-22,40},{-110,40}},color={0,0,127}));
  ...
  connect(resPI[1], bypValCon.trigger) annotation (Line(points={{-110,75},{-16,
          75},{-16,-86},{-8,-86},{-8,-62}}, color={255,0,255}));
  annotation (
    Icon(
      coordinateSystem(
